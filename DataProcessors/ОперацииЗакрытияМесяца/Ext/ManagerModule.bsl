...
// Процедура предназначена для вызова извне механики закрытия месяца - из механизма расчета любого этапа.
// Выполняет операции механизма закрытия месяца, вызываемые после расчета этапа, если расчет вызван не в рамках закрытия месяца.
// При закрытии месяца эти действия выполнит сам механизм закрытия месяца.
//
...
Функция ИнициализироватьТаблицуОписанияГруппЭтапов()
...
Функция ИнициализироватьТаблицуОписанияЭтапов()
...
Функция ИнициализироватьТаблицуСгруппированныхПояснений()
	
	ТаблицаПояснений = ИнициализироватьТаблицуПоясненийКЭтапам();
...
// Вызывается извне обработки.
//
Функция ИнициализироватьСтруктуруПараметрыРасчета(Период, МассивОрганизаций = Неопределено) Экспорт
...
Функция ИнициализироватьСтруктуруДанныхДляЗаполненияСостоянияЭтапов(Период,
			МассивОрганизаций = Неопределено, СтруктураДанных = Неопределено) Экспорт
	
	ПараметрыРасчета = ИнициализироватьСтруктуруПараметрыРасчета(Период, МассивОрганизаций);
...
Функция ИнициализироватьПараметрыЗапускаДляЗаполненияСостоянияЭтапов(СтруктураДанных, АдресХранилища = "",
	...
	ПараметрыЗапуска.Вставить("ОбновлятьСписокПроверок", 	 		 Истина);
...
Функция ИнициализироватьПараметрыОбработчикаЭтапа(СтруктураДанных, СтрокаТаблицыЭтапов,
			ДопСвойства = Неопределено, ИмяКлючевойОперации = Неопределено)
...
Процедура ЗаполнитьСправочникПроверокСостоянияСистемы(ПараметрыЗапуска, ОписаниеОбщегоЗамераПроизводительности)
	...
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(Т.КонтекстВыполнения) = &ТипКонтекстаВыполнения";
...
Процедура ВыполнитьПроверкиЭтапа(СтруктураДанных, ТекущаяСтрока, ПараметрыОбработчика, ВыполнятьПроверкиДоЭтапа, ВыполнятьПроверкиПослеЭтапа)
	...
	Для Каждого ТекущаяПроверка Из ТаблицаПроверок Цикл
		...
		Попытка
			...
			ЗафиксироватьЗамерВремениОпределенияСостоянияЭтапа(ПараметрыОбработчика, РезультатПроверки.ВыявленныеПроблемы.Количество());
...
Процедура ОпределитьСостояниеИЗаполнитьПоясненияКЭтапу(СтруктураДанных, ТекущаяСтрока, ПараметрыОбработчика)
	...
	Если ТекущаяСтрока.Состояние <> Перечисления.СостоянияОперацийЗакрытияМесяца.НеТребуется Тогда
		...
		Для Каждого СтрокаПояснения Из ТаблицаПроблем Цикл
			...
			Если СтрокаПояснения.Важность = Перечисления.ВариантыВажностиПроблемыСостоянияСистемы.ВажнаяИнформация Тогда
				ТекущаяСтрока.ЕстьВажнаяИнформация = Истина;
			ИначеЕсли СтрокаПояснения.Важность = Перечисления.ВариантыВажностиПроблемыСостоянияСистемы.ПолезныйСовет Тогда
	...
	Для Каждого ТекущаяСтрока Из СтрокиГрупп Цикл
		...
		Для Каждого ПодчиненнаяСтрока Из СтрокиПодчиненных Цикл
			...
			Если (НЕ ПодчиненнаяСтрока.Отключено И НЕ ПодчиненнаяСтрока.ВыполняетсяВручную)
			 ИЛИ (ПодчиненнаяСтрока.ВыполняетсяВручную И ЗакрытиеМесяцаСервер.ТребуетсяПересчетЭтапа(ПодчиненнаяСтрока)) Тогда
...
Функция ПроверитьДатыЗапретаИзменения(СтруктураДанных)
	...
	Для Каждого ТекущаяОрганизация Из СтруктураДанных.ПараметрыРасчета.МассивОрганизаций Цикл
	...
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(Т.Период, МЕСЯЦ)) КАК Период
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций КАК Т
	|ГДЕ
	...
	Запрос.УстановитьПараметр("МассивОрганизаций", СтруктураДанных.ПараметрыРасчета.МассивОрганизаций);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ПериодПолитики) Тогда
			ПериодПолитики = Выборка.Период;
		Иначе
			ПериодПолитики = Мин(ПериодПолитики, Выборка.Период);
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ПериодПолитики) Тогда
		...
		СтруктураДанных.ПоясненияКЗапретуИзменений = НСтр("ru='Отсутствуют учетные политики'");
	...
	ИначеЕсли ЗначениеЗаполнено(ДатаЗапрета) ИЛИ ПериодПолитики > ПериодПроверки Тогда
		...
		СтруктураДанных.ПоясненияКЗапретуИзменений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ПротоколРасчетаПартийИСебестоимости.ПредставлениеПериодаРасчета(Макс(ДатаЗапрета, ПериодПолитики)));
...
Функция ИнициализироватьПараметрыЗапускаДляРасчетаЭтапов(СтруктураДанных, РассчитатьТолькоОдинЭтап,	ПоследнийЭтапДляРасчета,
			АдресХранилища = "", ДопСвойства = Неопределено, ИдентификаторРасчета = Неопределено) Экспорт
	...
	Для НомерСтроки = НомерПервойСтроки По ТаблицаЭтапы.Количество() - 1 Цикл
		...
		Для Каждого ОписаниеОшибки Из ПараметрыОбработчика.ТаблицаПояснения Цикл
			...
			ЗакрытиеМесяцаСервер.ЗарегистрироватьПроблемуВыполненияРасчета(
				...
				ШаблонТекстаПроблемыОшибкаВыполненияКода(),
...
Функция ОписаниеОрганизацийДляРасчета(Период)
	
	УстановитьПривилегированныйРежим(Истина);
	
	МассивИнтеркампани = УниверсальныеМеханизмыПартийИСебестоимости.ВсеСвязиОрганизацийПоСхемеИнтеркампани(Период); // содержит все организации
