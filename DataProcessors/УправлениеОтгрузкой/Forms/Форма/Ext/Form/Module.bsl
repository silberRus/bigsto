...
Процедура ПриОткрытии(Отказ)
	...
	//+АТ ТерентьевЕМ
	ПодключитьОбработчикОжидания("КомандаОбновитьСписки", 20, Ложь);
	//-АТ ТерентьевЕМ
...
//+АТ ТерентьевЕМ
&НаКлиенте
Процедура КомандаОбновитьСписки()
	УстановитьОтборыВОсновныхСписках();
	УстановитьОтборыВЗависимыхСписках(ТекущиеРаспоряжения());
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РаспоряженияНаОтгрузку.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("РаспоряженияНаОтгрузку.Самовывоз");
...
Процедура ЗаполнитьРаспоряженияНаОтгрузку(ПроверятьИзменениеПараметров = Ложь)
	...
	ТаблицаРаспоряженияНаОтгрузку = Запрос.Выполнить().Выгрузить();
	ТаблицаРаспоряженияНаОтгрузку.Колонки.Добавить("ТипДокументаСтрока", Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(50)));
	Для Каждого Строка Из ТаблицаРаспоряженияНаОтгрузку Цикл
		Строка.ТипДокументаСтрока = Строка(Строка.ТипДокумента);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РаспоряженияНаОтгрузку.Дата КАК Дата,
	|	РаспоряженияНаОтгрузку.ИндексКартинки КАК ИндексКартинки,
	|	РаспоряженияНаОтгрузку.Номер КАК Номер,
	|	РаспоряженияНаОтгрузку.Получатель КАК Получатель,
	|	РаспоряженияНаОтгрузку.Распоряжение КАК Распоряжение,
	|	РаспоряженияНаОтгрузку.ТипДокументаСтрока КАК ТипДокумента,
	|	РаспоряженияНаОтгрузку.Состояние КАК Состояние
	|ПОМЕСТИТЬ ВТРаспоряженияНаОтгрузку
	|ИЗ
	|	&РаспоряженияНаОтгрузку КАК РаспоряженияНаОтгрузку
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТРаспоряженияНаОтгрузку.Дата КАК Дата,
	|	ВТРаспоряженияНаОтгрузку.ИндексКартинки КАК ИндексКартинки,
	|	ВТРаспоряженияНаОтгрузку.Номер КАК Номер,
	|	ВТРаспоряженияНаОтгрузку.Получатель КАК Получатель,
	|	ВТРаспоряженияНаОтгрузку.Распоряжение КАК Распоряжение,
	|	ВТРаспоряженияНаОтгрузку.ТипДокумента КАК ТипДокумента,
	|	ВТРаспоряженияНаОтгрузку.Состояние КАК Состояние,
	//|	ЕСТЬNULL(РеализацияТоваровУслуг.Комментарий, """") КАК Комментарий,
	// silber {
	|	ВЫБОР 
	|		КОГДА ПОДСТРОКА(ЕСТЬNULL(РеализацияТоваровУслуг.Комментарий, """"), 1, 1) = """"
	|		ТОГДА ЗаказКлиентаДокумента.Комментарий 
	|		ИНАЧЕ РеализацияТоваровУслуг.Комментарий 
	|	КОНЕЦ Комментарий,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Маршр.СпособДоставки, ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)) = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
	|		ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ Самовывоз
	// } silber
	//|	ВЫБОР
	//|		КОГДА ЕСТЬNULL(ЗаказКлиентаДокумента.СпособДоставки, ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента)) = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
	//|			ТОГДА ИСТИНА
	//|		ИНАЧЕ ЛОЖЬ
	//|	КОНЕЦ КАК Самовывоз
	|ИЗ
	|	ВТРаспоряженияНаОтгрузку КАК ВТРаспоряженияНаОтгрузку
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК ЗаказКлиентаДокумента
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	|			ПО ЗаказКлиентаДокумента.Ссылка = РеализацияТоваровУслуг.ЗаказКлиента
	|		ПО ВТРаспоряженияНаОтгрузку.Распоряжение = ЗаказКлиентаДокумента.Ссылка
	// silber {
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрСведений.АТ_МаршрутДоставки Маршр
	|ПО
	|	Маршр.Объект 	= ЗаказКлиентаДокумента.Ссылка И
	|	Отправитель 	= &Склад";
	Запрос.УстановитьПараметр("Склад", Склад);
	// } silber
	Запрос.УстановитьПараметр("РаспоряженияНаОтгрузку", ТаблицаРаспоряженияНаОтгрузку);
...
Процедура СоздатьОрдераСервер()
	...
	Если ПоВсемРаспоряжениям = 0 Тогда
		...
		Запрос.УстановитьПараметр("Склад", Склад);
		Запрос.УстановитьПараметр("ДатаОтгрузки", КонецДня(ДатаОтгрузки) + 1);
...
&НаКлиенте
Процедура ПечатьОрдера(Команда)
	
	Если Элементы.ОрдераВРаботе.ВыделенныеСтроки.Количество() <> 1 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ДокументСсылка Из Элементы.ОрдераВРаботе.ВыделенныеСтроки Цикл
		
		ВПФ = НайтиВнешнююПечатнуюФорму("ПФ_РасходныйОрдерДляКладовщика");
		...
		ПараметрыОткрытия.ПараметрыИсточника.ИдентификаторКоманды = "ПФ_РасходныйОрдерДляКладовщика";
...
&НаСервереБезКонтекста
Функция НайтиВнешнююПечатнуюФорму(НаименованиеВПФ)
...
Процедура ПечатьКомплектаДокументов(Команда)
	...
	Для Каждого ДокументСсылка Из Элементы.ОрдераВРаботе.ВыделенныеСтроки Цикл
		...
		Основание = НайтиРаспоряжениеПоОрдеру(ДокументСсылка);
...
&НаСервереБезКонтекста
Функция НайтиРаспоряжениеПоОрдеру(Ссылка)
	...
	|		ПО РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Номенклатура = РеализацияТоваровУслугТовары.Номенклатура
	|			И РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Количество = РеализацияТоваровУслугТовары.Количество
	|			И РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Распоряжение = РеализацияТоваровУслугТовары.Ссылка.ЗаказКлиента
	...
	|		ПО РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Номенклатура = ПеремещениеТоваровТовары.Номенклатура
	|			И РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Количество = ПеремещениеТоваровТовары.Количество
	|			И РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Распоряжение = ПеремещениеТоваровТовары.Ссылка.ЗаказНаПеремещение
