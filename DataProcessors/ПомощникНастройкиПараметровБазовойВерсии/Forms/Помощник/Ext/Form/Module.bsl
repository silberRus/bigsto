...
Процедура ЗаписатьПользователяСервер()
	...
	Иначе
		...
		Если ПользовательИБ = Неопределено Тогда
			...
			РольАдминистратораСистемы = Пользователи.РольАдминистратораСистемы();
...
Процедура ЗаписатьДанныеОрганизацииСервер()
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ТекОптовыйСклад) Тогда
			...
			ОптовыйСклад.КонтролироватьОбеспечение         = Истина;
	...
	ТекущаяУчетнаяПолитика = Справочники.УчетныеПолитикиОрганизаций.Выбрать();
	
	Если Не ТекущаяУчетнаяПолитика.Следующий() Тогда
		УчетнаяПолитикаОбъект = Справочники.УчетныеПолитикиОрганизаций.СоздатьЭлемент();
	Иначе
		УчетнаяПолитикаОбъект = ТекущаяУчетнаяПолитика.ПолучитьОбъект();
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(УчетнаяПолитикаОбъект.Наименование) Тогда
		УчетнаяПолитикаОбъект.Наименование = НСтр("ru='По умолчанию'") + " " + НачалоГода(ТекущаяДатаСеанса());
	КонецЕсли;
	
	УчетнаяПолитикаОбъект.МетодОценкиСтоимостиТоваров = Перечисления.МетодыОценкиСтоимостиТоваров.СредняяЗаМесяц;
	УчетнаяПолитикаОбъект.СистемаНалогообложения = ДанныеОрганизации.Организация.СистемаНалогообложения;
	УчетнаяПолитикаОбъект.ПрименяетсяЕНВД = ДанныеОрганизации.Организация.РозничнаяТорговляОблагаетсяЕНВД;
	...
	|	ИСТИНА
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций КАК Регистр");
	
	Если Запрос.Выполнить().Пустой() Тогда
		РегистрыСведений.УчетнаяПолитикаОрганизаций.СоздатьЗаписьРегистра(Организация.Ссылка,
		НачалоГода(ТекущаяДатаСеанса()),
		УчетнаяПолитикаОбъект.Ссылка);
	КонецЕсли;
...
Функция ПрочитатьДанныеОрганизации()
	...
	Если ТекущаяОрганизация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель Тогда
		ИндивидуальныйПредприниматель = 1;
		Элементы.СтраницыЗаголовокНаименования.ТекущаяСтраница = Элементы.СтраницаЗаголовокИП;
		ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель;
	Иначе
	...
	Если ЗначениеЗаполнено(ВидЦены) Тогда
		...
		ЦенаВключаетНДС = ВидЦены.ЦенаВключаетНДС;
		СпособЗаданияЦены = ВидЦены.СпособЗаданияЦены;
		
		Если СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаЦенуПоступления Тогда
			Наценка= ВидЦены.Наценка;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОчиститьДокументыВводаОстатокв()
	...
	Запрос.Текст = 
		...
		|				ТОГДА ИСТИНА
		|			КОГДА &ИзменитьТекущийОстатокБезналичныхДенежныхСредств
		|				ТОГДА ВводОстатков.ТипОперации = &НаБанковскихСчетах
		|			КОГДА &ИзменитьТекущийОстатокНаличныхДенежныхСредств
		|				ТОГДА ВводОстатков.ТипОперации = &ВКассе
		|		КОНЕЦ";
	...
	Запрос.УстановитьПараметр("НаБанковскихСчетах",Перечисления.ТипыОперацийВводаОстатков.ОстаткиНаБанковскихСчетах);
	Запрос.УстановитьПараметр("ВКассе", Перечисления.ТипыОперацийВводаОстатков.ОстаткиВКассах);
