...
Процедура ДалееКШагуСформированныеДокументы(Команда)
	...
	ОчиститьСообщения();
	ДалееКШагуСформированныеДокументыНаСервере();
...
Процедура СписокДокументыПриИзмененииВариантаОформления()
	...
	КонецЕсли;
	СписокДокументы.ТекстЗапроса = ТекстЗапроса;
...
&НаСервере
Процедура ДалееКШагуСформированныеДокументыНаСервере()
	...
	Для Каждого ЭлементФормы Из Элементы.ГруппаУстановитьСтатус.ПодчиненныеЭлементы Цикл
		...
		ЗаголовокКоманды = Команды[ЭлементФормы.ИмяКоманды].Заголовок;
...
Процедура УстановитьСтатус(Команда)
	...
	ИначеЕсли Команда = Команды.УстановитьСтатусЗаказовНаПеремещениеКОбеспечению.Имя Тогда
		
		ТипДокумента = Тип("ДокументСсылка.ЗаказНаПеремещение");
		Статус = ПредопределенноеЗначение("Перечисление.СтатусыВнутреннихЗаказов.КОбеспечению");
...
Процедура УстановитьСтатусНаСервере(МассивСсылок, Статус)
	...
	ОбеспечениеСервер.УстановитьСтатус(МассивСсылок, Статус);
...
Процедура НазадКШагуПеремещаемыеТоварыНаСервере()
	...
	СписокОшибок = ОбщегоНазначенияУТ.УдалитьДокументы(МассивДокументов);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.ДатаПотребности");
