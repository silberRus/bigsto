...
Процедура ПриОткрытии(Отказ)
	...
	АдресФайлаСоСпискомИБ = ПоместитьВХранилищеФайлСоСпискомБаз(УникальныйИдентификатор);
...
Процедура ИмяФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ПодключитьРасширениеРаботыСФайлами() Тогда
		...
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(,НСтр("ru = 'Для выбора каталога необходимо установить расширение для работы с файлами в Веб-клиенте.'"),10);
	КонецЕсли;
...
Процедура ОбработкаВыбораИБИзСпискаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗапуститьПриложение(СтрокаКоманды, ПутьКПлатформе, Истина, КодВозврата);
	...
	НачатьУдалениеФайлов(ОповещениеУдаление, ПутьКОбработкеСПравилами); 
...
Функция ПолучитьСсылкуНаСкачивание(ДанныеПользователя)
	...
	ИмяКонфигурации = ИнтернетПоддержкаПользователейКлиентСервер.ИмяКонфигурации();
