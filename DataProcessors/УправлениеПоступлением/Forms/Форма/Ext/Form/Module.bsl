...
Процедура ЗаполнитьРаспоряженияНаПоступление()
	...
	Если Метаданные.Документы.Найти("ДвижениеПродукцииИМатериалов") <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"&УсловиеИспользованияХозяйственнойОперацииРеестра",
			"ТоварыКПоступлениюОстаткиНаДату.ДокументПоступления ССЫЛКА Документ.ДвижениеПродукцииИМатериалов");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"&УсловиеИспользованияХозяйственнойОперацииРеестра", "ЛОЖЬ");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущееРаспоряжение) Тогда
		...
		КонецЕсли;
	ИначеЕсли РаспоряженияНаПоступление.Количество() > 0 Тогда
		Элементы.РаспоряженияНаПоступление.ТекущаяСтрока = РаспоряженияНаПоступление[0].ПолучитьИдентификатор();
	КонецЕсли;
