...
Процедура ДеревоЦенПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если ТекущиеДанные.Номенклатура <> СтараяНоменклатура Или Не ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) Тогда
		...
		// ( Storverk Тюняков Исправление типовой ошибки
		Если ТипЗнч(ДеревоЦен) = Тип("ДанныеФормыДерево") Тогда
		Иначе
			ЭлементыНоменклатура = ДеревоЦен;
		КонецЕсли;  	
		// ) Storverk Тюняков  
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	Макет = Документы.РегистрацияЦенНоменклатурыПоставщика.ПолучитьМакет("СТ_ЗагрузкаЦен");
	// */ Storverk Тюняков 7227 Загрузка прайс-листов поставщиков		
...
Процедура ЗавершитьРаботуПомощника()
	...
	Для Каждого СтрокаТЧ Из ДеревоЦен Цикл
		...
		Для Каждого ВидЦены Из УстановкаЦенКлиентСервер.ВыбранныеСтрокиТаблицыВидовЦен(ЭтаФорма) Цикл
			...
			// /* Storverk Тюняков 7227 Загрузка прайс-листов поставщиков
			НоваяСтрока.СТ_Количество = СтрокаТЧ["СТ_Количество" + ВидЦены.ИмяКолонки];
			// */ Storverk Тюняков 7227 Загрузка прайс-листов поставщиков
...
Процедура ДеревоЦенНоменклатураПриИзмененииСервер(ИдентификаторСтроки)
	...
	Для Каждого СтрокаВидЦены Из УстановкаЦенКлиентСервер.ВыбранныеСтрокиТаблицыВидовЦен(ЭтаФорма) Цикл
		...
		КонецЕсли;
		// /* Storverk Тюняков 7227 Загрузка прайс-листов поставщиков
		НоваяСтрока.СТ_Количество = ТекущаяСтрока["СТ_Количество" + ИмяКолонки];
		// */ Storverk Тюняков 7227 Загрузка прайс-листов поставщиков
...
Процедура СопоставитьДанныеТабличногоДокументаСНоменклатуройПоставщика()
	...
	Пока ЗаполненаНоменклатураАртикул Цикл
		...
		Для Каждого СтрокаТЧ Из УстановкаЦенКлиентСервер.ВыбранныеСтрокиТаблицыВидовЦен(ЭтаФорма) Цикл
			...
			// /* Storverk Тюняков 7227 Загрузка прайс-листов поставщиков
			СТ_Количество = ТабличныйДокумент.Область("R" + СтроковыйНомер + "C" + "3").Текст;
			НоваяСтрока["СТ_Количество"+СтрокаТЧ.ИмяКолонки] = СТ_Количество;
			// */ Storverk Тюняков 7227 Загрузка прайс-листов поставщиков
		КонецЦикла;
	...
	КонецЦикла;
	// - Не актуально после перехода с Пр-ль на Артикул
	//// /* Storverk Тюняков 7227 Загрузка прайс-листов поставщиков
	//// Заполняем номенклатуру по Пр_ль для Автотехника.
	//Для каждого СтрокаДереваЦен Из ДеревоЦен Цикл
	//	Если НЕ ЗначениеЗаполнено(СтрокаДереваЦен.Номенклатура)
	//		И НЕ ЗначениеЗаполнено(СтрокаДереваЦен.НоменклатураПоставщика) Тогда
	//		НайденнаяНоменклатура = СТ_ОбщийВызовСервера.ПолучитьНоменклатуруПоПр_ль(СтрокаДереваЦен.Артикул);
	//		Если ЗначениеЗаполнено(НайденнаяНоменклатура) Тогда
	//			СтрокаДереваЦен.Номенклатура = НайденнаяНоменклатура;			
	//		КонецЕсли; 
	//	КонецЕсли; 
	//КонецЦикла; 
	//// */ Storverk Тюняков 7227 Загрузка прайс-листов поставщиков
