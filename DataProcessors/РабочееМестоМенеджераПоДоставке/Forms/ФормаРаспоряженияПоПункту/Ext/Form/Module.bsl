...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ДатаВремяРейсаПланС) Тогда
		Если ЗначениеЗаполнено(РейсПункт) Тогда
			РейсПункт.Добавить(", ");
		КонецЕсли;
		РейсПункт.Добавить(Формат(Параметры.ДатаВремяРейсаПланС, "ДФ='dd.MM.yyyy HH:mm'"));
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(Параметры.ВремяС) Тогда
			...
			Если НачалоДня(Параметры.ВремяС) = НачалоДня(Параметры.ДатаВремяРейсаПланС) Тогда
				Если НачалоДня(Параметры.ВремяС) = НачалоДня(Параметры.ВремяПо) Тогда
					Текст = СтрЗаменить(ШаблонВремени, "%ВремяС%", Формат(Параметры.ВремяС, "ДФ='HH:mm'"));
					Текст = СтрЗаменить(Текст, "%ВремяПо%", Формат(Параметры.ВремяПо, "ДФ='HH:mm'"));
				Иначе
					Текст = СтрЗаменить(ШаблонВремени, "%ВремяС%", Формат(Параметры.ВремяС, "ДФ='dd.MM.yyyy HH:mm'"));
					Текст = СтрЗаменить(Текст, "%ВремяПо%", Формат(Параметры.ВремяПо, "ДФ='dd.MM.yyyy HH:mm'"));
				КонецЕсли;
			Иначе
				Если НачалоДня(Параметры.ВремяС) = НачалоДня(Параметры.ВремяПо) Тогда
					Текст = СтрЗаменить(ШаблонВремени, "%ВремяС%", Формат(Параметры.ВремяС, "ДФ='dd.MM.yyyy HH:mm'"));
...
#Область РасчетТоваров
	...
	Ссылки = Распоряжения.Выгрузить(,"Распоряжение").ВыгрузитьКолонку("Распоряжение");
...
Функция СохранитьРезультаты()
	...
	Для Каждого Стр Из ТаблицаТовары Цикл
		...
		УдалитьСтроку = Ложь;
		Если ТекущееРаспоряжение <> Стр.Распоряжение
			Или ТекущийСклад <> Стр.Склад Тогда
			ВсеТовары = Ложь;
