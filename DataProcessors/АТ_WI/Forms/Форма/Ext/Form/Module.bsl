...
Функция ПолучитьСписокUnitФункций() // ДОБАВЛЯТЬ НАЗВАНИЯ ФУНКЦИЙ ТЕСТОВ СЮДА
	...
	Список.Добавить("Корз_Проверить", 			"Проверить корзину");
	...
	Список.Добавить("Тов_search", 										"Получить 3 товара по артикулу");
	Список.Добавить("Тов_guid", 										"Получить товар по guid и аналоги");
	Возврат Список;
...
Функция Корз_Проверить()
	...
	Если Не Ошибка Тогда
		...
		Ответ = АТ_WI.Корзина_ПроверитьPOST(Новый Структура("Тело", Новый Структура("access_token", access_token)), Истина);
...
Функция Тов_search()
	...
	Ошибка = Ложь;
	access_token 	= access_token_ТестПользователя(Ошибка, Usr);
	Ответ = АТ_WI.ПоискPOST(Новый Структура("Тело", Новый Структура("sku, count, access_token", "1356", "3", access_token)), Истина);
	
	ПроверитьОжидание(ТипЗнч(Ответ.Тело) = Тип("ТаблицаЗначений") И 
					Ответ.Тело.Количество() = 3, Истина, Ошибка, "Ошибка при поиске товаров");
...
Функция Тов_guid()
	...
	Ошибка = Ложь;
	access_token 	= access_token_ТестПользователя(Ошибка, Usr);
	Ответ = АТ_WI.ПоискГУИДPOST(Новый Структура("Тело", Новый Структура("guid, address, access_token", "126f8d88-96be-11e6-810f-00155dfa1a0a", "444326d3-ad7e-11e7-8127-001dd8b75f1d", access_token)), Истина);
	
	ПроверитьОжидание(ТипЗнч(Ответ.Тело) = Тип("Массив") И 
					Ответ.Тело.Количество() > 0, Истина, Ошибка, "Ошибка получения товара и аналогов");
