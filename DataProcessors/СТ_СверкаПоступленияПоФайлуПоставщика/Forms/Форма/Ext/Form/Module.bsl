...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ТоварыИсходная) тогда
		ТоварыИсходная.Загрузить(Параметры.ТоварыИсходная.Выгрузить(, "ИсходныйНомерСтроки,ЗаказПоставщику,Номенклатура,,Количество,Цена"));				
	КонецЕсли;	
	
	ИнициализироватьТабличныйДокумент();
	
	ЦветТекстаУспех = ЦветаСтиля.ЦветТекстаУспех;
...
Функция ПроверитьКорректностьЗаполнения()
	...
	Для Каждого СтрокаТовары Из Объект.ТоварыСверка Цикл
		...
		Если СтрокаТовары.Количество_Файл <> СтрокаТовары.КоличествоРасшифровка Тогда
			
			ТекстСообщения = "Количества в файле ("+СтрокаТовары.Количество_Файл+") и в расшифровке("+СтрокаТовары.КоличествоРасшифровка+")";
			
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(ОшибкиПользователю,
				"Объект.ТоварыСверка[%1].Количество_Файл",
				ТекстСообщения,
				"Объект.ТоварыСверка.Количество_Файл", 
				СтрокаТовары.НомерСтроки,
				"В строке %1 не совпадают "+ТекстСообщения,
				СтрокаТовары.НомерСтроки-1);
...
Функция ПоместитьТоварыВХранилище()
	...
	тзРезультат = Объект.ТоварыСверка.Выгрузить();
	тзРезультат.Очистить();
	тзРезультат.Колонки.Добавить("ЗаказПоставщику", Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));
	
	Для Каждого СтрокаТовары Из Объект.ТоварыСверка Цикл		
		Для Каждого СтрокаРасшифровка Из СтрокаТовары.Расшифровка Цикл
			СтрокаРезультат = тзРезультат.Добавить();	
			ЗаполнитьЗначенияСвойств(СтрокаРезультат, СтрокаТовары);
			ЗаполнитьЗначенияСвойств(СтрокаРезультат, СтрокаРасшифровка);		
			Если СтрокаРезультат.Количество_Файл = 0 Тогда
				СтрокаРезультат.Действие = "Удалить строку";
			КонецЕсли; 
		КонецЦикла; 		
	КонецЦикла; 	
	
	Возврат ПоместитьВоВременноеХранилище(тзРезультат);
...
Процедура ЗаполнитьТоварыСверка()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТоварыИсходная.ИсходныйНомерСтроки,
		|	ТоварыИсходная.Номенклатура,
		|	ТоварыИсходная.ЗаказПоставщику,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втТоварыИсходная.ИсходныйНомерСтроки КАК НомерСтрокиИсходный,
		|	втТоварыИсходная.Номенклатура КАК Номенклатура,
		|	втТоварыИсходная.ЗаказПоставщику,
		|	втТоварыИсходная.Количество КАК Количество,
		|	втТоварыИсходная.Цена КАК Цена,
		|	втТоварыИсходная.Номенклатура.Артикул КАК Идентификатор,
		|	1 КАК ТипСтроки,
		|	ЕСТЬNULL(ЗаказПоставщикуТовары.Цена, 0) КАК ЦенаЗаказ
		|ИЗ
		|	втТоварыИсходная КАК втТоварыИсходная
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПоставщику.Товары КАК ЗаказПоставщикуТовары
		|		ПО втТоварыИсходная.ЗаказПоставщику = ЗаказПоставщикуТовары.Ссылка
		|			И втТоварыИсходная.Номенклатура = ЗаказПоставщикуТовары.Номенклатура
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтрокиИсходный
		|ИТОГИ
		|	МИНИМУМ(НомерСтрокиИсходный),
		|	СУММА(Количество)
		|ПО
		|	Номенклатура";
	...
	ВыборкаТоварыИсходная = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	КоличествоСтрокДокумент = 0;	
	Пока ВыборкаТоварыИсходная.Следующий() Цикл
		
		ВыборкаПоНоменклатуре = ВыборкаТоварыИсходная.Выбрать();
		...
		Если ВыборкаПоНоменклатуре.Количество() > 1 тогда			
			НоваяСтрока.НомерСтрокиИсходный = 0;
			НоваяСтрока.Количество = ВыборкаТоварыИсходная.Количество;
		КонецЕсли;	
		
		ВыборкаПоНоменклатуре.Сбросить();
		
		Пока ВыборкаПоНоменклатуре.Следующий() Цикл			
			НоваяСтрокаРасшифровка = НоваяСтрока.Расшифровка.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаРасшифровка, ВыборкаПоНоменклатуре);
			НоваяСтрокаРасшифровка.Цена_Заказ = ВыборкаПоНоменклатуре.ЦенаЗаказ;
		КонецЦикла;
	...
	ОтборПустогоЗаказа = Новый Структура("ЗаказПоставщику", Документы.ЗаказПоставщику.ПустаяСсылка());
	Для Каждого СтрокаСверка Из Объект.ТоварыСверка Цикл
		Если СтрокаСверка.ТипСтроки = 1 Тогда // Строки нет в файле
			Для каждого СтрокаРасшифровка Из СтрокаСверка.Расшифровка Цикл
				СтрокаРасшифровка.Количество_Файл = 0;
			КонецЦикла;
		ИначеЕсли СтрокаСверка.ТипСтроки = 2 Тогда // Строки нет в документе
			
			НоваяСтрокаРасшифровка = СтрокаСверка.Расшифровка.Добавить();				
			
			ЗаполнитьЗначенияСвойств(НоваяСтрокаРасшифровка, СтрокаСверка);
			НоваяСтрокаРасшифровка.ЗаказПоставщику = Документы.ЗаказПоставщику.ПустаяСсылка();
			НоваяСтрокаРасшифровка.НомерСтрокиИсходный = 0;
			НоваяСтрокаРасшифровка.Количество = 0;
			НоваяСтрокаРасшифровка.Цена = 0;
			НоваяСтрокаРасшифровка.Количество_Файл = СтрокаСверка.Количество_Файл;
			
		ИначеЕсли СтрокаСверка.ТипСтроки = 0 Тогда // СтрокаИзФайлаОпределена
			Если СтрокаСверка.Количество_Файл > СтрокаСверка.Количество Тогда				
				
				Для каждого СтрокаРасшифровка Из СтрокаСверка.Расшифровка Цикл
					СтрокаРасшифровка.Количество_Файл = СтрокаРасшифровка.Количество;	
				КонецЦикла; 
				
				СтрокиПустогоЗаказа = СтрокаСверка.Расшифровка.НайтиСтроки(ОтборПустогоЗаказа);				
				Если ЗначениеЗаполнено(СтрокиПустогоЗаказа) Тогда
					СтрокаИзлишка = СтрокиПустогоЗаказа[0];	
				Иначе	
					// Добавляем строку с пустым заказом для излишек
					СтрокаИзлишка = СтрокаСверка.Расшифровка.Добавить();								
					ЗаполнитьЗначенияСвойств(СтрокаИзлишка, СтрокаСверка);
					СтрокаИзлишка.ЗаказПоставщику = Документы.ЗаказПоставщику.ПустаяСсылка();
					СтрокаИзлишка.НомерСтрокиИсходный = 0;
					СтрокаИзлишка.Количество = 0;
					СтрокаИзлишка.Цена = 0;					
				КонецЕсли;
				СтрокаИзлишка.Количество_Файл = СтрокаИзлишка.Количество + СтрокаСверка.Количество_Файл - СтрокаСверка.Количество;
			ИначеЕсли СтрокаСверка.Количество_Файл < СтрокаСверка.Количество Тогда	
				//Распределяем количество файл по содержимому ТЧ расшифровка
				НаРаспределение = СтрокаСверка.Количество_Файл; 
				Для каждого СтрокаРасшифровка Из СтрокаСверка.Расшифровка Цикл
					СтрокаРасшифровка.Количество_Файл = Мин(СтрокаРасшифровка.Количество, НаРаспределение);
					НаРаспределение = НаРаспределение - СтрокаРасшифровка.Количество_Файл;
				КонецЦикла;			 						
			КонецЕсли;
		КонецЕсли;
		
		КоличествоРасшифровка = 0;
		Для каждого СтрокаРасшифровка Из СтрокаСверка.Расшифровка Цикл
			Если СтрокаСверка.ТипСтроки = 1 Тогда // Строки нет в файле
				СтрокаРасшифровка.Цена_Файл = 0;					
			Иначе
				СтрокаРасшифровка.Цена_Файл = СтрокаСверка.Цена_Файл;					
			КонецЕсли;	
			КоличествоРасшифровка = КоличествоРасшифровка + СтрокаРасшифровка.Количество_Файл;
		КонецЦикла;
		СтрокаСверка.КоличествоРасшифровка = КоличествоРасшифровка;
	КонецЦикла;		
...
&НаСервере
Функция ОбязательнаяКолонкаЗаполнена(НомерСтрокиИсходный)

	ЗначениеОбязательнойКолонки = ЗначениеЯчейкиТабДока(НомерСтрокиИсходный, "Идентификатор");
...
Процедура ТоварыПриАктивизацииСтроки(Элемент)
	...
	ОбновитьПодвалРасшифровки(ТекДанные);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьТекстПодвалаТекущейСтроки(ТекДанные)

	Возврат Строка(ТекДанные.КоличествоРасшифровка);	 
	
КонецФункции // ПолучитьТекстПодвалаТекущейСтроки()

&НаКлиенте
Процедура ОбновитьПодвалРасшифровки(ТекДанные)
	
	Элементы.ТоварыРаспределениеРаспределено.ТекстПодвала = ПолучитьТекстПодвалаТекущейСтроки(ТекДанные);
	Элементы.ТоварыРаспределениеЦена_Файл.ТекстПодвала = Формат(ТекДанные.Количество_Файл - ТекДанные.КоличествоРасшифровка, "ЧН=ОК");
	Элементы.ТоварыРаспределениеКоличество.ТекстПодвала = ТекДанные.Количество;	
	Если ТекДанные.Количество_Файл = ТекДанные.КоличествоРасшифровка Тогда
		Элементы.ТоварыРаспределениеЦена_Файл.ЦветТекстаПодвала = ЦветТекстаУспех;
	Иначе	
		Элементы.ТоварыРаспределениеЦена_Файл.ЦветТекстаПодвала = ЦветТекстаПроблема;
	КонецЕсли; 
	
КонецПроцедуры	

&НаКлиенте
Процедура ТоварыРаспределениеРаспределеноПриИзменении(Элемент)
	
	ТекДанные = Элементы.Товары.ТекущиеДанные;
	ОбновитьКоличествоРасшифровка(ТекДанные);
	ОбновитьПодвалРасшифровки(ТекДанные);
	Элементы.ТоварыРаспределениеРаспределено.ТекстПодвала = ПолучитьТекстПодвалаТекущейСтроки(ТекДанные);
	Элементы.ТоварыРаспределениеЦена_Файл.ТекстПодвала = Формат(ТекДанные.Количество_Файл - ТекДанные.КоличествоРасшифровка, "ЧН=ОК");
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьКоличествоРасшифровка(СтрокаСверка)
...
КонецПроцедуры // ОбновитьКоличествоРасшифровка()
