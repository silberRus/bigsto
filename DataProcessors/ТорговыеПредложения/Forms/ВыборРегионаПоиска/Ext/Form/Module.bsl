...
Процедура КнопкаОК(Команда)
	...
	Если РегионРежимОтбора = 2 И ПустаяСтрока(РегионПредставление) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран регион'"),, "РегионПредставление");
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуКонтактнойИнформации()
	...
	ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Введите регион абонента'"));
...
Процедура ОткрытьФормуКонтактнойИнформацииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		РегионЗначенияПолей = Результат.КонтактнаяИнформация;
	КонецЕсли;
