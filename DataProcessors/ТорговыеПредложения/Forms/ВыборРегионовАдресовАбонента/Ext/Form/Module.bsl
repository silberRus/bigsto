...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СжатьАдресаКонтактнойИнформации(АдресаАбонента, СжатьДоРегиона);
	
	Список.Загрузить(АдресаАбонента);
...
Процедура СжатьАдресаКонтактнойИнформации(АдресаАбонента, СжатьДоРегиона = Ложь)
	...
	Если СжатьДоРегиона Тогда
		Для Каждого СтрокаАдреса Из АдресаАбонента Цикл
			ТорговыеПредложения.ПолучитьРегионыКонтактнойИнформации(СтрокаАдреса.ЗначенияПолей, СтрокаАдреса.Представление);
		КонецЦикла;
	КонецЕсли;
