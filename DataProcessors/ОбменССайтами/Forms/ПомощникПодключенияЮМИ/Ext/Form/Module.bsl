...
Процедура Далее(Команда)
	...
	Если НомерСтраницы = 2 Тогда
		...
		Если ТипСайта=0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите тип сайта'"));
			Возврат;
		КонецЕсли;
		
		Если НЕ ПринимаюСоглашение Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Чтобы создать сайт, нужно принять лицензионное соглашение'"));
			Возврат;
		КонецЕсли;
		
		Если СокрЛП(EmailРегистрацииСайта)="" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите e-mail, на который будет зарегистрирован сайт'"));
			Возврат;
		КонецЕсли;
		...
		Если НЕ ИмяСайтаСвободно Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Предложенное имя сайта занято, выберите другое'"));
			Возврат;
		КонецЕсли;
...
Процедура СоздатьСайт(Команда)
	...
	Если НЕ ПринимаюСоглашение Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Чтобы создать сайт, нужно принять пользовательское соглашение'"));
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ОсновнаяОрганизация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите организацию, на которую будет зарегистрирован сайт'"),,
			"ОсновнаяОрганизация");
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ДанныеСозданияСайтаСтруктура.Свойство("Ошибка") Тогда
		ЗакрытьФормуДлительнойОперации();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ДанныеСозданияСайтаСтруктура.Ошибка);
	Иначе
...
Процедура УправлениеФормой()
	...
	ИначеЕсли НомерСтраницы = 2 Тогда
		...
		Элементы.Далее.Видимость = Истина;
		Элементы.СоздатьСайт.Видимость = ЗначениеЗаполнено(ОсновнаяОрганизация);
...
Функция СвойстваСайтаВФоне()
	...
	ИдентификаторКоманды = "Обработки.ОбменССайтами.СоздатьСайт";
	Заголовок = НСтр("ru = 'Создание сайта'");
	
	ПараметрыСозданияСайта = ПараметрыСозданияСайта();
	
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(ЭтотОбъект.УникальныйИдентификатор, ИдентификаторКоманды, ПараметрыСозданияСайта, Заголовок);
...
Функция СайтВФонеПродолжение(ПараметрыСозданияСайта)
	...
	ИдентификаторКоманды = "Обработки.ОбменССайтами.СоздатьСайтПродолжить";
...
Процедура СоздатьСайтПродолжить()
	...
	Если ДанныеСайтаСтруктура.Свойство("Ошибка") Тогда
		 ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ДанныеСайтаСтруктура.Ошибка);
...
Функция ДанныеHTTPЗапроса(HTTPМетод, ТелоЗапроса, АдресЗапроса, ЗаголовкиЗапроса, ContentType="", ТекстОшибки="")
	...
	Если ОтветHTTP.КодСостояния<>200 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КодСостояния);
	КонецЕсли;
