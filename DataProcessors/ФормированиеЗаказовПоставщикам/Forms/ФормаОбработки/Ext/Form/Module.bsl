...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	НеобходимоРазмещениеЗаказов = РазместитьВЗаказахПоставщику И НепоставленныеВГрафикДокументыКоличество < Документы.Количество();
	
	Если НеобходимоРазмещениеЗаказов И ЗавершениеРаботы Тогда
		Отказ = Истина;
		ТекстПредупреждения = НСтр("ru = 'При продолжении закрытия не будет выполнено размещение заказов на отгрузку
								|в сформированных заказах поставщикам.'");
		Возврат;
	КонецЕсли;
	
	Если НеобходимоРазмещениеЗаказов Тогда
		РазместитьЗаказыНаСервере();
	КонецЕсли;
...
Процедура ПометитьНаУдаление(Команда)
	...
	Если Строки.Количество() = 1 И Действие = "ПометитьНаУдаление" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Пометить ""%1"" на удаление'"), Строки[0]);

	ИначеЕсли Строки.Количество() = 1 И Действие = "СнятьПометкуУдаления" Тогда

		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Снять с ""%1"" пометку удаления'"), Строки[0]);
...
Процедура ЗаполнитьПоставщикаНаСервере(Ссылка)
	...
	ПараметрыПолученияЦен.Соглашение   = УсловияЗакупок.Соглашение;
	ПараметрыПолученияЦен.ЭтоВыкупТары = Ложь;
...
Процедура УдалитьДокументы()
	...
	МассивСсылокНаУдаление = Документы.Выгрузить(, "Документ").ВыгрузитьКолонку("Документ");
...
Процедура ОбновитьНадписьПредупреждениеОРазмещенииЗаказов()
	...
	ИначеЕсли ЕстьНепроведенныеДокументы Тогда
		...
		ПредупреждениеОРазмещенииЗаказов = НСтр("ru = 'Имеются непроведенные заказы на поставку. Заказы на отгрузку будут размещены не полностью.'");

	ИначеЕсли НепоставленныеВГрафикДокументыКоличество > 0 Тогда

		ПредупреждениеОРазмещенииЗаказов = НСтр("ru = 'Имеются заказы на поставку в статусах, не позволяющих разместить в них заказы на отгрузку.'");
...
&НаСервере
Процедура РазместитьЗаказыНаСервере()
