...
Функция СоздатьЗаказы(Объект) Экспорт
	...
	ТекстЗапроса =
		...
		|	ВсеСоглашения.СрокПоставки КАК ЧислоДней
		...
		|		ПО ИСТИНА
		|ГДЕ
		|	НЕ ВсеСоглашения.Ссылка ЕСТЬ NULL
		|ИНДЕКСИРОВАТЬ ПО
		|	Склад, Соглашение
		|;
		|
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|		ДОБАВИТЬКДАТЕ(&НачалоДня, ДЕНЬ, ЕСТЬNULL(Т.Соглашение.СрокПоставки, 0))) КАК ДатаПоступления,
		|
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПараметрыПоиска КАК ПараметрыПоиска
		|		ПО ПараметрыПоиска.Склад      = Т.Склад
		|		 И ПараметрыПоиска.Соглашение = Т.Соглашение
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтДатыГрафика КАК ДатыГрафика
		|		ПО ДатыГрафика.ЧислоДней   = ПараметрыПоиска.ЧислоДней
		|		 И ДатыГрафика.ДатаОтсчета = ПараметрыПоиска.ДатаОтсчета
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ЕСТЬNULL(ДатыГрафика.Дата,
		|		ДОБАВИТЬКДАТЕ(&НачалоДня, ДЕНЬ, ЕСТЬNULL(Т.Соглашение.СрокПоставки, 0))), Т.ПодразделениеПолучатель,
		|	Т.ВидЦеныПоставщика
		|УПОРЯДОЧИТЬ ПО
...
Процедура ПолучитьДанныеОЗапасах(Таблица, ВременнаяТаблицаТовары, Запрос, Параметры)
	...
	ТекстыЗапроса = Новый СписокЗначений;
	ТекстыЗапроса.Добавить("", "");// формируется таблица ВтВтТоварыПоддерживаемогоЗапаса
...
Функция ТаблицаЗаказыКПоступлению()
	...
	Текст =
		...
		|	Т.Склад              КАК Склад,
		|
		|	СУММА(Т.КоличествоИзЗаказовСНеподтвержденными)  КАК ЗаказыКПоступлению,
...
Функция РазмещениеЗаказовДоступно(Таблица) Экспорт
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если (Строка.Отметка И Строка.КЗаказу > 0 Или Строка.Требуется = Строка.Обеспечено)
			И Не ЭтоАкт(Строка.Заказ) И Не ЭтоРазборка(Строка.Заказ)
			И Не ЗначениеЗаполнено(Строка.Назначение) Тогда
...
Функция ЭтоРазборка(Заказ)
	...
	Возврат ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказНаСборку")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Заказ, "ХозяйственнаяОперация") = Перечисления.ХозяйственныеОперации.РазборкаТоваров;
