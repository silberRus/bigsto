...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьТорговыеОбъектыКонтрагентов = ИнтеграцияЕГАИСПереопределяемый.ИспользоватьТорговыеОбъектыКонтрагентов();
	...
	Иначе
		Элементы.ОрганизацииТорговыйОбъект.Видимость = ИнтеграцияЕГАИСПереопределяемый.ИспользоватьТорговыеОбъектыКонтрагентов();
	КонецЕсли;
	
	ПараметрыУказанияСерий = ГосударственныеИнформационныеСистемыПереопределяемый.ПараметрыУказанияСерийФормыОбъекта(Объект, Обработки.СопоставлениеКлассификаторовЕГАИС);
...
Процедура ЗаполнитьТаблицыДляСопоставленияПоТТН()
	...
	Запрос.Текст = ИнтеграцияЕГАИСПереопределяемый.ТекстЗапросаСопоставлениеОрганизацийЕГАИСПоТТН();
	...
	Для Каждого СтрокаТЧ Из ТаблицаНеНайденных Цикл
		...
		НайденныйКонтрагент = Справочники.Контрагенты.НайтиПоНаименованию(СтрокаТЧ.НаименованиеЕГАИС, Ложь);
	...
	Запрос.Текст = ИнтеграцияЕГАИСПереопределяемый.ТекстЗапросаСопоставлениеНоменклатурыПоТТН();
	Запрос.УстановитьПараметр("СписокТТН", ТоварноТранспортныеНакладные);
...
Процедура ЗаполнитьТаблицыДляСопоставленияПоВходящимДанным(Товары = Неопределено, ОрганизацииЕГАИС = Неопределено)
	...
	Если ОрганизацииЕГАИС <> Неопределено Тогда
		...
		Запрос.Текст = ИнтеграцияЕГАИСПереопределяемый.ТекстЗапросаСопоставлениеОрганизацийЕГАИСПоВходящимДанным();
	...
	Если Товары <> Неопределено Тогда
		...
		Запрос = Новый Запрос;
		Запрос.Текст = ИнтеграцияЕГАИСПереопределяемый.ТекстЗапросаСопоставлениеНоменклатурыПоВходящимДанным();
		Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	...
	Попытка
		...
		Для Каждого СтрокаТЧ Из НеСопоставленныеСтроки Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.Контрагент)
				И (ЗначениеЗаполнено(СтрокаТЧ.ТорговыйОбъект) ИЛИ НЕ ИнтеграцияЕГАИСПереопределяемый.ИспользоватьТорговыеОбъектыКонтрагентов()) Тогда
