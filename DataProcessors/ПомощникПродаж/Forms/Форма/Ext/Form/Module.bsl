...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ ВыполняетсяЗакрытие и Модифицированность И НЕ ЗавершениеРаботы Тогда
		...
		Отказ = Истина;
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), НСтр("ru = 'Работа помощника будет завершена, все введенные данные будут потеряны. Закрыть помощник?'"), СписокКнопок);
...
#Область ПроверкиРеквизитовШапки
	...
	Если СкладОбязателен И Не СкладГруппа И Не ЗначениеЗаполнено(Объект.Склад)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Склад"" не заполнено'"),,"Объект.Склад",, Отказ);
	...
	Если НаправлениеДеятельностиОбязательно 
		...
		Отказ = Истина;
		ТекстОшибки = НСтр("ru='Необходимо заполнить Направление деятельности'");
...
#Область ПроверкиТабличнойЧастиТовары
	...
	ИначеЕсли Объект.Товары.Итог("КоличествоУпаковок") = 0 Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не введено ни одной строки для оформления документов продажи в список ""Товары"". Оформление тлько документов'"),
			,
			"Объект.Товары",
			,
			ЕстьОшибкиТовары);
	...
	Для ТекИндекс = 0 По Объект.Товары.Количество()-1 Цикл
		...
		Если Не ЗначениеЗаполнено(Объект.Товары[ТекИндекс].КоличествоУпаковок) И НЕ ЗначениеЗаполнено(Объект.Товары[ТекИндекс].КоличествоУпаковокКВозврату) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Товары[ТекИндекс].Сумма) И НЕ ЗначениеЗаполнено(Объект.Товары[ТекИндекс].КоличествоУпаковокКВозврату)
			И ?(Объект.ВернутьМногооборотнуюТару И НЕ Объект.ТребуетсяЗалогЗаТару, Объект.Товары[ТекИндекс].ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.МногооборотнаяТара, Истина) Тогда
...
Процедура ЗадатьВопросОбеспечение()
	...
	Если ЗаполнитьОбособленно И Объект.УпрощенноеОбеспечение И НЕ Объект.НеЗадаватьВопросОбеспечение Тогда
...
Процедура Декорация5ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ДосьеПартнера" Тогда
		ОткрытьФорму("Отчет.ДосьеПартнера.Форма.ФормаОтчета", Новый Структура("Партнер", Объект.Партнер),ЭтаФорма);
	КонецЕсли;
	Если НавигационнаяСсылкаФорматированнойСтроки = "ФормаСегментовЗапретаОтгрузки" Тогда
		ОткрытьФорму("Справочник.Партнеры.Форма.ФормаСегментовЗапретаОтгрузки", Новый Структура("Партнер", Объект.Партнер), ЭтаФорма);
	КонецЕсли;
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "СписокЗаказов" Тогда
		ОткрытьФорму("Документ.ЗаказКлиента.Форма.ФормаСпискаДокументов", Новый Структура("Отбор", Новый Структура("Партнер", Объект.Партнер)),ЭтаФорма);
	КонецЕсли;
КонецПроцедуры
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(
		Истина, "Обработка.ПомощникПродаж.Форма.Форма.Элементы.Товары.Выбор");
...
Процедура ТоварыПриАктивизацииСтроки(Элемент)
	...
	СтрокаСтатистика = НСтр("ru='При среднедневной продаже %1 %2 в день, хватит на %3 дн.'") + " ";
	СтрокаОстаток =  НСтр("ru='Осталось у клиента %1 %2.'") + " ";
	СтрокаСреднедневнаяПродажа = НСтр("ru='Среднедневная продажа %1 %2 в день.'") + " ";
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(ТекущиеДанные.КоличествоПрогнозныйОстаток) И ЭтоТоварТара Тогда
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(ТекущиеДанные.КоличествоДнейДоОкончанияЗапаса) И ЭтоТоварТара Тогда
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		Если Не Копирование Тогда
			ЗаполнитьВариантОформленияВСтрокеТабличнойЧасти(ТекущиеДанные, Объект.ВариантОформленияДокументов, Объект.Дата);
		КонецЕсли;
		ОбновитьНадписьВсегоПодобраноПозиций(НадписьВсегоПодобраноПозиций, Объект.Товары.Количество());
	КонецЕсли;
...
Процедура ТоварыУпаковкаПриИзменении(Элемент)
	...
	Если Объект.СоздаватьЗаявкуНаВозвратТоваровОтКлиентов Тогда
		ПересчитатьКоличествоУпаковокКВозврату(ТекущаяСтрока, КэшированныеЗначения);
	КонецЕсли;
	...
	ИначеЕсли Элемент.ТекущийЭлемент = Элементы.ОформленныеДокументыПечать Тогда
		...
		Если Элементы.ОформленныеДокументы.ТекущиеДанные <> Неопределено Тогда
			...
			Если Не ДоступнаПечатьКомплекта(Элементы.ОформленныеДокументы.ТекущиеДанные.Документ) Тогда
				...
				ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Для выбранного документа печать недоступна'"));
...
Процедура НоваяПродажа(Команда)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("НоваяПродажаЗавершение", ЭтотОбъект), НСтр("ru = 'Все несохраненные данные будут потеряны. Начать новую продажу?'"), СписокКнопок);
		Возврат;
	...
	Для Каждого ТекСтрока Из Объект.Документы Цикл
		...
		Если ТекСтрока.Состояние = 0 Тогда
			...
			ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ЗаказКлиента") И Объект.ПечататьЗаказКлиента Тогда
				МассивДокументов.Добавить(ТекСтрока.Документ);
			ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") И Объект.ПечататьЗаявкуНаВозвратТоваровОтКлиентов Тогда
				МассивДокументов.Добавить(ТекСтрока.Документ);
...
Процедура Изменить(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ИзменитьЗавершение", ЭтотОбъект), НСтр("ru = 'Сформированные документы будут заменены. Изменить реквизиты текущей продажи?'"), СписокКнопок);
...
#Область ОтметкаНезаполненногоПодразделения
	...
	СписокВариантовОформления.Добавить(Перечисления.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг);
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ВариантОформленияДокументов");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
	ОтборЭлемента.ПравоеЗначение = СписокВариантовОформления;
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение, СписокСвойств = Неопределено)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если НЕ ЗначениеЗаполнено(СписокСвойств) Тогда
			СписокСвойств = "НоменклатураНабора, ХарактеристикаНабора, Номенклатура, Характеристика, Упаковка, ВидЦены, Цена, КоличествоУпаковок, СрокПоставки, ПроцентРучнойСкидки, ВариантОбеспечения, Серия, ДатаОтгрузки";
			Если ЗначениеЗаполнено(Объект.Склад) Тогда
				СписокСвойств = СписокСвойств + ", Склад";
			КонецЕсли;
		КонецЕсли;
		ЗаполнитьСклады = СтрНайти(СписокСвойств, "Склад") = 0; //Заполнить склады по шапке, если склады не пришли в таблице
		...
		ЗаполнитьВариантОформленияВСтрокеТабличнойЧасти(ТекущаяСтрока, Объект.ВариантОформленияДокументов, Объект.Дата);
		
		Если ИспользоватьРасширенныеВозможностиЗаказаКлиента 
			И (НЕ Объект.НеОтгружатьЧастями И ТекущаяСтрока.ДатаОтгрузки = НачалоДня(ТекущаяДатаСеанса()) 
			ИЛИ (Объект.НеОтгружатьЧастями И Объект.ДатаОтгрузки = НачалоДня(ТекущаяДатаСеанса()))
...
Функция ЗаписатьЗаявкуНаВозврат(МассивНайденныхДокументов, Склад, Отказ)
	...
	ДокументОбъект.Приоритет = Справочники.Приоритеты.ПолучитьПриоритетПоУмолчанию(ДокументОбъект.Приоритет);
	ДокументОбъект.ХозяйственнаяОперация = ПродажиСервер.ПолучитьХозяйственнуюОперациюВозвратаПоРеализации(Объект.ХозяйственнаяОперация);
	...
	КодСтроки = 0;
	Для Каждого ТекСтрока Из Объект.Товары Цикл
		Если ТекСтрока.КоличествоУпаковокКВозврату > 0 
			И ТекСтрока.Склад = Склад Тогда
			КодСтроки = КодСтроки + 1;
			ТекСтрока.КодСтроки = КодСтроки;
			
			НоваяСтрокаТовары = ДокументОбъект.ВозвращаемыеТовары.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаТовары, ТекСтрока);
			НоваяСтрокаТовары.ДатаПоступления = ?(ДокументОбъект.ПоступлениеОднойДатой, Объект.ДатаОтгрузки, ТекСтрока.ДатаОтгрузки);
...
Функция ЗаписатьРеализацию(МассивНайденныхДокументов, Отказ, БезПроведения = Ложь)
	...
	РаботаСКурсамиВалютУТ.ЗаполнитьКурсКратностьПоУмолчанию(ДокументОбъект.Курс, ДокументОбъект.Кратность, ДокументОбъект.Валюта, ДокументОбъект.ВалютаВзаиморасчетов);
...
Функция ЗаписатьДоверенностьНаПолучениеДС(МассивНайденныхДокументов, ОснованиеОбъект, Отказ)
	...
	Попытка
		...
		Если СозданНовыйДокумент Тогда
			ДобавитьСтрокуВТаблицуСформированныхДокументов(
				ДокументОбъект.Ссылка,
				ОснованиеОбъект.СуммаДокумента,
				ОснованиеОбъект.Валюта,
				9);
			МассивНайденныхДокументов.Добавить(ДокументОбъект.Ссылка);
		ИначеЕсли СтрокаТаблицы <> Неопределено Тогда
			ОбновитьСтрокуВТаблицуСформированныхДокументов(
...
Функция ЗаписатьПриходныйКассовыйОрдер(МассивНайденныхДокументов, ОснованиеОбъект, Отказ)
	...
	Если ТипЗнч(ОснованиеОбъект) = Тип("Массив") Тогда
		Для Каждого ПродажаОбъект Из ОснованиеОбъект Цикл
			Если ПродажаОбъект.ДатаПлатежа = НачалоДня(ТекущаяДатаСеанса()) Тогда
	...
	ИначеЕсли ТипЗнч(ОснованиеОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Тогда
		Если ОснованиеОбъект.ДатаПлатежа = НачалоДня(ТекущаяДатаСеанса()) Тогда
...
#Область СозданиеРеализацииТоваровИУслугАктаВыполненныхРабот
	...
	ИначеЕсли Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг И НЕ Отказ Тогда
		...
		Если Объект.ХозяйственнаяОперация <> ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию") 
			И НЕ Отказ И Объект.Товары.Итог("СуммаБезВозвратнойТары") > 0 Тогда
...
#Область СозданиеЗаявкиНаВозвратТоваровОтКлиента

	Если Объект.СоздаватьЗаявкуНаВозвратТоваровОтКлиентов И Не Отказ И Объект.Товары.Итог("КоличествоУпаковокКВозврату") > 0 Тогда
...
Функция ПровестиОтменитьПроведениеДокументовСервер(ВыделенныеДокументы, Проведение = Истина)
	...
	Попытка
		...
		Если Отказ Тогда
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Проведение документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Иначе
			УправлениеТранспортнымиНакладными();
...
Процедура ОбновитьТекстИнформационнойНадписиФормируемыеДокументы()
	...
	ИначеЕсли Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиента Тогда
		...
		СуммаЗаказа = ?(Объект.ТребуетсяЗалогЗаТару,
			Объект.Товары.Итог("СуммаСНДС"), Объект.Товары.Итог("СуммаСНДСБезВозвратнойТары"));
		ДобавитьТекстНадписиВМассив(МассивНадписей, НСтр("ru='Заказ'"), "%СуммаИтог%", СуммаЗаказа, Объект.СоздаватьСчетНаОплату И Объект.ПорядокРасчетов <> Перечисления.ПорядокРасчетов.ПоНакладным);
	...
	Если Объект.ВариантОформленияДокументов <> Перечисления.ВариантыОформленияДокументовПродажи.КоммерческоеПредложение
		...
		Если ОплатаУказываетсяВРеализации(Объект.ВариантОформленияДокументов, Объект.ПорядокРасчетов) Тогда
			...
			СуммаОплаты = 0;
			Если Объект.ДатаПлатежа = НачалоДня(ТекущаяДатаСеанса()) Тогда
	...
	Если Объект.СоздаватьЗаявкуНаВозвратТоваровОтКлиентов И Объект.Товары.Итог("СуммаСНДСВозврат") > 0 Тогда
	...
	НадписьСформированныеДокументы = СтрЗаменить(НадписьСформированныеДокументы, "%СуммаЗаявки%",     Формат(СуммаЗаявки, "ЧДЦ=2"));
...
Процедура ЗаполнитьУсловияПродажПоУмолчанию()
	...
	Если Не ЗначениеЗаполнено(Объект.НалогообложениеНДС) Или Не ИспользоватьСоглашенияСКлиентами Тогда
...
Процедура УстановитьВидимостьРеквизитовОплаты()
	...
	Элементы.ДекорацияЗаголовокОплата.Видимость = ВидимостьЭлемента;
КонецПроцедуры
...
Процедура ЗаполнитьВариантОформленияВТабличнойЧастиСервер(ЗаполнитьСЗаменой = Истина)
	...
	Для Каждого ТекСтрока Из Объект.Товары Цикл
		Если ЗаполнитьСЗаменой ИЛИ НЕ ЗначениеЗаполнено(ТекСтрока.ВариантОформления) Тогда
			ЗаполнитьВариантОформленияВСтрокеТабличнойЧасти(ТекСтрока, Объект.ВариантОформленияДокументов, Объект.Дата);
		КонецЕсли;
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьВариантОформленияВСтрокеТабличнойЧасти(ТекСтрока, ВариантОформленияДокументов, Дата)
	...
	Если ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг") Тогда
		Если ТекСтрока.ДатаОтгрузки = НачалоДня(Дата) Или ТекСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга") Или ТекСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа") Тогда
...
Процедура ИнициализироватьПомощникПродаж()
	...
	ТаблицаОснованийДляПечати.Загрузить(Обработки.ПомощникПродаж.ТаблицаОснованийДляПечати(
		Новый Структура("ХозяйственнаяОперация,Договор", 
			Объект.ХозяйственнаяОперация, 
			Объект.Договор)));
...
Процедура ПересчитатьКоличествоУпаковокОбъемПредыдущегоЗаказа(ТекущаяСтрока, КэшированныеЗначения)
	...
	Иначе
		...
		Если Коэффициент <> 0 Тогда
			ТекущаяСтрока["КоличествоУпаковокОбъемПредыдущегоЗаказа"] = ТекущаяСтрока["КоличествоОбъемПредыдущегоЗаказа"] / Коэффициент;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ПересчитатьКоличествоУпаковокКВозврату(ТекущаяСтрока, КэшированныеЗначения)
...
Процедура РассчитатьСуммуНДСВозврат(ТекущаяСтрока, ЦенаВключаетНДС)
	...
	Если НЕ ЦенаВключаетНДС Тогда
		ТекПроцентНДС = ЦенообразованиеКлиентСервер.ПолучитьСтавкуНДСЧислом(ТекущаяСтрока.СтавкаНДС, Ложь);
		СуммаВозврат = ТекущаяСтрока.Цена * ТекущаяСтрока.КоличествоУпаковокКВозврату;
		СуммаНДС = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(СуммаВозврат, ТекПроцентНДС, ЦенаВключаетНДС);
	КонецЕсли;
...
Функция ПоместитьЭтапыОплатыВоВременноеХранилищеНаСервере(ОплатаРеализации)
	...
	Если ОплатаРеализации Тогда
		...
		СтрокаПредоплаты.ДатаПлатежа          = ТекущаяДатаСеанса();
...
Процедура ОбработатьИзменениеПараметровСервер(ЗначенияПараметров)
	...
	ВариантыОбеспечения = ПродажиСервер.ВариантыОбеспеченияПоУмолчанию(Объект.Соглашение, Объект.СтатусЗаказаКлиента, ЗначенияПараметров.ВариантОбеспечения);
	...
	Если ЭтоРеализация(Объект.ВариантОформленияДокументов)
		...
		И Объект.СтатусРеализацииТоваровУслуг = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате Тогда
			...
			ЗаполнитьСтатусыУказанияСерий();
	КонецЕсли;
	Если Объект.УпрощенноеОбеспечение ИЛИ Объект.НеОтгружатьЧастями Тогда
		Объект.НеОтгружатьЧастями = Истина;
		УстановитьВидимостьЭлементовФормыДатОтгрузки();
	КонецЕсли;
	УстановитьВидимостьЭлементовПоНастройкам();
	...
	Иначе
		...
		Для Каждого СтрокаДокумент Из Объект.Документы Цикл 
			Если ТипЗнч(СтрокаДокумент.Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
				...
				РеализацияПомеченаНаУдаление = СтруктураРезультат.ПометкаУдаления;
				РеализацияПроведена 		 = СтруктураРезультат.Проведен;
				УдалитьДоверенность = НЕ СоздаватьДоверенностьНаПолучениеДС ИЛИ НЕ РеализацияПроведена ИЛИ РеализацияПомеченаНаУдаление;
			ИначеЕсли ТипЗнч(СтрокаДокумент.Документ) = Тип("ДокументСсылка.ДоверенностьВыданная") Тогда
				СтрокаДоверенность = СтрокаДокумент;
			КонецЕсли;
		КонецЦикла;
		Если СтрокаДоверенность <> Неопределено
			И (НЕ СоздаватьДоверенностьНаПолучениеДС ИЛИ УдалитьДоверенность
			ИЛИ НЕ РеализацияПроведена ИЛИ РеализацияПомеченаНаУдаление) Тогда 
			Объект.Документы.Удалить(СтрокаДоверенность);
		ИначеЕсли СтрокаДоверенность = Неопределено И РеализацияОснование <> Неопределено
			И СоздаватьДоверенностьНаПолучениеДС И РеализацияПроведена И НЕ РеализацияПомеченаНаУдаление
			И НЕ УдалитьДоверенность Тогда
			ЗаписатьДоверенностьНаПолучениеДС(Новый Массив, РеализацияОснование, Ложь);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция НадписьЭтапыОплаты(Форма)
	...
	Если Не ЭтоЗаказ И ОплатаУказываетсяВРеализации(Объект.ВариантОформленияДокументов, Объект.ПорядокРасчетов) Тогда
		...
		ПараметрыОплаты = ЭтапыОплатыКлиентСервер.СтруктураПолученияНадписиЭтаповОплатыДляРеализациииТоваровУслуг();
		ЗаполнитьЗначенияСвойств(ПараметрыОплаты, Форма.Объект);
		
		ОбщаяСуммаПредоплаты = Объект.СуммаПредоплаты + Объект.СуммаПредоплатыЗаТару;
		
		ПараметрыОплаты.Вставить("СуммаВсего",      Форма.СуммаВсего);
		ПараметрыОплаты.Вставить("СуммаПредоплаты", ОбщаяСуммаПредоплаты);
		ПараметрыОплаты.Вставить("ИспользоватьСтатусыРеализацийТоваровУслуг", Форма.ИспользоватьСтатусыРеализацийТоваровУслуг);
		ПараметрыОплаты.Вставить("ГрафикИсполненияВДоговоре", Форма.ГрафикИсполненияВДоговоре);
		
		Возврат ЭтапыОплатыКлиентСервер.НадписьЭтапыОплатыДляРеализациииТоваровУслуг(ПараметрыОплаты);
...
Процедура УстановитьВидимостьЭлементовФормыДатОтгрузки()
	...
	Элементы.ЖелаемаяДатаОтгрузки.Видимость = Не ЭтоКоммерческоеПредложение И НЕ Объект.УпрощенноеОбеспечение;
	Элементы.НеОтгружатьЧастями.Видимость = Не ЭтоКоммерческоеПредложение И НЕ Объект.УпрощенноеОбеспечение И НЕ Объект.ОтображатьРекомендацииКПокупке;
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураЗаполнитьХарактеристики = Новый Структура("ЗаполнитьПризнакХарактеристикиИспользуются", Новый Структура("Номенклатура", "ХарактеристикиИспользуются"));
	НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(
		Объект.Товары,
		СтруктураЗаполнитьХарактеристики);
	НаборыСервер.ЗаполнитьСлужебныеРеквизиты(ЭтаФорма);
...
Процедура УстановитьВидимостьОпераций()
	...
	Иначе
		...
		ВозможнаРеализацияБезПереходаПраваСобственности = 
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Соглашение, "ВозможнаРеализацияБезПереходаПраваСобственности");
...
Функция ПодготовитьДанныеДляОбеспеченияЗаказа()
	...
	ПараметрыФормы.НастройкаЭлементовФормы.Заголовок = НСтр("ru = 'Состояние обеспечения корзины (Помощник продаж)'");
	ПараметрыФормы.НастройкаЭлементовФормы.ТекстКомандыПеренестиВДокумент = НСтр("ru = 'Перенести в корзину'");
...
Процедура УстановитьВидимостьЭлементовПоНастройкам()
	...
	Элементы.ТоварыВариантОбеспечения.Видимость = НЕ Объект.УпрощенноеОбеспечение;
	Элементы.ЗаполнитьОбеспечение.Видимость = НЕ Объект.УпрощенноеОбеспечение;
...
Функция ЗаполнитьТоварыПоСоглашениюСервер()
	...
	Если ЗначениеЗаполнено(Объект.Соглашение) Тогда
		ДатаТекущейОтгрузки = ?(ЗначениеЗаполнено(Объект.ДатаОтгрузки),Объект.ДатаОтгрузки, ТекущаяДатаСеанса());
		...
		Если Объект.ОтображатьРекомендацииКПокупке
			...
			|ИЗ
			|	РегистрНакопления.ПланыПродаж.Обороты(&НачПериода, &КонПериода, Авто, Партнер = &Партнер 
			|		И (Соглашение = &Соглашение
			|			ИЛИ Соглашение = ЗНАЧЕНИЕ(Справочник.СоглашенияСКлиентами.ПустаяСсылка))) КАК ПланыПродажОбороты
			|ГДЕ
			|	НЕ ПланыПродажОбороты.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Набор)");
		...
		Иначе
			...
			Если Объект.ОтображатьРекомендацииКПокупке
				...
				ТаблицаНоменклатуры = Запрос.Выполнить().Выгрузить();
				СписокСвойств = "Номенклатура, Характеристика, КоличествоПрогноз, ДатаПредыдущегоЗаказа, КоличествоПрогнозныйОстаток, КоличествоДнейДоОкончанияЗапаса, КоличествоОбъемПредыдущегоЗаказа, СреднесуточнаяПродажа, ЕдиницаИзмеренияПрогноз";
		...
		СтруктураПараметров = Новый Структура("АдресТоваровВХранилище, МаксимальнаяДатаОтгрузки", АдресТаблицы, Объект.ДатаОтгрузки);
...
Процедура ЗаполнитьТоварыПоСоглашениюСерверСЗамером()
	Если Объект.ЗаполнятьТоварыПоСоглашению Тогда
		Замер = ОценкаПроизводительности.НачатьЗамерДлительнойОперации("Обработка.ПомощникПродаж.Форма.Форма.ЗаполнитьТоварыПоСоглашениюСервер");
...
Процедура ОбновитьИнформациюДосьеПартнераРасчеты()
	...
	ТекстЗапросаРасчеты = "ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ОбновитьИнформациюДосьеПартнераКонтакты()
	...
	ТекстЗапросаСписокЗаказов = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Если ЗначениеЗаполнено(Объект.Партнер) Тогда
		Инфо = НСтр("ru='Досье партнера'")+ Символы.ПС;
		...
		Если ВыборкаСписокЗаказов.Следующий() Тогда
		...
		ТаблицаКИ = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(МассивВладельцевКИ, МассивТипыКИ, , ТекущаяДатаСеанса());
