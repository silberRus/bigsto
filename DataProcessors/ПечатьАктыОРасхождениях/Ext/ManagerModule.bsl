...
Процедура ЗаполнитьТабличныйДокументТорг1(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПечатьВНоменклатуреПоставщкика)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьАктыОРасхождениях.ПФ_MXL_ТОРГ1");
	...
	ПересчитатьСуммыВВалютеРегламентированногоУчета(ТаблицаТоваров, ТаблицаКурсовВалют, Ложь);
...
Функция СформироватьПечатнуюФормуТОРГ2(СтруктураТипов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ДанныеДляПечати = МенеджерОбъекта.ПолучитьДанныеДляПечатнойФормыТОРГ2(ПараметрыПечати, СтруктураОбъектов.Значение);
...
Процедура ЗаполнитьТабличныйДокументТорг2(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПечатьВНоменклатуреПоставщкика)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьАктыОРасхождениях.ПФ_MXL_ТОРГ2");
	...
	ТаблицаКурсовВалют         = ДанныеДляПечати.ДанынеКурсовВалют.Выгрузить();
	...
	Если ПечатьВНоменклатуреПоставщкика Тогда
		КолонкаКодов = "НоменклатураПоставщикаАртикул";
	Иначе
		КолонкаКодов = ФормированиеПечатныхФорм.ИмяДополнительнойКолонки();
		Если НЕ ЗначениеЗаполнено(КолонкаКодов) Тогда
...
Процедура ЗаполнитьТабличныйДокументТорг3(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПечатьВНоменклатуреПоставщкика)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьАктыОРасхождениях.ПФ_MXL_ТОРГ3");
...
//
Процедура ПересчитатьСуммыВВалютеРегламентированногоУчета(ТаблицаТоваров, ТаблицаВалют, РассчитыватьНедостачиИзлишки = Истина)
