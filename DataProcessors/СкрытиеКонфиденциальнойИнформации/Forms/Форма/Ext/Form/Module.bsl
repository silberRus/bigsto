...
Процедура СохранитьНастройки(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("СохранитьНастройкиПродолжение", ЭтотОбъект);
...
Процедура ЗагрузитьНастройки(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗагрузитьНастройкиПродолжение", ЭтотОбъект);
	НачатьПодключениеРасширенияРаботыСФайлами(ОписаниеОповещения);
...
Процедура Подключаемый_ПроверитьСкрытиеКонфиденциальнойИнформации()
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		МодульДлительныеОперацииКлиент.ЗакрытьФормуДлительнойОперации(ФормаДлительнойОперации);
		ВызватьИсключение;
	КонецПопытки;
...
&НаКлиенте
Процедура СохранитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ПолучитьФайл(НастройкиОбработки(), "settings.xml", Истина);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗагрузитьНастройкиПродолжениеПослеПомещенияФайла", ЭтотОбъект);
...
&НаКлиенте
Процедура ЗагрузитьНастройкиПродолжениеПослеПомещенияФайла(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьНастройки(Адрес, Истина);
...
&НаСервере
Процедура УстановитьНастройки(Результат, РезультатВХранилище = Ложь)
	
	Если РезультатВХранилище Тогда
		ФайлНастроек = ПолучитьИзВременногоХранилища(Результат);
	Иначе
		ФайлНастроек = ПолучитьИзВременногоХранилища(Результат[0].Хранение);
	КонецЕсли;
