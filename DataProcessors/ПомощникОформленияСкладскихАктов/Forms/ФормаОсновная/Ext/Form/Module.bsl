...
&НаКлиенте
Процедура ОтбратьПоТипуОперацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;

	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("ВыбранныеОперации", ОтбратьПоТипуОперации.ВыгрузитьЗначения());
	
	Оповещение = Новый ОписаниеОповещения("ОтбратьПоТипуОперацииНачалоВыбораЗавершение", ЭтаФорма);
...
&НаКлиенте
Процедура ОтбратьПоТипуОперацииНачалоВыбораЗавершение(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	ИначеЕсли Результат.Количество() = 0 Тогда
		ОтбратьПоТипуОперации.Очистить();
	Иначе
		ОтбратьПоТипуОперации.ЗагрузитьЗначения(Результат);
		Для Каждого Строка Из Объект.Товары Цикл
			Строка.ПризнакДляОтбора = Не ОтбратьПоТипуОперации.НайтиПоЗначению(Строка.ОперацияТекст) = Неопределено;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ОтбратьПоТипуОперацииОчистка(Элемент, СтандартнаяОбработка)
...
Процедура ЗачестьПоПересортицеПоПорче(Команда)
	...
	Если ТекущиеДанныеСписания.СтавкаНДС <> ТекущиеДанныеОприходования.СтавкаНДС Тогда
		ТекстСообщения = НСтр("ru='Номенклатура списания и оприходования имеют разные ставки НДС, поэтому их нельзя зачесть'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура РаспределитьПоВладельцам(Команда)
	...
	КонецЦикла;
	ПараметрыФормы.Вставить("МассивВладельцев", МассивВладельцев);
...
Процедура РаспределитьПоВладельцамЗавершение(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Массив") И Результат.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбрано организаций для распределения.'"));
		Возврат;
	КонецЕсли;
	...
	Если СозданоСтрок = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не удалось провести автоматическое распределение по владельцам. Возможно у выбранных владельцев нет остатков по выбранным товарам.'"));
	КонецЕсли;
...
Процедура ТоварыНеРаспределенныеОбновитьФильтр()
	...
	Если ОтбратьПоТипуОперации.Количество() = 0 Тогда
		Элементы.ТоварыНеРаспределенные.ОтборСтрок = Новый ФиксированнаяСтруктура("НеРаспределено", Истина);
	Иначе
...
Процедура ОперациюРаспределитьНаОрганизациюСервер(СтрокаТаблицы, Организация, СтрокаРаспределение = Неопределено)
	...
	Если СтрокаРаспределение = Неопределено Тогда	
		Отбор.Вставить("Партнер", 	Справочники.Партнеры.ПустаяСсылка());
		Отбор.Вставить("Договор", 	Справочники.ДоговорыКонтрагентов.ПустаяСсылка());
	Иначе                           
		Отбор.Вставить("Партнер", 	СтрокаРаспределение.Партнер);
		Отбор.Вставить("Договор", 	СтрокаРаспределение.Договор);
	КонецЕсли;
	...
	Если МассивСтрок.Количество() = 0 Тогда
		...
		Если СтрокаРаспределение <> Неопределено Тогда 
			ЗаполнитьЗначенияСвойств(Строка, СтрокаРаспределение, "Партнер, Договор");
		КонецЕсли;
	Иначе
...
Функция ДанныеКРаспределению()
	...
	ВыбранныеВладельцы = Объект.ВыбранныеОрганизации.Выгрузить();
	...
	Запрос.УстановитьПараметр("ВыбранныеВладельцы", ВыбранныеВладельцы);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВыбранныеВладельцы.НомерСтроки КАК Порядок,
	|	ВыбранныеВладельцы.Организация КАК Организация,
	|	ВыбранныеВладельцы.Партнер КАК Партнер,
	|	ВыбранныеВладельцы.Договор КАК Договор
	|ПОМЕСТИТЬ ВыбранныеОрганизации
	|ИЗ
	|	&ВыбранныеВладельцы КАК ВыбранныеВладельцы
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация";
	...
	ДатаОстатков = ?(ДатаОформленияСкладскихАктов = Дата(1, 1, 1), ТекущаяДатаСеанса(), ДатаОформленияСкладскихАктов);
	...
	СписокПоклажедателей = Новый Массив;
	СписокВсехОрганизаций = Новый Массив;
	СписокОрганизацийПоклажедателей = Новый Массив;
	
	Для Каждого Владелец Из Объект.ВыбранныеОрганизации Цикл
		СписокВсехОрганизаций.Добавить(Владелец.Организация);
		
		Если ЗначениеЗаполнено(Владелец.Партнер) Тогда
			СписокПоклажедателей.Добавить(Владелец.Партнер);
			СписокОрганизацийПоклажедателей.Добавить(Владелец.Организация);
		Иначе
			СписокОрганизаций.Добавить(Владелец.Организация);
		КонецЕсли;
	КонецЦикла;
	
	ДокументОбъект.Вставить("Организация", СписокВсехОрганизаций);
	...
	ЕстьОтборПоОрганизациям = ЗначениеЗаполнено(СписокОрганизаций);
	ЕстьОтборПоПоклажедателям = ЗначениеЗаполнено(СписокПоклажедателей);
	...
	Если ЕстьОтборПоПоклажедателям Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ВыбранныеОрганизации.Организация КАК Организация,
		|	ВыбранныеОрганизации.Партнер     КАК Партнер
		|ПОМЕСТИТЬ ВыбранныеПоклажедатели
		|ИЗ
		|	ВыбранныеОрганизации КАК ВыбранныеОрганизации
		|ГДЕ
		|	ВыбранныеОрганизации.Партнер В(&Поклажедатели)
		|	И ВыбранныеОрганизации.Организация В(&ОрганизацииПоклажедателей)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	Партнер
		...
		|	ЕСТЬNULL(ТаблицаОстатков.ВидЗапасов.ВладелецТовара, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)) КАК Партнер,
		|	ЕСТЬNULL(ТаблицаОстатков.ВидЗапасов.Договор, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)) КАК Договор,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВыбранныеПоклажедатели КАК ВыбранныеПоклажедатели
		|		ПО ВыбранныеПоклажедатели.Организация = ТаблицаОстатков.Организация
		|			И ВыбранныеПоклажедатели.Партнер = ТаблицаОстатков.ВидЗапасов.ВладелецТовара
		|			И ТаблицаОстатков.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаОстатков.Организация,
		|	ЕСТЬNULL(ТаблицаОстатков.ВидЗапасов.ВладелецТовара, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)),
		|	ЕСТЬNULL(ТаблицаОстатков.ВидЗапасов.Договор, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)),
		...
		Запрос.УстановитьПараметр("Поклажедатели", СписокПоклажедателей);
		Запрос.УстановитьПараметр("ОрганизацииПоклажедателей", СписокОрганизацийПоклажедателей);
	КонецЕсли;
	...
	Запрос.Текст = СтрСоединить(ЧастиЗапроса,
		?(ЧастиЗапроса.Количество() > 1, ТекстОбъединения, ""));
	...
	Для Каждого Строка Из Объект.ТоварыРаспределенные Цикл
		...
		Отбор.Вставить("Партнер", 		Строка.Партнер);
	...
	ТаблицаРаспределений.Колонки.Добавить("Партнер", Новый ОписаниеТипов("СправочникСсылка.Партнеры"));
	ТаблицаРаспределений.Колонки.Добавить("Договор", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
...
Процедура ОформитьСкладскиеАктыСервер()
	...
	ДатаДокумента = ?(ДатаОформленияСкладскихАктов = Дата(1, 1, 1), ТекущаяДатаСеанса(), ДатаОформленияСкладскихАктов);
	Для Каждого ВидОперации Из МассивВидовОпераций Цикл
		...
		Владельцы = Операции.Скопировать(, "Организация, Партнер, Договор");
		Владельцы.Свернуть("Организация, Партнер, Договор");
		Для Каждого СтрокаВладелец Из Владельцы Цикл 
			Отбор = Новый Структура;
			Отбор.Вставить("Организация", 	СтрокаВладелец.Организация);
			Отбор.Вставить("Партнер", 		СтрокаВладелец.Партнер);
			Отбор.Вставить("Договор", 		СтрокаВладелец.Договор);
			Отбор.Вставить("ОперацияТекст", ВидОперации);
			Если ВидОперации = "Оприходование" Тогда
				ДокументСсылка = СоздатьОприходованиеИзлишковТоваровСервер(СтрокаВладелец.Организация, ДатаДокумента, Операции.НайтиСтроки(Отбор));
				СкладскиеАкты.Добавить(ДокументСсылка);
			ИначеЕсли ВидОперации = "Порча" Тогда
				ДокументСсылка = СоздатьПорчаТоваровСервер(СтрокаВладелец.Организация, ДатаДокумента, Операции.НайтиСтроки(Отбор));
				СкладскиеАкты.Добавить(ДокументСсылка);
			ИначеЕсли ВидОперации = "Пересортица" Тогда
				ДокументСсылка = СоздатьПересортицаТоваровСервер(СтрокаВладелец.Организация, ДатаДокумента, Операции.НайтиСтроки(Отбор));
				СкладскиеАкты.Добавить(ДокументСсылка);
			ИначеЕсли ВидОперации = "Списание" Тогда
				ДокументСсылка = СоздатьСписаниеНедостачТоваровСервер(СтрокаВладелец.Организация, ДатаДокумента, Операции.НайтиСтроки(Отбор));
				СкладскиеАкты.Добавить(ДокументСсылка);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция СоздатьОприходованиеИзлишковТоваровСервер(Знач Организация, Знач ДатаДокумента, Знач СтрокиРаспределения)
	...
	Если ЗначениеЗаполнено(ДокументОбъект.ВидЦены) И ДокументОбъект.Товары.Количество() > 0 Тогда
		...
		ПродажиСервер.ЗаполнитьЦены(ДокументОбъект.Товары,, ПараметрыЗаполнения, Действия);
...
Процедура УстановитьОтображениеПоСкладу()
	...
	Если ЗначениеЗаполнено(Объект.Склад) Тогда
		...
		Если ФлагПоСкладуЕстьНезавершенныеПересчеты Тогда
			МассивСтрок = Новый Массив;
			МассивСтрок.Добавить(НСтр("ru='Есть'") + " ");
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='пересчеты товаров'"),,,,"отчет"));
			МассивСтрок.Добавить(НСтр("ru=', по которым еще не завершено внесение результатов (они не проведены в статусе ""Выполнено"").'"));
...
Процедура СохранитьНастройкиСервер()
	...
	Если ТипЗнч(ВыбранныеВладельцыПоСкладам) = Тип("ТаблицаЗначений") Тогда
		МассивСтрок = ВыбранныеВладельцыПоСкладам.НайтиСтроки(Новый Структура("Склад", Объект.Склад));
		Для Каждого Строка Из МассивСтрок Цикл
			ВыбранныеВладельцыПоСкладам.Удалить(Строка);
		КонецЦикла;
		
		Если ВыбранныеВладельцыПоСкладам.Колонки.Найти("Партнер") = Неопределено Тогда
			ВыбранныеВладельцыПоСкладам.Колонки.Добавить("Партнер", Новый ОписаниеТипов("СправочникСсылка.Партнеры"));	
		КонецЕсли;
		Если ВыбранныеВладельцыПоСкладам.Колонки.Найти("Договор") = Неопределено Тогда
			ВыбранныеВладельцыПоСкладам.Колонки.Добавить("Договор", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));	
		КонецЕсли;	
...
Процедура УстановитьУсловноеОформлениеСервер()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыРаспределенныеОрганизация.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ТоварыРаспределенные.Партнер");
...
Функция ПодготовитьТаблицуТоваровДляОтчетаСервер()
	...
	ТипыОпераций = ОтбратьПоТипуОперации.ВыгрузитьЗначения();
...
Функция РаспределитьПоОрганизациямЗавершениеСервер(Знач Результат, Знач ДополнительныеПараметры)
	...
	Объект.ВыбранныеОрганизации.Очистить();
	Для Каждого Владелец Из Результат Цикл
...
Процедура СортироватьТаблицуРаспределенныхТоваров(Таблица)
	...
	Таблица.Сортировать("ОперацияТекст, Организация, Партнер, Договор, НоменклатураСписываемая, ХарактеристикаСписываемая,
		|НоменклатураПриходуемая, ХарактеристикаПриходуемая");
	
КонецПроцедуры

&НаСервере
Функция СтруктураДанныхЗаполнения(Организация, ДатаДокумента, СтруктураПоклажедатель = Неопределено)
	...
	Если СтруктураПоклажедатель <> Неопределено Тогда
		ДанныеЗаполнения.Вставить("МестоХранения", 	Объект.Склад);
		ДанныеЗаполнения.Вставить("Партнер", 		СтруктураПоклажедатель.Партнер);
		ДанныеЗаполнения.Вставить("Договор", 		СтруктураПоклажедатель.Договор);
	КонецЕсли;
