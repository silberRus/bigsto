...
// Возвращает текст запроса статистики потребления.
//
// Параметры:
//   СоставРазделовЗапроса - Структура, Неопределено - структура, содержит имена таблиц запроса
...
Функция СформироватьТекстЗапросаСтатистикиПотребления(СоставРазделовЗапроса = Неопределено) Экспорт
	...
	Если ВыполнятьРасчетПоКоличествуПродаж Тогда
		...
		Если УчитыватьСборкиТоваров И Не УчитыватьВнутреннееТовародвижение Тогда
			Текст = Справочники.ФорматыМагазинов.ТекстЗапросаВтФорматыСкладов();
			Текст = СтрЗаменить(Текст, "ВтТовары", "ТоварыСреднедневногоПотребления");
			ТекстЗапроса = ТекстЗапроса + Текст;
		КонецЕсли;
...
//
// Параметры:
//   СоставРазделовЗапроса - Структура, Неопределено - содержит имена таблиц запроса и индекс таблицы при получении результата выполнения пакета.
//
// Возвращаемое значение:
//   Строка - Текст запроса.
//
...
//
// Параметры:
//   КлючНоменклатуры - Структура - Структура с полями:
...
//    * ИспользоватьХарактеристикиНоменклатуры - Булево - значение соотвествующей ФО.
//    * НачалоПериода                          - Дата   - текущая дата.
//    * ПериодКлассификацииДатаНачала          - Дата   - дата начала периода расчета статистики.
//    * ПериодКлассификацииДатаОкончания       - Дата   - дата окончания периода расчета статистики.
//    * АдресГрафикаБезКалендаря - Строка - содержит адрес во временном харнилище таблицы с перечислением
//                                          всех дат в периоде расчета статистики.
//                                          Используется, если не задан календарь предприятия, для передачи
//                                          в запрос всех дней в периоде.
//
// Возвращаемое значение:
...
//    * ПотреблениеСтабильно     - Булево - истина, если СКО менее 5%, ложь - в противном случае.
//
Функция СтатистикаПотребления(КлючНоменклатуры, ПараметрыРасчета, АдресГрафикаБезКалендаря) Экспорт
	...
	|//////////////////////////////////////////////////////////////////////////
	|"
	+ СформироватьТекстЗапросаСтатистикиПотребления() +
	"ВЫБРАТЬ
...
// Создает таблицу значений, содержащую даты в периоде заданном параметрами. Если задан основной календарь предприятия, таблица остается пустой.
//
// Параметры:
//		ПериодКлассификацииДатаНачала - Дата - Начало периода.
//		ПериодКлассификацииДатаОкончания - Дата - Окончание периода.
//		УникальныйИдентификатор - УникальныйИдентификатор - Идентификатор формы, для освобождения хранилища при ее закрытии.
// Возвращаемое значение:
//		Строка - Адрес во временном хранилище куда помещена созданная таблица.
//
...
Функция ТаблицаЗапасы(Отбор, ТипОтбора, ТаблицаСпособовОбеспечения = Неопределено) Экспорт
	...
	Если ТипОтбора = "ОтборПоЗначениям" Тогда
		...
		Запрос.Текст =
			...
			|	Т.СпособОбеспечения.ФормироватьПлановыеЗаказы КАК ФормироватьПлановыеЗаказы,
	...
	Запрос.Текст = Запрос.Текст + Справочники.СпособыОбеспеченияПотребностей.ВременнаяТаблицаИнтерваловРаботыСкладов();
	...
	Запрос.Текст = РегистрыСведений.ТоварныеОграничения.ПодставитьСоединениеДляУпаковкиЗаказа(Запрос.Текст,
...
Функция ТаблицаПотребностейПоЗаказамИОстатков(Отбор, ТипОтбора, ТаблицаСпособовОбеспечения = Неопределено, ТаблицаЗапасы = Неопределено) Экспорт
	...
	Иначе
		...
		СхемаКомпоновкиДанных.НаборыДанных.Набор.Запрос = ВременнаяТаблицаТоваровДляОбеспеченияЗаказовБезОтборовПоРеквизитамДокументов(Истина);
		Запрос = ПолучитьЗапросСОтборамиКомпоновкиДанных(АдресСхемыКомпоновкиДанных, КомпоновщикНастроек, "Набор");

		Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц();
		
		// Проверка наличия отбора по реквизитам заказов.
		УстановитьПривилегированныйРежим(Истина);
		СхемаКомпоновкиДанных.НаборыДанных.Набор.Запрос = ВременнаяТаблицаТоваровДляОбеспеченияЗаказовБезОтборовПоРеквизитамДокументов(Ложь)
			+ ВременнаяТаблицаТоваровДляОбеспеченияЗаказов();
		ЗапросОтбораПоРеквизитам = ПолучитьЗапросСОтборамиКомпоновкиДанных(АдресСхемыКомпоновкиДанных, КомпоновщикНастроек, "Набор");
		УстановитьПривилегированныйРежим(Ложь);
		НачалоТекстЗапроса = СтрНайти(ЗапросОтбораПоРеквизитам.Текст, "ВЫБРАТЬ", НаправлениеПоиска.СКонца, , 2);
		ЗапросОтбораПоРеквизитам.Текст = Прав(ЗапросОтбораПоРеквизитам.Текст, СтрДлина(ЗапросОтбораПоРеквизитам.Текст) - НачалоТекстЗапроса + 1);
		
		ЕстьОтборПоРеквизитамДокументов = СтрНайти(Запрос.Текст, "НаправлениеДеятельности") > 0;
		Если Не ЕстьОтборПоРеквизитамДокументов Тогда
			
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВтТоварыБезОтборовПоРеквизитамДокументов", "ВтТовары");
			
		КонецЕсли;
		
		Запрос.Выполнить();
		
		Если ЕстьОтборПоРеквизитамДокументов Тогда
			
			ЗапросОтбораПоРеквизитам.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
			УстановитьПривилегированныйРежим(Истина);
			ЗапросОтбораПоРеквизитам.Выполнить();
			УстановитьПривилегированныйРежим(Ложь);
			Запрос = ЗапросОтбораПоРеквизитам;
	...
	Запрос.Выполнить();
	Запрос.Текст = Справочники.СпособыОбеспеченияПотребностей.ВременнаяТаблицаИнтерваловРаботыСкладов();
	...
	ТаблицаПолуфабрикатов.Колонки.Добавить("Заказ");
	ТаблицаПолуфабрикатов.Колонки.Добавить("Номенклатура");
	ТаблицаПолуфабрикатов.Колонки.Добавить("Характеристика");
	ТаблицаПолуфабрикатов.Колонки.Добавить("Назначение");
	ТаблицаПолуфабрикатов.Колонки.Добавить("ЭтоТовар");
	ТаблицаПолуфабрикатов.Колонки.Добавить("Количество");
	
	Если ТипОтбора = "ОтборПоЗначениям" Тогда

		ТаблицаОстатки               = Неопределено;
		ТаблицаОстатковНаВсехСкладах = Неопределено;
	...
	Иначе
		...
		Запрос.Текст = Запрос.Текст +
			...
			|		ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК ПодразделениеПолучатель,
			|		Т.Назначение     КАК Назначение,
			|		Т.КоличествоИзЗаказовСНеподтвержденнымиОстаток - Т.КоличествоИзЗаказовОстаток
			|			+ Т.КоличествоПодЗаказСНеподтвержденнымиОстаток - Т.КоличествоПодЗаказОстаток КАК КРезервированию
			|	ИЗ
			...
			|					ИЗ
			|						ВтРазличныеТовары КАК Фильтр)) КАК Т
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварныеОграничения КАК Настройка
			|			ПО &ПодстановкаТоварногоОграничения
			|	ГДЕ
			|		(Т.КоличествоИзЗаказовСНеподтвержденнымиОстаток > Т.КоличествоИзЗаказовОстаток
			|			ИЛИ Т.КоличествоПодЗаказСНеподтвержденнымиОстаток > Т.КоличествоПодЗаказОстаток)
			|		И ЕСТЬNULL(Настройка.ОбеспечениеЗаказовПриПоддержанииЗапаса, ЗНАЧЕНИЕ(Перечисление.ОбеспечениеЗаказовПриПоддержанииЗапаса.ЗаСчетЗапасов))
			|			<> ЗНАЧЕНИЕ(Перечисление.ОбеспечениеЗаказовПриПоддержанииЗапаса.НезависимоОтЗапасов)
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			...
			|			 И Т.Подразделение = Фильтр.ПодразделениеПолучатель
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварныеОграничения КАК Настройка
			|			ПО &ПодстановкаТоварногоОграничения
			|	ГДЕ
	...
	ЕстьПолуфабрикаты = ТаблицаПолуфабрикатов.Количество() > 0;
	...
	Возврат Новый Структура("ТаблицаПотребностей, ТаблицаОстатков, ТаблицаОстатковНаВсехСкладах", Таблица, ТаблицаОстатки, ТаблицаОстатковНаВсехСкладах);
...
Функция ВременнаяТаблицаТоваровДляОбеспеченияЗаказов()
	...
	ТекстЗапроса =
		...
		|ПОМЕСТИТЬ ВтТовары
		|ИЗ
		|	ВтТоварыБезОтборовПоРеквизитамДокументов КАК Товары
		|
		|		{ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыДокумента
		|		ПО РеквизитыДокумента.Ссылка = Товары.Заказ
...
Функция ВременнаяТаблицаТоваровДляОбеспеченияЗаказовБезОтборовПоРеквизитамДокументов(ДляВыборки)
	...
	ТекстЗапроса =
		...
		|
		|	СпрСпособ.Ссылка                     КАК СпособОбеспечения
		|ПОМЕСТИТЬ ВтТоварыБезОтборовПоРеквизитамДокументов
		|ИЗ
	...
	Если ДляВыборки Тогда
		...
		ТекстЗапроса = ТекстЗапроса +
			...
			|	РеквизитыДокумента.НаправлениеДеятельности КАК НаправлениеДеятельности}
			|ИЗ
			|	ВтТоварыБезОтборовПоРеквизитамДокументов КАК Т
			|
			|		{ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыДокумента
			|		ПО РеквизитыДокумента.Ссылка = Т.Заказ
			|		И НЕ РеквизитыДокумента.ДополнительнаяЗапись}";
...
Функция ВременнаяТаблицаСпособовОбеспечения()
	...
	ТекстЗапроса =
		...
		|
		|		{ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыДокумента
		|		ПО РеквизитыДокумента.Ссылка = Т.Регистратор
		|		И НЕ РеквизитыДокумента.ДополнительнаяЗапись}
		|ГДЕ
		|	Т.Активность
		|	И Т.КоличествоНеОбеспечено <> 0
		...
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыДокумента
		|		ПО РеквизитыДокумента.Ссылка = ОбеспечениеРаботами.Регистратор
		|		И НЕ РеквизитыДокумента.ДополнительнаяЗапись}
		|		 
		|		 
		|		
		...
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыДокумента
		|		ПО РеквизитыДокумента.Ссылка = ОбеспечениеЗаказов.Регистратор
		|		И НЕ РеквизитыДокумента.ДополнительнаяЗапись}
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОбеспеченияПотребностей КАК СпрСпособ
		|		ПО &ПодстановкаОсновногоСпособаОбеспечения
	...
	ТекстУсловияДоступаПоТипуОбеспечения = ТекстУсловияДоступаПоТипуОбеспечения + "
		|И ВЫБОР КОГДА СпрСпособ.ТипОбеспечения = ЗНАЧЕНИЕ(Перечисление.ТипыОбеспечения.Перемещение) ТОГДА
		|		ЕСТЬNULL(&ФОЗаказыНаПеремещенияВключена И НЕ Т.Склад.ЦеховаяКладовая
		|			ИЛИ &ФОПроизводство2_2Включена И Т.Склад.ЦеховаяКладовая, ЛОЖЬ)
		|	ИНАЧЕ ИСТИНА КОНЕЦ";
