...
Процедура ДекорацияУдостоверяющийЦентрНаименованиеНажатие(Элемент)
	...
	ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("http://ca.1c.ru/");
...
Функция ТелефонСоответствуетТребованиям(Знач ТелефонXML, ТекстСообщения)
	...
	ТекстСообщения = "";
	ТелефонСтруктура = МодульРаботаСАдресами.ПредыдущаяСтруктураКонтактнойИнформацииXML(ТелефонXML);
...
Функция ВладелецЗаполнен()
	...
	Если ДокументВид = "21"
	   И ПроверитьРеквизит(Отказ, "ДокументКодПодразделения") Тогда
		...
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументКодПодразделения", , Отказ);
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаявлениеXML(Билет)
	...
	ЗаписьXML.ЗаписатьАтрибут("ВерсПрог", "1С");
		...
		ЗаписьXML.ЗаписатьАтрибут("СНИЛС", ПредставлениеРеквизитаСтраховойНомерПФР(СтраховойНомерПФР));
			...
			ЗаписьXML.ЗаписатьАтрибут("УдостоверяющийЦентр", "ООО ""НПЦ ""1С"""); // Не локализуется (формат веб-сервиса).
...
Процедура СоздатьКлючИЗапросНаСертификатПослеВызоваВычислитьИдентификаторКлючаСубъекта(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Контекст.Вставить("РезультатПослеВызоваВычислитьИдентификаторКлючаСубъекта", Результат);
	
	Криптография.НачатьПолучениеСписокОшибок(Новый ОписаниеОповещения(
		"СоздатьКлючИЗапросНаСертификатПослеВызоваВычислитьИдентификаторКлючаСубъектаПослеПолученияСписокОшибок", ЭтотОбъект, Контекст));
	
КонецПроцедуры

// Продолжение процедуры СоздатьКлючИЗапросНаСертификат.
&НаКлиенте
Процедура СоздатьКлючИЗапросНаСертификатПослеВызоваВычислитьИдентификаторКлючаСубъектаПослеПолученияСписокОшибок(СписокОшибок, Контекст) Экспорт
	
	Если ЗначениеЗаполнено(СписокОшибок) Тогда
		СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваВычислитьИдентификаторКлючаСубъекта(СписокОшибок, Ложь, Контекст);
		Возврат;
	КонецЕсли;
	
	Результат = Контекст.РезультатПослеВызоваВычислитьИдентификаторКлючаСубъекта;
...
Функция ОписаниеЗапросаНаКвалифицированныйСертификат(Поля)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		ПолеОбщееИмя = СокрП(СтрЗаменить(Фамилия, " ", "_") + " " + СтрЗаменить(Имя, " ", "_")
	...
	ПолеОтчество = СокрП(СтрЗаменить(Имя, " ", "_") + " " + СтрЗаменить(Отчество, " ", "_"));
