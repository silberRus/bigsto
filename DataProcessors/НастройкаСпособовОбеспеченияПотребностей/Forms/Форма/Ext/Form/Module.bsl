...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Справочник.УпаковкиЕдиницыИзмерения.Форма.ФормаВыбораИзДокументов" Тогда
...
Процедура ОтображениеПриИзменении(Элемент)
	...
	Элементы.СтраницыРежимОтображения.ТекущаяСтраница = ?(Отображение = НСтр("ru = 'Форматы'"),
...
Процедура ЗаполнитьУпаковкуЗаказаНаСкладе(Команда)
	...
	Если ЕстьВыделенныеСтрокиВТаблице(Элементы.СписокСпособовОбеспеченияСклада) Тогда
		...
		Если Данные.УпаковкиИспользуются Тогда
			...
			ОткрытьФорму("Справочник.УпаковкиЕдиницыИзмерения.Форма.ФормаВыбораИзДокументов", ПараметрыФормы, ЭтаФорма,
				УникальныйИдентификатор);
		Иначе
...
Процедура ЗаполнитьСпособОбеспеченияНоменклатурой(Команда)
	...
	ТекущаяТаблица = ?(Отображение = НСтр("ru = 'Склады'"), Элементы.СписокСкладов, Элементы.ДеревоФорматов);
...
Процедура ЗаписатьНаКлиенте()
	Если Модифицированность ИЛИ НЕ ЗначениеЗаполнено(СхемаОбеспечения.Ссылка) Тогда
		Если ЗаписьНаСервере() Тогда
...
Процедура ВыполнитьКомандуПослеВопроса(Результат, ПараметрыВыполненияКоманды) Экспорт
	...
	Если ПараметрыВыполненияКоманды.Имя = "ЗаполнитьСпособОбеспеченияНоменклатурой" Тогда
		...
		Контекст = "СпособыОбеспеченияНоменклатурой";
		Если Отображение = НСтр("ru = 'Форматы'") Тогда
...
Процедура УстановитьВидимостьДоступностьПоРолямСхемаОбеспечения()
	...
	Элементы.ФормаЗаписатьИЗакрыть.Доступность = РедактированиеДоступно;
...
Процедура ЗаписатьПередВыполнениемКоманды(ИмяКоманды)
	...
	Если Параметры.Источник = "СхемаОбеспечения" И (НЕ ЗначениеЗаполнено(СхемаОбеспечения.Ссылка) ИЛИ Модифицированность) Тогда
...
Функция ЗаполнитьСпособОбеспеченияНоменклатуройНаСервере(СпособОбеспечения = Неопределено)
	...
	Если Отображение = НСтр("ru = 'Склады'") Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "Запись_СпособОбеспеченияПотребностей" Тогда
		Если Элементы.СтраницаСпособыОбеспеченияНоменклатурой.Видимость Тогда
			...
			Если Отображение = НСтр("ru = 'Форматы'")
