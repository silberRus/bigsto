...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВыполнитьСинхронизациюСБанком.Видимость = Пользователи.ЭтоПолноправныйПользователь()
														ИЛИ РольДоступна("ВыполнениеОбменаЭД");
...
Процедура ВывестиПодчиненныеДокументы(ТекущийОбъект, ДеревоРодитель)
	...
	Если ТипЗнч(ТекущийОбъект) = Тип("ДокументСсылка.СообщениеОбменСБанками") Тогда
		...
		|	СообщенияОбмена.ПредставлениеДокумента КАК Представление,
	...
	Иначе
		...
		|	СостоянияОбменСБанками.СообщениеОбмена.ПредставлениеДокумента КАК Представление,
...
Процедура ВывестиДокументИКартинку(СтрокаДерева, Макет, ЭтоТекущийДокумент = Ложь, ЭтоПодчиненный = Неопределено)
	...
	Иначе
		...
		ИначеЕсли СтрокаДерева.Направление = Перечисления.НаправленияЭД.Входящий И НЕ ТребуетсяПодпись
