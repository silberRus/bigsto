...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАдресВременногоХранилища(Параметры.АдресХранилищаДереваУпаковки) Тогда
		...
		Для Каждого СтрокаДерева Из ДеревоУпаковки.Строки Цикл
			...
			ЭтоУпаковка = ИнтеграцияЕГАИСКлиентСервер.ЭтоУпаковка(СтрокаДерева.ТипУпаковки);
...
Процедура ПоказатьИнформациюОПроблемахСАлкогольнойПродукцией(ИнформацияВДереве)
	...
	Если ИнформацияВДереве Тогда
		...
		Иначе
			...
			ЗначениеШтрихКода      = ДаннныеАлкогольнойПродукции.ШтрихкодУпаковки;
	...
	Иначе
		...
		Если ДанныеШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.DataMatrix Тогда
			...
			ПрефиксАлкогольнаяПродукцияНеИдентифицирована = НСтр("ru = 'Невозможно добавить неопознанную алкогольную продукцию с с data matrix  '");
			
		КонецЕсли;
		
		ЗначениеШтрихКода      = ДанныеШтрихкода.Штрихкод;
	...
	Иначе
		Префикс = СтрШаблон(ПрефиксАлкогольнаяПродукцияНеИдентифицирована);
	КонецЕсли;
	
	СтрокаШтриховойКод = Новый ФорматированнаяСтрока(ИнтеграцияЕГАИСКлиентСервер.ПредставлениеШтрихкода(ЗначениеШтрихКода),
	                                                 Новый Шрифт(,,,,Истина),
	                                                 ЦветаСтиля.ЦветГиперссылкиГИСМ,
	                                                 ,
	                                                 "СкопироватьШтриховойКодВБуферОбмена");
	
	ТекстСОшибкой = СтрШаблон(НСтр("ru = 'по причине:
	                                     |%1'"), ТекстОшибки);
