...
Функция СформироватьПодобранныеТовары()
	...
	Иначе
		...
		Если ОграничиватьВариантыОбеспечения Тогда
			ОсновнаяСтрока.ВариантОбеспечения = ВариантыОбеспечения[Строка(ОсновнаяСтрока.ТипНоменклатуры)];
		КонецЕсли;
		Если ВариантПредоставления = "Скидка" Тогда
		...
		Если Окр(ОсновнаяСтрока.КоличествоУпаковок, 3) > 0 Тогда
...
Процедура ОК(Команда)
	...
	Если РазборкаНаКомплектующие Тогда
		...
		Если ВариантыОбеспеченияСоответствие.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выбрано нескольно вариантов обеспечения, должно быть выбрано одно'"),,,,Отказ);
		КонецЕсли;
		Если КоличествоСерий > 1 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выбрано нескольно серий, должна быть выбрано одна'"),,,,Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура КУказаниюСерий(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ПодборТоваровВДокументПродажи.ЗапросКоличестваИСерий.Команда.КУказаниюСерий");
	
	ТекущиеДанные = Элементы.ТаблицаДоступныхДействийТекущая.ТекущиеДанные;
	ИдентификаторТекущейСтрокиТаблицаДоступныхДействийТекущая = ТекущиеДанные.ПолучитьИдентификатор();
	Если ТекущиеДанные.СтатусУказанияСерий = 0 Тогда
		ТекстСообщения = НСтр("ru='Для этого действия серии указывать не нужно.'");
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОграничиватьВариантыОбеспечения");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаОтмененнойСтрокиДокумента);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Без остатков'"));
...
Функция ТекущаяЕдиницаИзмерения()
	...
	ТекущаяЕдиницаИзмерения = НСтр("ru = 'ед'");
	Если ЭтоНабор Тогда
...
Процедура ОбновитьЦеныКомплектующих()
	...
	Если ПараметрыВариантаКомплектацииНоменклатуры.ВариантРасчетаЦеныНабора = Перечисления.ВариантыРасчетаЦенНаборов.РассчитываетсяИзЦенКомплектующих Тогда
		Элементы.ЦенаНабор.Доступность = Ложь;
		Цена = ЦенаНабора;
	Иначе
		Элементы.ЦенаНабор.Доступность = Истина;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьПараметрыПолученияТаблицыДоступныхДействий(ПараметрыПолученияТаблицыДоступныхДействий, ТипНоменклатуры)
	
	Если СписокВыбора <> Неопределено Тогда
		Если СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.Отгрузить) = Неопределено Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.Отгрузить);
		КонецЕсли;
		Если СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно) = Неопределено Тогда
	...
	Если ОграничиватьВариантыОбеспечения Тогда
		...
		ТипНоменклатурыСтрока = Строка(ТипНоменклатуры);
		
		Если ВариантыОбеспечения[ТипНоменклатурыСтрока] <> Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно);
		КонецЕсли;
		
		Если ВариантыОбеспечения[ТипНоменклатурыСтрока] <> Перечисления.ВариантыОбеспечения.Отгрузить Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.Отгрузить);
		КонецЕсли;
		
		Если ВариантыОбеспечения[ТипНоменклатурыСтрока] <> Перечисления.ВариантыОбеспечения.ИзЗаказов Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.ИзЗаказов);
		КонецЕсли;
		
		Если ВариантыОбеспечения[ТипНоменклатурыСтрока] <> Перечисления.ВариантыОбеспечения.СоСклада Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.СоСклада);
		КонецЕсли;
		
		Если ВариантыОбеспечения[ТипНоменклатурыСтрока] <> Перечисления.ВариантыОбеспечения.Обособленно Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.Обособленно);
		КонецЕсли;
		
		Если ВариантыОбеспечения[ТипНоменклатурыСтрока] <> Перечисления.ВариантыОбеспечения.Требуется Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.Требуется);
		КонецЕсли;
		
		Если ВариантыОбеспечения[ТипНоменклатурыСтрока] <> Перечисления.ВариантыОбеспечения.НеТребуется Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.НеТребуется);
		КонецЕсли;
...
&НаСервере
Процедура ПолучитьТаблицуДоступныхДействий(Отбор, АдресаВХранилище, НадписьОбеспечено, ВариантОбеспечения = Неопределено)
	...
	Если СтрокиОстатков.Количество() = 0 Тогда
		Если ПодборТоваров Тогда
			...
			ЗаполнитьПараметрыПолученияТаблицыДоступныхДействий(ПараметрыПолученияТаблицыДоступныхДействий, Отбор.ТипНоменклатуры);
		...
		Если КоличествоОформлено > 0 Тогда
			...
			СтруктураПоиска.ВариантОбеспечения = ВариантОбеспечения;
...
Процедура ДополнитьТаблицуДоступныхДействий()
	...
	Для Каждого Строка Из ТаблицаДоступныхДействий Цикл
		...
		ДатаСтрокой = Формат(Строка.ДатаОтгрузки, "ДЛФ=D");
		ЗаполненаДатаОтгрузки = (Строка.ДатаОтгрузки <> Дата(1, 1, 1, 0, 0, 0));
		
		Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ИзЗаказов Тогда
			
			Строка.Действие = СтрШаблон(НСтр("ru='Резервировать к %1'"), ДатаСтрокой);
			
		ИначеЕсли Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется И ЗаполненаДатаОтгрузки Тогда
			
			Строка.Действие = СтрШаблон(НСтр("ru='К обеспечению (срок %1)'"), ДатаСтрокой);
			
		ИначеЕсли Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно И ЗаполненаДатаОтгрузки Тогда
			
			Строка.Действие = СтрШаблон(НСтр("ru='Обеспечивать обособленно (срок %1)'"), ДатаСтрокой);
...
Процедура ОбновитьСписокВыбораДоступныхДействий()
	...
	Если ТаблицаДоступныхДействий.НайтиСтроки(СтруктураОтбора).Количество() > 0 Тогда
		Элементы.Действие.СписокВыбора.Добавить("Отгрузить", НСтр("ru = 'Отгрузить'"));
	КонецЕсли;
	...
	Если Действие = "Отгрузить" И Элементы.Действие.СписокВыбора.Количество() = 0 Тогда
		Действие = "ВсеДействия";
	КонецЕсли;
	Элементы.Действие.СписокВыбора.Добавить("РезервироватьОбеспечивать", НСтр("ru = 'Резервировать / Обеспечивать'"));
	Элементы.Действие.СписокВыбора.Добавить("ВсеДействия", НСтр("ru = 'Все действия'"));	
КонецПроцедуры
	...
	Если Не ПодборТоваров Тогда
		...
		Если ИзмененноеКоличество > ДоступноеИзмененненноеКоличество Тогда
			...
			ТекстСообщенияОформлено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Оформлено и изменить вариант обеспечения нельзя для %1 %2.'"),
					КоличествоОформлено,
					ЕдиницаИзмеренияНоменклатуры);
			
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Введено количества больше, чем требуется перераспределить.%1 Доступно для изменения %2 %3.'"),
...
Процедура ОстаткиСерийТекущиеКоличествоПриИзмененииНаСервере(ИдентификаторТекущейСтроки)
	...
	Если ТекущиеДанные.СтрокаЗаказа
		...
		Если ТекущиеДанные.Количество < КоличествоОформлено Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Введено количество меньше, чем уже оформлено. Оформлено %1 %2.'"),
			КоличествоОформлено,
			ЕдиницаИзмеренияНоменклатуры);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,);
		КонецЕсли;
