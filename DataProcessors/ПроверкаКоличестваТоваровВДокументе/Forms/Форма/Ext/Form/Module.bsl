...
Процедура СчитатьРеквизитыПроверитьОрдер()
	...
	Если Объект.РежимИсправления Тогда
		...
		Если НЕ РезультатЗапроса[0].Пустой() Тогда
			ТекстИсключения = НСтр("ru = 'В ордере есть строки с действием ""%Отобрать%"". Исправленение такого ордера не возможно. Его нужно проверять.'");
...
Процедура УстановитьУсловноеОформление()
	...
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтаФорма, 
																   "НедоборЕдиницаИзмерения", 
                                                                   "Объект.Недобор.Упаковка");
...
Функция ЗавершитьПроверкуСервер(ПереноситьРезультатВОрдер = Истина)
	...
	ВсеХорошо = ОбработкаОбъект.ЗавершитьПроверку(
...
Функция ЗавершитьПроверкуКлиент(Закрывать = Истина)
	...
	Иначе
		...
		Если ПереноситьРезультатВОрдер Тогда
			ВсеХорошо = ЗавершитьПроверкуСервер(Истина);
		Иначе
...
Процедура НеОтгружатьОбработчик()
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Для выполнения команды требуется выбрать строку.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Возврат;
	КонецЕсли;
