...
Процедура СохранитьНастройки()
	...
	Настройки.Вставить("ПериодЗаказа",              Объект.ПериодЗаказа);
	...
	Настройки.Вставить("ПериодПоступления",         Объект.ПериодПоступления);
...
Функция СписокПлановПоСтроке(КлючСтроки)
	...
	|				И Назначение = &Назначение) КАК ПланыЗакупокОбороты
...
Функция УдалитьЗаказыНаСервере(СсылкиНаУдаление)
	...
	Для Каждого Ссылка из ТабСсылок Цикл
		...
		Если Ссылка[0] <> Ссылка[1] Тогда
			...
			Если ТипЗнч(Ссылка[1]) = Тип("РегистрСведенийКлючЗаписи.ГрафикПлатежей")
				Или ТипЗнч(Ссылка[1]) = Тип("РегистрСведенийКлючЗаписи.СостоянияЗаказовПоставщикам")
