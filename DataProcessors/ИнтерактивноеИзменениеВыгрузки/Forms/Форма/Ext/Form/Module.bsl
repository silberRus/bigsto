...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ( Storverk Тюняков	
	СТ_ИнициализироватьНетиповойФункционал();
	Если Объект.ДополнительнаяРегистрация.Количество() > 0 Тогда
		ЭтотОбъект.СТ_ПериодДанных = Объект.ДополнительнаяРегистрация[0].Период;	
		ЭтотОбъект.СТ_ТолькоПроведенные = Истина;
	КонецЕсли; 
	// ) Storverk Тюняков  
КонецПроцедуры
...
&НаСервере
Процедура СТ_ПрименитьОбщиеНастройкиСервер()

	Для Каждого СтрокаНастроек Из Объект.ДополнительнаяРегистрация Цикл
		
		ЧастиИмени = СтрРазделить(СтрокаНастроек.ПолноеИмяМетаданных, ".");
		...
		Если КлассОМ = "Документ" Тогда
			
			Если ЗначениеЗаполнено(ЭтотОбъект.СТ_ПериодДанных) Тогда
			
				СтрокаНастроек.Период = ЭтотОбъект.СТ_ПериодДанных;	
				СтрокаНастроек.ВыборПериода = Истина;
			
			КонецЕсли; 
			
			МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(СтрокаНастроек.ПолноеИмяМетаданных);
			Если Метаданные.Документы[ИмяОМ].Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
					СтрокаНастроек.Отбор, 
					"Ссылка.Проведен", 
					Истина, 
					ВидСравненияКомпоновкиДанных.Равно,,
					ЭтотОбъект.СТ_ТолькоПроведенные);
			КонецЕсли;
				
			СтрокаНастроек.ОтборСтрокой = ПредставлениеОтбора(СтрокаНастроек.Период, СтрокаНастроек.Отбор);		
			
		КонецЕсли; 
		
	КонецЦикла;	

КонецПроцедуры // СТ_ПрименитьОбщиеНастройки()

&НаКлиенте
Процедура СТ_ПрименитьОбщиеНастройки(Команда)
	СТ_ПрименитьОбщиеНастройкиСервер();
КонецПроцедуры

&НаСервере
Процедура СТ_ИнициализироватьНетиповойФункционал()

	/////   РЕКВИЗИТЫ   //////////
	
	МассивДобавляемыхРеквизитов = Новый Массив;
	
	// ******   Реквизит формы "СТ_ПериодДанных"    ***********
	
	НовыйРеквизит = Новый РеквизитФормы("СТ_ПериодДанных", Новый ОписаниеТипов("СтандартныйПериод"));
	НовыйРеквизит.Заголовок = "Период данных";
	НовыйРеквизит.СохраняемыеДанные = Ложь;
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);
	
	// ******   Реквизит формы "СТ_ТолькоПроведенные"    ***********
	
	НовыйРеквизит = Новый РеквизитФормы("СТ_ТолькоПроведенные",Новый ОписаниеТипов("Булево"));
	НовыйРеквизит.Заголовок = "Только проведенные";
	НовыйРеквизит.СохраняемыеДанные = Ложь;
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);
	...
	// ******   Команда формы "СТ_ПрименитьОбщиеНастройки"    ***********
	
	НоваяКоманда = ЭтаФорма.Команды.Добавить("СТ_ПрименитьОбщиеНастройки");
	НоваяКоманда.Действие = "СТ_ПрименитьОбщиеНастройки";
	НоваяКоманда.Заголовок = "Применить общие настройки";
	НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
	НоваяКоманда.Отображение = ОтображениеКнопки.Авто;
	НоваяКоманда.Картинка = БиблиотекаКартинок.ЗаполнитьПоШаблону;
	НоваяКоманда.Подсказка = "Применить общие настройки к выбранным типам документов";
	
	/////   ЭЛЕМЕНТЫ   //////////
	
	
	// ******   Группа формы "ГруппаПериод"    ***********
	
	НовыйЭлемент = Этаформа.Элементы.Вставить("ГруппаПериод",Тип("ГруппаФормы"),Неопределено,ЭтаФорма.Элементы.ДополнительнаяРегистрация);
	НовыйЭлемент.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	НовыйЭлемент.Заголовок = "Период";
	НовыйЭлемент.Подсказка = "Группа период";
	...
	// ******   Кнопка формы "СТ_ПрименитьОбщиеНастройки"    ***********
	
	НовыйЭлемент = Этаформа.Элементы.Вставить("СТ_ПрименитьОбщиеНастройки",Тип("КнопкаФормы"),ЭтаФорма.Элементы.ГруппаПериод,Неопределено);
	НовыйЭлемент.Заголовок = "Применить";
	НовыйЭлемент.ИмяКоманды = "СТ_ПрименитьОбщиеНастройки";
	НовыйЭлемент.Отображение = ОтображениеКнопки.КартинкаИТекст;
	
	// ******   Поле формы "СТ_ТолькоПроведенные"    ***********
	
	НовыйЭлемент = Этаформа.Элементы.Вставить("СТ_ТолькоПроведенные",Тип("ПолеФормы"),ЭтаФорма.Элементы.ГруппаПериод,ЭтаФорма.Элементы.СТ_ПрименитьОбщиеНастройки);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	НовыйЭлемент.ПутьКДанным = "СТ_ТолькоПроведенные";
	
	// ******   Поле формы "СТ_ПериодДанных"    ***********
	
	НовыйЭлемент = Этаформа.Элементы.Вставить("СТ_ПериодДанных",Тип("ПолеФормы"),ЭтаФорма.Элементы.ГруппаПериод,ЭтаФорма.Элементы.СТ_ТолькоПроведенные);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.Заголовок = "За период";
	НовыйЭлемент.Подсказка = "Общий отбор по периоду, применяется для всех документов";
	НовыйЭлемент.ПутьКДанным = "СТ_ПериодДанных";	

КонецПроцедуры // СТ_ИнициализироватьНетиповойФункционал()
