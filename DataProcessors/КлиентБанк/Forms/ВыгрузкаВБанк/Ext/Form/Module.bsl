	...
	Для каждого Счет из Объект.БанковскиеСчета Цикл
		...
		Если Счет.ПрямойОбмен Тогда
			...
			Иначе
				...
				ВыгрузитьПлатежиПоСчетуПрямойОбмен(Счет.Ссылка);
...
Процедура ВыгрузитьПлатежиПоСчетуПрямойОбмен(БанковскийСчет)
	...
	Для каждого СтрокаКВыгрузке из СтрокиКВыгрузке Цикл
		МассивДокументов.Добавить(СтрокаКВыгрузке.Ссылка);
	КонецЦикла;
...
Процедура УдалитьИЗакрыть(Команда)
	...
	Для каждого Счет Из Объект.БанковскиеСчета Цикл
		Если Счет.Выгружен И Счет.ОбменЧерезФайл И ЗначениеЗаполнено(Счет.НастройкаОбмена) Тогда
			УдалитьФайлы(Счет.НастройкаОбмена);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработатьЗавершениеВыгрузки()
	...
	ОбработатьЗавершениеВыгрузкиНаСервере(ЕстьНеобходимостьОбновленияСпискаПлатежей);
...
&НаСервере
Процедура ОбработатьЗавершениеВыгрузкиНаСервере(ЕстьНеобходимостьОбновленияСпискаПлатежей)
...
&НаКлиенте
Процедура ВыборФайлаОбменаСБанком(ТекущиеДанные) Экспорт
...
Процедура Подключаемый_ПроверитьФайлыВыгрузки()
	...
	Для каждого Счет Из Объект.БанковскиеСчета Цикл
		Если Счет.Выгружен И Счет.ОбменЧерезФайл И ЗначениеЗаполнено(Счет.НастройкаОбмена) Тогда
			...
			ПроверитьФайлНаКлиенте(Счет, ОписаниеОповещения);
		КонецЕсли;
	КонецЦикла;
