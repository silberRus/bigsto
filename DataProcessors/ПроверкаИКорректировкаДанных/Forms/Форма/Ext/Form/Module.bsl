...
Процедура ПрочитатьРезультатыПоследнихПроверок()
	...
	Оповещение = Новый ОписаниеОповещения("ПоискФайловРезультовЗавершение", ЭтотОбъект);
	НачатьПоискФайлов(Оповещение, Объект.КаталогСохраненияРезультата,  "*", Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ПоискФайловРезультовЗавершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
...
Процедура СохранитьРезультатВКаталог()
	...
	// Дейтвия при сохранении результатов в каталог:
	// 1. Создать каталоги.
...
Процедура СоздатьКаталоги(ИмяКаталога, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.СоздатьКаталоги.Количество() = 0 Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПоискСтарыхФайловРезультовЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ПоискСтарыхФайловРезультовЗавершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
...
Процедура ЗаписатьРезультатВКаталог() 
	...
	Для Каждого Проверка Из Объект.Проверки Цикл
		...
		ЗаписьXML.ОткрытьФайл(ИмяКаталогаРезультатаПроверки(Проверка) + "Результат.xml");
