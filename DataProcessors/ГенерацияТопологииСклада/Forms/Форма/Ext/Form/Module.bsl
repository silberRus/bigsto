...
Процедура ПриОткрытии(Отказ)
	...
	Для каждого СтрМас из МассивЧастейАдреса Цикл
		...
		Объект["НаборБукв"+СтрМас] = НСтр("ru='РУС'");
		Объект["ТипНомера"+СтрМас] = НСтр("ru='Числовой'");
...
Процедура ПредварительныйПросмотр(Команда)
	...
	Если ТабДок <> Неопределено Тогда
		ТабДок.Показать(НСтр("ru = 'Предварительный просмотр структуры склада'"));
	КонецЕсли;
КонецПроцедуры
...
Функция  СгенерироватьСтруктуруСклада(ВТабличныйДокумент = Истина)
	...
	Для Каждого СтрСекций из МассивСекций Цикл
		...
		Если Объект["Секции"]
			...
			Адрес = АдресПоНомерам(СтрСекций, Неопределено);
...
Функция НомераЗаполненыВерно()
	...
	Для Каждого ИмяУровня из МассивЧастейАдреса Цикл
		Если Объект[ИмяУровня] Тогда
			...
			ИмяУровняЛокализованный = Элементы[ИмяУровня].Заголовок;
			
			Если Объект["ТипНомера" + ИмяУровня] <> НСтр("ru='Числовой'") Тогда
