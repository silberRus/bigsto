...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Справочник.ХарактеристикиНоменклатуры.Форма.ФормаВыбора" Тогда
		...
		ДобавитьПоддержаниеЗапасаХарактеристикНаСервере(ТекущаяНоменклатура, ВыбранноеЗначение, Объект.Склад, ТекущаяГруппировка);
		Элементы.СписокПоддерживаемыхЗапасов.Обновить();
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Обработка.ЗагрузкаДанныхИзВнешнихФайлов.Форма.Форма" Тогда
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение.АдресТоваровВХранилище) Тогда
			ДобавитьПоддержанниеЗапасаТоваровНаСервере(ВыбранноеЗначение.АдресТоваровВХранилище, Объект.Склад, ТекущаяГруппировка);
			Элементы.СписокПоддерживаемыхЗапасов.Обновить();
		КонецЕсли;
...
Процедура СписокПоддерживаемыхЗапасовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.СписокПоддерживаемыхЗапасовНоменклатура
		...
		Если Элемент.ТекущиеДанные.ЭтоГруппа Тогда
			ТоварнаяГруппировка = Элемент.ТекущиеДанные.Группировка;
	...
	ИначеЕсли Элемент.ТекущиеДанные.ЭтоГруппа ИЛИ НЕ ЗначениеЗаполнено(Элемент.ТекущиеДанные.Группировка) Тогда
...
Процедура ДобавитьТоварыПоОтбору(Команда)
	...
	ОткрытьФорму("Обработка.ПодборТоваровПоОтбору.Форма.Форма", ПараметрыФормы, ЭтаФорма,,,,
		Новый ОписаниеОповещения("ДобавитьТоварыПоОтборуЗавершение", ЭтотОбъект),
...
Процедура ДобавитьХарактеристики(Команда)
	...
	Иначе
		ТекущаяНоменклатура = Элементы.СписокПоддерживаемыхЗапасов.ТекущиеДанные.Номенклатура;
		ПараметрыФормы = Новый Структура;
...
Процедура ПодобратьТовары(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗакрыватьПриВыборе", Ложь);
	ПараметрыФормы.Вставить("Отбор", Новый Структура("ТипНоменклатуры",
		Новый ФиксированныйМассив(НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТаре(Ложь))));
...
Процедура РежимПросмотраИерархия(Команда)
	...
	Если Элементы.СписокПоддерживаемыхЗапасов.ТекущиеДанные <> Неопределено
		...
		ТекущаяГруппировка = Элементы.СписокПоддерживаемыхЗапасов.ТекущиеДанные.Группировка;
	КонецЕсли;
...
Процедура НастроитьСписокПоддерживаемыхЗапасов(Команда)
	...
	ОткрытьФорму("Обработка.НастройкаПоддержанияЗапасов.Форма.ФормаНастройкаСписка", ПараметрыФормыНастройки(),
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент = ДобавитьЭлементОформленияСУсловием(УсловноеОформлениеСписка, ОформляемыеПоля,
		"Группировка",, ВидСравненияКомпоновкиДанных.Заполнено);
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	...
	Элемент = ДобавитьЭлементОформленияСУсловием(УсловноеОформлениеСписка, "Группировка",
		"ЭтоГруппа", Ложь);
	Элемент.Оформление.УстановитьЗначениеПараметра("Отображать",	Ложь);
...
Процедура ДобавитьТоварыПоОтборуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		ДобавитьПоддержанниеЗапасаТоваровНаСервере(Результат, Объект.Склад, ТекущаяГруппировка);
	КонецЕсли;
	Элементы.СписокПоддерживаемыхЗапасов.Обновить();
...
Процедура НастроитьФормуПоПараметрам()
	...
	ИначеЕсли Источник = "ЦеховыеКладовые" Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ПолучитьОбъектКомпоновкиПоТипу(Тип("ОтборКомпоновкиДанных")),
...
Процедура УстановитьДоступностьКнопокДобавления(Форма, Доступно)
	...
	Форма.Элементы.СписокПоддерживаемыхЗапасовДобавитьХарактеристики.Доступность				= Доступно;
	Форма.Элементы.СписокПоддерживаемыхЗапасовКонтекстноеМенюДобавитьХарактеристики.Доступность	= Доступно;
...
Процедура ПереместитьВГруппуНаСервере(МассивЗаписей, Группировка, Склад)
	...
	Попытка
		...
		Для каждого КлючЗаписиРегистра Из МассивЗаписей Цикл
			...
			Если КлючЗаписиРегистра.Группировка = КлючГруппировки.Группировка Тогда
				Продолжить;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(КлючЗаписиРегистра.Номенклатура) Тогда
				КлючЗаписиГруппировки = РегистрыСведений.ТоварныеОграничения.КлючЗаписиГруппировки(КлючЗаписиРегистра.Группировка);
				ГруппировкиНаУдаление.Добавить(КлючЗаписиРегистра.Группировка);
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(КлючиЗаписей,
					РегистрыСведений.ТоварныеОграничения.КлючиЗаписейТовараВГруппировке(КлючЗаписиРегистра.Группировка, Склад));
				Продолжить;
...
&НаСервере
Процедура СинхронизироватьНастройкиКомпановщиков()
...
Функция ПараметрыФормыНастройки()
	...
	СинхронизироватьНастройкиКомпановщиков();
...
Функция ТекущаяГруппаПуста(ТекущаяГруппировка, ОтборПоСкладу)
	...
	Если ТекущаяГруппировка <> Неопределено Тогда
		...
		Запрос.УстановитьПараметр("Группировка", ТекущаяГруппировка);
...
Процедура ОткрытьФормуРедактированияПараметров(Данные)
	...
	Если Данные.ЭтоГруппа Тогда
		...
		КлючПотребности.Вставить("Группировка", Данные.Группировка);
	Иначе
...
Процедура УстановитьРежимРекомендацийИПросмотра();
	...
	ИначеЕсли РежимПросмотра = 1 Тогда
		Если ЗначениеЗаполнено(Объект.Склад) Тогда
			НовыйЗапрос = СтрЗаменить(НовыйЗапрос, "&РежимПросмотра",
			"(ТоварныеОграниченияПереопределяемый.Группировка = ЗНАЧЕНИЕ(Справочник.ГруппировкиТоварныхОграничений.ПустаяСсылка)
	...
	Иначе
		Если ЗначениеЗаполнено(Объект.Склад) Тогда
			НовыйЗапрос = СтрЗаменить(НовыйЗапрос, "&РежимПросмотра",
			"ТоварныеОграниченияПереопределяемый.Группировка = &Группировка
			|	И ТоварныеОграниченияПереопределяемый.Склад = &Склад");
		Иначе
			НовыйЗапрос = СтрЗаменить(НовыйЗапрос, "&РежимПросмотра",
			"ТоварныеОграниченияПереопределяемый.Группировка = &Группировка
	...
	СписокПоддерживаемыхЗапасов.ТекстЗапроса = НовыйЗапрос;
