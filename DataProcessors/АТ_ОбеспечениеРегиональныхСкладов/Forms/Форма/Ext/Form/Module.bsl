...
Процедура ВыполнитьРасчетМинМаксНаСервере()
	...
	СписокСкладов.Добавить(НастройкаСклада.Склад);
	СписокСкладов.Добавить(НастройкаСклада.ИсточникОбеспечения);
	...
	// 04 пакет - ВТ ОстаткиНаКаждыйДень - остатки на каждый день по методу Гилева http://www.gilev.ru/остатки-на-каждый-день/ , остатки уитываются как на складе для которого берется расчет, так и на источнике его обеспечения
	...
	// 06 пакет - ВТ Даты - даты документов поступления (ордеров, перемещений) без повторов для определения периодов
	// 07 пакет - ВТ Периоды - используя даты перемещений, расчет периодов пежду перемещениями, одновременно происходит нумерация периодов с конца
	// 08 пакет - ВТ ПериодыНоменклатура - перекрестное соединение периодов с номенклатуров, для последующего определяния нулевых продаж по периодам
	// 09 пакет - ВТ ПродажиПоПериодам - аналогично 5 пакету, продажи номенклатуры, но уже по периодам
	// 10 пакет - ВТ СредняяПродажаПоПериодам - аналогично 6 пакету, расчет показателя средней продажи по периодам
	// 11 пакет - ВТ СредняяПродажаМаксимумБезКлиентов - определяем максимум из значений средней продажи периодов
	// 12 пакет - ВТ СредняяПродажаМаксимум - добавляем к таблице значение количества клиентов которым реализованы товары в максимальный период
	// 13 пакет - ВТ ПродажиПоПериодамАнализ - продажи номенклатуры за анализируемые периоды, с выводом нулевых продаж
	// 14 пакет - ВТ ПериодыПодрядВНоль - анализируем периоды, вычисляем номенклатуру где были продажи в ноль подряд
	// 15 пакет - основной расчет минимумов и максимумов. Округление и соответствие кратности вынесено во вложенный запрос.
	...
	|ИЗ
	|	Дни КАК Дни,
	|	РегистрНакопления.ТоварыНаСкладах.ОстаткиИОбороты(&ДатаНачала, , День, , Склад В(&СписокСкладов)) КАК ТоварыНаСкладахОстаткиИОбороты
	...
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ЕСТЬNULL(СреднедневноеПотребление.Значение, 0) < &СредняяПродажаГраница1
	|						ТОГДА 0
	|					КОГДА ЕСТЬNULL(СреднедневноеПотребление.Значение, 0) > &СредняяПродажаГраница2
	|							И СправочникНоменклатура.Ликвидность = ЗНАЧЕНИЕ(Справочник.ВидыЛиквидностиНоменклатуры.AКласс)
	|						ТОГДА СреднедневноеПотребление.Значение * &ОбеспечиваемыйПериод * &К1А
	|					КОГДА ЕСТЬNULL(СреднедневноеПотребление.Значение, 0) > &СредняяПродажаГраница2
	|						ТОГДА СреднедневноеПотребление.Значение * &ОбеспечиваемыйПериод * &К1Прочее
	...
	|	ИЗ
	|		ОбеспечениеСрезПоследних КАК ОбеспечениеСрезПоследних
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АТ_СреднедневноеПотребление КАК СреднедневноеПотребление
	|			ПО ОбеспечениеСрезПоследних.Номенклатура = СреднедневноеПотребление.Номенклатура
	|			И СреднедневноеПотребление.Склад = &Склад
...
Процедура ЗаполнитьЗаказНаПеремещение(ДокументОбъект, МинМакс, СкладДляПеремещения)
	...
	ДокументОбъект.Комментарий = "Плановое перемещение. Обработка ""Обеспечение региональных складов.""";
	...
	|	ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВНаличииОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеСоСкладаОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеПодЗаказОстаток, 0) - ЕСТЬNULL(СреднедневноеПотребление.Значение, 0) * &МинимальныйЗапасИсточник КАК Доступно,
	|	ВЫБОР
	|		КОГДА МинМакс.Макс - ЕСТЬNULL(ТоварыОрганизаций.КоличествоОстаток, 0) + ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток, 0) + ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток, 0) <= ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВНаличииОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеСоСкладаОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеПодЗаказОстаток, 0) - (ВЫРАЗИТЬ(ЕСТЬNULL(СреднедневноеПотребление.Значение, 0) * &МинимальныйЗапасИсточник КАК ЧИСЛО(10, 0)))
	...
	|		ИНАЧЕ ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВНаличииОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеСоСкладаОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеПодЗаказОстаток, 0) - (ВЫРАЗИТЬ(ЕСТЬNULL(СреднедневноеПотребление.Значение, 0) * &МинимальныйЗапасИсточник КАК ЧИСЛО(10, 0)))
	|	КОНЕЦ КАК Отгрузить,
	|	ВЫБОР
	|		КОГДА МинМакс.Макс - ЕСТЬNULL(ТоварыОрганизаций.КоличествоОстаток, 0) + ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток, 0) + ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток, 0) <= ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВНаличииОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеСоСкладаОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеПодЗаказОстаток, 0) - (ВЫРАЗИТЬ(ЕСТЬNULL(СреднедневноеПотребление.Значение, 0) * &МинимальныйЗапасИсточник КАК ЧИСЛО(10, 0)))
	|			ТОГДА 0
	|		КОГДА ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВНаличииОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеСоСкладаОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстаткиИсточник.ВРезервеПодЗаказОстаток, 0) - (ВЫРАЗИТЬ(ЕСТЬNULL(СреднедневноеПотребление.Значение, 0) * &МинимальныйЗапасИсточник КАК ЧИСЛО(10, 0))) > 0
	...
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ТоварыОрганизацийОстатки.АналитикаУчетаНоменклатуры.Номенклатура) КАК ТоварыОрганизаций
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АТ_СреднедневноеПотребление КАК СреднедневноеПотребление
	|		ПО МинМакс.Номенклатура = СреднедневноеПотребление.Номенклатура
	|			И (СреднедневноеПотребление.Склад = &СкладИсточник)
	|ГДЕ
