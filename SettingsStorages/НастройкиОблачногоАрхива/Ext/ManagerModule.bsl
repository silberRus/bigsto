...
Процедура ОбработкаЗагрузки(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если (КлючОбъекта = "ПараметрыАвторизацииИПП") Тогда
		...
		ОбработкаЗагрузкиЛокальная(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ);
	Иначе
...
#Область АдресаВебСервисов
		...
		//
		РезультатПоУмолчанию = Новый Структура;
		РезультатПоУмолчанию.Вставить("АдресWSDLАвторизации1С", ОблачныйАрхивКлиентСервер.ПолучитьАдресWSDLАвторизации1С());
		РезультатПоУмолчанию.Вставить("АдресWSDLБэкап1СПубличный", ОблачныйАрхивКлиентСервер.ПолучитьАдресWSDLБэкап1СПубличный());
		РезультатПоУмолчанию.Вставить("АдресWSDLБэкап1СПриватный", ОблачныйАрхивКлиентСервер.ПолучитьАдресWSDLБэкап1СПриватный());
		РезультатПоУмолчанию.Вставить("АдресWSDLАгентаКопирования", ОблачныйАрхивКлиентСервер.ПолучитьАдресRESTAPIВерсийАгентаКопирования());
		Результат = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(РезультатПоУмолчанию);
...
#Область ПараметрыОкруженияСервер
		...
		РезультатПоУмолчанию = Новый Структура;
		РезультатПоУмолчанию.Вставить("ПодсистемаНастроена", 0);
		РезультатПоУмолчанию.Вставить("ПредыдущееСостояниеПодсистемаНастроена", 0);
		РезультатПоУмолчанию.Вставить("ДатаИзмененияПодсистемаНастроена", '00010101');
		РезультатПоУмолчанию.Вставить("ВерсияПодсистемы", ИнтернетПоддержкаПользователейКлиентСервер.ВнутреннееПредставлениеНомераВерсии(ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()));
		РезультатПоУмолчанию.Вставить("ИмяИБ", ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(СокрЛП(Метаданные.Синоним),""));
		РезультатПоУмолчанию.Вставить("ИдентификаторИБ", лкИдентификаторИБ);
		РезультатПоУмолчанию.Вставить("ИдентификаторИБ_СтрокаСоединения", лкСтрокаСоединения);
		РезультатПоУмолчанию.Вставить("ИдентификаторИБ_ИмяКомпьютера", лкИмяКомпьютера);
		РезультатПоУмолчанию.Вставить("ИнформационнаяБазаФайловая", ОбщегоНазначения.ИнформационнаяБазаФайловая());
		Результат = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(РезультатПоУмолчанию);
...
#Область НастройкиАгентаКопированияКлиент
		...
		РезультатПоУмолчанию = Новый Структура;
		РезультатПоУмолчанию.Вставить("ДатаПроверки", '00010101'); // ДатаПроверки - оставить пустой, чтобы данные обновились, если их нет.
		РезультатПоУмолчанию.Вставить("ИмяКомпьютера", КлючНастроек);
		РезультатПоУмолчанию.Вставить("РасписаниеАвтоматическогоРезервногоКопирования", ПустоеРасписание);
		РезультатПоУмолчанию.Вставить("КаталогСтатистики", "%TEMP%\Acronis\Statistics");
		РезультатПоУмолчанию.Вставить("КаталогРабочий", "%TEMP%\Acronis\WorkingDirectory");
		РезультатПоУмолчанию.Вставить("КаталогЛогов", "%TEMP%\Acronis\Logs");
...
#Область ИнформацияОКлиенте
		...
		РезультатПоУмолчанию = Новый Структура;
		РезультатПоУмолчанию.Вставить("ДатаПроверки", '00010101'); // ДатаПроверки - оставить пустой, чтобы данные обновились, если их нет.
		РезультатПоУмолчанию.Вставить("ИдентификаторКомпьютера", "");
		РезультатПоУмолчанию.Вставить("ТипОС", СокрЛП(лкСистемнаяИнформация.ТипПлатформы));
		РезультатПоУмолчанию.Вставить("ВерсияОС", лкСистемнаяИнформация.ВерсияОС);
		РезультатПоУмолчанию.Вставить("ИмяКомпьютера", КлючНастроек);
		РезультатПоУмолчанию.Вставить("ТипПлатформы", СокрЛП(лкСистемнаяИнформация.ТипПлатформы));
		РезультатПоУмолчанию.Вставить("ВерсияПлатформы", ИнтернетПоддержкаПользователейКлиентСервер.ВнутреннееПредставлениеНомераВерсии(лкСистемнаяИнформация.ВерсияПриложения));
		РезультатПоУмолчанию.Вставить("КаталогУстановкиАгентаКопирования", "c:\Program Files (x86)\Acronis\BackupAgent\");
		РезультатПоУмолчанию.Вставить("ВерсияУстановленногоАгентаКопирования", "00.00.000.0000");
		РезультатПоУмолчанию.Вставить("АгентКопированияУстановлен", Ложь); // АгентКопированияУстановлен (рассчитывается по совокупности нескольких факторов);
		РезультатПоУмолчанию.Вставить("АгентКопированияАктивирован_ЕстьФайлыСертификатов", Ложь);
		РезультатПоУмолчанию.Вставить("ЭтоАдминистраторWindows", ЭтоАдминистраторWindows);
		РезультатПоУмолчанию.Вставить("ЛогинАдминистратораОС", "");
...
#Область АктивацииАгентовКопирования
		...
		РезультатПоУмолчанию.Вставить("ИдентификаторКомпьютера", "");
		РезультатПоУмолчанию.Вставить("ИмяКомпьютера", КлючНастроек);
		РезультатПоУмолчанию.Вставить("АгентКопированияАктивированНаДругойЛогин", Ложь);
		РезультатПоУмолчанию.Вставить("АгентКопированияАктивирован_ДанныеВебСервисов", Ложь);
		РезультатПоУмолчанию.Вставить("СрокОкончанияПодписки", '00010101');
...
#Область РасписаниеАвтоматическогоРезервногоКопирования
		...
		//    *** НомерНедели (Число 1..5) (first, second, third, fourth, last);
		//    *** ДниНедели (Массив (числа 1..7)).
		Расписание_Ежедневно_ОдинРазВДень = Новый Структура("Время",
			'00010101180000');
		Расписание_Ежедневно_НесколькоРазВДень = Новый Структура("ВремяНачала,ВремяОкончания,КоличествоЧасовПовтора",
			'00010101000000',
			'00010101235959',
			6);
		Расписание_Еженедельно = Новый Структура("Время, ДниНедели", 
			'00010101180000',
			Новый Массив());
		Расписание_Ежемесячно_ПоДням = Новый Структура("Время, ДниМесяца",
			'00010101180000',
			Новый Массив());
		Расписание_Ежемесячно_ПоДнямНедели = Новый Структура("Время, НомерНедели, ДниНедели",
			'00010101180000',
			1,
			Новый Массив());

		РезультатПоУмолчанию = Новый Структура;
		РезультатПоУмолчанию.Вставить("РасписаниеВключено", Ложь); // По-умолчанию, расписание не настроено.
		РезультатПоУмолчанию.Вставить("Вариант", "Ежедневно_ОдинРазВДень");
		РезультатПоУмолчанию.Вставить("Ежедневно_ОдинРазВДень", Расписание_Ежедневно_ОдинРазВДень);
		РезультатПоУмолчанию.Вставить("Ежедневно_НесколькоРазВДень", Расписание_Ежедневно_НесколькоРазВДень);
		РезультатПоУмолчанию.Вставить("Еженедельно", Расписание_Еженедельно);
		РезультатПоУмолчанию.Вставить("Ежемесячно_ПоДням", Расписание_Ежемесячно_ПоДням);
		РезультатПоУмолчанию.Вставить("Ежемесячно_ПоДнямНедели", Расписание_Ежемесячно_ПоДнямНедели);
		...
		Результат = Новый Структура("РасписаниеВключено, Вариант, Ежедневно_ОдинРазВДень",
			...
			"Ежедневно_ОдинРазВДень",
			Расписание_Ежедневно_ОдинРазВДень);
...
Процедура ОбработкаЗагрузкиЛокальная(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
