...
Функция ПолучитьЗаписиЖурналаСчетовФактур(СтруктураПараметров) Экспорт
	...
	|	ЖурналУчетаСчетовФактур.СуммаПоСчетуФактуреКомиссия КАК СуммаДокументаКомиссия,
	|	ЖурналУчетаСчетовФактур.СуммаПоСчетуФактуреРазницаУменьшениеКомиссия КАК СуммаДокументаРазницаУменьшениеКомиссия,
	|	ЖурналУчетаСчетовФактур.СуммаПоСчетуФактуреРазницаУвеличениеКомиссия КАК СуммаДокументаРазницаУвеличениеКомиссия
	...
	|		ИНАЧЕ ЖурналУчетаСчетовФактур.КППКонтрагента
	|	КОНЕЦ,
	|	ЖурналУчетаСчетовФактур.СуммаПоСчетуФактуреКомиссия,
	|	ЖурналУчетаСчетовФактур.СуммаПоСчетуФактуреРазницаУменьшениеКомиссия,
	|	ЖурналУчетаСчетовФактур.СуммаПоСчетуФактуреРазницаУвеличениеКомиссия
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЧастьЖурнала
	|;
...
Функция ТекстЗапросаЖурналСчетовФактур981()
	...
	// 2. ВТ_ЖурналУчетаСчетовФактур  - итоговая таблица для формирования отчета Журнала учета счетов-фактур.
	...
	|		КОГДА НЕ ДанныеСФОтПродавца.Документ ЕСТЬ NULL
	...
	|		ПО ЖурналУчетаСчетовФактурСрезПоследних.СчетФактураПолученныйОтПродавца = ДанныеСФОтПродавца.Документ
	|			И ЖурналУчетаСчетовФактурСрезПоследних.Организация = ДанныеСФОтПродавца.Организация
	|			И (ЖурналУчетаСчетовФактурСрезПоследних.ДатаВыставленияПолучения МЕЖДУ &НачПериода И &КонПериода)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СводныеКомиссионныеСчетаФактуры КАК ВТ_СводныеКомиссионныеСчетаФактуры
	|		ПО ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура = ВТ_СводныеКомиссионныеСчетаФактуры.СчетФактура
	|			И ЖурналУчетаСчетовФактурСрезПоследних.Организация = ВТ_СводныеКомиссионныеСчетаФактуры.Организация
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_КорректировочныеСчетаФактуры КАК ВТ_КорректировочныеСчетаФактуры
	...
	|		ПО ЖурналУчетаСчетовФактурСрезПоследних.Организация = ДанныеПервичныхДокументов.Организация
	|			И ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура = ДанныеПервичныхДокументов.Документ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетФактура
	|;
...
Процедура ЗаполнитьСтрокуЖурналаУчетаСчетовФактур(
	...
	ТекстБезНДС = "без НДС"; // Не локализуется, используется для выгрузки формате в ФНС.
	ТекстПустаяСтрока = "";
