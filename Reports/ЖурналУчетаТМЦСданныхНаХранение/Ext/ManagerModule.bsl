...
Функция ПолучитьДанныеДляПечатнойФормыМХ2(Отчет, СписокДокументов)
	...
	|		ПО ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Склад = Склады.Ссылка
	|ГДЕ
	...
	|	ВЫРАЗИТЬ(ЕСТЬNULL(Цены.Цена, 0) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК ЧИСЛО(15, 2)) КАК Цена
