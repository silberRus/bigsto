...
Функция РезультатЗапросаПоДаннымОтчета(СтруктураФормированияОтчета)
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|  НаименованиеПартнера"  + "%УпорядочитьКИПартнера%" + "%Упорядочить%
	|ИТОГИ ПО
...
Процедура ДобавитьВЗапросДанныеКонтактныхЛиц(ТекстЗапроса, СтруктураФормированияОтчета,СоответствиеПараметровЗапросаВидовКИ)
	...
	Для каждого СтрокаРоли Из СтруктураФормированияОтчета.РолиКЛ Цикл
		...
		ТекстУсловияПоРолям = ТекстУсловияПоРолям + "
				|		КонтактныеЛицаПартнеровРолиКонтактногоЛица.РольКонтактногоЛица " + СтрокаПараметрЗапросаРоль + " ИЛИ ";
				
	КонецЦикла;
	
	СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(ТекстУсловияПоРолям,5);
	ТекстУсловияПоРолям = ТекстУсловияПоРолям + ")";
	...
	Для каждого СтрокаКИ Из СтруктураФормированияОтчета.ВыбраннаяКИКЛ Цикл
		...
		Если СтрокаКИ.Пометка Тогда
			...
			|	 КонтактныеЛицаПартнеровКонтактнаяИнформация.Вид = &" + СтрокаПараметрЗапроса + " ИЛИ ";
			
		КонецЕсли;
		
	КонецЦикла;
	
	СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(ТекстУсловияПоВидамКИ,5);
	ТекстУсловияПоВидамКИ = ТекстУсловияПоВидамКИ + ")";
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"%Упорядочить%",ТекстЗапросаДопУпорядочивание);
	...
	|		ПО (КонтактныеЛицаПартнеровКонтактнаяИнформация.Ссылка = КонтактныеЛицаПартнеров.Ссылка) И " + ТекстУсловияПоВидамКИ + "
	|ГДЕ " + ТекстУсловияПоРолям + "
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|";
...
Процедура ДобавитьВЗапросДанныеКИПартнера(ТекстЗапроса, СтруктураФормированияОтчета, СоответствиеПараметровЗапросаВидовКИ)
	...
	Если НЕ СтруктураФормированияОтчета.ГруппироватьПоПартнеру Тогда
		...
		Для каждого Строка Из СтруктураФормированияОтчета.ВыбранныеПоляПартнера Цикл
			...
			ТекстСоединенияКИПартнера = ТекстСоединенияКИПартнера + "ПартнерыКонтактнаяИнформация.Вид = &" + СтрокаПараметрЗапроса + " ИЛИ ";
			
		КонецЦикла;
		
		Если НЕ ПустаяСтрока(ТекстСоединенияКИПартнера) Тогда
			
			СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(ТекстСоединенияКИПартнера,5);
			ТекстСоединенияКИПартнера = "
