...
Процедура ЗаполнитьДеревоСобытий()
	...
	Если ЕстьПравоКоммерческиеПредложения Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "КоммерческиеПредложения", НСтр("ru = 'Коммерческие предложения'"));
	КонецЕсли;
	...
	Если ЕстьПравоВнутреннееПотребление И ИспользоватьВнутреннееПотребление Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "ВнутреннееПотребление", НСтр("ru = 'Внутреннее потребление товаров'"));
	КонецЕсли;
	УдалитьРодительскийУзелДереваЕслиПустой(УзелДерева);
