...
Процедура НастроитьПараметрыОтчетаПоВариантуОтчета(НастройкиОтчета, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД)
	...
	Если ПредопределенныйВариант.КлючВарианта = "ДоходыИРасходыПоПредприятию" Тогда
		
		СписокВыбора.Добавить(1, НСтр("ru = 'В валюте упр. учета с НДС'"));
		ПараметрПоПредприятию.Значение = Истина;
		
	ИначеЕсли ПредопределенныйВариант.КлючВарианта = "ДоходыИРасходыПоОрганизациям" Тогда
	...
	ИначеЕсли ПредопределенныйВариант.КлючВарианта = "КонтрольРаспределенияДоходовИРасходов" Тогда
		
		СписокВыбора.Добавить(1, НСтр("ru = 'В валюте упр. учета с НДС'"));
		Если ВестиУправленческийУчетОрганизаций Тогда
			СписокВыбора.Добавить(3, НСтр("ru = 'В валюте упр. учета без НДС'"));
		КонецЕсли;
		Если ИспользоватьУчетПрочихДоходовРасходовРегл Тогда
			СписокВыбора.Добавить(4, НСтр("ru = 'В валюте регл. учета'"));
		КонецЕсли;
	...
	//АТ Терентьев
	Если ЗначениеПараметраДанныеОтчета = Неопределено Тогда
		Возврат;
	КонецЕсли;
	//АТ-
...
Функция ПолучитьПредопределенныйВариант(Знач Вариант)
	...
	КлючиВариантов.Добавить("КонтрольРаспределенияДоходовИРасходов");
...
Функция СтруктураЗаголовковПолей()
	...
	Если ПараметрДанныеОтчета.Значение = 4 Тогда
