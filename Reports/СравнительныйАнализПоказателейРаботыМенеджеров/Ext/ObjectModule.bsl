...
Функция ТекстЗапросаОПоступленииДС(СоответствиеЗапросыДанные,НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		|ИЗ
		|	(ВЫБРАТЬ
		|		РасчетыСКлиентамиПоДокументам.РасчетныйДокумент.Менеджер КАК Менеджер,
		...
		|		И НЕ РасчетыСКлиентамиПоДокументам.РасчетныйДокумент.Менеджер ЕСТЬ NULL 
		|		И РасчетыСКлиентамиПоДокументам.РасчетныйДокумент.Менеджер В
		|				(ВЫБРАТЬ
		...
		|				ИЛИ РасчетыСКлиентамиПоДокументам.Регистратор ССЫЛКА Документ.СписаниеЗадолженности)
		|		И РасчетыСКлиентамиПоДокументам.Период МЕЖДУ &НачалоПериода И &ОкончаниеПериода
		|		И РасчетыСКлиентамиПоДокументам.АналитикаУчетаПоПартнерам.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.РозничныйПокупатель)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		РасчетыСКлиентамиПоДокументам.Регистратор.Менеджер,
		...
		|		И НЕ РасчетыСКлиентамиПоДокументам.Регистратор.Менеджер ЕСТЬ NULL 
		|		И РасчетыСКлиентамиПоДокументам.Регистратор.Менеджер В
		|				(ВЫБРАТЬ
		|					ОтборПоМенеджерам.Ссылка
		|				ИЗ
		|					ОтборПоМенеджерам КАК ОтборПоМенеджерам)
		...
		|			РегистрНакопления.РасчетыСКлиентамиПоДокументам КАК РасчетыСКлиентамиПоДокументам
		|		ГДЕ
		|			НЕ РасчетыСКлиентамиПоДокументам.ЗаказКлиента.Менеджер ЕСТЬ NULL
...
Функция ТекстЗапросаОПредоставленныхРучныхСкидках(СоответствиеЗапросыДанные,НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Иначе
		...
		|							ОтборПоМенеджерам.Ссылка
		|						ИЗ
		|							ОтборПоМенеджерам КАК ОтборПоМенеджерам) И АналитикаУчетаПоПартнерам.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.РозничныйПокупатель)) КАК ВыручкаИСебестоимостьПродажОбороты,
		|			Константы КАК Константы
		|		
		|		СГРУППИРОВАТЬ ПО
		|			ВыручкаИСебестоимостьПродажОбороты.ЗаказКлиента.Менеджер,
...
Процедура ВывестиТаблицу(ТаблицаДанных,ТаблицаОтчета,ПараметрыФормирования,ИмяНабора, СтруктураПериодовФормированияОтчета, Макет)
	...
	Для инд = 0 По ТаблицаПодготовленныеДанные.Колонки.Количество() - 1 Цикл
		...
		Область.Параметры.ИмяПоказателя = ТаблицаПодготовленныеДанные.Колонки[инд].Заголовок;
		ТаблицаОтчета.Присоединить(Область);
...
Функция ТаблицаДанныхДляТабличногоВывода(ИмяНабора,ТаблицаДанных,ЕстьДополнительныйПериод)
	...
	Если НЕ ЕстьДополнительныйПериод Тогда
		...
		ИначеЕсли ИмяНабора = "ПредоставленныеРучныеСкидки" Тогда
			...
			Для Инд = 1 По ТаблицаКВозврату.Колонки.Количество() - 1 Цикл
				ТаблицаКВозврату.Колонки[инд].Заголовок = ЗаголовокПоказателя(ТаблицаКВозврату.Колонки[инд].Имя);
			КонецЦикла;
		...
		ИначеЕсли ИмяНабора = "ПричиныПроигрышаСделок" ИЛИ ИмяНабора = "ОсновныеЭтапыПотерь" 
			...
			ТаблицаКВозврату.Колонки[0].Заголовок = ЗаголовокПоказателя(ИмяНабора);
	...
	Иначе
		...
		Если ИмяНабора = "Прибыль" ИЛИ ИмяНабора = "Продажи" 
			...
			ТаблицаКВозврату.Сортировать(ТаблицаДанных.Колонки[1].Имя + " DESC");
			Для каждого СтрокаТаблицы Из ТаблицаКВозврату Цикл
			...
			ТаблицаКВозврату.Колонки[1].Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 в текущем периоде'"),ЗаголовокПоказателя(ИмяНабора));
			ТаблицаКВозврату.Колонки[2].Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 в предыдущем периоде'"),ЗаголовокПоказателя(ИмяНабора));
			ТаблицаКВозврату.Колонки[3].Заголовок = НСтр("ru='Абсолютное изменение'");
			ТаблицаКВозврату.Колонки[4].Заголовок = НСтр("ru='Относительное изменение(%)'");
			
		ИначеЕсли ИмяНабора = "ПредоставленныеРучныеСкидки" Тогда
			
			ТаблицаКВозврату.Сортировать(ТаблицаДанных.Колонки[3].Имя + " DESC," + ТаблицаДанных.Колонки[1].Имя + " DESC");
			Для Инд = 1 По ТаблицаКВозврату.Колонки.Количество() - 1 Цикл
				ТаблицаКВозврату.Колонки[инд].Заголовок = ЗаголовокПоказателя(ТаблицаКВозврату.Колонки[инд].Имя);
			КонецЦикла;
		...
		ИначеЕсли ИмяНабора = "ЭффективностьВеденияСделок" Тогда
			...
			ТаблицаКВозврату.Сортировать(ТаблицаДанных.Колонки[3].Имя + " DESC," + ТаблицаДанных.Колонки[1].Имя + " DESC,"+ ТаблицаДанных.Колонки[2].Имя + " АSC");
			Для Инд = 1 По ТаблицаКВозврату.Колонки.Количество() - 1 Цикл
				ТаблицаКВозврату.Колонки[инд].Заголовок = ЗаголовокПоказателя(ТаблицаКВозврату.Колонки[инд].Имя);
			КонецЦикла;
		...
		ИначеЕсли ИмяНабора = "ЭффективностьВзаимодействий" Тогда
			...
			ТаблицаКВозврату.Сортировать(ТаблицаДанных.Колонки[4].Имя + " DESC," + ТаблицаДанных.Колонки[1].Имя + " DESC");
			Для Инд = 1 По ТаблицаКВозврату.Колонки.Количество() - 1 Цикл
				ТаблицаКВозврату.Колонки[инд].Заголовок = ЗаголовокПоказателя(ТаблицаКВозврату.Колонки[инд].Имя);
			КонецЦикла;
		...
		ИначеЕсли ИмяНабора = "ПричиныПроигрышаСделок" ИЛИ ИмяНабора = "ОсновныеЭтапыПотерь" 
			...
			ТаблицаКВозврату.Колонки[0].Заголовок = ЗаголовокПоказателя(ИмяНабора + "ИмяПоказателяТекущий");
			ТаблицаКВозврату.Колонки[1].Заголовок = ЗаголовокПоказателя(ИмяНабора + "ЗначениеПоказателяТекущий");
			ТаблицаКВозврату.Колонки[2].Заголовок = ЗаголовокПоказателя(ИмяНабора + "ИмяПоказателяПредыдущий");
			ТаблицаКВозврату.Колонки[3].Заголовок = ЗаголовокПоказателя(ИмяНабора + "ЗначениеПоказателяПредыдущий");
...
Функция ЗаголовокПоказателя(ИмяПоказателя)
	...
	Если СтрЧислоВхождений(ЗаголовокПоказателя,"Текущий") > 0 Тогда 
		...
		ЗаголовокПоказателя = Лев(ЗаголовокПоказателя,СтрДлина(ЗаголовокПоказателя) - 7);
		ЭтоВыводЗаголовкаТекущегоПериода = Истина;
		
	ИначеЕсли СтрЧислоВхождений(ЗаголовокПоказателя,"Предыдущий") > 0 Тогда
		
		ЗаголовокПоказателя = Лев(ЗаголовокПоказателя,СтрДлина(ЗаголовокПоказателя) - 10);
	...
	ИначеЕсли ЗаголовокПоказателя = "ПричиныПроигрышаСделок" ИЛИ ЗаголовокПоказателя = "ПричиныПроигрышаСделокИмяПоказателя" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='""Менеджер - причина проигрыша""'");
		
	ИначеЕсли ЗаголовокПоказателя = "ОсновныеЭтапыПотерь" ИЛИ ЗаголовокПоказателя = "ОсновныеЭтапыПотерьИмяПоказателя" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='""Менеджер - этап сделки""'");
		
	ИначеЕсли ЗаголовокПоказателя = "ОсновныеЭтапыПотерьЗначениеПоказателя" ИЛИ ЗаголовокПоказателя = "ПричиныПроигрышаСделокЗначениеПоказателя"
		ИЛИ ЗаголовокПоказателя = "ПричиныВозникновенияПретензийЗначениеПоказателя" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Количество'");
		
	ИначеЕсли ЗаголовокПоказателя = "НеудовлетворениеПервичногоСпроса" ИЛИ ЗаголовокПоказателя = "НеудовлетворениеПервичногоСпросаИмяПоказателя" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='""Менеджер - причина неудовлетворения""'");
		
	ИначеЕсли ЗаголовокПоказателя = "СуммаПервичногоСпроса" ИЛИ ЗаголовокПоказателя = "НеудовлетворениеПервичногоСпросаЗначениеПоказателя" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Сумма неудовлетворенного спроса'");
		
	ИначеЕсли ЗаголовокПоказателя = "ПричиныВозникновенияПретензий" ИЛИ ЗаголовокПоказателя = "ПричиныВозникновенияПретензийИмяПоказателя" Тогда
...
Функция СтруктураПериодовФормированияОтчетов(ПараметрыФормирования)
	...
	ПериодФормированияОтчета = ПараметрыФормирования.ПериодФормированияОтчета;
	ДополнительныйПериодДляАнализа = ПараметрыФормирования.ДополнительныйПериодДляАнализа;
...
Процедура ВывестиШапкуНабораДанных(ТаблицаОтчета,ТекстЗаголовка,ЕстьДополнительныйПериод, Макет)
	...
	Если ЕстьДополнительныйПериод Тогда
		Область.Параметры.ТекстЗаголовка = "";
		ТаблицаОтчета.Присоединить(Область);
	КонецЕсли;
