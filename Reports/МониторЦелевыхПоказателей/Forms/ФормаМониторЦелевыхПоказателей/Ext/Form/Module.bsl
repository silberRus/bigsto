...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Параметры.ИдентификаторВариантаАнализа) Тогда
		...
		Заголовок = НСтр("ru='Анализируемый показатель'") + " """ + НаименованиеВариантаАнализа + """";
...
Процедура Печать(Команда)
	Если СоставМонитораНеопределен Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Сформировать сводный отчет невозможно, т.к. монитор показателей не настроен.'"),, НСтр("ru='Ошибка формирования сводного отчета'"));
	ИначеЕсли СоставМонитораПустойПоОтбору Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Сформировать сводный отчет невозможно, т.к. в текущий отбор не входит ни один показатель.'"),, НСтр("ru='Ошибка формирования сводного отчета'"));
	Иначе 
...
Функция СтильГруппыЭлементовПоказателя()
	...
	Стиль.Вставить("Вид",						ВидГруппыФормы.ОбычнаяГруппа);
...
Функция СтильОтображенияТекстовых()
	...
	Стиль.Вставить("Вид",						ВидПоляФормы.ПолеНадписи);
...
Функция СтильОтображенияДиаграмм()
	...
	Стиль.Вставить("Вид",						ВидПоляФормы.ПолеДиаграммы);
...
Функция СтильКомандныхПанелейПоказателей()
	...
	Стиль.Вставить("Вид",						ВидГруппыФормы.КоманднаяПанель);
...
Процедура ДобавитьГруппыКолонкиЭлементов(РодительскаяГруппа, КлючУникальностиГруппы, ВариантОтображенияПоказателейСтроки = Неопределено)
	...
	Если ВариантОтображенияПоказателейСтроки = ВариантыОтображенияВариантовАнализа.Кратко Тогда
		Для Сч = 1 По СтруктураНастроек.КоличествоКолонокМонитораПоказателей Цикл 
			ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаЭлементовКратко" + КлючУникальностиГруппы + "__" + Сч, НСтр("ru='Колонка №'") + Сч, РодительскаяГруппа);
	...
	Иначе
		Если Элементы.Найти("ГруппаКолонкаЭлементовПодробно" + КлючУникальностиГруппы) = Неопределено Тогда
			Для Сч = 1 По СтруктураНастроек.КоличествоКолонокМонитораПоказателей Цикл 
				ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаЭлементовПодробно" + КлючУникальностиГруппы + "__" + Сч, НСтр("ru='Колонка №'") + Сч, РодительскаяГруппа);
				ЗаполнитьЗначенияСвойств(ГруппаКолонкаЭлементов, СтильГруппыКолонкиПоказателей());
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьДинамическийПараметрДляТаблицы(ВариантАнализа, ПараметрыИсточникаДанных, ИдентификаторЭлементаФормы)
	...
	Иначе
		Если Не ИсточникДанныхВариантаАнализа.РасчетыАктуальны Тогда
			ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы] = МониторингЦелевыхПоказателей.ПолучитьДиаграммуПриОшибкахРасчета(ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы], НСтр("ru= 'Данные неактуальны.'"));
		ИначеЕсли ИсточникДанныхВариантаАнализа.ОшибкаРасчета Тогда
			ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы] = МониторингЦелевыхПоказателей.ПолучитьДиаграммуПриОшибкахРасчета(ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы], НСтр("ru= 'Ошибка при расчете значений показателя.'"));
		Иначе
			ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы] = МониторингЦелевыхПоказателей.ПолучитьДиаграммуПриОшибкахРасчета(ЭтаФорма["Диаграмма__" + ИдентификаторЭлементаФормы], НСтр("ru= 'Недостаточно данных для расчета показателя.'"));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура НарисоватьДиаграмму(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(
		Новый РеквизитФормы("Диаграмма__" + ИдентификаторЭлементаФормы, ОписаниеТипов, "", НСтр("ru='Диаграмма для'") + " " + ВариантАнализа));
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("НаименованиеПоказателя__" + ИдентификаторЭлементаФормы, ОписаниеТипов, "", НСтр("ru= 'Наименование показателя'")));
	
	ИзменитьРеквизиты(МассивРеквизитов);
	
	// Добавим поля формы
	ГруппаЗаголовокПоказателя = ГоризонтальнаяГруппа("Заголовок__" + ИдентификаторЭлементаФормы, НСтр("ru= 'Статусы'"), ГруппаЭлементов);
	...
	ПолеНаименования.Подсказка = ЭтаФорма["НаименованиеПоказателя__" + ИдентификаторЭлементаФормы];
...
Процедура НарисоватьТаблицу(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	Если Не ПустойИсточникДанных И Не РасчетыНеактуальны Тогда
		...
		МассивРеквизитов.Добавить(
			Новый РеквизитФормы("ТаблицаЗначений__" + ИдентификаторЭлементаФормы, ОписаниеТипов, "", НСтр("ru='Таблица для'") + " " + ВариантАнализа));
	Иначе
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(Тип("Диаграмма"));
		ОписаниеТипов = Новый ОписаниеТипов(МассивТипов);
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("НаименованиеПоказателя__" + ИдентификаторЭлементаФормы, ОписаниеТиповФорматированныхСтрок, "", НСтр("ru='Наименование показателя'")));
	...
	// Добавим поля формы
	ГруппаЗаголовокПоказателя = ГоризонтальнаяГруппа("Заголовок__" + ИдентификаторЭлементаФормы, НСтр("ru= 'Заголовок показателя'"), ГруппаЭлементов);
	ЗаполнитьЗначенияСвойств(ГруппаЗаголовокПоказателя, СтильГруппыЭлементовЗаголовка());
...
Процедура НарисоватьКратко(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	Если Не ПустойИсточникДанных И Не РасчетыНеактуальны Тогда
		МассивРеквизитов.Добавить(
			Новый РеквизитФормы("Значение__" + ИдентификаторЭлементаФормы, ОписаниеТиповФорматированныхСтрок, "", НСтр("ru='Значение показателя'")));
	Иначе
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(Тип("Диаграмма"));
		ОписаниеТипов = Новый ОписаниеТипов(МассивТипов);
...
Процедура НарисоватьПоляСправкиОПоказателе(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦельЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Цель'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦелевойТрендЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Целевой тренд'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦелевыеЗначенияЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Целевые значения'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическоеСостояниеЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Состояние'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическийТрендЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Тренд'")));
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ОтветственныйЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Ответственный'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФормулаЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Формула'")));
	
	ИзменитьРеквизиты(МассивРеквизитов);
	
	ПолеЗначения = Элементы.Вставить("ЦельЗначение__" + ИдентификаторЭлементаФормы, Тип("ПолеФормы"), ГруппаЭлементов);
	ПолеЗначения.ПутьКДанным = "ЦельЗначение__" + ИдентификаторЭлементаФормы;
	ЗаполнитьЗначенияСвойств(ПолеЗначения, СтильСправочнойИнформации());
	
	ПолеЗначения = Элементы.Вставить("ЦелевойТрендЗначение__" + ИдентификаторЭлементаФормы, Тип("ПолеФормы"), ГруппаЭлементов);
	ПолеЗначения.ПутьКДанным = "ЦелевойТрендЗначение__" + ИдентификаторЭлементаФормы;
	ЗаполнитьЗначенияСвойств(ПолеЗначения, СтильСправочнойИнформации());
	
	ПолеЗначения = Элементы.Вставить("ЦелевыеЗначенияЗначение__" + ИдентификаторЭлементаФормы, Тип("ПолеФормы"), ГруппаЭлементов);
	ПолеЗначения.ПутьКДанным = "ЦелевыеЗначенияЗначение__" + ИдентификаторЭлементаФормы;
	ЗаполнитьЗначенияСвойств(ПолеЗначения, СтильСправочнойИнформации());
	
	ПолеЗначения = Элементы.Вставить("ФактическоеСостояниеЗначение__" + ИдентификаторЭлементаФормы, Тип("ПолеФормы"), ГруппаЭлементов);
	ПолеЗначения.ПутьКДанным = "ФактическоеСостояниеЗначение__" + ИдентификаторЭлементаФормы;
	ЗаполнитьЗначенияСвойств(ПолеЗначения, СтильСправочнойИнформации());
	
	ПолеЗначения = Элементы.Вставить("ФактическийТрендЗначение__" + ИдентификаторЭлементаФормы, Тип("ПолеФормы"), ГруппаЭлементов);
	ПолеЗначения.ПутьКДанным = "ФактическийТрендЗначение__" + ИдентификаторЭлементаФормы;
	ЗаполнитьЗначенияСвойств(ПолеЗначения, СтильСправочнойИнформации());
	
	ПолеЗначения = Элементы.Вставить("ФактическоеЗначениеЗначение__" + ИдентификаторЭлементаФормы, Тип("ПолеФормы"), ГруппаЭлементов);
	...
	ПолеЗначения = Элементы.Вставить("ОтветственныйЗначение__" + ИдентификаторЭлементаФормы, Тип("ПолеФормы"), ГруппаЭлементов);
	ПолеЗначения.ПутьКДанным = "ОтветственныйЗначение__" + ИдентификаторЭлементаФормы;
	ЗаполнитьЗначенияСвойств(ПолеЗначения, СтильСправочнойИнформации());
	
	ПолеЗначения = Элементы.Вставить("ФормулаЗначение__" + ИдентификаторЭлементаФормы, Тип("ПолеФормы"), ГруппаЭлементов);
...
Процедура НарисоватьЭлементыМонитораСУчетомВидаАнализаВзаимосвязаных(КоллекцияВариантовАнализа, ВидПоискаВзаимосвязанных)
	...
	Для Каждого ЭлементМонитора Из КоллекцияВариантовАнализа Цикл 
		...
		ГруппаЭлемент = Элементы.Добавить("ЭлементДиаграмма__" + ИдентификаторЭлементаФормы, Тип("ГруппаФормы"), ГруппаКолонкаЭлементов);
		...
		ГруппаСправка = Элементы.Добавить("ЭлементСправкаОПоказателе__" + ИдентификаторЭлементаФормы, Тип("ГруппаФормы"), ГруппаКолонкаЭлементов);
...
Процедура НарисоватьЭлементыМонитораСУчетомСостояния(КоллекцияВариантовАнализа, Состояние)
	...
	Для Каждого ЭлементМонитора Из КоллекцияВариантовАнализа Цикл 
		...
		ГруппаКолонкаЭлементов = Элементы["ГруппаКолонкаЭлементов" + СуффиксИмениГруппы + Состояние + "__" + СчетчикЭлементовСтроки];
		
		ГруппаЭлементовПоказателя = Элементы.Добавить("Элемент__" + ИдентификаторЭлементаФормы, Тип("ГруппаФормы"), ГруппаКолонкаЭлементов);
		ЗаполнитьЗначенияСвойств(ГруппаЭлементовПоказателя, СтильГруппыЭлементовПоказателя());
		
		Если ВидПредставленияЧисел = ВидыПредставленияЧисел.БезИзменений Тогда
			ГруппаЭлементовПоказателя.Заголовок = ВариантАнализа.Наименование 
				+ ?(Не ПустаяСтрока(ДинамическиеСвойстваВариантаАнализа.Размерность)," (" + ДинамическиеСвойстваВариантаАнализа.Размерность + ")", "");
		Иначе
			ГруппаЭлементовПоказателя.Заголовок = ВариантАнализа.Наименование 
				+ " (" + МониторингЦелевыхПоказателей.ПолучитьСтрокуПредставленияЧисел(ВариантАнализа) + ?(Не ПустаяСтрока(ДинамическиеСвойстваВариантаАнализа.Размерность), " " + ДинамическиеСвойстваВариантаАнализа.Размерность, "") + ")";
		КонецЕсли;
		
		// Наполним элемент контентом по шаблонам вариантов
		НарисоватьЭлементыПоказателя(ВариантАнализа, ИсточникДанных, ВариантОтображения, ГруппаЭлементовПоказателя, ИдентификаторЭлементаФормы);
		...
		Если КоличествоНенарисованныхПоказателей = 0 И СчетчикЭлементовСтроки < СтруктураНастроек.КоличествоКолонокМонитораПоказателей Тогда
			Для СчВыравнивающихЭлементов = 1 По (СтруктураНастроек.КоличествоКолонокМонитораПоказателей - СчетчикЭлементовСтроки) Цикл 
				ГруппаКолонкаЭлементов = Элементы["ГруппаКолонкаЭлементов" + СуффиксИмениГруппы + Состояние + "__" + (СчетчикЭлементовСтроки + СчВыравнивающихЭлементов)];
				
				ГруппаЭлементовПоказателя = Элементы.Добавить("КонтейнерВыравнивающегоЭлемента__" + ИдентификаторЭлементаФормы + "__" + СчВыравнивающихЭлементов, Тип("ГруппаФормы"), ГруппаКолонкаЭлементов);
				ЗаполнитьЗначенияСвойств(ГруппаЭлементовПоказателя, СтильГруппыЭлементовПоказателя());
				
				ГруппаЭлементовПоказателя.Заголовок = НСтр("ru= 'Контейнер выравнивающего элемента'");
				
				НарисоватьПустойЭлемент(ГруппаЭлементовПоказателя, ИдентификаторЭлементаФормы + "__" + СчВыравнивающихЭлементов, СтильОтображенияПоказателяБазовый());
			КонецЦикла;
		КонецЕсли;
...
Процедура НарисоватьЭлементыМонитораСУчетомГрупп(КоллекцияВариантовАнализа, ИдентификаторГруппы)
	...
	Для Каждого ЭлементМонитора Из КоллекцияВариантовАнализа Цикл 
		...
		ГруппаКолонкаЭлементов = Элементы["ГруппаКолонкаЭлементов" + СуффиксИмениГруппы + ИдентификаторГруппы + "__" + СчетчикЭлементовСтроки];
		...
		Иначе
			ГруппаЭлементовПоказателя.Заголовок = ВариантАнализа.Владелец.КраткоеНаименованиеЦелевогоПоказателя 
				+ " (" + МониторингЦелевыхПоказателей.ПолучитьСтрокуПредставленияЧисел(ВариантАнализа) + " " + ДинамическиеСвойстваВариантаАнализа.Размерность + ")";
		КонецЕсли;
		...
		Если КоличествоНенарисованныхПоказателей = 0 И СчетчикЭлементовСтроки < СтруктураНастроек.КоличествоКолонокМонитораПоказателей Тогда
			Для СчВыравнивающихЭлементов = 1 По (СтруктураНастроек.КоличествоКолонокМонитораПоказателей - СчетчикЭлементовСтроки) Цикл 
				ГруппаКолонкаЭлементов = Элементы["ГруппаКолонкаЭлементов" + СуффиксИмениГруппы + ИдентификаторГруппы + "__" + (СчетчикЭлементовСтроки + СчВыравнивающихЭлементов)];
			...
			Для Каждого ЭлементВывода Из СтруктураВывода.ПорядокВывода Цикл
				...
				Если СтруктураВывода[ЭлементВывода].Количество() > 0 Тогда
					...
					Если СтрНайти(ЭлементВывода, "Кратко") > 0 Тогда
						ДобавитьГруппыКолонкиЭлементов(ГруппаПоказателя, Состояние, ВариантыОтображенияВариантовАнализа.Кратко);
					Иначе
		...
		ИначеЕсли СтруктураНастроек.ВариантГруппировкиПоказателей = "ПоКатегориямЦелей" Тогда
			Для Каждого ЭлементВывода Из СтруктураВывода.ПорядокВывода Цикл
				...
				Если СтруктураВывода[ЭлементВывода].Количество() > 0 Тогда
					...
					Если СтрНайти(ЭлементВывода, "Кратко") > 0 Тогда
						ДобавитьГруппыКолонкиЭлементов(ГруппаТекстовыеЗаголовок, Идентификатор, ВариантыОтображенияВариантовАнализа.Кратко);
					Иначе
