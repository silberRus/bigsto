...
Процедура ЗаполнитьПараметрыИВывестиОбласть(СтруктураОбластей, Выборка, СтруктураРасшифровки, СоответствиеПоказателей)
	...
	Область = СтруктураОбластей.ОбластьУровень;
	ОбластьПоказатели = СтруктураОбластей.ОбластьУровень_Показатели;
	...
	Если НЕ ПустаяСтрока(ИмяГруппировки) Тогда
		...
		Иначе
			Область.Параметры.ПредставлениеГруппировки=Выборка["Наименование"+ИмяГруппировки];
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьУсловияИПараметры(Запрос, ТекстЗапроса, ЭлементыОтбора, ОтборПоИсполнению)
	...
	СформироватьТекстУсловия(Запрос,
							 ...
							 УсловиеГруппы,
							 СоответствиеПолейУсловия);
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"%УсловиеМагазиныКвотыИАссортимент%", СокрЛП(ТекстУсловия));
	...
	СформироватьТекстУсловия(Запрос,
							 ...
							 УсловиеГруппы,
							 СоответствиеПолейУсловия);
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"%УсловиеОстатки%", СокрЛП(ТекстУсловия));
	...
	Если НЕ ПустаяСтрока(ОтборПоИсполнению) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"%УсловиеИмеющие%", СокрЛП(ТекстУсловияИмеющие));
