...
Функция ПолучитьПолеРасшифровки(Расшифровка)
	...
	КоллекцияРасшифровки = ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки);
...
&НаСервере
Процедура ЗаполнитьСтруктуруПараметровПоИерархии(Расшифровка, КоллекцияЭлементовРасшифровки, СтруктураПараметров)
	
	КоллекцияРодителей = КоллекцияЭлементовРасшифровки[Расшифровка].ПолучитьРодителей();
	
	Для Каждого Родитель ИЗ КоллекцияРодителей Цикл
		
		ЗаполнитьСтруктуруПараметровПоИерархии(Родитель.Идентификатор, КоллекцияЭлементовРасшифровки, СтруктураПараметров);
		Если ТипЗнч(Родитель)  = Тип("ЭлементРасшифровкиКомпоновкиДанныхПоля") Тогда
			
			ИдентификаторПоля 	= Родитель.Идентификатор;
			ПоляВыбора 			= КоллекцияЭлементовРасшифровки[ИдентификаторПоля].ПолучитьПоля();
...
КонецПроцедуры
&НаСервере
Процедура ЗаполнитьСтруктуруПараметров(Расшифровка, СтруктураПараметров)
	
	ЗаполнитьСтруктуруПараметровПоИерархии(Расшифровка, ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки).Элементы, СтруктураПараметров);
	
КонецПроцедуры
&НаКлиенте
Функция ОткрытьФормуРасшифровкиФакта(Расшифровка)
	
	Перем НайденноеЗначение;
	
	СтруктураПараметров = Новый Структура("Период, ДатаНачала, ДатаОкончания, Подразделение, СтатьяБюджета");
	
	ЗаполнитьСтруктуруПараметров(Расшифровка, СтруктураПараметров); 
	...
	Если СтруктураПараметров.Подразделение = Неопределено Тогда
		Для Каждого ЭлементНастроек Из Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
			Если ТипЗнч(ЭлементНастроек) = Тип("ЭлементОтбораКомпоновкиДанных") И ТипЗнч(ЭлементНастроек.ПравоеЗначение) = Тип("СправочникСсылка.СтруктураПредприятия") И ЭлементНастроек.Использование Тогда
				СтруктураПараметров.Вставить("Подразделение", ЭлементНастроек.ПравоеЗначение);
			ИначеЕсли ТипЗнч(ЭлементНастроек) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") И ТипЗнч(ЭлементНастроек.Значение) = Тип("СправочникСсылка.СценарииПланирования") И ЭлементНастроек.Использование Тогда
				СтруктураПараметров.Вставить("Сценарий", ЭлементНастроек.Значение);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;	
	...
	ЕстьРасшифровка = ПоказателиСуммаОткрытиеНаСервере(СтруктураПараметров, РезультатРасшифровки, ДанныеВложеннойРасшифровки, АдресСхемы, Новый УникальныйИдентификатор);
	Если ЕстьРасшифровка Тогда
	
		ОткрытьФорму("Документ.Бюджет.Форма.ФормаРасшифровки", Новый Структура("Результат, ДанныеРасшифровки, АдресСхемы", РезультатРасшифровки, ДанныеВложеннойРасшифровки, АдресСхемы), ЭтаФорма, Строка(УникальныйИдентификатор) + Строка(Расшифровка));
...
КонецФункции
&НаСервере
Функция ПоказателиСуммаОткрытиеНаСервере(СтруктураПараметров, РезультатРасшифровки, ДанныеВложеннойРасшифровки, АдресСхемы, ГУИД)
	
	Возврат Документы.Бюджет.ОбработкаРасшифровкиФактическойСуммыБюджетирования(СтруктураПараметров, РезультатРасшифровки, ДанныеВложеннойРасшифровки, АдресСхемы, ГУИД);
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ПолеРасшифровки = "СуммаФакт" Тогда
		СтандартнаяОбработка = Не ОткрытьФормуРасшифровкиФакта(Расшифровка);
	КонецЕсли;
