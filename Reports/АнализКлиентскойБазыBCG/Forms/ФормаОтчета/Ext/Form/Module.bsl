...
Процедура ДатаПредыдущегоСрезаПриИзменении(Элемент)
	...
	Иначе
		ДатаСреза = ДатаСреза(ДатаПредыдущегоСреза);
...
Процедура СформироватьОтчет(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ДатаТекущегоСреза) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнена дата текущего среза. Данное значение заполняется автоматически при выборе даты анализа, ближайшми значеним периода рассчета ABC и XYZ классификации.'"),,"ДатаТекущегоСреза");
	КонецЕсли;
