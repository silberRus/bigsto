...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если ПараметрСоглашение <> Неопределено И ПараметрПартнер <> Неопределено И ПараметрВалюта <> Неопределено Тогда
		...
		Запрос = Новый Запрос("
			...
			|			ВЫБОР
			|				КОГДА ЕСТЬNULL(КурсВалютыОтчетаСрезПоследних.Кратность, 0) > 0
			|					И ЕСТЬNULL(КурсВалютыОтчетаСрезПоследних.Курс, 0) > 0
			|					И ЕСТЬNULL(КурсыВалютСрезПоследнихВалютаДокумента.Кратность, 0) > 0
			|					И ЕСТЬNULL(КурсыВалютСрезПоследнихВалютаДокумента.Курс, 0) > 0
			|				ТОГДА
			|					(КурсыВалютСрезПоследнихВалютаДокумента.Курс * КурсВалютыОтчетаСрезПоследних.Кратность)
			|					/ (КурсВалютыОтчетаСрезПоследних.Курс * КурсыВалютСрезПоследнихВалютаДокумента.Кратность)
			|				ИНАЧЕ
			|					0
