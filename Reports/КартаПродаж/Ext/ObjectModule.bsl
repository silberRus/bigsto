...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	МасштабГеографическойСхемы = Новый РеквизитФормы("МасштабСхемы", ОписаниеТипов, "", НСтр("ru='Масштаб'"));
	ШиринаГеографическойСхемы = Новый РеквизитФормы("ШиринаСхемы", ОписаниеТипов, "", НСтр("ru='Ширина'"));
	ВысотаГеографическойСхемы = Новый РеквизитФормы("ВысотаСхемы", ОписаниеТипов, "", НСтр("ru='Высота'"));
	
	МассивРеквизитов = Новый Массив;
	МассивРеквизитов.Добавить(МасштабГеографическойСхемы);
	МассивРеквизитов.Добавить(ШиринаГеографическойСхемы);
	МассивРеквизитов.Добавить(ВысотаГеографическойСхемы);
	ЭтаФорма.ИзменитьРеквизиты(МассивРеквизитов);
	
	ЭтаФорма.ПостоянныеРеквизиты.Добавить(МасштабГеографическойСхемы.Имя);
	ЭтаФорма.ПостоянныеРеквизиты.Добавить(ШиринаГеографическойСхемы.Имя);
	ЭтаФорма.ПостоянныеРеквизиты.Добавить(ВысотаГеографическойСхемы.Имя);
	
	ЭтаФорма.МасштабСхемы = 100;
	
	Экран = ПолучитьИнформациюЭкрановКлиента();
	ЭтаФорма.ШиринаСхемы = Экран[0].Ширина / Экран[0].DPI * 25.4; // Ширина рисунка геосхемы в мм
	ЭтаФорма.ВысотаСхемы = ЭтаФорма.ШиринаСхемы * 3 / 5; // Высота рисунка геосхемы в мм
	
	ГруппаМасштабированиеСхемы = Элементы.Вставить("ГруппаМасштабированиеСхемы", Тип("ГруппаФормы"));
	ГруппаМасштабированиеСхемы.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаМасштабированиеСхемы.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Горизонтальная;
	ГруппаМасштабированиеСхемы.Отображение = ОтображениеОбычнойГруппы.Нет;
	ГруппаМасштабированиеСхемы.ОтображатьЗаголовок = Ложь;
	
	Декорация = Элементы.Вставить("ДекорацияПустышка", Тип("ДекорацияФормы"), ГруппаМасштабированиеСхемы);
	Декорация.Заголовок = "";
	Декорация.РастягиватьПоГоризонтали = Истина;
	
	Ползунок = Элементы.Вставить("МасштабСхемы", Тип("ПолеФормы"), ГруппаМасштабированиеСхемы);
	Ползунок.Вид = ВидПоляФормы.ПолеПолосыРегулирования;
	Ползунок.ПутьКДанным = "МасштабСхемы";
	Ползунок.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Ползунок.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Ползунок.РастягиватьПоГоризонтали = Ложь;
	Ползунок.Высота = 1;
	Ползунок.Ширина = 30;
	Ползунок.МинимальноеЗначение = 50;
	Ползунок.МаксимальноеЗначение = 300;
	Ползунок.Шаг = 10;
	Ползунок.ШагРазметки = 10;
	Ползунок.Заголовок = "Масштаб " + Строка(ЭтаФорма.МасштабСхемы) + "%";
	
	Ползунок.УстановитьДействие("ПриИзменении", "Подключаемый_Команда");
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(ЭтаФорма, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	// Установка масштаба схемы в 100%
	ДопСвойства.Вставить("МасштабСхемы", 100);
	
	Экран = ПолучитьИнформациюЭкрановКлиента();
	Ширина = Экран[0].Ширина / Экран[0].DPI * 25.4; // Ширина экрана в мм
	Высота = Ширина * 3 / 5;
...
Процедура ВывестиСхему(ДокументРезультат, ТаблицаДанных)
	...
	Масштаб = 100;
	Если Не КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("МасштабСхемы", Масштаб) Тогда
		КоэффМасштаба = 1;
	Иначе
	...
	СлойГорода = Схема.Слои.Найти("Столицы государств");
	...
	Если Схема.Слои.Найти("Регионы России") <> Неопределено
		...
		НеСтолицы = СлойГорода.ВыбратьОбъекты(Новый Структура(СлойГорода.Серии.Столица.Имя, Ложь));
			...
			Если Регион <> Неопределено Тогда
				...
				Если ПараметрВариантАнализа.Значение = 0 Тогда
					...
					Если ИтогПоРегиону.КоэффициентНасыщенности > 0 Тогда
						МониторингЦелевыхПоказателей.ПолучитьRGBпоHSV(
							220, // тон
				...
				ИначеЕсли ПараметрВариантАнализа.Значение = 1 Тогда // Изменение выручки относительно предыдущего периода
					...
					Если ИтогПоРегиону.ЕстьПадение Тогда
						МониторингЦелевыхПоказателей.ПолучитьRGBпоHSV(0, ИтогПоРегиону.КоэффициентНасыщенности, 100, Red, Green, Blue);
						Регион.Цвет = Новый Цвет(Red, Green, Blue);
					ИначеЕсли ИтогПоРегиону.ЕстьРост Тогда
						МониторингЦелевыхПоказателей.ПолучитьRGBпоHSV(120, ИтогПоРегиону.КоэффициентНасыщенности, 100, Red, Green, Blue);
						Регион.Цвет = Новый Цвет(Red, Green, Blue);
					ИначеЕсли ИтогПоРегиону.ЕстьРостНижеСреднего Тогда
						МониторингЦелевыхПоказателей.ПолучитьRGBпоHSV(60, ИтогПоРегиону.КоэффициентНасыщенности, 100, Red, Green, Blue);
						Регион.Цвет = Новый Цвет(Red, Green, Blue);
					КонецЕсли;
...
Процедура ДобавитьОтносительныеКоэффициентыНасыщенности(ТаблицаЗначений, Топ)
	...
	// Для относительных значений изменения выручки 5 областей: две градации падения, одна роста ниже среднего, две - роста выше среднего
