...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Контролировать Тогда
		Заколовок = НСтр("ru = 'Номенклатура без контроля обеспечения (Склад: %1)'");
	Иначе
		Заколовок = НСтр("ru = 'Номенклатура с контролем обеспечения (Склад: %1)'");
	КонецЕсли;
	Заголовок = СтрЗаменить(Заколовок, "%1",Параметры.Склад);
	
	КонтролироватьОбеспечение = Параметры.Контролировать;
...
Процедура ОбновитьСписок(ТекущаяСтрока = Неопределено)
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	Т.Склад = &Склад
		|	И Т.Контролировать <> &Контролировать
	...
	Запрос.УстановитьПараметр("Контролировать", Параметры.Контролировать);
...
Функция ЗаписатьНастройкиКонтроляВРегистр()
	...
	Если ДублиОтсутсвуют Тогда
		...
		СтруктураЗаполнения = Новый Структура("Склад, Контролировать", Склад, НЕ КонтролироватьОбеспечение);
		...
		СтрокаЗаписи.Контролировать = КонтролироватьОбеспечение;
