...
Процедура ОбновитьДанныеФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список,"ДатаРеализации",
		КонецДня(КонецПериода),ВидСравненияКомпоновкиДанных.МеньшеИлиРавно,, ЗначениеЗаполнено(КонецПериода));
...
Процедура ПодтверждениеСтавкиНДССервер(ПараметрыПодтверждения)
	...
	Попытка
		...
		Если ПерезаписатьДокумент Тогда
			ИмяДокумента = ДокументРеализации.Метаданные().Имя;
			Документы[ИмяДокумента].СформироватьЗаписиКнигиПродаж(ДокументРеализации, ДатаПодтверждения, ОрганизацияРеализации);
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.СтавкиНДС.ПустаяСсылка();
...
Процедура ЗаполнитьИзФормыРучногоВвода(ВыбранноеЗначение)
	...
	ПараметрыПодтверждения.ДокументРеализации = ТекущаяСтрока.Регистратор;
