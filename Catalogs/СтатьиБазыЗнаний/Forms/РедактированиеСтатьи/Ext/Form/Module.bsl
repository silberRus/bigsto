...
Процедура УстановитьОформлениеФорматированногоДокумента(СтрокаОформления, ЦветФона)
	...
	// silber {
	СчетчикБезумногоЦикла = 50; // какогото хрена зацикливается
	// } silber
	...
	Пока ОбластьОписания <> Неопределено Цикл
		ОбластьОкончания = ЭтотОбъект.ТекстСтатьи.НайтиТекст("']", ОбластьОписания.ЗакладкаНачала,,Истина);
		...
		ОбластьОписания = ЭтотОбъект.ТекстСтатьи.НайтиТекст("[" + СтрокаОформления + "='", ЗакладкаКонец,,Истина);
		
		// silber {
		Если Не СчетчикБезумногоЦикла Тогда
			Прервать;
		КонецЕсли;
		СчетчикБезумногоЦикла = СчетчикБезумногоЦикла - 1;
		// } silber
