...
Процедура ПолучениеПараметровФискальногоУстройства_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		НовыйФормат = ?(ПараметрыККТ.Свойство("РевизияИнтерфейса"), ПараметрыККТ.РевизияИнтерфейса > 2001, Ложь);
		Элементы.МестоПроведенияРасчетов.Видимость = НовыйФормат;
		Элементы.ГруппаНастройкиККТ15.Видимость    = НовыйФормат;
		Элементы.ПризнакиАгента.Видимость          = НовыйФормат;
...
Процедура ПродолжитьОперацию(Команда)
	...
	Если Элементы.ГруппаПричиныИзмененияСведенийОККТ.Видимость И ПричиныИзмененияСведенийОККТ.Количество() > 0 Тогда
		...
		Для Каждого  ЭлементПричинаИзмененияСведений Из ПричиныИзмененияСведенийОККТ Цикл
			КодПричиныИзмененияСведений = МенеджерОборудованияКлиентСервер.ПолучитьКодПричиныИзмененияСведенийККТ(ЭлементПричинаИзмененияСведений.ПричинаИзмененияСведений);
