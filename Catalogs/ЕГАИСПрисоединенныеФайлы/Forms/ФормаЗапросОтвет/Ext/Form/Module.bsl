...
Процедура ДобавлениеЭлементов(СтрокаТЧ, РеквизитыСообщений)
	...
	Если СтрокаТЧ.ТипСообщения = Перечисления.ТипыЗапросовЕГАИС.Входящий Тогда
		Страница.Картинка = БиблиотекаКартинок.ВходящийЗапросГИСМ;
	ИначеЕсли СтрокаТЧ.ТипСообщения = Перечисления.ТипыЗапросовЕГАИС.Исходящий Тогда
		...
		Для Каждого Колонка Из Колонки Цикл
			Если ПоследовательностьСообщений.Найти(СтрокаТЧ[Колонка.Имя]) = Неопределено Тогда
				...
				ИначеЕсли Операция = Перечисления.ВидыДокументовЕГАИС.КвитанцияПолученЕГАИС Тогда
					НоваяСтрока.Заголовок = НСтр("ru = 'Квитанция ЕГАИС'");
					НоваяСтрока.Подсказка = Операция;
				ИначеЕсли НоваяСтрока.ТипСообщения = Перечисления.ТипыЗапросовЕГАИС.Входящий
...
Функция ТекстыСообщения(Сообщение)
	...
	ТекстСообщенияXML = ИнтеграцияЕГАИС.ТекстСообщенияXMLИзПротокола(Сообщение);
	
	Попытка
		ФорматированныйТекстСообщенияXML = ИнтеграцияЕГАИС.ФорматироватьXMLСПараметрами(ТекстСообщенияXML, ИнтеграцияЕГАИС.ПараметрыФорматированияXML(Истина, "  "));
	Исключение
