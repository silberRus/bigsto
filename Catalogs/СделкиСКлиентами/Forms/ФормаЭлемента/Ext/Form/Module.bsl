...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПроцессыИспользуются И Не СделкаПроиграна И НЕ ОтветОФиксацииПроигрышаПолучен
		...
		ОписаниеОповещениВопрос = Новый ОписаниеОповещения("ВопросФиксацияПроигрышаСделкиПриЗавершении", ЭтотОбъект, ПараметрыЗаписи);
		ПоказатьВопрос(ОписаниеОповещениВопрос ,НСтр("ru='Фиксация проигрыша сделки на текущем этапе. Продолжить?'"),
...
Процедура ПросмотрПланаПроцессаЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    Новый Структура("Ключ", Объект.Ссылка),,,,, Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗафиксироватьПроигрыш(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗафиксироватьПрогрышЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.Отмена);

КонецПроцедуры

&НаКлиенте
Процедура ЗафиксироватьПрогрышЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
