...
Процедура FTPСерверИКаталогПриИзменении(Элемент)
	ВыбранноеЗначение = РассылкаОтчетовКлиентСервер.РазобратьFTPАдрес(FTPСерверИКаталог);
...
Процедура FTPСерверИКаталогОткрытие(Элемент, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(ПолныйАдрес);
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось добавить отчет по причине:
					|%1'"), СтруктураВыбора.СОшибками.Текст);
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось добавить отчеты по причине:
					|%1'"), СтруктураВыбора.СОшибками.Текст);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		КонецЕсли;
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(Неопределено, ТекстСообщения, РежимДиалогаВопрос.ОК);
...
Процедура ПредварительныйПросмотрОтчетаПродолжение(РезультатВыбора, ПараметрыОтчета) Экспорт
	...
	Если ТипЗнч(ПользовательскиеНастройкиКД) = Тип("ПользовательскиеНастройкиКомпоновкиДанных") Тогда
		ПерсонализированныеНастройки = ОтчетыКлиентСервер.НайтиНастройки(ПользовательскиеНастройкиКД.Элементы, Фильтр);
	Иначе
		ПерсонализированныеНастройки = ОтчетыКлиентСервер.НайтиНастройки(ПользовательскиеНастройкиКД, Фильтр);
	КонецЕсли;
	Если Объект.Персонализирована Тогда
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаРасписание Тогда
		...
		Если Объект.ВыполнятьПоРасписанию Тогда
			Элементы.ПредставлениеРасписания.Заголовок = СтандартныеПодсистемыКлиентСервер.ПредставлениеРасписания(Форма.Расписание);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
