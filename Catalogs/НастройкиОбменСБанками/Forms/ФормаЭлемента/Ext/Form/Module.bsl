...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если (Объект.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АльфаБанкОнлайн")
		...
		И НЕ ОбменСБанкамиСлужебныйКлиентСервер.ПравильныйФорматАдреса(Объект.АдресСервера) И НЕ Объект.Недействительна Тогда
...
Процедура ПослеЗагрузкиФайлаВнешнейКомпонентыСбербанка(Адрес)
	...
	ДополнительныеВнешниеКомпонентыКлиент.ПодключитьВнешнююКомпонентуПоИдентификатору(Оповещение, Объект.ИмяВнешнегоМодуля, Ложь);
...
Процедура ЗаполнитьВидыЭДДоступнымиЗначениями()
	...
	ОбменСБанкамиСлужебныйВызовСервера.ЗаполнитьВидыЭДДоступнымиЗначениями(Объект);
	ЗаполнитьСлужебныеРеквизитыТаблицыИсходящихДокументов()
...
 
	...
	// Убираем отметку незаполненного для недействительных настроек
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ИсходящиеДокументыИсходящийДокумент.Имя);
