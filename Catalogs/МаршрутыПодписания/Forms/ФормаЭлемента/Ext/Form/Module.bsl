...
Процедура ПроверитьВалидностьМаршрутаПоВсемЗависимымНастройкам(Отказ)
	...
	Если ЕстьОбменСКонтрагентами ИЛИ ЕстьОбменСБанками Тогда
		...
		Если ЕстьОбменСКонтрагентами Тогда
			...
			|ГДЕ
			|	ПрофилиНастроекЭДОИсходящиеДокументы.МаршрутПодписания = &МаршрутПодписания
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|ГДЕ
			|	СоглашенияОбИспользованииЭДИсходящиеДокументы.МаршрутПодписания = &МаршрутПодписания
			|	И СоглашенияОбИспользованииЭДИсходящиеДокументы.Ссылка.РасширенныйРежимНастройкиСоглашения";
		...
		Пока ВыборкаНастроек.Следующий() Цикл
			...
			ВыборкаВидов = ВыборкаНастроек.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			...
			ЭлектронноеВзаимодействиеСлужебный.ВывестиРезультатыПроверкиМаршрута(РезультатыПроверки, 
				ВыборкаНастроек.НастройкаОбмена, Объект.Наименование, Отказ, "НетДоступныхСертификатов");
		КонецЦикла;
	КонецЕсли;
...
Процедура ДобавитьНовуюСтрокуВДерево(ЭтоУсловие = Ложь, Требование = Неопределено)
	...
	Если ЕстьУсловия И Не ЭтоУсловие ИЛИ ЕстьПодписанты И ЭтоУсловие Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения, Объект.Ссылка, "ДеревоТребований"); 
