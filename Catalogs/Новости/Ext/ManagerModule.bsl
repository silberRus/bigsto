...
Процедура ПолучитьСписокНовостей(
	...
	ИначеЕсли лкВариантОтбора = 3 Тогда // Контекстные новости, должен быть передан отбор в виде ключей "Метаданные" и "Форма".
		...
		Если (ТипЗнч(лкМетаданные) = ТипСтрока)
			...
			лкТаблицаКонтекстныхНовостей = ОбработкаНовостейВызовСервера.ПолучитьКонтекстныеНовости(
				СписокЛентНовостей,
				лкМетаданные,
				лкФорма,
				Неопределено,
	...
	ИначеЕсли лкВариантОтбора = 4 Тогда // Помеченные новости, должен быть передан ключ "Тег".
		...
		Если Найти(ВРег(лкТег), ВРег("Пометка")) = 1
			...
			ШаблонТекстаЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "И &Мета_УсловияПометки", "И ЕСТЬNULL(Рег.Пометка, 0) = " + Прав(лкТег, 1));
		Иначе
			ШаблонТекстаЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "И &Мета_УсловияПометки", "");
		КонецЕсли;
		// Отбор по важности (важные и очень важные).
		...
		Если ВыполнениеРазрешено = Истина Тогда
			...
			Если ЕстьЛентыНовостей = Истина Тогда
				...
				Для Каждого ТекущаяЛентаНовостей Из ЛентыНовостей Цикл
					...
					ОбработкаНовостейВызовСервера.ПолучитьДополнительныеСтандартныеЗначенияНовостей(ТекущаяЛентаНовостей.ЛентаНовостей, ПоставляемыеДанныеОбъекта);
