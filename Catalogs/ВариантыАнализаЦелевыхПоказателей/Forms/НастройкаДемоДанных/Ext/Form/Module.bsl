...
Процедура ЗаполнитьПоДаннымИБ(Команда)
	...
	Если Не РезультатЗаполнения.ДанныеЗаполнены Тогда
		ПоказатьПредупреждение(, РезультатЗаполнения.ОписаниеОшибки,,НСтр("ru = 'Ошибка заполнения'"));
	КонецЕсли;
КонецПроцедуры
...
Функция ПараметрыПоказателя(ВариантАнализа)
	...
	ПараметрыПоказателя.Вставить("ВыделятьМаксимальноеЗначениеДляПокомпонентногоСравнения", );
	ПараметрыПоказателя.Вставить("ГрадиентДляПокомпонентногоСравнения", );
