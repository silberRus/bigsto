...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВариантОбластиДействия = НСтр("ru='конкретный товар'");
	...
	ИначеЕсли ЗначениеЗаполнено(ВидНоменклатуры) Тогда
		...
		НоменклатураПриИзмененииСервер();	
		ВариантОбластиДействия = НСтр("ru='вид номенклатуры'");	
	Иначе
		НоменклатураПриИзмененииСервер();
	КонецЕсли;	
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ВариантОбластиДействия = НСтр("ru='конкретный товар'") 
	...
	ИначеЕсли ВариантОбластиДействия = НСтр("ru='вид номенклатуры'") 
	...
	ИначеЕсли ВариантОбластиДействия = НСтр("ru='список товаров'") Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ВариантОбластиДействия = НСтр("ru='конкретный товар'") Тогда		
	...
	ИначеЕсли ВариантОбластиДействия = НСтр("ru='вид номенклатуры'") Тогда
		НоваяСтрокаОбластиДействия 				   = ТаблицаОбластиДействия.Добавить();
		НоваяСтрокаОбластиДействия.ВидНоменклатуры = ВидНоменклатуры;
	Иначе
...
Процедура ВариантОбластиДействияПриИзменении(Элемент)
	...
	Если ТекущаяОбластьДействия.ВариантОбластиДействия = НСтр("ru='список товаров'") Тогда
...
Процедура ПросмотретьИзображение()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
			...
			УникальныйИдентификатор));
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Отсутствует изображение для просмотра'"),, "АдресКартинки");
		КонецЕсли;
...
Процедура ДобавитьИзображение(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ЗаблокироватьДанныеФормыДляРедактирования();
		РаботаСФайламиКлиент.ДобавитьФайлы(Объект.Ссылка, УникальныйИдентификатор, НоменклатураКлиент.ФильтрФайловИзображений());
	Иначе                        
		ДобавитьИзображениеНеПрисоединенное();
	КонецЕсли;
...
Процедура ЗаполнитьОбластьДействия(СсылкаСертификатаНоменклатуры)
	...
	ИначеЕсли КоличествоЭлементовТаблицы > 0 Тогда
		ВариантОбластиДействия = НСтр("ru='список товаров'");
...
Процедура УстановитьТекущуюОбластьДействия()
	...
	Если ВариантОбластиДействия = НСтр("ru='конкретный товар'")
		Или ВариантОбластиДействия = НСтр("ru='вид номенклатуры'") Тогда
...
Процедура ДобавитьИзображениеНеПрисоединенноеДиалогЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	СтруктураДанныйФайлаКартинки = Новый Структура("ПолноеИмя, ИмяБезРасширения, Расширение, ВремяИзменения, УниверсальноеВремяИзменения");
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ДобавитьИзображениеНеПрисоединенноеИнициализацияФайла",
		ЭтотОбъект,
		Новый Структура("ИмяФайла, ДанныеФайлаКартинки",
			ИмяФайла, СтруктураДанныйФайлаКартинки));
