...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("НастройкаЭДО", НастройкаЭДО) И ЗначениеЗаполнено(НастройкаЭДО) Тогда
		...
		Если Не ЗначениеЗаполнено(ПараметрыНастроекЭДО.ТекстПриглашения) И ЗначениеЗаполнено(НастройкаЭДО) Тогда
...
Процедура ПолучитьИдентификаторыНаСервисе1СЭДО()
	...
	ИмяРеквизитаИННПолучателя = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННКонтрагента");
	ИмяРеквизитаКПППолучателя = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("КППКонтрагента");
	...
	Если ЗначениеЗаполнено(СокрЛП(ПараметрыПолучателя[ИмяРеквизитаИННПолучателя])) Тогда
		
		ПараметрПоиска = СокрЛП(ПараметрыПолучателя[ИмяРеквизитаИННПолучателя]);
		Если ЗначениеЗаполнено(ПараметрыПолучателя[ИмяРеквизитаКПППолучателя]) Тогда
			ПараметрПоиска = ПараметрПоиска + "_" + СокрЛП(ПараметрыПолучателя[ИмяРеквизитаКПППолучателя]);
		КонецЕсли;
