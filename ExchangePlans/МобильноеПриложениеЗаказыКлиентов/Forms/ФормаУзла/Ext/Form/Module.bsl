...
Процедура ИнициализироватьКомпоновщикСервер(НастройкиОбмена = Неопределено, НачальныеНастройки = Ложь)
	...
	Если НастройкиОбмена = Неопределено Тогда
		...
		Иначе
			...
			Иначе
				НастройкаКомпоновки = Объект.ВидНастройкиОбмена.НастройкиОбмена.Получить();
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Процедура СостояниеОбменаДанными()
	...
	|					И СостоянияОбменовДанными.ДатаОкончания = СостоянияОбменовДанными1.ДатаОкончания) КАК СостоянияОбменовДанными
