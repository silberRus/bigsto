...
Процедура НастроитьПереходы(АдресСайта)
	...
	Если ЗначениеЗаполнено(АдресСайта) Тогда
		...
		#Если ВебКлиент Тогда
			ДополнительныеПараметры = Новый Структура("АдресСайта", АдресСайта);
		...
		Если ТипЗнч(ПараметрыПрикладногоРешения) = Тип("Структура") Тогда
			Для Каждого КлючЗначение Из ПараметрыПрикладногоРешения Цикл
				Попытка
					...
					КонецЕсли;
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка параметров обмена с сайтами'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
