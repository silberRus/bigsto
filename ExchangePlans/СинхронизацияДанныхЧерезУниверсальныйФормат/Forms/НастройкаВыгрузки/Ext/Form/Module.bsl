...
Функция ДоступныеОрганизацииУзла(УзелИнформационнойБазы)
	...
	ИспользоватьОтборПоОрганизациям = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелИнформационнойБазы, "ИспользоватьОтборПоОрганизациям");
	
	Если ИспользоватьОтборПоОрганизациям Тогда
		// Организации из табличной части 
		ЗапросИсточника = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
