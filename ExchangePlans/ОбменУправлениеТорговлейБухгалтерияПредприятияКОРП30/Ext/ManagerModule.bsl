...
Процедура ПриПолученииОписанияВариантаНастройки(ОписаниеВарианта, ИдентификаторНастройки, ПараметрыКонтекста) Экспорт
	...
	ОписаниеВарианта.ПутьКФайлуКомплектаПравилНаПользовательскомСайте = "https://users.v8.1c.ru/distribution/project/AccountingCorp30";
	ОписаниеВарианта.ПутьКФайлуКомплектаПравилВКаталогеШаблонов = "\1c\AccountingCorp";
	
	ОписаниеВарианта.ИмяФайлаНастроекДляПриемника 							= "Настройки обмена для БП";
	...
	ОписаниеВарианта.ПодробнаяИнформацияПоОбмену							= "ПланОбмена.ОбменУправлениеТорговлейБухгалтерияПредприятияКОРП30.Форма.ПодробнаяИнформация";
	ОписаниеВарианта.ПояснениеДляНастройкиПараметровУчетаБазыКорреспондента = ПояснениеДляНастройкиПараметровУчетаБазыКорреспондента();
...
Функция КраткаяИнформацияПоОбмену()
	...
	ПоясняющийТекст = НСтр("ru = 'Позволяет синхронизировать данные между конфигурациями ""Управление торговлей"", ред. 11, и ""Бухгалтерия предприятия КОРП"", ред. 3.
...
Функция НастройкаОтборовНаУзле(ИдентификаторНастройки = "")
	...
	СтруктураНастроек.Вставить("ПравилаВыгрузкиПодразделений",         "ТолькоОбособленные");
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	...
	Если НастройкаОтборовНаУзле.СворачиватьДокументыПоСкладу Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Все документы сварачиваются по складу: %ОбобщенныйСклад%;'");
	...
	Иначе
		...
		Если Не НастройкаОтборовНаУзле.ПравилаОтправкиСправочников = "НеСинхронизировать" Тогда
			...
			Если НастройкаОтборовНаУзле.ВыгружатьУправленческуюОрганизацию Тогда
				ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Отправлять данные по всем организациям, включая управленческую'");
			Иначе
				ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Отправлять данные по всем организациям, исключая управленческую'");
			КонецЕсли;
	...
	Если Не НастройкаОтборовНаУзле.ПравилаОтправкиСправочников = "НеСинхронизировать" Тогда
		Если НастройкаОтборовНаУзле.ПравилаВыгрузкиПодразделений = "ТолькоОбособленные" Тогда
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Будут отправлены только обособленные подразделения используемые в документах;'");
		Иначе
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Все подразделения используемые в документах, будут отправлены;'");
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеЗначенийПоУмолчанию(ЗначенияПоУмолчаниюНаУзле, ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	...
	ТекстОписания = НСтр("ru = 'Подразделение для загружаемых документов по умолчанию: '");
	...
	Иначе
		ТекстОписания = ТекстОписания + НСтр("ru = 'не указано'");
	КонецЕсли;
...
Процедура ПриПодключенииККорреспонденту(ВерсияКорреспондента) Экспорт
	...
	Если ВерсияКорреспондента = "0.0.0.0" Тогда
		ВерсияКорреспондента = "3.0.33.1";
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияКорреспондента, "3.0.33.0") < 0 Тогда
		
		ВызватьИсключение НСтр("ru = 'Настройка синхронизации данных поддерживается только с конфигурацией
			|""Бухгалтерия предприятия КОРП"" версии 3.0.33 и выше.'"
		);
	КонецЕсли;
...
Функция НастройкаОтборовНаУзлеБазыКорреспондента(ИдентификаторНастройки = "")
	...
	СтруктураНастроек.Вставить("Организации",                     СтруктураТабличнойЧастиОрганизации);
	СтруктураНастроек.Вставить("РежимВыгрузкиПриНеобходимости",   Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости);
	СтруктураНастроек.Вставить("ВыгружатьДанныеОРасходахФОТ",     Ложь);
...
Функция ОписаниеЗначенийПоУмолчаниюБазыКорреспондента(ЗначенияПоУмолчаниюНаУзле, ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	...
	ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Статья прочих доходов и расходов при оприходовании товаров: %Значение%';");
...
Функция ПояснениеДляНастройкиПараметровУчетаБазыКорреспондента()
	...
	|из приложения Управление торговлей ред. 11, а также заполнить счета учета по умолчанию в регистрах сведений Счета учета номенклатуры и Счета расчетов с контрагентами.
