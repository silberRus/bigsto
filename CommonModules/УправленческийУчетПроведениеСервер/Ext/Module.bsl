...
//	Период - Дата - Период в котором необходимо сформировать движения.
...
//
// Параметры:
//	МассивДокументовРасчетов - Массив - массив документов для которых неоходимо сформировать движения
//
Процедура СформироватьДвиженияКонтрагентКонтрагент(МассивДокументовРасчетов) Экспорт
		...
		Для Каждого Документ Из ДокументыДебета.Строки Цикл
			...
			Для Каждого Организация Из Документ.Строки Цикл
				...
				Если НЕ Взаимозачет Тогда
					Отбор.Вставить("ВладелецОбъектаРасчета", Организация.ВладелецОбъектаРасчета);
				КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Отложенное проведение взаимозачета'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
// Формирование отложенных движений в регистре "Движения Контрагент - Доходы/Расходы".
//
// Параметры:
//	МассивДокументовРасчетов - Массив - массив документов для которых неоходимо сформировать движения
//
Процедура СформироватьДвиженияКонтрагентДоходыРасходы(МассивДокументовРасчетов) Экспорт
	...
	Попытка
		...
		// Проведем взаимозачет кредиторской задолжености
		Запрос = Новый Запрос(ТекстЗапросаДвиженияКонтрагентДоходыРасходы());
		Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		Запрос.УстановитьПараметр("МассивДокументов", МассивДокументовРасчетов);
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить отложенное проведение по регистру ""Движения Контрагент - Доходы/Расходы""
									|по причине: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Отложенное проведение взаимозачета'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
//
// Параметры:
//  ОбъектДокумента - ДокументОбъект - документ для которого необходимо сформировать движения по управленческому балансу.
//
Процедура ДобавитьДвиженияАктивовПассивов(ОбъектДокумента) Экспорт
	
	Обработки.ДвиженияАктивовПассивов.ДобавитьДвиженияДокумента(ОбъектДокумента);
...
//
// Параметры:
//  РегистраторыКОтражению - Массив - массив документов затронытх отложенным распределение взамиорасчетов с партнерами.
//  ПараметрыОтладки - Структура - сруктруа параметров отладки, подробнее см. Обработки.ДвиженияАктивовПассивов.ИнициализироватьПараметрыДвижений.
//
...
//  Массив, типов регистраторов регистра накполения.
//
...
Функция ДанныеДляФормированияДвиженийДенежныеСредстваКонтрагент(Документ, Период, ТаблицыДляДвижений)
	...
	|	ИНАЧЕ ВЫРАЗИТЬ(РасчетыСПартнерами.Долг * КурсВалютыВзаиморасчетов.Курс * КурсВалютыПлатежа.Курс /
	...
	|			ТОГДА ДанныеРегистра.СуммаОплатыВВалютеПлатежа
	|	ИНАЧЕ ВЫРАЗИТЬ(РасчетыСПартнерами.Предоплата * КурсВалютыВзаиморасчетов.Курс * КурсВалютыПлатежа.Курс /
	|				  (КурсВалютыВзаиморасчетов.Кратность * КурсВалютыПлатежа.Курс) КАК ЧИСЛО(15, 2))
...
Функция ДвиженияДенежныеСредстваКонтрагент(МенеджерВременныхТаблиц)
	...
	|	И ДанныеРегистра.ОтложенноеПроведение
	|	И ДанныеРегистра.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТекстЗапросаРасчетыСПартнерами()
	...
	|	РасчетыСПоставщиками.Регистратор,
	|	КлючиАналитики.Организация,
	|	КлючиАналитики.НаправлениеДеятельности,
	|	КлючиАналитики.Партнер,
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыСуммЗадолженности.АвансВыданный)
	|	КОНЕЦ,
	|	КлючиАналитики.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие),
	|	РасчетыСПоставщиками.ДолгРегл < 0 ИЛИ РасчетыСПоставщиками.ПредоплатаРегл < 0,
	|	ТИПЗНАЧЕНИЯ(КлючиАналитики.Контрагент) = ТИП(Справочник.Организации),
	...
	|
	|	КлючиАналитики.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие) КАК НашеПредприятие,
	|	РасчетыСКлиентами.ДолгРегл < 0 ИЛИ РасчетыСКлиентами.ПредоплатаРегл < 0 КАК Сторно,
	|	ТИПЗНАЧЕНИЯ(КлючиАналитики.Контрагент) = ТИП(Справочник.Организации) КАК КонтрагентОрганизация,
	...
	|	КОНЕЦ,
	|	КлючиАналитики.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие),
	|	РасчетыСКлиентами.ДолгРегл < 0 ИЛИ РасчетыСКлиентами.ПредоплатаРегл < 0,
	|	ТИПЗНАЧЕНИЯ(КлючиАналитики.Контрагент) = ТИП(Справочник.Организации),
...
Функция ТекстЗапросаДвиженияКонтрагентДоходыРасходы()
	...
	|	АналитикаПартнеров.Договор КАК Договор,
	|	АналитикаПартнеров.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Расчеты.ЗаказПоставщику КАК ОбъектРасчетов,
	|	СУММА(ВЫБОР КОГДА Расчеты.Долг <> 0
	|		ТОГДА ВЫБОР КОГДА Расчеты.Долг < 0 ТОГДА -Расчеты.Долг ИНАЧЕ Расчеты.Долг КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА Расчеты.Предоплата + Расчеты.КВозврату < 0 ТОГДА -(Расчеты.Предоплата + Расчеты.КВозврату) ИНАЧЕ Расчеты.Предоплата + Расчеты.КВозврату КОНЕЦ
	|	КОНЕЦ) КАК Сумма,
	|	СУММА(ВЫБОР КОГДА Расчеты.ДолгУпр <> 0
	|		ТОГДА ВЫБОР КОГДА Расчеты.ДолгУпр < 0 ТОГДА -Расчеты.ДолгУпр ИНАЧЕ Расчеты.ДолгУпр КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА Расчеты.ПредоплатаУпр < 0 ТОГДА -Расчеты.ПредоплатаУпр ИНАЧЕ Расчеты.ПредоплатаУпр КОНЕЦ
	|	КОНЕЦ) КАК СуммаУпр,
	|	СУММА(ВЫБОР КОГДА Расчеты.ДолгРегл <> 0
	|		ТОГДА ВЫБОР КОГДА Расчеты.ДолгРегл < 0 ТОГДА -Расчеты.ДолгРегл ИНАЧЕ Расчеты.ДолгРегл КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА Расчеты.ПредоплатаРегл < 0 ТОГДА -Расчеты.ПредоплатаРегл ИНАЧЕ Расчеты.ПредоплатаРегл КОНЕЦ
	|	КОНЕЦ) СуммаРегл,
	|	Расчеты.ХозяйственнаяОперация КАК ХозяйственнаяОперация
	|ПОМЕСТИТЬ втРасчетыДоГруппировки
	|ИЗ
	|	РегистрНакопления.РасчетыСПоставщикамиПоДокументам КАК Расчеты
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаПартнеров
	|	ПО Расчеты.АналитикаУчетаПоПартнерам = АналитикаПартнеров.КлючАналитики
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(Расчеты.Регистратор) = ТИП(Документ.СписаниеЗадолженности)
	|	И Расчеты.Регистратор В(&МассивДокументов)
	|	
	|СГРУППИРОВАТЬ ПО
	|	Расчеты.Регистратор,
	...
	|ПОМЕСТИТЬ втРасчеты
	|ИЗ
	|	втРасчетыДоГруппировки КАК Расчеты
	|
	|СГРУППИРОВАТЬ ПО
	|	Расчеты.Регистратор,
	|	Расчеты.Организация,
	...
	|	КонтрагентДоходыРасходы.НомерСтроки КАК НомерСтроки,
	|	КонтрагентДоходыРасходы.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	
	|	КонтрагентДоходыРасходы.Организация КАК Организация,
	|	КонтрагентДоходыРасходы.Подразделение КАК Подразделение,
	|	
	|	КонтрагентДоходыРасходы.НаправлениеДеятельностиКонтрагента КАК НаправлениеДеятельностиКонтрагента,
	|	КонтрагентДоходыРасходы.Партнер КАК Партнер,
	|	КонтрагентДоходыРасходы.Контрагент КАК Контрагент,
	|	КонтрагентДоходыРасходы.Договор КАК Договор,
	|	КонтрагентДоходыРасходы.ОбъектРасчетов КАК ОбъектРасчетов,
	|	
	|	КонтрагентДоходыРасходы.НаправлениеДеятельностиСтатьи КАК НаправлениеДеятельностиСтатьи,
	|	КонтрагентДоходыРасходы.СтатьяДоходовРасходов КАК СтатьяДоходовРасходов,
	|	КонтрагентДоходыРасходы.АналитикаДоходов КАК АналитикаДоходов,
	|	КонтрагентДоходыРасходы.АналитикаРасходов КАК АналитикаРасходов,
	|	КонтрагентДоходыРасходы.АналитикаАктивовПассивов КАК АналитикаАктивовПассивов,
	|	
	|	ЕСТЬNULL(Расчеты.СуммаУпр, КонтрагентДоходыРасходы.Сумма) КАК Сумма,
	|	КонтрагентДоходыРасходы.СуммаБезНДС КАК СуммаБезНДС,
	|	ЕСТЬNULL(Расчеты.СуммаРегл, КонтрагентДоходыРасходы.СуммаРегл) КАК СуммаРегл,
	|	КонтрагентДоходыРасходы.СуммаРеглБезНДС КАК СуммаРеглБезНДС,
	|	
	|	КонтрагентДоходыРасходы.Валюта КАК Валюта,
	|	ЕСТЬNULL(Расчеты.Сумма, КонтрагентДоходыРасходы.СуммаВВалюте) КАК СуммаВВалюте,
	|	КонтрагентДоходыРасходы.СуммаБезНДСВВалюте КАК СуммаБезНДСВВалюте,
	|	КонтрагентДоходыРасходы.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	КонтрагентДоходыРасходы.СуммаВВалютеВзаиморасчетов КАК СуммаВВалютеВзаиморасчетов,
	|	КонтрагентДоходыРасходы.СуммаБезНДСВВалютеВзаиморасчетов КАК СуммаБезНДСВВалютеВзаиморасчетов,
	|	
	|	КонтрагентДоходыРасходы.ИсточникГФУРасчетов КАК ИсточникГФУРасчетов
	|ИЗ
	|	РегистрНакопления.ДвиженияКонтрагентДоходыРасходы КАК КонтрагентДоходыРасходы
	|	ЛЕВОЕ СОЕДИНЕНИЕ втРасчеты КАК Расчеты
	|	ПО КонтрагентДоходыРасходы.Регистратор = Расчеты.Регистратор
	|		И КонтрагентДоходыРасходы.Организация = Расчеты.Организация
	|		И КонтрагентДоходыРасходы.Партнер = Расчеты.Партнер
	|		И КонтрагентДоходыРасходы.Контрагент = Расчеты.Контрагент
	|		И КонтрагентДоходыРасходы.Договор = Расчеты.Договор
	|		И КонтрагентДоходыРасходы.НаправлениеДеятельностиКонтрагента = Расчеты.НаправлениеДеятельности
	|		И КонтрагентДоходыРасходы.ОбъектРасчетов = Расчеты.ОбъектРасчетов
	|		И ТИПЗНАЧЕНИЯ(КонтрагентДоходыРасходы.Регистратор) = ТИП(Документ.СписаниеЗадолженности)
	|ГДЕ
	|	КонтрагентДоходыРасходы.Регистратор В(&МассивДокументов)
	|	
	|УПОРЯДОЧИТЬ ПО
	|	Регистратор,
	|	НомерСтроки
...
Функция НеКонтролироватьИзмененияРегистра(Источник)
	...
	Возврат
		...
		ИЛИ УниверсальныеМеханизмыПартийИСебестоимости.ДвиженияЗаписываютсяРасчетомПартийИСебестоимости(Источник);
