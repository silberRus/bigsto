			...
			Для Каждого ПРО Из ПравилаРегистрацииОбъекта Цикл
				...
				Если ЗначениеЗаполнено(ПРО.ИмяРеквизитаФлага) Тогда
					...
					#Если ВнешнееСоединение ИЛИ ТолстыйКлиентОбычноеПриложение Тогда
						...
						Получатели = ОбменДаннымиВызовСервера.УзлыДляРегистрацииПоУсловиюВыгружатьВсегда(ИмяПланаОбмена, ПРО.ИмяРеквизитаФлага);
				...
				Если ЗначениеЗаполнено(ПРО.ИмяРеквизитаФлага) Тогда
					...
					Если Не Объект.ЭтоНовый() Тогда
						...
						#Если ВнешнееСоединение ИЛИ ТолстыйКлиентОбычноеПриложение Тогда
							...
							Получатели = ОбменДаннымиВызовСервера.УзлыДляРегистрацииПоУсловиюВыгружатьПриНеобходимости(Объект.Ссылка, ИмяПланаОбмена, ПРО.ИмяРеквизитаФлага);
...
Процедура ОпределитьМассивУзловДляОбъекта(МассивУзловРезультат,
	...
	Если ПроверятьСсылку Тогда
		...
		#Если ВнешнееСоединение ИЛИ ТолстыйКлиентОбычноеПриложение Тогда
			...
			ЗначенияСвойствСсылки = ОбменДаннымиВызовСервера.ЗначенияСвойствДляСсылки(Источник.Ссылка, ПРО.СвойстваОбъекта, ПРО.СвойстваОбъектаСтрокой, ПРО.ОбъектМетаданныхИмя);
...
Процедура ПроверитьУстранениеПроблемыДокументаОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	// silber { по не понятным причнам сюда в некоторых случаях прилетает ссылка, а не объект изза чего все ломается,
	//			поэтому я отключаю эту функцию для ссылок
	Если ТипЗнч(Источник) = Тип(СтрЗаменить(Источник.Метаданные().ПолноеИмя(), ".", "Ссылка.")) Тогда
		ВОЗВРАТ;
	КонецЕсли;
	// } silber
