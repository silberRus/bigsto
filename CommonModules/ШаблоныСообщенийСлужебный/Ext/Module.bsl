...
Функция СформироватьСообщениеВнешнейОбработкой(ПараметрыШаблона, СведенияОШаблоне, ПараметрыОтправки)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		Если ТипЗнч(Сообщение) = Тип("Структура") И Сообщение.Свойство("СтруктураВложений") Тогда
	...
	Если Предмет <> Неопределено 
		...
		Если ПараметрыТекстаСообщения[ИмяОбъекта] <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ПараметрыШаблона.Макет) Тогда
				ЗаполнитьРеквизитыПоСКД(ПараметрыТекстаСообщения[ИмяОбъекта], Предмет, ПараметрыШаблона);
			Иначе
				ЗаполнитьРеквизитыПоПредмету(ПараметрыТекстаСообщения[ИмяОбъекта], Предмет);
			КонецЕсли;
			ЗаполнитьРеквизитыСвойствИКонтактнойИнформации(ПараметрыТекстаСообщения[ИмяОбъекта], Предмет);
			Сообщение.ЗначенияРеквизитов = ПараметрыТекстаСообщения[ИмяОбъекта];
			
			Если ПараметрыТекстаСообщения[ИмяОбъекта]["ВнешняяСсылкаНаОбъект"] <> Неопределено Тогда
				ПараметрыТекстаСообщения[ИмяОбъекта]["ВнешняяСсылкаНаОбъект"] = ВнешняяСсылкаНаОбъект(Предмет);
			КонецЕсли;
		Иначе
		...
		Для каждого Картинка Из ДокументHTML.Картинки Цикл
			...
			Для Каждого ПрисоединенныйФайл Из СписокФайлов Цикл
				Если СтрЧислоВхождений(АтрибутИсточникКартинки.Значение, ПрисоединенныйФайл.ИДФайлаЭлектронногоПисьма) > 0 Тогда
					ДанныеФайла = МодульРаботаСФайлами.ДанныеФайла(ПрисоединенныйФайл.Ссылка, ПараметрыШаблона.УникальныйИдентификатор);
...
//Процедура ОбработатьКартинкуВТекстеHTMLДляФорматированногоДокумента(Картинка, ПрисоединенныйФайл, СформированноеСообщение, АтрибутИсточникКартинки, ПараметрыОтправки)
...
Функция ОбработатьОпределитьРеквизитыДляЗапросаПоМетаданным(ПараметрыОснования, МетаданныеОбъекта)
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Формирование шаблона сообщений'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
