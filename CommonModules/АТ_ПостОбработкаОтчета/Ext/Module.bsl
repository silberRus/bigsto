...
Процедура ДополнитьНомераКолонок(ДокументРезультат, ТабПравил)
	...
	Для НомерСтроки = 1 По ДокументРезультат.ВысотаТаблицы Цикл
		Для НомКолонки = 1 По ДокументРезультат.ШиринаТаблицы Цикл
			...
			Если Не ПустаяСтрока(Текст) И ТабПравил.Найти(Текст, "Представление") <> Неопределено Тогда
				
				ТабПравил.Найти(Текст, "Представление").НомерКолонки = НомКолонки;
				КолПолей = КолПолей - 1;
	...
	ВызватьИсключение "Не удалось найти текст номер колонки по тексту:" + СтрСоединить(ТабПравил.ВыгрузитьКолонку("Представление"), ", ");
...
Процедура ЗначениеДереваНаТаблицу(ТабличныйДокумент, СтрокаДерева, ИменаКолонок)
	...
	Для Каждого Строка Из СтрокаДерева.Строки Цикл
		...
		Для Каждого Колонка Из ИменаКолонок Цикл
			ТабличныйДокумент.Область(Строка[Колонка + "Коорд"]).Текст = Строка[Колонка + "Значение"];
		КонецЦикла;
