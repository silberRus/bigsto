...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	ПараметрыРаботыКлиента = Новый Структура("ЧасовойПоясСеанса, ХешПользователя, РегистрироватьБизнесСтатистику,
											...
											|ЗапросНаПолучениеДампов,ЗапросНаОтправку");
	...
	НастройкиЦентраМониторинга.Вставить("ВключитьОповещение", Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ТекущиеДела"));
	ЦентрМониторингаПереопределяемый.ПриОпределенииНастроек(НастройкиЦентраМониторинга);
	
	ПараметрыРаботыКлиента.ВыводитьЗапросПолныхДампов = Пользователи.ЭтоПолноправныйПользователь(, Истина) И НастройкиЦентраМониторинга.ВключитьОповещение;	
...
Функция СформироватьСтруктуруJSONДляОтправки(Параметры)
	...
	JSONСтруктура.Вставить("versionPacket",  "1.0.3.0");
...
Функция HTTPСервисОтправитьДанныеСлужебный(Параметры)
	Если Параметры.ЗащищенноеСоединение Тогда
		HTTPСоединение = Новый HTTPСоединение(Параметры.Сервер, Параметры.Порт,,,,Параметры.Таймаут, Новый ЗащищенноеСоединениеOpenSSL(Неопределено, Неопределено));
	Иначе
		HTTPСоединение = Новый HTTPСоединение(Параметры.Сервер, Параметры.Порт,,,,Параметры.Таймаут);
	КонецЕсли;
...
Функция ПолучитьПараметрыПоУмолчанию()
	...
	// Хранит список компьютеров, где включен сбор полных дампов.
...
Функция НайтиКаталогДампов(КаталогНастроек, ТипДампа, ПрекратитьСборПолных) 
	...
	Если Файл.Существует() Тогда
		...
		Если ПараметрыДампов <> Неопределено Тогда
			...
			Если ТипДампа = "0" И (ПараметрыДампов.type = "0" ИЛИ СтрНайти(КаталогДампов.Путь, "80af5716-b134-4b1c-a38d-4658d1ac4196") > 0) Тогда
				КаталогДампов.УдалятьДампы = Истина;
			КонецЕсли;
			ЧтениеXML.Закрыть();
			
			Если ПараметрыДампов.type <> "3" И ТипДампа = "3" Тогда
				СоздатьСекциюСбораДампов(Файл, ЧтениеXML, КаталогДампов, ТипДампа);
			ИначеЕсли ПараметрыДампов.type = "3" И ПрекратитьСборПолных Тогда
				СоздатьСекциюСбораДампов(Файл, ЧтениеXML, КаталогДампов, ТипДампа);
...
Функция ДанныеОбИспользованииРолей()
	...
	РолиПрофилей = РезультатПакет[7].Выгрузить();
	...
	ДанныеПрофилей = РезультатПакет[6].Выгрузить();
...
Процедура СобратьИОтправитьДампы(СКлиентаНаСервере = Ложь, ИдентификаторЗадания = "") Экспорт
	...
	Если ПараметрыСбораИОтправкиДампов.ОтправлятьФайлыДампов = 0 Тогда
		УстановитьПривилегированныйРежим(Истина);
		УстановитьПараметрЦентраМониторинга("РезультатОтправки", Нстр("ru = 'Пользователь отказал в предоставлении полных дампов.'"));
		УстановитьПривилегированныйРежим(Ложь);
		ПрекратитьСборПолныхДампов();
		Возврат;
	КонецЕсли;
	...
	Если ПустаяСтрока(ПараметрыСбораИОтправкиДампов.ВариантДампа) Тогда
		Возврат;
	КонецЕсли;
	...
	ТипДампа = ?(ПараметрыСбораИОтправкиДампов.ОтправлятьФайлыДампов = 2,"0","3");
	...
	Если ПараметрыСбораИОтправкиДампов.СборПолныхДамповВключен[ИмяКомпьютера] = Истина Тогда
		...
		// Проверим, достаточно ли свободного места.
		Если РезультатЗамера.Значение/1024 < ПараметрыСбораИОтправкиДампов.РезервМестаВключен Тогда
			УстановитьПривилегированныйРежим(Истина);
	...
	Иначе
		// Если сбор выключен.
		// Проверим, достаточно ли свободного места.
		Если РезультатЗамера.Значение/1024 < ПараметрыСбораИОтправкиДампов.РезервМестаВыключен Тогда
			УстановитьПривилегированныйРежим(Истина);
	...
	// Если запрошенный дамп больше не требуется, отправку необходимо прекратить.
	Если НЕ ДампТребуется(ПараметрыСбораИОтправкиДампов.ВариантДампа, "None") Тогда
		ПрекратитьСборПолныхДампов();
	КонецЕсли;
	
	УдалитьУстаревшиеФайлы(ПараметрыСбораИОтправкиДампов.ВариантДампа, КаталогДампов.Путь);
...
Процедура СобратьДампы(Параметры)
	...
	ИмяСвойства = ?(Параметры.СпрашиватьПередОтправкой, "ВариантыДампов", "ВариантыДамповОдобренные");
	...
	Для Каждого ФайлДампа Из ФайлыДампов Цикл	    
		...
		Если ДампТребуется(ВариантДампа, Параметры.ВариантДампа) Тогда
...
Процедура ОтправитьДампы(Параметры)
	...
	Если Параметры.СпрашиватьПередОтправкой Тогда
	...
	Для Каждого Запись Из Параметры.ВариантыДамповОдобренные Цикл
		...
		Если ИмяКомпьютера <> Запись.Значение.ИмяКомпьютера Тогда
			Продолжить;
		КонецЕсли;
		Если ОтправкаДампа(Запись.Ключ, Запись.Значение, ТребуемыйДамп) Тогда
...
Функция СвободноеМестоНаДиске(БукваДиска, СКлиентаНаСервере)
	...
	РезультатЗапроса = Новый Структура("Значение, Успешно, ОписаниеОшибки", 0, Истина, ""); 
		
	КоманднаяСтрока = "typeperf ""\LogicalDisk("+БукваДиска+")\Free Megabytes"" -sc 1";
	ПараметрыЗапускаПрограммы = Новый Структура;
	ПараметрыЗапускаПрограммы.Вставить("ТекущийКаталог", "");
	...
	ПараметрыЗапускаПрограммы.Вставить("СКлиентаНаСервере", СКлиентаНаСервере);
		
	РезультатЗапуска = ЗапуститьПрограмму(КоманднаяСтрока, ПараметрыЗапускаПрограммы, "437");
...
Функция ДампТребуется(ВариантДампа, ТребуемыйДамп)
	...
	GUID = Строка(Параметры.ИдентификаторИнформационнойБазы);
	АдресРесурса = АдресРесурса + "IsDumpNeeded" + "/" + GUID + "/" + ВариантДампа;
	...
	// Эта ветка для проверки востребованности запрошенного варианта дампа.
	// Если ответа от веб-сервиса нет, то продолжаем сбор.
	ИначеЕсли ТребуемыйДамп = "None" Тогда
		Результат = Истина;	
	// Эта ветка для проверки востребованности собранного варианта дампа.
	// Если ответа от веб-сервиса нет, то собранный дамп должен быть равен запрошенному. 
	Иначе
		Результат = ВариантДампа = ТребуемыйДамп;                          		
	КонецЕсли;
...
Функция МожноЗагружатьДамп(ВариантДампа)
	...
	GUID = Строка(Параметры.ИдентификаторИнформационнойБазы);
	АдресРесурса = АдресРесурса + "CanLoadDump" + "/" + GUID + "/" + ВариантДампа;
...
Функция ОтправкаДампа(ВариантДампа, Данные, ТребуемыйДамп)
	...
	АдресРесурса = АдресРесурса + "LoadDump" + "/" + GUID + "/" + ВариантДампа + "/" + HASH;
...
Функция ПараметрыЗапускаПрограммы()
	...
	Параметры.Вставить("СКлиентаНаСервере", Ложь);
...
//
Функция ЗапуститьПрограмму(Знач СтрокаКоманды, ПараметрыЗапускаПрограммы = Неопределено, Кодировка = "") 
	
	Если СодержитНебезопасныеДействия(СтрокаКоманды) Тогда 
		ВызватьИсключение НСтр("ru = 'Недопустимое значение параметра СтрокаКоманды.
	...
	СКлиентаНаСервере 	        = ПараметрыЗапускаПрограммы.СКлиентаНаСервере;
...
Процедура УдалитьВременныйФайл(ПолноеИмяФайла)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Базовая функциональность'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция СодержитНебезопасныеДействия(СтрокаКоманды)
