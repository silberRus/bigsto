...
Процедура ОткрытьТекущиеДелаЭДО() Экспорт
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"ОбщийМодуль.ОбменСКонтрагентамиКлиент.ОткрытьТекущиеДелаЭДО");
...
Процедура ПерезаполнитьДокумент(ПараметрКоманды, Источник = Неопределено, СопоставлениеУжеВыполнено = Ложь, ЭД = Неопределено, СпособОбработки = "") Экспорт
	...
	Результат = ОбменСКонтрагентамиСлужебныйВызовСервера.ПерезаполнитьДокумент(ПараметрыЗаполнения);
	...
	Если Результат.СопоставитьНоменклатуру Тогда
		...
		Если Результат.МассивДокументов.Количество() = 0 Тогда
			...
			Возврат;
		Иначе
			Оповещение = Новый ОписаниеОповещения("ЗаполнитьДокументПоЭД", ЭтотОбъект,
				ПараметрыСопоставления.ДополнительныеПараметры);
			Режим = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
			ОткрытьФорму(ПараметрыСопоставления.ИмяФормы, ПараметрыСопоставления.ПараметрыОткрытияФормы,,,,,Оповещение, Режим);
		КонецЕсли;
...
Процедура НачатьПроверкуОрганизацииВ1СЭДО(Организация, ОповещениеОЗавершении) Экспорт
	...
	Если ТипЗнч(ОповещениеОЗавершении.Модуль) = Тип("УправляемаяФорма") Тогда
