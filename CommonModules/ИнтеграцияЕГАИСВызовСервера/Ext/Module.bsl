...
//
Функция ПодготовитьКПередаче(ВходящиеДанные, ДальнейшееДействие, ДополнительныеПараметры = Неопределено, Немедленно = Ложь) Экспорт
	...
	ВозвращаемоеЗначение = ПодготовитьСообщенияКПередаче(Сообщения, Немедленно);
...
//  Структура - см. функцию ПодготовитьСообщенияКПередаче().
//
Функция ПодготовитьЗапросПроверкиПодключенияКУТМ(АдресУТМ, ПортУТМ, Таймаут, ОбменНаСервере) Экспорт
	...
	ВозвращаемоеЗначение = ПодготовитьСообщенияКПередаче(Сообщения, Истина, НастройкиОбменаЕГАИС);
...
//  Структура - см. функцию ПодготовитьСообщенияКПередаче().
//
Функция ПодготовитьЗапросДанныхКлассификатораКПередаче(ОрганизацияЕГАИС, Операция, ИмяПараметра, ЗначениеПараметра) Экспорт
...
//  Структура - см. функцию ПодготовитьСообщенияКПередаче().
//
Функция ПодготовитьЗапросДанныхТТНКПередаче(ОрганизацияЕГАИС, ИдентификаторТТН) Экспорт
...
//  Структура - см. функцию ПодготовитьСообщенияКПередаче().
//
Функция ПодготовитьЗапросДанныхСправкиКПередаче(ОрганизацияЕГАИС, Операция, РегистрационныйНомер) Экспорт
...
//  Структура - см. функцию ПодготовитьСообщенияКПередаче().
//
Функция ПодготовитьИнформациюОФорматеОбменаКПередаче(ОрганизацияЕГАИС, ИспользуемыйФорматОбмена) Экспорт
...
//
Функция ПодготовитьСообщенияКПередаче(Сообщения, Немедленно = Ложь, НастройкиОбменаЕГАИС = Неопределено)
...
//
Функция ВыполнитьОбмен(ОрганизацииЕГАИС = Неопределено, ДатыПоследнегоЗапуска = Неопределено) Экспорт
...
//
Функция ПроверитьВходящиеДокументы(ОрганизацииЕГАИС = Неопределено) Экспорт
...
Функция ПолучитьВходящиеДокументы(НастройкиОбменаЕГАИС) Экспорт
	...
	Для Каждого ЭлементДанных Из Результат.Изменения Цикл
		...
		ОрганизацияЕГАИС = ЭлементДанных.ОрганизацияЕГАИС;
		...
		Для Каждого Данные Из ЭлементДанных.СлужебныеДанные Цикл
			...
			РезультатОперации = ИнтеграцияЕГАИСКлиентСервер.ОбработатьРезультатОтправкиHTTPЗапроса(
				ИнтеграцияЕГАИСКлиентСервер.ОтправитьHTTPЗапрос(
					НастройкиОбменаЕГАИС.Получить(ОрганизацияЕГАИС),
			...
			Если Не РезультатОперации.ТекстСообщенияXMLПолучен Тогда
				...
				ТекстОшибки = 
					СтрШаблон(НСтр("ru = 'Не удалось удалить входящий документ %1 (%2)
						                 |%3'"),
						Данные.Операция,
						Данные.АдресЗапроса,
						РезультатОперации.ТекстОшибки);
...
//   * Изменения - Массив - Массив структур, см. функцию ИнтеграцияЕГАИСКлиентСервер.СтруктураИзменения().
//
Функция ОбработатьВходящиеДокументы(ДокументыКЗагрузке) Экспорт
	...
	Для Каждого КлючИЗначение Из ДокументыКЗагрузке Цикл
		...
		Для Каждого ВходящиеДанные Из КлючИЗначение.Значение Цикл
			...
			ДанныеОбработки = ОбработатьОтветНаЗапросПолученияДокумента(ВходящиеДанные, ТаблицаСоответствияДокументовТипамЕГАИС);
		...
		Для Каждого ЭлементОчереди Из Очередь Цикл
			...
			Исключение
				...
				Если Не (Не ИнтеграцияЕГАИС.СообщатьОбОшибкахПриЗагрузкеДанных()
					И ( СтрНайти(НРег(ТекстОшибки), НСтр("ru = 'не найден документ'")) > 0
					...
					ИнтеграцияЕГАИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(ТекстОшибкиЖурналРегистрации);
		...
		Для Каждого ДанныеДокументаТТН Из ВходящиеТТН Цикл
			...
			Если ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда
				...
				Если СтрНачинаетсяС(ИдентификаторТТН, ИнтеграцияЕГАИС.ПрефиксИдентификатораРежимаОтладкиОбщий())
					И СтрНайти(ИдентификаторТТН, ИнтеграцияЕГАИС.ПрефиксИдентификатораРежимаОтладки()) = 0 Тогда
					Продолжить;
				КонецЕсли;
			...
			Попытка
				...
				ДанныеЗагруженногоДокумента = ИнтеграцияЕГАИС.ЗагрузитьТТНВходящуюЕГАИС(
					ДанныеДокументаТТН,
					ДанныеДокументаУведомлениеОРегистрации, ОрганизацияЕГАИС);
...
Функция ОбработатьОчередьПередачиДанных(НастройкиОбменаЕГАИС, СообщенияXMLКПередаче = Неопределено, Немедленно = Ложь) Экспорт
	...
	Для Каждого Выборка Из ТаблицаОчередь Цикл
		...
		ИначеЕсли Выборка.ПодготовленоКПередаче И НастройкаОбменаНаКлиенте <> Неопределено Тогда
			...
			СообщениеXML.ТекстСообщенияXML = ИнтеграцияЕГАИС.ТекстСообщенияXMLИзПротокола(Выборка.ИсходящееСообщение);
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(ТекстОшибки) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'ЕГАИС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПриПолученииФормыСправочника(ИмяСправочника, ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	...
	СобытияФормЕГАИСПереопределяемый.ПриПолученииФормыСправочника(ИмяСправочника, ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка);
...
Процедура ПриПолученииФормыДокумента(ИмяДокумента, ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	...
	СобытияФормЕГАИСПереопределяемый.ПриПолученииФормыДокумента(ИмяДокумента, ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка);
...
Процедура ПриПолученииФормыРегистраСведений(ИмяРегистра, ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	...
	СобытияФормЕГАИСПереопределяемый.ПриПолученииФормыРегистраСведений(ИмяРегистра, ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка);
...
// Возвращаемое значение:
//  Структура - Структура со свойствами:
//   * ВидАлкогольнойПродукцииЕГАИС - ОпределяемыйТип.ВидАлкогольнойПродукции - Вид алкогольной продукции.
...
Функция ПередатьСообщениеВУТМ(ИсходящееСообщение, АдресЗапроса, НастройкаОбменаНаСервере)
	...
	ТекстСообщенияXML = ИнтеграцияЕГАИС.ТекстСообщенияXMLИзПротокола(ИсходящееСообщение);
...
//  ДальнейшееДействие - Перечисление.ДальнейшиеДействияПоВзаимодействиюЕГАИС - Дальнейшее действие.
//  ДополнительныеПараметры - Структура - Дополнительные параметры.
// 
// Возвращаемое значение:
...
Функция ОтменитьОперацию(ДокументСсылка, ДанныеПоследнегоСообщения)
	...
	Если ПредыдущаяОперация = Неопределено Тогда
		...
		СтатусПоУмолчанию = МенеджерОбъекта.СтатусПоУмолчанию();
		
		ПараметрыОбновления = РегистрыСведений.СтатусыДокументовЕГАИС.ВозвращаемоеЗначениеДальнейшиеДействияСтатус(СтатусПоУмолчанию, ДальнейшиеДействия, Истина);
	...
	Иначе
		...
		Если ПредыдущаяОперация.ТипСообщения = Перечисления.ТипыЗапросовЕГАИС.Исходящий Тогда
			ПараметрыОбновления = МенеджерОбъекта.СтатусПослеПередачиДанных(
				ДокументСсылка,
				ПредыдущаяОперация.Операция, Перечисления.СтатусыОбработкиСообщенийЕГАИС.ПереданоВУТМ, ДополнительныеПараметры);
		Иначе
			ПараметрыОбновления = МенеджерОбъекта.СтатусПослеПолученияДанных(
				ДокументСсылка,
				ПредыдущаяОперация.Операция, ДополнительныеПараметры);
		КонецЕсли;
...
Функция ПоследнееСообщение(ДокументСсылка, ТипПоследнегоСообщения = "ПереданоВУТМ") Экспорт
	...
	Если ТипПоследнегоСообщения = "ПереданоВУТМ" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Условие",
			"
			|ВЫБОР КОГДА ЕГАИСПрисоединенныеФайлы.ТипСообщения = ЗНАЧЕНИЕ(Перечисление.ТипыЗапросовЕГАИС.Исходящий)
	...
	ИначеЕсли ТипПоследнегоСообщения = "ВОчереди" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Условие",
			"
			|ВЫБОР КОГДА ОчередьПередачиДанныхЕГАИС.Сообщение ЕСТЬ NULL ТОГДА
			|	ЛОЖЬ
			|ИНАЧЕ
			|	ИСТИНА
			|КОНЕЦ
...
Функция ПриЗавершенииПередачиДанных(РезультатыПередачиСообщенийПоОрганизациямЕГАИС) Экспорт
	...
	СтатусыЦветТекстаТребуетВниманияЕГАИС = ИнтеграцияЕГАИС.СтатусыЦветТекстаТребуетВниманияЕГАИС();
	...
	Для Каждого КлючИЗначение Из РезультатыПередачиСообщенийПоОрганизациямЕГАИС Цикл
		...
		Для Каждого ЭлементДанных Из КлючИЗначение.Значение Цикл
			...
			ТекстОшибки = "";
			Если ЗначениеЗаполнено(Результат.РезультатОтправкиЗапроса.ТекстОшибки)
				И ЗначениеЗаполнено(Результат.РезультатОбработкиОтвета.ТекстОшибки) Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(Результат.РезультатОбработкиОтвета.ТекстОшибки) Тогда
				ТекстОшибки = Результат.РезультатОбработкиОтвета.ТекстОшибки;
			КонецЕсли;
			
			Выполнено = СтатусыЦветТекстаТребуетВниманияЕГАИС.Найти(Результат.РезультатОбработкиОтвета.НовыйСтатус) <> Неопределено
			            Или Не ЗначениеЗаполнено(ТекстОшибки);
...
//
Функция ОбработатьОтветНаЗапросПолученияДокумента(ВходящиеДанные, ТаблицаСоответствияДокументовТипамЕГАИС = Неопределено)
	...
	Исключение
		...
		Попытка
			...
			ОбъектXDTO = ИнтеграцияЕГАИС.ПреобразоватьПроизвольныйОбъектXDTOВОбъектXDTO(
				ИнтеграцияЕГАИС.ПроизвольныйОбъектXDTOПоТекстуСообщенияXML(ВходящиеДанные.ТекстXML),
...
Функция КонтрагентТорговогоОбъекта(ТорговыйОбъект) Экспорт
	...
	Возврат ИнтеграцияЕГАИСПереопределяемый.КонтрагентТорговогоОбъекта(ТорговыйОбъект);
...
Функция ТорговыйОбъектКонтрагента(Контрагент) Экспорт
	...
	Возврат ИнтеграцияЕГАИСПереопределяемый.ТорговыйОбъектКонтрагента(Контрагент);
...
Функция ПроверитьСопоставлениеКлассификаторовПрикладногоДокумента(ДокументОснование, УникальныйИдентификатор) Экспорт
	...
	РезультатПроверки = ИнтеграцияЕГАИСПереопределяемый.ПроверитьСопоставлениеКлассификаторовПрикладногоДокумента(
		ДокументОснование,
...
Функция ЕстьРасхожденияМеждуДокументомПоступленияИТТНЕГАИС(ДокументСсылка, ДокументПоступления) Экспорт
	...
	Возврат ИнтеграцияЕГАИСПереопределяемый.ЕстьРасхожденияМеждуДокументомПоступленияИТТНЕГАИС(ДокументСсылка, ДокументПоступления);
...
// Возвращаемое значение:
//  Структура - Структура со свойствами:
//   * Организация - ОпределяемыйТип.ОрганизацияКонтрагентЕГАИС - Организация.
//   * ТорговыйОбъект - ОпределяемыйТип.ТорговыйОбъектЕГАИС     - Торговый объект.
//   * Контрагент - ОпределяемыйТип.ОрганизацияКонтрагентЕГАИС  - Контрагент.
//
...
//  ДокументПоступления - ДокументСсылка - Документ поступления.
//
