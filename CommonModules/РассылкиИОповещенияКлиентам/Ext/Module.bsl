...
Функция ТаблицаПараметровСообщения(СтрокаДанныхШаблона, ДанныеШаблона, ДанныеТипаСобытия)
	...
	СоответствиеПараметров = СоответстиеПараметровТекстаСообщения.Получить(ДанныеТипаСобытия.Имя);
	
	Если СоответствиеПараметров <> Неопределено Тогда
		
		Для Каждого Параметр Из СоответствиеПараметров Цикл
			Попытка
				НоваяСтрока = ТаблицаПараметровСообщения.Добавить();
				НоваяСтрока.Ключ = ДанныеТипаСобытия.Имя + "." + Параметр.Ключ;
				НоваяСтрока.Значение = СтрокаДанныхШаблона[Параметр.Ключ];
			Исключение
				Продолжить;
			КонецПопытки;
		КонецЦикла;
		
		Если СоответствиеПараметров.Получить("ВнешняяСсылкаНаОбъект") <> Неопределено Тогда
			НоваяСтрока = ТаблицаПараметровСообщения.Добавить();
			НоваяСтрока.Ключ = ДанныеТипаСобытия.Имя + ".ВнешняяСсылкаНаОбъект";
			НоваяСтрока.Значение = ОбщегоНазначения.АдресПубликацииИнформационнойБазыВИнтернете() + "#" +  ПолучитьНавигационнуюСсылку(Параметр);
		КонецЕсли;
		
	КонецЕсли;
	
	//Заполнение общих реквизитов
	ИмяУзлаОбщихРеквизитов = ШаблоныСообщенийСлужебный.ИмяУзлаОбщихРеквизитов(); 
	СоответствиеОбщихРеквизитов = СоответстиеПараметровТекстаСообщения[ИмяУзлаОбщихРеквизитов];
	ШаблоныСообщенийСлужебный.ЗаполнитьОбщиеРеквизиты(СоответствиеОбщихРеквизитов);
	 
	Если СоответствиеОбщихРеквизитов <> Неопределено Тогда
		Для Каждого Параметр Из СоответствиеОбщихРеквизитов Цикл
			Попытка
				НоваяСтрока = ТаблицаПараметровСообщения.Добавить();
				НоваяСтрока.Ключ = ИмяУзлаОбщихРеквизитов + "." + Параметр.Ключ;
		...
		КонецЦикла;
	КонецЕсли;
	//Конец заполнения общих реквизитов
...
Функция СоответствиеПараметровСообщения(ДанныеШаблона)
	...
	Возврат ШаблоныСообщений.ПараметрыИзТекстаСообщения(ДанныеШаблона);
...
Функция ДанныеШаблонаСообщений(ДатаОтработки, ВидОповещения, ШаблонСообщения, ПараметрыУсловия, ПредназначенДляПисем, ДанныеТипаСобытия)
	...
	СтруктураВозврата.Вставить("ДанныеШаблона", ДанныеШаблона);
	
	СоответстиеПараметровТекстаСообщения = ШаблоныСообщений.ПараметрыИзТекстаСообщения(ДанныеШаблона);
	...
	Для каждого ЭлементСоответствия Из СоответстиеПараметровТекстаСообщения Цикл
		...
		Попытка
			...
			Для Каждого ВыбранноеПоле Из ЭлементСоответствия.Значение Цикл 
				ОтчетыКлиентСервер.ДобавитьВыбранноеПоле(КомпоновщикНастроек, ВыбранноеПоле.Ключ);
			КонецЦикла;
...
Процедура ОбработатьТекстЗапросаПоКонтактнымЛицамДляОповещений(ШаблонЗапроса, ДанныеВидаОповещения, МассивИсточниковОповещений, ДанныеТипаСобытия, ИтоговыйЗапросСодержитТекст)
	...
	Если НЕ ПустаяСтрока(ТекстКонтактнаяИнформацияКонтактныеЛицаОповещения) 
		И СтрНайти(ШаблонЗапроса, "ВЫБРАТЬ") > 0 Тогда
...
Процедура ПодготовитьНомерТелефона(ЗначенияПолей, Представление, НомерТелефона)
	...
	СтруктураПолей = РаботаСАдресами.ПредыдущаяСтруктураКонтактнойИнформацииXML(ЗначенияПолейXML);
	...
	Если Не ПустаяСтрока(НомерТелефона) Тогда
		...
		Если СтрДлина(НомерТелефонаДляОтправки) = 10 Тогда
			НомерТелефона = "+7" + НомерТелефонаДляОтправки;
