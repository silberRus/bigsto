...
//
Процедура ПодготовитьОбластьИзВыгрузки(Знач КодОбластиДанных, Знач ИдентификаторВыгрузки, Знач СопоставлениеПользователей) Экспорт
	...
	Попытка
		...
		ДобавитьЗаписьВРСОбластиДанных(КодОбластиДанных, Перечисления.СтатусыОбластейДанных.ПустаяСсылка(), ИдентификаторВыгрузки, 0);
...
Процедура УстановитьПраваПользователяПоУмолчанию(Знач ИдентификаторПользователяСервиса, Знач ДоступРазрешен = Истина) Экспорт
	...
	Попытка
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("_ДемоТехнологияСервиса") Тогда
		    ГруппаДоступа = Справочники.ГруппыДоступа.НайтиПоНаименованию("Пользователи");
			ОбщийМодуль_ДемоТехнологияСервиса = ОбщегоНазначения.ОбщийМодуль("_ДемоТехнологияСервиса");
		    Если ДоступРазрешен Тогда
		        ОбщийМодуль_ДемоТехнологияСервиса.УстановитьПрава(ПользовательОбластиДанных, ГруппаДоступа);
		    Иначе
		        ОбщийМодуль_ДемоТехнологияСервиса.СнятьПрава(ПользовательОбластиДанных, ГруппаДоступа);
			КонецЕсли;
		КонецЕсли;
		// _Демо конец примера
        Если ДоступРазрешен Тогда
...
Процедура УстановитьДоступКAPIОбластиДанных(Знач ИдентификаторПользователяСервиса, Знач ДоступРазрешен) Экспорт
	...
	Попытка
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("_ДемоТехнологияСервиса") Тогда
			ОбщийМодуль_ДемоТехнологияСервиса = ОбщегоНазначения.ОбщийМодуль("_ДемоТехнологияСервиса");
		    ГруппаДоступа = ОбщийМодуль_ДемоТехнологияСервиса.ГруппаДоступаКAPI();
		    Если ДоступРазрешен Тогда
		        ОбщийМодуль_ДемоТехнологияСервиса.УстановитьПрава(ПользовательОбластиДанных, ГруппаДоступа);
		    Иначе
		        ОбщийМодуль_ДемоТехнологияСервиса.СнятьПрава(ПользовательОбластиДанных, ГруппаДоступа);
...
Функция СобытиеЖурналаРегистрацииУдаленноеАдминистрированиеУдалитьОбласть() Экспорт
	...
	Возврат НСтр("ru = 'Удаленное администрирование в модели сервиса.Удалить область'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Процедура ДобавитьЗаписьВРСОбластиДанных(Знач КодОбластиДанных, Знач Статус, Знач ИдентификаторВыгрузки = Неопределено, 
	Знач Повтор = 0, Знач Вариант = "")
