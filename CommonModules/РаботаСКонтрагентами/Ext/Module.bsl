...
Функция РеквизитыЮридическогоЛицаПоИНН(Знач ИНН) Экспорт
	...
	АдресСервиса = АдресСервисаКонтрагенты();
	ИмяМетода    = "getCorporationRequisitesByINN";
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ОбъектXDTO = Неопределено;
		Прокси = ПроксиСервиса(
			АдресСервиса,
			ПространствоИменСервисКонтрагенты(),
			ИмяСервисаКонтрагенты(),
			ИмяТочкиПодключенияКонтрагенты(),
			ОписаниеОшибки);
	КонецЕсли;
	
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ЗаполнитьТикетАутентификации(Прокси, АдресСервиса, ИмяМетода, ОписаниеОшибки);
	КонецЕсли;
	
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ВходныеПараметры = Прокси.ФабрикаXDTO.Создать(
			Прокси.ФабрикаXDTO.Тип(ПространствоИменСервисКонтрагенты(), "getCorporationRequisitesByINN"));
...
Функция РеквизитыПредпринимателяПоИНН(Знач ИНН) Экспорт
	...
	АдресСервиса = АдресСервисаКонтрагенты();
	ИмяМетода    = "getEntrepreneurRequisitesByINN";
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ОбъектXDTO = Неопределено;
		Прокси = ПроксиСервиса(
			АдресСервиса,
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ВходныеПараметры = Прокси.ФабрикаXDTO.Создать(
			Прокси.ФабрикаXDTO.Тип(ПространствоИменСервисКонтрагенты(), "getEntrepreneurRequisitesByINN"));
		ВходныеПараметры.INN = ИНН;
		ВходныеПараметры.configurationName = Метаданные.Имя;
		Попытка
...
Функция РеквизитыЮридическихЛицПоНаименованию(Знач Наименование, Знач КодРегиона = "", Знач Адрес = "") Экспорт
	...
	АдресСервиса = АдресСервисаКонтрагенты();
	ИмяМетода    = "getCorporationRequisitesByNameAndAddress";
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ОбъектXDTO = Неопределено;
		Прокси = ПроксиСервиса(
			АдресСервиса,
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ВходныеПараметры = Прокси.ФабрикаXDTO.Создать(
			Прокси.ФабрикаXDTO.Тип(ПространствоИменСервисКонтрагенты(), "getCorporationRequisitesByNameAndAddress"));
...
Функция ИнформацияОСвязяхЮридическогоЛицаПоИНН(Знач ИНН) Экспорт
	...
	АдресСервиса = АдресСервисаКонтрагенты();
	ИмяМетода    = "checkCorporationTrustability";
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ОбъектXDTO = Неопределено;
		Прокси = ПроксиСервиса(
			АдресСервиса,
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ВходныеПараметры = Прокси.ФабрикаXDTO.Создать(
			Прокси.ФабрикаXDTO.Тип(ПространствоИменСервисКонтрагенты(), "checkCorporationTrustability"));
		ВходныеПараметры.INN = ИНН;
		ВходныеПараметры.configurationName = Метаданные.Имя;
		Попытка
...
Функция ИнформацияОСвязяхПредпринимателяПоИНН(Знач ИНН) Экспорт
	...
	АдресСервиса = АдресСервисаКонтрагенты();
	ИмяМетода    = "checkPersonTrustabilityByInn";
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ОбъектXDTO = Неопределено;
		Прокси = ПроксиСервиса(
			АдресСервиса,
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ВходныеПараметры = Прокси.ФабрикаXDTO.Создать(
			Прокси.ФабрикаXDTO.Тип(ПространствоИменСервисКонтрагенты(), "checkPersonTrustabilityByInn"));
		ВходныеПараметры.INN = ИНН;
		ВходныеПараметры.configurationName = Метаданные.Имя;
		Попытка
...
Функция ИнформацияОПроверкахКонтролирующимиОрганамиПоИНН(Знач ИНН, Плановые, Внеплановые, Завершенные, Незавершенные) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получение информации о проверках контрагента с ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		ВходныеПараметры = Прокси.ФабрикаXDTO.Создать(
			Прокси.ФабрикаXDTO.Тип(ПространствоИменСервисКонтрагенты(), "getInspectionsByInn"));
		ВходныеПараметры.INN = ИНН;
		ВходныеПараметры.configurationName = Метаданные.Имя;
		Попытка
	...
	Если ИнформацияОПроверках.ОжиданиеОтвета Тогда
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ожидание ответа получение информации о проверках контрагента с ИНН %1'"),
				ИНН),
			"Контрагент",
			ДополнительноеСобытиеПолучениеДанных());
		Возврат ИнформацияОПроверках;
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Успешно завершено получение информации о проверках контрагента с ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
Функция РеквизитыНалоговогоОрганаПоКоду(Знач КодНалоговогоОргана) Экспорт
	...
	АдресСервиса = АдресСервисаГосударственныеОрганы();
	ИмяМетода    = "findIfnsByCode";
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		Прокси = ПроксиСервиса(
			АдресСервисаГосударственныеОрганы(),
			ПространствоИменГосударственныеОрганы(),
			ИмяСервисаГосударственныеОрганы(),
			ИмяТочкиПодключенияГосударственныеОрганы(),
			ОписаниеОшибки);
	КонецЕсли;
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		Попытка
			Ответ = Прокси.findIfnsByCode(Результат.Код, Метаданные.Имя);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция РеквизитыОтделенияФССПоКоду(Знач КодОтделенияФСС) Экспорт
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		Попытка
			Ответ = Прокси.findFssByCode(Результат.Код, Метаданные.Имя);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция РеквизитыОтделенияПФРПоКоду(Знач КодОтделенияПФР) Экспорт
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		Попытка
			Ответ = Прокси.findPfrByCode(Результат.Код, Метаданные.Имя);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ПриДобавленииПараметровРаботыКлиента(ПараметрыИПП) Экспорт
	...
	// Свойства справочников контрагентов.
	СвойстваСправочниковМассив = Новый Массив;
	Для Каждого СвойстваСправочникаСтрока Из СвойстваСправочниковТаблица Цикл
		...
		КонецЦикла;
		СвойстваСправочниковМассив.Добавить(СвойстваСправочникаСтруктура);
	КонецЦикла;
	ПараметрыРаботаСКонтрагентами.Вставить("СвойстваСправочниковКонтрагентов", СвойстваСправочниковМассив);
...
Функция ИмяСобытияЖурналаРегистрации(ИдентификаторСервиса)
	...
	Если ИдентификаторСервиса = "Контрагент" Тогда
		Возврат НСтр("ru = 'Сервис данных единых гос_реестров'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли ИдентификаторСервиса = "ГосОрганы" Тогда
		Возврат НСтр("ru = 'Сервис данных гос_органов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли ИдентификаторСервиса = "ПроверкаКонтрагентов" Тогда
		Возврат НСтр("ru = 'Проверка контрагентов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Возврат НСтр("ru = 'Работа с контрагентами'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	...
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, """STATUS"":401") > 0
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка аутентификации в сервисе по переданному тикету аутентификации.
						|%1'"),
						ОписаниеОшибки),
				Сервис,
				НСтр("ru='Аутентификация'", КодОсновногоЯзыка));
...
Функция ДополнительноеСобытиеПолучениеДанных()
	...
	Возврат НСтр("ru='Получение данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

#Область Контрагенты

Функция АдресСервисаКонтрагенты()
...
Функция ПространствоИменСервисКонтрагенты()

	Возврат "http://ws.orgregister.company1c.com/";

КонецФункции

Функция ИмяСервисаКонтрагенты()
	
	Возврат "RequisitesWebServiceEndpointImpl7Service";
	
КонецФункции

Функция ИмяТочкиПодключенияКонтрагенты()
	
	Возврат "RequisitesWebServiceEndpointImpl7Port";
...
Функция АдресСервисаГосударственныеОрганы()
	
	Возврат "https://api.taxregister.1c.ru/taxregister/v2?wsdl";
	
КонецФункции

Функция ПространствоИменГосударственныеОрганы()
	
	Возврат "http://ws.taxregister.company1c.com/";
	
КонецФункции

Функция ИмяСервисаГосударственныеОрганы()
	
	Возврат "TaxWsImpl2Service";
	
КонецФункции

Функция ИмяТочкиПодключенияГосударственныеОрганы()
	
	Возврат "TaxWsImpl2Port";
