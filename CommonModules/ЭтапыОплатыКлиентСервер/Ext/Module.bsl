...
Функция НадписьЭтапыОплатыДляРеализациииТоваровУслуг(СтруктураДокумента) Экспорт
	...
	Иначе
		...
		Иначе
			МассивСтрок.Добавить(", ");
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
				НСтр("ru='не указана дата платежа'"), , Оформление.ЦветПредупреждение));
		КонецЕсли;
...
//
Процедура РаспределитьСуммуПоЭтапамГрафикаОплаты(ЭтапыГрафикаОплаты,
	Знач СуммаКРаспределениюОплаты, Знач СуммаКРаспределениюЗалога = 0) Экспорт
...
//
Процедура ДобавитьЭтапОплатыПоУмолчанию(Объект, ВариантОплаты, Знач ЖелаемаяДата,
	Знач СуммаОплатыПоДокументу, Знач СуммаЗалогаПоДокументу = 0, Знач СуммаОтклоненияМерныхТоваров = 0) Экспорт
	...
	ДанныеЭтапа.Вставить("СуммаПлатежа",                 СуммаОплатыПоДокументу - СуммаОтклоненияМерныхТоваров);
	...
	ДанныеЭтапа.Вставить("СуммаОтклоненияМерныхТоваров", СуммаОтклоненияМерныхТоваров);
