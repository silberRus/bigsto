		...
		Если ВидЭД <> Перечисления.ВидыЭД.АктЗаказчик
			...
			Пока Результат.Следующий() Цикл
				...
				Если СтрокаВидЭД = Неопределено Тогда
					...
					Иначе
						ВерсияФормата = ОбменСКонтрагентамиСлужебный.АктуальнаяВерсияФорматаЭД(ВидЭД);
					КонецЕсли;
...
Процедура ИнициализироватьЗапросЭлектронныхДокументовНаПодписи(Запрос, ДляРабочегоСтола = Истина, СтруктураДопОтборов = Неопределено) Экспорт
	...
	|		ПО ЭДНаПодпись.СоглашениеЭД = СоглашенияЭДСертификаты.Ссылка
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ
	...
	|		ПО ЭДНаПодпись.СоглашениеЭД.ЭтоИнтеркампани
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	СертификатыПодписиДляЭД.Сертификат,
	|	СертификатыПодписиДляЭД.ЭД
	...
	|				 	И СоглашенияЭД.ЭтоИнтеркампани)
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Пока ВыборкаЭД.Следующий() Цикл	
				...
				ДанныеФайла = РаботаСФайлами.ДанныеФайла(ВыборкаЭД.Ссылка,, Истина);
