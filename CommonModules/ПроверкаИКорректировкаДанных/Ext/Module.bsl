...
//   СохранятьРезультат - Булево - есил Истина, тогда результат будет сохранен в присоединенный файл.
//
// Возвращаемое значение:
...
// Возвращется строку сообщения, с которой начинается сообщение-прогресс.
//
// Возвращаемое значение:
...
//     * Пометка - Булево - флаг исопльзования проверки.
...
//     * ТребуетсяЗаполнитьНастройки - Булево - если в результате проверки оказалось, что настройки не заполнены, то будет Истина.
...
Функция ПроверитьДанныеОбщая(Проверки, Исправлять, СохранятьРезультат, Адрес = Неопределено)
	...
	Комментарий = СтрШаблон(НСтр("ru = 'Количество проверок: %1.'", Метаданные.ОсновнойЯзык.КодЯзыка), КоличествоПроверок);
	...
	Если Не ТребуетсяЗаполнитьНастройки(Проверки, Исправлять, Обработчики) Тогда
		...
		Для Каждого Проверка Из Проверки Цикл
			...
			Комментарий = НСтр("ru = 'Начало выполнения проверки.'", Метаданные.ОсновнойЯзык.КодЯзыка);
			...
			Комментарий = СтрШаблон(НСтр("ru = 'Результат проверки: %1.'", Метаданные.ОсновнойЯзык.КодЯзыка), РезультатПроверки.ПредставлениеРезультата);
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, , , Комментарий);
	...
	Если Результат.Выполнено Тогда
		Комментарий = НСтр("ru = 'Проверки выполнены.'", Метаданные.ОсновнойЯзык.КодЯзыка);
	Иначе
		Комментарий = НСтр("ru = 'Требуется заполнение настроек'", Метаданные.ОсновнойЯзык.КодЯзыка);
	КонецЕсли;
...
//     * ТабличныйДокумент - ТабличныДокумент - отчет о проверке.
//
// Возвращаемое значение:
//   СправочникСсылка.ИсторияПроверкиИКорректировкиДанныхПрисоединенныеФайлы - присоединенный файл, куда сохранен табличный документ.
//
...
Функция ШаблонОшибкиВыполненияПроверки()
	...
	Возврат НСтр("ru = 'Не удалось выполнить проверку по причине: %1'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Функция СобытиеЗапускЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Запуск'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Функция СобытиеЗавершениеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Завершение'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Функция СобытиеПроверкиЖурналаРегистрации(Проверка)
	...
	Возврат СтрШаблон(НСтр("ru = 'Проверка и корректировка данных.%1'", Метаданные.ОсновнойЯзык.КодЯзыка), Проверка);
