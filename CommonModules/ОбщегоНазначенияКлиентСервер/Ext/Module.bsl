...
#Иначе
	...
	Журнал.Добавить(
		НСтр("ru = 'Журнал диагностики:
		           |Выполнена проверка возможности установить соединение.
	...
	Если СоединениеЧерезПрокси Тогда 
		...
		Описание.Добавить(
			НСтр("ru = 'Диагностика соединения не выполнена, т.к. настроен прокси-сервер. Обратитесь к администратору.'"));
		
	Иначе 
		
		СтруктураСсылки = СтруктураURI(URL);
		АдресСервераРесурса = СтруктураСсылки.Хост;
		АдресСервераКонтрольнойПроверки = "1c.eu";
		...
		Иначе 
			...
			Если Не РезультатКонтрольнойПроверки.Доступен Тогда
				...
				ЖурналТрассировки = ЖурналТрассировкиМаршрутаСервера(АдресСервераКонтрольнойПроверки);
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда 
		// В модели сервиса результат диагностики не возвращаем. В журнал не пишем.
		// Демаскировать внутреннюю архитектуру запрещено.
...
Функция ПроверитьДоступностьСервера(АдресСервера)
	...
	Если ЭтоWindows Тогда
		ШаблонКоманды = "ping %1 -n 2";
	Иначе
		ШаблонКоманды = "ping -c 2 %1";
	КонецЕсли;
...
Функция ЖурналТрассировкиМаршрутаСервера(АдресСервера)
	...
	Если ЭтоWindows Тогда
		ШаблонКоманды = "tracert -w 500 %1";
	Иначе 
		ШаблонКоманды = "traceroute -w 500 %1";
		// Если вдруг пакет не установлен - в потоке вывода будет ошибка.
		// Ну и ладно - мы результат все равно не разбираем, администратор поймет что ему надо доставить.
	КонецЕсли;
