	...
	Для каждого ПозицияЧека Из ОсновныеПараметры.ПозицииЧека Цикл
		Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
			...
			Если БезПередачиТовара Тогда
				...
				ИсправленыОсноныеПараметры = Истина;
			КонецЕсли;
			
			Если НужнаПроверкаСтавокНДС Тогда
				Если ПозицияЧека.Свойство("СтавкаНДС") Тогда
					Если ПозицияЧека.СтавкаНДС = 18 ИЛИ ПозицияЧека.СтавкаНДС = 10 Тогда
	...
	Если НужноПриводитьСуммы Тогда
		...
		Для каждого СтрокаОплат Из ОсновныеПараметры.ТаблицаОплат Цикл
			Если СтрокаОплат.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипыОплатыККТ.Наличные")
				...
				СуммаФактмческихОплат = СуммаФактмческихОплат + СтрокаОплат.Сумма;
			КонецЕсли;
		КонецЦикла;
		
		СуммаФактмческихОплат = СуммаФактмческихОплат - СуммаПолнойОплаты;
		Если СуммаФактмческихОплат < 0 Тогда
			Отказ = Истина;
			ОписаниеОшибки = НСтр("ru = 'Сумма по строкам с указанием признака полной оплаты меньше чем сумма самих оплат'") ;
			Возврат;
		ИначеЕсли СуммаФактмческихОплат = 0 Тогда
			Отказ = Истина;
		...
		Если СуммаФактмческихОплат < СуммаНеобходимойПолнойОплаты Тогда
			Коэффициент = СуммаФактмческихОплат / СуммаНеобходимойПолнойОплаты;
		...
		Отклонение = СуммаФактмческихОплат - РассчитаннаяСуммаЧека;
		...
		Если Макс(Отклонение, -Отклонение) > 0.01 Тогда
			...
			Если МаксимальнаяПозиция.Свойство("СуммаНДС") Тогда
				// Максимальная сумма не можеть быть равна или меньше нуля
		...
		СуммаИсправленияОплат = СуммаНеобходимойПолнойОплаты - СуммаФактмческихОплат;
		
		Если СуммаИсправленияОплат > 0 Тогда
			Для каждого СтрокаОплат Из ОсновныеПараметры.ТаблицаОплат Цикл
				Если СтрокаОплат.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипыОплатыККТ.Постоплата") Тогда
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				Если ОсновныеПараметры.Свойство("ВыводитьПризнакСпособаРасчета") Тогда
					...
					Если ПозицияЧека.Свойство("ПризнакСпособаРасчета", ПризнакСпособаРасчета)
						...
						СтруктураТесктовойСтроки = МенеджерОборудованияКлиентСервер.ПараметрыТекстовойСтрокиЧека(ПризнакСпособаРасчета);
						ПозицииЧека.Добавить(СтруктураТесктовойСтроки);
					КонецЕсли;
				КонецЕсли;
				...
				Если ОсновныеПараметры.Свойство("ВыводитьПризнакПредметаРасчета") Тогда
					...
					Если ПозицияЧека.Свойство("ПризнакПредметаРасчета", ПризнакПредметаРасчета)
						...
						СтруктураТесктовойСтроки = МенеджерОборудованияКлиентСервер.ПараметрыТекстовойСтрокиЧека(ПризнакПредметаРасчета);
						ПозицииЧека.Добавить(СтруктураТесктовойСтроки);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
