...
Функция ИспользованиеПроверкиВозможно() Экспорт
	...
	Возврат ЕстьПравоНаИспользованиеПроверки() И ПроверкаКонтрагентовВключена();
...
Процедура ПроверитьКонтрагентовПослеВключенияПроверкиФоновоеЗадание() Экспорт
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("ru = 'Фоновое задание по обработке непроверенных контрагентов после включения'",
	...
	Если ДанныеКонтрагентов.Количество() > 0 Тогда
		...
		Исключение
			...
			РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
				"ПроверкаКонтрагентов",
				НСтр("ru = 'Фоновое задание в документе'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ИтоговоеСостояниеКонтрагентаВДокументе(ДанныеКонтрагентов, Отбор = Неопределено) Экспорт
	...
	Иначе
		...
		Для каждого ДанныеПоКонтрагенту Из ДанныеПоКонтрагентам Цикл
			...
			ИначеЕсли ПроверкаКонтрагентовКлиентСервер.ЭтоСостояниеКонтрагентаСОшибкой(ДанныеПоКонтрагенту.Состояние) Тогда
			...
			ИначеЕсли ПроверкаКонтрагентовКлиентСервер.ЭтоСостояниеДействующегоКонтрагента(ДанныеПоКонтрагенту.Состояние)
...
Процедура ИнициализироватьРеквизитыФормыОтчета(Форма, ОтчетСРазделами = Ложь) Экспорт
	...
	НовыеРеквизиты.Вставить("ПроверкаИспользуется", 	ИспользованиеПроверкиВозможно());
...
Процедура ПриСозданииНаСервереКонтрагент(Форма, Параметры) Экспорт
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ПроверкаИспользуется Тогда
		...
		СвойстваСправочникаКонтрагенты = РаботаСКонтрагентами.СвойстваСправочникаКонтрагенты(КонтрагентСсылка);
...
Процедура ПриСозданииНаСервереСписокКонтрагентов(Список) Экспорт
	...
	Состояния = ПроверкаКонтрагентовКлиентСервер.СостоянияНедействующегоКонтрагента();
...
Процедура ИнициализироватьРеквизитыФормыКонтрагент(Форма) Экспорт
	...
	ПроверкаИспользуется = ИспользованиеПроверкиВозможно();
	// Включена ли проверка и есть ли доступ на использование проверки.
...
Процедура УдалитьРезультатПроверкиКонтрагентов(Контрагенты = Неопределено) Экспорт 
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			"ПроверкаКонтрагентов",
			НСтр("ru = 'Очистка состояний контрагентов'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура УдалитьРезультатПроверкиКонтрагентовПоДокументам(Документы = Неопределено) Экспорт 
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			"ПроверкаКонтрагентов",
			НСтр("ru = 'Очистка состояний документов'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
			"ПроверкаКонтрагентов",
			НСтр("ru = 'Ошибка при проверке из карточки контрагента'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ЦветаРезультатовПроверки() Экспорт
	...
	Возврат РаботаСКонтрагентамиСлужебныйПовтИсп.ЦветаРезультатовПроверки();
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			...
			"ПроверкаКонтрагентов",
			НСтр("ru = 'Ошибка доступа к веб-сервису'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ОбработатьОтветСервиса(
	...
	ВозвращенОтветПоВсемКонтрагентам = СервисВернулОтветПоВсемКонтрагентам(МаксимальныйНомерКонтрагента, NdsResponse);
	
	Если НЕ ВозвращенОтветПоВсемКонтрагентам Тогда
		Возврат;
	КонецЕсли;
...
Функция СервисВернулОтветПоВсемКонтрагентам(МаксимальныйНомерКонтрагента, NdsResponse)
	
	КоличествоЗапрошенныхОтветов = МаксимальныйНомерКонтрагента + 1;
	КоличествоПолученныхОтветов  = NdsResponse.NP.Количество();
	
	ВозвращенОтветПоВсемКонтрагентам = (КоличествоЗапрошенныхОтветов = КоличествоПолученныхОтветов);
	
	Если НЕ ВозвращенОтветПоВсемКонтрагентам Тогда
		
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сервис ФНС вернул ответы не по всем контрагентам.
					|Должно быть %1 ответов, сервис ФНС вернул %2.'"),
				КоличествоЗапрошенныхОтветов,
	...
	Возврат ВозвращенОтветПоВсемКонтрагентам;
...
Функция ЭтоМедленныйРежимРаботы() Экспорт
	...
	ОперативнаяПамять 	= ОбщегоНазначенияКлиентСервер.ОперативнаяПамятьДоступнаяКлиентскомуПриложению();
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
			"ПроверкаКонтрагентов",
			НСтр("ru = 'Запись результатов проверки контрагентов в регистр'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ВыполнитьПредварительнуюПроверкуДанныхПоВсемКонтрагентам(ДанныеКонтрагентов, ДополнительныеПараметры = Неопределено)
	...
	Для каждого ДанныеКонтрагента Из ДанныеКонтрагентов Цикл
		...
		Если НЕ ПроверкаКонтрагентовКлиентСервер.ЭтоСостояниеКонтрагентаСОшибкой(ДанныеКонтрагента.Состояние) Тогда
...
Процедура ОставитьРазрешенныхКонтрагентов(ДанныеКонтрагентов)
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		// Формирование текста запроса к справочникам всех контрагентов,
		// вычисление пустых ссылок контрагентов.
		ТекстЗапросаВсеКонтрагенты = "";
		ПустыеСсылкиКонтрагентов = Новый Массив;
		ПустыеСсылкиКонтрагентов.Добавить(Неопределено);
		СвойстваСправочников = РаботаСКонтрагентами.СвойстваСправочниковКонтрагентов();
		Для Каждого Справочник Из СвойстваСправочников Цикл
			Если ПустаяСтрока(ТекстЗапросаВсеКонтрагенты) Тогда
				// Это первая таблица в запросе.
				ТекстЗапросаВсеКонтрагенты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
					...
					Справочник.Имя);
			Иначе
				ТекстЗапросаВсеКонтрагенты = ТекстЗапросаВсеКонтрагенты + Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"ОБЪЕДИНИТЬ ВСЕ
						|ВЫБРАТЬ РАЗЛИЧНЫЕ
						|	ВсеДанныеКонтрагентов.Контрагент КАК Контрагент
						...
						Справочник.Имя);
			КонецЕсли;
			ПустыеСсылкиКонтрагентов.Добавить(Справочники[Справочник.Имя].ПустаяСсылка());
		КонецЦикла;
		...
		Запрос.Текст = 
			...
			+ Символы.ПС + ";"
			+ Символы.ПС + ТекстЗапросаВсеКонтрагенты + Символы.ПС + ";" + Символы.ПС
			+ "////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			|	ВсеДанныеКонтрагентов КАК ВсеДанныеКонтрагентов
			|ГДЕ
			|	ВсеДанныеКонтрагентов.Контрагент В (&ПустыеСсылкиКонтрагентов)";
		...
		Запрос.УстановитьПараметр("ПустыеСсылкиКонтрагентов", ПустыеСсылкиКонтрагентов);
...
Функция ЭтоЮридическоеЛицо(ДанныеКонтрагента)
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		СтандартнаяОбработка = Истина;
		РаботаСКонтрагентамиПереопределяемый.ПриОпределенииВидаКонтрагента(СтандартнаяОбработка, ЭтоЮридическоеЛицо, ДанныеКонтрагента);
...
Процедура ПолучитьСостоянияКонтрагентовИзКэша(ДанныеКонтрагентов, ПолучатьВсеСостоянияИзРегистра = Ложь)
	...
	Запрос.УстановитьПараметр("СостоянияСОшибками", 			ПроверкаКонтрагентовКлиентСервер.ВсеОшибочныеСостоянияКонтрагента(Ложь));
	Запрос.УстановитьПараметр("ДанныеКонтрагентов", 			ДанныеКонтрагентов);
...
Процедура ДобавитьТекстПодзапросаНепроверенныеКонтрагенты(
	ЧастиЗапроса,
	СвойстваСправочника,
	ЭтоПерваяТаблица,
	СостояниеПустаяСсылка,
	ДобавлятьРазделитель,
	РазмерПорции)
	
	Если Не ЭтоПерваяТаблица Тогда
		ЧастиЗапроса.Добавить("ОБЪЕДИНИТЬ ВСЕ");
	КонецЕсли;
	Если РазмерПорции = Неопределено Тогда
		ЧастиЗапроса.Добавить("ВЫБРАТЬ РАЗЛИЧНЫЕ");
	Иначе
		ЧастиЗапроса.Добавить("ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ " + Формат(РазмерПорции, "ЧГ=0"));
	КонецЕсли;
	ЧастиЗапроса.Добавить("Контрагенты.Ссылка" + ?(ЭтоПерваяТаблица, " КАК Контрагент,", ","));
	ЧастиЗапроса.Добавить("Контрагенты." + СвойстваСправочника.РеквизитИНН + ?(ЭтоПерваяТаблица, " КАК ИНН,", ","));
	ЧастиЗапроса.Добавить("Контрагенты." + СвойстваСправочника.РеквизитКПП + ?(ЭтоПерваяТаблица, " КАК КПП,", ","));
	Если ДобавлятьРазделитель Тогда
		ЧастиЗапроса.Добавить("&ОбластьДанныхВспомогательныеДанные"
			+ ?(ЭтоПерваяТаблица, " КАК ОбластьДанныхВспомогательныеДанные,", ","));
	КонецЕсли;
	ЧастиЗапроса.Добавить("ЛОЖЬ" + ?(ЭтоПерваяТаблица, " КАК ЭтоИностранныйКонтрагент,", ","));
	ЧастиЗапроса.Добавить(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"ВЫБОР
			|	КОГДА ПОДСТРОКА(Контрагенты.%1, 12, 1) <> """"
			|		ТОГДА ЛОЖЬ
			|	ИНАЧЕ ИСТИНА
			|КОНЕЦ",
			СвойстваСправочника.РеквизитИНН)
		+ ?(ЭтоПерваяТаблица, " КАК ЭтоЮридическоеЛицо,", ","));
	ЧастиЗапроса.Добавить("&Дата" + ?(ЭтоПерваяТаблица, " КАК Дата", ""));
	ЧастиЗапроса.Добавить(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"ИЗ
	...
	Если СостояниеПустаяСсылка Тогда
		ЧастиЗапроса.Добавить(
...
Процедура ПроверитьНепроверенныхКонтрагентов(ЭтоОбновлениеИБВМоделиСервиса, Параметры = Неопределено)
	...
	// Формирование текста запроса к справочникам контрагентов.
	ЧастиЗапроса = Новый Массив;
	СвойстваСправочников = РаботаСКонтрагентами.СвойстваСправочниковКонтрагентов();
	ЭтоПерваяТаблица = Истина;
	ПодсистемаСуществуетРаботаВМоделиСервиса = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса");
	...
	Для Каждого СвойстваСправочника Из СвойстваСправочников Цикл
		
		// Запрос контрагентов, отсутствующих в регистре
		// сведений СостоянияКонтрагентов.
		ДобавитьТекстПодзапросаНепроверенныеКонтрагенты(
			ЧастиЗапроса,
			СвойстваСправочника,
			ЭтоПерваяТаблица,
			Ложь,
			ПодсистемаСуществуетРаботаВМоделиСервиса,
			РазмерПорции);
		
		// Запрос контрагентов, существующих в регистре сведений
		// СостоянияКонтрагентов, имеющих незаполненное состояние проверки.
		Если Не ЭтоОбновлениеИБВМоделиСервиса Тогда
			// При выполнении отложенного обработчика обновления в области данных
			// при работе в Модели сервиса проверка контрагентов с незаполненным
			// состоянием не выполняется, т.к. цель обработчика - заполнение регистра
			// сведений данными о контрагентах, отсутствующих в регистре сведений
			// при переходе на версию конфигурации со встроенной подсистемой
			// "Работа с контрагентами" с версии без подсистемы.
			ДобавитьТекстПодзапросаНепроверенныеКонтрагенты(
				ЧастиЗапроса,
				СвойстваСправочника,
				ЭтоПерваяТаблица,
	...
	Если ПодсистемаСуществуетРаботаВМоделиСервиса Тогда
		МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
...
Функция ОпределитьНедействующихКонтрагентовВОтчете(ПроверяемыеКонтрагенты)
	...
	Состояния = ПроверкаКонтрагентовКлиентСервер.НекорректныеСостоянияКонтрагента();
...
Функция ШаблонТаблицыДанныеКонтрагента()
	...
	Для Каждого СвойстваСправочника Из СвойстваСправочников Цикл
		ТипыСсылок.Добавить(Тип("СправочникСсылка." + СвойстваСправочника.Имя));
	КонецЦикла;
	...
	ДанныеКонтрагентов.Колонки.Добавить("Контрагент", 				Новый ОписаниеТипов(ТипыСсылок));
...
Процедура ДобавитьСтрокуВДанныеКонтрагентовВДокументе(
	...
	СвойстваСправочника = РаботаСКонтрагентами.СвойстваСправочникаКонтрагенты(Контрагент);
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяПоляИНН", "Контрагенты." + СвойстваСправочника.РеквизитИНН);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяПоляКПП", "Контрагенты." + СвойстваСправочника.РеквизитКПП);
...
Процедура УстановитьУсловноеОформлениеСпискаДокументов(Список)
	...
	Если Не (ЕстьПравоНаИспользованиеПроверки() И ПроверкаКонтрагентовВключена()) Тогда
		Возврат;
	КонецЕсли;
	...
	Цвета = ЦветаРезультатовПроверки(); 
...
Процедура УстановитьПараметрыДинамическогоСписка(Список)
	...
	Список.Параметры.УстановитьЗначениеПараметра(
		...
		ЕстьПравоНаИспользованиеПроверки() И ПроверкаКонтрагентовВключена());
...
Процедура ЗапомнитьНаличиеНекорректныхКонтрагентов(Форма)
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ЭтоСчетФактура Тогда
		...
		Форма.РеквизитыПроверкиКонтрагентов.ЭтоСчетФактураСОшибкой = 
			ПроверкаКонтрагентовКлиентСервер.ЭтоНекорректноеСостояниеКонтрагента(Форма.РеквизитыПроверкиКонтрагентов.СостояниеКонтрагентов);
	...
	Иначе
		...
		Если Форма.РеквизитыПроверкиКонтрагентов.ЭтоДокументСоСчетомФактуройВПодвале Тогда
			Форма.РеквизитыПроверкиКонтрагентов.ЭтоСчетФактураСОшибкой = 
				ПроверкаКонтрагентовКлиентСервер.ЭтоНекорректноеСостояниеКонтрагента(Форма.РеквизитыПроверкиКонтрагентов.СостояниеКонтрагентовВСчетеФактуре);
		КонецЕсли;
			...
			Для каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
				...
				Для каждого СтрокиДанныхКонтрагента Из СтрокиДанныхКонтрагентов Цикл
					...
					СтрокаТабличнойЧасти.ЭтоНекорректныйКонтрагент 	= 
						ПроверкаКонтрагентовКлиентСервер.ЭтоНекорректноеСостояниеКонтрагента(СтрокиДанныхКонтрагента.Состояние);
...
Функция ЭтоДокументСОшибкой(Форма)
	...
	Если ЭтоДокументСКонтрагентомВШапке Тогда
		ЕстьОшибкаВШапке = ПроверкаКонтрагентовКлиентСервер.ЭтоНекорректноеСостояниеКонтрагента(Форма.РеквизитыПроверкиКонтрагентов.СостояниеКонтрагента);
	...
	Если ЭтоДокументСКонтрагентомВТабличнойЧасти Тогда
		...
		Для каждого ОписаниеТабличнойЧасти Из Форма.РеквизитыПроверкиКонтрагентов.ОписаниеТабличныхЧастей Цикл
			...
			Для каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
				ЕстьОшибкаВТабличнойЧасти = ПроверкаКонтрагентовКлиентСервер.ЭтоНекорректноеСостояниеКонтрагента(СтрокаТабличнойЧасти.Состояние);
...
Процедура СохранитьРезультатПроверкиКонтрагентаПриЗаписи(КонтрагентОбъект, Отказ) Экспорт 
	...
	СвойстваСправочника = РаботаСКонтрагентами.СвойстваСправочникаКонтрагенты(КонтрагентОбъект.Ссылка);
	Если НЕ ИспользованиеПроверкиВозможно()
		ИЛИ СвойстваСправочника.Иерархический И КонтрагентОбъект.ЭтоГруппа Тогда
		Возврат;
	КонецЕсли;
	...
	ДанныеКонтрагента.Вставить("ИНН", 			КонтрагентОбъект[СвойстваСправочника.РеквизитИНН]);
	ДанныеКонтрагента.Вставить("КПП",			КонтрагентОбъект[СвойстваСправочника.РеквизитКПП]);
...
Процедура УстановитьУсловноеОформлениеСпискаКонтрагентов(Список)
	...
	Если ЗначениеЗаполнено(Список.ОсновнаяТаблица) Тогда
		ИмяСправочника = Сред(Список.ОсновнаяТаблица, СтрНайти(Список.ОсновнаяТаблица, ".") + 1);
		СвойстваСправочниковКонтрагентов = РаботаСКонтрагентами.СвойстваСправочниковКонтрагентов();
		СвойстваСправочника = СвойстваСправочниковКонтрагентов.Найти(ИмяСправочника, "Имя");
		Если СвойстваСправочника = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ИмяСправочника);
		КонецЕсли;
		ВыделитьКраснымИННиКППНекорректныхКонтрагентов(Список, СвойстваСправочника);
		ВыделитьКраснымНекорректныеИНН(Список, СвойстваСправочника);
		ВыделитьКраснымНекорректныеКПП(Список, СвойстваСправочника);
	КонецЕсли;
...
Процедура ВыделитьСерымНедействующихКонтрагентов(Список)
	...
	// Цвет
	ЦветаРезультатовПроверки = ЦветаРезультатовПроверки();
	ДобавитьНовоеОформление(НовоеУсловноеОформление, ЦветаРезультатовПроверки.ЦветТекстаКонтрагентаПрекратившегоДеятельность);
	
КонецПроцедуры

Процедура ВыделитьКраснымИННиКППНекорректныхКонтрагентов(Список, СвойстваСправочника)
	...
	// Оформляемые поля
	ДобавитьНовоеОформляемоеПоле(НовоеУсловноеОформление, "РеквизитИНН", СвойстваСправочника);
	ДобавитьНовоеОформляемоеПоле(НовоеУсловноеОформление, "РеквизитКПП", СвойстваСправочника);
...
Процедура ВыделитьКраснымНекорректныеИНН(Список, СвойстваСправочника)
...
Процедура ВыделитьКраснымНекорректныеКПП(Список, СвойстваСправочника)
...
Процедура ДобавитьНовоеОформляемоеПоле(НовоеУсловноеОформление, Свойство, СвойстваСправочника)
	
	ОформляемоеПоле                = НовоеУсловноеОформление.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле           = Новый ПолеКомпоновкиДанных(СвойстваСправочника[Свойство]);
...
Процедура ЗапуститьПроверкуПриСозданииНаСервереКонтрагента(Форма, Параметры, КонтрагентСсылка)
	...
	Если ЗначениеЗаполнено(КонтрагентСсылка)
		Или ЗначениеЗаполнено(Параметры.ЗначениеКопирования)
		Или Форма.РеквизитыПроверкиКонтрагентов.ЗапускатьПроверкуПриОткрытииФормыНового Тогда
...
// Включение настройки проверки контрагентов при начальном заполнении
// информационной базы в Модели сервиса.
//
Процедура ВключитьПроверкуКонтрагентовВМоделиСервиса() Экспорт
