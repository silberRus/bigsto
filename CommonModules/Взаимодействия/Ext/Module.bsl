...
Функция ПриПолученииЧитаемыхОбъектовОбработчикаОбновленияУчетныхЗаписейЭлектроннойПочты(ЧитаемыеОбъекты) Экспорт
	ЧитаемыеОбъекты.Добавить("РегистрСведений.НастройкиУчетныхЗаписейЭлектроннойПочты");
КонецФункции

Функция ПередУстановкойБлокировкиВОбработчикеОбновленияУчетныхЗаписейЭлектроннойПочты(Блокировка) Экспорт
...
Функция ВыполнитьОтправкуПисьма(Объект, Соединение = Неопределено, ПараметрыПисьма = Неопределено, ПротоколПочты = "") Экспорт
	...
	ИначеЕсли ПротоколПочты = "Все" Тогда
		...
		ПараметрыПисьма.Вставить("ПротоколПочты","IMAP");
		Объект.ИдентификаторСообщенияОтправкаIMAP = РаботаСПочтовымиСообщениями.ОтправитьПочтовоеСообщение(Объект.УчетнаяЗапись, ПараметрыПисьма ,Соединение, "IMAP");
	КонецЕсли;
...
Функция СформироватьДокументHTMLНаОснованииПисьма(Письмо,
	...
	Кодировка = "",
	ОбработатьКартинки = Ложь) Экспорт
...
Функция СодержимоеТегаHTML(Текст, ИмяТега, ВключатьОткрывающийЗакрывающийТег = Ложь, НомерПоПорядку = 1)
	...
	НайденнаяПозицияНачало = СтрНайти(Текст, Начало, НаправлениеПоиска.СНачала, 1, НомерПоПорядку);
	НайденнаяПозицияОкончание = СтрНайти(Текст, Окончание, НаправлениеПоиска.СНачала, 1, НомерПоПорядку);
	...
	Содержимое = Сред(Текст,
	                  ...
	                  НайденнаяПозицияОкончание - НайденнаяПозицияНачало);
...
Процедура СформироватьШапкуИПодвалПечатнойФормыПисьма(Письмо, ДокументHTML, Выборка)
	...
	Если ТипЗнч(Письмо) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
		...
		Для Каждого ПисьмоВложение Из ТаблицаПисемВложений Цикл
			...
			НоваяСтрока.ИмяФайла                  = ВзаимодействияКлиентСервер.ПредставлениеПисьма(ПисьмоВложение.Тема, ПисьмоВложение.Дата) + ".eml";
...
Процедура УстановитьПризнакРассмотрено(МассивВзаимодействий, ЗначениеПризнака, ЕстьИзменения) Экспорт
	...
	Если МассивВзаимодействий.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ВзаимодействияВызовСервера.ЗаписьРегистрРеквизитыВзаимодействия(Выборка.Взаимодействие, СтруктураДляЗаписи);
			ЕстьИзменения = Истина;
...
Процедура ПриЗаписиДокумента(ДокументОбъект) Экспорт
	...
	ОписаниеТиповКонтактов = Новый ОписаниеТипов(ВзаимодействияКлиентСервер.ТипыКонтактов());
