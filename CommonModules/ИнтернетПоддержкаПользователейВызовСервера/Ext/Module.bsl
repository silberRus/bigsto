...
Процедура ОбработкаКомандСервиса(
	...
	Если Не ИнтернетПоддержкаПользователей.ПравоИспользованияБизнесПроцессов() Тогда
...
Процедура ВыполнениеКомандыПрочитатьПараметры(
	...
	Если Не ПустаяСтрока(ИменаСтартовыхПараметров) Тогда
		ИнтернетПоддержкаПользователей.ДобавитьЗначенияСтартовыхПараметров(
			ПараметрыЗапроса,
			ИменаСтартовыхПараметров,
			ОсновныеПараметры.ИмяWSОпределения,
			КСКонтекст);
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ДобавитьСессионныеПараметрыКЗапросу(КСКонтекст, ПараметрыЗапроса);
