...
//           - Описание оповещения для выполнения.
//  ОбъектДрайвера          - <*>
//           - Объект драйвера торгового оборудования.
//  Параметры               - <Структура>
//           - Параметры устройства.
...
Процедура НачатьЗагрузкуДокументов(ОповещениеПриЗавершении, Параметры, ВходныеПараметры, ВыходныеПараметры, Команда)
	...
	ДополнительныеПараметры.Вставить("ПараметрыЗапроса",        ПараметрыЗапроса);
	ДополнительныеПараметры.Вставить("НаименованиеКоманды",     Команда);
	ДополнительныеПараметры.Вставить("НачалоВыгрузки",          Параметры.ПериодВыгрузки.ДатаНачала);
...
Процедура НачатьПринятиеДанных(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		СтруктураОтвета = ОфлайнОборудование1СЭвоторВызовСервера.ЗаполнитьСтруктуруОтвета(ТелоЗапроса[0], ВыходныеПараметры);
	Иначе
	...
	ВыполнитьОбработкуОповещения(ОповещениеОбработкиДанных, ВыходныеПараметры);
...
Процедура НачатьОбработкуДанных(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Если Не ЗначениеЗаполнено(ДополнительныеПараметры.ВыходныеПараметры) Тогда
		...
		Иначе
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Наименование%", ДополнительныеПараметры.ВыходныеПараметры[2]);
			СоздатьСообщениеОбОшибке(Результат, ТекстСообщения);
		КонецЕсли;
	...
	Иначе
		...
		Если ДополнительныеПараметры.НаименованиеКоманды = "ЗагрузитьДанные" Тогда
			ПараметрыКоманды.Вставить("ДатаНачалаВыгрузки", ДополнительныеПараметры.НачалоВыгрузки);
		КонецЕсли;
...
Функция ЗаполнитьПараметрыЗапроса(КомандаЗапроса, Параметры)
	...
	ИначеЕсли КомандаЗапроса = "ЗагрузитьДанные" Тогда
		...
		НачалоВыгрузки = ОфлайнОборудование1СЭвоторВызовСервера.ПреобразоватьДату(Параметры.ПериодВыгрузки.ДатаНачала - 86400);
		Если ЗначениеЗаполнено(Параметры.ПериодВыгрузки.ДатаНачала) Тогда
			АдресЗапроса = АдресЗапроса + "&gtCloseDate=" + НачалоВыгрузки;
		Иначе
			АдресЗапроса = АдресЗапроса + "&gtCloseDate=";
		КонецЕсли;
		
		ОкончаниеВыгрузки = ОфлайнОборудование1СЭвоторВызовСервера.ПреобразоватьДату(Параметры.ПериодВыгрузки.ДатаОкончания);
		Если ЗначениеЗаполнено(Параметры.ПериодВыгрузки.ДатаОкончания) Тогда
