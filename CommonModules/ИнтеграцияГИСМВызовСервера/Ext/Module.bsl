	...
	Если ЗначениеЗаполнено(ВерсияСервера) Тогда
		...
		Для Каждого КлючИЗначение Из СоответствиеВерсийСхемОбмена Цикл
			...
			Если КлючИЗначение.Ключ = ВерсияСервера Тогда
				...
				ВерсияСервераНайдена = Истина;
			КонецЕсли;
			
		КонецЦикла;
		
		Если Не ВерсияСервераНайдена
			И Не ИнтеграцияГИСМ.ВерсияНеМенееТребуемой(ИнтеграцияГИСМ.ВерсияСхемОбменаПоследняяПоддерживаемая(), ВерсияСервера) Тогда
			НоваяВерсия = ВерсияСервера;
		КонецЕсли;
...
Функция ПодготовитьСообщенияКПередаче(Сообщения) Экспорт
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		СтрокаРезультата.Вставить("ОбновленоСостояниеПодтверждения", ОбновленоСостояниеПодтверждения);
...
Функция ПодготовитьСообщениеКПередаче(ТекстСообщенияXML, Реквизиты) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("ИсходящееСообщение");
	ВозвращаемоеЗначение.Вставить("ТекстОшибки");
	ВозвращаемоеЗначение.Вставить("ОбновленоСостояниеПодтверждения");
	
	ОбновитьСостояниеПодтверждения = (Реквизиты.Операция = Перечисления.ОперацииОбменаГИСМ.ПередачаПодтверждения);
	Если ОбновитьСостояниеПодтверждения Тогда
		...
		Исключение
			// Выполнить блокировку объекта не удалось, операция будет выполнена в следующей итерации.
			ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось заблокировать документ %1.'"), Реквизиты.Документ);
			Возврат ВозвращаемоеЗначение;
		КонецПопытки;
	...
	Попытка
		...
		Если ОбновитьСостояниеПодтверждения Тогда
			...
			СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.ПодготовленоКПередаче;
			Состояния = МенеджерОбъекта.ОбновитьСостояниеПодтверждения(ДокументОбъект, Реквизиты.Операция, ИсходящееСообщение, СтатусОбработки);
			ДокументОбъект.Записать(РежимЗаписиДокумента.Запись, РежимПроведенияДокумента.Неоперативный);
			ДокументОбъект.Разблокировать();
		...
		ВозвращаемоеЗначение.ОбновленоСостояниеПодтверждения = ОбновитьСостояниеПодтверждения;
...
Функция КонвертSOAPИзПротокола(Сообщение) Экспорт
	...
	Исключение
		ИнтеграцияГИСМВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
// Прочитать первичные данные входящего сообщения XML.
//
// Параметры:
...
// Возвращаемое значение:
//  Структура - Структура со свойствами:
//   * Версия - Строка - Версия сообщения XML.
//   * ТекстСообщенияXML - Строка - Текст сообщения XML.
//   * ОбъектXDTO - ОбъектXDTO - Объект XDTO.
//
Функция ПрочитатьПервичныеДанныеВходящегоСообщенияXML(ТекстСообщенияXML) Экспорт
	...
	Для Каждого СвойствоИсточника Из ОбъектXDTOИсточник.Свойства() Цикл
		...
		Иначе
			...
			Иначе
				...
				Исключение
					// Необработанное исключение - означает пропуск присвоения значения свойства.
					// Такое поведение требуется, если присваиваемое значение не соответствует схеме XDTO.
					ИнтеграцияГИСМВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				КонецПопытки;
