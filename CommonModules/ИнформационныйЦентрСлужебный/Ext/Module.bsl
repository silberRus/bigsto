...
Функция ЕстьФормаПоПолномуПути(ПолныйПутьКФорме, ЗаписыватьОшибкуВЖР = Истина)
	...
	Если ЗаписыватьОшибкуВЖР Тогда
		ИмяСобытияЖР = ИнформационныйЦентрСервер.ПолучитьИмяСобытияДляЖурналаРегистрации()
			+ "."
			+ НСтр("ru = 'Информационные ссылки'", ТехнологияСервисаИнтеграцияСБСП.КодОсновногоЯзыка());
		
		ЗаписьЖурналаРегистрации(ИмяСобытияЖР,
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			НСтр("ru = 'При обновлении списка форм, в конфигурации не найдена в форма с полным путем:'", ТехнологияСервисаИнтеграцияСБСП.КодОсновногоЯзыка())
				+ ПолныйПутьКФорме);
	КонецЕсли;
