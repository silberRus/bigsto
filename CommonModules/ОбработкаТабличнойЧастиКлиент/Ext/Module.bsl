...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакМаркируемаяАлкогольнаяПродукция") Тогда
		ТекущаяСтрока.МаркируемаяАлкогольнаяПродукция = Ложь;
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакАлкогольнаяПродукция") Тогда
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакАлкогольнаяПродукция") Тогда
		ТекущаяСтрока.АлкогольнаяПродукция = 0;
	КонецЕсли;
...
Функция НеобходимВызовСервера(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ПроверитьХарактеристикуКиЗПоВладельцу")
		И ЗначениеЗаполнено(ТекущаяСтрока.НоменклатураКиЗ) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакМаркируемаяАлкогольнаяПродукция")
		И ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакАлкогольнаяПродукция")
		И ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Если СтруктураДействий.Свойство("ЗаполнитьТипСклада", ПараметрыДействия)
		И КэшированныеЗначения.СвойстваСкладов.Получить(ПараметрыДействия.Склад) = Неопределено Тогда
		Возврат Истина;
	КонецЕсли; 
...
Функция ПолучитьТекущуюСтрокуСтруктурой(ТекущаяСтрока, СтруктураДействий)
	...
	Если СтруктураПолейТЧ.Свойство("Цена")
		И ТекущаяСтрока.Свойство("ЦенаСоСкидкой") Тогда
		
		ЦенаСоСкидкой = СтруктураПолейТЧ.Цена;
		Если ЗначениеЗаполнено(ТекущаяСтрока.ЦенаСоСкидкой) Тогда
			ЦенаСоСкидкой = ТекущаяСтрока.ЦенаСоСкидкой;
		КонецЕсли; 
		СтруктураПолейТЧ.Вставить("ЦенаСоСкидкой", ЦенаСоСкидкой);
