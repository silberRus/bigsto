...
Процедура ВыгрузитьКлиентовДляОбмена(ЗаписьXML, УзелОбмена, ДопСвойства, ВыгружатьПолноеДосье = Ложь) Экспорт
	...
	ЗаголовокЗаписи = НСтр("ru = 'Обмен партнерами.Начало пакета выгрузки партнеров'", КодЯзыка);
	...
	ЗаголовокЗаписи = НСтр("ru = 'Обмен партнерами.Конец пакета выгрузки партнеров'", КодЯзыка);
...
Процедура ВыгрузитьПрайсЛистДляОбмена(ЗаписьXML, СообщениеОбмена, УзелОбмена, НомерСообщения, ВсеЦены, ДопСвойства) Экспорт
	...
	Если ТаблицаПрайсЛиста.Количество() = 0 Тогда
		...
		СообщениеОбмена = НСтр("ru = 'Нет доступных цен.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Возврат;
	КонецЕсли;
	ВремяНачалаОбмена = ТекущаяДатаСеанса();
	ТребуетсяПродолжение = Ложь;
...
Процедура СоздатьОбновитьЗаявкуНаВозвратТоваровОтКлиента(МобильныеДанные, Сообщение) Экспорт
	...
	Если СозданНаОсновании Тогда
		ТекстКомментария = СтрШаблон(НСтр("ru = '<Создан по документу: %1/>'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), СокрЛП(ДокументОснование));
...
Процедура СоздатьОбновитьКлиента(МобильныйКлиент, УзелОбмена, Сообщение) Экспорт
	...
	Если Партнер = Неопределено Тогда
		...
		Партнер.ОсновнойМенеджер = ПользователиКлиентСервер.ТекущийПользователь();
	...
	//+ УП 2.2.
	ЗаполнитьДополнительныеРеквизитыОбъекта(МобильныйКлиент, Партнер);
	//- УП 2.2.
...
Процедура СоздатьОбновитьКонтактноеЛицоКлиента(МобильныеДанные, Сообщение) Экспорт
	...
	Если КонтактноеЛицо = Неопределено Тогда
		...
		КонтактноеЛицо.Автор = ПользователиКлиентСервер.ТекущийПользователь();
...
//  Объект - Объект метаданных - объект для которого регистрируются изменения.
//
...
//
// Возвращаемое значение:
//  НайденныйУзел - ПланОбменаСсылка - Узел обмена "Мобильное приложение "1С: Заказы клиентов"".
//
...
//
// Параметры:
//  Ссылка - ПланОбменаСсылка - Узел обмена "Мобильное приложение "1С: Заказы клиентов"";
...
Процедура ЗаполнитьТаблицуСохраненныхРеквизитов(СохраненныеРеквизитыОбмена) Экспорт
	...
	НастройкиРеквизитов = ГлавныйУзел.НастройкиРеквизитовОбмена.Получить();
...
Процедура ЗаписатьКонтактнуюИнформациюСправочника(ИнформацияПоОбъекту, Знач ТаблицаКИ, Знач ИмяСправочника)
	...
	ИмяСправочника = СтрЗаменить(ИмяСправочника, НСтр("ru = 'Партнер'"), НСтр("ru = 'Клиент'"));
...
Процедура ЗаписатьСправочник(ЗаписьXML, Знач СтруктураСправочника, Знач ИмяСправочника)
	...
	ТипОбъекта = ФабрикаXDTO.Тип(МобильноеПриложениеЗаказыКлиентов.URIПространстваИмен(), "CatalogObject." + ИмяСправочника);
	ОбъектОбмена = ФабрикаXDTO.Создать(ТипОбъекта);
	...
	Если СтруктураСправочника.Свойство("КонтактнаяИнформация") Тогда
		ЗагрузитьКИ = Истина;
		ИсключаемоеСвойство = ИсключаемоеСвойство + ",КонтактнаяИнформация";
	КонецЕсли;
	
	ЗагрузитьДопРеквизиты = Ложь;
	Если СтруктураСправочника.Свойство("ДополнительныеРеквизиты") Тогда
		ЗагрузитьДопРеквизиты = Истина;
		ИсключаемоеСвойство = ИсключаемоеСвойство + ",ДополнительныеРеквизиты";
	КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Обмен мобильное приложение 1С:Заказы.Запись справочника'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ЗагрузитьДопРеквизиты Тогда
		Для Каждого СтрокаДопРеквизиты Из СтруктураСправочника.ДополнительныеРеквизиты Цикл
			ОбъектОбмена.ДополнительныеРеквизиты.Добавить(СтрокаДопРеквизиты);
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаписатьВидЦеныСтроки(ЗаписьXML, Знач СтрокаДанных)
	...
	ИнформацияНДС = СтрНайти(СтрокаДанных.ВидЦеныНаименование, НСтр("ru = 'НДС'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	Наименование = СтрокаДанных.ВидЦеныНаименование;
	
	Если ИнформацияНДС = 0 Тогда
		Если СтруктураВидаЦены.ЦенаВключаетНДС Тогда
			ТекстПроНДС = НСтр("ru = '(с НДС)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Иначе
			ТекстПроНДС = НСтр("ru = '(без НДС)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли;
...
Процедура ЗаписатьОрганизациюСтроки(ЗаписьXML, Знач СтрокаДанных)
	...
	СтруктураОрганизации.ИНН = СтрокаДанных.ОрганизацияИНН;
	СтруктураОрганизации.КПП = СтрокаДанных.ОрганизацияКПП;
	СтруктураОрганизации.СвидетельствоСерияНомер = СтрокаДанных.ОрганизацияСвидетельствоСерияНомер;
	СтруктураОрганизации.СвидетельствоДатаВыдачи = СтрокаДанных.ОрганизацияСвидетельствоДатаВыдачи;
	СтруктураОрганизации.НаименованиеПолное = СтрокаДанных.ОрганизацияНаименованиеПолное;
	
	Если НЕ ЗначениеЗаполнено(СтрокаДанных.ОрганизацияЮрФизЛицо) Тогда
		ЮрФизЛицоСтрокой = "";
	Иначе
		ЮрФизЛицо = Перечисления.ЮрФизЛицо.Индекс(СтрокаДанных.ОрганизацияЮрФизЛицо);
		ЮрФизЛицоСтрокой = Метаданные.Перечисления.ЮрФизЛицо.ЗначенияПеречисления.Получить(ЮрФизЛицо).Имя;
	КонецЕсли;
	СтруктураОрганизации.ЮрФизЛицо = ЮрФизЛицоСтрокой;
	...
	СтруктураОрганизации.Руководитель = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ОтветственныеЛица.Руководитель);
	СтруктураОрганизации.ГлавныйБухгалтер = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ОтветственныеЛица.ГлавныйБухгалтер);
	
	ЗаписатьДополнительныеРеквизиты(СтруктураОрганизации, СтрокаДанных.ОрганизацияСсылка, "Организации", Истина);
...
Функция КомпоновщикНастроекВыгрузкиИндивидуальныхПрайсов(Узел, СтрокаСоглашения)
	...
	НастройкиКомпоновщикаИзНастройкиОбмена = МобильноеПриложениеЗаказыКлиентовПовтИсп.НастройкиОбмена(Узел);
...
Процедура ЗаполнитьШапкуДокумента(ДокументОбъект, МобильныеДанные, ТипДокумента)
	...
	Если НЕ ЗначениеЗаполнено(ДокументОбъект.НалогообложениеНДС) Тогда
		ДокументОбъект.НалогообложениеНДС =
			ЗначениеНастроекПовтИсп.ПолучитьНалогообложениеНДС(ДокументОбъект.Организация, ДокументОбъект.Склад, ДокументОбъект.Дата);
	КонецЕсли;
	...
	ДокументОбъект.ПорядокРасчетов = ВзаиморасчетыСервер.ПорядокРасчетовПоУмолчанию(ДокументОбъект);
...
Процедура ВыгрузитьКонтактныхЛицКлиента(ЗаписьXML, Клиент, УдалениеДанных)
	...
	Для каждого СтрокаДанных Из Результат Цикл
		...
		Если УдалениеДанных Тогда
			МобильноеПриложениеЗаказыКлиентов.УдалениеДанных(Данные);
			Если ЗначениеЗаполнено(СтрокаДанных.Ссылка) И Данные = Неопределено Тогда
				Данные = Новый УдалениеОбъекта(СтрокаДанных.Ссылка);
			КонецЕсли;
		КонецЕсли;
		Если ТипЗнч(Данные) = Тип("УдалениеОбъекта") Тогда
			СериализаторXDTO.ЗаписатьXML(ЗаписьXML, Данные);
...
Процедура ВыгрузитьОбъектыСсылочногоТипа(ЗаписьXML, ВыгруженныеОбъекты, СтрокаПрайсЛиста, УзелОбмена)
	...
	Если ЗначениеЗаполнено(СтрокаПрайсЛиста.УпаковкаСсылка)
		...
		Если ЗначениеЗаполнено(СтрокаПрайсЛиста.УпаковкиНоменклатурыВесЕдиницаИзмерения) 
			...
			КонецЕсли;
		Иначе
			СтруктураУпаковки.ВесЕдиницаИзмерения = ПустойИдентификатор;
			СтруктураУпаковки.Вес = 0;
		КонецЕсли;
		
		СтруктураУпаковки.Штрихкод = ШтрихкодНоменклатуры(СтрокаПрайсЛиста.НоменклатураСсылка,
	...
	Если ЗначениеЗаполнено(СтрокаПрайсЛиста.Номенклатура) И (СтрокаПрайсЛиста.ТребуетсяОтправкаТовара)
		...
		СтруктураНоменклатуры.СтавкаНДСЧислом		=
			ЦенообразованиеКлиентСервер.ПолучитьСтавкуНДСЧислом(СтрокаПрайсЛиста.СтавкаНДССсылка);
		...
		СтруктураНоменклатуры.Штрихкод = ШтрихкодНоменклатуры(СтрокаПрайсЛиста.НоменклатураСсылка,
			СтрокаПрайсЛиста.ХарактеристикаСсылка, СтруктураНоменклатуры.ЕдиницаЗаказа);
		...
		Иначе
			СтруктураНоменклатуры.ВесЕдиницаИзмерения = ПустойИдентификатор;
		...
		Иначе
			СтруктураНоменклатуры.ФайлКартинки = ПустойИдентификатор;
		КонецЕсли;
		
		ЗаписатьДополнительныеРеквизиты(СтруктураНоменклатуры, СтрокаПрайсЛиста.НоменклатураСсылка, "Номенклатура", Истина);
	...
	Если Данные.Товары.Количество() > 0 Тогда
		Для Каждого СтрокаТЧ Из Данные.Товары Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.Упаковка) Тогда
				...
				Если ТипЗнч(СтрокаТЧ.Упаковка.Владелец) = Тип("СправочникСсылка.Номенклатура") 
	...
	Пока УпаковкиНоменклатуры.Следующий() Цикл
		...
		Если ВыгруженныеОбъекты.Получить(СоответствиеУпаковки) = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(УпаковкиНоменклатуры.ЕдиницаИзмерения) Тогда
				...
				КонецЕсли;
			Иначе
				СтруктураУпаковки.ЕдиницаИзмерения = ПустойИдентификатор;
			КонецЕсли;
			...
			СтруктураУпаковки.Штрихкод = ШтрихкодНоменклатуры(СтруктураЗаписиУпаковки.Номенклатура,
...
Процедура ЗаписатьИнформациюПоСтатусуЗаказаКлиента(Сообщение, Знач ЗаказКлиента, Знач Изменен = Ложь, Ошибка = Ложь)
	...
	ИнформацияПоСтатусуЗаказа.Вставить("СуммаДокументаУП", 0);
	ИнформацияПоСтатусуЗаказа.Вставить("СуммаНДСДокументаУП", 0);
	ИнформацияПоСтатусуЗаказа.Вставить("ДатаСледующегоПлатежа", Дата("00010101000000"));
	ИнформацияПоСтатусуЗаказа.Вставить("ДатаПредоплатыПоДокументу", Дата("00010101000000"));
	ИнформацияПоСтатусуЗаказа.Вставить("СуммаПредоплатыПоДокументу", 0);
	
	//+ УП 2.2.
	ИнформацияПоСтатусуЗаказа.Вставить("ДополнительныеРеквизиты");
	//- УП 2.2.
...
Процедура ЗаписатьИнформациюПоСтатусуЗаявкиНаВозврат(Сообщение, Знач ЗаявкаНаВозвратТоваров, Знач Изменен = Ложь, Ошибка = Ложь)
	...
	//+ УП 2.2.
	ИнформацияПоСтатусу.Вставить("ДополнительныеРеквизиты");
	//- УП 2.2.
...
Процедура ЗаписатьСегментыПартнера(Партнер, УзелОбмена)
	...
	НастройкиКомпоновщикаИзНастройкиОбмена = МобильноеПриложениеЗаказыКлиентовПовтИсп.НастройкиОбмена(УзелОбмена);
...
Процедура ЗаполнитьСправочникСоглашенияПриИзменении(СоглашениеОбъект)
	...
	СоглашениеОбъект.ГруппаФинансовогоУчета = 
		Справочники.ГруппыФинансовогоУчетаРасчетов.ПолучитьГруппуФинансовогоУчетаПоУмолчанию(СоглашениеОбъект.ПорядокОплаты, СоглашениеОбъект.ХозяйственнаяОперация);
...
Функция ЗаменитьПартнераНаКлиента(Знач Строка)
	...
	СтрокаЗамены = Строка;
	СтрокаЗамены = СтрЗаменить(СтрокаЗамены, НСтр("ru = 'Партнер'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		НСтр("ru = 'Клиент'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	СтрокаЗамены = СтрЗаменить(СтрокаЗамены, НСтр("ru = 'партнер'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		НСтр("ru = 'клиент'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ИдентификаторФайлаКартинки(ЗаписьXML, ВыгруженныеОбъекты, СтрокаПрайса, УзелОбмена)
	...
	Если ВыгруженныеОбъекты.Получить(ИдентификаторФайлаКартинки) = Неопределено 
		...
		Исключение
			ДанныеФайла = "";
		КонецПопытки;
		РеквизитыФайлаКартинкиXDTO.Вставить("ХранимыйФайл", Новый ХранилищеЗначения(ДанныеФайла, Новый СжатиеДанных(9)));
...
Функция ИндивидуальныйПрайс(УзелОбмена, СтрокаСоглашения)
	...
	Для Каждого СтрокаПрайса Из ТаблицаПрайса Цикл
		...
		СтрокаПрайса.ЦеноваяГруппа = ?(ИспользоватьЦеновыеГруппы,
			СокрЛП(СтрокаПрайса.ЦеноваяГруппаСсылка.УникальныйИдентификатор()), ПустойИдентификатор);
...
Функция ИнформацияОКонтрагенте(Знач СтрокаТаблицы)
	...
	СтруктураКонтрагента.ИНН = СтрокаТаблицы.КонтрагентИНН;
	СтруктураКонтрагента.ИННКПП = СокрЛП(СтрокаТаблицы.КонтрагентИНН)
...
Функция ИнформацияОПартнере(Знач СтрокаТаблицы, ВыгружатьПолноеДосье)
	...
	СтруктураПартнера.ВидЦены						= ПустойИдентификатор;
	...
	СтруктураПартнера.ДополнительнаяИнформация 		= ДосьеПартнера(СтрокаТаблицы.ПартнерКВыгрузке, ВыгружатьПолноеДосье);
	...
	//
	ЗаписатьДополнительныеРеквизиты(СтруктураПартнера, СтрокаТаблицы.ПартнерКВыгрузке, "Партнеры", Истина);
	//- УП 2.2.
...
Функция ИнформацияПоСтатусамЗаказовКлиентов(ЗаказыКлиентов)
	...
	Пока ВыборкаДокументов.Следующий()Цикл
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьРасширенныеВозможностиЗаказаКлиента") Тогда
			...
			КонецЕсли;
		Иначе
			СостояниеИндекс = Перечисления.СостоянияЗаказовКлиентов.Индекс(Перечисления.СостоянияЗаказовКлиентов.Закрыт);
			Состояние = Метаданные.Перечисления.СостоянияЗаказовКлиентов.ЗначенияПеречисления.Получить(СостояниеИндекс).Имя;
		КонецЕсли;
		...
		Если ЗаказКлиентаИБ.ПорядокРасчетовДоговор Тогда
			...
			Если ЗаказКлиентаИБ.УстановленСрокОплаты Тогда
				Структура.Вставить("ДатаСледующегоПлатежа", ЗаказКлиентаИБ.СрокОплаты);
			КонецЕсли;
		Иначе
		...
		//+ УП 2.2.
		Структура.Вставить("ДополнительныеРеквизиты");
		ЗаписатьДополнительныеРеквизиты(Структура, ЗаказКлиентаИБ.Ссылка, "ЗаказКлиента", Ложь);
		//- УП 2.2.
...
Функция ИнформацияПоСтатусамЗаявокНаВозвратТоваров(ЗаявкиНаВозврат)
	...
	Пока ВыборкаДокументов.Следующий()Цикл
		...
		//+ УП 2.2.
		Структура.Вставить("ДополнительныеРеквизиты");
		ЗаписатьДополнительныеРеквизиты(Структура, ЗаявкаНаВозвратИБ.Ссылка, "ЗаявкаНаВозвратТоваровОтКлиента", Ложь);
		//- УП 2.2.
...
Функция СоглашениеСКлиентомПоОрганизацииИВидуЦен(СтрокаТаблицы)
	...
	СтрокаСоглашения.Пояснение = СтрШаблон(НСтр("ru = '%1 (%2, %3, %4)'"),
		ПредставлениеДоговора, СокрЛП(СтрокаТаблицы.КонтрагентСсылка),
	...
	СтрокаСоглашения.РасчетыПоДоговору = СтрокаТаблицы.РасчетыПоДоговору;
	СтрокаСоглашения.СуммаДолгаПоДоговору = СтрокаТаблицы.СуммаДолгаПоДоговору;
...
Функция СтруктураУпаковки()
	...
	СтруктураУпаковки.Вставить("Штрихкод");
...
Процедура УстановитьРеквизитыБанковскогоСчетаОрганизации(СтруктураРеквизитов, БанковскийСчет)
	...
	Иначе
		СтруктураРеквизитов.Вставить("НомерСчета", "");
		СтруктураРеквизитов.Вставить("Банк", "");
		СтруктураРеквизитов.Вставить("БИК", "");
		СтруктураРеквизитов.Вставить("ГородБанка", "");
...
Процедура ЗаписатьДополнительныеРеквизиты(ИнформацияПоОбъекту, Знач СсылкаНаОбъект, Знач ИмяОбъекта, Знач ЭтоСправочник)
	
	Если ЭтоСправочник Тогда
		ТаблицаОбъекта = "Справочник." + ИмяОбъекта + ".ДополнительныеРеквизиты";
		ЛокальноеИмяОбъекта = "CatalogTabularSectionRow." + ИмяОбъекта + ".ДополнительныеРеквизиты";
		ЛокальноеИмя = "Справочник." + ИмяОбъекта;
	Иначе
		ТаблицаОбъекта = "Документ." + ИмяОбъекта + ".ДополнительныеРеквизиты";
		ЛокальноеИмяОбъекта = "DocumentTabularSectionRow." + ИмяОбъекта + ".ДополнительныеРеквизиты";
		ЛокальноеИмя = "Документ." + ИмяОбъекта;
	КонецЕсли;
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	...
	ОписаниеТиповЗначенияДопРеквизита = МобильноеПриложениеЗаказыКлиентовПовтИсп.ОписаниеТиповЗначенияДопРеквизита();
	...
	ЗаписатьДополнительныеРеквизитыОбъекта(ИнформацияПоОбъекту, РеквизитыОбмена, ЛокальноеИмяОбъекта);
КонецПроцедуры
...
Процедура ЗаписатьДополнительныеРеквизитыОбъекта(ИнформацияПоОбъекту, Знач ДопРеквизиты, Знач ЛокальноеИмяОбъекта)
	...
	ИнформацияПоОбъекту.ДополнительныеРеквизиты = МассивСтрокДопРеквизитов;
КонецПроцедуры
...
Процедура ЗаписатьДополнительныйРеквизит(ЗаписьXML, Знач СтрокаДанных)
	...
	// Если реквизит объекта или доподлнительный реквизит объекта больше не участвует в обмене,
	...
	СтруктураДопРеквизита = МобильноеПриложениеЗаказыКлиентовПовтИсп.СтруктураДопРеквизита();
	...
	Иначе
		...
		ЗаполнитьЗначенияСвойств(СтруктураДопРеквизита, ЗначенияРеквизитовДопРеквизита,,
			"ВладелецДополнительныхЗначений, НаборСвойств");
	...
	ЗаписатьСправочник(ЗаписьXML, СтруктураДопРеквизита, "ДополнительныеРеквизитыИСведения");
...
Процедура ЗаписатьЗначенияСвойствОбъектов(ЗаписьXML, Владелец, Удаление = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			МобильноеПриложениеЗаказыКлиентовПовтИсп.СтруктураЗначенияСвойствОбъекта(Иерархия, Выборка.IsFolder);
			...
			?(Иерархия, "ЗначенияСвойствОбъектовИерархия", "ЗначенияСвойствОбъектов"));
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаполнитьДополнительныеРеквизитыОбъекта(МобильныеДанные, Объект)
	...
	Если НЕ МобильныеДанные.Свойства().Получить("ДополнительныеРеквизиты") = Неопределено Тогда
		...
		Для Каждого СтрокаДоп Из МобильныеДанные.ДополнительныеРеквизиты Цикл
			...
			СоответсвиеРеквизита = СохраненныеРеквизитыОбмена.Найти(СокрЛП(СтрокаДоп.Свойство),
				"УникальныйИдентификаторСвойства");
			Если СоответсвиеРеквизита = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ЭтоДопРеквизит = НЕ ТипЗнч(СоответсвиеРеквизита.Реквизит) = Тип("Строка");
			
			Если ЭтоДопРеквизит Тогда
				СтрокаДопРеквизит = ДополнительныеРеквизитыОбъекта.Найти(СоответсвиеРеквизита.Реквизит);
				Если НЕ СтрокаДопРеквизит = Неопределено Тогда
					СтрокаДопРеквизит.ТекстоваяСтрока = СтрокаДоп.ТекстоваяСтрока;
					Если СоответсвиеРеквизита.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")) Тогда
						Значение = Справочники.ЗначенияСвойствОбъектов.ПолучитьСсылку(
							Новый УникальныйИдентификатор(СокрЛП(СтрокаДоп.Значение)));
					ИначеЕсли СоответсвиеРеквизита.ТипЗначения.СодержитТип(
				...
				Иначе
					...
					Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
						...
						СтрокаДопРеквизит.Свойство = СоответсвиеРеквизита.Реквизит;
						СтрокаДопРеквизит.ТекстоваяСтрока = СтрокаДоп.ТекстоваяСтрока;
						СтрокаДопРеквизит.Значение = Значение;
					Иначе
				...
				КонецЕсли;
			Иначе
				РеквизитОбъекта = Новый Структура(СокрЛП(СоответсвиеРеквизита.Реквизит), СтрокаДоп.Значение);
...
Процедура ЗаполнитьЗависимостиСвойствДопРеквизита(ДопРеквизит, СтруктураДопРеквизита)
	...
	Если НЕ ТипЗнч(ДопРеквизит) = Тип("Строка") Тогда
		...
		Если НЕ Результат.Пустой() Тогда
			...
			ТипОбъекта = ФабрикаXDTO.Тип(МобильноеПриложениеЗаказыКлиентов.URIПространстваИмен(),
				"CatalogTabularSectionRow.ДополнительныеРеквизитыИСведения.ЗависимостиДополнительныхРеквизитов");
