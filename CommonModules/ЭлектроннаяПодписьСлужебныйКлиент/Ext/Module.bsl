...
Процедура НайтиУстановленныеПрограммы(Оповещение, ОписаниеПрограмм, ПроверятьНаСервере) Экспорт
	...
	Если ПроверятьНаСервере = Неопределено Тогда
		...
		ПроверятьНаСервере = ОбщиеНастройки.ПроверятьЭлектронныеПодписиНаСервере
...
Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	// Здесь проверяется что это Linux или OS X клиент.
	Контекст.Вставить("ЭтоLinux", Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент());
...
Процедура ДобавитьСертификатПослеВыбораНазначения(Назначение, ПараметрыСоздания) Экспорт
	...
	Если Назначение = "ЗаявлениеНаВыпускСертификата" Тогда
		...
		ИмяОбработкиЗаявления = "Обработка.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.Форма.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата";
		ОткрытьФорму(ИмяОбработкиЗаявления, ПараметрыФормы);
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайла(ПараметрыСоздания) Экспорт
	...
	Контекст = Новый Структура("ПараметрыСоздания", ПараметрыСоздания);
	НачатьПодключениеРасширенияРаботыСФайлами(Новый ОписаниеОповещения(
...
Процедура СохранитьДанныеСертификатаВместеСПодписьюЦиклВнутреннийЦиклНачало(Контекст)
	...
	Иначе
		СохранитьДанныеСертификатаВместеСПодписьюЦиклВнутреннийЦиклПослеИнициализацииФайла(Ложь, Контекст);
	КонецЕсли;
...
Процедура ОткрытьИнструкциюПоРаботеСПрограммами() Экспорт
	...
	Если Раздел = "УчетВГосударственныхУчреждениях" Тогда
		ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("http://its.1c.ru/bmk/bud/digsig");
	Иначе
		ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("http://its.1c.ru/bmk/comm/digsig");
	КонецЕсли;
...
// Продолжение процедуры ПроверитьСертификат.
Процедура ПроверитьСертификатВМоделиСервиса(Результат, Контекст) Экспорт
...
// Продолжение процедуры ВыполнитьНаСтороне.
Процедура ВыполнитьНаСторонеВМоделиСервиса(Результат, Контекст) Экспорт
