...
// Ложь - в противном случае.
//
Функция ПравоИспользованияБизнесПроцессов() Экспорт
...
Процедура ДобавитьЗначенияСтартовыхПараметров(ПараметрыЗапроса, ИменаСтартовыхПараметров, ИмяWSОпределения, КСКонтекст) Экспорт
	...
	Если СтрНайти(ИменаСтартовыхПараметров, ",") > 0 Тогда
		...
		Для Каждого КлючЗначение Из ДанныеИПП Цикл
			...
			Если НРег(СокрЛП(КлючЗначение.Ключ) <> "password") Тогда
				СессионныйПараметр = ИнтернетПоддержкаПользователейКлиентСервер.НовыйПараметрИПП(
					КлючЗначение.Ключ,
					КлючЗначение.Значение,
					"startup");
