...
Процедура ПриУдаленииНеактуальныхСообщенийОбмена() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураЗаписи.Вставить("ОбластьДанныхВспомогательныеДанные", Выборка.ОбластьДанныхВспомогательныеДанные);
...
Процедура РегистрацияИнтерфейсовПринимаемыхСообщений(МассивОбработчиков) Экспорт
	...
	МассивОбработчиков.Добавить(СообщенияРаспределенноеВыполнениеКомандИнтерфейс);
					...
					Если Не РезультатЗапроса2.Пустой() Тогда
						...
						Если Выборка2.Следующий() Тогда
							...
							Если КонечнаяТочкаКорреспондента = Неопределено Тогда
								...
								ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не определена конечная точка корреспондента для адреса ""%1"".'"),
									Выборка2.Адрес);
							КонецЕсли;
...
Процедура ПриОтправкеДанныхПодчиненному(ЭлементДанных, ОтправкаЭлемента, Знач СозданиеНачальногоОбраза, Получатель) Экспорт
	...
	ИначеЕсли СозданиеНачальногоОбраза
		...
		И (ОбщегоНазначения.ЭтоРазделенныйОбъектМетаданных(ЭлементДанных.Метаданные(), ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных())
		ИЛИ ОбщегоНазначения.ЭтоРазделенныйОбъектМетаданных(ЭлементДанных.Метаданные(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных())) Тогда
		
		ОтправкаЭлемента = ОтправкаЭлементаДанных.Игнорировать;
		
		Если ОбщегоНазначения.ЭтоРазделенныйОбъектМетаданных(ЭлементДанных.Метаданные(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
...
Функция ДатаПоследнейУспешнойЗагрузкиДляВсехУзловИнформационнойБазы() Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиТранспортаОбменаОбластиДанных КАК НастройкиТранспортаОбмена
	|		ПО НастройкиТранспортаОбмена.Корреспондент = СостоянияУспешныхОбменовДанными.УзелИнформационнойБазы
	|ГДЕ
...
Процедура СоздатьНастройкуОбмена(
	...
	Попытка
		...
		Если Не ПустаяСтрока(ПсевдонимЭтогоУзла)
			И ВерсияБСП243 Тогда
...
Функция НайтиУзелИнформационнойБазы(Знач ИмяПланаОбмена, Знач КодУзла)
	...
	// Поиск узла по префиксу в формате "S00000123".
	Если Результат = Неопределено
		...
		Шаблон1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Exchange %1-%2", КодУзлаСПрефиксом, КодЭтогоУзла);
		Шаблон2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Exchange %1-%2", КодЭтогоУзла, КодУзлаСПрефиксом);
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.КаталогОбменаИнформацией = Шаблон1
				...
				Результат = Выборка.Корреспондент;
				Прервать;
			КонецЕсли;
		КонецЦикла;
