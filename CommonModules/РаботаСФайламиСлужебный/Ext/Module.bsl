...
Процедура ПеренестиРезультатыПроверкиПодписей(ПодписиВФорме, ПодписанныйФайл) Экспорт
	...
	МодульЭлектроннаяПодписьСлужебный = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодписьСлужебный");
	Если Не МодульЭлектроннаяПодписьСлужебный.ДоступнаЭлектроннаяПодпись(ПодписанныйФайл) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьИнформациюОШифровании(ФайлСсылка, Зашифровать, МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор, 
	...
	Попытка
		...
		Если АдресВременногоХранилищаТекстаТекущейВерсии <> "" Тогда
			Текст = ПолучитьСтрокуИзВременногоХранилища(АдресВременногоХранилищаТекстаТекущейВерсии);
...
#Область ПроверкаНаличияСсылокНаНесуществующиеФайлыВТоме
	...
	Для Каждого ВидОбъектаМетаданных Из ВидыОбъектовМетаданных Цикл
		Для Каждого ОбъектМетаданных Из ВидОбъектаМетаданных Цикл
			...
			Если Не ПроверитьОбъектПрисоединенныхФайлов(ОбъектМетаданных) Тогда
				Продолжить;
			КонецЕсли;
			ПоискСсылокНаНесуществующиеФайлыВТоме(ОбъектМетаданных, ПараметрыПроверки);
		КонецЦикла;
	КонецЦикла;
...
Функция ДобавитьФайлыВТома(ПутьКАрхивуWindows, ПутьКАрхивуLinux) Экспорт
	...
	НачатьТранзакцию();
	Попытка
		РаботаСФайламиСлужебныйВызовСервера.ДобавитьФайлыВТомаПриРазмещении(
			СоответствиеПутейФайлов, ТипХраненияФайлов);
	...
	Для Каждого ПланОбмена Из Метаданные.ПланыОбмена Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ПланОбменаОбъект.Ссылка <> ЭтотУзел Тогда
				РаботаСФайламиСлужебныйВызовСервера.УдалитьРегистрациюИзменений(ПланОбменаОбъект.Ссылка);
			КонецЕсли;
		КонецЦикла;
...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	Запрос       = Новый Запрос;
	Запрос.Текст = РаботаСФайламиСлужебныйВызовСервера.ТекстЗапросаДляИзвлеченияТекста();
	...
	Для Каждого Строка Из ТаблицаВыгрузки Цикл
		...
		Попытка
			...
			// упр. блокировка
			БлокировкаДанных = Новый БлокировкаДанных;
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(Метаданные.НайтиПоТипу(ТипЗнч(Строка.Ссылка)).ПолноеИмя());
			...
			Если ЭтоЭлементРаботаСФайлами(ФайлОбъект.Ссылка) Тогда
				...
				Если ВозможностьХранитьВерсии И ЗначениеЗаполнено(ФайлОбъект.ТекущаяВерсия.Ссылка) Тогда
					ИмяФайлаСПутем = РаботаСФайламиСлужебныйВызовСервера.ПолучитьИмяФайлаСПутемКДвоичнымДанным(ФайлОбъект.ТекущаяВерсия.Ссылка, Истина);
				Иначе
					ИмяФайлаСПутем = РаботаСФайламиСлужебныйВызовСервера.ПолучитьИмяФайлаСПутемКДвоичнымДанным(ФайлОбъект.Ссылка, Истина);
				КонецЕсли;
			КонецЕсли;
			...
			Если ПустаяСтрока(ИмяФайлаСПутем) Тогда
				...
				Текст = "";
			Иначе
				Текст = РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекст(ИмяФайлаСПутем, Отказ, Строка.Кодировка);
			КонецЕсли;
			...
			Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе
			...
			ПриЗаписиИзвлеченногоТекста(ФайлОбъект);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Обработчик.Процедура = "РаботаСФайламиСлужебныйВызовСервера.ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений";
		Обработчик.РежимВыполнения = "Отложенно";
		Обработчик.ОчередьОтложеннойОбработки = 1;
		Обработчик.ПроцедураЗаполненияДанныхОбновления = "РаботаСФайламиСлужебныйВызовСервера.ЗарегистрироватьОбъектыДляПереносаЭлектронныхПодписейИСертификатовШифрования";
		Обработчик.ЧитаемыеОбъекты      = СтрСоединить(РаботаСФайламиСлужебныйВызовСервера.ПолныеИменаСправочниковПрисоединенныхФайлов(), ", ");
		Обработчик.ИзменяемыеОбъекты    = РаботаСФайламиСлужебныйВызовСервера.ИзменяемыеОбъектыПриПереносеЭлектронныхПодписейИСертификатовШифрования();
		...
		Приоритет.Порядок = "Любой";
		Приоритет = Обработчик.ПриоритетыВыполнения.Добавить();
		Приоритет.Процедура = "РаботаСФайламиСлужебныйВызовСервера.ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы";
		Приоритет.Порядок = "Любой";
		...
		Обработчик.Процедура = "РаботаСФайламиСлужебныйВызовСервера.ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы";
		Обработчик.РежимВыполнения = "Отложенно";
		Обработчик.ОчередьОтложеннойОбработки = 2;
		Обработчик.ПроцедураЗаполненияДанныхОбновления = "РаботаСФайламиСлужебныйВызовСервера.ЗарегистрироватьОбъектыДляПереносаЭлектронныхПодписейСВерсийФайловНаФайлы";
		Обработчик.ЧитаемыеОбъекты      = РаботаСФайламиСлужебныйВызовСервера.ОбъектыПриПереносеЭлектронныхПодписейСВерсийФайловНаФайлы(Истина);
		Обработчик.ИзменяемыеОбъекты    = РаботаСФайламиСлужебныйВызовСервера.ОбъектыПриПереносеЭлектронныхПодписейСВерсийФайловНаФайлы(Ложь);
		Обработчик.ПроцедураПроверки    = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
		Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
		Приоритет = Обработчик.ПриоритетыВыполнения.Добавить();
		Приоритет.Процедура = "РегистрыСведений.ДвоичныеДанныеФайлов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
		...
		Приоритет.Порядок = "Любой";
		Приоритет = Обработчик.ПриоритетыВыполнения.Добавить();
		Приоритет.Процедура = "РаботаСФайламиСлужебныйВызовСервера.ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений";
		Приоритет.Порядок = "Любой";
	КонецЕсли;
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ЧитаемыеОбъекты      = РегистрыСведений.СведенияОФайлах.ОбновляемыеОбъекты();
	Обработчик.ИзменяемыеОбъекты    = "РегистрСведений.СведенияОФайлах";
	Обработчик.БлокируемыеОбъекты   = РегистрыСведений.СведенияОФайлах.ОбновляемыеОбъекты();
...
Процедура ПриПолученииФайла(ЭлементДанных, ПолучениеЭлемента, Отправитель = Неопределено)
	...
	Если ОбработатьПолученныеФайлы Тогда
		...
		Если ТипХраненияФайлов() = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВладелецФайла", Метаданные.НайтиПоТипу(ТипЗнч(ЭлементДанных))) Тогда
				// Это справочник файлов.
				НомерВерсии = "";
			Иначе
				// Это справочник версий файлов.
			...
			СведенияОФайле = ДобавитьФайлВТом(ЭлементДанных.ФайлХранилище.Получить(), 
...
Процедура Расшифровать(Знач ПрисоединенныйФайл, Знач РасшифрованныеДанные) Экспорт
	...
	Попытка
		...
		Если ЭтоАдресВременногоХранилища(РасшифрованныеДанные.АдресВременногоХранилищаТекста) Тогда
			ИзвлеченныйТекст = ПолучитьСтрокуИзВременногоХранилища(РасшифрованныеДанные.АдресВременногоХранилищаТекста);
			СтатусИзвлеченияТекста = Перечисления.СтатусыИзвлеченияТекстаФайлов.Извлечен;
			
		ИначеЕсли НЕ ИзвлекатьТекстыФайловНаСервере() Тогда
...
//
Функция ПолучитьКодировкуВерсииФайла(ВерсияФайла)
	
	Возврат РаботаСФайламиСлужебныйВызовСервера.ПолучитьКодировкуВерсииФайла(ВерсияФайла);
...
Функция ПолучениеФайлаЗапрещено(ЭлементДанных)
	...
	Возврат ЭлементДанных.ЭтоНовый()
	      И Не РаботаСФайламиСлужебныйКлиентСервер.ПроверитьРасширениеФайлаДляЗагрузки(
...
Процедура ОбработатьОтправкуФайлаПоТипуХранения(ЭлементДанных)
	...
	Иначе
		...
		Попытка
			...
			АдресВоВременномХранилище = РаботаСФайламиСлужебныйВызовСервера.ПолучитьНавигационнуюСсылкуВоВременномХранилище(ИсточникДвоичныхДанных);
...
Функция ИзвлечьТекст(Знач ДвоичныеДанные, Знач Расширение, ИзвлеченныйТекст) Экспорт
	...
	Если ЭтоПлатформаWindows()
		И ИзвлекатьТекстыФайловНаСервере() Тогда
		
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла(Расширение);
		ДвоичныеДанные.Записать(ИмяВременногоФайла);
		
		Отказ = Ложь;
		ИзвлеченныйТекст = РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекст(ИмяВременногоФайла, Отказ);
...
Функция СоздатьВерсию(ФайлСсылка, СведенияОФайле) Экспорт
	...
	ИначеЕсли Не ПустаяСтрока(СведенияОФайле.АдресВременногоХранилищаТекста) Тогда
		Текст = ПолучитьСтрокуИзВременногоХранилища(СведенияОФайле.АдресВременногоХранилищаТекста);
...
Процедура ОбновитьФайл(ИнформацияОФайле, ПрисоединенныйФайл) Экспорт
	...
	Если ЭтоАдресВременногоХранилища(ИнформацияОФайле.АдресВременногоХранилищаТекста) Тогда
		...
		ИзвлеченныйТекст = ПолучитьСтрокуИзВременногоХранилища(ИнформацияОФайле.АдресВременногоХранилищаТекста);
...
Процедура ФормаЭлементаПриСозданииНаСервере(Контекст, Отказ, СтандартнаяОбработка, Параметры, ТолькоПросмотр, НастраиватьОбъектФормы = Ложь) Экспорт
	...
	ИмяСправочникаХранилищаВерсийФайлов = ИмяСправочникаХраненияВерсийФайлов(
		ТипЗнч(ОбъектЗначение.ВладелецФайла), "", ЗаголовокОшибки, ОкончаниеОшибки);
	...
	Если ТолькоПросмотр
		ИЛИ НЕ ПравоДоступа("Изменение", Контекст.Объект.ВладелецФайла.Метаданные()) Тогда
...
Процедура НастроитьОбъектФормы(Знач НовыйОбъект, Контекст)
	...
	Размещение = НовыйОбъект.ВладелецФайла;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Для Каждого ЭлектроннаяПодписьФайла Из ЭлектронныеПодписи Цикл
			...
			ИмяФайлаПодписи = МодульЭлектроннаяПодписьСлужебныйКлиентСервер.ИмяФайлаПодписи(СоставИмениФайлаДанных.ИмяБезРасширения,
				Строка(ЭлектроннаяПодписьФайла.КомуВыданСертификат));
...
Функция ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	...
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Не удалось синхронизировать файл по адресу %2, т.к. сервер вернул HTTP код: %1.'");
		
		Результат.Успешно = Ложь;
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, Ответ.КодСостояния, РаскодироватьСтроку(АдресСервера, СпособКодированияСтроки.URLВКодировкеURL));
...
Функция СоздатьHTTPСоединениеWebdav(СтруктураHref, ПараметрыСинхронизации, Таймаут)
	...
	Если СтруктураHref.Схема = "https" Тогда 
		...
		ЗащищенноеСоединение = Новый ЗащищенноеСоединениеOpenSSL(, Сертификаты);
	КонецЕсли;
...
Функция ЗагрузитьФайлССервера(ПараметрыФайла, ЭтоФайл = Неопределено)
	...
	Если РезультатЗагрузки.Успешно И РезультатЗагрузки.АдресЗагруженногоФайла <> Неопределено Тогда
		...
		Иначе
			...
			Результат = РаботаСФайламиСлужебныйВызовСервера.СохранитьИзмененияФайла(СсылкаСуществующегоФайла, СведенияОФайле, Истина, "", "", Ложь);
...
Процедура ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСообщения, УчетнаяЗапись, УстанавливаемыйУровеньЖурналаРегистрации = Неопределено)
	...
	ЗаписьЖурналаРегистрации(РаботаСФайламиКлиентСервер.СобытиеЖурналаРегистрацииСинхронизация(),
	...
	Для каждого СтрокаДереваФайлов Из СтрокиДереваФайлов Цикл
		...
		Если СтрокаДереваФайлов.ЭтоПапка Тогда
			...
			Если (ТекущаяПапкаФайлов = Неопределено) И (ТаблицаФайлов.Найти(СтрокаДереваФайлов.Href, "Href") = Неопределено) Тогда
				...
				Если Не ЭтоПапкаФайлов(ОбъектВладелец) Тогда
					Продолжить;
				КонецЕсли;
...
Функция УдалитьФайлВОблачномСервисе(Знач ПараметрыСинхронизации, Знач СтрокаТаблицы)
	...
	Попытка
		...
		Если ЭтоФайл(СтрокаТаблицы.ФайлСсылка) Тогда
...
Функция ВыбратьДанныеПоПравилам(УчетнаяЗапись, Синхронизировать = ИСТИНА)
	...
	ЗапросНастройкиСинхронизации.УстановитьПараметр("Синхронизировать", Синхронизировать);
	...
	Для Каждого Настройка Из НастройкиСинхронизации Цикл
		...
		СправочникФайлов = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(Настройка.ТипВладельцаФайла);
		...
		Для Каждого СтрокаФайлов Из ДеревоФайлов.Строки Цикл
			...
			Если ВозможностьСоздаватьГруппы Тогда
				Если ЗначениеЗаполнено(СтрокаФайлов.ФайлСсылка.Родитель) Тогда
					НоваяСтрока.Родитель = СтрокаФайлов.ФайлСсылка.Родитель;
				КонецЕсли;
			КонецЕсли;
...
Процедура ОсвободитьЗахваченныеФайлы()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НЕ УчетныеЗаписиСинхронизацииФайлов.ПометкаУдаления
		|	ИЛИ НЕ УчетныеЗаписиСинхронизацииФайлов.Ссылка В
		|				(ВЫБРАТЬ
		|					НастройкиСинхронизацииФайлов.УчетнаяЗапись
		|				ИЗ
		|					РегистрСведений.НастройкиСинхронизацииФайлов КАК НастройкиСинхронизацииФайлов
...
Процедура ПриЗаписиИзвлеченногоТекста(ФайлОбъект)
...
Функция ДанныеОтбораЖурналаРегистрации(УчетнаяЗапись) Экспорт
	...
	Отбор.Вставить("СобытиеЖурналаРегистрации ", РаботаСФайламиКлиентСервер.СобытиеЖурналаРегистрацииСинхронизация());
...
Функция СведенияОСинхронизации(ВладелецФайла = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|ГДЕ
		|	СтатусыСинхронизацииФайловСОблачнымСервисом.ВладелецФайла = &ВладелецФайла";
...
Процедура ПоискСсылокНаНесуществующиеФайлыВТоме(ОбъектМетаданных, ПараметрыПроверки)
