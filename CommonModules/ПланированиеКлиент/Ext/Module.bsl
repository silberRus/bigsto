		...
		Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			...
			Для каждого Период Из Периоды Цикл
				...
				Иначе
					...
					СтрокаТЧ["Формула_" + Период.ИмяКолонки] = СтрЗаменить(ШаблонФормулы, "%ТекущаяФормула%", СтрокаТЧ["Формула_" + Период.ИмяКолонки]);
				КонецЕсли; 
				РезультатВычисления = ПланированиеКлиентСервер.ВычислитьПоФормуле(СтрокаТЧ["Формула_" + Период.ИмяКолонки], СтрокаТЧ, Периоды, Период);
	...
	Иначе
		...
		Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			...
			Иначе
				...
				СтрокаТЧ.Формула = СтрЗаменить(ШаблонФормулы, "%ТекущаяФормула%", СтрокаТЧ.Формула);
			КонецЕсли; 
			РезультатВычисления = ПланированиеКлиентСервер.ВычислитьПоФормуле(СтрокаТЧ.Формула, СтрокаТЧ, Неопределено, Неопределено);
