...
//
Процедура ПодключитьВнешнююКомпонентуПоИдентификатору(Оповещение, ИмяМодуля, ПроверятьВерсию = Истина) Экспорт
	...
	Если АдресВК = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'В информационной базе на найдена внешняя компонента %1.'");
...
//
Процедура ПодключитьВнешнююКомпонентуПоСсылке(Оповещение, АдресВК, ИмяМодуля, ПроверятьВерсию = Ложь) Экспорт
