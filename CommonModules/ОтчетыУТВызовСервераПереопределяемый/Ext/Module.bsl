...
// Вызывается для отчета "УправленческийБаланс"
//
Функция НастройкиРасшифровкиСтатьиАктиваПассива(КомпоновщикНастроек, Расшифровка, АдресРасшифровки) Экспорт
	...
	ОтборАктивовПассивов = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(Отбор);
	#КонецОбласти
...
#Область ИндивидуальныеПараметрыИОтборы
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("ДенежныеСредстваУПодотчетныхЛиц") Тогда
		СписокОтчетов.Добавить("ВедомостьПоДенежнымСредствам", НСтр("ru = 'Денежные средства у подотчетных лиц'"));
		ПараметрыОтчетаРасшифровки.Вставить("ИмяОтчета", "ВедомостьПоДенежнымСредствам");
		ПараметрыОтчетаРасшифровки.Вставить("КлючВарианта", "ДенежныеСредстваУПодотчетныхЛиц");
		ИмяПараметраПериода = "ПериодОтчета";
		// параметры текущего отчета
		ПараметрыСКД.Вставить("ДанныеОтчета", ЗначениеПараметраДанныеРасчета(ВалютаОтчета));
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("ЗадолженностьПоКредитам")
		...
		СписокОтчетов.Добавить("АнализСтатейКредитовИДепозитов", НСтр("ru = 'Обороты по кредитам и депозитам'"));
		ПараметрыОтчетаРасшифровки.Вставить("ИмяОтчета", "АнализСтатейКредитовИДепозитов");
		ПараметрыОтчетаРасшифровки.Вставить("КлючВарианта", "АнализСтатейКредитовДепозитов");
		ИмяПараметраПериода = "ПериодОтчета";
		Если Отбор.Свойство("НаправлениеДеятельности") Тогда
		...
		Если ПоляРасшифровки.Показатель = СтатьяАктиваПассива("ЗадолженностьПоКредитам") Тогда
			ХарактерДоговора.ПравоеЗначение = Перечисления.ХарактерДоговораКредитовИДепозитов.КредитИлиЗайм;
		ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("ДепозитыВБанках") Тогда
			ХарактерДоговора.ПравоеЗначение = Перечисления.ХарактерДоговораКредитовИДепозитов.Депозит;
		ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("ЗаймыВыданные") Тогда
			ХарактерДоговора.ПравоеЗначение = Перечисления.ХарактерДоговораКредитовИДепозитов.ЗаймВыданный;
		КонецЕсли;
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("ТоварыНаОптовыхСкладах")
		...
		СписокОтчетов.Добавить("АнализСебестоимостиТоваровРасширенный", НСтр("ru = 'Себестоимость товаров'"));
		ПараметрыОтчетаРасшифровки.Вставить("ИмяОтчета", "АнализСебестоимостиТоваровРасширенный");
		ПараметрыОтчетаРасшифровки.Вставить("КлючВарианта", "СебестоимостьТоваровПоПредприятию");
		// параметры текущего отчета
		ПараметрыСКД.Вставить("ДанныеОтчета", 1);// Валюте упр. учета с НДС
		
		// отбор текущего отчета
		Если Отбор.Свойство("Организация") Тогда
			Отбор.Организация.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СтруктураПредприятия.Организация");
		КонецЕсли;
		Если Отбор.Свойство("НаправлениеДеятельности") Тогда
	...
	ИначеЕсли ПоляРасшифровки.Показатель = СтатьяАктиваПассива("НезавершенноеПроизводство") Тогда
		СписокОтчетов.Добавить("АнализЗатрат", НСтр("ru = 'Производственные затраты'"));
		ПараметрыОтчетаРасшифровки.Вставить("ИмяОтчета", "АнализЗатрат");
		ПараметрыОтчетаРасшифровки.Вставить("КлючВарианта", "АнализПроизводственныхЗатрат");
		// параметры текущего отчета
		ПараметрыСКД.Вставить("ДанныеОтчета", ЗначениеПараметраДанныеРасчета(ВалютаОтчета));
...
Функция ПартииДляОтчетовАктуализированы(Знач ПараметрыФормы) Экспорт
	...
	Возврат УниверсальныеМеханизмыПартийИСебестоимости.ПартииДляОтчетовАктуализированы(ПараметрыФормы);
