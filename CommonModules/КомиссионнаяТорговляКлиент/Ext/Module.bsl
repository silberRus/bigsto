...
//  Форма - УправляемаяФорма - форма, в которой вызывается процедура.
//
...
//
// Параметры:
//  СпособРасчетаВознаграждения - Перечисление.СпособыРасчетаКомиссионногоВознаграждения - способ расчета.
//
...
//
// Параметры:
//  Объект - ДанныеФормыКоллекция - объект, в форме которого вызывается процедура расчета.
...
Функция НеобходимоРассчитатьКомиссионноеВознаграждение(ОписаниеОповещения, Форма)
	...
	Если Форма.Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Рассчитать комиссионное вознаграждение?'"),
			Кнопки,,
			Истина);
	...
	Если НеобходимоРассчитать Тогда
		...
		Если ДополнительныеПараметры.Форма.Объект.ПроцентВознаграждения = 0 Тогда
			ПоказатьВводЧисла(
				...
				НСтр("ru = 'Введите процент вознаграждения'"),
				5, 2); 
...
Функция НеобходимоОчиститьКомиссионноеВознаграждение(ОписаниеОповещения, Форма)
	...
	Если Форма.Объект.Товары.Итог("СуммаВознаграждения") <> 0 Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещения,
			НСтр("ru = 'Комиссионное вознаграждение будет очищено, продолжить?'"),
			Кнопки,,
			Истина);
