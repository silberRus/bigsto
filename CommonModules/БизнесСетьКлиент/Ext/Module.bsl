...
Процедура ОтправитьЧерезБизнесСетьПродолжить(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ИначеЕсли Не ВозможнаОтправка Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗарегистрироватьОрганизациюПослеВопроса", ЭтотОбъект, Организация);
...
Процедура ОткрытьПрофильУчастника(ПараметрыОткрытия) Экспорт
	...
	Если Отказ Тогда
		...
		ВидОперации = НСтр("ru = 'Получение профиля участника'");
		ТекстСообщения = НСтр("ru = 'Ошибка получения профиля участника 1С:Бизнес-сеть.'");
		ПодробныйТекстОшибки = ТекстСообщения;
		Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("КодСостояния")
			И Результат.Свойство("Данные") И ТипЗнч(Результат.Данные) = Тип("Структура")
			И Результат.Данные.Свойство("message") Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации,
			ПодробныйТекстОшибки, ТекстСообщения, "БизнесСеть", Ссылка);
		Возврат;
	...
	ИначеЕсли Результат.КодСостояния = 404 Тогда
		Если ЗначениеЗаполнено(Ссылка) Тогда
			Если УчастникЯвляетсяОрганизацией Тогда
				...
				ОписаниеОповещения = Новый ОписаниеОповещения("ЗарегистрироватьОрганизациюПослеВопроса", ЭтотОбъект, Ссылка);
				ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
			Иначе
	...
	ПараметрыОткрытияПрофиля = Новый Структура;
	ПараметрыОткрытияПрофиля.Вставить("Ссылка", Ссылка);
	ПараметрыОткрытияПрофиля.Вставить("Реквизиты", ПараметрыУчастника);
	ПараметрыОткрытияПрофиля.Вставить("ИНН", ПараметрыКоманды.ИНН);
	ПараметрыОткрытияПрофиля.Вставить("КПП", ПараметрыКоманды.КПП);
...
Процедура ЗарегистрироватьОрганизациюПослеВопроса(Результат, Параметры) Экспорт
	...
	ОткрытьФорму("Обработка.БизнесСеть.Форма.РегистрацияУчастников", ПараметрыОткрытия);
