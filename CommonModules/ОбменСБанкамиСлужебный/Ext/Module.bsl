...
Функция ОбработатьЭлектронныеДокументы(Параметры, АдресРезультата) Экспорт
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Отправить") Тогда
		...
		Если ТЗ_ЭД_КОтправке.Количество() > 0 Тогда
			...
			СтруктураКОтправке.БезПодписи = ВременнаяТЗ.ВыгрузитьКолонку("СсылкаНаСообщениеОбмена");
			
			Отбор = Новый Структура("ТребуетсяПодпись", Истина);
			ВременнаяТЗ = ТЗ_ЭД_КОтправке.Скопировать(Отбор);
			МассивПодписанныхСообщенийОбменаКОтправке = ВременнаяТЗ.ВыгрузитьКолонку("СсылкаНаСообщениеОбмена");
...
//
// Параметры:
//  ФродЭлемент - ОбъектXDTO - обяект, в котором требуется заполнить фрод-параметры.
	...
	Иначе
		...
		Если ЭД.Свойства().Получить("Errors") <> Неопределено Тогда
			Если ЭД.Errors.Error.Code = "106" Тогда
				ВызватьИсключение ЭД.Errors.Error.Desc;
			Иначе
				ТекстСообщения = "";
				ОбменСБанкамиСлужебныйВызовСервера.ОбработатьИзвещениеСОшибкойСбербанк(Ответ, ТекстСообщения);
				ВызватьИсключение ТекстСообщения;
...
Процедура ПолучитьОтветыПоТикетамСбербанк(Знач НастройкаОбмена, Знач МассивТикетов, Знач ИдентификаторСессии, Знач ИдентификаторОрганизации, МассивОтветов) Экспорт
	...
	МаксимальноеКоличествоЗапросов = 100;
	МассивОтветов = Новый Массив;
	...
	Пока МассивТикетов.Количество() Цикл
		...
		Для Каждого Тикет Из МассивТикетов Цикл
			...
			Счетчик = Счетчик + 1;
			Если Счетчик = МаксимальноеКоличествоЗапросов Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		МассивТикетов = ОбщегоНазначенияКлиентСервер.СократитьМассив(МассивТикетов, ОтправленныеТикеты);
...
Процедура СформироватьИОтправитьЗапросыВыписокСбербанк(Параметры, АдресРезультата) Экспорт
	...
	Для Счетчик = 1 По КоличествоДней Цикл
		...
		Для Каждого НомерСчета Из МассивБанковскихСчетов Цикл
			...
			ДайджестBase64 = ОбменСБанкамиСлужебныйВызовСервера.Дайджест(АдресФайла, Параметры.НастройкаОбмена);
...
Процедура ОтправитьДанныеВСбербанк(Знач НастройкаОбмена, Знач ИдентификаторСессии, Знач Данные, Тикеты, ТребуетсяАутентификация = Ложь, МассивСообщенийОбмена = Неопределено) Экспорт
	...
	Пока Данные.Количество() Цикл
		...
		Данные = ОбщегоНазначенияКлиентСервер.СократитьМассив(Данные, ОтправленныеДанные);
...
Процедура СохранитьСообщениеОбмена(Знач ПараметрыСообщения, НовоеСообщениеСсылка = Неопределено) Экспорт
	...
	Если ПараметрыСообщения.Свойство("ВнешнийИдентификатор")
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				ВидОперации, ТекстСообщения, ТекстСообщения, "ОбменСБанками", Выборка.Ссылка);
			Возврат;
		КонецЕсли;
	КонецЕсли;
		...
		Если НЕ ResultBank.Success = Неопределено 
			...
			Для Каждого Document Из ResultBank.Success.GetPacketResponse.Document Цикл
				...
				ИначеЕсли Document.dockind = "15" Тогда // Выписка банка
					...
					Если ЗначениеЗаполнено(СообщениеЗапрос) Тогда
						НазваниеЗапроса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СообщениеЗапрос, "ПредставлениеДокумента");
				...
				ИначеЕсли Document.dockind = "06" Тогда // настройки обмена
					...
					СоздатьНастройкуОбменаИзФайла(АдресХранилища, Организация, , , , НастройкаОбмена);
...
Процедура ОбработатьОтветИзБанка(ОтветБанка, ОтправленноеСообщениеОбмена, ПолученноеСообщениеОбмена = Неопределено, ЕстьОшибка = Неопределено) Экспорт
	...
	Иначе
		...
		НаименованиеЗапроса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтправленноеСообщениеОбмена, "ПредставлениеДокумента");
...
//
Функция СформироватьДеревоРазбора(ДанныеФайла, НаправлениеЭД = Неопределено) Экспорт
	...
	Результат = ПрочитатьФайлПоСхеме(ДанныеФайла, ДеревоРазбора, НовыйЭД);
...
Функция МассивСтатусовЭД(НастройкиОбмена) Экспорт
	...
	Если НастройкиОбмена.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
		...
		Иначе
			...
			Если НастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.ОбменЧерезВК Тогда
...
Процедура ЗагрузитьСписокDirectBank(ЗапускИзРегламентногоЗадания = Истина) Экспорт
	...
	Если НЕ ЗапускИзРегламентногоЗадания И ОбщегоНазначения.РазделениеВключено() Тогда
		Возврат;
	КонецЕсли;
...
//
Функция СоздатьНастройкуОбменаИзФайла(НастройкиЗаполнения, Организация, Недействительна = Ложь, МассивСертификатов = Неопределено, ЗагружатьВК = Истина, НастройкаОбмена = Неопределено) Экспорт
...
Функция ПрочитатьФайлПоСхеме(ДанныеФайла, ДеревоРазбора, НовыйЭД, ПоказыватьОшибки = Ложь)
...
Процедура ПрочитатьПлатежноеПоручениеXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	Если Не ПлатежныйДокумент.ПлатежиВБюджет = Неопределено Тогда
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ЭтоПлатежВБюджет",  Истина);
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
	...
	Иначе
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ЭтоПлатежВБюджет",  Ложь);
	КонецЕсли;
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
...
Процедура СформироватьПлатежноеПоручениеСбербанк(СсылкаНаДокумент, ДеревоДанных, АдресФайлаВоВременномХранилище, ИспользуетсяПодпись)
	...
	Если ЗначениеЗаполнено(ИнформацияОРасположенииБанка.НазваниеНаселенногоПункта) Тогда
		ЗаполнитьСвойствоXDTO(
			Bank, "BankCity", ИнформацияОРасположенииБанка.НазваниеНаселенногоПункта, Истина, ТекстОшибки);
		ЗаполнитьСвойствоXDTO(
	...
	Иначе
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС)
			+ НСтр("ru = 'Некорректные данные в поле ""Город"" банка организации.'");
	КонецЕсли;
	...
	Иначе
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС)
			+ НСтр("ru = 'Некорректные данные в поле ""Город"" банка контрагента.'");
	КонецЕсли;
...
Процедура ПрочитатьКвитанциюСбербанкXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	ТикетСББОЛ = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "docId");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ТикетСББОЛ", ТикетСББОЛ);
	ДатаСозданияКвитка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "createTime");
	Если ЗначениеЗаполнено(ДатаСозданияКвитка) Тогда
		ДатаБезВремени = Сред(ДатаСозданияКвитка, 1, 19);
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДатаСозданияКвитка", ДатаБезВремени);
	КонецЕсли;
	
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
		НовыйЭД, "КодСостоянияДокумента", ЭД.Info.statusStateCode);
	ИдентификаторОрганизации = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.orgId");
	Если НЕ ИдентификаторОрганизации = Неопределено Тогда
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
			НовыйЭД, "ИдентификаторОрганизации", ИдентификаторОрганизации);
	КонецЕсли;
		
	ИдентификаторДокумента = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.docExtId");
	Если НЕ ИдентификаторДокумента = Неопределено Тогда
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
			НовыйЭД, "ИдентификаторДокумента", ИдентификаторДокумента);
	КонецЕсли;
	Если НЕ ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.BankDate") = Неопределено Тогда
		ДатаСписанияСоСчетаПлательщика = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(
			ЭД, "Info.BankDate.chargeOffDate");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
			НовыйЭД, "ДатаСписанияСоСчетаПлательщика", ДатаСписанияСоСчетаПлательщика);
		ДатаПостановкиВКартотеку = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.BankDate.fileDate");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
			НовыйЭД, "ДатаПостановкиВКартотеку", ДатаПостановкиВКартотеку);
		ДатаОтметкиБанкомПлательщика = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.BankDate.signDate");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
			НовыйЭД, "ДатаОтметкиБанкомПлательщика", ДатаОтметкиБанкомПлательщика);
		ДатаПоступленияВБанкПлательщика = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(
			ЭД, "Info.BankDate.receiptDate");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
			НовыйЭД, "ДатаПоступленияВБанкПлательщика", ДатаПоступленияВБанкПлательщика);
		ДатаПеречисленияПлатежа = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.BankDate.dpp");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
			НовыйЭД, "ДатаПеречисленияПлатежа", ДатаПеречисленияПлатежа);
		ДатаОтметкиБанкомПолучателя = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.BankDate.recDate");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
			НовыйЭД, "ДатаОтметкиБанкомПолучателя", ДатаОтметкиБанкомПолучателя);
	КонецЕсли;
	Если НЕ ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.MsgFromBank") = Неопределено Тогда
		АвторСообщения = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.MsgFromBank.author");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "АвторСообщения", АвторСообщения);
		СообщениеИзБанка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Info.MsgFromBank.Message");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "СообщениеИзБанка", СообщениеИзБанка);
	КонецЕсли;
...
Процедура ПрочитатьВыпискуСбербанкXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	НаименованиеОрганизации = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "orgName");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
		НовыйЭД, "НаименованиеОрганизации", НаименованиеОрганизации);
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Получатель", НаименованиеОрганизации);
	...
	ДатаПоследнейОперации = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "lastMovetDate");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
	...
	НачальныйОстаток = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "enterBal");
	НачальныйОстаток = ?(НачальныйОстаток = Неопределено, НачальныйОстаток, Число(НачальныйОстаток));
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "НачальныйОстаток", НачальныйОстаток);
	КонечныйОстаток = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "outBal");
	КонечныйОстаток = ?(КонечныйОстаток = Неопределено, КонечныйОстаток, Число(КонечныйОстаток));
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КонечныйОстаток", КонечныйОстаток);
	...
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ВсегоСписано", ВсегоСписано);
	ВсегоПоступило = ?(ЭД.debetSum = Неопределено, ЭД.creditSum, Число(ЭД.creditSum));
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ВсегоПоступило", ВсегоПоступило);
	ДатаПредыдущейОперации = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "DatePLast");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(
		НовыйЭД, "ДатаПредыдущейОперации", ДатаПредыдущейОперации);
	КурсНаНачалоПериода = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "rateIn");
	КурсНаНачалоПериода = ?(КурсНаНачалоПериода = Неопределено, КурсНаНачалоПериода, Число(КурсНаНачалоПериода));
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КурсНаНачалоПериода", КурсНаНачалоПериода);
	КурсНаКонецПериода = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "rateOut");
	КурсНаКонецПериода = ?(КурсНаКонецПериода = Неопределено, КурсНаКонецПериода, Число(КурсНаКонецПериода));
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КурсНаКонецПериода", КурсНаКонецПериода);
	ВходящийОстатокВНациональнойВалюте = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "enterBalNat");
	...
	ИсходящийОстатокВНациональнойВалюте = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "outBalNat");
	...
	ПлановыйИсходящийОстаток = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "planOutBal");
	...
	ПлановыйИсходящийОстатокВНациональнойВалюте = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "planOutBalNat");
	...
	Номер = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "docNum");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Номер", Номер);
	УчетнаяЗапись = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "accountName");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "УчетнаяЗапись", УчетнаяЗапись);
	ДебетВНациональнойВалюте = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "debetSumNat");
	...
	КредитВНациональнойВалюте = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "creditSumNat");
	...
	Исполнитель = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "author");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Исполнитель", Исполнитель);
	ДопИнформация = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "docComment");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДопИнформация", ДопИнформация);
	...
	Если НЕ ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Docs") = Неопределено Тогда
		Операции = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "Docs.TransInfo");
	...
	Если НЕ ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "InfoForStamp") = Неопределено Тогда
		
		ПараметрыШтампа = Новый Структура;
		ПараметрыШтампа.Вставить("БИК", ЭД.bic);
		НазваниеБанка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "InfoForStamp.BankName");
		ПараметрыШтампа.Вставить("НазваниеБанка", НазваниеБанка);
		ГородБанка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "InfoForStamp.BranchName");
		ПараметрыШтампа.Вставить("ГородБанка", ГородБанка);
		Офис = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "InfoForStamp.SubBranchName");
		ПараметрыШтампа.Вставить("Офис", Офис);
		Филиал = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ЭД, "InfoForStamp.SubBranchNum");
...
Процедура ПрочитатьОперациюВыпискиСбербанк(Документ, НовыйЭД)
	...
	ДатаСписания = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "chargeOffDate");
	РеквизитыДокумента.Добавить(ДатаСписания, "ДатаСписания");
	ДатаПеречисления = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "dpp");
	РеквизитыДокумента.Добавить(ДатаПеречисления, "ДатаПеречисления");
	ДатаПостановкиВКартотеку = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "fileDate");
	РеквизитыДокумента.Добавить(ДатаПостановкиВКартотеку, "ДатаПостановкиВКартотеку");
	ДатаОтметкиБанкаПолучателя = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "recDate");
	РеквизитыДокумента.Добавить(ДатаОтметкиБанкаПолучателя, "ДатаОтметкиБанкаПолучателя");
	ДатаПоступленияВБанкПлательщика = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "receiptDate");
	РеквизитыДокумента.Добавить(ДатаПоступленияВБанкПлательщика, "ДатаПоступленияВБанкПлательщика");
	ДатаОтметкиБанкомПлательщика = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "signDate");
	РеквизитыДокумента.Добавить(ДатаОтметкиБанкомПлательщика, "ДатаОтметкиБанкомПлательщика");
	НомерДокументаБанка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "bankNumDoc");
	РеквизитыДокумента.Добавить(НомерДокументаБанка, "НомерДокументаБанка");
	КодПодразделения = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "branchCode");
	...
	Код = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "uip");
	РеквизитыДокумента.Добавить(Код, "Код");
	ВалютаПлатежа = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "docCurr");
	...
	СуммаДокументаВНациональнойВалюте = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "docSumNat");
	...
	Очередность = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "paymentOrder");
	РеквизитыДокумента.Добавить(Очередность, "Очередность");
	ВидПлатежа = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "paytKind");
	РеквизитыДокумента.Добавить(ВидПлатежа, "ВидПлатежа");
	НазначениеПлатежа = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "purpose");
	РеквизитыДокумента.Добавить(НазначениеПлатежа, "НазначениеПлатежа");
	РеквизитыДокумента.Добавить(Документ.transKind, "ВидОплаты");
	ДопУслуга = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "urgentSBRF");
	РеквизитыДокумента.Добавить(ДопУслуга, "ДопУслуга");
	ИдентификаторДокумента = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "docId");
	...
	ПолучательНаименование = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payeeName");
	...
	ПолучательИНН = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payeeINN");
	РеквизитыДокумента.Добавить(ПолучательИНН, "ПолучательИНН");
	ПолучательБИКБанка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payeeBankBic");
	РеквизитыДокумента.Добавить(ПолучательБИКБанка, "ПолучательБИКБанка");
	СчетБанкаПолучателя = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payeeBankCorrAcc");
	РеквизитыДокумента.Добавить(СчетБанкаПолучателя, "СчетБанкаПолучателя");
	ПолучательНаименованиеБанка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payeeBankName");
	РеквизитыДокумента.Добавить(ПолучательНаименованиеБанка, "ПолучательНаименованиеБанка");
	ПлательщикНаименование = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payerName");
	...
	ПлательщикИНН = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payerINN");
	РеквизитыДокумента.Добавить(ПлательщикИНН, "ПлательщикИНН");
	ПлательщикБИКБанка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payerBankBic");
	РеквизитыДокумента.Добавить(ПлательщикБИКБанка, "ПлательщикБИКБанка");
	СчетБанкаПлательщика = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payerBankCorrAcc");
	РеквизитыДокумента.Добавить(СчетБанкаПлательщика, "СчетБанкаПлательщика");
	ПлательщикНаименованиеБанка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "payerBankName");
	РеквизитыДокумента.Добавить(ПлательщикНаименованиеБанка, "ПлательщикНаименованиеБанка");
	Переоценка = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "s_TI");
	РеквизитыДокумента.Добавить(Переоценка, "Переоценка");
	ДополнительнаяИнформация = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "Info");
	РеквизитыДокумента.Добавить(ДополнительнаяИнформация, "ДополнительнаяИнформация");

	Если Не ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo") = Неопределено Тогда
		РеквизитыДокумента.Добавить(Истина, "ЭтоПлатежВБюджет");
		ПоказательКБК = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo.cbc");
		РеквизитыДокумента.Добавить(ПоказательКБК, "ПоказательКБК");
		ПоказательДаты = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo.docDate");
		РеквизитыДокумента.Добавить(ПоказательДаты, "ПоказательДаты");
		ПоказательНомера = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo.docNo");
		РеквизитыДокумента.Добавить(ПоказательНомера, "ПоказательНомера");
		СтатусСоставителя = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(
			Документ, "DepartmentalInfo.drawerStatus");
		РеквизитыДокумента.Добавить(СтатусСоставителя, "СтатусСоставителя");
		ПлательщикКПП = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo.kpp102");
		РеквизитыДокумента.Добавить(ПлательщикКПП, "ПлательщикКПП");
		ПолучательКПП = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo.kpp103");
		РеквизитыДокумента.Добавить(ПолучательКПП, "ПолучательКПП");
		КодОКАТО = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo.okato");
		РеквизитыДокумента.Добавить(КодОКАТО, "КодОКАТО");
		ПоказательОснования = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(
			Документ, "DepartmentalInfo.paytReason");
		РеквизитыДокумента.Добавить(ПоказательОснования, "ПоказательОснования");
		ПоказательТип = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo.taxPaytKind");
		РеквизитыДокумента.Добавить(ПоказательТип, "ПоказательТип");
		ПоказательПериода = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DepartmentalInfo.taxPeriod");
		РеквизитыДокумента.Добавить(ПоказательПериода, "ПоказательПериода");
	Иначе
		РеквизитыДокумента.Добавить(Ложь, "ЭтоПлатежВБюджет");
	КонецЕсли;
	
	Если Не ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc") = Неопределено Тогда
		РеквизитыДокумента.Добавить(Истина, "ЭтоКартотека");
		
		ДатаДокументаКартотека = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.docDateCard");
		...
		КонецЕсли;
		НомерДокументаКартотека = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.docNumberCard");
		РеквизитыДокумента.Добавить(НомерДокументаКартотека, "НомерДокументаКартотека");
		ШифрДокументаКартотека = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.docShifr");
		РеквизитыДокумента.Добавить(ШифрДокументаКартотека, "ШифрДокументаКартотека");
		СрокАкцепта = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.letterOfCreditAcceptDate");
		РеквизитыДокумента.Добавить(СрокАкцепта, "СрокАкцепта");
		ДополнУсловия = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.letterOfCreditAddCond");
		РеквизитыДокумента.Добавить(ДополнУсловия, "ДополнУсловия");
		ПлатежПоПредст = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(
			Документ, "DiffDoc.letterOfCreditDemandDocs");
		РеквизитыДокумента.Добавить(ПлатежПоПредст, "ПлатежПоПредст");
		НомерСчетаПоставщика = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(
			Документ, "DiffDoc.letterOfCreditPayAcc");
		РеквизитыДокумента.Добавить(НомерСчетаПоставщика, "НомерСчетаПоставщика");
		Условие1 = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.letterOfCreditPaymCond");
		РеквизитыДокумента.Добавить(Условие1, "Условие1");
		СрокПлатежа = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.letterOfCreditPeriodVal");
		РеквизитыДокумента.Добавить(СрокПлатежа, "СрокПлатежа");
		ВидАккредитива = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.docDateCard");
		РеквизитыДокумента.Добавить(ВидАккредитива, "ВидАккредитива");
		НомерПлатежа = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.NumPaymentCard");
		РеквизитыДокумента.Добавить(Документ.DiffDoc.NumPaymentCard, "НомерПлатежа");
		СодержаниеОперации = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.OperContent");
		РеквизитыДокумента.Добавить(СодержаниеОперации, "СодержаниеОперации");
		УсловиеОплаты = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.PayingCondition");
		РеквизитыДокумента.Добавить(УсловиеОплаты, "УсловиеОплаты");
		СуммаОстаткаПлатежа = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "DiffDoc.SumRestCard");
	...
	Если Не ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "Cur") = Неопределено Тогда
		
		МассивРеквизитов = СлужебныеРеквизитыВыпискиСбербанк();
		Для Каждого Реквизит Из МассивРеквизитов Цикл
			Значение = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Документ, "Cur." + Реквизит);
...
Процедура СохранитьШтампыБанка(ВыпискаБанка)
	...
	СтруктураРазбораФайла = СформироватьДеревоРазбора(ДвоичныеДанныеЭД);
...
Функция ГотовыеВыпискиСбербанка(Знач НастройкаОбмена, Знач МассивСчетов, Знач ДатаНачала, Знач ДатаОкончания, Знач ДатаЗапроса = Неопределено)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	СообщениеОбменСБанками.НомерСчета
	               |ИЗ
	               |	Документ.СообщениеОбменСБанками КАК СообщениеОбменСБанками
	               |ГДЕ
	               |	СообщениеОбменСБанками.НастройкаОбмена = &НастройкаОбмена
