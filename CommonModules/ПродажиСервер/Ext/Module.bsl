...
Функция СкорректироватьСтрокиЗаказа(СтруктураПараметров) Экспорт
	...
	|	СУММА(ЗаказыКлиентовОстатки.КОформлениюРасход)          КАК КОформлениюРасход,
	|	СУММА(ЗаказыКлиентовОстатки.СуммаРасход)                КАК СуммаРасход,
	|	СУММА(ЗаказыКлиентовОстатки.СуммаКонечныйОстаток)       КАК СуммаКонечныйОстаток
	...
	|		,, ЗаказКлиента = &ЗаказКлиента) КАК ЗаказыКлиентовОстатки
	|СГРУППИРОВАТЬ ПО
	...
	|		ИЗ
	|			РегистрНакопления.ЗаказыКлиентов.ОстаткиИОбороты(&НачПериодЗаказыКлиентов,&КонПериодЗаказыКлиентов,,,
	|				ЗаказКлиента = &ЗаказКлиента) КАК Остатки
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|		, ЗаказКлиента = &ЗаказКлиента) КАК ТоварыКОтгрузке
	|ГДЕ
	...
	Если СкладВСтроках Тогда
		Запрос.УстановитьПараметр("ТаблицаТовары",
			ТаблицаТовары.Выгрузить(,"НомерСтроки, КодСтроки, Номенклатура, Характеристика, Серия, Склад, Упаковка, Количество, СуммаСНДС, Отменено, ВариантОбеспечения"));
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"&СкладЗаказа","ВЫРАЗИТЬ(ТаблицаТовары.Склад КАК Справочник.Склады)");
	Иначе
		Запрос.УстановитьПараметр("ТаблицаТовары",
			ТаблицаТовары.Выгрузить(,"НомерСтроки, КодСтроки, Номенклатура, Характеристика, Серия, Упаковка, Количество, СуммаСНДС, Отменено, ВариантОбеспечения"));
	...
	ОтгруженныеТовары = ПакетРезультатов[6].Выгрузить();
	ОтгруженныеТовары.Свернуть("Номенклатура, Характеристика, Серия, Склад","Количество, ДопустимоеОтклонение");
	
	ОтгруженныеТовары.Индексы.Добавить("Номенклатура, Характеристика, Серия");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СкладВСтроках Тогда
			ПараметрыПоиска = Новый Структура("Номенклатура,Характеристика, Серия, Склад", 
				Выборка.Номенклатура,
				Выборка.Характеристика,
				Выборка.Серия, 
				Выборка.Склад);
		Иначе
			ПараметрыПоиска = Новый Структура("Номенклатура,Характеристика, Серия", 
				Выборка.Номенклатура,
				Выборка.Характеристика, 
				Выборка.Серия);
		КонецЕсли;
		...
		Если Выборка.КОформлениюОстаток = 0 И ОтгруженноеКоличество = 0 Тогда
			...
			СуммаОтменить = Выборка.СуммаСНДС;
		...
		Для Сч = 0 По СтрокиЗаказа.Количество()-1 Цикл
			...
			Если ОтменитьВТекущей >= Строка.Количество Тогда
				...
				ОтменитьКоличество = ОтменитьКоличество - Строка.Количество;
			...
			ИначеЕсли ОтменитьВТекущей > 0 Тогда
				...
				Реквизиты = "Сумма, СуммаНДС, СуммаРучнойСкидки, СуммаАвтоматическойСкидки";
...
Функция СообщитьОбОшибкахФормированияДанныхЗаполненияРеализации(ВыборкаРеквизитыШапки) Экспорт
	...
	Если ВыборкаРеквизитыШапки.ЕстьНесовместимыеУчетыСырьяПоНазначениям Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='В одном из выделенных распоряжений учет сырья по назначениям, несовместим с другими выделенными распоряжениями.'"),
			,
			,
			,
			Отказ);
...
Функция ЗаполнитьПоФактическойОтгрузке(ОбъектДокумента, СтруктураДокументаОтгрузки) Экспорт
	...
	ГраницыОборотов = ОбщегоНазначенияУТ.ГраницыОборотовРегистра("ЗаказыКлиентов",
																"ЗаказКлиента В (&ДокументыОтгрузки)",
																Запрос.Параметры);
	...
	ГраницыОборотов = ОбщегоНазначенияУТ.ГраницыОборотовРегистра("ЗаказыКлиентов",
																"ЗаказКлиента В (&ДокументыОтгрузки)И КодСтроки<>0",
																Запрос.Параметры);
...
Процедура ПроверитьКорректностьЗаполненияДокументаПродажи(Знач ДокументПродажи, Отказ) Экспорт
	...
	Для Каждого ТекЭлемент Из МассивПроверок Цикл
		...
		ИначеЕсли ТекЭлемент = "КорректностьУказанияТовараСОбратнымОбложениемНДСВТабличнойЧастиТовары" Тогда
			...
			СформироватьЗапросКорректностьУказанияТовараСОбратнымОбложениемНДСВТабличнойЧастиТовары(ТекстЗапроса);
			
		ИначеЕсли ТекЭлемент = "КорректностьУказанияСтавкиНДС" Тогда
			
			СформироватьЗапросКорректностьУказанияСтавкиНДС(ТекстЗапроса);
			
		ИначеЕсли ТекЭлемент = "КорректностьСуммыНДС" Тогда
			
			СформироватьЗапросКорректностьСуммыНДС(ТекстЗапроса);
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "КорректностьУказанияТовараСОбратнымОбложениемНДСВТабличнойЧастиТовары" Тогда

			СообщитьОбОшибкахКорректностьУказанияТовараСОбратнымОбложениемНДСВТабличнойЧастиТовары(Выборка, ДокументПродажи, Отказ);

		ИначеЕсли МассивПроверок[ТекИндекс] = "КорректностьУказанияСтавкиНДС" Тогда

			СообщитьОбОшибкахКорректностьУказанияСтавкиНДС(Выборка, ДокументПродажи, Отказ);
			
		ИначеЕсли МассивПроверок[ТекИндекс] = "КорректностьСуммыНДС" Тогда 
			
			СообщитьОбОшибкахКорректностьСуммыНДС(Выборка, ДокументПродажи, Отказ);
...
Процедура ПроверитьКорректностьЭтаповГрафикаОплаты(Знач ДокументПродажи,
	...
	Иначе
		...
		Если ДокументПродажи.ЭтапыГрафикаОплаты.Количество() > 0 Тогда
			...
			Если ИтогСуммаПлатежа + СуммаЗалогаВсего + ИтогСуммаОтклонения
					-(СуммаЗалогаВсего-ИтогСуммаЗалогаЗаТару) <> СуммаОплатыВсего Тогда
			...
			Если ИтогСуммаЗалогаЗаТару + ИтогСуммаОтклонения -(СуммаОплатыВсего - СуммаЗалогаВсего - ИтогСуммаПлатежа)
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "КорректностьЭтаповГрафикаОплаты" Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(Выборка.Календарь) Тогда
					...
					ДатаПлатежаШаблона = КалендарныеГрафики.ДатаПоКалендарю(Выборка.Календарь,
...
Процедура ПроверитьКорректностьВозвращаемыхТоваров(Знач ДокументПродажи, ИмяТаблицы, Отказ, ТаблицаПроверяемыеТовары = Неопределено) Экспорт
	...
	Если ТаблицаПроверяемыеТовары = Неопределено Тогда
		...
		ТаблицаПроверяемыеТовары.Свернуть("Номенклатура,Характеристика,Серия,Назначение,ДокументРеализации,СтатусУказанияСерий", "Количество, КоличествоУпаковок");
	...
	Если ТаблицаПроверяемыеТовары.Количество() > 0 Тогда
		...
		Если НЕ ВозвратПоЧеку Тогда
			...
			|			И (ПроверяемыеТовары.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
			|				ИЛИ ДанныеРеализации.Назначение = ПроверяемыеТовары.Назначение)
			|ГДЕ
...
Функция ПолучитьУсловияПродаж(Знач Соглашение,
	...
	Запрос = Новый Запрос(
		...
		СтруктураРеквизитов.НалогообложениеНДСПоДаннымОрганизации = Ложь;
		Если Не ЗначениеЗаполнено(СтруктураРеквизитов.НалогообложениеНДС)
			...
			СтруктураРеквизитов.НалогообложениеНДС = ЗначениеНастроекПовтИсп.ПолучитьНалогообложениеНДС(
				...
				СтруктураРеквизитов.Склад, 
				ТекущаяДатаСеанса());
			СтруктураРеквизитов.НалогообложениеНДСПоДаннымОрганизации = Истина;
		КонецЕсли;
...
Функция ПолучитьУсловияПродажПоУмолчанию(Знач Партнер = Неопределено, ПараметрыОтбора = Неопределено) Экспорт
	...
	Если ИспользоватьСоглашенияСКлиентами Тогда
		...
		Запрос = Новый Запрос("
			...
			|" + ?(Не ЗначениеЗаполнено(Партнер),"СоглашениеСКлиентом.Типовое И СоглашениеСКлиентом.СегментПартнеров = ЗНАЧЕНИЕ(Справочник.СегментыПартнеров.ПустаяСсылка)) ", "
			|
	...
	Иначе //соглашения не используются
		...
		СтруктураРеквизитов.Вставить("ИспользуютсяДоговорыКонтрагентов",              ИспользоватьДоговорыСКлиентами);
		СтруктураРеквизитов.Вставить("ВозможнаРеализацияБезПереходаПраваСобственности", ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности"));
	КонецЕсли;
	
	СтруктураРеквизитов.Валюта = ДоходыИРасходыСервер.ПолучитьВалютуУправленческогоУчета(СтруктураРеквизитов.Валюта);
	СтруктураРеквизитов.ВалютаВзаиморасчетов = ДоходыИРасходыСервер.ПолучитьВалютуУправленческогоУчета(СтруктураРеквизитов.ВалютаВзаиморасчетов);
...
//
Функция ПредставлениеСчетаФактурыВДокументеПродажи(Основание, Организация, НеТребуется = Ложь) Экспорт
	
	Перем РеквизитыСчетаФактуры;
	
	ПараметрыОтбора = Новый Структура("Организация", Организация);
	
	СчетаФактуры = Документы.СчетФактураВыданный.СчетаФактурыПоОснованию(Основание, ПараметрыОтбора, РеквизитыСчетаФактуры);
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
		ВозможноПробитиеЧека = Истина;
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВыкупВозвратнойТарыКлиентом") Тогда
		ВозможноПробитиеЧека = Истина;
	КонецЕсли;
	...
	Если СчетаФактуры.Количество() > 0 Тогда
		...
		Если СчетаФактуры.Количество() > 1 Тогда
			...
			СтрокаГиперссылки = Новый ФорматированнаяСтрока(
				...
				"ОткрытьСписокСчетовФактурВыданных");
			
		Иначе
			
			СтрокаГиперссылки = Новый ФорматированнаяСтрока(
				ПредставлениеСчетаФактуры(РеквизитыСчетаФактуры.Номер, РеквизитыСчетаФактуры.Дата, РеквизитыСчетаФактуры.Проведен), ,
				ЦветаСтиля.ЦветГиперссылки, ,
		...
		ТипыКорректировок = Метаданные.Документы.КорректировкаРеализации.Реквизиты.ДокументОснование.Тип;
		
		Если Не РеквизитыСчетаФактуры.Корректировочный 
				И ТипыКорректировок.СодержитТип(ТипЗнч(Основание))
				И Документы.СчетФактураВыданный.ТребуетсяСозданиеКорректировочного(РеквизитыСчетаФактуры.Ссылка) Тогда
			
			РазрешеноСоздание = ПравоДоступа("ИнтерактивноеДобавление", Метаданные.Документы.СчетФактураВыданный);
			
			СтрокаГиперссылки = Новый ФорматированнаяСтрока(
				НСтр("ru='Требуется корректировочный счет-фактура'"), , 
				WebЦвета.Кирпичный, ,
				?(РазрешеноСоздание, "СоздатьКорректировочныйСчетФактуру", ""));
	...
	ИначеЕсли НеТребуется Тогда
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			ТекстСчетФактура = НСтр("ru='Счет-фактура выданный или пробитие чека не требуется'");
		Иначе
			ТекстСчетФактура = НСтр("ru='Счет-фактура или пробитие чека не требуется'");
		КонецЕсли;
	...
	ИначеЕсли Не ПравоДоступа("ИнтерактивноеДобавление", Метаданные.Документы.СчетФактураВыданный) Тогда
		...
		Если Не ВозможноПробитиеЧека
			...
			Если ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
				ТекстСчетФактура = НСтр("ru='Счет-фактура выданный не введен'");
			Иначе
		...
		Если ВозможноПробитиеЧека И ФискальнаяОперацияДанныеЖурнала = Неопределено Тогда
			...
			Если Не ПравоДоступа("ИнтерактивноеПроведение", Основание.Метаданные())
				И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "Проведен") Тогда
				МассивСтрок.Добавить(
	...
	Иначе
		...
		Если Не ВозможноПробитиеЧека
			...
			Если ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
				ТекстСчетФактура = НСтр("ru='Оформить счет-фактуру выданный'");
			Иначе
...
Функция ПредставлениеСчетаФактуры(Номер, Дата, Проведен = Истина) Экспорт
	...
	Возврат НСтр("ru='Счет-фактура'") + ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(СтруктураШапки) + " " + ?(НЕ Проведен, НСтр("ru='(не проведен)'"),"");
...
//
// Параметры:
//  Склад - СправочникСсылка.Склады - поле формы, в котором необходимо установить режим.
//
...
Процедура ЗаполнитьРеализацииИЦены(ДокументВозврата, ИмяТаблицы) Экспорт
	...
	Для Каждого СтрокаТовары Из ТабличнаяЧастьВозврата Цикл
		НайденныеСтроки = РезультатЗапроса.НайтиСтроки(Новый Структура("Номенклатура, Характеристика, Серия, Назначение",
		...
		Для ТекИндекс = 0 По ЭтапыГрафикаОплаты.Количество()-1 Цикл
			...
			Если Не ЗначениеЗаполнено(ДанныеЭтапа.ДатаПлатежа) Тогда
				...
				Если УпрощенныйРежимДиалога Тогда
					...
					КонецЕсли;
				Иначе
					ТекстОшибки = НСтр("ru='Не заполнена колонка ""Дата платежа""'") + " " + АдресОшибки;
...
// Формирует временную таблицу, содержащую данные документа по постащику: головную организацию, КПП поставщика
//
// Параметры:
...
// Получает необходимость заполнения налогообложения из соглашения
//
// Параметры:
// 	Соглашение - СправочникСсылка.СоглашенияСКлиентами - соглашение, выбранное в документе
//
// Возвращаемое значение:
// 	Булево - флаг, если истина, то налогообложение определяется из соглашения
//
Функция НалогообложениеОпределяетсяИзСоглашения(Соглашение) Экспорт
	
	Если Не (ЗначениеЗаполнено(Соглашение) И ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСКлиентами")) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	НалогообложениеНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Соглашение, "НалогообложениеНДС");
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат ЗначениеЗаполнено(НалогообложениеНДС);
...
//
Функция ТипыСсылокДокументыРеализации() Экспорт
	
	ТипыСсылок = Новый Массив;
	Если ПравоДоступа("Чтение", Метаданные.Документы.РеализацияТоваровУслуг) Тогда
		ТипыСсылок.Добавить(СтандартныеПодсистемыПовтИсп.ИдентификаторОбъектаМетаданныхПоПолномуИмени(Метаданные.Документы.РеализацияТоваровУслуг.ПолноеИмя()));
	КонецЕсли;
	
	Если ПравоДоступа("Чтение", Метаданные.Документы.РеализацияУслугПрочихАктивов) Тогда
		ТипыСсылок.Добавить(СтандартныеПодсистемыПовтИсп.ИдентификаторОбъектаМетаданныхПоПолномуИмени(Метаданные.Документы.РеализацияУслугПрочихАктивов.ПолноеИмя()));
	КонецЕсли;
	
	Если ПравоДоступа("Чтение", Метаданные.Документы.АктВыполненныхРабот) Тогда
		ТипыСсылок.Добавить(СтандартныеПодсистемыПовтИсп.ИдентификаторОбъектаМетаданныхПоПолномуИмени(Метаданные.Документы.АктВыполненныхРабот.ПолноеИмя()));
	КонецЕсли;
	
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказКлиента)
		И ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыКлиентов") Тогда
		ТипыСсылок.Добавить(СтандартныеПодсистемыПовтИсп.ИдентификаторОбъектаМетаданныхПоПолномуИмени(Метаданные.Документы.ЗаказКлиента.ПолноеИмя()));
	КонецЕсли;
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаявкаНаВозвратТоваровОтКлиента)
		...
		ТипыСсылок.Добавить(СтандартныеПодсистемыПовтИсп.ИдентификаторОбъектаМетаданныхПоПолномуИмени(Метаданные.Документы.ЗаявкаНаВозвратТоваровОтКлиента.ПолноеИмя()));
	КонецЕсли;
...
// Переопределенное соответствие (см. ОбеспечениеКлиентСервер.ВариантыОбеспеченияПоУмолчанию) на базе статуса и соглашения документа продажи.
// 
// Параметры:
//   Соглашение - СправочникСсылка.СоглашенияСКлиентами - соглашение с клиентом документа продажи.
//   Статус - ПеречислениеСсылка.СтатусыЗаказовКлиентов, ПеречислениеСсылка.СтатусыЗаявокНаВозвратТоваровОтКлиентов - статус документа продажи.
//   ВариантПоУмолчанию - ПеречислениеСсылка.ВариантыОбеспечения - вариант обеспечения по умолчанию.
//
// Возвращаемое значение:
//  Соответствие - соответствие с ключами:
//    * ПеречислениеСсылка.ТипыНоменклатуры.Товар,
//    * ПеречислениеСсылка.ТипыНоменклатуры.МногооборотнаяТара,
//    * ПеречислениеСсылка.ТипыНоменклатуры.Работа,
//    * ПеречислениеСсылка.ТипыНоменклатуры.Услуга.
//    Значение соответствия - ПеречислениеСсылка.ВариантыОбеспечения - Вариант обеспечения, используемый по умолчанию для данного типа номенклатуры.
Функция ВариантыОбеспеченияПоУмолчанию(Соглашение, Статус, ВариантПоУмолчанию = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВариантПоУмолчанию) Тогда
		ВариантыОбеспечения = ОбеспечениеКлиентСервер.ВариантыОбеспеченияПоУмолчанию(Ложь);
		...
		КонецЕсли;
		Если ВариантПоУмолчанию = Отгрузить ИЛИ ВариантПоУмолчанию = ОтгрузитьОбособленно Тогда
			
			ВариантыОбеспечения.Товар  = Отгрузить;
			ВариантыОбеспечения.Тара   = Отгрузить;
			ВариантыОбеспечения.Работа = Отгрузить;
			ВариантыОбеспечения.Услуга = Отгрузить;
			
		ИначеЕсли ВариантПоУмолчанию = Резервировать Тогда
			ВариантыОбеспечения.Товар  = ?(ЗаполнитьОбособленно, Обособленно, Резервировать);
			ВариантыОбеспечения.Тара   = Резервировать;
			ВариантыОбеспечения.Работа = ?(ЗаполнитьОбособленно, Обособленно, НеОбеспечивать);
			ВариантыОбеспечения.Услуга = НеОбеспечивать;
		ИначеЕсли ВариантПоУмолчанию = РезервироватьКДате Тогда
			ВариантыОбеспечения.Товар  = ?(ЗаполнитьОбособленно, Обособленно, РезервироватьКДате);
			ВариантыОбеспечения.Тара   = РезервироватьКДате;
			ВариантыОбеспечения.Работа = ?(ЗаполнитьОбособленно, Обособленно, НеОбеспечивать);
			ВариантыОбеспечения.Услуга = НеОбеспечивать;
		ИначеЕсли ВариантПоУмолчанию = КОбеспечению Тогда
			ВариантыОбеспечения.Товар  = ?(ЗаполнитьОбособленно, Обособленно, КОбеспечению);
			ВариантыОбеспечения.Тара   = КОбеспечению;
			ВариантыОбеспечения.Работа = ?(ЗаполнитьОбособленно, Обособленно, НеОбеспечивать);
			ВариантыОбеспечения.Услуга = НеОбеспечивать;
		ИначеЕсли ВариантПоУмолчанию = Обособленно Тогда
		...
		ИначеЕсли ВариантПоУмолчанию = НеОбеспечивать Тогда
			ВариантыОбеспечения.Товар  = НеОбеспечивать;
			ВариантыОбеспечения.Тара   = НеОбеспечивать;
			ВариантыОбеспечения.Работа = НеОбеспечивать;
			ВариантыОбеспечения.Услуга = НеОбеспечивать;
		КонецЕсли;
		
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьПострочнуюОтгрузкуВЗаказеКлиента") Тогда
	...
	Иначе
		...
		ИначеЕсли СтатусыСоСклада.Найти(Статус) <> Неопределено Тогда
			...
			ВариантыОбеспечения.Товар              = Резервировать;
			ВариантыОбеспечения.Тара               = Резервировать;
			ВариантыОбеспечения.Работа             = НеОбеспечивать;
			ВариантыОбеспечения.Услуга             = НеОбеспечивать;
...
Функция ТекстЗапросаПоФактическойОтгрузке(СтруктураДокументаДляЗаполнения, СтруктураРезультатаЗапроса = Неопределено)
	...
	|ИЗ
	|	РегистрНакопления.ТоварыКОтгрузке.Обороты(&НачПериодТоварыКОтгрузке, &КонПериодТоварыКОтгрузке , , ДокументОтгрузки В (&ДокументыОтгрузки)) КАК ТоварыКОтгрузкеОбороты
	|ГДЕ
	...
	|ИЗ
	|	РегистрНакопления.ЗаказыКлиентов.Обороты(&НачПериодЗаказыКлиентов, &КонПериодЗаказыКлиентов, Регистратор, ЗаказКлиента В (&ДокументыОтгрузки)) КАК ЗаказыОстатки
	|ГДЕ
	...
	|	ИЗ
	|		РегистрНакопления.ЗаказыКлиентов.ОстаткиИОбороты(&НачПериодЗаказыКлиентовОстИОб, &КонПериодЗаказыКлиентовОстИОб,
	|			, , ЗаказКлиента В (&ДокументыОтгрузки) И КодСтроки<>0) КАК ЗаказыОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ИЗ
	|		РегистрНакопления.ЗаказыКлиентов.Обороты(&НачПериодЗаказыКлиентовОстИОб, &КонПериодЗаказыКлиентовОстИОб, 
	|			Регистратор, ЗаказКлиента В (&ДокументыОтгрузки)И КодСтроки<>0) КАК ЗаказыОбороты
	|	ГДЕ
...
Функция ДополнительныеТаблицыЗаказов(МассивЗаказовКлиентов, Валюта)
	...
	Если ТаблицаКурсовВалют.Количество() > 0 Тогда
		СтруктураКурсовНовойВалюты = РаботаСКурсамиВалют.ПолучитьКурсВалюты(Валюта, ТекущаяДатаСеанса());
	КонецЕсли;
...
Процедура СформироватьЗапросВременнаяТаблицаШапкаДокументаПродажи(ТекстЗапроса,
	...
	Если ПоСсылке Тогда
		...
		ИначеЕсли ИмяТаблицы = "Документ.ЗаказКлиента" Тогда
			...
			ВременныйТекст = "
				...
				|	ДокументПродажи.ФормаОплаты           КАК ФормаОплаты,
				|	ДокументПродажи.ПорядокОплаты         КАК ПорядокОплаты,
				|	ДокументПродажи.ПорядокРасчетов       КАК ПорядокРасчетов,
				|	NULL                                  КАК ВалютаВзаиморасчетов,
				|	NULL                                  КАК Регулярное,
				|	NULL                                  КАК Типовое,
	...
	Иначе
		...
		ТаблицаДокумента.Колонки.Добавить("ПорядокОплаты",                                 Новый ОписаниеТипов("ПеречислениеСсылка.ПорядокОплатыПоСоглашениям"));
...
Процедура СформироватьЗапросВременнаяТаблицаТоварыДокументаПродажи(ТекстЗапроса,
	...
	// silber {
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%ПодготовкаВременныхТаблицНаборы%", СформироватьТекстЗапросаВременныеТаблицыНаборы(ИмяТаблицы = "Документ.ЗаказКлиента"));
...
//
Процедура СформироватьЗапросКорректностьУказанияТовараСОбратнымОбложениемНДСВТабличнойЧастиТовары(ТекстЗапроса)
...
// ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса
//
Процедура СформироватьЗапросКорректностьУказанияСтавкиНДС(ТекстЗапроса)
	...
	|				ТОГДА ИСТИНА
	|			КОГДА ВременнаяТаблицаДокументПродажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя)
	|					И ВременнаяТаблицаТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)
	|				ТОГДА ИСТИНА
	|			КОГДА (ВременнаяТаблицаДокументПродажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|					ИЛИ ВременнаяТаблицаДокументПродажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД))
...
// ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса
//
Процедура СформироватьЗапросКорректностьСуммыНДС(ТекстЗапроса)
	...
	|				ВременнаяТаблицаТовары.СуммаНДС <> ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.18 /1.18) КАК ЧИСЛО(15,2))
	|			ИНАЧЕ
	|				ВременнаяТаблицаТовары.СуммаНДС <> ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.18) КАК ЧИСЛО(15,2))
	|			КОНЕЦ
	...
	|			ВЫБОР КОГДА ВременнаяТаблицаДокументПродажи.ЦенаВключаетНДС ТОГДА
	|				ВременнаяТаблицаТовары.СуммаНДС <> ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.10 /1.10) КАК ЧИСЛО(15,2))
	|			ИНАЧЕ
	|				ВременнаяТаблицаТовары.СуммаНДС <> ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.10) КАК ЧИСЛО(15,2))
	|			КОНЕЦ
	|		ИНАЧЕ
	|			ЛОЖЬ
	...
	|		КОГДА ВременнаяТаблицаТовары.СтавкаНДС В (ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)) ТОГДА
	|			ВЫБОР КОГДА ВременнаяТаблицаДокументПродажи.ЦенаВключаетНДС ТОГДА
	|				ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.18 /1.18) КАК ЧИСЛО(15,2))
	|			ИНАЧЕ
	|				ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.18) КАК ЧИСЛО(15,2))
	|			КОНЕЦ
	|		КОГДА ВременнаяТаблицаТовары.СтавкаНДС В (ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10_110)) ТОГДА
	|			ВЫБОР КОГДА ВременнаяТаблицаДокументПродажи.ЦенаВключаетНДС  ТОГДА
	|				ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.10 /1.10) КАК ЧИСЛО(15,2))
	|			ИНАЧЕ
	|				ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.10) КАК ЧИСЛО(15,2))
	|			КОНЕЦ
	|		ИНАЧЕ
	|			0
...
Процедура ВыполнитьКонтрольЗаказаПослеПроведения(КонтролируемыйЗаказ, Отказ) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|			И (ВЫРАЗИТЬ(ДокументОтгрузки КАК Документ.ЗаказКлиента).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.Закрыт)
		|				ИЛИ ВЫРАЗИТЬ(ДокументОтгрузки КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Выполнена))
...
Процедура СформироватьЗапросШапкаДокументаПродажи(ТекстЗапроса, ИмяТаблицы)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ВЫБОР
		|		КОГДА
		|			СоглашениеСКлиентом.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка) ИЛИ
		|				ДокументПродажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка) ИЛИ
		|				НЕ &ИспользоватьСоглашенияСКлиентами
		|		ТОГДА
		|			ЛОЖЬ
		...
		|	ВЫБОР
		|		КОГДА НЕ СоглашениеСКлиентом.ПорядокОплаты ЕСТЬ NULL
		|			И НЕ ДокументПродажи.ПорядокОплаты ЕСТЬ NULL
		|			И ДокументПродажи.ПорядокОплаты <> ВЫБОР
		|													КОГДА СоглашениеСКлиентом.ИспользуютсяДоговорыКонтрагентов 
		|														ТОГДА "
		+ ?(ИмяТаблицы = "Справочник.СоглашенияСКлиентами","ДокументПродажи.ПорядокОплаты","ДокументПродажи.Договор.ПорядокОплаты") + "
		|													ИНАЧЕ СоглашениеСКлиентом.ПорядокОплаты
		|												КОНЕЦ
		|		ТОГДА
		|			ИСТИНА
		|		ИНАЧЕ
		|			ЛОЖЬ
		|	КОНЕЦ КАК ЕстьОшибкиЗаполненияПорядокОплаты,
		|","
		|	ЛОЖЬ КАК ЕстьОшибкиЗаполненияПорядокОплаты,
		|") + "
		|" + ?(ИмяТаблицы <> "Документ.КоммерческоеПредложениеКлиенту" И ИмяТаблицы <> "Документ.ЗаданиеТорговомуПредставителю","
		|	ВЫБОР
		...
		+ ?(ИмяТаблицы = "Справочник.СоглашенияСКлиентами","СоглашениеСКлиентом.ВалютаВзаиморасчетов","
		|	ВЫБОР
		|		КОГДА ДокументПродажи.Договор ЕСТЬ НЕ NULL И СоглашениеСКлиентом.ИспользуютсяДоговорыКонтрагентов 
		|			ТОГДА ВЫРАЗИТЬ(ДокументПродажи.Договор КАК Справочник.ДоговорыКонтрагентов).ВалютаВзаиморасчетов
		|		ИНАЧЕ СоглашениеСКлиентом.ВалютаВзаиморасчетов
		...
		+ ?(ИмяТаблицы = "Справочник.СоглашенияСКлиентами","СоглашениеСКлиентом.ПорядокОплаты","
		|	ВЫБОР
		|		КОГДА ДокументПродажи.Договор ЕСТЬ НЕ NULL И СоглашениеСКлиентом.ИспользуютсяДоговорыКонтрагентов 
		|			ТОГДА ВЫРАЗИТЬ(ДокументПродажи.Договор КАК Справочник.ДоговорыКонтрагентов).ПорядокОплаты
		|		ИНАЧЕ СоглашениеСКлиентом.ПорядокОплаты
		|	КОНЕЦ ") + "                                                КАК ПорядокОплатыСоглашение,
...
Процедура СформироватьЗапросТоварыДокументаПродажи(ТекстЗапроса, ИмяТаблицы)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		// silber { Цену, если она из КП не сравниваем с ценой соглашения 
		
		|" + ?(ИмяТаблицы = "Документ.ЗаказКлиента", "
		|		КОГДА ЭтоИзКП ТОГДА ЛОЖЬ", "") + "
...
Процедура СформироватьЗапросКорректностьДатыПлатежа(ТекстЗапроса, ПараметрыЗапроса, ДокументПродажи, ИмяТаблицы)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ВЫБОР 
		|		КОГДА ВременнаяТаблицаДокументПродажи.ХозяйственнаяОперация = 
		|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности)
		|				И ВременнаяТаблицаДокументПродажи.Статус В (
		|					ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.ВПути),
		|					ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)) Тогда
		|			ВременнаяТаблицаДокументПродажи.ДатаПереходаПраваСобственности
		|		ИНАЧЕ
		|			ВременнаяТаблицаДокументПродажи.Дата
		|	КОНЕЦ                                                             КАК Дата,
...
// Отказ           - Булево - Флаг отказа от проведения документа
//
Процедура СообщитьОбОшибкахКорректностьСуммыНДС(Знач Выборка,
	                                                     Знач ДокументПродажи,
	                                                     Отказ)
...
//
Процедура СообщитьОбОшибкахКорректностьУказанияСтавкиНДС(Знач Выборка, Знач ДокументПродажи,Отказ)
...
Процедура СообщитьПользователюОбОшибке(ТекстОшибки, ДокументПродажи, Поле="", Отказ=Ложь)
...
Процедура СообщитьОбОшибкахШапкаДокументаПродажи(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ЕстьОшибкиЗаполненияНалогообложениеНДС Тогда
			...
			Иначе 
				Если Выборка.ЭтоСоглашение Тогда
					ТекстОшибки = НСтр("ru='""%НалогообложениеНДС%"" отличается от налогообложения в типовом соглашении ""%НалогообложениеНДССоглашение%""'");
				Иначе
					ТекстОшибки = НСтр("ru='""%НалогообложениеНДС%"" отличается от налогообложения в соглашении ""%НалогообложениеНДССоглашение%""'");
				КонецЕсли;
					
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НалогообложениеНДС%", ДокументПродажи.НалогообложениеНДС); 
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НалогообложениеНДССоглашение%", Выборка.НалогообложениеНДССоглашение);
				Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru='Налогообложение НДС'"), ТекстОшибки, МассивОтклонений);
			КонецЕсли;
		КонецЕсли;
		...
		Если Выборка.ЕстьОшибкиМинимальнаяСуммаЗаказа Тогда
			...
			Если Выборка.ВалютаСоглашение <> Выборка.Валюта Тогда
				...
				НоваяМинимальнаяСуммаЗаказа = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(
					Выборка.МинимальнаяСуммаЗаказа,
					Выборка.ВалютаСоглашение,
					Выборка.Валюта,
					СтруктураКурсовСтаройВалюты.Курс,
					СтруктураКурсовНовойВалюты.Курс,
					СтруктураКурсовСтаройВалюты.Кратность,
		...
		Если ЗначениеЗаполнено(Календарь) Тогда
			...
			Если Выборка.ВариантОплатыШаблона = Перечисления.ВариантыОплатыКлиентом.КредитПослеОтгрузки Тогда
				Если ЗначениеЗаполнено(Выборка.ЖелаемаяДатаОтгрузки) Тогда
					ДатаПлатежаШаблона = КалендарныеГрафики.ДатаПоКалендарю(Календарь, Выборка.ЖелаемаяДатаОтгрузки, Выборка.Сдвиг );
...
Процедура СообщитьОбОшибкахКорректностьДатыПлатежа(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ДатаПлатежа > ДатаПлатежаШаблона Тогда
			...
			Если ВыводитьВОтчет Тогда
				Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru='Дата платежа'"), ТекстОшибки, МассивОтклонений);
			Иначе
				СообщитьПользователюОбОшибке(
					ТекстОшибки,
					ДокументПродажи,
...
Процедура СообщитьОбОшибкахКорректностиВозвращаемыхТоваров(Знач ТаблицаОшибок, ВозвратПоЧеку)
	...
	Для каждого СтрокаОшибки Из ТаблицаОшибок Цикл
		СообщениеОбОшибке = НСтр("ru='Возврат по номенклатуре %Номенклатура% \ %Характеристика%%Серия% превышает количество реализованных товаров по документу продажи %НомерРеализации% на %Количество% %ЕдиницаИзмерения%'");
...
Функция ПолучитьТекстЗапросаВременнойТаблицыСебестоимостей(ИмяТаблицы, ИмяВременнойТаблицыТоваров)
	...
	|						 + ЕСТЬNULL(ЦеныНоменклатуры.ПостатейныеПеременныеСНДС, 0)) КАК ЧИСЛО (15,2)) 
	|		* МАКСИМУМ(ВЫБОР
	|			КОГДА &Валюта <> ВалютаУпр.Значение
	|				ТОГДА ВЫБОР
	|						КОГДА ЕСТЬNULL(КурсВалютыЦены.Кратность, 0) > 0
	|							И ЕСТЬNULL(КурсВалютыЦены.Курс, 0) > 0
	|							И ЕСТЬNULL(КурсВалютыДокумента.Кратность, 0) > 0
	|							И ЕСТЬNULL(КурсВалютыДокумента.Курс, 0) > 0
	|						ТОГДА 
	|							(КурсВалютыЦены.Курс * КурсВалютыДокумента.Кратность)
	|							/ (КурсВалютыДокумента.Курс * КурсВалютыЦены.Кратность)
	|						ИНАЧЕ 0
	|					КОНЕЦ
	|			ИНАЧЕ 1
	|		КОНЕЦ) КАК Цена
...
Функция ПолучитьТекстЗапросаВременнойТаблицыЦенПоСоглашению(ИмяТаблицы, ИмяВременнойТаблицыТоваров)
	...
	|				И НЕ РегУчетнаяПолитика.УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС)
	|			ТОГДА ВременнаяТаблицаТовары.Номенклатура.СтавкаНДС
...
Функция ШаблонУсловийПродаж()
	...
	Возврат Новый Структура("
		...
		|ЦенаВключаетНДС,
		|НалогообложениеНДС,
		|НалогообложениеНДСПоДаннымОрганизации,
		|ИспользуетсяВРаботеТорговыхПредставителей,
		|Склад,
		|СегментНоменклатуры,
		|Типовое,
...
// silber {
Функция СформироватьТекстЗапросаВременныеТаблицыНаборы(ЕстьКП = Ложь)
// } silber
	...
	// silber {
	|" + ?(ЕстьКП, "Таблица.ЭтоИзКП ЭтоИзКП,", "") + "
	// } silber
