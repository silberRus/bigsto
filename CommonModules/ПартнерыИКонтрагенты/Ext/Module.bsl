...
//
// Параметры:
//  ЮрФизЛицо  - Перечисления.ЮрФизЛицо - переданное значение.
//
// Возвращаемое значение:
...
//  СтранаРегистрации  - СправочникСсылка.СтраныРегистрации - страна регистрации контрагента для поиска в базе.
//  ИсключаяСсылку  - СправочникСсылка.Контрагенты - контрагент, который исключается при поиске.
//
// Возвращаемое значение:
...
//   *Представление  - Строка - престставление возможножного головного контрагента.
...
// Если контрагент является обосоленным подразделением и головной не заполнен, то элемент выделяется.
//
// Параметры:
...
//
// Параметры:
//  Массив - Массив - массив, содержащий партнеров, для которых необходимо выполнить запись в РС "Иерархия партнеров".
//
...
Процедура УстановитьОтборыДинамическийСписокПартнеры(Форма) Экспорт
	...
	Если Форма.СписокОтборПоТипуПартнера.Количество() > 0 Тогда
		...
		Для каждого ЭлементСписка Из Форма.СписокОтборПоТипуПартнера Цикл
			...
			Иначе
				МассивУсловийОтбора.Добавить(Метаданные.Справочники.Партнеры.Реквизиты[ЭлементСписка.Значение].Комментарий);
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьДанныеКонтрагентаПартнера(Приемник, Партнер, СИсторическимиДанными = Истина) Экспорт
	...
	|	Контрагенты.ИсторияКПП.(
	|		Период КАК Период,
	|		КПП КАК КПП
	|	)                                    КАК ИсторияКПП,
	|	Контрагенты.ИсторияНаименований.(
	|		Период КАК Период,
	...
	Если Выборка.Следующий() Тогда
		...
		ЗаполнитьЗначенияСвойств(Приемник, Выборка,,"ИсторияКПП, ИсторияНаименований");
		Если СИсторическимиДанными Тогда
			Приемник.ИсторияКПП.Загрузить(Выборка.ИсторияКПП.Выгрузить());
			Приемник.ИсторияНаименований.Загрузить(Выборка.ИсторияНаименований.Выгрузить());
		КонецЕсли;
...
Процедура ЗаполнитьКонтактнуюИнформациюКонтрагентаПартнера(КонтрагентОбъект, Форма, КонтактнаяИнформацияПартнера, ПереноситьЮридическийАдрес = Истина) Экспорт
	...
	Если ПереноситьЮридическийАдрес
		И (Не ПустаяСтрока(Форма.ЮридическийАдресКонтрагентаПредставление)
...
Процедура УправлениеЭлементамиЮридическихРеквизитов(Форма, Знач ЮрФизЛицо, Знач ОбособленноеПодразделение, Знач ИНН) Экспорт
	...
	ЭтоИндПредприниматель = (ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель);
	ЭтоНеРезидент         = (ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент);
	...
	Форма.Элементы.ГруппаСтраницыДанныхРегистрацииКонтрагента.ТекущаяСтраница = 
		?(ЭтоНеРезидент, 
		Форма.Элементы.ГруппаСтраницаИностраннойРегистрации, Форма.Элементы.ГруппаСтраницаРоссийскогоКонтрагента);
		
	Если Не ЭтоНеРезидент Тогда
		
		Если ЭтоФизЛицо Или ЭтоИндПредприниматель Тогда
			Форма.Элементы.СтраницаИННФизЛицо.Видимость = Истина;
			Форма.Элементы.СтраницаИННЮрЛицо.Видимость  = Ложь;
		Иначе
			Форма.Элементы.СтраницаИННФизЛицо.Видимость = Ложь;
			Форма.Элементы.СтраницаИННЮрЛицо.Видимость  = Истина;
		КонецЕсли;
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗаголовокИНН", "Доступность",
		ЭтоЮрЛицо Или ЭтоФизЛицо Или ЭтоИндПредприниматель);
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КодПоОКПО", "Доступность",
		ЭтоЮрЛицо Или ЭтоИндПредприниматель);
...
Процедура ОбновитьСтрокиИстории(Знач ИсторияКПП, Знач ИсторияНаименований, ПереходКИсторииКПП, ПереходКИсторииНаименования) Экспорт
	...
	ПереходКИсторииКПП = Новый ФорматированнаяСтрока(ТекстКомандыИсторииКПП,,ЦветаСтиля.ЦветГиперссылки,, "ИсторияКПП");
	...
	ПереходКИсторииНаименования = Новый ФорматированнаяСтрока(ТекстКомандыИсторииНаименования,,ЦветаСтиля.ЦветГиперссылки,, "ИсторияНаименований");
...
//
// Параметры:
//  Форма  - УправляемаяФорма - форма, для которой создаются реквизиты.
//
...
//
// Параметры:
//  Форма          - УправляемаяФорма - форма, для которой заполняются реквизиты.
...
// Изменяет отбор в форме списка или выбора справочника "Партнеры" при изменении типа панели навигации или перепозиционировании в панели навигации.
//
// Параметры:
...
// Создает группу отбора для отображения только значимых элеменов в динамическом списке,
//  который выступает панелью навигации для списка партнеров.
// Параметры:
//  ОтборДляИзменения  - ДинамическийСписок.Отбор - отбор, для которого добавляется группа.
//
// Возвращаемое значение:
...
// согласно позиционированю в панели навигации.
// Параметры:
...
// 
// Параметры:
//  ГдеИскать - ДанныеФормыДерево - дерево, в котором выполняентся поиск.
//  Значение  - Произвольный      - значение, которое ищем.
//  Колонка   - Строка            - имя колонки, в которой ищем.
//  Колонка   - Булево            - признак необходимости поиска в подчиненных строках.
//
// Возвращаемое значение:
...
Процедура ПроверитьКорректностьЗаполненияКонтрагента(Объект, Контрагент, Отказ) Экспорт
	...
	Если Объект.ОбособленноеПодразделение И ЗначениеЗаполнено(Объект.ГоловнойКонтрагент) Тогда
		...
		Иначе
			...
			СвойстваГоловногоКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				Объект.ГоловнойКонтрагент, "ЮрФизЛицо, Наименование, ОбособленноеПодразделение, ИНН");
			...
			ИначеЕсли ЗначениеЗаполнено(СвойстваГоловногоКонтрагента.ИНН) И СвойстваГоловногоКонтрагента.ИНН <> Объект.ИНН Тогда
	...
	Иначе
		...
		Если НЕ УпрощенныйВводДоступен() Тогда
...
Процедура ПриСозданииНаСервере_КонтактнаяИнформацияКонтрагентаПартнера(Форма) Экспорт
	...
	ЮридическийАдресКонтрагентаВидКонтактнойИнформации.Вставить("АдресТолькоРоссийский",        Ложь);
...
Функция ДанныеПартнераДляПанелиИнформации(Партнер) Экспорт
	...
	Если ВыборкаКоличествоКонтрагентов.Следующий() Тогда
		...
		ДанныеПартнера.ЕстьОшибкиПроверкаКонтрагентов = ВыборкаКоличествоКонтрагентов.ЕстьОшибкиПроверкаКонтрагентов;
...
//
//Параметры:
//  Форма        - УправляемаяФорма - форма, для которой формируется список выбора.
...
Процедура ДанныеКонтрагентаПоИННФоновоеЗадание(ПараметрыФормирования, АдресХранилища) Экспорт
	...
	Если ПараметрыФормирования.ЭтоЮридическоеЛицо Тогда
		РезультатФормирования = РаботаСКонтрагентами.РеквизитыЮридическогоЛицаПоИНН(ПараметрыФормирования.ИНН);
	Иначе
