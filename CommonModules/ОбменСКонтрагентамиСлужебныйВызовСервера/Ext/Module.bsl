...
Функция НастройкаЭДСуществует(СсылкаНаВладельца, ПараметрыЭД = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыЭД.Контрагент) И ЗначениеЗаполнено(ПараметрыЭД.Организация)
		И ЗначениеЗаполнено(ПараметрыЭД.ВидЭД) Тогда
			...
			|			ИЛИ Т.Ссылка.СтатусСоглашения = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийЭД.Действует))
			|
			|УПОРЯДОЧИТЬ ПО
			|	Порядок";
						...
						Если НЕ ЭлектронныйДокумент.ПроизвольныйЭД Тогда
							...
							Иначе
								...
								Иначе
									...
									ФорматЭлектронногоДокумента = ОбменСКонтрагентамиСлужебный.ФорматПоПространствуИмен(СтруктураЭД.ВидЭД,
						...
						Если Не ОбменСКонтрагентамиСлужебный.ЭтоОтветныйТитул(СтруктураЭД.ТипЭлементаВерсииЭД) Тогда
							...
							Если НЕ ЭлектронныйДокумент.ПроизвольныйЭД Тогда
								...
								Если НЕ ВРег(СпособОбработки) = ВРег("Вручную") 
									...
									СведенияОФормате = ОбменСКонтрагентамиСлужебный.СведенияОФормате(СтруктураЭД.ВидЭД, ИдентификаторФормата);
								...
								Если НЕ ((ТипЗнч(СпособОбработки) = Тип("Структура") И ВРег(СпособОбработки.СчетФактура) = ВРег("Вручную"))
									...
									Если ЗначениеЗаполнено(ДокументыУчета) Тогда
										Для каждого ДокументУчета Из ДокументыУчета Цикл
											...
											ДокументОбъект.Записать();
											ОбменСКонтрагентамиСлужебный.УстановитьСсылкуДляВладельцаВРегистреСостояний(ДокументУчета, ВладелецЭД);
										КонецЦикла;
									КонецЕсли;
	...
	Для Каждого СертификатМассивЭД Из СоответствиеСертификатовПодписаннымЭд Цикл
		...
		Для Каждого ПрисоединенныйФайл Из МассивЭДКОбновлениюСтатуса Цикл
			...
			Попытка
				...
				ПредставлениеПрогрессаПодписания = ОбменСКонтрагентамиСлужебный.ПредставлениеПрогрессаПодписания(
...
Процедура ЗаполнитьТекстПриглашенияКЭДО(ТекстПриглашения, ПараметрыЭД, СсылкаНаВладельца, Гиперссылка) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыЭД.Организация) И ЗначениеЗаполнено(ПараметрыЭД.Контрагент)
		...
		ОбменСКонтрагентамиСлужебныйВызовСервера.ОпределитьДействующуюНастройкуЭДО(СсылкаНаВладельца, ПараметрыЭлектронногоДокумента);
...
Функция ЭтоСлужебныйДокумент(ФайлЭД) Экспорт 
	...
	Если ЗначениеЗаполнено(ТипЭлементаВерсииЭД) Тогда
		...
		ОсновныеТипыЭД = ОбменСКонтрагентамиСлужебныйКлиентСервер.ОсновныеТипыЭД();
...
Функция ПерезаполнитьДокумент(Знач ПараметрыЗаполнения) Экспорт
	...
	МассивПроведенныхДокументов = ОбменСКонтрагентамиСлужебныйВызовСервера.МассивПроведенныхДокументов(
	...
	МассивСсылок = ОбщегоНазначенияКлиентСервер.СократитьМассив(
	...
	Для каждого ЭлементСоответствия Из СоответствиеВладельцевИЭД Цикл
		...
		ОбменСКонтрагентамиСлужебныйВызовСервера.ПерезаполнитьДокументыИБПоЭД(ДокументИБ, СсылкаНаЭД,
...
Функция ПолучитьДвоичныеДанныеФайла(СсылкаНаФайлЭД, СертификатПодписи) Экспорт
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		...
		// Если уже установлена хотя бы одна подпись, документ менять нельзя, иначе подпись станет невалидной.
...
Функция ВыполнитьДействияПоЭД(Знач МассивСсылокНаОбъект,
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Подписать") Тогда
		...
		|	ВТ_ЭД.Ссылка КАК СсылкаНаЭД,
		|	ВТ_ЭД.ВладелецФайла КАК ВладелецФайла,
		|	NULL КАК ПрофильНастроекЭДО,
		|	ЗНАЧЕНИЕ(Перечисление.СпособыОбменаЭД.ПустаяСсылка) КАК СпособОбменаЭД,
		|	ИСТИНА КАК СертификатДоступен
		|ИЗ
		|	ВТ_ЭД КАК ВТ_ЭД
		...
		ПрофилиЭДОБезСертификатов = Результат[5];
		ДоступныеСертификаты = Результат[6].Выгрузить();
		ТЗ_Сертификатов = Результат[10].Выгрузить();
		ТЗ_УстановленныхПодписей = Результат[11].Выгрузить();
		...
		ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.РаботаВМоделиСервиса.ЭлектроннаяПодписьВМоделиСервиса") Тогда
			...
			Если МодульЭлектроннаяПодписьВМоделиСервисаКлиентСервер.ИспользованиеВозможно()
				И НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ИспользованиеСертификатовОблачногоСервисаВозможно() Тогда
...
Процедура ИзменитьПоСсылкеПрисоединенныеФайлы(Файлы, СтруктураЭД, ПоверятьОбязательныеРеквизиты = Истина) Экспорт
	Для Каждого Файл Из Файлы Цикл
		ИзменитьПоСсылкеПрисоединенныйФайл(Файл, СтруктураЭД, ПоверятьОбязательныеРеквизиты);
	КонецЦикла;	
КонецПроцедуры	
...
// Подготавливает параметры открытия и имя формы для выбора договора.
//
// Параметры:
//  ИмяФормыВыбора			 - Строка - в данную переменную будет возвращено имя формы для открытия.
//  ПараметрыОткрытия		 - Структура - в эту переменную будет возвращены параметры открытия формы.
//  ПараметрыОтбора			 - Структура - содержит значения отбора со свойствами:
//     * Организация - ОпределяемыйТип.Организация - отбор договора по организации;
//     * Контрагент  - ОпределяемыйТип.УчастникЭДО - отбор договора по контрагенту.
//  ПараметрыОбработкиОшибки - Структура - содержит параметры для записи ошибки:
//     * ВидОперации - Строка - вид операции;
//     * ТекстСообщения - Строка - текст сообщения для пользовательского режима.
// 
// Возвращаемое значение:
//  Булево - Истина, подготовка параметров прошла успешно.
//
Функция ПодготовитьПараметрыДляВыбораДоговора(ИмяФормыВыбора, ПараметрыОткрытия, ПараметрыОтбора, ПараметрыОбработкиОшибки) Экспорт

	Результат = Ложь;
	
	Организация = Неопределено;
	Контрагент = Неопределено;
	ПараметрыОтбора.Свойство("Организация", Организация);
	ПараметрыОтбора.Свойство("Контрагент", Контрагент);
	
	ВидОперации = ПараметрыОбработкиОшибки.ВидОперации;
	ТекстСообщения = ПараметрыОбработкиОшибки.ТекстСообщения;
	
	// Подготовим параметры открытия формы в зависимости от архитектуры конфигурации.
	ТипыДоговора = Метаданные.ОпределяемыеТипы.ДоговорСКонтрагентом.Тип.Типы();
	
	Если ТипыДоговора.Количество() Тогда
		ТипДоговора = ТипыДоговора[0];
		Если ОбщегоНазначения.ЭтоСсылка(ТипДоговора) Тогда
			ПараметрыОткрытия = Новый Структура;
			
			ДоговорКонтрагентМетаданные = Метаданные.НайтиПоТипу(ТипДоговора);
			КонтрагентМетаданные = Метаданные.НайтиПоТипу(ТипЗнч(Контрагент));
			ЭлементОтбора = Неопределено;
			Если Не (КонтрагентМетаданные = Неопределено) Тогда 
			...
			Если ДоговорКонтрагентМетаданные.Реквизиты.Найти("Организация") <> Неопределено
				И ДоговорКонтрагентМетаданные.Реквизиты.Организация.Тип.СодержитТип(ТипЗнч(Организация))Тогда
			...
			Если ЭлементОтбора <> Неопределено Тогда
				СтруктураОтбора.Вставить(ЭлементОтбора, Контрагент);
			КонецЕсли;
			...
			ИмяФормыВыбора = "Справочник."
				+ ДоговорКонтрагентМетаданные.Имя + "."
				+ ДоговорКонтрагентМетаданные.ОсновнаяФормаДляВыбора.Имя;
				
			Результат = Истина;
		Иначе			
			ТекстОшибки = НСтр("ru = 'Некорректно настроена библиотека БЭД. Проверьте определяемый тип ""Договор с контрагентом""'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ТекстОшибки, ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьНедоступныеДляФормированияЭДОбъекты(МассивСсылок)
	...
	|			ИЛИ СостоянияЭД.ЭлектронныйДокумент.СостояниеЭДО <> ЗНАЧЕНИЕ(Перечисление.СостоянияВерсийЭД.ТребуетсяУточнитьДокумент))";
...
//
Функция ОпределитьСводнуюИнформациюПоСтатусуЭД(ФайлЭД) Экспорт
...
Функция ПодписатьЭДОпределеннымСертификатом(ДобавленныеФайлы,
	...
	ПодписанныеЭД = ОбщегоНазначенияКлиентСервер.СократитьМассив(ФайлыНаПодпись, МассивНеПодписанныхЭД);
...
Функция ПараметрыОтклоненияЭД(МассивЭД, ФормироватьУОУ = Ложь) Экспорт
	...
	|									ТОГДА ВЫБОР
	|											КОГДА ЭДПрисоединенныеФайлы.СтатусЭД = ЗНАЧЕНИЕ(Перечисление.СтатусыЭД.Получен)
	|												ТОГДА ИСТИНА
	|											ИНАЧЕ ЛОЖЬ
	|										КОНЕЦ
		...
		Если НЕ ЭДОтклонен(Выборка.СтатусЭД) Тогда
			...
			ИначеЕсли НЕ ЭтоСлужебныйДокумент(Выборка.ТипЭлементаВерсииЭД)
				...
				И НЕ (Выборка.ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД 
					И Выборка.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий) Тогда
				МожноОтклонить = Истина;
				ФормироватьУОУТекущее = (НЕ Выборка.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий);
			КонецЕсли;
			
			Если МожноОтклонить Тогда
				ВозвращаемоеЗначение.Добавить(ОбменСКонтрагентамиСлужебныйКлиентСервер.НовыеПараметрыОтклоненияАннулирования(
...
Функция ПараметрыАннулированияЭД(МассивЭД) Экспорт
	...
	Запрос.УстановитьПараметр("ОсновныеТипыЭД", ОбменСКонтрагентамиСлужебныйКлиентСервер.ОсновныеТипыЭД());
