...
Процедура ВыполнитьОбменССайтом(Параметры, РезультатОбмена, ТаблицаИнформации) Экспорт
	...
	Если Параметры.ВыгружатьНаСайт Тогда
		...
		// ( Storverk Тюняков Разделим каталоги выгрузки для разных серверных баз
		...
		Если ПараметрыСоединения.Свойство("Ref") И ПараметрыСоединения.Свойство("Srvr") Тогда
			...
			КаталогВыгрузки = КаталогВыгрузки + Infobase + "\";
		КонецЕсли;
		// ) Storverk Тюняков Разделим каталоги выгрузки для разных серверных баз
	...
	//офинт.ру 29.10.2015 +++	
	Если Параметры.УзелОбмена.ОбменКонтрагентами Тогда
		Успешно = ВыгрузитьКонтрагентов(Параметры, ТаблицаИнформации);
	КонецЕсли;
	
	Если Параметры.УзелОбмена.ОбменМенеджерами Тогда
		Успешно = ВыгрузитьМенеджеров(Параметры, ТаблицаИнформации);		
	КонецЕсли;
...
Процедура ЗаполнитьДанныеВладельцаКаталога(Параметры, ДанныеОрганизацииВладельцаКаталога)
	...
	ИмяРеквизитаИНН = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННОрганизации");
	ИмяРеквизитаКПП = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("КППОрганизации");
	ИмяРеквизитаНаименование = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("НаименованиеОрганизации");
	ИмяРеквизитаПолноеНаименование = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ПолноеНаименованиеОрганизации");
	ИмяРеквизитаЮрФизЛицо = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ЮридическоеФизическоеЛицо");
...
Процедура ЗаполнитьТаблицуДляОтправкиДаннымиНоменклатуры(МассивНоменклатуры, СтруктураИсходныхТаблиц, ТаблицаПриемник)
	...
	Для Каждого ТекСтрока Из СтруктураИсходныхТаблиц.Номенклатура Цикл
		...
		Свойства.Колонки.Добавить("ЗначениеСвойстваТекст", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(0, ДопустимаяДлина.Переменная)));
		//изм офинт.ру 15.10.2015 +++
		ТекАналоги 	= "";
		Первый 		= Истина;
		...
		Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ Артикул ИЗ РегистрСведений.АналогиТоваров.СрезПоследних(,Номенклатура = &Номенклатура) ГДЕ НЕ Удалено");
		Запрос.УстановитьПараметр("Номенклатура", ТекСтрока.Номенклатура);
		ТекАналог = СтрШаблон("АНАЛОГИ %1", СтрСоединить(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Артикул"), " "));
		
		////РегистрАналогов = РегистрыСведений.СТ_Аналоги.СрезПоследних(ТекДата, Отбор);
		//РегистрАналогов = РегистрыСведений.АналогиТоваров.СрезПоследних(ТекДата, Отбор);
		//// } silber
		//
		//Если РегистрАналогов.Количество() > 0 Тогда
		//	Для Каждого СтрАналог Из РегистрАналогов Цикл
		//		//silber закоментарил Если СтрАналог.Действует Тогда 
		//		Если Не СтрАналог.Удалено Тогда
		...
		//			КонецЕсли;
		//			// silber {
		//			//ТекАналог = ТекАналог + СокрЛП(СтрАналог.Аналог); 	
		//			ТекАналог = ТекАналог + СокрЛП(СтрАналог.Артикул); 	
		//		КонецЕсли;
		//		//КонецЕсли;
		//		// } silber
		//	КонецЦикла;
		...
		НовСвойство 					= Свойства.Добавить();
		НовСвойство.ЗначениеСвойства 	= ТекАналог;
		НовСвойство.Номенклатура 		= ТекСтрока.Номенклатура;
		НовСвойство.ЗначениеСвойстваТекст = ТекАналог;
		
		НовСвойство 					= Свойства.Добавить();
		НовСвойство.ЗначениеСвойства 	= "codeforbitrix " + СокрЛП(ТекСтрока.Номенклатура.Код);
		НовСвойство.Номенклатура 		= ТекСтрока.Номенклатура;
...
Функция ВыгрузитьКаталог(Параметры, СтрокаТаблицыКаталога, ТаблицаИнформации, КаталогИПакетПредложений = Ложь)
	...
	ТаблицаДляФормированияКаталога.Колонки.Добавить("Характеристики"); // в эту колонку поместим таблицу значений, содержащую характеристики
...
Процедура ДобавитьСвойстваНоменклатурыВКлассификаторXDTO(КлассификаторXDTO, ПакетCML, ВыборкаСвойствНоменклатуры) Экспорт
	...
	// ( изм офинт.ру +++ 14.10.2015 добавляем свойства Аналоги
	ТипСвойство = ПакетCML.Получить("Свойство");
	СвойствоXDTO = ФабрикаXDTO.Создать(ТипСвойство);
	
	СвойствоXDTO.ИД = Строка("analogforbitrix");
	СвойствоXDTO.Наименование = ФорматНаименованияДляXDTO("Аналоги");
	...
	СвойствоXDTO.ИД = Строка("codeforbitrix");
	СвойствоXDTO.Наименование = ФорматНаименованияДляXDTO("Код");
...
Процедура ДобавитьНоменклатуруXDTO(ТоварыXDTO, ПакетCML, ДанныеНоменклатуры, Параметры)
	...
	// silber {
	Если ЗначениеЗаполнено(ДанныеНоменклатуры.Номенклатура.Производитель) Тогда
	
	
		//Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеНоменклатуры.НоменклатураПроизводитель) Тогда
		Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеНоменклатуры.Номенклатура.Производитель) Тогда
			//ИДИзготовитель = УникальныйИдентификаторОбъекта(ДанныеНоменклатуры.НоменклатураПроизводитель)
			ИДИзготовитель = УникальныйИдентификаторОбъекта(ДанныеНоменклатуры.Номенклатура.Производитель)
		Иначе
		...
		ИзготовительXDTO.Наименование = Строка(ДанныеНоменклатуры.Номенклатура.Производитель);
		// } silber
	...
	//изм офинт.ру
	ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, 
		"Производитель", ДанныеНоменклатуры.Номенклатура.Производитель);
		
	Если ЗначениеЗаполнено(ДанныеНоменклатуры.Номенклатура.Производитель) Тогда
		ИдПроизв = УникальныйИдентификаторОбъекта(ДанныеНоменклатуры.Номенклатура.Производитель);
	Иначе
		ИдПроизв = "";
	КонецЕсли;
		
	ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, 
		"ПроизводительИД", ИдПроизв);
...
Процедура ДобавитьАдресаИзображенийНоменклатурыXDTO(ТоварXDTO, ДанныеНоменклатуры, ОписанияФайлов, Параметры)
	...
	Для Каждого ТекФайл Из ТаблицаФайлов Цикл
		...
		Если РасширенияКартинок.Найти(НРег(ТекФайл.НоменклатураФайлРасширение)) = Неопределено Тогда
		// ) Storverk Тюняков  
			Продолжить;
		КонецЕсли;
...
Процедура ДобавитьЗначенияСвойствНоменклатурыXDTO(ТоварXDTO, ТаблицаСвойств)
	...
	Для Каждого СтрокаТаблицыСвойств Из ТаблицаСвойств Цикл
		...
		//изм офинт.ру 15.10.2015 +++
		Если Найти(СтрокаТаблицыСвойств.ЗначениеСвойства, "АНАЛОГИ") > 0 Тогда
			ЗначенияСвойстваXDTO.Ид = Строка("analogforbitrix");
			//ЗначениеСвойства = Лев(СтрЗаменить(СтрокаТаблицыСвойств.ЗначениеСвойства,"АНАЛОГИ ",""), 1000);			
			ЗначениеСвойства = СтрЗаменить(СтрокаТаблицыСвойств.ЗначениеСвойстваТекст,"АНАЛОГИ ","");			
		ИначеЕсли Найти(СтрокаТаблицыСвойств.ЗначениеСвойства, "codeforbitrix") > 0 Тогда
		    ЗначенияСвойстваXDTO.Ид = Строка("codeforbitrix");
			ЗначениеСвойства = Строка(СтрЗаменить(СтрокаТаблицыСвойств.ЗначениеСвойства,"codeforbitrix ",""));
		Иначе	
		// ) изм офинт.ру 15.10.2015 +++
		...
		КонецЕсли; //изм офинт.ру 15.10.2015 +++
...
Функция ЗагрузитьЗаказыССайта(Параметры, ОписаниеОшибки)
	...
	ОтветСервера = ПолучитьДанныеССервера(
		...
		ОтветСАвторизацией(АдресДляРаботы + Параметры.НастройкиПодключения.ПараметрЗапросаHTTP_ПолучитьДанные, ОтветСервера),
		//} silber
		ЗаголовкиЗапросов);
...
Функция ЗагрузитьЗаказы(ДанныеЗаказов, Параметры, ОписаниеОшибки) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка документов с сайта'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Для Каждого СсылкаНаДокумент Из СтруктураСтатистики.Загружено Цикл
		//офинт заказы
		Если СтруктураСтатистики.Создано.Найти(СсылкаНаДокумент) = Неопределено Тогда
		//офинт заказы
	...
	Для Каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Иначе
			...
			//									СвойстваЗаказа, Параметры, ОписаниеОшибки);
												СвойстваЗаказа, Параметры, ОписаниеОшибки, Успешно);
			Если НЕ Успешно Тогда
				Продолжить;									
				Возврат Ложь;
			КонецЕсли;
			...
			//		СтрТовар.ВариантОбеспечения	= Перечисления.ВариантыОбеспечения.Требуется;
			//		Если НЕ ЗначениеЗаполнено(СтрТовар.ДатаОтгрузки) Тогда
			//		    СтрТовар.ДатаОтгрузки = ДокументОбъект.Дата;											
			//		КонецЕсли; 
			//	КонецЦикла;	
			//	
			//	Попытка
...
Процедура ИзвлечьСвойстваЗаказаИзДанныхДокументаXDTO(СтруктураРеквизитыЗаказа, ДокументXDTO,
	...
	// ( офинт.ру
	ИдентифицироватьКонтрагента(СтруктураРеквизитыЗаказа, ДанныеКонтрагента,
															 Параметры.ПрикладныеПараметры, ОписаниеОшибки);
															 
															 
	СтруктураРеквизитыЗаказа.Вставить("Соглашение", Неопределено);
	
	Попытка
		УникальныйИдентификаторСогл = Новый УникальныйИдентификатор(ДокументXDTO.Соглашение);
		СтруктураРеквизитыЗаказа.Соглашение = Справочники.СоглашенияСКлиентами.ПолучитьСсылку(УникальныйИдентификаторСогл);
		Если Найти(СокрЛП(СтруктураРеквизитыЗаказа.Соглашение), "Объект не найден") Тогда
			СтруктураРеквизитыЗаказа.Соглашение = Справочники.СоглашенияСКлиентами.ПустаяСсылка();
		КонецЕсли;
	Исключение
		СтруктураРеквизитыЗаказа.Соглашение = Справочники.СоглашенияСКлиентами.ПустаяСсылка();
	КонецПопытки;
	...
	Если СтруктураРеквизитыЗаказа.Контрагент = Справочники.Контрагенты.ПустаяСсылка() ИЛИ  СтруктураРеквизитыЗаказа.Контрагент = Неопределено Тогда
		Успешно = Ложь;
		Возврат;
	КонецЕсли;
	// ) офинт.ру															 
	...
	Если НЕ ДокументXDTO.Свойства().Получить("Склады") = Неопределено Тогда
		...
		Если ТипЗнч(ДокументXDTO.Склады) = Тип("СписокXDTO") И ДокументXDTO.Склады.Количество() Тогда
			ИзвлечьРеквизитыСклада(ДокументXDTO.Склады[0].Склад, ДанныеСклада);
		Иначе
...
Процедура ЗаполнитьДанныеКонтрагента(ДанныеКонтрагента, ДокументXDTO)
	...
	//офинт.ру+++
	Попытка
		ДанныеКонтрагента.Вставить("ИдКлиента", КонтрагентXDTO.ИдКлиента);
		ДанныеКонтрагента.Вставить("ИдКонтрагента", КонтрагентXDTO.ИдКонтрагента);
	Исключение
		ДанныеКонтрагента.Вставить("ИдКлиента", "");
		ДанныеКонтрагента.Вставить("ИдКонтрагента", "");
	КонецПопытки;
	//офинт.ру ---
...
Процедура ИзвлечьРеквизитыСклада(СкладXDTO, ДанныеСклада)
	...
	// /* Storverk Тюняков 7167 обновление 11.1.9.51 - 11.1.10.199
	// Закоментировал типовой код (при обмене вываливается по ошибке т.к. кроме ид в реквизитах склада больше ничего нет)
	...
	//АдресСклада.Вставить("ТипАдреса", СкладXDTO.Адрес.АдресноеПоле.Тип);
	//АдресСклада.Вставить("ЗначениеАдреса", СкладXDTO.Адрес.АдресноеПоле.Значение);
	АдресСклада.Вставить("ТипАдреса");
	АдресСклада.Вставить("ЗначениеАдреса");
	ДанныеСклада.Вставить("Адрес", АдресСклада);
	...
	ДанныеСклада.Вставить("Контакты", КонтактыСклада);
	//ДанныеСклада.Вставить("Наименование", СкладXDTO.Наименование);
	ДанныеСклада.Вставить("Наименование");
	// */ Storverk Тюняков 7167 обновление 11.1.9.51 - 11.1.10.199
КонецПроцедуры
...
Функция ИдентифицироватьНоменклатуру(РеквизитыЗаказа, ДокументXDTO, Параметры, ОписаниеОшибки)
	...
	Для каждого ТоварXDTO Из МассивТоваровXDTO Цикл
		...
		ИначеЕсли ОбъектXDTOСодержитСвойство(ТоварXDTO, "Скидка") И ТоварXDTO.Скидка <> Неопределено Тогда
			СуммаСкидки = Число(ТоварXDTO.Скидка); 
		...
		Попытка
			Если ТипЗнч(ДокументXDTO.Склады) = Тип("СписокXDTO") И ДокументXDTO.Склады.Количество() Тогда
				УникальныйИдентификаторСклада = Новый УникальныйИдентификатор(ДокументXDTO.Склады[0].Склад.ид);
			Иначе
				УникальныйИдентификаторСклада = Новый УникальныйИдентификатор(ТоварXDTO.Склад);
			КонецЕсли;
		//	УникальныйИдентификаторСклада = Новый УникальныйИдентификатор(ТоварXDTO.Склад);
			Склад = Справочники.Склады.ПолучитьСсылку(УникальныйИдентификаторСклада);
			Если Найти(СокрЛП(Склад), "Объект не найден") Тогда
		...
		Попытка
			УникальныйИдентификаторЦен = Новый УникальныйИдентификатор(ТоварXDTO.ТипЦены);
			ТекТипЦен = Справочники.СоглашенияСКлиентами.ПолучитьСсылку(УникальныйИдентификаторЦен);
			Если Найти(СокрЛП(ТекТипЦен), "Объект не найден") Тогда
				ТипЦены = Справочники.ВидыЦен.ПустаяСсылка();
			Иначе
				ТипЦены = ТекТипЦен.ВидЦен;
			КонецЕсли;
		Исключение
			ТипЦены = Справочники.ВидыЦен.ПустаяСсылка();
		КонецПопытки;
		...
		НоваяСтрока.ВидЦены = ТипЦены;
		
		НоваяСтрока.ЦенаСоСкидкой = ?(ЦенаЗаЕдиницу > 0, ЦенаЗаЕдиницу, НоваяСтрока.Сумма / НоваяСтрока.Количество);
		
		//НоваяСтрока.ПроцентАвтоматическойСкидки = ЧислоИзСтроки(ТоварXDTO.Скидка);
		//ТекЦена = ?(ЦенаЗаЕдиницу > 0, ЦенаЗаЕдиницу, НоваяСтрока.Сумма / НоваяСтрока.Количество);
		//Если НоваяСтрока.ПроцентАвтоматическойСкидки <> 0 Тогда
		//	НоваяСтрока.Цена = ТекЦена / (1-НоваяСтрока.ПроцентАвтоматическойСкидки/100);
		//Иначе
...
Функция ИнициализироватьТаблицуЗапасов()
	...
	ТаблицаЗапасы.Колонки.Добавить("ПроцентАвтоматическойСкидки");
	ТаблицаЗапасы.Колонки.Добавить("ВидЦены");
	ТаблицаЗапасы.Колонки.Добавить("ЦенаСоСкидкой");
...
Процедура ДобавитьТоварыЗаказаXDTO(ДокументXDTO, ТекСтрока, ДеревоСвойствХарактеристик, Параметры)
	...
	// silber { ХакТул: Врезаемся в и меняем таблицу заказа таблицой из КП
	ПодменитьСтрокуЗаказа(ТекСтрока);
	// } silber
...
Процедура ДобавитьЗначенияРеквизитовЗаказаXDTO(ДокументXDTO, ПакетCML, ДанныеДокумента, Параметры)
	...
	//изм ЯАЮ 23.03.2016 +++
	Если СокрЛП(ДанныеДокумента.ЗаказСтатусЗаказа) = "Готов к отгрузке" Тогда
		ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, "СтатусЗаказа", "P");
	ИначеЕсли СокрЛП(ДанныеДокумента.ЗаказСтатусЗаказа) = "Закрыт" Тогда
		ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, "СтатусЗаказа", "F");	
	ИначеЕсли СокрЛП(ДанныеДокумента.ЗаказСтатусЗаказа) = "В процессе отгрузки" Тогда // storverk
		ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, "СтатусЗаказа", "P");	
	Иначе	
		Если ДанныеДокумента.ЗаказСтатусЗаказа = Перечисления.СтатусыЗаказовКлиентов.КОтгрузке или ДанныеДокумента.ЗаказСтатусЗаказа = Перечисления.СтатусыЗаказовКлиентов.КОбеспечению Тогда 
			ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, "СтатусЗаказа", "P");
		иначе
			ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, "СтатусЗаказа", ДанныеДокумента.ЗаказСтатусЗаказа);
		КонецЕсли;
	КонецЕсли;
	...
	//изм ЯАЮ 23.03.2016 +++															
	Если ДанныеДокумента.ЗаказПроведен И ДанныеДокумента.ЗаказСтатусЗаказа = Перечисления.СостоянияЗаказовКлиентов.Закрыт Тогда
...
Функция ВыгрузитьНаСайт(Параметры,
	...
	ОтветСервера = ПолучитьДанныеССервера(
		...
		АдресДляРаботы + НастройкиПодключения.ПараметрЗапросаHTTP_Инициализация +
		ОтветСАвторизацией(АдресДляРаботы + НастройкиПодключения.ПараметрЗапросаHTTP_Инициализация, ОтветСервера, СесияИД),
		//} silber
		ЗаголовкиЗапросов);
	...
	Для Каждого ТекФайл Из СписокФайловДляОтправки Цикл
		...
		ОтветСервера = ОтправитьФайлНаСервер(ТекФайл.Значение,
			...
			ОтветСАвторизацией(АдресДляРаботы + НастройкиПодключения.ПараметрЗапросаHTTP_ПередачаФайла + ТекФайл.Представление, ОтветСервера, СесияИД),
			// } silber
			ЗаголовкиЗапросов);
		...
		Для Каждого ТекФайл Из МассивИсходныхXMLФайлов Цикл
			...
			Пока ИмпортПродолжается Цикл
				...
				ОтветСервера = ПолучитьДанныеССервера(
					...
					ОтветСАвторизацией(АдресДляРаботы + НастройкиПодключения.ПараметрЗапросаHTTP_ИмпортФайлаСервером + ТекФайл.Имя, ОтветСервера, СесияИД),
					// } silber
					ЗаголовкиЗапросов);
...
Функция ОтветСАвторизацией(Суффикс, ОтветСервера, СесияИД = "")
	
	Если СтрЧислоСтрок(ОтветСервера) > 3 И СтрНачинаетсяС(СтрПолучитьСтроку(ОтветСервера, 4), "sessid=") Тогда
		СесияИД = "&" + СтрПолучитьСтроку(ОтветСервера, 4);
	КонецЕсли;
	
	return Суффикс + СесияИД;
		
КонецФункции

#Область ОФИНТ

// ВНИМАНИЕ!!! В пакете XDTO CML208 (urn:1C.ru:commerceml_2) внесено изменение:
//	Увеличена максимальная длина типа значений "ЗначениеТип" с 1000 до 3000.
//	Это сделано для того, чтобы сохранить текущий нетиповой механизм выгрузки 
//	свойства "Аналоги" для номенклатуры с аналогами более 1000 символов
...
Функция ВыгрузитьДанныеНаСайт_ОФИНТ(Параметры, СтрокаТаблицыИнформации, ТипСоединения = Неопределено)
	...
	Успешно = ВыгрузитьНаСайт_ОФИНТ(Параметры, МассивПодкаталогов, ОписаниеОшибки, Истина, ТипСоединения);
...
//	Булево				- Истина, если успех.
//
Функция ВыгрузитьНаСайт_ОФИНТ(Параметры,
	...
	АдресДляОтправки = НастройкиПодключения.АдресСкрипта + "?type=" + "catalog"; 
	...
	//офинт+++
	ZIPФайлыРазрешены = Ложь;
	ОграничениеРазмераФрагментаФайлаОбмена = 204800;
	...
	Для Каждого ТекФайл Из СписокФайловДляОтправки цикл
		...
		СтрокаДляПодстановки = НастройкиПодключения.ПараметрЗапросаHTTP_ПередачаФайла;		
		
		//Если Найти(ТекФайл,"payment") = 1 Тогда
		//	ОтветСервера = ОтправитьФайлНаСервер(ТекФайл.Значение,
		//		Соединение,
		//		ОписаниеОшибки,
		//		АдресДляОтправки + СтрокаДляПодстановки + ТекФайл.Представление,
		//		ЗаголовкиЗапросов);
		//Иначе	
			ОтветСервера = ОтправитьФайлНаСервер(ТекФайл.Значение,
				Соединение,
				...
				//silber {
				//АдресДляОтправки + СтрокаДляПодстановки + ТекФайл.Представление,
				ОтветСАвторизацией(АдресДляОтправки + СтрокаДляПодстановки + ТекФайл.Представление, ОтветСервера, СесияИД),
				//} silber
				ЗаголовкиЗапросов);
		//КонецЕсли;
	...
	//ОФИНТ_КОПИРУЕМ ДЛЯ МЕНЕДЖЕРОВ
		//СтрокаДляПодстановки = "";
		//
		//Если ТипСоединения = "managers" Тогда
		//	СтрокаДляПодстановки = "&iblock_managers=14&file_name=";	
		//ИначеЕсли ТипСоединения = "clients_counteragents" Тогда
		//	СтрокаДляПодстановки = "&iblock_managers=14&iblock_conventions=19&iblock_catalog=15&file_name=";	
		//Иначе
		//	СтрокаДляПодстановки = НастройкиПодключения.ПараметрЗапросаHTTP_ПередачаФайла;
		//КонецЕсли;
		...
		Для Каждого ТекФайл Из МассивИсходныхXMLФайлов Цикл
			...
			Пока ИмпортПродолжается Цикл
				...
				ИначеЕсли ТипСоединения = "clients_counteragents" Тогда
					СтрокаДляПодстановки = "&iblock_managers=14&iblock_conventions=19&iblock_catalog=15&file_name=";	
				Иначе
					СтрокаДляПодстановки =  НастройкиПодключения.ПараметрЗапросаHTTP_ИмпортФайлаСервером;
				КонецЕсли;
				...
				ОтветСервера = ПолучитьДанныеССервера(
					...
					ОписаниеОшибки,
					//silber {
					//АдресДляРаботы + СтрокаДляПодстановки + ТекФайл.Имя,
					ОтветСАвторизацией(АдресДляРаботы + СтрокаДляПодстановки + ТекФайл.Имя, ОтветСервера, СесияИД),
					//} silber
					ЗаголовкиЗапросов);
...
//ОФИНТ
Функция ВыгрузитьКонтрагентов(Параметры, ТаблицаИнформации)
	...
	URIПространстваИмен = "urn:1C.ru:commerceml_ofint";
	ПакетCML = ФабрикаXDTO.Пакеты.Получить(URIПространстваИмен);
	
	КлассификаторТип = ПакетCML.Получить("ПартнерыКонтрагенты");
	КлассификаторXDTO = ФабрикаXDTO.Создать(КлассификаторТип);
	...
	Владелец = ПолучитьКонтрагентаXDTO_Офинт(ДанныеОрганизацииВладельцаКаталога);	
	КлассификаторXDTO.Владелец = Владелец;
	...
	ВидКИАдресДляРегистрации = Справочники.ВидыКонтактнойИнформации.НайтиПоНаименованию("АДРЕС ДЛЯ РЕГИСТРАЦИИ С САЙТА",,Справочники.ВидыКонтактнойИнформации.СправочникКонтрагенты);
	
	ПартнерыТип 		= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПакетCML.Получить("ПартнерыКонтрагенты"), "Партнеры");
	ПартнерыXDTO 		= ФабрикаXDTO.Создать(ПартнерыТип);
	...
	|	И Партнеры.УН_Партнера <> """"";	
	
	СППартнеров = Новый СписокЗначений;
	ЗапросДляОтчета = Запрос.Выполнить().Выбрать();
	Пока ЗапросДляОтчета.Следующий() Цикл
		СППартнеров.Добавить(ЗапросДляОтчета.Ссылка);
	КонецЦикла;
	
	// ( Storverk Тюняков 
	//ТЗПросрочки = ПолучитьДанныеДебеторки(СППартнеров);
	ТЗПросрочки = ПолучитьТаблицуПлатежей(Параметры.ДатаФормирования, СППартнеров.ВыгрузитьЗначения());
	КолвоПросрочки = ТЗПросрочки.Количество();
	// ) Storverk Тюняков  Не отправляем данные по просрочке - она некорректная
	
	ЗапросПартнеров = Запрос.Выполнить().Выбрать();
	Пока ЗапросПартнеров.Следующий() Цикл
		ПартнерXDTO 				= ФабрикаXDTO.Создать(ПакетCML.Получить("Партнер"));
		ИдГруппы 					= УникальныйИдентификаторОбъекта(ЗапросПартнеров.Ссылка);
		ИмяГруппы 					= ЗапросПартнеров.Ссылка.Наименование;
		ПартнерXDTO.Ид				= ИдГруппы;
		ПартнерXDTO.Наименование 	= ФорматНаименованияДляXDTO(ИмяГруппы);
		
		
		//получение долга партнера
		//Запрос = Новый Запрос;
		//Запрос.УстановитьПараметр("Партнер", ЗапросПартнеров.Ссылка);
		//Запрос.УстановитьПараметр("ТекДата", ТекДата);
		//Запрос.Текст = 
		//"ВЫБРАТЬ
		//|	РасчетыСКлиентамиОстатки.СуммаОстаток
		//|ИЗ
		//|	РегистрНакопления.РасчетыСКлиентами.Остатки(&ТекДата, АналитикаУчетаПоПартнерам.Партнер = &Партнер) КАК РасчетыСКлиентамиОстатки";
		//ЗапросДолга = Запрос.Выполнить().Выбрать();
		//Если ЗапросДолга.Следующий() Тогда
		//	ПартнерXDTO.Долг = ЗапросДолга.СуммаОстаток;
		//Иначе
		//	ПартнерXDTO.Долг = 0;
		//КонецЕсли; 
		
		//изм офинт.ру 25.12.2105
		
		Отбор 		= Новый Структура;
		Отбор.Вставить("Партнер", ЗапросПартнеров.Ссылка);
		Просрочка = 0;
		Если КолвоПросрочки > 0 Тогда
		
			НайдСтр = ТЗПросрочки.НайтиСтроки(Отбор);
			Для Каждого СтрНайд Из НайдСтр Цикл
                // ( Storverk Тюняков 
				//Если СтрНайд.ПериодыПросрочки > 0 Тогда
				//	Просрочка		= ?(СтрНайд.Долг=NULL,0,СтрНайд.Долг) + Просрочка;
				//КонецЕсли;
				Просрочка = Просрочка + СтрНайд.ДолгКлиентаПросроченоВВалютеОтчета
				// ) Storverk Тюняков  
			КонецЦикла;
		КонецЕсли;
		
		ПартнерXDTO.Долг = Просрочка;
		
		Если ЗначениеЗаполнено(ЗапросПартнеров.ОсновнойМенеджер) Тогда
			ПартнерXDTO.ИдМенеджера = УникальныйИдентификаторОбъекта(ЗапросПартнеров.ОсновнойМенеджер);
		Иначе
			ПартнерXDTO.ИдМенеджера = "";
		КонецЕсли;		
		
		ЭтоЮрЛицо = Ложь;
		
		Если ЗапросПартнеров.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо ИЛИ ЗапросПартнеров.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
			ЭтоЮрЛицо = Истина;	
		ИначеЕсли ЗапросПартнеров.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо ИЛИ ЗапросПартнеров.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель Тогда
			ЭтоЮрЛицо = Ложь;
		ИначеЕсли ТипЗнч(ЗапросПартнеров.ЮрФизЛицо) = Тип("ПеречислениеСсылка.ЮридическоеФизическоеЛицо") Тогда
		 	ЭтоЮрЛицо = ЗапросПартнеров.ЮрФизЛицо <> Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
		КонецЕсли;

		ПартнерXDTO.ПолноеНаименование 	= ЗапросПартнеров.Ссылка.НаименованиеПолное;
		ПартнерXDTO.УН_Партнера 		= ЗапросПартнеров.Ссылка.УН_Партнера;
		
		Город = "";
		Для Каждого СтрТабЧасть Из ЗапросПартнеров.Ссылка.ДополнительныеРеквизиты Цикл
			Если СтрТабЧасть.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Город") Тогда
				Попытка
					Город = СтрТабЧасть.Значение.Наименование;
				Исключение
					Город = "";	
				КонецПопытки;	
			КонецЕсли;			
		...
		|	И Контрагенты.УН_Контрагента <> """"
		|	И Контрагенты.Партнер = &Партнер";	
		ЗапросКонтрагентов = Запрос.Выполнить().Выбрать();

		КонтрагентыТип 		= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПакетCML.Получить("Партнер"), "Контрагенты");
		КонтрагентыXDTO		= ФабрикаXDTO.Создать(КонтрагентыТип);
		ИНН					= "";
		
		сч = 1;
		Пока ЗапросКонтрагентов.Следующий() Цикл
			//Если СокрЛП(ЗапросКонтрагентов.Ссылка.ИНН) = "" Тогда
			//	Продолжить;
			//КонецЕсли;
			...
			Если ЗапросКонтрагентов.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо ИЛИ ЗапросКонтрагентов.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
				ЭтоЮрЛицо = Истина;	
			ИначеЕсли ЗапросКонтрагентов.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо ИЛИ ЗапросКонтрагентов.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель Тогда
				ЭтоЮрЛицо = Ложь;
			ИначеЕсли ТипЗнч(ЗапросКонтрагентов.ЮрФизЛицо) = Тип("ПеречислениеСсылка.ЮридическоеФизическоеЛицо") Тогда
			 	ЭтоЮрЛицо = ЗапросПартнеров.ЮрФизЛицо <> Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
			КонецЕсли;

			ИНН = СокрЛП(ЗапросКонтрагентов.Ссылка.ИНН);
			КонтрагентXDTO.ИНН = ИНН;
			
			КПП = СокрЛП(ЗапросКонтрагентов.Ссылка.КПП);
			КонтрагентXDTO.КПП = КПП;
			
			Если ЭтоЮрЛицо Тогда
				КонтрагентXDTO.ОКПО = ЗапросКонтрагентов.Ссылка.КодПоОКПО;
			Иначе
				КонтрагентXDTO.ОКПО = "";
			КонецЕсли;

			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("Контрагент", ЗапросКонтрагентов.Ссылка);
			...
			ЗапросЛимит = Запрос.Выполнить().Выгрузить();
			Если ЗапросЛимит.Количество() = 0 Тогда
				КредитныйЛимит = 0;
			Иначе
				КредитныйЛимит = ЗапросЛимит[0].ДопустимаяСуммаЗадолженности;
			КонецЕсли;
		
			ИдГруппы 							= УникальныйИдентификаторОбъекта(ЗапросКонтрагентов.Ссылка);
			ИмяГруппы 							= ЗапросКонтрагентов.Ссылка.Наименование;
			КонтрагентXDTO.Ид					= ИдГруппы;
			КонтрагентXDTO.Наименование 		= ФорматНаименованияДляXDTO(ИмяГруппы);
			КонтрагентXDTO.ПолноеНаименование 	= ЗапросКонтрагентов.Ссылка.НаименованиеПолное;
			КонтрагентXDTO.УН_Контрагента 		= ЗапросКонтрагентов.Ссылка.УН_Контрагента;
			КонтрагентXDTO.КредитныйЛимит 		= КредитныйЛимит;
			
			
			ФактическийАдрес = "";
			Город = "";
			Для Каждого СтрИнф Из ЗапросКонтрагентов.Ссылка.КонтактнаяИнформация Цикл
				Если СтрИнф.Вид = Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента Тогда
					ФактическийАдрес = СтрИнф.Представление;
				ИначеЕсли СтрИнф.Вид = ВидКИАдресДляРегистрации Тогда
					Город = СтрИнф.Представление;	
				КонецЕсли;
			КонецЦикла;	
			
			КонтрагентXDTO.Город			 	= Город;
			КонтрагентXDTO.ФактическийАдрес 	= ФактическийАдрес;
			КонтрагентыXDTO.Контрагент.Добавить(КонтрагентXDTO);	
		КонецЦикла;	
		
		ПартнерXDTO.ИНН 		= ИНН;
		
		ПартнерXDTO.Контрагенты = КонтрагентыXDTO;

		//СОГЛАШЕНИЯ
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Партнер", 	ЗапросПартнеров.Ссылка);
		//изм 29.02.2016 +++
		Запрос.УстановитьПараметр("Организация",Параметры.ВладелецКаталога); 
		//изм 29.02.2016 ---
		...
		|	СоглашенияСКлиентами.Статус,
		|	ЕСТЬNULL(СоглашенияСКлиентами.Контрагент.УН_Контрагента, """") КАК УН_Контрагента
		|ИЗ
		...
		ЗапросСоглашений = Запрос.Выполнить().Выбрать();

		СолашенияТип 		= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПакетCML.Получить("Партнер"), "Соглашения");

		СоглашенияXDTO		= ФабрикаXDTO.Создать(СолашенияТип);

		Пока ЗапросСоглашений.Следующий() Цикл
			СоглашениеXDTO 						= ФабрикаXDTO.Создать(ПакетCML.Получить("Соглашение"));	
			
			ЭтоЮрЛицо = Ложь;
			Если ЗапросСоглашений.Статус = Перечисления.СтатусыСоглашенийСКлиентами.Действует Тогда
				АктивностьСоглашения = "Y";	
			Иначе
				АктивностьСоглашения = "N";	
			КонецЕсли; 
			
			ИдГруппы 							= УникальныйИдентификаторОбъекта(ЗапросСоглашений.Ссылка);
			ИмяГруппы 							= ЗапросСоглашений.Наименование;
			СоглашениеXDTO.ИдКонтрагента		= ЗапросСоглашений.УН_Контрагента;
			СоглашениеXDTO.Ид					= ИдГруппы;
			СоглашениеXDTO.Наименование 		= ФорматНаименованияДляXDTO(ИмяГруппы);
			СоглашениеXDTO.ACTIVE				= АктивностьСоглашения;
			
			//найдем типовое соглашения с таким же типом цены и выгрузим ИД 
			Если ЗначениеЗаполнено(ЗапросСоглашений.ВидЦен) Тогда
				Запрос = Новый Запрос;
				Запрос.УстановитьПараметр("ТипЦены", ЗапросСоглашений.ВидЦен);
				Запрос.Текст =
				"ВЫБРАТЬ
				|	СоглашенияСКлиентами.Ссылка
				|ИЗ
				...
				|	И СоглашенияСКлиентами.ВидЦен = &ТипЦены
				|	И СоглашенияСКлиентами.ДоступноВнешнимПользователям = ИСТИНА
				|	И СоглашенияСКлиентами.ПометкаУдаления = ЛОЖЬ";
				ЗапросТиповых = Запрос.Выполнить().Выгрузить();
				Если ЗапросТиповых.Количество() = 0 Тогда
					СоглашениеXDTO.ИдТипЦены		= "";
				Иначе
					СоглашениеXDTO.ИдТипЦены		= УникальныйИдентификаторОбъекта(ЗапросТиповых[0].Ссылка);
				КонецЕсли;	
			Иначе
				СоглашениеXDTO.ИдТипЦены		= "";
			КонецЕсли;
			
			СоглашениеXDTO.ИдСклад				= УникальныйИдентификаторОбъекта(ЗапросСоглашений.Склад);
			
			//добавим скидки по производителям
			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("Соглашение", ЗапросСоглашений.Ссылка);
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	СТ_ДополнительныеСкидки.Период,
			|	СТ_ДополнительныеСкидки.Соглашение,
			|	СТ_ДополнительныеСкидки.Производитель,
			|	СТ_ДополнительныеСкидки.Группа,
			|	СТ_ДополнительныеСкидки.Скидка,
			|	СТ_ДополнительныеСкидки.Производитель.Наименование КАК ПроизводительНаименование
			|ИЗ
			|	РегистрСведений.СТ_ДополнительныеСкидки КАК СТ_ДополнительныеСкидки
			|ГДЕ
			|	СТ_ДополнительныеСкидки.Соглашение = &Соглашение";
			ЗапросСкидок = Запрос.Выполнить().Выбрать();
			
			Скидки1Тип 		= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПакетCML.Получить("Соглашение"), "СкидкаПоПроизводителю");
			Скидки1XDTO		= ФабрикаXDTO.Создать(Скидки1Тип);
			
			Пока ЗапросСкидок.Следующий() Цикл
				Если ЗначениеЗаполнено(ЗапросСкидок.Группа) Тогда 
				
					СкидкаXDTO 							= ФабрикаXDTO.Создать(ПакетCML.Получить("Discount"));	
					СкидкаXDTO.РазмерСкидки 			= ЗапросСкидок.Скидка;
					Если Не ЗначениеЗаполнено(ЗапросСкидок.Группа) Тогда
						ИдГруппы = "";
					Иначе
						ИдГруппы = УникальныйИдентификаторОбъекта(ЗапросСкидок.Группа);
					КонецЕсли;
					СкидкаXDTO.Группа				= ИдГруппы;
					Если Не ЗначениеЗаполнено(ЗапросСкидок.Производитель) Тогда
						ИмяГруппы = "";
					Иначе
						ИмяГруппы = ЗапросСкидок.ПроизводительНаименование;
					КонецЕсли;	
					СкидкаXDTO.Производитель 			= ФорматНаименованияДляXDTO(ИмяГруппы);
					
					СкидкаXDTO.Период 					= ЗапросСкидок.Период; 
					
					Скидки1XDTO.Discount.Добавить(СкидкаXDTO);
					
				КонецЕсли;	
			КонецЦикла;
			
			СоглашениеXDTO.СкидкаПоПроизводителю.Добавить(Скидки1XDTO);
			...
			|	МАКСИМУМ(ЕСТЬNULL(ДействиеСкидокНаценокСрезПоследних.СкидкаНаценка.ЗначениеСкидкиНаценки, 0)) КАК РазмерСкидки
			|ИЗ
			...
			Скидки2Тип 		= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПакетCML.Получить("Соглашение"), "СкидкаПоСоглашению");
			Скидки2XDTO		= ФабрикаXDTO.Создать(Скидки2Тип);

			РазмерСкидки = 0;
			Пока ЗапросСкидок.Следующий() Цикл
				РазмерСкидки = ЗапросСкидок.РазмерСкидки;				
			КонецЦикла;
			
			СкидкаXDTO = ФабрикаXDTO.Создать(ПакетCML.Получить("Discount"));	
			СкидкаXDTO.РазмерСкидки = РазмерСкидки;
			Скидки2XDTO.Discount.Добавить(СкидкаXDTO);

			СоглашениеXDTO.СкидкаПоСоглашению.Добавить(Скидки2XDTO);
						
			СоглашенияXDTO.Соглашение.Добавить(СоглашениеXDTO);	
		КонецЦикла;	
		
		ПартнерXDTO.Соглашения = СоглашенияXDTO;

		//КонтрагентXDTO.Проверить();
		
		ПартнерыXDTO.Партнер.Добавить(ПартнерXDTO);
	...
	Если ПартнерыXDTO.Партнер.Количество() > 0 Тогда
		КлассификаторXDTO.Партнеры = ПартнерыXDTO;
	КонецЕсли;
	
	КоммерческаяИнформацияXDTO.ПартнерыКонтрагенты = КлассификаторXDTO;
	...
	СтрокаТаблицыИнформации.Описание = Строка(ТекущаяДата()) + " " + НСтр("ru = 'Запуск выгрузки контрагентов'");
	...
	//
	ТекстСообщения = "ru = 'Завершена выгрузка контрагентов'";
	...
	ИмяФайлаКаталога = ПодготовитьПутьДляПлатформы(Параметры.ПлатформаWindows,
			Параметры.КаталогНаДиске + "\clients_counteragent"
			+ ".xml");
	...
	// ++++ офинт.ру создаем файлы с просрочкой
	
	Для Каждого СтрПартнеров Из СППартнеров Цикл
		
		ТекПартнер 	= СтрПартнеров.Значение;
		Отбор 		= Новый Структура;
		Отбор.Вставить("Партнер", ТекПартнер.Ссылка);
		
		
		ГлавныйТип = ПакетCML.Получить("ДолгиПартнеров");
		ГлавныйXDTO = ФабрикаXDTO.Создать(ГлавныйТип);
		
		
		КоммИнфXDTO 					= ФабрикаXDTO.Создать(КоммерческаяИнформацияТип);
		КоммИнфXDTO.ВерсияСхемы 		= ВерсияСхемы();
		КоммИнфXDTO.ДатаФормирования 	= Параметры.ДатаФормирования;

		ГлавныйXDTO.Владелец 			= Владелец;
		ГлавныйXDTO.ИД					= УникальныйИдентификаторОбъекта(ТекПартнер); 
		// ( Storverk Тюняков 
		ГлавныйXDTO.Наименование 		= ФорматНаименованияДляXDTO(ТекПартнер.Наименование);
		// ) Storverk Тюняков  
		ГлавныйXDTO.УН_Партнера 		= ТекПартнер.Ссылка.УН_Партнера;
		
		
		ОписаниеОшибки = "";
		
		
		ГрафикТип 		= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПакетCML.Получить("ДолгиПартнеров"), "ГрафикПлатежей");
		ГрафикXDTO 		= ФабрикаXDTO.Создать(ГрафикТип);
		
		Если ТЗПросрочки.Количество() > 0 Тогда
		
			НайдСтр = ТЗПросрочки.НайтиСтроки(Отбор);
			//НайдСтр.Сортировать("РеализацияДата Возр");
			Для Каждого СтрНайд Из НайдСтр Цикл
				ПлатежXDTO 					= ФабрикаXDTO.Создать(ПакетCML.Получить("Платеж"));
				// ( Storverk Тюняков 
				//ПлатежXDTO.Дата 			= СтрНайд.РеализацияДата;
				//ПлатежXDTO.Номер 			= СтрНайд.РеализацияНомер;
				//ПлатежXDTO.Сумма 			= ?(СтрНайд.РеализацияяСумма=NULL,0,СтрНайд.РеализацияяСумма);
				//ПлатежXDTO.Оплата 			= ?(СтрНайд.ПоступлениеДенежныхСредствСумма=NULL,0,СтрНайд.ПоступлениеДенежныхСредствСумма);
				//Если СтрНайд.ПериодыПросрочки > 0 Тогда
				//	ПлатежXDTO.Просрочка		= ?(СтрНайд.Долг=NULL,0,СтрНайд.Долг);
				//	ПлатежXDTO.ПросроченоДней	= ?(СтрНайд.ПериодыПросрочки=NULL,0,СтрНайд.ПериодыПросрочки);
				//Иначе
				//	ПлатежXDTO.Просрочка		= 0;
				//	ПлатежXDTO.ПросроченоДней	= 0;
				//КонецЕсли;
				//ПлатежXDTO.СрокКредита		=?(СтрНайд.СрокКредита=NULL,0,СтрНайд.СрокКредита);
				Если НЕ ЗначениеЗаполнено(СтрНайд.ДолгКлиентаВВалютеОтчета)
					И НЕ ЗначениеЗаполнено(СтрНайд.СуммаДокумента)
					И НЕ ЗначениеЗаполнено(СтрНайд.ДолгКлиентаПросроченоВВалютеОтчета) Тогда
				   	Продолжить;			
				КонецЕсли;
				ПлатежXDTO.Дата 			= ?(СтрНайд.Дата=NULL,				ТекущаяДата(),	СтрНайд.Дата);
				ПлатежXDTO.Номер 			= ?(СтрНайд.Номер=NULL,				"",				СтрНайд.Номер);
				ПлатежXDTO.Сумма 			= ?(СтрНайд.СуммаДокумента=NULL,	0,				СтрНайд.СуммаДокумента);
				ПлатежXDTO.Оплата 			= ?(СтрНайд.Оплата=NULL,			0,				СтрНайд.Оплата);
				ПлатежXDTO.Просрочка		= ?(СтрНайд.ДолгКлиентаПросроченоВВалютеОтчета=NULL,0,СтрНайд.ДолгКлиентаПросроченоВВалютеОтчета);
				ПлатежXDTO.ПросроченоДней	= ?(СтрНайд.КоличествоДней=NULL,	0,				СтрНайд.КоличествоДней);
				ПлатежXDTO.СрокКредита		= ?(СтрНайд.СрокКредита=NULL,		0,				СтрНайд.СрокКредита);//СтрНайд.ДатаПлатежа;
				// ) Storverk Тюняков  			
				ГрафикXDTO.Платеж.Добавить(ПлатежXDTO);
			КонецЦикла;	
		КонецЕсли;
		
		Если ГрафикXDTO.Платеж.Количество() > 0 Тогда
			ГлавныйXDTO.ГрафикПлатежей = ГрафикXDTO;
		КонецЕсли;
		
		КоммИнфXDTO.ДолгиПартнеров = ГлавныйXDTO;
		
		ИмяФайлаКаталога = ПодготовитьПутьДляПлатформы(Параметры.ПлатформаWindows,
			Параметры.КаталогНаДиске + "\"+ "payment_" + ТекПартнер.Ссылка.УН_Партнера
			+ ".xml");
		...
		ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, КоммИнфXDTO, "КоммерческаяИнформация");
		ЗаписьXML.Закрыть();
		
	КонецЦикла;
	
	Если Параметры.ВыгружатьНаСайт Тогда
		УдалитьРегистрацию = ВыгрузитьДанныеНаСайт_ОФИНТ(Параметры, СтрокаТаблицыИнформации, "clients_counteragents");
...
Функция ВыгрузитьКонтрагентовУСТАРЕЛА(Параметры, ТаблицаИнформации)
	...
	Пока ЗапросПартнеров.Следующий() Цикл
		...
		Пока ЗапросКонтрагентов.Следующий() Цикл
			...
			Для Каждого СтрИнф Из ЗапросКонтрагентов.Ссылка.КонтактнаяИнформация Цикл
				Если СтрИнф.Вид = Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента Тогда
					ФактическийАдрес = СтрИнф.Представление;
				ИначеЕсли СтрИнф.Вид = Справочники.ВидыКонтактнойИнформации.НайтиПоНаименованию("АДРЕС ДЛЯ РЕГИСТРАЦИИ С САЙТА",,Справочники.ВидыКонтактнойИнформации.СправочникКонтрагенты) Тогда
					Город = СтрИнф.Представление;	
				КонецЕсли;
			КонецЦикла;	
		...
		Пока ЗапросСоглашений.Следующий() Цикл
			...
			ИдГруппы 							= УникальныйИдентификаторОбъекта(ЗапросСоглашений.Ссылка);
			ИмяГруппы 							= ЗапросСоглашений.Ссылка.Наименование;
			СоглашениеXDTO.Ид					= ИдГруппы;
			СоглашениеXDTO.Наименование 		= ФорматНаименованияДляXDTO(ИмяГруппы);
			
			//найдем типовое соглашения с таким же типом цены и выгрузим ИД 
			Если ЗначениеЗаполнено(ЗапросСоглашений.Ссылка.ВидЦен) Тогда
				Запрос = Новый Запрос;
				Запрос.УстановитьПараметр("ТипЦены", ЗапросСоглашений.Ссылка.ВидЦен);
				Запрос.Текст =
				"ВЫБРАТЬ
				|	СоглашенияСКлиентами.Ссылка
				|ИЗ
			...
			СоглашениеXDTO.ИдСклад				= УникальныйИдентификаторОбъекта(ЗапросСоглашений.Ссылка.Склад);
			...
			Пока ЗапросСкидок.Следующий() Цикл
				Если ЗначениеЗаполнено(ЗапросСкидок.Группа) Тогда 
					...
					Если Не ЗначениеЗаполнено(ЗапросСкидок.Группа) Тогда
						ИдГруппы = "";
					Иначе
						ИдГруппы = УникальныйИдентификаторОбъекта(ЗапросСкидок.Группа.Ссылка);
					КонецЕсли;
					СкидкаXDTO.Группа				= ИдГруппы;
					Если Не ЗначениеЗаполнено(ЗапросСкидок.Производитель) Тогда
						ИмяГруппы = "";
					Иначе
						ИмяГруппы = ЗапросСкидок.Производитель.Наименование;
					КонецЕсли;	
					СкидкаXDTO.Производитель 			= ФорматНаименованияДляXDTO(ИмяГруппы);
			...
			|ИЗ
			|	РегистрСведений.ДействиеСкидокНаценок.СрезПоследних(&Дата, ) КАК ДействиеСкидокНаценок
			|ГДЕ
			|	ДействиеСкидокНаценок.Источник = &Соглашение
			|	И ДействиеСкидокНаценок.Статус = &Статус";
			ЗапросСкидок = Запрос.Выполнить().Выбрать();
			...
			Пока ЗапросСкидок.Следующий() Цикл
				СкидкаXDTO 							= ФабрикаXDTO.Создать(ПакетCML.Получить("Discount"));	
				СкидкаXDTO.РазмерСкидки 			= ЗапросСкидок.СкидкаНаценка.ЗначениеСкидкиНаценки;
	...
	//	
	//	КоммИнфXDTO 					= ФабрикаXDTO.Создать(КоммерческаяИнформацияТип);
	//	КоммИнфXDTO.ВерсияСхемы 		= ОбменССайтомПовтИсп.ВерсияСхемы();
	//	КоммИнфXDTO.ДатаФормирования 	= Параметры.ДатаФормирования;

	//	ГлавныйXDTO.Владелец 			= Владелец;
	//	ГлавныйXDTO.ИД					= УникальныйИдентификаторОбъекта(ТекПартнер); 
	//	ГлавныйXDTO.Наименование 		= ФорматНаименованияДляXDTO(ИмяГруппы);
	//	ГлавныйXDTO.УН_Партнера 		= ТекПартнер.Ссылка.УН_Партнера;
	//	
	//	
	//	ОписаниеОшибки = "";
...
//ОФИНТ
Функция ВыгрузитьМенеджеров(Параметры, ТаблицаИнформации)
	...
	КлассификаторТип = ПакетCML.Получить("КлиентыМенеджеры");
	КлассификаторXDTO = ФабрикаXDTO.Создать(КлассификаторТип);
	...
	ЗапросПользователей = Запрос.Выполнить().Выбрать();
	
	ГруппыКлассификатораТип = ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПакетCML.Получить("КлиентыМенеджеры"), "Менеджеры");
	ГруппыКлассификатораXDTO = ФабрикаXDTO.Создать(ГруппыКлассификатораТип);
	
	
	Пока ЗапросПользователей.Следующий() Цикл
		ГруппаXDTO = ФабрикаXDTO.Создать(ПакетCML.Получить("Менеджер"));
		ИдГруппы 	= УникальныйИдентификаторОбъекта(ЗапросПользователей.Ссылка);
		ИмяГруппы 	= ЗапросПользователей.Ссылка.Наименование;
		ГруппаXDTO.Ид			= ИдГруппы;
		ГруппаXDTO.Наименование = ФорматНаименованияДляXDTO(ИмяГруппы);
		
		Если ЗначениеЗаполнено(ЗапросПользователей.Ссылка.ФизическоеЛицо) Тогда
			ГруппаXDTO.ФИО = ЗапросПользователей.Ссылка.ФизическоеЛицо.Наименование; 
		Иначе	
			ГруппаXDTO.ФИО = "";
		КонецЕсли;
		
		Для Каждого СтрКонтакт Из ЗапросПользователей.Ссылка.КонтактнаяИнформация Цикл
			Если СтрКонтакт.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты Тогда
				ГруппаXDTO.Email 	= ФорматНаименованияДляXDTO(СокрЛП(СтрКонтакт.Представление));
			ИначеЕсли СтрКонтакт.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда	
				ГруппаXDTO.Телефон = ФорматНаименованияДляXDTO(СокрЛП(СтрКонтакт.Представление));
			КонецЕсли;
		КонецЦикла;
		ГруппыКлассификатораXDTO.Менеджер.Добавить(ГруппаXDTO);
	КонецЦикла;
	
	Если ГруппыКлассификатораXDTO.Менеджер.Количество() > 0 Тогда
		КлассификаторXDTO.Менеджеры = ГруппыКлассификатораXDTO;
	КонецЕсли;
	...
	КоммерческаяИнформацияXDTO.КлиентыМенеджеры = КлассификаторXDTO;

	
	СтрокаТаблицыИнформации = ТаблицаИнформации.Добавить();
	СтрокаТаблицыИнформации.ДействиеПриОбмене = Перечисления.ДействияПриОбмене.ВыгрузкаДанных;
	СтрокаТаблицыИнформации.Описание = Строка(ТекущаяДата()) + " " + НСтр("ru = 'Запуск выгрузки менеджеров'");
	
	ТекстСообщения = "ru = 'Завершена выгрузка менеджеров'";
	...
	ИмяФайлаКаталога = ПодготовитьПутьДляПлатформы(Параметры.ПлатформаWindows,
			Параметры.КаталогНаДиске + "\managers"
			+ ".xml");
	...
	Если Параметры.ВыгружатьНаСайт Тогда
		...
		УдалитьРегистрацию = ВыгрузитьДанныеНаСайт_ОФИНТ(Параметры, СтрокаТаблицыИнформации, "managers");
...
Процедура ЗаполнитьЗаказ(ДокументОбъект, РеквизитыЗаказа, СвойстваЗаказа,
                         ПрикладныеПараметры, ОписаниеОшибки, Успешно)
	...
	ДокументОбъект.Менеджер      = ДокументОбъект.Партнер.ОсновнойМенеджер;
	...
	СуммаЗаказано       = ПолучитьСуммуЗаказанныхСтрок(Ложь, ДокументОбъект);
	СуммаЗалогаЗаТару   = ПолучитьСуммуЗаказанныхСтрок(Истина, ДокументОбъект);
...
//
Процедура ЗаполнитьТабличнуюЧастьДокумента(Документ, ТаблицаТоваровСайт, ПрикладныеПараметры)
	...
	Для Каждого ТекСтрока Из ТаблицаТоваровСайт Цикл
		...
		НоваяСтрока.ДатаОтгрузки = Документ.Дата;
		//НоваяСтрока.СуммаАвтоматическойСкидки = (НоваяСтрока.Цена*НоваяСтрока.КоличествоУпаковок) - НоваяСтрока.Сумма;
	КонецЦикла;
	
	Документ.Товары.Загрузить(ТаблицаТоварыДокумент);
	
	// Storverk Тюняков 7194 Удалить нетиповую простановку обеспечения
...
//офинт.ру 25.11.2015
// Ищет кон.трагента согласно настройкам плана обмена.
// Параметры:
...
Процедура ИдентифицироватьКонтрагента(СтруктураСвойстваЗаказа, ДанныеКонтрагента, ПрикладныеПараметры, ОписаниеОшибки) Экспорт
	...
	ИдКлиента 	  = ДанныеКонтрагента.ИдКлиента;
	ИдКонтрагента = ДанныеКонтрагента.ИдКонтрагента;
	...
	Если ПустаяСтрока(ИдКонтрагента) Тогда
		
		КонтрагентСсылка = Справочники.Контрагенты.ПустаяСсылка();
		
	КонецЕсли;
	
	Запрос.Текст = Запрос.Текст + " ГДЕ Контрагенты.УН_Контрагента = &ИдКонтрагента";
	Запрос.УстановитьПараметр("ИдКонтрагента", СокрЛП(ИдКонтрагента));
	...
	//	НСтр("ru = 'Наименование ИД партнера должно быть заполнено!'"));
	//	
	//	Возврат ;
	//	
	//КонецЕсли;
	...
	//		
	//		ТекстСообщенияНайденоНесколькоКонтрагентов = 
			//СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ' по ИНН: %1, КПП: %2'"),
	//		                            ИНН, КПП);
	//		
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Контрагент заказа %1 от %2 не найден в базе. (Наименование: ""%3""; УН: %4)'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, 
			СтруктураСвойстваЗаказа.Ид, 
			СтруктураСвойстваЗаказа.Дата,
			Наименование, 
			ИдКонтрагента);
		...
		ОбменССайтом.ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекстСообщения);
...
//офинт.ру+++
Функция ПолучитьДанныеДебеторки(ТекПартнер)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ПОМЕСТИТЬ ВТ_Закрыт
	               |ИЗ
	               |	РегистрСведений.ДополнительныеСведения КАК ДС_Закрыт
	               |ГДЕ
	               |	ДС_Закрыт.Свойство.Заголовок = ""Сделка закрыта""
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |	Т.Менеджер,
	               |	Т.Реализация,
	               |	Т.РеализацияДата КАК РеализацияДата,
	               |	Т.РеализацияНомер,
	               |	Т.СебеСтоим,
	               |	Т.ЗаказСумма,
	               |	Т.РеализацияяСумма,
	               |	Т.ПоступлениеДенежныхСредствСумма,
	               |	Т.Долг,
	               |	Т.СрокКредита,
	               |	Т.ДнейПослеРеализацииДоОплаты,
	               |	Т.КредитПривышен,
	               |	Т.ПоступлениеДенежныхсредствДата КАК ДатаПоследнейОплаты,
	               |	Т.ДнейПослеРеализации - ЕСТЬNULL(Т.СрокКредита, 0) КАК ПериодыПросрочки,
	               |	Т.Реализация.Партнер КАК Партнер
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		ЗаказДок.Ссылка КАК Заказ,
	               |		ЗаказДок.Дата КАК ЗаказДата,
	               |		ЗаказДок.Номер КАК ЗаказНомер,
	               |		РеализацияТоваровУслуг.Менеджер КАК Менеджер,
	               |		РеализацияТоваровУслуг.Ссылка КАК Реализация,
	               |		РеализацияТоваровУслуг.Дата КАК РеализацияДата,
	               |		РеализацияТоваровУслуг.Номер КАК РеализацияНомер,
	               |		СебСт.СебеСтоим КАК СебеСтоим,
	               |		ЗаказДок.СуммаДокумента КАК ЗаказСумма,
	               |		РеализацияТоваровУслуг.СуммаДокумента КАК РеализацияяСумма,
	               |		ПоступлениеДСДатаСумма.Сумма КАК ПоступлениеДенежныхСредствСумма,
	               |		РеализацияТоваровУслуг.СуммаДокумента - ЕСТЬNULL(ПоступлениеДСДатаСумма.Сумма, 0) КАК Долг,
	               |		Согл.СрокКредита КАК СрокКредита,
	               |		ПоступлениеДСДатаСумма.ПоступлениеДенежныхсредствДата КАК ПоступлениеДенежныхсредствДата,
	               |		РАЗНОСТЬДАТ(РеализацияТоваровУслуг.Дата, &ТекДата, ДЕНЬ) КАК ДнейПослеРеализации,
	               |		РАЗНОСТЬДАТ(РеализацияТоваровУслуг.Дата, ЕСТЬNULL(ПоступлениеДСДатаСумма.ПоступлениеДенежныхсредствДата, &ТекДата), ДЕНЬ) КАК ДнейПослеРеализацииДоОплаты,
	               |		ВЫБОР
	               |			КОГДА РАЗНОСТЬДАТ(РеализацияТоваровУслуг.Дата, ЕСТЬNULL(ПоступлениеДСДатаСумма.ПоступлениеДенежныхсредствДата, &ТекДата), ДЕНЬ) > Согл.СрокКредита
	               |				ТОГДА ИСТИНА
	               |			ИНАЧЕ ЛОЖЬ
	               |		КОНЕЦ КАК КредитПривышен
	               |	ИЗ
	               |		(ВЫБРАТЬ
	               |			ЗаказДок.Ссылка КАК Ссылка,
	               |			ЗаказДок.Заказ КАК Заказ,
	               |			ЗаказДок.Дата КАК Дата,
	               |			ЗаказДок.Номер КАК Номер,
	               |			ЗаказДок.СуммаДокумента КАК СуммаДокумента
	               |		ИЗ
	               |			(ВЫБРАТЬ
	               |				ЗаказДок.Ссылка КАК Ссылка,
	               |				ЗаказДок.Ссылка КАК Заказ,
	               ...
	               |				ЗаказДок.СуммаДокумента КАК СуммаДокумента
	               |			ИЗ
	               |				Документ.ЗаказКлиента КАК ЗаказДок
	               |			ГДЕ
	               |				ЗаказДок.Проведен = ИСТИНА
	               |			
	               |			ОБЪЕДИНИТЬ ВСЕ
	               |			
	               |			ВЫБРАТЬ
	               ...
	               |				ЗаказДок.Дата,
	               |				ЗаказДок.Номер,
	               |				ЗаказДок.СуммаДокумента
	               |			ИЗ
	               |				Документ.РеализацияТоваровУслуг КАК ЗаказДок
	               |			ГДЕ
	               |				ЗаказДок.Проведен = ИСТИНА
	               |				И ЗаказДок.РеализацияПоЗаказам = ЛОЖЬ) КАК ЗаказДок
	               |		{ГДЕ
	               |			ЗаказДок.Ссылка.Партнер.*,
	               |			ЗаказДок.Ссылка.Менеджер.*}) КАК ЗаказДок
	               ...
	               |				КОНЕЦ = ЗаказДок.Ссылка)
	               |				И (РеализацияТоваровУслуг.Проведен = ИСТИНА)
	               |				И (&ПоВсем = ИСТИНА
	               |					ИЛИ РеализацияТоваровУслуг.Менеджер = &Менеджер)
	               |			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Закрыт КАК Закр
	               |			ПО (Закр.Объект = РеализацияТоваровУслуг.Ссылка)
	               |			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               |				МАКСИМУМ(Т.ПоступлениеДенежныхсредствДата) КАК ПоступлениеДенежныхсредствДата,
	               |				Т.Заказ КАК Заказ,
	               |				СУММА(Т.Сумма) КАК Сумма
	               |			ИЗ
	               |				(ВЫБРАТЬ
	               |					МАКСИМУМ(ПоступлениеДС.Ссылка.Дата) КАК ПоступлениеДенежныхсредствДата,
	               |					ПоступлениеДС.Заказ КАК Заказ,
	               |					СУММА(ПоступлениеДС.Сумма) КАК Сумма
	               |				ИЗ
	               |					Документ.ПоступлениеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ПоступлениеДС
	               |				ГДЕ
	               |					ПоступлениеДС.Ссылка.Проведен = ИСТИНА
	               |				
	               |				СГРУППИРОВАТЬ ПО
	               |					ПоступлениеДС.Заказ
	               |				
	               ...
	               |				
	               |				ВЫБРАТЬ
	               |					МАКСИМУМ(ПоступлениеДС.Ссылка.Дата),
	               |					ПоступлениеДС.Заказ,
	               |					СУММА(ПоступлениеДС.Сумма)
	               |				ИЗ
	               ...
	               |					МАКСИМУМ(ПоступлениеДС.Ссылка.Дата),
	               |					ВЫБОР
	               |						КОГДА ПоступлениеДС.ДокументРеализации.РеализацияПоЗаказам
	               |							ТОГДА ПоступлениеДС.ДокументРеализации.ЗаказКлиента
	               |						ИНАЧЕ ПоступлениеДС.ДокументРеализации
	               |					КОНЕЦ,
	               |					СУММА(ПоступлениеДС.СуммаДокумента)
	               |				ИЗ
	               |					Документ.ВозвратТоваровОтКлиента КАК ПоступлениеДС
	               |				ГДЕ
	               |					ПоступлениеДС.Ссылка.Проведен = ИСТИНА
	               |				
	               |				СГРУППИРОВАТЬ ПО
	               ...
	               |					МАКСИМУМ(ПоступлениеДС.Ссылка.Дата),
	               |					ВЫБОР
	               |						КОГДА ПоступлениеДС.Заказ ССЫЛКА Документ.ВозвратТоваровОтКлиента
	               |							ТОГДА ПоступлениеДС.Заказ.ДокументРеализации.ЗаказКлиента
	               |						ИНАЧЕ ПоступлениеДС.Заказ
	               |					КОНЕЦ,
	               |					-СУММА(ПоступлениеДС.Сумма)
	               |				ИЗ
	               |					Документ.РасходныйКассовыйОрдер.РасшифровкаПлатежа КАК ПоступлениеДС
	               |				ГДЕ
	               |					ПоступлениеДС.Ссылка.Проведен = ИСТИНА
	               |				
	               |				СГРУППИРОВАТЬ ПО
	               ...
	               |					-СУММА(ПоступлениеДС.Сумма)
	               |				ИЗ
	               |					Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ПоступлениеДС
	               |				ГДЕ
	               |					ПоступлениеДС.Ссылка.Проведен = ИСТИНА
	               |				
	               |				СГРУППИРОВАТЬ ПО
	               |					ПоступлениеДС.Заказ) КАК Т
	               |			
	               |			СГРУППИРОВАТЬ ПО
	               |				Т.Заказ) КАК ПоступлениеДСДатаСумма
	               |			ПО (ПоступлениеДСДатаСумма.Заказ = ЗаказДок.Ссылка)
	               |			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               |				СебСт.Релизация КАК Релизация,
	               |				СУММА(СебСт.СтоимостьРегл) КАК СебеСтоим
	               |			ИЗ
	               |				(ВЫБРАТЬ
	               |					СебСт.Регистратор КАК Релизация,
	               |					СебСт.СтоимостьРегл КАК СтоимостьРегл
	               |				ИЗ
	               |					РегистрНакопления.СебестоимостьТоваров КАК СебСт
	               |				
	               |				ОБЪЕДИНИТЬ ВСЕ
	               |				
	               |				ВЫБРАТЬ
	               |					СебСт.ДокументДвижения,
	               |					СебСт.СтоимостьРегл
	               |				ИЗ
	               |					РегистрНакопления.СебестоимостьТоваров КАК СебСт) КАК СебСт
	               |			
	               |			СГРУППИРОВАТЬ ПО
	               |				СебСт.Релизация) КАК СебСт
	               |			ПО (СебСт.Релизация = РеализацияТоваровУслуг.Ссылка)
	               |			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               |				Согл.Ссылка.Партнер КАК Партнер,
	               |				МАКСИМУМ(Согл.Сдвиг) КАК СрокКредита
	               |			ИЗ
	               |				Справочник.СоглашенияСКлиентами.ЭтапыГрафикаОплаты КАК Согл
	               |			ГДЕ
	               |				Согл.Ссылка.ПометкаУдаления = ЛОЖЬ
	               |			
	               |			СГРУППИРОВАТЬ ПО
	               |				Согл.Ссылка.Партнер) КАК Согл
	               |			ПО (Согл.Партнер = РеализацияТоваровУслуг.Партнер)
	               |	ГДЕ
	               |		ЕСТЬNULL(Закр.Значение, ЛОЖЬ) = ЛОЖЬ) КАК Т
	               |ГДЕ
	               |	Т.Реализация.Партнер В(&Партнер)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	РеализацияДата";
	ЗапросДолгов = Запрос.Выполнить().Выгрузить();	
	
	Возврат ЗапросДолгов;
...
//	КонтрагентXDTO
//
Функция ПолучитьКонтрагентаXDTO_Офинт(ДанныеКонтрагента) Экспорт
...
//
Функция ПолучитьСуммуЗаказанныхСтрок(ТолькоЗалогЗаТару = Ложь, ТекОбъект) Экспорт
	...
	Запрос.УстановитьПараметр("Товары", ТекОбъект.Товары.Выгрузить(,"Номенклатура,СуммаСНДС,Отменено"));
	Запрос.УстановитьПараметр("ВернутьМногооборотнуюТару", ТекОбъект.ВернутьМногооборотнуюТару);
	Запрос.УстановитьПараметр("ТребуетсяЗалогЗаТару", ТекОбъект.ТребуетсяЗалогЗаТару);
...
Процедура ПолучитьТекстЗапросаЗаказы(ТекстЗапроса) Экспорт
	...
	|		МАКСИМУМ(ДвижениеТоваров.Регистратор) КАК Регистратор
	|	ИЗ
	|		ЗаказыДляВыгрузки КАК ЗаказыДляВыгрузки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДвижениеТоваров КАК ДвижениеТоваров
	|			ПО ЗаказыДляВыгрузки.Заказ = ДвижениеТоваров.Распоряжение
	|				И (ДвижениеТоваров.ПланируемаяОтгрузкаСоСклада < 0)
	|				И (ДвижениеТоваров.Корректировка = ЛОЖЬ)
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|			ТОГДА ДанныеНоменлатуры.ЕдиницаИзмерения
	|		ИНАЧЕ ЗаказКлиентаТовары.Упаковка.ЕдиницаИзмерения
	|	КОНЕЦ КАК ТоварыЕдиницаИзмерения,
	|	ВЫБОР
	|		КОГДА ЗаказКлиентаТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ДанныеНоменлатуры.ЕдиницаИзмерения.Код
	|		ИНАЧЕ ЗаказКлиентаТовары.Упаковка.ЕдиницаИзмерения.Код
	|	КОНЕЦ КАК ТоварыЕдиницаИзмеренияКод,
	|	ВЫБОР
	|		КОГДА ЗаказКлиентаТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ДанныеНоменлатуры.ЕдиницаИзмерения.Наименование
	|		ИНАЧЕ ЗаказКлиентаТовары.Упаковка.ЕдиницаИзмерения.Наименование
	|	КОНЕЦ КАК ТоварыЕдиницаИзмеренияНаименование,
	|	ВЫБОР
	|		КОГДА ЗаказКлиентаТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ДанныеНоменлатуры.ЕдиницаИзмерения.НаименованиеПолное
	|		ИНАЧЕ ЗаказКлиентаТовары.Упаковка.ЕдиницаИзмерения.НаименованиеПолное
	|	КОНЕЦ КАК ТоварыЕдиницаИзмеренияНаименованиеПолное,
	|	ВЫБОР
	|		КОГДА ЗаказКлиентаТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ДанныеНоменлатуры.ЕдиницаИзмерения.МеждународноеСокращение
	|		ИНАЧЕ ЗаказКлиентаТовары.Упаковка.ЕдиницаИзмерения.МеждународноеСокращение
...
// Процедура создана на основе одноименной процедуры отчета "Дебиторская задолженность" УТ 11.3.3.163
//	Изменения в типовой процедуре закомментированы "/+"
Функция ПолучитьТаблицуПлатежей(ДатаОтчета, Партнеры)
	...
	// ( Storverk Тюняков 
	|	ВЫБОР
	|		КОГДА ТаблицаЗадолженностей.ЗаказКлиента ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА 
	|			Выразить(ТаблицаЗадолженностей.ЗаказКлиента КАК Документ.РеализацияТоваровУслуг).Номер
	|		ИНАЧЕ ТаблицаЗадолженностей.РасчетныйДокумент.Номер
	|	КОНЕЦ КАК Номер,
	|	ВЫБОР
	|		КОГДА ТаблицаЗадолженностей.ЗаказКлиента ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА 
	|			Выразить(ТаблицаЗадолженностей.ЗаказКлиента КАК Документ.РеализацияТоваровУслуг).Дата
	|		ИНАЧЕ ТаблицаЗадолженностей.РасчетныйДокумент.Дата
	|	КОНЕЦ КАК Дата,
	|	ВЫБОР
	|		КОГДА ТаблицаЗадолженностей.ЗаказКлиента ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА 
	|			Выразить(ТаблицаЗадолженностей.ЗаказКлиента КАК Документ.РеализацияТоваровУслуг).СуммаДокумента
	|		ИНАЧЕ ТаблицаЗадолженностей.ДолгКлиентаВВалютеОтчета
	|	КОНЕЦ КАК СуммаДокумента,
	|	ВЫБОР
	|		КОГДА ТаблицаЗадолженностей.ЗаказКлиента ССЫЛКА Документ.РеализацияТоваровУслуг  И Выразить(ТаблицаЗадолженностей.ЗаказКлиента КАК Документ.РеализацияТоваровУслуг).СуммаДокумента > ТаблицаЗадолженностей.ДолгКлиента ТОГДА 
	|			Выразить(ТаблицаЗадолженностей.ЗаказКлиента КАК Документ.РеализацияТоваровУслуг).СуммаДокумента - ТаблицаЗадолженностей.ДолгКлиента
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Оплата,
	|	ВЫБОР
	|		КОГДА ТаблицаЗадолженностей.ЗаказКлиента ССЫЛКА Документ.РеализацияТоваровУслуг И ТаблицаЗадолженностей.ДатаПлатежа <> ДАТАВРЕМЯ(1, 1, 1) ТОГДА 
	|			РАЗНОСТЬДАТ(Выразить(ТаблицаЗадолженностей.ЗаказКлиента КАК Документ.РеализацияТоваровУслуг).Дата, ТаблицаЗадолженностей.ДатаПлатежа, ДЕНЬ)
	|		ИНАЧЕ 0
	|	КОНЕЦ  КАК СрокКредита, 
	// ) Storverk Тюняков  
	...
	// ( Storverk Тюняков 
	|ГДЕ
	|	ТаблицаЗадолженностей.Партнер В (&Партнеры)
	|	
	|Упорядочить ПО 
	...
	// ( Storverk Тюняков
	ДатаОстатков = Новый Граница(КонецДня(ДатаОтчета), ВидГраницы.Включая);
	ДанныеОтчета = 1;
	...
	СтрокаСостояниеВзаиморасчетов = "Состояние взаиморасчетов";
	СтрокаДолгНеПросрочен = "Не просрочено";
	...
	// ( Storverk Тюняков ДЛЯ ОТЛАДКИ 
	//ВнешниеОбработки.Создать("G:\1C\vigruzki\Запросник\zapr.epf",Ложь).Дамп(Запрос);
	// ) Storverk Тюняков  
...
Функция ПолучитьСоотвСтатусовКП()
	
	Соотв = Новый Соответствие;
	Соотв.Вставить(Перечисления.СтатусыКП.Новый, 			Перечисления.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки);
	Соотв.Вставить(Перечисления.СтатусыКП.ВРаботе, 			Перечисления.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки);
	Соотв.Вставить(Перечисления.СтатусыКП.Актуальное, 		Перечисления.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки);
	Соотв.Вставить(Перечисления.СтатусыКП.Согласовано, 		Перечисления.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки);
	Соотв.Вставить(Перечисления.СтатусыКП.Сборка, 			Перечисления.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки);
	Соотв.Вставить(Перечисления.СтатусыКП.ГотовКОтгрузке, 	Перечисления.СостоянияЗаказовКлиентов.ГотовКОтгрузке);
	Соотв.Вставить(Перечисления.СтатусыКП.Выполнено, 		Перечисления.СостоянияЗаказовКлиентов.Закрыт);
	Соотв.Вставить(Перечисления.СтатусыКП.Закрыто, 			Перечисления.СостоянияЗаказовКлиентов.Закрыт);
	
	Возврат Соотв;
	
КонецФункции
Функция ПолучитьТоварыЗаказаИзКП(КПСсылка, ЗаказСсылка)
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ 
	|	Номенклатура.ЕдиницаИзмерения						ТоварыЕдиницаИзмерения,
	|	Номенклатура.ЕдиницаИзмерения.Код					ТоварыЕдиницаИзмеренияКод,
	|	Номенклатура.ЕдиницаИзмерения.МеждународноеСокращение 	ТоварыЕдиницаИзмеренияМеждународноеСокращение,
	|	Номенклатура.ЕдиницаИзмерения.Наименование 			ТоварыЕдиницаИзмеренияНаименование,
	|	Номенклатура.ЕдиницаИзмерения.НаименованиеПолное	ТоварыЕдиницаИзмеренияНаименованиеПолное,
	|	Номенклатура.ЕдиницаИзмерения.Код					ТоварыЕдиницаИзмеренияПересчетЕдиницаКод,
	|	1 													ТоварыЕдиницаИзмеренияПересчетКоэффициент,
	|	Количество 											ТоварыКоличество,
	|	Номенклатура										ТоварыНоменклатура,
	|	Номенклатура.Артикул								ТоварыНоменклатураАртикул,
	|	ПРЕДСТАВЛЕНИЕ(Номенклатура.ТипНоменклатуры)			ТоварыНоменклатураТипНоменклатуры,
	|	ПроцентРучнойСкидки + ПроцентАвтоматическойСкидки	ТоварыПроцентСкидки,
	|	СтавкаНДС											ТоварыСтавкаНДС,
	|	Сумма												ТоварыСумма,
	|	СуммаНДС											ТоварыСуммаНДС,
	|	СуммаРучнойСкидки + СуммаАвтоматическойСкидки		ТоварыСуммаСкидки,
	|	НЕОПРЕДЕЛЕНО										ТоварыХарактеристика,
	|	Цена												ТоварыЦена
	|ИЗ
	|	Документ.КП.Товары
	|ГДЕ
	|	Ссылка = &Ссылка И ЗаказОснование = &Заказ
	|");
...
Процедура ПодменитьСтрокуЗаказа(СтрокаВыгрузки)
	
	// Изменяет значения в строке выгрузке
	// в этой строке приходит информация для выгрузке заказа на сайт
	//  так как заказ связан с КП то в строке изменяется статус и табличные часть товаров
	//	которая считывается из КП
	
	СвязанноеКП = Документы.КП.ПолучитьКПИзЗаказаПоОснованию(СтрокаВыгрузки.Заказ);
	Если Не СвязанноеКП.Пустая() Тогда
		
		ТоварыЗаказа = ПолучитьТоварыЗаказаИзКП(СвязанноеКП, СтрокаВыгрузки.Заказ);
		
		СтрокаВыгрузки.ЗаказСтатусЗаказа	= ПолучитьСоотвСтатусовКП()[СвязанноеКП.Статус];
		СтрокаВыгрузки.Товары 				= ТоварыЗаказа;
		СтрокаВыгрузки.ЗаказСуммаДокумента 	= ТоварыЗаказа.Итог("ТоварыСумма");
