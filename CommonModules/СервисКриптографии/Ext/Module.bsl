...
Функция Зашифровать(Данные, Получатели, ТипШифрования = "CMS", ПараметрыШифрования = Неопределено) Экспорт
	...
	Попытка
		...
		ВернутьРезультатКакАдрес = ВернутьРезультатКакАдресВоВременномХранилище(Данные);
...
//                                     		   который необходимо зашифровать.
//
//   Получатели - ДвоичныеДанные, Структура, ФиксированнаяСтруктура - сертификат получателя зашифрованного сообщения.
//                Либо ДвоичныеДанные файлов сертификатов или Структура с параметрами для поиска сертификатов в хранилище.
//     Структура - ключевые параметры сертификата, используемые для поиска.
//                 Отпечаток или пара СерийныйНомер и Издатель, или Сертификат с двоичными данными.
//       * Отпечаток     - ДвоичныеДанные, Строка - отпечаток сертификат.
...
Функция Расшифровать(ЗашифрованныеДанные, Сертификат, ТипШифрования = "CMS", ПараметрыШифрования = Неопределено) Экспорт
	...
	Попытка
		...
		ВернутьРезультатКакАдрес = ВернутьРезультатКакАдресВоВременномХранилище(ЗашифрованныеДанные);
...
//                                                  который необходимо расшифровать.
//
//   Получатель - Структура - сертификат, который необходимо использовать для расшифровки.
//     * Идентификатор - идентификатор сертификата.
//
...
Функция РасшифроватьБлок(ЗашифрованныеДанные, Получатель, КлючеваяИнформация, ПараметрыШифрования = Неопределено) Экспорт
	...
	Попытка
		...
		ПараметрыМетода.Вставить("security_token", МаркерБезопасности);
		ПараметрыМетода.Вставить("data", ЗашифрованныеДанные);
		ПараметрыМетода.Вставить("ephemeral_key", ДвоичныеДанныеВBase64ПриНеобходимости(КлючеваяИнформация.ephemeral_key));
		ПараметрыМетода.Вставить("session_key", ДвоичныеДанныеВBase64ПриНеобходимости(КлючеваяИнформация.session_key));
		ПараметрыМетода.Вставить("iv_data", ДвоичныеДанныеВBase64ПриНеобходимости(КлючеваяИнформация.iv_data));
...
Функция ДвоичныеДанныеВBase64ПриНеобходимости(Знач Параметр)
...
Функция НайтиСертификат(Сертификат)
	...
	ИначеЕсли (ТипЗнч(Сертификат) = Тип("Структура")
		...
		СвойстваСертификата = СервисКриптографии.ПолучитьСвойстваСертификата(Сертификат.Сертификат);
	Иначе
...
Функция ИмяСобытияШифрованиеБлока()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Шифрование блока'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
