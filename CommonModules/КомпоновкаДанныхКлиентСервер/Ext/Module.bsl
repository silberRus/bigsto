...
// Добавляет элементы отбора в настройки компоновки данных. При этом, если отбор включен в пользовательские настройки,
// то редактируется пользовательская настройка с соответствующим идентификатором. Применяется в случаях, когда
// когда требуется программно установить пользовательскую настройку.
//
// Параметры:
//	КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - элемент структуры КД
//	КоллекцияЭлементовОтбора - КоллекцияЭлементовОтбораКомпоновкиДанных - элементы отбора
//
Процедура ДобавитьОтборыВПользовательскиеНастройки(КомпоновщикНастроек, КоллекцияЭлементовОтбора) Экспорт
	
	Для Каждого ЭлементКДобавлению Из КоллекцияЭлементовОтбора Цикл
		
		Если ТипЗнч(ЭлементКДобавлению) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
		
		// поиск подходящего пользовательского отбора
		ИдентификаторПользовательскойНастройки = "";
		ЭлементыОтборовКомпоновщика = КомпоновщикНастроек.Настройки.Отбор.Элементы;
		Для Каждого ЭлементКомпоновщика Из ЭлементыОтборовКомпоновщика Цикл
			Если ТипЗнч(ЭлементКомпоновщика) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
				Продолжить;
			КонецЕсли;
			Если ЭлементКомпоновщика.ЛевоеЗначение = ЭлементКДобавлению.ЛевоеЗначение Тогда
				ИдентификаторПользовательскойНастройки = ЭлементКомпоновщика.ИдентификаторПользовательскойНастройки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		// если элемент отбора найден,то редактируем его
		Если ЗначениеЗаполнено(ИдентификаторПользовательскойНастройки) Тогда
			...
			ЭлементОтбора.Использование		= ЭлементКДобавлению.Использование;
			ЭлементОтбора.ЛевоеЗначение		= ЭлементКДобавлению.ЛевоеЗначение;
			ЭлементОтбора.ВидСравнения		= ЭлементКДобавлению.ВидСравнения;
			ЭлементОтбора.ПравоеЗначение	= ЭлементКДобавлению.ПравоеЗначение;
			
		Иначе
			// если элемент отбора не найден,то добавляем
			
			ДополнительныеПараметры = Новый Структура("ВПользовательскиеНастройки, ЗаменятьСуществующий", Истина, Истина);
			ДобавитьОтбор(КомпоновщикНастроек, ЭлементКДобавлению.ЛевоеЗначение, ЭлементКДобавлению.ПравоеЗначение, ЭлементКДобавлению.ВидСравнения, ЭлементКДобавлению.Использование, ДополнительныеПараметры);
...
Процедура ДобавитьПараметрыВПользовательскиеНастройки(КомпоновщикНастроек, МассивПараметров) Экспорт
	...
	Для Каждого ЭлементКДобавлению Из МассивПараметров Цикл
		...
		Иначе
			...
			УстановитьПараметр(КомпоновщикНастроек.ПользовательскиеНастройки, ЭлементКДобавлению.Имя, ЭлементКДобавлению.Значение, Истина);
		КонецЕсли;
...
Функция ИспользуетсяОтбор(Элементы, ПолеКомпоновки, ВключаяВложенныеПоля = Ложь)
