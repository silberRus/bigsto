...
Процедура ФормаНастройкиУзловПриСозданииНаСервере(Форма, Отказ) Экспорт
	...
	Если Параметры.Свойство("ПолучитьЗначенияПоУмолчанию") Тогда
		...
		НастройкаОтборовНаУзле                   = НастройкаОтборовНаУзле(ИмяПланаОбмена, Параметры.ВерсияКорреспондента,, 
									ИдентификаторНастройки);
		НастройкаОтборовНаУзлеБазыКорреспондента = НастройкаОтборовНаУзлеБазыКорреспондента(ИмяПланаОбмена, 
									Параметры.ВерсияКорреспондента,, ИдентификаторНастройки);
...
Процедура ФормаНастройкиУзлаБазыКорреспондентаПриСозданииНаСервере(Форма, ИмяПланаОбмена, Данные = Неопределено) Экспорт
	...
	Если Данные <> Неопределено И ТипЗнч(Данные) = Тип("Соответствие") Тогда
		...
		Подключение = ВнешнееСоединениеСБазой(Форма.ПараметрыВнешнегоСоединения);
...
//
Функция ТаблицыКорреспондентаДляЗначенийПоУмолчанию(Знач ИмяПланаОбмена, Знач ВерсияКорреспондента, Знач ИдентификаторНастройки = "") Экспорт
	
	Результат = Новый Массив;
	
	ЗначенияПоУмолчанию = ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента(ИмяПланаОбмена, ВерсияКорреспондента,, ИдентификаторНастройки);
...
Функция ДанныеТаблицКорреспондента(Таблицы, Знач ИмяПланаОбмена) Экспорт
	...
	ДополнительныеДанные = ОписаниеВариантаНастройки.ДополнительныеДанныеДляКорреспондента;
...
Функция УзелПланаОбменаПоКоду(ИмяПланаОбмена, КодУзла) Экспорт
	...
	Возврат ОбменДаннымиПовтИсп.УзелПланаОбменаПоКоду(ИмяПланаОбмена, КодУзла);
...
Процедура УдалитьЗаписиСостоянияОбменовДанными(СтруктураЗаписи) Экспорт
	...
	УдалитьНаборЗаписейВРегистреСведений(СтруктураЗаписи, "СостоянияОбменовДанными");
...
Процедура УдалитьЗаписиСостоянияУспешныхОбменовДанными(СтруктураЗаписи) Экспорт
	...
	УдалитьНаборЗаписейВРегистреСведений(СтруктураЗаписи, "СостоянияУспешныхОбменовДанными");
...
Процедура ЗавершитьЗаписьФайлаСДаннымиОбновления(Параметры) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("Константа.ДанныеДляОтложенногоОбновления");
		...
		ЗначениеКонстанты.Добавить(Хранилище);
...
Функция ТаблицаМонитораОбменаДанными(Знач ПланыОбмена, Знач ДополнительныеСвойстваПланаОбмена = "", Знач ТолькоОшибочные = Ложь) Экспорт
	...
	ПолучитьРезультатыОбменаДляМонитора(МенеджерВременныхТаблиц, ПланыОбмена);
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[ДополнительныеСвойстваПланаОбмена]", ДополнительныеСвойстваПланаОбменаСтрокой(ДополнительныеСвойстваПланаОбмена));
	...
	Для Каждого НастройкаСинхронизации Из НастройкиСинхронизации Цикл
		...
		НастройкаСинхронизации.ИмяПланаОбмена = ЗначениеНастройкиПланаОбмена(
			НастройкаСинхронизации.УзелИнформационнойБазы.Метаданные().Имя,
			"НаименованиеКонфигурацииКорреспондента",
			НастройкаСинхронизации.ВариантОбменаДанными);
			...
			Если НЕ ЗначениеЗаполнено(КодПредопределенногоУзла)
				...
				Если ЗначениеЗаполнено(КодПредопределенногоУзла) Тогда
					...
					ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ВЫБРАТЬ
							...
							|ГДЕ
							|	%1.ЭтотУзел = ЛОЖЬ
							|	И %1.ПометкаУдаления = ЛОЖЬ", УзелОбъект.Метаданные().Имя);
					Запрос = Новый Запрос(ТекстЗапроса);
...
Функция ПредопределенныеУзлыПлановОбменаБСП()
	...
	Для Каждого ИмяПланаОбмена Из ОбменДаннымиПовтИсп.ПланыОбменаБСП() Цикл
		...
		ТекстЗапроса = ТекстЗапроса + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ВЫБРАТЬ
			...
			|	ПланОбмена.%1 КАК %1
			|ГДЕ
			|	%1.ЭтотУзел = ИСТИНА", ИмяПланаОбмена);
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	ЭтоПодчиненныйУзелРИБТребующийУстановкиОбновления = 
		...
		И ОбменДаннымиВызовСервера.ТребуетсяУстановкаОбновления();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ОбменЧерезВнешнееСоединение Тогда
			...
			ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыПоВнешнемуСоединению(ОтказПоСтрокеСценария, Выборка.УзелИнформационнойБазы, Выборка.ВыполняемоеДействие, КоличествоЭлементовВТранзакции);
		...
		ИначеЕсли Выборка.ОбменЧерезВебСервис Тогда
			...
			ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыЧерезWebСервис(Выборка.УзелИнформационнойБазы,
				Выборка.ВыполняемоеДействие, ПараметрыОбмена, ОтказПоСтрокеСценария);
			
		Иначе
			
			// ИНИЦИАЛИЗАЦИЯ ОБМЕНА ДАННЫМИ
			СтруктураНастроекОбмена = ОбменДаннымиПовтИсп.НастройкиОбменаДанными(Выборка.НастройкаВыполненияОбмена, Выборка.НомерСтроки);
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталог(Отказ, УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена, ВыводитьСообщения = Истина) Экспорт
	...
	СтруктураНастроекОбмена = ОбменДаннымиПовтИсп.НастройкиТранспортаУзлаПланаОбмена(УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена);
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервис(
	...
	Прокси = WSПроксиДляУзлаИнформационнойБазы(УзелИнформационнойБазы, СтрокаСообщенияОбОшибке, ПараметрыАутентификации);
	...
	Если ЗначениеЗаполнено(ПсевдонимУзла) Тогда
		...
		// Тогда псевдоним больше не нужен.
		Если ЗначениеЗаполнено(ПсевдонимУзла) Тогда
			// Следует проверить есть ли в корреспонденте такой узел (возможно он уже перекодировался).
			УзелПсевдонимСуществует = Истина;
			СообщениеОбОшибке = "";
			ПараметрыПриемникаПрокси = Прокси.GetIBParameters(ИмяПланаОбмена, ПсевдонимУзла, СообщениеОбОшибке);
			Попытка
			...
			Если ПараметрыПриемника.Свойство("УзелСуществует") Тогда
				УзелПсевдонимСуществует = (ПараметрыПриемника.УзелСуществует = Истина);
			КонецЕсли;
			Если УзелПсевдонимСуществует Тогда
				КодТекущегоУзлаПланаОбмена = ПсевдонимУзла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыЧерезWebСервис(УзелИнформационнойБазы, ДействиеПриОбмене, ПараметрыОбмена, Отказ)
	...
	// ИНИЦИАЛИЗАЦИЯ ОБМЕНА ДАННЫМИ
	СтруктураНастроекОбмена = ОбменДаннымиПовтИсп.НастройкиОбменаУзлаИнформационнойБазы(УзелИнформационнойБазы, ДействиеПриОбмене, Перечисления.ВидыТранспортаСообщенийОбмена.WS, Ложь);
		...
		Иначе
			...
			Попытка
				Если ЗначениеЗаполнено(ПсевдонимУзла) Тогда
					...
					УзелПсевдонимСуществует = Истина;
					СообщениеОбОшибке = "";
					ПараметрыПриемникаПрокси = Прокси.GetIBParameters(СтруктураНастроекОбмена.ИмяПланаОбмена, ПсевдонимУзла, СообщениеОбОшибке);
					Попытка
						ПараметрыПриемника = СериализаторXDTO.ПрочитатьXDTO(ПараметрыПриемникаПрокси);
					Исключение
						ПараметрыПриемника = ЗначениеИзСтрокиВнутр(ПараметрыПриемникаПрокси);
					...
					КонецЕсли;
					Если УзелПсевдонимСуществует Тогда
						СтруктураНастроекОбмена.ТекущийУзелПланаОбменаКод = ПсевдонимУзла;
					КонецЕсли;
				КонецЕсли;
	...
	ИначеЕсли СтруктураНастроекОбмена.ПроизводитьВыгрузкуДанных Тогда
		...
		Если ЗначениеЗаполнено(ПсевдонимУзла) Тогда
			...
			УзелПсевдонимСуществует = Истина;
			СтрокаСообщенияОбОшибке = "";
			ПараметрыПриемникаПрокси = Прокси.GetIBParameters(СтруктураНастроекОбмена.ИмяПланаОбмена, ПсевдонимУзла, СтрокаСообщенияОбОшибке);
			Попытка
				ПараметрыПриемника = СериализаторXDTO.ПрочитатьXDTO(ПараметрыПриемникаПрокси);
			Исключение
				ПараметрыПриемника = ЗначениеИзСтрокиВнутр(ПараметрыПриемникаПрокси);
	...
	Если ЭтоПланОбменаXDTO Тогда
		...
		Если ПроверятьНаличиеУзлаВКорреспонденте Тогда
			...
			Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
				...
				КонецЕсли;
			КонецЕсли;  // Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
		КонецЕсли; // Если ПроверятьНаличиеУзлаВКорреспонденте Тогда
	КонецЕсли; //Если ЭтоПланОбменаXDTO Тогда
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	// Если используются транзакции, открываем новую
	Если ИспользоватьТранзакции Тогда
		НачатьТранзакцию();
	КонецЕсли;
	Попытка
		...
		Если РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка Тогда
			ВызватьИсключение(НСтр("ru = 'В процессе получения данных возникли ошибки.'"));
		КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхВСервисПередачиФайлов() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Сервис передачи файлов.Выгрузка данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаДанныхИзСервисаПередачиФайлов() Экспорт
	
	Возврат НСтр("ru = 'Обмен данными.Сервис передачи файлов.Загрузка данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПолучитьФайлИзХранилищаВСервисе(Знач ИдентификаторФайла, Знач УзелИнформационнойБазы, Знач РазмерЧасти = 1024, Знач ПараметрыАутентификации = Неопределено) Экспорт
	...
	Прокси = WSПроксиДляУзлаИнформационнойБазы(УзелИнформационнойБазы, , ПараметрыАутентификации);
...
Функция НастройкаОтборовНаУзле(Знач ИмяПланаОбмена, Знач ВерсияКорреспондента, ИмяФормы = "", ИдентификаторНастройки = "") Экспорт
	...
	ИмяФормы = ОписаниеВариантаНастройки.ИмяФормыОтборов;
...
Функция ЗначенияПоУмолчаниюНаУзле(Знач ИмяПланаОбмена, Знач ВерсияКорреспондента, ИмяФормы = "", ИдентификаторНастройки = "") Экспорт
	...
	Если ПустаяСтрока(ВерсияКорреспондента) Тогда
		ВерсияКорреспондента = "0.0.0.0";
	КонецЕсли;
	НастройкиПланаОбмена = ОбменДаннымиПовтИсп.НастройкиПланаОбмена(ИмяПланаОбмена, ВерсияКорреспондента);
	ОписаниеВариантаНастройки = ОбменДаннымиПовтИсп.ОписаниеВариантаНастройки(ИмяПланаОбмена, 
								ИдентификаторНастройки, ВерсияКорреспондента);
	ИмяФормы = ОписаниеВариантаНастройки.ИмяФормыЗначенийПоУмолчанию;
	Результат = Неопределено;
	Если ЗначениеЗаполнено(ОписаниеВариантаНастройки.ЗначенияПоУмолчанию) Тогда
		Результат = ОписаниеВариантаНастройки.ЗначенияПоУмолчанию;
	КонецЕсли;
...
Функция ОписаниеЗначенийПоУмолчанию(Знач ИмяПланаОбмена, Знач Настройка, Знач ВерсияКорреспондента, 
									ИдентификаторНастройки = "") Экспорт
	Если НЕ ЕстьАлгоритмМенеджераПланаОбмена("ОписаниеЗначенийПоУмолчанию",ИмяПланаОбмена) Тогда
		Возврат "";
	ИначеЕсли ПустаяСтрока(ВерсияКорреспондента) Тогда
		ВерсияКорреспондента = "0.0.0.0";
	КонецЕсли;
...
Функция ОбщиеДанныеУзлов(Знач ИмяПланаОбмена, Знач ВерсияКорреспондента, Знач ИдентификаторНастройки, 
						ИмяФормы = "") Экспорт
	...
	ОписаниеВариантаНастройки = ОбменДаннымиПовтИсп.ОписаниеВариантаНастройки(ИмяПланаОбмена, 
								ИдентификаторНастройки, ВерсияКорреспондента);
	ИмяФормы = ОписаниеВариантаНастройки.ИмяФормыОбщиеДанныеУзлов;
...
Функция НастройкаОтборовНаУзлеБазыКорреспондента(Знач ИмяПланаОбмена, Знач ВерсияКорреспондента, 
									ИмяФормы = "", ИдентификаторНастройки = "") Экспорт
	...
	ОписаниеВариантаНастройки = ОбменДаннымиПовтИсп.ОписаниеВариантаНастройки(ИмяПланаОбмена,  
								ИдентификаторНастройки, ВерсияКорреспондента);
	ИмяФормы = ОписаниеВариантаНастройки.ИмяФормыОтборовКорреспондента;

	Результат = Неопределено;

	Если ЗначениеЗаполнено(ОписаниеВариантаНастройки.ОтборыКорреспондента) Тогда
		Результат = ОписаниеВариантаНастройки.ОтборыКорреспондента;
	КонецЕсли;
...
Функция ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента(Знач ИмяПланаОбмена, Знач ВерсияКорреспондента, 
											ИмяФормы = "", ИдентификаторНастройки = "") Экспорт
	...
	ОписаниеВариантаНастройки = ОбменДаннымиПовтИсп.ОписаниеВариантаНастройки(ИмяПланаОбмена, 
								ИдентификаторНастройки, ВерсияКорреспондента);
	ИмяФормы = ОписаниеВариантаНастройки.ИмяФормыЗначенийПоУмолчаниюКорреспондента;
	Результат = Неопределено;
	Если ЗначениеЗаполнено(ОписаниеВариантаНастройки.ЗначенияПоУмолчаниюКорреспондента) Тогда
		Результат = ОписаниеВариантаНастройки.ЗначенияПоУмолчаниюКорреспондента;
	КонецЕсли;
...
Функция ОписаниеОграниченийПередачиДанныхБазыКорреспондента(Знач ИмяПланаОбмена, Знач НастройкаОтборовНаУзле, Знач ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	Если НЕ ЕстьАлгоритмМенеджераПланаОбмена("ОписаниеОграниченийПередачиДанныхБазыКорреспондента", ИмяПланаОбмена) Тогда
		Возврат "";
	ИначеЕсли ПустаяСтрока(ВерсияКорреспондента) Тогда
		ВерсияКорреспондента = "0.0.0.0";
	КонецЕсли;
...
Функция ОписаниеЗначенийПоУмолчаниюБазыКорреспондента(Знач ИмяПланаОбмена, Знач ЗначенияПоУмолчаниюНаУзле, Знач ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	Если НЕ ЕстьАлгоритмМенеджераПланаОбмена("ОписаниеЗначенийПоУмолчаниюБазыКорреспондента", ИмяПланаОбмена) Тогда
		Возврат "";
	ИначеЕсли ПустаяСтрока(ВерсияКорреспондента) Тогда
		ВерсияКорреспондента = "0.0.0.0";
	КонецЕсли;
...
Функция ЕстьАлгоритмМенеджераПланаОбмена(ИмяАлгоритма, ИмяПланаОбмена) Экспорт
	НастройкиПланаОбмена = ОбменДаннымиПовтИсп.НастройкиПланаОбмена(ИмяПланаОбмена);
	АлгоритмыМодуля = НастройкиПланаОбмена.Алгоритмы;
	АлгоритмНайден = Неопределено;
	АлгоритмыМодуля.Свойство(ИмяАлгоритма, АлгоритмНайден);
...
Процедура ВыполнитьКонтрольЗаписиНеразделенныхДанных(Знач Данные) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ТекстИсключения = НСтр("ru = 'Недостаточно прав для выполнения действия.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ШаблонИмениФайлаСообщения(ТекущийУзелПланаОбмена, УзелИнформационнойБазы, ЭтоИсходящееСообщение, Транслитерация = Ложь, ИспользоватьВиртуальныйКодУзлаПриПолучении = Ложь)
	...
	КодОтправителя            = СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОтправитель, "Код"));
	КодПолучателя = СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелПолучатель, "Код"));
	...
	Если Транслитерация Тогда
		КодОтправителя = СтроковыеФункцииКлиентСервер.СтрокаЛатиницей(КодОтправителя);
		КодПолучателя = СтроковыеФункцииКлиентСервер.СтрокаЛатиницей(КодПолучателя);
	КонецЕсли;
	
	Возврат ИмяФайлаСообщенияОбмена(КодОтправителя, КодПолучателя, ЭтоИсходящееСообщение);
...
Функция ПсевдонимПредопределенногоУзла(УзелКорреспондента) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	КодУзла
	|ИЗ РегистрСведений.ПсевдонимыПредопределенныхУзлов
	|ГДЕ Корреспондент = &УзелИнформационнойБазы";
	...
	Если УзелКорреспондентаПерекодирован Тогда
		...
		Если ЗначениеЗаполнено(ПсевдонимПредопределенногоУзла)
			...
			Если СокрЛП(КодПредопределенногоУзла) = РезультатАнализаДанныхКЗагрузке.To Тогда
				УдалитьНаборЗаписейВРегистреСведений(Новый Структура("Корреспондент", УзелИнформационнойБазы),
...
Функция КоличествоПроблемОбменаДанными(УзлыОбмена = Неопределено)
	...
	Возврат РегистрыСведений.РезультатыОбменаДанными.КоличествоПроблем(УзлыОбмена, Неопределено);
...
Функция КоличествоНастроенныхОбменов(Знач ПланыОбмена) Экспорт
	...
	ПолучитьПланыОбменаДляМонитора(МенеджерВременныхТаблиц, ПланыОбмена, "");
...
Функция ПараметрыЗапросаПланыОбменаДляМонитора()
	...
	ПараметрыЗапроса.Вставить("ПланыОбменаМассив");
...
//
Функция ТекстЗапросаПланыОбменаДляМонитора(ПараметрыЗапроса = Неопределено)
...
Процедура ПолучитьРезультатыОбменаДляМонитора(МенеджерВременныхТаблиц, ПланыОбменаМассив)
...
Процедура ВыполнитьСравнениеИОбъединениеТаблицФормы(Форма, Отказ)
	...
	ПараметрыИзФормы = Новый Структура("ВерсияКорреспондента, ИдентификаторНастройки");
	ЗаполнитьЗначенияСвойств(ПараметрыИзФормы, Форма);
	ДанныеЭтойИнформационнойБазы = ДанныеДляТабличныхЧастейУзловЭтойИнформационнойБазы(ИмяПланаОбмена,
		ПараметрыИзФормы.ВерсияКорреспондента, ПараметрыИзФормы.ИдентификаторНастройки);
	
	ТабличныеЧастиПланаОбмена = ОбменДаннымиПовтИсп.ТабличныеЧастиПланаОбмена(ИмяПланаОбмена,
		Форма.ВерсияКорреспондента, ПараметрыИзФормы.ИдентификаторНастройки);
...
Процедура ЗаполнитьДанныеФормы(Форма)
	...
	Для Каждого ЭлементНастройки Из НастройкаОтборовНаУзле Цикл
		...
		Если ТипЗнч(РеквизитФормы) = Тип("ДанныеФормыКоллекция") Тогда
			...
			// В случае начального заполнения отбора-коллекции значениями по умолчанию
			// необходимо выполнять преобразование значения отбора, представленного в виде структуры.
			// Значения для заполнения получаются из первого элемента структуры.
			Если ТипЗнч(ЗначениеОтбора) = Тип("Структура") Тогда
				
				НовоеЗначениеОтбора = Новый Массив;
				Для Каждого ЭлементСтруктурыОтбора Из ЗначениеОтбора Цикл
					
					ЗначенияИзСтруктуры = ЭлементСтруктурыОтбора.Значение;
					Для Каждого ЗначениеИзСтруктуры Из ЗначенияИзСтруктуры Цикл
						
						ОписаниеЗначения = Новый Структура();
						ОписаниеЗначения.Вставить("Использовать",  Истина);
						ОписаниеЗначения.Вставить("Представление", ЗначениеИзСтруктуры);
						
						УникальныйИдентификаторСсылки = ?(ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ЗначениеИзСтруктуры)),
							ЗначениеИзСтруктуры.УникальныйИдентификатор(),
							Неопределено);
						ОписаниеЗначения.Вставить("УникальныйИдентификаторСсылки", Строка(УникальныйИдентификаторСсылки));
						
						НовоеЗначениеОтбора.Добавить(ОписаниеЗначения);
				...
				ЗначениеОтбора = НовоеЗначениеОтбора;
				НастройкаОтборовНаУзле[ЭлементНастройки.Ключ] = ЗначениеОтбора;
			...
			Если ТипЗнч(ЗначениеОтбора) = Тип("Массив")
				...
				Для Каждого СтрокаТаблицы Из ЗначениеОтбора Цикл
		...
		Иначе
			...
			Форма[ИмяРеквизита] = ЗначениеОтбора;
...
Функция ОбщиеДанныеУзловКорреспондента(Знач ИмяПланаОбмена, Знач ПараметрыПодключения, Отказ)
	...
	Если ПараметрыПодключения.ТипСоединения = "ВнешнееСоединение" Тогда
		...
		Подключение = ВнешнееСоединениеСБазой(ПараметрыПодключения);
		СтрокаСообщенияОбОшибке = Подключение.ПодробноеОписаниеОшибки;
		ВнешнееСоединение       = Подключение.Соединение;
...
Функция ПараметрыИнформационнойБазы(Знач ИмяПланаОбмена, Знач КодУзла, СообщениеОбОшибке) Экспорт
	...
	Результат.Вставить("УзелСуществует"); // Начиная с версии БСП 2.4.2(?).
	...
	Если Результат.ПланОбменаСуществует Тогда
		...
		Результат.УзелСуществует                            = ?(ЗначениеЗаполнено(КодУзла),
		                                                           ЗначениеЗаполнено(ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзла))
		                                                           , Ложь);
...
//
Процедура ПроверитьПодключениеОбработкиТранспортаСообщенийОбмена(Отказ, СтруктураНастроек, ВидТранспорта, СообщениеОбОшибке = "", НовыйПароль = Неопределено) Экспорт
	...
	Если СтруктураНастроек.Свойство("Узел") Тогда
		...
		Иначе
			...
			Пароли.Вставить("COMПарольПользователя", НовыйПароль);
			Пароли.Вставить("FTPСоединениеПароль", НовыйПароль);
			Пароли.Вставить("WSПароль", НовыйПароль);
			Пароли.Вставить("ПарольАрхиваСообщенияОбмена", НовыйПароль);
		КонецЕсли;
...
Функция WSПроксиДляУзлаИнформационнойБазы(УзелИнформационнойБазы, СтрокаСообщенияОбОшибке = "", ПараметрыАутентификации = Неопределено)
	...
	Если ВерсияКорреспондента_2_1_1_7 Тогда
		...
		Попытка
			...
			Если НЕ ЕстьПодключение Тогда
				Если ЭтоПланОбменаXDTO(Корреспондент) Тогда
...
Функция УникальноеИмяФайлаСообщенияОбмена(Расширение = "xml")
...
//      * ВерсииФорматаОбмена                            - Структура - Соответствие номеров поддерживаемых версий формата
...
Функция НастройкиПланаОбменаПоУмолчанию(ИмяПланаОбмена)
	...
	Параметры.Вставить("ВидГруппыДляВариантовНастроек",                    ВидГруппыФормы.Подменю);
	Параметры.Вставить("ИмяКонфигурацииИсточника",                         "");
	Параметры.Вставить("ВерсииФорматаОбмена",                              Новый Структура);
	...
	Алгоритмы.Вставить("ОписаниеОграниченийПередачиДанныхБазыКорреспондента",    Ложь);
	Алгоритмы.Вставить("ОписаниеЗначенийПоУмолчанию",                            Ложь);
	Алгоритмы.Вставить("ОписаниеЗначенийПоУмолчаниюБазыКорреспондента",          Ложь);
...
Функция ОписаниеВариантаНастройкиОбменаПоУмолчанию(ИмяПланаОбмена)
	...
	ОписаниеВарианта.Вставить("ОтборыКорреспондента",                                  Новый Структура);
	ОписаниеВарианта.Вставить("ЗначенияПоУмолчаниюКорреспондента",                     Новый Структура);
	ОписаниеВарианта.Вставить("ИмяФормыОтборов",                                       "ФормаНастройкиУзла");
	ОписаниеВарианта.Вставить("ИмяФормыЗначенийПоУмолчанию",                           "ФормаНастройкиЗначенийПоУмолчанию");
	ОписаниеВарианта.Вставить("ИмяФормыОтборовКорреспондента",                         "ФормаНастройкиУзлаБазыКорреспондента");
	ОписаниеВарианта.Вставить("ИмяФормыЗначенийПоУмолчаниюКорреспондента",             "ФормаНастройкиЗначенийПоУмолчаниюБазыКорреспондента");
	ОписаниеВарианта.Вставить("ИмяФормыОбщиеДанныеУзлов",                              "ФормаНастройкиУзлов");
	...
	ОписаниеВарианта.Вставить("ДополнительныеДанныеДляКорреспондента",                 Новый Структура);
	ОписаниеВарианта.Вставить("ПояснениеДляНастройкиПараметровУчета",                  "");
	ОписаниеВарианта.Вставить("ПояснениеДляНастройкиПараметровУчетаБазыКорреспондента","");
	...
	ОписаниеВарианта.Вставить("ОтображатьНастройкуОтборовНаУзле",                      Истина);
	ОписаниеВарианта.Вставить("ОтображатьЗначенияПоУмолчаниюНаУзле",                   Истина);
	ОписаниеВарианта.Вставить("ОтображатьНастройкуОтборовНаУзлеБазыКорреспондента",    Истина);
	ОписаниеВарианта.Вставить("ОтображатьЗначенияПоУмолчаниюНаУзлеБазыКорреспондента", Истина);
	ОписаниеВарианта.Вставить("НаименованиеКонфигурацииКорреспондента",                МетаданныеПланаОбмена.Синоним);
...
Функция КодПредопределенногоУзлаПланаОбмена(ИмяПланаОбмена) Экспорт
	...
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбменДаннымиПовтИсп.ПолучитьЭтотУзелПланаОбмена(ИмяПланаОбмена), "Код");
КонецФункции
