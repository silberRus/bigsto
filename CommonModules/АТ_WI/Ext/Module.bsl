	...
	Если Не ЭтоОтладка Тогда
		Для Каждого Элемент Из Запрос.ПараметрыURL Цикл
			Если ПараметрыССайта = Неопределено Тогда
				ПараметрыССайта = Новый Структура;
			КонецЕсли;
			ПараметрыССайта.Вставить(Элемент.Ключ, Элемент.Значение);
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьКонтрагентовPOST(ЗапросHTTP, ЭтоОтладка = Ложь) Экспорт
	...
	Если АвторизацияПоКлючуПройдена(ЗапросHTTP, Ответ, ЭтоОтладка, ПараметрыКлюча) Тогда
		...
		|			ВЫБОР КОГДА Адр.БизнесРегион В ИЕРАРХИИ(&РегионыЭкспресс) ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ КОНЕЦ КАК Экспресс
		...
		Экспресс = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Справочники.АТ_ВидыДоставок.Экспресс, "Наименование, Номенклатура, Цена, БизнесРегионы");
		
		Запрос.УстановитьПараметр("Партнер", ПараметрыКлюча.Usr.Партнер);
		Запрос.УстановитьПараметр("РегионыЭкспресс", Экспресс.БизнесРегионы.Выгрузить().ВыгрузитьКолонку("Регион"));
		...
		Пока ВыборкаКонтра.Следующий() Цикл
			...
			Пока ВыборкаАдрес.Следующий() Цикл
				...
				Если ВыборкаАдрес.Экспресс Тогда
					Доставка.Добавить(Новый Структура("guid,name,price", Экспресс.Номенклатура, Экспресс.Наименование, Экспресс.Цена));
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ВыборкаАдрес.Адрес) Тогда
					Адреса.Добавить(Новый Структура("guid,name,delivery", ВыборкаАдрес.Адрес, ВыборкаАдрес.АдресПредставление, Доставка));
				КонецЕсли;
			КонецЦикла;
...
Функция Корзина_ПроверитьPOST(ЗапросHTTP, ЭтоОтладка = Ложь) Экспорт
	
	Перем Ответ, ПараметрыКлюча;
	
	Если АвторизацияПоКлючуПройдена(ЗапросHTTP, Ответ, ЭтоОтладка, ПараметрыКлюча) Тогда
		
		Соглашение = АТ_Сервер.ПолучитьСформироватьСоглашение(ПараметрыКлюча.Usr.Партнер, Новый Структура("ФормаОплаты", Перечисления.ФормыОплаты.Безналичная));
		СкидкаПоСоглашению = СТ_ОбщегоНазначения.ПолучитьПроцентСкидкиПоСоглашению(Соглашение);
		
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	АТ_ИнтернетКорзина.Номенклатура КАК guid,
		|	АТ_ИнтернетКорзина.Номенклатура.Артикул КАК sku,
		|	АТ_ИнтернетКорзина.Номенклатура.Наименование КАК name,
		|	АТ_ИнтернетКорзина.Номенклатура.Производитель КАК manufacturer,
		|	ПРЕДСТАВЛЕНИЕ(АТ_ИнтернетКорзина.Склад) КАК store,
		|	АТ_ИнтернетКорзина.Количество КАК qty,
		|	АТ_ИнтернетКорзина.Цена КАК old_price,
		|	СвободныеОстатки.ВНаличииОстаток - СвободныеОстатки.ВРезервеСоСкладаОстаток - СвободныеОстатки.ВРезервеПодЗаказОстаток КАК available_qty,
		|	ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) * ((100 - ВЫБОР
		|		КОГДА ЕСТЬNULL(ДополнительнаяСкидка.Скидка, 0) = 0
		|			ТОГДА &СкидкаПоСоглашению
		|		ИНАЧЕ ЕСТЬNULL(ДополнительнаяСкидка.Скидка, 0)
		|	КОНЕЦ) / 100) КАК price,
		|	ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) * ((100 - ВЫБОР
		|		КОГДА ЕСТЬNULL(ДополнительнаяСкидка.Скидка, 0) = 0
		|			ТОГДА &СкидкаПоСоглашению
		|		ИНАЧЕ ЕСТЬNULL(ДополнительнаяСкидка.Скидка, 0)
		|	КОНЕЦ) / 100) * АТ_ИнтернетКорзина.Количество КАК full_price
		|ИЗ
		|	РегистрСведений.АТ_ИнтернетКорзина КАК АТ_ИнтернетКорзина
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СвободныеОстатки.Остатки КАК СвободныеОстатки
		|		ПО АТ_ИнтернетКорзина.Номенклатура = СвободныеОстатки.Номенклатура
		|			И АТ_ИнтернетКорзина.Склад = СвободныеОстатки.Склад
		|			И (СвободныеОстатки.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(, ВидЦены = &ВидыЦены) КАК ЦеныНоменклатуры
		|		ПО АТ_ИнтернетКорзина.Номенклатура = ЦеныНоменклатуры.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СТ_ДополнительныеСкидки.СрезПоследних(, Соглашение = &Соглашение) КАК ДополнительнаяСкидка
		|		ПО АТ_ИнтернетКорзина.Номенклатура.Родитель = ДополнительнаяСкидка.Группа
		|			И АТ_ИнтернетКорзина.Номенклатура.Производитель = ДополнительнаяСкидка.Производитель
		|ГДЕ
		|	АТ_ИнтернетКорзина.ПользовательИнтернет = &ПользовательИнтернет
		|	И (АТ_ИнтернетКорзина.Количество <> СвободныеОстатки.ВНаличииОстаток - СвободныеОстатки.ВРезервеСоСкладаОстаток - СвободныеОстатки.ВРезервеПодЗаказОстаток
		|			ИЛИ АТ_ИнтернетКорзина.Цена <> ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) * ((100 - ВЫБОР
		|				КОГДА ЕСТЬNULL(ДополнительнаяСкидка.Скидка, 0) = 0
		|					ТОГДА &СкидкаПоСоглашению
		|				ИНАЧЕ ЕСТЬNULL(ДополнительнаяСкидка.Скидка, 0)
		|			КОНЕЦ) / 100))";
		...
		Запрос.УстановитьПараметр("ПользовательИнтернет", ПараметрыКлюча.Usr);
		Запрос.УстановитьПараметр("ВидыЦены", Соглашение.ВидЦен);
		Запрос.УстановитьПараметр("Соглашение", Соглашение);
		Запрос.УстановитьПараметр("СкидкаПоСоглашению", СкидкаПоСоглашению);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Если РезультатЗапроса.Пустой() Тогда
			
			Ответ = ВернутьОтвет(Новый Структура("Тело", Истина), ЭтоОтладка);
			
		Иначе
			
			Выгрузка = РезультатЗапроса.Выгрузить();
			full_price = Выгрузка.Итог("full_price");
			
			Ответ = ВернутьОтвет(Новый Структура("Тело", Новый Структура("full_price, cart", full_price, Выгрузка)), ЭтоОтладка);
...
Функция ПоискPOST(ЗапросHTTP, ЭтоОтладка = Ложь) Экспорт
	
	Перем Ответ, ПараметрыКлюча;
	
	Если АвторизацияПоКлючуПройдена(ЗапросHTTP, Ответ, ЭтоОтладка, ПараметрыКлюча) Тогда
		
		Параметры = ПолучитьСтруктураИзТелаИПроверить(ЗапросHTTP, Ответ, "sku,count",, ЭтоОтладка);
		
		Если Параметры <> Неопределено Тогда
			
			Параметры.Вставить("Usr", ПараметрыКлюча.Usr);
			
			SKU = СТ_ОбщегоНазначения.ПолучитьКлючПоСтроке(Параметры.sku);
			
			Запрос = Новый Запрос;
			Запрос.Текст =
			СтрШаблон("ВЫБРАТЬ РАЗЛИЧНЫЕ %1
			|	СправочникНоменклатура.Ссылка КАК guid,
			|	СправочникНоменклатура.Артикул КАК sku,
			|	СправочникНоменклатура.Наименование КАК name,
			|	ПРЕДСТАВЛЕНИЕ(СправочникНоменклатура.Производитель) КАК manufacturer
			|ИЗ
			|	РегистрСведений.АналогиТоваров.СрезПоследних(
			|			,
			|			АртикулДляПоиска ПОДОБНО &Артикул
			|				И НЕ Удалено) КАК АналогиТоваровСрезПоследних
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
			|		ПО АналогиТоваровСрезПоследних.Номенклатура = СправочникНоменклатура.Ссылка
			|	УПОРЯДОЧИТЬ ПО sku",
			"ПЕРВЫЕ " + Параметры.count);
			Запрос.УстановитьПараметр("Артикул", SKU + "%");
			РезультатЗапроса = Запрос.Выполнить();
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Возврат ВернутьОтвет(Новый Структура("Тело", Товары), ЭтоОтладка);
...
Функция ПоискГУИДPOST(ЗапросHTTP, ЭтоОтладка = Ложь) Экспорт
	
	Перем Ответ, ПараметрыКлюча;
	
	Если АвторизацияПоКлючуПройдена(ЗапросHTTP, Ответ, ЭтоОтладка, ПараметрыКлюча) Тогда
		
		Параметры = ПолучитьСтруктураИзТелаИПроверить(ЗапросHTTP, Ответ, "guid, address",, ЭтоОтладка);
		...
		Если Параметры <> Неопределено Тогда
			...
			Запрос.Текст =
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	АналогиТоваров.Номенклатура КАК guid,
			|	СправочникНоменклатура.Артикул КАК sku,
			|	СправочникНоменклатура.Наименование КАК name,
			|	СправочникНоменклатура.Производитель.Наименование КАК manufacturer,
			|	ВЫБОР
			|		КОГДА АналогиТоваров.Номенклатура = &Ссылка
			|			ТОГДА 1
			|		КОГДА СвободныеОстаткиОстатки.ВНаличииОстаток - СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток - СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток <= 0
			|			ТОГДА -1
			|		ИНАЧЕ 0
			|	КОНЕЦ КАК Порядок,
			|	СвободныеОстаткиОстатки.Склад КАК store_guid,
			|	СвободныеОстаткиОстатки.ВНаличииОстаток - СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток - СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток КАК store_qty,
			|	СвободныеОстаткиОстатки.Склад.Наименование КАК store_name,
			|	ЕСТЬNULL(ЦенаНоменклатуры.Цена, 0) * ((100 - ВЫБОР
			|		КОГДА ЕСТЬNULL(ДополнительнаяСкидка.Скидка, 0) = 0
			|			ТОГДА &СкидкаПоСоглашению
			|		ИНАЧЕ ЕСТЬNULL(ДополнительнаяСкидка.Скидка, 0)
			|	КОНЕЦ) / 100) КАК store_price,
			|	ЕСТЬNULL(АТ_СрокиДоставки.ВремяВДнях, 0) КАК store_delivery_time
			|ИЗ
			|	РегистрСведений.АналогиТоваров.СрезПоследних(, НЕ Удалено) КАК АналогиТоваров
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналогиТоваров.СрезПоследних(
			|				,
			|				Номенклатура = &Ссылка
			|					И НЕ Удалено) КАК АналогиТоваровСрезПоследних
			|		ПО АналогиТоваров.Артикул = АналогиТоваровСрезПоследних.Артикул
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
			|		ПО АналогиТоваров.Номенклатура = СправочникНоменклатура.Ссылка
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СвободныеОстатки.Остатки КАК СвободныеОстаткиОстатки
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АТ_СрокиДоставки КАК АТ_СрокиДоставки
			|			ПО СвободныеОстаткиОстатки.Склад = АТ_СрокиДоставки.Склад
			|				И (АТ_СрокиДоставки.БизнесРегион = &БизнесРегион)
			|		ПО АналогиТоваров.Номенклатура = СвободныеОстаткиОстатки.Номенклатура
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(, ВидЦены = &ВидЦены) КАК ЦенаНоменклатуры
			|		ПО АналогиТоваров.Номенклатура = ЦенаНоменклатуры.Номенклатура
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СТ_ДополнительныеСкидки.СрезПоследних(, Соглашение = &Соглашение) КАК ДополнительнаяСкидка
			|		ПО АналогиТоваров.Номенклатура.Производитель = ДополнительнаяСкидка.Производитель
			|			И АналогиТоваров.Номенклатура.Родитель = ДополнительнаяСкидка.Группа
			|
			|УПОРЯДОЧИТЬ ПО
			|	Порядок УБЫВ,
			|	store_price
			...
			//|	Спр.Производитель КАК manufacturer,
			//|	ВЫБОР
			//|		КОГДА Спр.Ссылка = &Ссылка
			//|			ТОГДА 1
			//|		ИНАЧЕ 0
			//|	КОНЕЦ КАК Порядок,
			//|	СвободныеОстаткиОстатки.Склад КАК Склад,
			//|	СвободныеОстаткиОстатки.ВНаличииОстаток - СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток - СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток КАК qty,
			//|	СвободныеОстаткиОстатки.Склад.Наименование КАК storename,
			//|	ЦеныНоменклатурыСрезПоследних.Цена КАК price
			//|ИЗ
			//|	Справочник.Номенклатура КАК Спр
			//|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
			//|			АналогиТоваров.Номенклатура КАК Номенклатура
			//|		ИЗ
			//|			РегистрСведений.АналогиТоваров.СрезПоследних(, НЕ Удалено) КАК АналогиТоваров
			//|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналогиТоваров.СрезПоследних(, Номенклатура = &Ссылка И НЕ Удалено) КАК АналогиТоваровСрезПоследних
			//|				ПО АналогиТоваров.Артикул = АналогиТоваровСрезПоследних.Артикул
			//|		СГРУППИРОВАТЬ ПО
			//|			АналогиТоваров.Номенклатура) КАК Аналоги
			//|		ПО Спр.Ссылка = Аналоги.Номенклатура
			//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СвободныеОстатки.Остатки КАК СвободныеОстаткиОстатки
			...
			Запрос.УстановитьПараметр("Ссылка", Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(Параметры.guid)));
			Запрос.УстановитьПараметр("ВидЦены", Соглашение.ВидЦен);
			Запрос.УстановитьПараметр("Соглашение", Соглашение);
			Запрос.УстановитьПараметр("БизнесРегион", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справочники.БП_АдресаДоставкиКонтрагентов.ПолучитьСсылку(Новый УникальныйИдентификатор(Параметры.address)), "БизнесРегион"));
			Запрос.УстановитьПараметр("СкидкаПоСоглашению", СкидкаПоСоглашению);
			РезультатЗапроса = Запрос.Выполнить();
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Пока ВыборкаПоНоменклатуре.Следующий() Цикл
					...
					СтрокаНоменклатура = Новый Структура("guid, sku, name, manufacturer, stores");
					...
					Пока Выборка.Следующий() Цикл
						СтрокаСклад = Новый Структура("store_guid, store_name, store_qty, store_delivery_time, store_price");
						ЗаполнитьЗначенияСвойств(СтрокаСклад, Выборка);
						Склады.Добавить(СтрокаСклад);
					КонецЦикла;
					
					
					СтрокаНоменклатура.Вставить("stores", Склады);
					Результат.Добавить(СтрокаНоменклатура);
					
				КонецЦикла;
				
				Возврат ВернутьОтвет(Новый Структура("Тело", Результат), ЭтоОтладка);
