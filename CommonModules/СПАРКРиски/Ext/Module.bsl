...
Функция ИндексыСПАРККонтрагента(Контрагент, ПризнакЗагрузкиДанных = Ложь) Экспорт
	...
	Если УслугаПодключена <> Истина Тогда
		...
		Результат.Вставить("ИнформацияОбОшибке", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если ТипЗнч(Контрагент) = ТипСтрока Тогда
		...
		Если СПАРКРискиКлиентСервер.ИННСоответствуетТребованиям(КонтрагентИНН, ТекстОшибки) = Истина Тогда
			// Если работаем "в коробке" и не указаны данные подключения к интернет-поддержке, то сгенерировать ошибку.
			...
			Если ДанныеАутентификации = Неопределено Тогда
				...
				Результат.Вставить("ВидОшибки", Перечисления.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена);
			Иначе
...
Процедура ОтобразитьИндексыСПАРК(
	...
	Если ИспользованиеРазрешено
			И (ТипЗнч(РезультатИндексыКонтрагента) <> ТипСтруктура) Тогда
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	НастройкиВарианта = ВариантыОтчетов.ОписаниеВарианта(Настройки, Метаданные.Отчеты.ИндексыСПАРКРиски, "ИндексыСПАРКРиски");
	...
	НастройкиВарианта = ВариантыОтчетов.ОписаниеВарианта(Настройки, Метаданные.Отчеты.СобытияМониторингаСПАРКРиски, "Основной");
...
#КонецОбласти
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ИнтернетПоддержкаПользователей.ЭтоРазделеннаяКонфигурация(),
...
Процедура ОбновлениеИнформационнойБазы_АктуализироватьОписаниеТиповСобытий() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ТипСобытияОбъект);
	КонецЦикла;
