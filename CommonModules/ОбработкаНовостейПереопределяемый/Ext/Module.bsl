...
//  МассивСтруктурНовостей           - Массив - Этот массив новостей можно редактировать. Например, удалить некоторые
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию() Экспорт
	...
	ПараметрыПоиска.Вставить("Метаданные", Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей);
	ТекущееРегламентноеЗадание = РегламентныеЗаданияСервер.НайтиЗадания(ПараметрыПоиска);
	
	Для Каждого РегЗадание Из ТекущееРегламентноеЗадание Цикл
...
Функция ПолучитьЗначениеПредопределеннойКатегории(Категория, Знач ПредыдущийРезультат) Экспорт
	...
	Если ВРег(КатегорияКод) = ВРег("ProductName") Тогда // В плане видов характеристик "КатегорииНовостей" такого элемента нет.
		...
		Если ПолучитьФункциональнуюОпцию("УправлениеПредприятием") Тогда
			НовыйРезультат = "Enterprise20";
		ИначеЕсли ПолучитьФункциональнуюОпцию("КомплекснаяАвтоматизация") Тогда
			НовыйРезультат = "ARAutomation20";
		Иначе
			Если ПолучитьФункциональнуюОпцию("БазоваяВерсия") Тогда
				НовыйРезультат = "TradeBase";
			Иначе
				НовыйРезультат = "Trade";
			КонецЕсли;
		КонецЕсли;
...
// Процедура заполняет массив кодами категорий, для которых необходимо найти значения категорий в новостях,
//   для дальнейшего расчета соответствия этих значений настройкам базы.
//
// Параметры:
//  КодыКатегорийНовостей - Массив - Массив кодов категорий новостей;
//  ОбластьВидимости      - Строка - "Общее" или "ДляОбластиДанных";
