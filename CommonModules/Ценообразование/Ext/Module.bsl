...
//
// Параметры:
//  Документ - ДокументОбъект.РеализацияТоваровУслуг, ДокументОбъект.ПриобретениеТоваровУслуг
//  ИмяТабличнойЧасти - Строка - Имя табличная части объекта документа
//
Процедура РассчитатьСуммыВзаиморасчетовВТабличнойЧасти(Документ, ИмяТабличнойЧасти, СтруктураКурса = Неопределено, Дата = Неопределено) Экспорт
...
Процедура СообщитьОбОшибкахКорректностьЗаполненияНоменклатурыПоставщика(Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru='В строке не может быть выбрана ""%НоменклатураПоставщика%"", т.к. в предыдущих строках для номенклатуры поставщика указано другое соответствие номенклатуре'");
...
#Область ПроцедурыИФункцииДляЗаполненияЦенПоВидамЦен
...
Функция ЦеныНоменклатурыПоставщика(Таблица, ПараметрыПолученияЦен) Экспорт
	...
	Запрос.УстановитьПараметр("Соглашение",   ПараметрыПолученияЦен.Соглашение);
	Запрос.УстановитьПараметр("ЭтоВыкупТары", ПараметрыПолученияЦен.ЭтоВыкупТары);
