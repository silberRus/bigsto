...
//
// Параметры:
//  ЭлементыФормы		 - ВсеЭлементыФормы	 - все элементы формы для управления свойствами и переключения закладок,
...
Процедура ПриЧтенииСозданииРаспоряженийНаСервере(ЭлементыФормы,
	ДокОбъект, НакладнаяПоЗаказу = Ложь, ПараметрыЗаполнения = Неопределено, СводнаяИнформация = Неопределено) Экспорт
...
Процедура УстановитьДоступностьАдресовЗаполнитьСпискиВремени(ЭлементыФормы) Экспорт
	...
	РекурсивноУстановитьДоступностьАдресовЗаполнитьСпискиВремени(ЭлементыФормы, РедактированиеРазрешено);
...
//
// Параметры:
//	ЭлементыФормы - ВсеЭлементыФормы - элементы формы, для которых нужно настроить пользовательскую видимость;
//	ДокОбъект - ДокументОбъект.КоммерческоеПредложениеКлиенту - документ, в котором заполняется способ доставки.
//
...
Процедура ОбновитьСписокРаспоряженийНаДоставку(Форма, ОбновляемыеРаспоряжения = Неопределено) Экспорт
	...
	Если ИсходящаяДоставка Тогда
		...
		ТекстЗапроса = ?(СкладыСервер.ЕстьОрдерныйНаОтгрузкуСклад(Склад),
			ТекстЗапросаРаспоряженияНаДоставкуПолучателямПоОрдернойСхеме(ВыборочноеОбновление),
			ТекстЗапросаРаспоряженияНаДоставкуПолучателямБезОрдернойСхемы(ВыборочноеОбновление));
	...
	Иначе
		...
		ТекстЗапроса = ?(ЕстьОрдерныйНаПоступлениеСклад,
			ТекстЗапросаРаспоряженияНаДоставкуОтОтправителейПоОрдернойСхеме(ВыборочноеОбновление),
			ТекстЗапросаРаспоряженияНаДоставкуОтОтправителейБезОрдернойСхемы(ВыборочноеОбновление));
	...
	// ( Storverk Тюняков 30.12.2016
	//Если Форма.ИмяФормы = "Обработка.РабочееМестоМенеджераПоДоставке.Форма.Форма" Тогда
	СТ_ДоставкаТоваров.ЗаполнитьРеализацииВДеревеРаспоряжений(РаспоряженияНаДоставку, ЗонаГруппаИлиПустая);
	//КонецЕсли; 
	// ) Storverk Тюняков 30.12.2016
...
Процедура ЗаполнитьПризнакДоставляетсяПолностью(ТоварыРаспоряжений,
	...
	|ГДЕ
	|	ОстаткиПоРаспоряжению.Распоряжение ЕСТЬ NULL
	|	И НЕ ОстаткиПоРаспоряжению.Распоряжение ССЫЛКА Справочник.СоглашенияСПоставщиками
	|	И НЕ ОстаткиПоРаспоряжению.Распоряжение ССЫЛКА Справочник.ДоговорыКонтрагентов" + ОбщегоНазначенияУТ.РазделительЗапросовПакета();
...
//
// Параметры:
//  ТранспортноеСредство - СправочникСсылка.ТранспортноеСредство, СправочникСсылка.ТипТранспортногоСредства	 - транспортное средство,
...
//  *ГрузоподъемностьВТоннах - Число - грузподъемность в тоннах
...
//
// Параметры:
//  КонтейнерРеквизитовДоставки	 - СправочникСсылка.СоглашенияСПоставщиками,
...
Функция ТекстЗапросаЗаполненияЗаданияНаПеревозкуПоРасходнымОрдерам()
	...
	|	ТоварыОрдеровИКОтгрузке.Склад,
	|	ТоварыОрдеровИКОтгрузке.Распоряжение,
	|	ТоварыОрдеровИКОтгрузке.Получатель" + ОбщегоНазначенияУТ.РазделительЗапросовПакета();
	...
	ТекстЗапроса = ТекстЗапроса + "
	|ИНДЕКСИРОВАТЬ ПО
	|	РаспоряженияНаДоставку.Ссылка" + ОбщегоНазначенияУТ.РазделительЗапросовПакета() + 
	"ВЫБРАТЬ
...
Функция ПолучитьПараметрыПриемки(ОбъектСсылка, Знач ВариантПриемкиТоваров = Неопределено, НакладнаяПоЗаказам = Ложь)
	...
	Если Справочники.СоглашенияСПоставщиками.СоглашениеИспользуетсяПриПриемке(ВариантПриемкиТоваров) Тогда
		...
		Если ВариантПриемкиТоваров = ПредопределенноеЗначение("Перечисление.ВариантыПриемкиТоваров.НеРазделенаПоНакладным") Тогда
			...
			СтруктураВозврата.РаспоряжениеНаПриемку		= Тип("ДокументСсылка.ПриобретениеТоваровУслуг");
		...
		ИначеЕсли ВариантПриемкиТоваров = ПредопределенноеЗначение("Перечисление.ВариантыПриемкиТоваров.НеРазделенаПоЗаказамИНакладным") Тогда
			...
			СтруктураВозврата.ДокументПоступление		= Тип("СправочникСсылка.СоглашенияСПоставщиками");
			СтруктураВозврата.РаспоряжениеНаПриемку		= Тип("ДокументСсылка.ЗаказПоставщику");
		...
		ИначеЕсли ВариантПриемкиТоваров = ПредопределенноеЗначение("Перечисление.ВариантыПриемкиТоваров.МожетПроисходитьБезЗаказовИНакладных") Тогда 
			...
			СтруктураВозврата.ДокументПоступление		= Тип("СправочникСсылка.СоглашенияСПоставщиками");
			СтруктураВозврата.РаспоряжениеНаПриемку		= Тип("ДокументСсылка.РегистраторГрафикаДвиженияТоваров");
	...
	СтруктураВозврата.ЭтоРаспоряжение					= ТипЗнч(Ссылка) = СтруктураВозврата.РаспоряжениеНаПриемку;
...
Функция РекурсивноУстановитьДоступностьАдресовЗаполнитьСпискиВремени(ЭлементыФормы, РедактированиеРазрешено)
...
Функция ТекстЗапросаРаспоряженияНаДоставкуПолучателямПоОрдернойСхеме(ВыборочноеОбновление)
	...
	|	ГДЕ
	|		ТоварыКДоставке.Распоряжение ЕСТЬ NULL
	|		И РаспоряженияНаДоставку.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуСоСклада), ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуВПункте))) КАК ТоварыКОтгрузкеДетально
...
Функция ТекстЗапросаРаспоряженияНаДоставкуПолучателямБезОрдернойСхемы(ВыборочноеОбновление)
	...
	|ИЗ
	|	РегистрСведений.СостоянияИРеквизитыДоставки КАК РаспоряженияНаДоставку
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварыКДоставке КАК ПолностьюДоставленныеЗаказы
	|		ПО (ПолностьюДоставленныеЗаказы.Распоряжение = РаспоряженияНаДоставку.Распоряжение)
	|			И (ПолностьюДоставленныеЗаказы.Склад = РаспоряженияНаДоставку.Склад)
	|			И (ПолностьюДоставленныеЗаказы.ВсеТовары)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОбновляемыеРаспоряжения КАК ОбновляемыеРаспоряжения
	|		ПО РаспоряженияНаДоставку.Распоряжение = ОбновляемыеРаспоряжения.Распоряжение
	|			И РаспоряженияНаДоставку.Склад = ОбновляемыеРаспоряжения.Склад
	|ГДЕ
	|	(ПолностьюДоставленныеЗаказы.Распоряжение ЕСТЬ NULL
	|			ИЛИ ПолностьюДоставленныеЗаказы.ЗаданиеНаПеревозку = &ЗаданиеНаПеревозкуПодбор
	|			ИЛИ НЕ ПолностьюДоставленныеЗаказы.ЗаданиеНаПеревозку.Проведен)
	|	И РаспоряженияНаДоставку.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияДоставки.ТребуетсяДоставка)
	|	И ВЫБОР
	|			КОГДА &ОтборПоТипуИсполнителей = ЗНАЧЕНИЕ(Перечисление.ТипыИсполнителейЗаданийНаПеревозку.НашаТранспортнаяСлужба)
	|				ТОГДА РаспоряженияНаДоставку.СпособДоставки В (&СпособыДоставкиНашимиСиламиКПолучателю)
	...
	|			ПО (ТоварыКДоставке.Распоряжение = РаспоряженияНаДоставку.Распоряжение)
	|	ГДЕ
	|		РаспоряженияНаДоставку.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуСоСклада), ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуВПункте))
	|		И ТоварыКДоставке.Распоряжение ЕСТЬ NULL) КАК ВЗ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДопРеквизиты.Зона.Представление,
	|	ДопРеквизиты.Зона,
	|	ПолеУпрядочивания,
	|	ДопРеквизиты.ПолучательОтправитель,
	|	ДопРеквизиты.Распоряжение,
	|	ДопРеквизиты.Склад,
	|	ВЗ.Номенклатура,
	|	ВЗ.Характеристика,
	|	ВЗ.Назначение,
...
Функция ТекстЗапросаРаспоряженияНаДоставкуОтОтправителейПоОрдернойСхеме(ВыборочноеОбновление)
	...
	|	И (РаспоряженияНаДоставку.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуВПункте)
	|			ИЛИ РаспоряженияНаДоставку.Склад В ИЕРАРХИИ (&Склад))
	|	И НЕ РаспоряженияНаДоставку.Распоряжение ССЫЛКА Справочник.СоглашенияСПоставщиками
	...
	|					РаспоряженияНаДоставку.Распоряжение
	|				ИЗ
	|					РаспоряженияНаДоставку КАК РаспоряженияНаДоставку)) КАК ТоварыКПоступлениюОстатки
	|ГДЕ
	...
	|	СостоянияИРеквизитыДоставки.Номер КАК Номер,
	|	СостоянияИРеквизитыДоставки.Адрес КАК Адрес,
	|	ВЫРАЗИТЬ(СостоянияИРеквизитыДоставки.АдресЗначенияПолей КАК СТРОКА(200)) КАК АдресЗначенияПолей,
	...
	|	ВЫРАЗИТЬ(СостоянияИРеквизитыДоставки.АдресЗначенияПолей КАК СТРОКА(200)),
	...
	|			ТОГДА НАЧАЛОПЕРИОДА(ВЫРАЗИТЬ(РаспоряженияНаДоставку.Распоряжение КАК Документ.ПоручениеЭкспедитору).Дата, ДЕНЬ)
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ
	|ИЗ
	|	РаспоряженияНаДоставку КАК РаспоряженияНаДоставку
...
Функция ТекстЗапросаРаспоряженияНаДоставкуОтОтправителейБезОрдернойСхемы(ВыборочноеОбновление)
	...
	|			ПО (ТоварыКДоставке.Распоряжение = РаспоряженияНаДоставку.Распоряжение)
	|	ГДЕ
	|		РаспоряженияНаДоставку.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуНаСклад), ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуВПункте))
	|		И ТоварыКДоставке.Распоряжение ЕСТЬ NULL) КАК ДетальныеДанные
	|ГДЕ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДетальныеДанные.ЗонаПредставление,
	|	ДетальныеДанные.Зона,
	|	ДетальныеДанные.ПолеУпорядочения,
	|	ДетальныеДанные.ПолучательОтправитель,
	|	ДетальныеДанные.Распоряжение,
	|	ДетальныеДанные.Склад,
	|	ДетальныеДанные.Номенклатура,
	|	ДетальныеДанные.Характеристика,
	|	ДетальныеДанные.Назначение,
	|	ДетальныеДанные.Дата";
...
Функция ТекстЗапросаРаспоряженияНаДоставкуОтОтправителей(ВыборочноеОбновление)
	...
	|	ВЗ.Дата КАК Дата,
	|	ВЗ.Номенклатура КАК Номенклатура,
	|	ВЗ.МернаяЕдиницаИзмерения КАК МернаяЕдиницаИзмерения,
	|	ВЗ.Характеристика КАК Характеристика,
	|	ВЗ.Назначение КАК Назначение,
	|	ВЗ.Серия КАК Серия,
	|	ВЗ.НачальнаяПотребностьПоДатам - ВЗ.ВычетТоварыКДоставке КАК Количество,
	|	ВЗ.НачальнаяПотребностьПоДатамВес - ВЗ.ВычетТоварыКДоставкеВес КАК Вес,
	|	ВЗ.НачальнаяПотребностьПоДатамОбъем - ВЗ.ВычетТоварыКДоставкеОбъем КАК Объем,
	|	ВЗ.Склад КАК Склад,
	|	ВЗ.Распоряжение КАК Распоряжение,
	|	ВЗ.ПолучательОтправитель КАК ПолучательОтправитель,
	|	ВЗ.ВидРаспоряжения КАК ВидРаспоряжения,
	|	ВЗ.Номер КАК Номер,
	|	ВЗ.Адрес КАК Адрес,
	|	ВЗ.АдресЗначенияПолей КАК АдресЗначенияПолей,
	|	ВЗ.СпособДоставки КАК СпособДоставки,
	|	ВЗ.Картинка КАК Картинка,
	|	ВЗ.Зона КАК Зона,
	|	ВЗ.ЗонаПредставление КАК ЗонаПредставление,
	|	ВЗ.Перевозчик КАК Перевозчик,
	|	ВЗ.ВремяС КАК ВремяС,
	|	ВЗ.ВремяПо КАК ВремяПо,
	|	ВЗ.ДополнительнаяИнформация КАК ДополнительнаяИнформация,
	|	ВЗ.ОсобыеУсловияПеревозки КАК ОсобыеУсловияПеревозки,
	|	ВЗ.ОсобыеУсловияПеревозкиОписание КАК ОсобыеУсловияПеревозкиОписание,
	|	ВЗ.ПолеУпорядочения КАК ПолеУпорядочивания
	|ИЗ
	|	(ВЫБРАТЬ
	|		НАЧАЛОПЕРИОДА(ТоварыКДоставкеПоДням.Период, ДЕНЬ) КАК Дата,
	|		ТоварыКДоставкеПоДням.Номенклатура КАК Номенклатура,
	|		ТоварыКДоставкеПоДням.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины <> ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук) КАК МернаяЕдиницаИзмерения,
	...
	|		И ТоварыКДоставке.Распоряжение ЕСТЬ NULL) КАК ВЗ
	|ГДЕ
	|	(ВЗ.НачальнаяПотребностьПоДатам = 0
	|			ИЛИ ВЗ.НачальнаяПотребностьПоДатам - ВЗ.ВычетТоварыКДоставке > 0)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВЗ.ЗонаПредставление,
	|	ВЗ.Зона,
	|	ВЗ.ПолеУпорядочения,
	|	ВЗ.ПолучательОтправитель,
	|	ВЗ.Распоряжение,
	|	ВЗ.Склад,
	|	ВЗ.Номенклатура,
	|	ВЗ.Характеристика,
...
#Область ДоработкиДляВнешнейОбработкиДоставкаТерентьевЕМ

//ТерентьевЕМ
//
Процедура ОбновитьСписокРаспоряженийНаДоставку_АТ(Форма, ОбновляемыеРаспоряжения = Неопределено) Экспорт
	...
	ОтборПоТипуИсполнителей 	= Форма.ОтборПоТипуИсполнителей;
	Зона 						= Форма.Зона;
	Склады 						= Форма.Склады.ВыгрузитьЗначения();
	ДатаРаспоряжений 			= Форма.ДатаРаспоряжений;
	ЗонаГруппаИлиПустая 		= (НЕ ЗначениеЗаполнено(Зона) ИЛИ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Зона,"ЭтоГруппа"));
	ОтгрузкаПоОрдерам 			= Ложь;
	...
	ТекстЗапроса = ТекстЗапросаРаспоряженияНаДоставку_АТ(ВыборочноеОбновление);
	...
	Пока Выборка.СледующийПоЗначениюПоля("Зона") Цикл
		...
		ИначеЕсли ЗонаГруппаИлиПустая Тогда
			ТекущаяВетка = КоллекцияРаспоряжений.Добавить();
			ТекущаяВетка.АдресПогрузки = Строка(Выборка.Зона);
			ТекущаяВетка.Зона = Выборка.Зона;
			КоллекцияТекущаяВетка = ТекущаяВетка.ПолучитьЭлементы();
		Иначе
			КоллекцияТекущаяВетка = КоллекцияРаспоряжений;
...
//ТерентьевЕМ
//
Функция ТекстЗапросаРаспоряженияНаДоставку_АТ(ВыборочноеОбновление)
	...
	|	РаспоряженияНаДоставку.Дата КАК Дата,
	|	РаспоряженияНаДоставку.ОсобыеУсловияПеревозки КАК ОсобыеУсловияПеревозки
	|ПОМЕСТИТЬ РаспоряженияНаДоставкуПолучателям
	|ИЗ
	|	РегистрСведений.СостоянияИРеквизитыДоставки КАК РаспоряженияНаДоставку
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварыКДоставке КАК ПолностьюДоставленныеЗаказы
	|		ПО (ПолностьюДоставленныеЗаказы.Распоряжение = РаспоряженияНаДоставку.Распоряжение)
	...
	|			ИЛИ РаспоряженияНаДоставку.Зона В ИЕРАРХИИ (&Зона))
	|	И (РаспоряженияНаДоставку.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуВПункте)
	|			ИЛИ РаспоряженияНаДоставку.Склад В (&Склад))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЧастичноДоставленныеЗаказы.Назначение КАК Назначение,
	|	ЧастичноДоставленныеЗаказы.Серия КАК Серия,
	|	ЧастичноДоставленныеЗаказы.Количество КАК Количество,
	|	НАЧАЛОПЕРИОДА(РаспоряженияНаДоставкуПолучателям.Распоряжение.ДатаОтгрузки, ДЕНЬ) КАК Дата,
	|	РаспоряженияНаДоставкуПолучателям.СпособДоставки КАК СпособДоставки,
	|	РаспоряженияНаДоставкуПолучателям.ОсобыеУсловияПеревозки КАК ОсобыеУсловияПеревозки
	|ПОМЕСТИТЬ ДоставленныеЗаказы
	|ИЗ
	|	РаспоряженияНаДоставкуПолучателям КАК РаспоряженияНаДоставкуПолучателям
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТоварыКДоставке КАК ЧастичноДоставленныеЗаказы
	|		ПО РаспоряженияНаДоставкуПолучателям.Распоряжение = ЧастичноДоставленныеЗаказы.Распоряжение
	|			И РаспоряженияНаДоставкуПолучателям.Склад = ЧастичноДоставленныеЗаказы.Склад
	|			И (НЕ ЧастичноДоставленныеЗаказы.ВсеТовары)
	|ГДЕ
	|	(ЧастичноДоставленныеЗаказы.ЗаданиеНаПеревозку <> &ЗаданиеНаПеревозкуПодбор
	|				И ЧастичноДоставленныеЗаказы.ЗаданиеНаПеревозку.Проведен
	...
	|	ВР.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины <> ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук) КАК МернаяЕдиницаИзмерения,
	|	ВР.Характеристика КАК Характеристика,
	|	ВР.Назначение КАК Назначение,
	|	ВР.Серия КАК Серия,
	|	ВР.Количество КАК Количество,
	|	ВР.Количество * &ТекстЗапросаВесУпаковки КАК Вес,
	|	ВР.Количество * &ТекстЗапросаОбъемУпаковки КАК Объем,
	|	ВР.Склад КАК Склад,
	|	ВР.Распоряжение КАК Распоряжение,
	|	ВР.Распоряжение.Дата КАК ВремяДокумента,
	|	ДопРеквизиты.ПолучательОтправитель КАК ПолучательОтправитель,
	|	ТИПЗНАЧЕНИЯ(ВР.Распоряжение) КАК ВидРаспоряжения,
	|	ДопРеквизиты.Номер КАК Номер,
	|	ДопРеквизиты.Адрес КАК Адрес,
	|	ПРЕДСТАВЛЕНИЕ(ВР.Склад) КАК АдресПогрузки,
	|	ВЫРАЗИТЬ(ДопРеквизиты.АдресЗначенияПолей КАК СТРОКА(1000)) КАК АдресЗначенияПолей,
	|	ВР.СпособДоставки КАК СпособДоставки,
	|	ВР.Картинка КАК Картинка,
	|	ЕСТЬNULL(ДопРеквизиты.Зона, ЗНАЧЕНИЕ(Справочник.ЗоныДоставки.ПустаяСсылка)) КАК Зона,
	|	ДопРеквизиты.Зона.Представление КАК ЗонаПредставление,
	|	ДопРеквизиты.Перевозчик КАК Перевозчик,
	|	ДопРеквизиты.ВремяС КАК ВремяС,
	...
	|	ВЫРАЗИТЬ(ДопРеквизиты.ОсобыеУсловияПеревозкиОписание КАК СТРОКА(500)) КАК ОсобыеУсловияПеревозкиОписание,
	|	ВЫБОР
	|		КОГДА ВР.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика)
	|			ТОГДА ПРЕДСТАВЛЕНИЕ(ДопРеквизиты.Перевозчик)
	|		ИНАЧЕ ДопРеквизиты.Адрес
	|	КОНЕЦ КАК ПолеУпорядочения
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		МАКСИМУМ(ИтоговыеДанные.Дата) КАК Дата,
	|		ИтоговыеДанные.Распоряжение КАК Распоряжение,
	|		ИтоговыеДанные.Склад КАК Склад,
	|		ИтоговыеДанные.Номенклатура КАК Номенклатура,
	|		ИтоговыеДанные.Характеристика КАК Характеристика,
	|		ИтоговыеДанные.Назначение КАК Назначение,
	|		ИтоговыеДанные.Серия КАК Серия,
	|		ИтоговыеДанные.СпособДоставки КАК СпособДоставки,
	|		СУММА(ИтоговыеДанные.Количество) КАК Количество,
	|		ВЫБОР
	|			КОГДА ИтоговыеДанные.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика))
	|				ТОГДА 0
	|			КОГДА ИтоговыеДанные.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента), ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу))
	|				ТОГДА 1
	|			ИНАЧЕ 2
	|		КОНЕЦ + ВЫБОР
	|			КОГДА ИтоговыеДанные.ОсобыеУсловияПеревозки
	|				ТОГДА 3
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК Картинка
	|	ИЗ
	...
	|			РаспоряженияНаДоставкуПолучателям.Распоряжение КАК Распоряжение,
	|			РаспоряженияНаДоставкуПолучателям.Склад КАК Склад,
	|			ТоварыКОтгрузке.Номенклатура КАК Номенклатура,
	|			ТоварыКОтгрузке.Характеристика КАК Характеристика,
	|			ТоварыКОтгрузке.Назначение КАК Назначение,
	|			ТоварыКОтгрузке.Серия КАК Серия,
	|			РаспоряженияНаДоставкуПолучателям.СпособДоставки КАК СпособДоставки,
	|			ЕСТЬNULL(ТоварыКОтгрузке.КОтгрузке, 0) КАК Количество,
	|			РаспоряженияНаДоставкуПолучателям.ОсобыеУсловияПеревозки КАК ОсобыеУсловияПеревозки
	|		ИЗ
	|			РаспоряженияНаДоставкуПолучателям КАК РаспоряженияНаДоставкуПолучателям
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОтгрузке КАК ТоварыКОтгрузке
	|				ПО РаспоряженияНаДоставкуПолучателям.Распоряжение = ТоварыКОтгрузке.ДокументОтгрузки
	|					И РаспоряженияНаДоставкуПолучателям.Склад = ТоварыКОтгрузке.Склад
	|		ГДЕ
	|			ТоварыКОтгрузке.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	|			ДоставленныеЗаказы.Дата,
	|			ДоставленныеЗаказы.Распоряжение,
	|			ДоставленныеЗаказы.Склад,
	|			ДоставленныеЗаказы.Номенклатура,
	|			ДоставленныеЗаказы.Характеристика,
	|			ДоставленныеЗаказы.Назначение,
	|			ДоставленныеЗаказы.Серия,
	|			ДоставленныеЗаказы.СпособДоставки,
	|			-ДоставленныеЗаказы.Количество,
	|			ДоставленныеЗаказы.ОсобыеУсловияПеревозки
	|		ИЗ
	|			ДоставленныеЗаказы КАК ДоставленныеЗаказы) КАК ИтоговыеДанные
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ИтоговыеДанные.Распоряжение,
	|		ИтоговыеДанные.Склад,
	|		ИтоговыеДанные.Номенклатура,
	|		ИтоговыеДанные.Характеристика,
	|		ИтоговыеДанные.Назначение,
	|		ИтоговыеДанные.Серия,
	|		ИтоговыеДанные.СпособДоставки,
	|		ВЫБОР
	|			КОГДА ИтоговыеДанные.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика))
	|				ТОГДА 0
	|			КОГДА ИтоговыеДанные.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента), ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу))
	...
	|	
	|	ИМЕЮЩИЕ
	|		СУММА(ИтоговыеДанные.Количество) > 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		НАЧАЛОПЕРИОДА(РаспоряженияНаДоставкуПолучателям.Дата, ДЕНЬ),
	|		РаспоряженияНаДоставкуПолучателям.Распоряжение,
	|		РаспоряженияНаДоставкуПолучателям.Склад,
	|		NULL,
	|		NULL,
	|		NULL,
	|		NULL,
	|		РаспоряженияНаДоставкуПолучателям.СпособДоставки,
	|		NULL,
	|		4
	|	ИЗ
	|		РаспоряженияНаДоставкуПолучателям КАК РаспоряженияНаДоставкуПолучателям
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварыКДоставке КАК ТоварыКДоставке
	|			ПО (ТоварыКДоставке.Распоряжение = РаспоряженияНаДоставкуПолучателям.Распоряжение)
	|	ГДЕ
	|		РаспоряженияНаДоставкуПолучателям.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуСоСклада), ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуВПункте))
	|		И ТоварыКДоставке.Распоряжение ЕСТЬ NULL) КАК ВР
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияИРеквизитыДоставки КАК ДопРеквизиты
	|		ПО ВР.Распоряжение = ДопРеквизиты.Распоряжение
	|			И ВР.Склад = ДопРеквизиты.Склад
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ВР.Номенклатура = СправочникНоменклатура.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВЗ.Дата,
	|	ВЗ.Номенклатура,
	|	ВЗ.МернаяЕдиницаИзмерения,
	|	ВЗ.Характеристика,
	|	ВЗ.Назначение,
	|	ВЗ.Серия,
	|	ВЗ.НачальнаяПотребностьПоДатам - ВЗ.ВычетТоварыКДоставке,
	|	ВЗ.НачальнаяПотребностьПоДатамВес - ВЗ.ВычетТоварыКДоставкеВес,
	|	ВЗ.НачальнаяПотребностьПоДатамОбъем - ВЗ.ВычетТоварыКДоставкеОбъем,
	|	ВЗ.ПолучательОтправитель,                                                                 //
	|	ВЗ.Распоряжение,
	|	ВЗ.Распоряжение.Дата,
	|	ВЗ.Склад,                                                                                 //
	|	ВЗ.ВидРаспоряжения,
	|	ВЗ.Номер,
	|	ПРЕДСТАВЛЕНИЕ(ВЗ.Склад),                                        //адрес
	|	ВЗ.Адрес,
	|	ВЗ.АдресЗначенияПолей,
	|	ВЗ.СпособДоставки,
	|	ВЗ.Картинка,
	|	ВЗ.Зона,
	|	ВЗ.ЗонаПредставление,
	...
	|	ВЗ.ВремяС,
	|	ВЗ.ВремяПо,
	|	ВЗ.ДополнительнаяИнформация,
	|	ВЗ.ОсобыеУсловияПеревозки,
	|	ВЗ.ОсобыеУсловияПеревозкиОписание,
	|	ВЗ.ПолеУпорядочения
	|ИЗ
	|	(ВЫБРАТЬ
	|		НАЧАЛОПЕРИОДА(ТоварыКДоставкеПоДням.Период, ДЕНЬ) КАК Дата,
	...
	|ВЫБРАТЬ
	|	ТР.Дата КАК Дата,
	|	ТР.Номенклатура КАК Номенклатура,
	|	ТР.МернаяЕдиницаИзмерения КАК МернаяЕдиницаИзмерения,
	|	ТР.Характеристика КАК Характеристика,
	|	ТР.Назначение КАК Назначение,
	|	ТР.Серия КАК Серия,
	|	ТР.Количество Количество,
	|	ТР.Вес КАК Вес,
	|	ТР.Объем КАК Объем,
	|	ТР.Склад КАК Склад,
	|	ТР.Распоряжение КАК Распоряжение,
	|	ТР.ВремяДокумента КАК ВремяДокумента,
	|	ТР.ПолучательОтправитель КАК ПолучательОтправитель,
	|	ТР.ВидРаспоряжения КАК ВидРаспоряжения,
	|	ТР.Номер КАК Номер,
	|	ТР.Адрес КАК Адрес,
	|	ТР.АдресПогрузки КАК АдресПогрузки,
	|	ТР.АдресЗначенияПолей КАК АдресЗначенияПолей,
	|	ТР.СпособДоставки КАК СпособДоставки,
	|	ТР.Картинка КАК Картинка,
	|	ТР.Зона КАК Зона,
	|	ТР.ЗонаПредставление КАК ЗонаПредставление,
	|	ТР.Перевозчик КАК Перевозчик,
	|	ТР.ВремяС КАК ВремяС,
	|	ТР.ВремяПо КАК ВремяПо,
	|	ТР.ДополнительнаяИнформация КАК ДополнительнаяИнформация,
	|	ТР.ОсобыеУсловияПеревозки КАК ОсобыеУсловияПеревозки,
	|	ТР.ОсобыеУсловияПеревозкиОписание КАК ОсобыеУсловияПеревозкиОписание,
	|	ТР.ПолеУпорядочения КАК ПолеУпорядочения,
	|	ЕСТЬNULL(Реализация.Ссылка, Значение(Документ.РеализацияТоваровУслуг.ПустаяСсылка)) КАК ДокументРеализация,
	|	ЕСТЬNULL(РеализацияНомер.Номер, """") КАК ДокументРеализацияНомер,
	|	ЕСТЬNULL(СчетФактураВыданный.Ссылка, Значение(Документ.СчетФактураВыданный.ПустаяСсылка)) КАК УПД,
	|	ЕСТЬNULL(СчетФактураВыданный.Номер, """") КАК УПДНомер
	|ИЗ ТаблицаРезультат КАК ТР
	...
	|		И НЕ РеализацияТоваровУслуг.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|
	|	СГРУППИРОВАТЬ ПО
	|		РеализацияТоваровУслуг.ЗаказКлиента) КАК Реализация
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ПО Реализация.Ссылка = СчетФактураВыданный.ДокументОснование И СчетФактураВыданный.Проведен
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияНомер
	|		ПО Реализация.Ссылка = РеализацияНомер.Ссылка
	|	ПО ТР.Распоряжение = Реализация.ЗаказКлиента
	|	
	|ГДЕ
	|	ВЫБОР КОГДА ТР.Распоряжение ССЫЛКА Документ.ЗаказКлиента ТОГДА НЕ Реализация.Ссылка ЕСТЬ NULL
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|УПОРЯДОЧИТЬ ПО
	|	ЗонаПредставление,
