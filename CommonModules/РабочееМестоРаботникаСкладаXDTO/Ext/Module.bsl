...
// 
// Возвращаемое значение:
//  Структура - параметры складского задания задания
...
Функция ПроксиВебСервиса(ИмяПользователя, Пароль, Знач МестоположениеWSDL) Экспорт
	...
	Попытка
		URIИмен     = URIПространстваИмен();
		Определение = Новый WSОпределения(МестоположениеWSDL, ИмяПользователя, Пароль, , 60);
		
		Прокси = Новый WSПрокси(Определение, URIИмен, "WMSMobileClientExchange", "WMSMobileClientExchangeSoap", , 60);
	Исключение
