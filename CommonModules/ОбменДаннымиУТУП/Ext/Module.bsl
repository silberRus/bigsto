...
Функция ОпределитьПараметрыЗапросаОбъекта(ЭлементДанных, ТипЗначенияОбъекта = Неопределено, ФормироватьИерархию = Истина) Экспорт
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("ДокументОбъект.ПоступлениеТоваров") Тогда
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("ДокументОбъект.РегистраторГрафикаДвиженияТоваров") Тогда
		
		ЗначенияФильтра_РегистраторГрафикаДвиженияТоваров(ЭлементДанных, ПараметрыЗапроса, ФормироватьИерархию);
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("ДокументОбъект.КассовыйЧекКоррекции") Тогда
		
		ЗначенияФильтра_КассовыйЧекКоррекции(ЭлементДанных, ПараметрыЗапроса, ФормироватьИерархию);
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("РегистрНакопленияЗапись.ГрафикПоступленияТоваров") Тогда
		
		ЗначенияФильтра_РегистрыНакопления_ГрафикПоступленияТоваров(ЭлементДанных, ПараметрыЗапроса, ФормироватьИерархию);
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("РегистрНакопленияЗапись.РезервыТоваровОрганизаций") Тогда
		
		ЗначенияФильтра_РегистрыНакопления_РезервыТоваровОрганизаций(ЭлементДанных, ПараметрыЗапроса, ФормироватьИерархию);
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("РегистрНакопленияЗапись.ТоварыКОформлениюТаможенныхДеклараций") Тогда
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("РегистрНакопленияЗапись.ТоварыПринятые") Тогда
		
		ЗначенияФильтра_РегистрыНакопления_ТоварыПринятые(ЭлементДанных, ПараметрыЗапроса, ФормироватьИерархию);
		
	ИначеЕсли ТипЗначенияОбъекта = Тип("РегистрНакопленияЗапись.ТоварыУПартнеров") Тогда
		
		ЗначенияФильтра_РегистрыНакопления_ТоварыУПартнеров(ЭлементДанных, ПараметрыЗапроса, ФормироватьИерархию);
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("РегистрСведенийЗапись.УдалитьНДСДокументыДляРаспределения") Тогда
		
		ЗначенияФильтра_РегистрыСведений_НДСДокументыДляРаспределения(ЭлементДанных, ПараметрыЗапроса, ФормироватьИерархию);
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("РегистрСведенийЗапись.СостоянияБлокировкиВычетаНДСПоСчетамФактурам") Тогда
		
		ЗначенияФильтра_РегистрыСведений_СостоянияБлокировкиВычетаНДСПоСчетамФактурам(ЭлементДанных, ПараметрыЗапроса, ФормироватьИерархию);
...
Процедура ЗначенияФильтра_РегистраторГрафикаДвиженияТоваров(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	
	Если ПроверитьНаличиеСвязаннойСсылки(Объект.Распоряжение, ПараметрыЗапроса) Тогда
		
		СоглашениеСПоставщиком = Объект.Распоряжение.ПолучитьОбъект();
		ПараметрыЗапроса       = ОпределитьПараметрыЗапросаОбъекта(СоглашениеСПоставщиком, ТипЗнч(СоглашениеСПоставщиком), ФормироватьИерархию);
		
		ПараметрыЗапроса.РеквизитПроверкиПодразделения = НСтр("ru = 'Соглашение документа'");
...
Процедура ЗначенияФильтра_ТТНВходящаяЕГАИС(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	...
	Если ЗначениеЗаполнено(Объект.Склад) И ТипЗнч(Объект.Склад) = Тип("СправочникСсылка.Склады") Тогда
		ОтборПоПодразделениюИзРеквизитаИОрганизации(Объект, ПараметрыЗапроса, ФормироватьИерархию, "Склад");
		ПараметрыЗапроса.РеквизитПроверкиПодразделения = НСтр("ru = 'Подразделение указанное в складе'");
	КонецЕсли;
...
Процедура ЗначенияФильтра_ВводОстатков(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	...
	ИначеЕсли Объект.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ДенежныеСредстваКПоступлениюОтЭквайера Тогда
...
Процедура ЗначенияФильтра_КассовыйЧекКоррекции(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	ОтборПоПодразделениюИзРеквизитаИОрганизации(Объект, ПараметрыЗапроса, ФормироватьИерархию, "КассаККМ");
	ПараметрыЗапроса.РеквизитПроверкиПодразделения = НСтр("ru = 'Подразделение указанное в кассе ККМ'");
КонецПроцедуры
...
Процедура ЗначенияФильтра_РегистрыНакопления_СебестоимостьТоваров(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	...
	ТипСкладаАналитикиУчетаНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.АналитикаУчетаНоменклатуры, "Склад");
...
Процедура ЗначенияФильтра_РегистрыНакопления_РезервыТоваровОрганизаций(Объект, ПараметрыЗапроса, ФормироватьИерархию)
...
Процедура ЗначенияФильтра_РегистрыНакопления_ТоварыПринятые(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	
	СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Договор, "Организация, Подразделение");
	
	МассивПодразделений = СформироватьИерархиюЭлементов(СтруктураРеквизитов.Подразделение, ФормироватьИерархию);
	...
	Если ЗначениеЗаполнено(СтруктураРеквизитов.Организация) Тогда
		ПараметрыЗапроса.Организации         = СформироватьМассивОрганизаций(СтруктураРеквизитов.Организация); 	
		ПараметрыЗапроса.ФильтрПоОрганизации = Истина;
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗначенияФильтра_РегистрыНакопления_ТоварыУПартнеров(Объект, ПараметрыЗапроса, ФормироватьИерархию)
...
Процедура ЗначенияФильтра_РегистрыНакопления_ГрафикПоступленияТоваров(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	ОтборПоПодразделениюИзРеквизита(Объект, ПараметрыЗапроса, ФормироватьИерархию, "Склад");
КонецПроцедуры
...
Процедура ЗначенияФильтра_РегистрыНакопления_ДвиженияДенежныхСредств(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|				ПО ЭквайринговыеТерминалы.БанковскийСчет = БанковскиеСчетаОрганизаций.Ссылка) КАК ЭквайринговыеТерминалы
...
Процедура ПараметрыФильтровДвиженияДенежныхСредств(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	...
	Если ПараметрыЗапросаИзРегистратора Тогда
		...
		Если ОбщегоНазначения.СсылкаСуществует(Объект.Регистратор) Тогда
...
Процедура ЗначенияФильтра_РегистрыСведений_ЦеныНоменклатурыПоставщиков(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	...
	Запрос.Текст = ДобавитьИтогиКЗапросу(Запрос.Текст, ФормироватьИерархию);
	Запрос.УстановитьПараметр("Ссылка", Объект.УдалитьСоглашение);
...
Процедура ЗначенияФильтра_РегистрыСведений_НДСДокументыДляРаспределения(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	
	Если ПроверитьНаличиеСвязаннойСсылки(Объект.ДокументПартии, ПараметрыЗапроса) Тогда
		ПараметрыЗапроса = ОпределитьПараметрыЗапросаОбъекта(Объект.ДокументПартии.ПолучитьОбъект(),,ФормироватьИерархию);
	КонецЕсли;
...
Процедура ЗначенияФильтра_РегистрыСведений_СостоянияБлокировкиВычетаНДСПоСчетамФактурам(Объект, ПараметрыЗапроса, ФормироватьИерархию)
...
Процедура ЗначенияФильтра_Справочники_ЭквайринговыеТерминалы(Объект, ПараметрыЗапроса, ФормироватьИерархию)
	...
	Если Не Объект.ЭтоГруппа Тогда
		Подразделение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчет, "Подразделение");
