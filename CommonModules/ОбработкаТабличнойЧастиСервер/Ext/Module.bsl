...
Процедура ПересчитатьЦенуСНДС(ТекущаяСтрока, СтруктураДействий)
	...
	Если СтруктураДействий.Свойство("ПересчитатьЦенуСНДС", СтруктураПараметровДействия) Тогда
		...
		ПроцентНДС = ЦенообразованиеКлиентСервер.ПолучитьСтавкуНДСЧислом(СтавкаНДС, НДСпоСтавкам4и2);
...
Процедура ПересчитатьСуммуНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) 
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДС", СтруктураПараметровДействия) Тогда
		...
		Если НДСпоСтавкам4и2 Или ТекПроцентНДС = Неопределено Тогда
...
Процедура ПересчитатьСуммуНДССуффиксВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДССуффикс", СтруктураПараметровДействия) Тогда
		...
		Если НДСпоСтавкам4и2
			Или ТекПроцентНДС = Неопределено Тогда
		...
		Если СтруктураПараметровДействия <> Неопределено
			И СтруктураПараметровДействия.Свойство("Суффикс", Суффикс)
		...
		ТекущаяСтрока["СуммаНДС" + Суффикс] = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(
			ТекущаяСтрока["Сумма" + Суффикс], ТекПроцентНДС, СтруктураПараметровДействия.ЦенаВключаетНДС);
...
Процедура ПроверитьЗаполнитьОбеспечениеВСтрокеТЧДокументаПродажи(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВДокументеПродажи", ПараметрыДействия) Тогда
		...
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Номенклатура,
			"ТипНоменклатуры,ОбособленнаяЗакупкаПродажа");
		
		ВариантыОбеспечения = ПараметрыДействия.ВариантыОбеспечения;
		
		СохранитьВариантНеОбеспечивать = Ложь;
		Если ВариантыОбеспечения = Неопределено Тогда
			
			ВариантыОбеспечения = ОбеспечениеКлиентСервер.ВариантыОбеспеченияПоУмолчанию(Реквизиты.ОбособленнаяЗакупкаПродажа);
			СохранитьВариантНеОбеспечивать = ТекущаяСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.НеТребуется;
			
		КонецЕсли;
		
		ВариантОбеспечения = ВариантыОбеспечения[Строка(Реквизиты.ТипНоменклатуры)];
		
		Если СохранитьВариантНеОбеспечивать И ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется Тогда
		...
		Если ПараметрыДействия.ЖелаемаяДатаОтгрузки <> Неопределено Тогда
			
			Если ЗначениеЗаполнено(ТекущаяСтрока.ДатаОтгрузки)
					Или ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.Требуется
							И ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.Обособленно Тогда
				Возврат; // в этом случае дату отгрузки заполнять не следует.
			КонецЕсли;
			
			ДанныеСтроки = Новый Структура();
			ДанныеСтроки.Вставить("Номенклатура",         ТекущаяСтрока.Номенклатура);
			ДанныеСтроки.Вставить("Характеристика",       ТекущаяСтрока.Характеристика);
			ДанныеСтроки.Вставить("Склад",                ?(ПараметрыДействия.Склад <> Неопределено, ПараметрыДействия.Склад, ТекущаяСтрока.Склад));
			ДанныеСтроки.Вставить("ЖелаемаяДатаОтгрузки", ПараметрыДействия.ЖелаемаяДатаОтгрузки);
			
			ТаблицаЗначений = ОбеспечениеСервер.ТаблицаЗначенийИзСтруктуры(ДанныеСтроки);
			ТаблицаЗначений = ОбеспечениеСервер.РассчитатьДатуОтгрузкиВТаблице(ТаблицаЗначений);
			
			ТекущаяСтрока.ДатаОтгрузки = ТаблицаЗначений[0].ДатаОтгрузки;
...
Процедура ЗаполнитьПризнакАналитикаАктивовПассивовНеИспользуется(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	Перем Параметры;
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакАналитикаАктивовПассивовНеИспользуется", Параметры) Тогда
		
		Если ПустаяСтрока(Параметры) Тогда
			Параметры = "СтатьяАктивовПассивов, АналитикаАктивовПассивов";
		КонецЕсли;
		...
		Для Каждого КлючИЗначение Из СтруктураРеквизитов Цикл
			...
			БезАналитики = Ложь;
			Если ЗначениеЗаполнено(Статья) И ТипЗнч(Статья) = Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
				БезАналитики = КэшированныеЗначения.ИспользоватьАналитикуАктивовПассивов[Статья];
				Если БезАналитики = Неопределено Тогда
					БезАналитики = (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Статья, "БезАналитики") = Истина);
					КэшированныеЗначения.ИспользоватьАналитикуАктивовПассивов.Вставить(Статья, БезАналитики);
				КонецЕсли;
			КонецЕсли;
			ТекущаяСтрока[КлючИЗначение.Ключ + "НеИспользуется"] = БезАналитики;
			Статья = Неопределено;
...
Процедура ЗаполнитьПризнакРаспределениеНДС(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	
	Перем Параметры;
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакРаспределениеНДС", Параметры) Тогда
		
		Статья = ТекущаяСтрока["СтатьяРасходов"];
		
		РаспределениеНДС = Ложь;
		Если ЗначениеЗаполнено(Статья) И ТипЗнч(Статья) = Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
			
			РаспределениеНДС = КэшированныеЗначения.РаспределятьНДСпоСтатьеРасходов[Статья];
			Если РаспределениеНДС = Неопределено Тогда
				
				Вариант = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Статья, "ВариантРаздельногоУчетаНДС");
				РаспределениеНДС = (Вариант = Перечисления.ВариантыРаздельногоУчетаНДС.Распределение);
				КэшированныеЗначения.РаспределятьНДСпоСтатьеРасходов.Вставить(Статья, РаспределениеНДС);
				
			КонецЕсли;
			
		КонецЕсли;
		ТекущаяСтрока["РаспределениеНДС"] = РаспределениеНДС;
...
Функция ПоддерживаемыеДействияЗаполненияТЧ() Экспорт
	...
	Действия.Добавить("ЗаполнитьПризнакМаркируемаяАлкогольнаяПродукция");
	Действия.Добавить("ЗаполнитьПризнакАлкогольнаяПродукция");
...
Функция ШаблонПоляВыборкиПоКлючуДействия(КлючДействия, ТолькоПроверитьПоддерживаемыеКлючи = Ложь)
	...
	Если КлючДействия = "ЗаполнитьПризнакМаркируемаяАлкогольнаяПродукция" Тогда
		...
		|		ЛОЖЬ
		|	ИНАЧЕ
		|		ЕСТЬNULL(втТаблицаНоменклатуры.%Ключ%.ВидАлкогольнойПродукции.Маркируемый, ЛОЖЬ)
		|	КОНЕЦ КАК %Значение%";
	КонецЕсли;
