	...
	Если ЭтоПоставляемаяОбработка(Источник.Ссылка) Тогда
		...
		Для Каждого КонтролируемыйРеквизит Из КонтролируемыеРеквизиты Цикл
			...
			Если ИсходныйРеквизит <> РезультирующийРеквизит Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Недопустимая попытка изменения значения реквизита %1 для дополнительной обработки %2, полученной из каталога дополнительных обработок менеджера сервиса!'"), 
...
Процедура ПриПодключенииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ЕстьЗащитаОтОпасныхДействий() Тогда
			...
			Результат = Менеджер.Подключить(АдресВоВременномХранилище, ПараметрыПодключения.ИмяОбъекта, БезопасныйРежим, ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений());
...
Процедура ПриПолученииРазрешенийСессииБезопасногоРежима(Знач КлючСессии, ОписанияРазрешений, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			...
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Не обнаружена поставляемая обработка для ключа запуска %1!'"),
				Строка(КлючСессии));
...
Процедура ПередОбновлениемЗадания(Объект, Команда, Задание, Изменения) Экспорт
	...
	Если Задание.Расписание.ТребуетсяВыполнение(ПроверяемаяДата, ИсходнаяДата) Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Расписание, задаваемое для выполнения команд дополнительного отчета или обработки в качестве заданий, должно быть не чаще, чем 1 раз в %1 секунд!'"), МинимальныйИнтервал);
	КонецЕсли;
КонецПроцедуры
...
Процедура АктуализацияНастроекИспользуемойОбработки(Знач Ссылка) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Дополнительная обработка с идентификатором %1 не является поставляемой!'"),
...
Процедура ЗапроситьОбновлениеДополнительныхОтчетовИОбработок() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Запрос обновлений дополнительных отчетов и обработок'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Процедура ПроверкаВозможностиВыполненияПоставляемойОбработки(Знач ИспользуемаяОбработка)
	...
	Если ЗначениеЗаполнено(ПоставляемаяОбработка) Тогда
		...
		Если ПараметрыПубликацииИспользуемойОбработки.Версия <> ПараметрыПубликацииПоставляемойОбработки.Версия Тогда
			ВызватьИсключение НСтр(
				"ru = 'Использование дополнительной обработки временно недоступно по причине выполнения ее обновления. Данный процесс может занять несколько минут. Приносим извинения на доставленные неудобства.'");
		КонецЕсли;
