	 ...
	 И Источник.ТипЭлементаВерсииЭД <> Перечисления.ТипыЭлементовВерсииЭД.ПДП Тогда
		...
		Если Источник.ДополнительныеСвойства.Свойство("СостояниеВерсииЭД") Тогда
			...
			Если ОбновитьВерсию Тогда
				...
				ОбменСКонтрагентамиСлужебныйВызовСервера.ОбновитьВерсиюЭД(Источник, ПринудительнаяСменаСостоянияВерсии, ВерсияФорматаПакета, ОтклонениеАннулирования);
			КонецЕсли;
...
Процедура ПолучитьНовыеЭлектронныеДокументы() Экспорт
	...
	Попытка
		...
		Если ОбменСКонтрагентамиСлужебныйВызовСервера.ДоступныПараметрыДляАвторизацииНаСервереОператора( , СоотвСоглашений) Тогда
			...
			ОбменСКонтрагентамиСлужебныйВызовСервера.ДополнитьПакетамиКРаспаковке(НовыеДокументы.МассивПакетовЭД, НовыеДокументы.Профили);
			...
			ИначеЕсли НастройкиОбмена <> Неопределено Тогда
				...
				Если ВерсияФорматаПакетаЭД = Перечисления.ВерсииФорматаПакетаЭД.Версия30
					...
					Если (ТекущийСтатус = Перечисления.СтатусыЭД.Получен 
							И НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоСлужебныйДокумент(РеквизитыФайлаЭД.ТипЭлементаВерсииЭД)
...
Функция ЗаполнитьПараметрыЭДПоИсточнику(Источник, ФорматCML = Ложь, ВидЭД = Неопределено, БыстрыйОбмен = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыЭД.ВидЭД)
		И ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЕстьПравоЧтенияЭД(Ложь) Тогда
	...
	Если ЗначениеЗаполнено(ПрисоединенныйФайлСсылка) Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыФайла.ИмяФайла) Тогда
			...
			Если ДанныеФайла <> Неопределено Тогда
				...
				ФорматЭлектронногоДокумента = ОбменСКонтрагентамиСлужебный.ФорматПоПространствуИмен(ВидЭД, ПространствоИмен);
...
Функция НастройкиОбменаЭД(ФайлЭД) Экспорт
	...
	Если ФайлЭД <> Справочники.ЭДПрисоединенныеФайлы.ПустаяСсылка() Тогда
		...
		Если ЗначениеЗаполнено(РеквизитыЭД) Тогда
			...
			ЭтоСлужебныйЭД = Ложь;
			Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоСлужебныйДокумент(РеквизитыФайла.ТипЭлементаВерсииЭД) Тогда
	...
	Исключение
		...
		Если ЗначениеЗаполнено(Результат) Тогда
			...
			Если МассивПараметровОшибки.Количество() >= 7 Тогда
				...
				ОписаниеОшибки = ТекстЗаголовкаСообщения + Символы.ПС + ТекстОшибки;
			КонецЕсли;
		КонецЕсли;
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ТекстЗаголовкаСообщения, Результат);
	...
	Если Не Отказ Тогда
		Попытка
			...
			ИначеЕсли УПДДокументОПередаче Тогда
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУПДДокументОПередаче(СтрокаОбъекта.ЗначениеРеквизита, ДокументСсылка, Записывать);
			ИначеЕсли УКДДокументОбИзмененииСтоимости Тогда
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУКДДокументОбИзмененииСтоимости(СтрокаОбъекта.ЗначениеРеквизита, ДокументСсылка, Записывать);
			Иначе
...
//
Функция ПредставлениеПрогрессаПодписания(ЭД, ВесМаршрута) Экспорт
...
Функция ДокументОтправленНаПодпись(ЭД)
	...
	ЭтоСлужебныйДокумент 			= ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоСлужебныйДокумент(РеквизитыЭД.ТипЭлементаВерсииЭД);
