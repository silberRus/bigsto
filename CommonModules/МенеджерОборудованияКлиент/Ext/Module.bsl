...
Процедура НачатьВыполнениеКоманды_ПослеПодтвержденияЗакрытияСмены(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяПодсистемыФискальныхУстройств() Тогда
				МодульКассовыеСменыВызовСервера = МенеджерОборудованияКлиентПовтИсп.ОбщийМодуль("КассовыеСменыВызовСервера");
				НоваяКассоваяСмена = МодульКассовыеСменыВызовСервера.НоваяКассоваяСмена(Параметры.ИдентификаторУстройства);
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'Модуль КассовыеСменыВызовСервера не найден'");
...
Процедура НачатьВыполнениеКоманды_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если МенеджерОборудованияКлиентСервер.ТребуетсяВызовСобытияПослеВыполненияКомандыФискальнымУстройством(Параметры) Тогда
			...
			Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяПодсистемыФискальныхУстройств() Тогда
				МодульКассовыеСменыВызовСервера = МенеджерОборудованияКлиентПовтИсп.ОбщийМодуль("КассовыеСменыВызовСервера");
				МодульКассовыеСменыВызовСервера.ПослеВыполненияКомандыФискальнымУстройством(СтруктураПараметров);
			КонецЕсли;
		КонецЕсли;
		...
		Если Параметры.ВыполняемаяКоманда = "CheckFiscalization" И МенеджерОборудованияКлиентПовтИсп.ИспользуетсяПодсистемыФискальныхУстройств() Тогда
			...
			МодульКассовыеСменыВызовСервера = МенеджерОборудованияКлиентПовтИсп.ОбщийМодуль("КассовыеСменыВызовСервера");
			МодульКассовыеСменыВызовСервера.ИнкрементироватьТекущийНомерЧекаККТ(Параметры.ИдентификаторУстройства, Параметры.КассоваяСмена);
...
Процедура НачатьЗакрытиеСменыНаФискальномУстройстве(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено, КассоваяСмена = Неопределено) Экспорт
	...
	Если КассоваяСмена = Неопределено Тогда
		...
		Иначе
			...
			Если МенеджерОборудованияКлиентПовтИсп.ИспользуетсяПодсистемыФискальныхУстройств() Тогда
				МодульКассовыеСменыВызовСервера = МенеджерОборудованияКлиентПовтИсп.ОбщийМодуль("КассовыеСменыВызовСервера");
				ОписаниеПоследнейСмены = МодульКассовыеСменыВызовСервера.ОписаниеПоследнейКассовойСмены(ИдентификаторУстройства);
			Иначе
				ОписаниеПоследнейСмены = Неопределено;
			КонецЕсли;
...
// 
Функция ПолучитьШаблонАтрибутовЧекаДляОтсылки(РегистрационныйНомерККТ, ЗаводскойНомерФН, ФискальныйПризнак, СуммаЧека, АдресСайтаПроверки, ДатаСеанса) Экспорт 
	
	ТекстСообщения  = НСтр("ru='ККТ№:'") + РегистрационныйНомерККТ + ";" + Символы.НПП 
		+ НСтр("ru='СУММА:'") + Формат(СуммаЧека, "ЧРД=.;ЧЦ=12;ЧДЦ=2;ЧН=0.00;ЧГ=0") + ";" + Символы.НПП 
		+ Формат(ДатаСеанса, "ДФ:""дд.ММ.гггг ЧЧ:мм""") + ";" + Символы.НПП 
		+ НСтр("ru='ФН№:'") + ЗаводскойНомерФН + ";" + Символы.НПП 
