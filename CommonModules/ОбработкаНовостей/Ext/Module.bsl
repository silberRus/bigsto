...
Процедура УстановитьСостояниеНовостейДляПользователей(
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(МассивПользователей, ", "));
	КонецЕсли;
	Если (ТипЗнч(СтруктураОтборов) = ТипСтруктура) Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2", ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(СтруктураОтборов, " = ", Символы.ПС));
	Иначе
	...
	Если (ТипЗнч(ЗначенияСостояний) = ТипСтруктура) Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%3", ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(ЗначенияСостояний, " = ", Символы.ПС));
	Иначе
...
//
// Параметры:
//  МассивНовостей - Массив - массив ссылок на новости;
...
//  ДатаНачалаОповещения - Дата - дата, начиная с которой необходимо включить всплывание.
//
...
//
// Параметры:
//  ЛентыНовостей - СправочникСсылка.ЛентаНовостей, Массив - Ленты новостей,
...
//  МассивОбластейДанных - Массив, Неопределено - при запуске в неразделенном сеансе в модели сервиса можно передать
...
Процедура ПриИзмененииЗначенийОтборовОбщих() Экспорт
	...
	КонтекстВыполненияВложенный = ИнтернетПоддержкаПользователейКлиентСервер.НоваяЗаписьРезультатовВыполненияОпераций(); // Этот контекст.
	КодРезультата = 0;
	ОписаниеРезультата = "";
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Общие'"), // ИдентификаторШага
			?(КодРезультата > 0,
...
Процедура ПриИзмененииЗначенийОтборовДляОбластиДанных() Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. ДляОбластиДанных'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПриИзмененииЗначенийОтборовПользовательских(Пользователь = Неопределено) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Пользовательские'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "2.2.2.23";
	Обработчик.Процедура           = "ОбработкаНовостей.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию_2_2_2";
	Обработчик.ОбщиеДанные         = Истина;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
//  Булево.
//
Функция СоединитьсяССерверомНовостей(ИмяСервера, Протокол, Логин, Пароль, ХттпСоединение, КонтекстВыполнения = Неопределено) Экспорт
...
// Процедура возвращает список значений очень важных и просто важных новостей
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ДругиеПараметрыПрограммногоОкружения
	...
	ВременныйТекст = СтрЗаменить(ВременныйТекст, "%ТекущийПользователь_РазделениеДанных%", ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(ПользователиИнформационнойБазы.ТекущийПользователь().РазделениеДанных, "=", "; "));
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗадания
	...
	РезультатДанныеОПрограммномОкружении_РасписаниеРегламентногоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ТекущееРегламентноеЗадание.Использование,
...
Функция НайтиНовости(СтруктураПараметровПоиска) Экспорт
	...
	лкСтрокаПоиска = СтрЗаменить(лкСтрокаПоиска, Символ(34), Символ(34) + Символ(34)); // Заменить одну двойную кавычку на две двойные кавычки
...
Функция ПолучитьКонтекстныеНовости(
	...
	Если Вариант = "Для форм, все поля" Тогда
		...
		Запрос.Текст = "
			...
			|	И ЕСТЬNULL(РегРассчитанныеОтборы_РедкоМеняющиеся.РезультатОтбора, ИСТИНА) = ИСТИНА // Отбор по версии платформы, версии конфигурации
...
#Область ВсеОбновленияНовостей_ЗагрузкаНовостей
			...
			КонтекстВыполненияВложенныйВложенный = ИнтернетПоддержкаПользователейКлиентСервер.НоваяЗаписьРезультатовВыполненияОпераций(); // Этот контекст.
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				...
				Если МассивЛентНовостей.Количество() > 0 Тогда
					ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(МассивЛентНовостей, ";"));
					ПолучитьИОбработатьНовостиПоЛентамНовостей(МассивЛентНовостей, КонтекстВыполненияВложенныйВложенный);
				Иначе
					ОписаниеРезультатаВложенный = НСтр("ru='Нет лент новостей для регламентного обновления'");
				КонецЕсли;
...
#Область ВсеОбновленияНовостей_ОбновлениеКлассификаторов
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
					...
					ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Разрешена работа через интернет
					...
					Если ДатаСледующегоОбновления <= ТекущаяДатаДляРегистров Тогда
						ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
							+ Символы.ПС + НСтр("ru='Наступил срок выполнения.'");
						ОбновитьСтандартныеСпискиДляНовостей_Ежедневно(КонтекстВыполненияВложенныйВложенный);
					Иначе
						ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
							+ Символы.ПС + НСтр("ru='Срок не наступил.'");
					КонецЕсли;

				Иначе
					ОписаниеРезультатаВложенный = НСтр("ru='Обновление классификаторов через интернет отключено'");
				КонецЕсли;
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru='Новости. Все обновления новостей'"), // ИдентификаторШага
		?(КодРезультата > 0,
			УровеньЖурналаРегистрации.Ошибка,
			УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
...
Процедура УдалитьНовости_Ежедневно(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновлениеПериодическихСвойствНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновлениеПривязокКМетаданным(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПолучитьИОбработатьНовостиПоЛентамНовостей(МассивЛентНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
			НСтр("ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновитьСтандартныеСпискиДляНовостей_Ежедневно(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			// Зашли в конфигурацию под пользователем с разделением (с входом в область данных).
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("ru='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, при работе в модели сервиса запускать получение классификаторов вручную можно только в неразделенном сеансе.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЕстьОшибки = Ложь Тогда
			...
			Если РазрешенаРаботаСНовостями Тогда
				...
				Если РазрешенаРаботаСНовостямиЧерезИнтернет Тогда
					ОписаниеРезультата = ОписаниеРезультата
						+ Символы.ПС
						+ НСтр("ru='Обновление классификаторов запущено.'");
...
#Область ОбновлениеКатегорийИЗначений
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					Если ЕстьДанныеДляОбработки = Истина Тогда
						...
						ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
							+ НСтр("ru='Есть данные для обработки.'")
							+ Символы.ПС;
								...
								Для каждого ТекущаяКатегория Из ТаблицаКатегорий Цикл
									...
									Если (КатегорияНовостей <> Неопределено) И (НЕ КатегорияНовостей.Пустая()) Тогда
										...
										НайденныеСтроки = ТаблицаКатегорийИЗначенийПростые.НайтиСтроки(
											Новый Структура("КатегорияНовостейСтрока",
												ТекущаяКатегория.КатегорияНовостейСтрока));
...
#Область ОчисткаМусора
				...
				Если ЕстьДанныеДляОбработки = Истина Тогда
					...
					ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						...
						Результат = Запрос.Выполнить(); // ПересчитатьНеобработанныеКатегорииНовостей(), Регистр сведений "КатегорииНовостейПростыеНеобработанные", потерянные записи.
						Если НЕ Результат.Пустой() Тогда
							ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ПересчетОтборов_ВыполнениеЗапроса
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			РезультатЗапроса = Запрос.Выполнить(); // ПересчитатьОтборыПоНовостям_Общие()
...
#Область ПересчетПользовательскихОтборов
		...
		Если ПересчитатьПоТекущемуПользователю = Истина Тогда
			...
			Если (КоличествоОтборов.КоличествоПользовательскихОтборов > 0) Тогда
				...
				ПересчитатьОтборыПоНовостям_Пользовательские(ПользователиКлиентСервер.ТекущийПользователь(), КонтекстВыполненияВложенный);
			КонецЕсли;
		Иначе
			// Пересчитать отборы по всем пользователям.
			// Может работать в модели сервиса.
...
#Область ЗаполнениеЗначенийАвтоматическихОтборов
		...
		// Отбора по области данных должны быть заполнены или для локального режима,
...
#Область ПересчетОтборов_ВыполнениеЗапроса
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			РезультатЗапроса = Запрос.Выполнить(); // ПересчитатьОтборыПоНовостям_ДляОбластиДанных()
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. ДляОбластиДанных'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ПолучениеСпискаПользователей
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			РезультатЗапроса = Запрос.Выполнить(); // ПересчитатьОтборыПоНовостям_Пользовательские(), Получить список пользователей, по которым установлены отборы
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Результат запроса: %1
					|МассивПользователей.Количество(): %2
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Пользовательские'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ЕдинственноеЗначениеОтбора
	...
	РезультатЗапроса = Запрос.Выполнить(); // ЗаполнитьЗначенияАвтоматическихОтборов_Общие / ЕдинственноеЗначениеОтбора.
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
...
#Область МножественноеЗначениеОтбора
	...
	Если ТипЗнч(МассивКодовКатегорий) <> ТипМассив Тогда
		МассивКодовКатегорий = Новый Массив;
	КонецЕсли;
	МассивКодовКатегорий.Добавить("SubsystemRealized");
	МассивКодовКатегорий.Добавить("SubsystemNotRealized");
	МассивКодовКатегорий.Добавить("FnOptionOn");
	МассивКодовКатегорий.Добавить("FnOptionOff");
	...
	Запрос.Текст = "
		...
		|ИЗ
		|	ВТ_ЛентыНовостей КАК ВТ
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ // Если какие-то категории не настроены в новостях, то и не рассчитывать их
		|	РегистрСведений.КатегорииНовостейПростые КАК Рег
		|	ПО
		|		ВТ.ЛентаНовостей = Рег.Новость.ЛентаНовостей
		|		И ВТ.КатегорияНовостей = Рег.КатегорияНовостей
		|;
		|
		|// 2. Общий список лент новостей и значений категорий (для заполнения общего отбора)
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВТ.КодКатегорииНовостей      КАК КодКатегорииНовостей,
		|	ВТ.КатегорияНовостей         КАК КатегорияНовостей,
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.СледующийПоЗначениюПоля("КатегорияНовостей") Цикл
			...
			//  - в реальности эта подсистема НЕ внедрена, значит запись в регистр сведений ОтборыПоЛентамНовостейОбщие
			//      для этой категории не будет добавлена, т.е. регистр сведений ОтборыПоЛентамНовостейОбщие - пустой;
			//  - если регистр сведений ОтборыПоЛентамНовостейОбщие - пустой, то значит
...
#Область МножественноеЗначениеОтбора
		...
		Если ТипЗнч(МассивКодовКатегорий) <> ТипМассив Тогда
			МассивКодовКатегорий = Новый Массив;
		КонецЕсли;
		МассивКодовКатегорий.Добавить("ServiceDataAreaOn");
		МассивКодовКатегорий.Добавить("ServiceDataAreaOff");
		МассивКодовКатегорий.Добавить("FnOptionDataAreaOn");
		МассивКодовКатегорий.Добавить("FnOptionDataAreaOff");
		...
		Запрос.Текст = "
			...
			Если ИспользоватьНомерОбластиДанных = Истина Тогда
				Запрос.Текст = Запрос.Текст + "
					...
					|	ВТ.ЛентаНовостей                       КАК ЛентаНовостей
					|ИЗ
					|	РегистрСведений.ОбластиДанных КАК Рег
					|	ЛЕВОЕ СОЕДИНЕНИЕ
					|	ВТ_ЗначенияКатегорийПоЛентамНовостей КАК ВТ
					|	ПО
					|		ИСТИНА // Перемножение таблиц
					|ГДЕ
					|	Рег.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбластейДанных.Используется)
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Пока Выборка.СледующийПоЗначениюПоля("ОбластьДанных") Цикл
				Пока Выборка.СледующийПоЗначениюПоля("КатегорияНовостей") Цикл
					...
					//      сведений ОтборыПоЛентамНовостейОбщие для этой категории не будет добавлена,
					//      т.е. регистр сведений ОтборыПоЛентамНовостейОбщие - пустой;
					//  - если регистр сведений ОтборыПоЛентамНовостейОбщие - пустой, то значит
					//      подходит любой отбор, т.е. "для всех".
					НоваяЗапись = НаборЗаписей.Добавить();
					Если ИспользоватьНомерОбластиДанных = Истина Тогда
	...
	Если СтандартнаяОбработка <> Ложь Тогда
		...
		ИначеЕсли (ВРег(КодКатегорииНовостей) = ВРег("FnOptionOn"))
			...
			Исключение
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
					НСтр("ru='Новости. Сервис и регламент. РассчитатьОтборПоКатегории. Общие'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
		...
		ИначеЕсли (ВРег(КодКатегорииНовостей) = ВРег("FnOptionDataAreaOn"))
			...
			Исключение
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
					НСтр("ru='Новости. Сервис и регламент. РассчитатьОтборПоКатегории. ДляОбластиДанных'"), // ИдентификаторШага
...
Процедура ПроверитьИзменениеВерсииПлатформы(КонтекстВыполнения = Неопределено)
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если (ТипЗнч(лкПоследняяВерсия) <> ТипСтрока) // Первый запуск
				...
				КонецЕсли;
				ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%СохраненнаяВерсияПлатформы%", лкПоследняяВерсия);
				ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТекущаяВерсияПлатформы%", лкТекущаяВерсия);
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
			НСтр("ru='Новости. Все обновления новостей. Проверка версии платформы'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
//     у который реквизит УстановленоПрограммно = Истина);
...
#Область ВыполнениеЗапроса
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(РезультатыЗапроса[1].Пустой(),
				НСтр("ru='пустой'"),
				НСтр("ru='есть данные'")),
			?(РезультатыЗапроса[2].Пустой(),
				НСтр("ru='пустой'"),
				НСтр("ru='есть данные'")),
			?(РезультатыЗапроса[3].Пустой(),
				НСтр("ru='пустой'"),
				НСтр("ru='есть данные'"))),
		Неопределено);
...
#Область Оптимизация_УдалениеЗначенийОтборов_ДляОбластиДанных
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если ОптимизироватьОтборыПоНовостямДляОбластиДанных = Истина Тогда
			...
			РезультатЗапроса = Запрос.Выполнить(); // ОптимизироватьОтборыПоНовостям() / Оптимизация отборов по области данных.
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если ОптимизироватьОтборыПоНовостямПользовательские = Истина Тогда
			...
			Если ОбщегоНазначения.РазделениеВключено()
					...
					И ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей()
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ОптимизацияОтборовПоНовостям'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура НайтиИОбработатьНеактивныеЛентыНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Результат = Запрос.Выполнить(); // НайтиИОбработатьНеактивныеЛентыНовостей.
			Если НЕ Результат.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					Иначе
						лкЧастотаОбновления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если (ДанныеДляУдаления.Свойство("КатегорииНовостейПростыеНеобработанные"))
				...
				КонецЦикла;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Было удалено %1 необработанных простых категорий.'"),
					ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество());
			Иначе
			...
			Если (ДанныеДляУдаления.Свойство("КатегорииНовостейИнтервалыВерсийНеобработанные"))
				...
				КонецЦикла;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Было удалено %1 необработанных категорий интервалов версий.'"),
					ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество());
			Иначе
...
Процедура ЗагрузитьВсеСтандартныеЗначения(КонтекстВыполнения = Неопределено)
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
//
// Параметры:
//  Форма                                    - УправляемаяФорма - форма, в которой необходимо включить
...
//                                              идентификатором в самой новости;
//  НастройкиКнопкиНовостей                  - Структура, Неопределено - структура настроек для создания
//                                              кнопки / подменю новостей
//                                             или Неопределено, если кнопку / подменю создавать не надо. Ключи:
//   * ЭлементФормыДляРазмещениеКомандыНовостей - ЭлементФормы -  Группа формы, в которой необходимо разместить
//                                              создаваемую кнопку / подменю новостей;
...
//  ЗаголовокФормыНовостей                   - Строка - заголовок формы, которая появляется при нажатии на кнопку
...
//  ИдентификаторыСобытийПриОткрытии         - Строка, Массив - строка или массив строк идентификаторов
...
Процедура КонтекстныеНовости_ПриСозданииНаСервере(
	...
	ОписаниеНовостей.Вставить("Новости", Новый Массив);
...
#Область ОсновныеДействия
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Проверка правильности внедрения подсистемы. Начало.'")
					+ Символы.ПС;
				...
				КонецЕсли;
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Проверка правильности внедрения подсистемы. Конец.'")
					+ Символы.ПС;

				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Установка основных констант. Начало.'")
					+ Символы.ПС;
				Константы.РазрешенаРаботаСНовостями.Установить(Истина);
				Константы.РазрешенаРаботаСНовостямиЧерезИнтернет.Установить(Истина);
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Установка основных констант. Конец.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Очистка регистра ДатыЗагрузкиПоследнихНовостей. Начало.'")
					+ Символы.ПС;
				НаборЗаписей = РегистрыСведений.ДатыЗагрузкиПоследнихНовостей.СоздатьНаборЗаписей();
				НаборЗаписей.Записать(Истина);
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Очистка регистра ДатыЗагрузкиПоследнихНовостей. Конец.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Загрузка стандартных значений. Начало.'")
					+ Символы.ПС;
				ЗагрузитьВсеСтандартныеЗначения(КонтекстВыполненияВложенныйВложенный);
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Загрузка стандартных значений. Конец.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Установка даты загрузки новостей на -3 месяца. Начало.'")
					+ Символы.ПС;
				Запрос = Новый Запрос;
				Запрос.Текст = "
					|ВЫБРАТЬ
					...
					|";
				РезультатЗапроса = Запрос.Выполнить(); // ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск.
				Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				КонецЕсли;
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Установка даты загрузки новостей на -3 месяца. Конец.'")
					+ Символы.ПС;

				// Установить / переустановить значение расписания для регламентного задания ВсеОбновленияНовостей.
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru='Установка расписания регламентного задания. Начало.'")
					+ Символы.ПС;
				ТекущееРегламентноеЗадание = РегламентныеЗаданияСервер.ПолучитьРегламентноеЗадание(Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей);
				Если ТекущееРегламентноеЗадание <> Неопределено Тогда
					ТекущееРегламентноеЗадание.Наименование = НСтр("ru='Все обновления новостей'");
					...
					ТекущееРегламентноеЗадание.Записать();
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
						+ НСтр("ru='Установка расписания регламентного задания. Конец.'")
						+ Символы.ПС;
				Иначе
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
						+ НСтр("ru='Регламентное задание ВсеОбновленияНовостей не найдено.'")
						+ Символы.ПС
						+ НСтр("ru='Установка расписания регламентного задания. Конец.'")
						+ Символы.ПС;
				КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ОсновныеДействия
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Иначе
					ОписаниеРезультатаВложенный = НСтр("ru='Работа с новостями отключена. Обработчик пропущен.'");
				КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Если лкРазрешенаРаботаСНовостями = Истина Тогда
					...
					Запрос.Текст = "
						...
						|	Спр.ЗагруженоССервера = ИСТИНА
						|";
					РезультатЗапроса = Запрос.Выполнить(); // ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию.
					Если НЕ РезультатЗапроса.Пустой() Тогда
						ДатаЗагрузкиПоследнихНовостей = ДобавитьМесяц(ТекущаяУниверсальнаяДата(), -3);
						Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
						Пока Выборка.Следующий() Цикл
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
// Процедура - для общих данных.
//
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию_2_2_2() Экспорт

	НаименованиеПроцедурыФункции = "ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию_2_2_2"; // Идентификатор.
	КонтекстВыполнения = ИнтернетПоддержкаПользователейКлиентСервер.НоваяЗаписьРезультатовВыполненияОпераций(); // Этот контекст.
	ЗаписыватьВЖурналРегистрации = Истина;
...
#Область ОсновныеДействия_НастройкаПараметровДоступаКСерверамНовостей
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Было:
						|%1'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(НастройкиПолученияНовостей, "=", Символы.ПС));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если (ДанныеЛентыНовостей.ЗагруженоССервера = Ложь)
		...
		Если МассивПользователейСРольюРедакторНовостей.Количество() > 0 Тогда
			Если ВыводитьВсехПользователей = Истина Тогда
				...
				РезультатЗапроса = Запрос.Выполнить(); // ПолучитьСписокРедакторовЛентыНовостей / ВыводитьВсехПользователей - истина.
				Если НЕ РезультатЗапроса.Пустой() Тогда
					Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
					Пока Выборка.Следующий() Цикл
						ОписаниеПользователя = Новый Структура;
			...
			Иначе
				...
				Запрос.УстановитьПараметр("МассивПользователейСРольюРедакторНовостей", МассивПользователейСРольюРедакторНовостей);
				Запрос.УстановитьПараметр("ЛентаНовостей", ДанныеЛентыНовостей.Ссылка);
				РезультатЗапроса = Запрос.Выполнить(); // ПолучитьСписокРедакторовЛентыНовостей / ВыводитьВсехПользователей - ложь.
				Если НЕ РезультатЗапроса.Пустой() Тогда
					Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
					Пока Выборка.Следующий() Цикл
						ОписаниеПользователя = Новый Структура;
