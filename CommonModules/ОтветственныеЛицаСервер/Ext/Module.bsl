...
Функция СтруктураДанныхОтветственныхЛицДокумента(Объект)
	...
	Если ЭтоТорговыеДокументы_АВТ(Объект) Тогда 
		СтруктураДанных.Вставить("Менеджер", Объект.Менеджер);
	КонецЕсли;	
	// - АВТ_Новосельцев 09.03.2017 - 1	
...
Функция ПроверитьСоответствиеРеквизитовИОтветственногоЛица(РеквизитыДляПроверки, ОтветственноеЛицоДляПроверки)
	...
	// + АВТ_Новосельцев 10.03.2017 - 1
	Если РеквизитыДляПроверки.Свойство("ФизическоеЛицо") Тогда
		РеквизитыОтветственногоЛица.Вставить("ФизическоеЛицо", ОтветственноеЛицоДляПроверки.ФизическоеЛицо);
	КонецЕсли;
	// - АВТ_Новосельцев 10.03.2017 - 1
	...
	Если РеквизитыОтветственногоЛица.Владелец <> РеквизитыДляПроверки.Организация
	 ...
	 Или (РеквизитыДляПроверки.Свойство("ПометкаУдаления")
		...
		Возврат Ложь;
	// + АВТ_Новосельцев 10.03.2017 - 1
	ИначеЕсли РеквизитыДляПроверки.Свойство("ФизическоеЛицо") Тогда
		Если НЕ РеквизитыОтветственногоЛица.ФизическоеЛицо = РеквизитыДляПроверки.ФизическоеЛицо Тогда 
			Возврат Ложь;
		КонецЕсли;
	// - АВТ_Новосельцев 10.03.2017 - 1	
	КонецЕсли;
...
Функция СкорректироватьОтветственныхЛицДокумента(СтруктураДанныхОтветственныхЛиц)
	...
	// + АВТ_Новосельцев 09.03.2017 - 1
	Если СтруктураДанныхОтветственныхЛиц.Свойство("Менеджер") Тогда
		Если НЕ СтруктураДанныхОтветственныхЛиц.Менеджер.ФизическоеЛицо.Пустая() Тогда
			Если СтруктураДанныхОтветственныхЛиц.Организация.Пустая() Тогда
				ТаблицаОтветственных = ПолучитьТаблицуОтветственныхЛицПоОтбору(
				Новый Структура("Дата, ФизическоеЛицо", Дата, СтруктураДанныхОтветственныхЛиц.Менеджер.ФизическоеЛицо));
			Иначе	
				ТаблицаОтветственных = ПолучитьТаблицуОтветственныхЛицПоОтбору(
				Новый Структура("Дата, Организация, ФизическоеЛицо", Дата, СтруктураДанныхОтветственныхЛиц.Организация, СтруктураДанныхОтветственныхЛиц.Менеджер.ФизическоеЛицо));
			КонецЕсли;	
		КонецЕсли;	
	КонецЕсли;
	// - АВТ_Новосельцев 09.03.2017 - 1	
	...
	Для Каждого КлючИЗначение Из РеквизитыОтветственныеЛица Цикл
		...
		// + АВТ_Новосельцев 10.03.2017 - 1
		Если СтруктураДанныхОтветственныхЛиц.Свойство("Менеджер") Тогда
			РеквизитыДляПроверки.Вставить("ФизическоеЛицо", СтруктураДанныхОтветственныхЛиц.Менеджер.ФизическоеЛицо);	
		КонецЕсли;
		// - АВТ_Новосельцев 10.03.2017 - 1
...
Процедура ЗаполнитьОтветственныхЛицДокумента(Объект, ДанныеЗаполнения = Неопределено, ПерезаполнятьНепустыеРеквизиты = Ложь)
	...
	// + АВТ_Новосельцев 09.03.2017 - 1
	Если ЭтоТорговыеДокументы_АВТ(Объект) Тогда 
		РеквизитыДляОтбора.Вставить("Менеджер", Объект.Менеджер);
	КонецЕсли;	
	// - АВТ_Новосельцев 09.03.2017 - 1	
	...
	// + АВТ_Новосельцев 11.03.2017 - 6
	Если ЭтоКорректировочныеДокументы_АВТ(Объект) Тогда 
		Объект.Отпустил 			= СтруктураДанныхОтветственныхЛиц.Руководитель.ФизическоеЛицо;
		Объект.ОтпустилДолжность    = СтруктураДанныхОтветственныхЛиц.Руководитель.Должность;
	КонецЕсли;	
	// - АВТ_Новосельцев 11.03.2017 - 6	
...
// + АВТ_Новосельцев 09.03.2017 - 1
Функция ЭтоТорговыеДокументы_АВТ(Объект) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Ссылка") Тогда
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") 
			ИЛИ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
...
Функция ЭтоКорректировочныеДокументы_АВТ(Объект) Экспорт
