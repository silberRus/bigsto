...
Процедура ЗаписатьДанныеВыгрузкиВФайл(ИмяФайла, ДанныеДляВыгрузки, Результат, ВыходныеПараметры, ВерсияФорматаОбмена) Экспорт
	
	ТекстXML = ПолучитьТекстXMLДанныхВыгрузки(ДанныеДляВыгрузки, ВерсияФорматаОбмена);
...
Функция УстановитьФлагДанныеЗагружены(ИменаФайлов, ВыходныеПараметры, ВерсияФорматаОбмена) Экспорт
	
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		
		ТекстXML = ПолучитьТекстXMLДанныеЗагружены(ВерсияФорматаОбмена);
		...
		Если НЕ ОтчетОПродажахXTDO.Чеки = Неопределено Тогда
			...
			Для Каждого ЧекXDTO Из ОтчетОПродажахXTDO.Чеки.Чек Цикл
				...
				Если НЕ ЧекXDTO.КодСистемыНалогообложения = Неопределено Тогда
					ЧекККМ.СистемаНалогообложения =
					МенеджерОборудованияКлиентСервер.ПолучитьТипСистемыНалогообложенияККТПоКоду(ЧекXDTO.КодСистемыНалогообложения);
				КонецЕсли;
				
				Если НЕ ЧекXDTO.КодТипаРасчета = Неопределено Тогда
					ЧекККМ.ТипРасчета =
					МенеджерОборудованияКлиентСервер.ПолучитьТипРасчетаДенежнымиСредствамиПоКоду(ЧекXDTO.КодТипаРасчета);
				КонецЕсли;
				...
				Если НЕ ЧекXDTO.Товары = Неопределено Тогда
					...
					Для Каждого СтрокаТоварXDTO Из ЧекXDTO.Товары.ЧекТовар Цикл
						...
						Если НЕ СтрокаТоварXDTO.КодПризнакаСпособаРасчета = Неопределено Тогда
							ТоварЧекаККМ.ПризнакСпособаРасчета =
							МенеджерОборудованияКлиентСервер.ПолучитьПризнакСпособаРасчетаПоКоду(СтрокаТоварXDTO.КодПризнакаСпособаРасчета);
						КонецЕсли;
						...
						Если ЗначениеЗаполнено(СтрокаТоварXDTO.ШтрихкодАлкогольнойПродукции) Тогда
							ТоварЧекаККМ.ШтрихкодАлкогольнойПродукции.Добавить(СтрокаТоварXDTO.ШтрихкодАлкогольнойПродукции);
						КонецЕсли;
...
Функция ПолучитьТекстXMLДанныхВыгрузки(ДанныеДляВыгрузки, ВерсияФорматаОбмена)
...
Функция ПолучитьТекстXMLДанныеЗагружены(ВерсияФорматаОбмена)
...
Функция ПолучитьЗаполнитьОбъектНастройкиККМ(ВыгружаемыеНастройкиККМ, ВерсияФорматаОбмена, ЕстьВыгружаемыеНастройки = Ложь)
	...
	Если ЗначениеЗаполнено(ВыгружаемыеНастройкиККМ.СистемыНалогообложения) Тогда
		...
		Для Каждого СистемаНалогообложенияККТ Из ВыгружаемыеНастройкиККМ.СистемыНалогообложения Цикл
			...
			ОбъектСистемаНалогообложенияЗапись.КодСистемыНалогообложения =
				МенеджерОборудованияКлиентСервер.ПолучитьКодТипаСистемыНалогообложенияККТ(СистемаНалогообложенияККТ);
			
			ОбъектСистемыНалогообложения.СистемаНалогообложенияЗапись.Добавить(ОбъектСистемаНалогообложенияЗапись);
	...
	Если ЗначениеЗаполнено(ВыгружаемыеНастройкиККМ.ВидыЭлектроннойОплаты) Тогда
		...
		Для Каждого ВидОплаты Из ВыгружаемыеНастройкиККМ.ВидыЭлектроннойОплаты Цикл
			...
			ОбъектВидыЭлектроннойОплаты.ВидЭлектроннойОплатыЗапись.Добавить(ОбъектВидЭлектроннойОплатыЗапись);
...
Функция ПолучитьЗаполнитьОбъектПрайсЛист(ВыгружаемыйПрайсЛист, ВерсияФорматаОбмена, ЕстьВыгружаемыеДанные = Ложь)
	...
	Если ЗначениеЗаполнено(ВыгружаемыйПрайсЛист.Товары) Тогда
		...
		Для Каждого Товар Из ВыгружаемыйПрайсЛист.Товары Цикл
			...
			Если ЗначениеЗаполнено(Товар.ПризнакПредметаРасчета) Тогда
				ЗаписьТовар.ПризнакПредметаРасчета =
					МенеджерОборудованияКлиентСервер.ПолучитьКодПризнакаПредметаРасчета(Товар.ПризнакПредметаРасчета);
			Иначе
			...
			ОбъектТовары.ТоварЗапись.Добавить(ЗаписьТовар);
	...
	Если ЗначениеЗаполнено(ВыгружаемыйПрайсЛист.ГруппыТоваров) Тогда
		...
		Для Каждого Группа Из ВыгружаемыйПрайсЛист.ГруппыТоваров Цикл
			...
			ОбъектГруппыТоваров.ГруппаТоваровЗапись.Добавить(ЗаписьГруппа);
		КонецЦикла;
	...
	Если ЗначениеЗаполнено(ВыгружаемыйПрайсЛист.ЕдиницыИзмерения) Тогда
		...
		Для Каждого ЕИ Из ВыгружаемыйПрайсЛист.ЕдиницыИзмерения Цикл
			...
			ОбъектЕдиницыИзмерения.ЕдиницаИзмеренияЗапись.Добавить(ЗаписьЕИ);
		...
		Для Каждого ДанныеАгента Из ВыгружаемыйПрайсЛист.ДанныеАгентов Цикл
			...
			Если ЗначениеЗаполнено(ДанныеАгента.ПризнакАгента) Тогда
				ЗаписьДанныеАгента.ПризнакАгента =
					МенеджерОборудованияКлиентСервер.ПолучитьКодПризнакаАгента(ДанныеАгента.ПризнакАгента);
			КонецЕсли;
			...
			ОбъектДанныеАгентов.ДанныеАгентаЗапись.Добавить(ЗаписьДанныеАгента);
	...
	Если ЗначениеЗаполнено(ВыгружаемыйПрайсЛист.ДанныеПоставщиков) Тогда
		...
		Для Каждого ДанныеПоставщика Из ВыгружаемыйПрайсЛист.ДанныеПоставщиков Цикл
			...
			ОбъектДанныеПоставщиков.ДанныеПоставщикаЗапись.Добавить(ЗаписьДанныеПоставщика);
	...
	Для Каждого Заказ Из ВыгружаемыеЗаказы Цикл
		...
		Если ЗначениеЗаполнено(Заказ.Товары) Тогда
			...
			Для Каждого Товар Из Заказ.Товары Цикл
				...
				ОбъектЗаказТовары.ЗаказТоварыЗапись.Добавить(ЗаписьТовар);
			КонецЦикла;
		...
		Если ЗначениеЗаполнено(Заказ.Оплаты) Тогда
			...
			Для Каждого Оплата Из Заказ.Оплаты Цикл
				...
				ОбъектОплаты.Оплата.Добавить(ЗаписьОплата);
			КонецЦикла;
			
			ЗаписьЗаказ.Оплаты = ОбъектОплаты;
		КонецЕсли;
		
		ОбъектЗаказы.Заказы.Добавить(ЗаписьЗаказ);
...
Функция ЗаписатьХарактеристики(Товар, ВерсияФорматаОбмена)
	...
	Для Каждого Характеристика Из Товар.Характеристики Цикл
		...
		ОбъектХарактеристики.ХарактеристикаЗапись.Добавить(ЗаписьХарактеристики);
...
Функция ЗаписатьУпаковки(Источник, ВерсияФорматаОбмена)
	...
	Для Каждого Упаковка Из Источник.Упаковки Цикл
		...
		ОбъектУпаковки.УпаковкаЗапись.Добавить(ЗаписьУпаковки);
...
Функция ЗаписатьШтрихкоды(URIИмен, Штрихкоды)
	...
	Для Каждого ДанныеШтрихкода Из Штрихкоды Цикл
		...
		ОбъектШтрихкоды.ШтрихкодЗапись.Добавить(ЗаписьШтрихкод);
	КонецЦикла;
