...
Процедура АТ_РасчетСреднедневногоПотребления() Экспорт
	...
	|ИЗ
	|	Дни КАК Дни,
	|	РегистрНакопления.ТоварыНаСкладах.ОстаткиИОбороты(ДОБАВИТЬКДАТЕ(&ТекущаяДата, МЕСЯЦ, -12), , День, , ) КАК ТоварыНаСкладахОстаткиИОбороты
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ОстаткиНаКаждыйДень.Склад КАК Склад,
	...
	|					И АналитикаУчетаПоПартнерам.Организация = &Организация) КАК ВыручкаИСебестоимостьПродаж
	...
	|	ВыручкаИСебестоимостьПродаж.АналитикаУчетаНоменклатуры.Склад КАК Склад,
	...
	|	ВыручкаИСебестоимостьПродаж.АналитикаУчетаНоменклатуры.Склад
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПродажиЗаГод.Номенклатура КАК Номенклатура,
	|	ПродажиЗаГод.Склад КАК Склад,
	|	ВЫБОР
	...
	|			И (ОстаткиНаКаждыйДеньРасширенные.Склад = ПродажиЗаГод.Склад)
	...
	ВыборкаПоСкладам = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока ВыборкаПоСкладам.Следующий() Цикл
		
		НаборЗаписей = РегистрыСведений.АТ_СреднедневноеПотребление.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Склад.Установить(ВыборкаПоСкладам.Склад);
