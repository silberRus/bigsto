...
Процедура СерииНоменклатурыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ВидНоменклатуры") Тогда
		...
		|				ТОГДА &ДатаПроизводстваНеУказан
		|						ИЛИ СерииНоменклатуры.ДатаПроизводства = &ДатаПроизводства
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ВЫБОР
		|			КОГДА СерииНоменклатуры.ВидНоменклатуры.ИспользоватьПроизводителяЕГАИССерии
		|				ТОГДА &ПроизводительЕГАИСНеУказан
		|						ИЛИ СерииНоменклатуры.ПроизводительЕГАИС = &ПроизводительЕГАИС
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ВЫБОР
		|			КОГДА СерииНоменклатуры.ВидНоменклатуры.ИспользоватьСправку2ЕГАИССерии
		|				ТОГДА &Справка2ЕГАИСНеУказан
		|						ИЛИ СерииНоменклатуры.Справка2ЕГАИС = &Справка2ЕГАИС
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|
		|УПОРЯДОЧИТЬ ПО
		...
		Запрос.УстановитьПараметр("ДатаПроизводстваНеУказан",     Не ЗначениеЗаполнено(Параметры.ДатаПроизводства));
		Запрос.УстановитьПараметр("ПроизводительЕГАИС",		    Параметры.ПроизводительЕГАИС);
		Запрос.УстановитьПараметр("ПроизводительЕГАИСНеУказан", Не ЗначениеЗаполнено(Параметры.ПроизводительЕГАИС));
		Запрос.УстановитьПараметр("Справка2ЕГАИС",		        Параметры.Справка2ЕГАИС);
		Запрос.УстановитьПараметр("Справка2ЕГАИСНеУказан",      Не ЗначениеЗаполнено(Параметры.Справка2ЕГАИС));
		...
		Пока Выборка.Следующий() Цикл
			...
			Значение = Новый Структура("Серия,Номер,ГоденДо,НомерКиЗГИСМ,RFIDTID,RFIDUser,RFIDEPC,EPCGTIN,ДатаПроизводства,ПроизводительЕГАИС,Справка2ЕГАИС");
...
Процедура УпаковкиЕдиницыИзмеренияОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ВыборРодителя") Тогда
		...
		|	УпаковкиНоменклатуры.ПометкаУдаления КАК ПометкаУдаление,
...
Процедура ЗаполнитьДанныеВыбораУпаковками(Номенклатура, ДанныеВыбора, СтрокаПоиска, Знач ПоказыватьМерныеЕдиницыИзмерения, ДобавлятьЕдиницуХранения, ПоказыватьТоварныеМеста)
	...
	|			И (УпаковкиНоменклатуры.ТипИзмеряемойВеличины = Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины)
	|ГДЕ
