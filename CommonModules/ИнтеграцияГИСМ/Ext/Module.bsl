...
Процедура ВыполнитьОбменРегламентноеЗадание() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОтправкаПолучениеДанныхГИСМ);
...
//
Функция ПредставлениеСтатусаГИСМ(СтатусГИСМ, ВходящиеДальнейшиеДействия, ДопустимыеДействия) Экспорт
	...
	Если ТипЗнч(ВходящиеДальнейшиеДействия) = Тип("ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюГИСМ") И ЗначениеЗаполнено(ВходящиеДальнейшиеДействия) Тогда
		ДальнейшиеДействия.Добавить(ВходящиеДальнейшиеДействия);
	ИначеЕсли ТипЗнч(ВходящиеДальнейшиеДействия) = Тип("Массив") Тогда
		ДальнейшиеДействия = ВходящиеДальнейшиеДействия;
	КонецЕсли;
	
	СтрокиДальнейшееДействие = Новый Массив;
	СтрокиДальнейшееДействие.Добавить(СтатусГИСМПредставление);
	...
	Если СтрокиДальнейшееДействие.Количество() > 2 Тогда
		СтатусГИСМПредставление = Новый ФорматированнаяСтрока(СтрокиДальнейшееДействие);
	КонецЕсли;
...
Процедура ЗаписатьВерсииСхемОбмена(Клиент, Сервер, ОбработчикОбновления = Ложь) Экспорт
	...
	Если Не ОбработчикОбновления Тогда
		Константы.ВерсииСхемОбменаГИСМ.Установить(Клиент + "," + Сервер);
	Иначе
		МенеджерЗначения = Константы.ВерсииСхемОбменаГИСМ.СоздатьМенеджерЗначения();
		МенеджерЗначения.Значение = Клиент + "," + Сервер;
...
Функция ПроверитьОбновитьВерсиюСхемОбмена(НоваяВерсияСхемОбмена, ОбработчикОбновления = Ложь) Экспорт
	...
	Если ВерсииСхемОбмена.Клиент <> НоваяВерсияСхемОбмена
		...
		Если СоответствиеВерсийСхемОбмена().Получить(НоваяВерсияСхемОбмена) <> Неопределено Тогда
			...
			ЗаписатьВерсииСхемОбмена(НоваяВерсияСхемОбмена, НоваяВерсияСхемОбмена, ОбработчикОбновления);
		...
		Иначе
			...
			Если ВерсииСхемОбмена.Сервер <> НоваяВерсияСхемОбмена Тогда
				ЗаписатьВерсииСхемОбмена(ВерсииСхемОбмена.Клиент, НоваяВерсияСхемОбмена, ОбработчикОбновления);
			КонецЕсли;
	...
	ИначеЕсли ВерсииСхемОбмена.ТребуетсяОбновление Тогда
		...
		ЗаписатьВерсииСхемОбмена(ВерсииСхемОбмена.Клиент, ВерсииСхемОбмена.Сервер, ОбработчикОбновления);
...
Функция ОбъектXDTOВXML(ОбъектXDTO, ИмяТипа, Версия = Неопределено) Экспорт
	...
	ТекстXML = СтрЗаменить(ТекстXML, "xmlns:d2p1=""" + URIПространстваИменИспользуемойВерсииСхемОбмена(Версия) + """", "");
	ТекстXML = СтрЗаменить(ТекстXML, URIПространстваИменИспользуемойВерсииСхемОбмена(Версия), "");
...
//
Функция ИдентификаторЗнака(СтрокаТЧ, НаименованиеИдентификатора) Экспорт
