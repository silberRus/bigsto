...
//
Функция СтруктураСообщений(Объект) Экспорт
...
// Параметры:
//  Объект - ДанныеФормыСтруктура - Объект.
//  ЭлементФормы - ЭлементФормы - Элемент формы.
...
Процедура НастроитьКомандуПоказатьСообщения(Объект, ЭлементФормы, Видимость = Истина) Экспорт
	...
	СтруктураСообщений = СтруктураСообщений(Объект);
...
// Возвращаемое значение:
//  Структура - Структура со свойствами:
//   * ДеревоСкидок - ДеревоЗначений - Дерово скидок (наценок).
...
Функция Рассчитать(Объект, ВходныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту")
	// silber {
		Или ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.КП") Тогда
	// } silber
...
// Параметры:
//  Объект - ДокументОбъект, ДанныеФормыСтруктура - Объект, в котором требуется рассчитать скидки (наценки).
//  ПримененныеСкидки - Структура со свойствами:
//   Структура - Структура со свойствами:
//    * ДеревоСкидок - ДеревоЗначений - Дерово скидок (наценок).
//    * ТаблицаСкидкиНаценки - ТаблицаЗначений - Таблица с рассчитанными скидками.
//    * ПараметрыРасчета - Структура - Структура параметров расчета.
...
Процедура ПрименитьРезультатРасчета(Объект, ПримененныеСкидки, РеализацияСверхЗаказа = Ложь) Экспорт
	...
	ИначеЕсли 	ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту") Или
				ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.КП") Тогда
		// } silber
...
//    * ПараметрыРасчета - Структура - Структура параметров расчета.
//
Процедура ПрименитьРезультатРасчетаКРеализацииТоваровУслуг(Объект, ПримененныеСкидкиНаценки, РеализацияСверхЗаказа) Экспорт
...
//  ИмяТаблицыВыделенныхСтрок	 - Строка - Имя таблицы выделенных строк
...
Процедура СформироватьИнформационнуюНадписьИспользованиеСкидокНаценок(ИнформацияОДействииСкидок,
	...
	Если ИспользованиеСкидкиНаценки = Неопределено Тогда
		...
		Если ДополнительнаяКоманда = "НастроитьСкидки" Тогда
			ИнформацияОДействииСкидок = Новый ФорматированнаяСтрока(НСтр("ru = 'Настроить скидки (наценки)'"),, ЦветаСтиля.ЦветГиперссылки,,"НастроитьСкидки");
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если ДополнительнаяКоманда <> Неопределено
		...
		Если ДополнительнаяКоманда = "Изменить" Тогда
			СтрокаИзменить = Новый ФорматированнаяСтрока(НСтр("ru = 'Изменить'"),, ЦветаСтиля.ЦветГиперссылки,,"Изменить");
		ИначеЕсли ДополнительнаяКоманда = "НастроитьСкидки" Тогда
			СтрокаИзменить = Новый ФорматированнаяСтрока(НСтр("ru = 'Настроить скидки (наценки)'"),, ЦветаСтиля.ЦветГиперссылки,,"НастроитьСкидки");
		ИначеЕсли ДополнительнаяКоманда = "ИзменитьОбщийСтатус" Тогда
			СтрокаИзменить = Новый ФорматированнаяСтрока(НСтр("ru = 'Изменить общий статус'"),, ЦветаСтиля.ЦветГиперссылки,,"ИзменитьОбщийСтатус");
		Иначе
			СтрокаИзменить = "";
		КонецЕсли;
...
Процедура ПрименитьРезультатРасчетаКОбъекту(Объект, 
	...
	// silber { Если есть флаг "ЭтоИзКП" тогда не расчитываем автоматическую скидку
	ПроверятьКП = Объект.Ссылка.Метаданные().ТабличныеЧасти[ИмяТЧ].Реквизиты.Найти("ЭтоИзКП") <> Неопределено;
	// } silber
	
	Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
		
		// silber {
		Если ПроверятьКП И СтрокаТЧ.ЭтоИзКП Тогда
			Продолжить;
		КонецЕсли;
		// } silber
		...
		ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьЦенуСоСкидкой(СтрокаТЧ, ИмяКоличества);
		// Storverk Тюняков "ЦенаСоСкидкой"
...
Функция ЗапросСкидкиНаценки(ПараметрыРасчета, РасширенныйВариант = Истина)
	...
	|	//РасширенныйВариант 	КОГДА СкидкиНаценки.СпособПримененияСкидки = ЗНАЧЕНИЕ(Перечисление.СпособыПримененияСкидокНаценок.ПрименитьВМоментРасчетаСкидокНаценок)
	|	//РасширенныйВариант 		И (СкидкиНаценки.СпособПредоставления = ЗНАЧЕНИЕ(Перечисление.СпособыПредоставленияСкидокНаценок.Сумма) ИЛИ СкидкиНаценки.СпособПредоставления = ЗНАЧЕНИЕ(Перечисление.СпособыПредоставленияСкидокНаценок.СуммаДляКаждойСтроки))
...
Процедура ПриСозданииЧтенииНаСервере(Форма, ТекущийОбъект = Неопределено) Экспорт
	...
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("СправочникСсылка.УсловияПредоставленияСкидокНаценок") Тогда
		...
		ЭлементыВариантОтбораНоменклатуры.Добавить("ГруппаВариантОтбораНоменклатурыЗаНакопленныйОбъемПродаж");
		ЭлементыВариантОтбораНоменклатуры.Добавить("ГруппаВариантОтбораНоменклатуры");
		Для Каждого ИмяЭлемента Из ЭлементыВариантОтбораНоменклатуры Цикл
			Форма.Элементы[ИмяЭлемента + "СегментНоменклатуры"].Видимость = ИспользоватьСегментыНоменклатуры;
		КонецЦикла;
