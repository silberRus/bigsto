...
Процедура ОбработатьОтклонениеАннулированиеЭД(ЭД, Отклонить, ОтклонитьАннулирование = Ложь, ОбработчикОповещения = Неопределено) Экспорт
	...
	Если Отклонить Тогда
		...
		Иначе
			...
			Если ПараметрыОбработкиЭД.Количество() > 0 И ПараметрыОбработкиЭД[0].ЭтоСчетФактура Тогда
...
//
Процедура ПолучитьОтпечаткиСертификатов(Оповещение, Личные, ВыдаватьОшибку) Экспорт
	
	Контекст = Новый Структура("Оповещение", Оповещение);
	ЭлектроннаяПодписьКлиент.ПолучитьОтпечаткиСертификатов(
		Новый ОписаниеОповещения("ПолучитьОтпечаткиВСервисеПослеПолученияОтпечатковНаКлиенте", ЭтотОбъект, Контекст), Личные, Ложь);
...
Процедура ОткрытьФормуРегламентаЭДО(ОписаниеОповещения, ПараметрыФормы, ВладелецФормы) Экспорт 
	...
	ОткрытьФорму("ОбщаяФорма.НастройкаРегламентаЭДО", 
		ПараметрыФормы,
		ВладелецФормы,
		,,,ОписаниеОповещения,
...
//  ОбработкаПродолжения - ОписаниеОповещения - оповещение, которое будет вызвано после выбора договора.
//  СтандартнаяОбработка - Булево - признак выполнения стандартной обработки.
//
Процедура ОткрытьФормуВыбораДоговора(Организация, Контрагент, ОбработкаПродолжения, СтандартнаяОбработка = Ложь) Экспорт
	
	ВидОперации = НСтр("ru = 'Открытие формы выбора договора'");
	ТекстСообщения = НСтр("ru = 'Ошибка открытия формы выбора договора'");
	ПараметрыОбработкиОшибки = Новый Структура("ВидОперации, ТекстСообщения", ВидОперации, ТекстСообщения);
	
	ИмяФормыВыбора = "";
	ПараметрыОткрытия = Неопределено;
	ПараметрыОтбора = Новый Структура("Организация, Контрагент", Организация, Контрагент);
	Если ОбменСКонтрагентамиСлужебныйВызовСервера.ПодготовитьПараметрыДляВыбораДоговора(ИмяФормыВыбора, ПараметрыОткрытия,
		ПараметрыОтбора, ПараметрыОбработкиОшибки) Тогда
...
Процедура ОбработатьЭДПослеПолученияОтпечатков(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.Свойство("ПрофилиЭДОБезСертификатов") Тогда
			ПараметрыОповещения = Новый Структура;
			ПараметрыОповещения.Вставить("ФормаПомощникаОткрыта", Ложь);
			ПараметрыОповещения.Вставить("АдресТаблицыДанных", Результат.ПрофилиЭДОБезСертификатов);
			...
			Если НЕ ПараметрыОповещения.ФормаПомощникаОткрыта Тогда
				ПараметрыФормы = Новый Структура("АдресТаблицыДанных", Результат.ПрофилиЭДОБезСертификатов);
