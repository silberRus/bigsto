...
Процедура ЗаполнитьТаблицуИзКроссТаблицы(Таблица, КроссТаблица, Параметры, СтруктураДействий = Неопределено, 
	...
	НоваяСтрокаСтруктура = Новый Структура();
	Для каждого Колонка Из Таблица.Выгрузить().Колонки Цикл
		НоваяСтрокаСтруктура.Вставить(Колонка.Имя);
	КонецЦикла;
	...
	Если ПолуфабрикатыПоДатам <> Неопределено Тогда
		...
		Для каждого СтрокаКроссТаблицы Из ПолуфабрикатыПоДатам Цикл
			...
			Для каждого СтрокаПериод Из ТаблицаПериоды Цикл
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрокаСтруктура,СтрокаКроссТаблицы);
				
				Для каждого Элемент Из СтруктураРеквизитовПериода Цикл
					НоваяСтрокаСтруктура[Элемент.Значение] = СтрокаПериод[Элемент.Ключ];
				КонецЦикла;
				
				Для каждого Элемент Из СтруктураРеквизитов Цикл
					
					НоваяСтрокаСтруктура[Элемент.Значение] = СтрокаКроссТаблицы[Элемент.Ключ + СтрокаПериод.ИмяКолонки];
					
				КонецЦикла;
				
				Если НоваяСтрокаСтруктура.КоличествоУпаковок <> 0 Тогда
					НоваяСтрока = Таблица.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, НоваяСтрокаСтруктура);
				Иначе
					Продолжить;
				КонецЕсли;
...
//
Процедура УстановитьНачалоОкончаниеПериодаПлана(Знач Периодичность, НачалоПериода, ОкончаниеПериода, КоличествоПериодов = 1) Экспорт 
	...
	Иначе
		...
		#КонецЕсли
		НачалоПериода = ОбщегоНазначенияУТКлиентСервер.РассчитатьДатуНачалаБлижайшегоПериода(ТекущаяДатаСеанса, Периодичность);
...
Функция ОтборДляЗаполненияПоказателей(ТипПлана, Параметры, ПараметрыВидаПлана) Экспорт
	...
	Если ПараметрыВидаПлана.Свойство("ПланированиеПоНазначениям") И ПараметрыВидаПлана.ПланированиеПоНазначениям Тогда
...
//
Функция ВычислитьПоФормуле(Знач ИсходнаяФормула, Знач ТекущиеДанные, Знач Периоды, Знач Период, Знач Представление = Неопределено) Экспорт
	
	РасчетнаяФормула = ИсходнаяФормула;
	ВыводитьПромежуточныеВычисления = Ложь;
	Постфикс = Неопределено;
	
	ПараметрыИсполняемойФункции = Новый Структура();
	ПараметрыИсполняемойФункции.Вставить("Период", Период);
	ПараметрыИсполняемойФункции.Вставить("Периоды", Периоды);
	ПараметрыИсполняемойФункции.Вставить("ТекущиеДанные", ТекущиеДанные);
	...
	РасчетнаяФормула = ДобавитьПараметрыКФункциям(РасчетнаяФормула, "ПараметрыИсполняемойФункции");
	...
	РасчетнаяФормула = УдалитьПараметрыВФункциях(РасчетнаяФормула, "ПараметрыИсполняемойФункции");
			...
			Если ТекущийЭлементПолеФормы.Имя = Префикс + Период.ИмяКолонки
				...
				Для каждого ДопРеквизит Из ДопРеквизиты Цикл
					...
					Если ДопРеквизит.Имя = "ФормулаВычисление" И ДопРеквизит.СоздаватьРеквизит 
						...
						Форма.Элементы[ДопРеквизит.ИмяЭлемента].Гиперссылка = Истина;
					КонецЕсли;
	...
	Если НЕ ПериодНайден Тогда
		...
		Для каждого ДопРеквизит Из ДопРеквизиты Цикл
			Если ДопРеквизит.Имя = "ФормулаВычисление" И ДопРеквизит.СоздаватьРеквизит Тогда
				...
				Форма.Элементы[ДопРеквизит.ИмяЭлемента].Гиперссылка = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Функция ДобавитьПараметрыКФункциям(РасчетнаяФормула, Параметры)
	
	МассивФункций = ФункцииФормулы();
	
	Для каждого ТекущаяФункция Из МассивФункций Цикл
		РасчетнаяФормула = СтрЗаменить(РасчетнаяФормула, ТекущаяФункция, ТекущаяФункция + Параметры + ",");
	КонецЦикла;
...
Функция УдалитьПараметрыВФункциях(РасчетнаяФормула, Параметры)
