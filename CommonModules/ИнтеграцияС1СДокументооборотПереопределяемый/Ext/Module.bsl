...
Процедура ЗаполнитьТаблицуМенеджеровПечатиОбъекта(ИмяМенеджера, ПечатныеФормы) Экспорт
	...
	КомандыПечати = ИнтеграцияС1СДокументооборот.КомандыПечатиФормы(ИмяМенеджера + ".Форма.ФормаСписка");
...
Функция ПодходящийТип(ТипОбъектаXDTO, ТипыКандидаты) Экспорт
	...
	Если ТипОбъектаXDTO = "DMCorrespondent" и ТипыКандидаты.Найти(Тип("СправочникСсылка.Контрагенты")) <> Неопределено Тогда
		Возврат Тип("СправочникСсылка.Контрагенты");
	КонецЕсли;
	
	Если ТипОбъектаXDTO = "DMOrganization" и ТипыКандидаты.Найти(Тип("СправочникСсылка.Организации")) <> Неопределено Тогда
...
Функция ПредельноеКоличествоВыбираемыхОбъектов(Тип) Экспорт
	...
	Возврат 23; // Если Тип = "DMCorrespondent" Тогда Возврат 50 ИначеЕсли <...>
...
Процедура ПриДобавленииПодсистемыВДеревоИнтегрируемыхОбъектов(Подсистема, Отказ) Экспорт
	...
	Если Подсистема.Имя = "ПродажиБазовая"
		Или Подсистема.Имя = "ЗакупкиБазовая" Тогда
...
// Пример реализации:
//
//	Если Правило.ТипОбъектаПотребителя = "Документ.ПриобретениеТоваровУслуг" Тогда 
...
//			Если ВидОперации = Перечисления.ВидыОперацийПриобретениеТоваровУслуг.Товары Тогда
//				ПредставлениеОбъектаПотребителя = НСтр("ru = 'Приобретение товаров'");
//			ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПриобретениеТоваровУслуг.Услуги Тогда
//				ПредставлениеОбъектаПотребителя = НСтр("ru = 'Приобретение услуг'");
//			КонецЕсли;
//		КонецЕсли;
//	КонецЕсли;
//
