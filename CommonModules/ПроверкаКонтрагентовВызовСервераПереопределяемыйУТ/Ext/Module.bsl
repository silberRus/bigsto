...
//
// Параметры:
//  ДокументСсылка  - ДокументСсылка - документ, чьи счета-фактуры будут получены.
//
// Возвращаемое значение:
//   ТаблицаЗначений, Неопределено   - таблица, содержащая данные о счет-фактурах, выписанных на основании документа.
//
Функция ПолучитьКонтрагентовСчетФактурДокумента(ДокументСсылка) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.СчетФактураПолученный) Тогда
		...
		|	КОНЕЦ КАК Дата,
		|	ЛОЖЬ КАК ЕстьОшибки,
		|	ТИПЗНАЧЕНИЯ(СчетФактураПолученный.Ссылка) КАК Поле3
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.СчетФактураПолученныйНалоговыйАгент) Тогда
		...
		|;
		|";
		ТекстОбъединенияЗапросаСчетФактураПолученныйНА = "ВЫБРАТЬ РАЗЛИЧНЫЕ * ИЗ СчетФактураПолученныйВременная";
...
//
// Параметры:
//  Форма  - УправляемаяФорма - форма, для которой дополняются действия при создании на сервере.
//
...
// Управление видимостью элементов, относящихся к проверке контрагентов в формах справочника "Контрагенты" при создании на сервере.
// Параметры:
