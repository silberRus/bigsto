...
Функция КоэффициентыРаспределения(ТабличнаяЧасть, Параметры) Экспорт
	...
	КонецЦикла;
	ПараметрыРасчета.Вставить("РасчитыватьНаборы", Ложь);
...
//
// Возвращаемое значение:
//  Булево - облать набора должна быть использована в печатной форме.
//
...
Функция СоздатьТаблицуСоставаНабора(КолонкиНабора) Экспорт
	...
	Если КолонкиНабора.Найти("Цена") <> Неопределено Тогда
		Комплектующие.Колонки.Добавить("Цена", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15,2));
	КонецЕсли;
	...
	Если КолонкиНабора.Найти("СуммаРучнойСкидки") <> Неопределено Тогда
		Комплектующие.Колонки.Добавить("СуммаРучнойСкидки", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15,2));
	КонецЕсли;
	...
	Если КолонкиНабора.Найти("СуммаАвтоматическойСкидки") <> Неопределено Тогда
		Комплектующие.Колонки.Добавить("СуммаАвтоматическойСкидки", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15,2));
	КонецЕсли;
	...
	Если КолонкиНабора.Найти("СуммаНДС") <> Неопределено Тогда
		Комплектующие.Колонки.Добавить("СуммаНДС", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15,2));
	КонецЕсли;

	Если КолонкиНабора.Найти("СуммаСНДС") <> Неопределено Тогда
		Комплектующие.Колонки.Добавить("СуммаСНДС", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15,2));
	КонецЕсли;

	Если КолонкиНабора.Найти("Сумма") <> Неопределено Тогда
		Комплектующие.Колонки.Добавить("Сумма", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15,2));
	КонецЕсли;
	...
	Если КолонкиНабора.Найти("ДокументРеализации") <> Неопределено Тогда
		Комплектующие.Колонки.Добавить("ДокументРеализации", Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг, ДокументСсылка.ОтчетОРозничныхПродажах"));
	КонецЕсли;
