...
Функция АдресаПоПочтовомуИндексуКлассификатора(Индекс, ДополнительныеПараметры) Экспорт
	...
	Если Вариант = "КЛАДР" Тогда
		Уровни = АдресныйКлассификаторКлиентСервер.УровниКлассификатораКЛАДР();
	ИначеЕсли Вариант = "ФИАС" Тогда
		Уровни = АдресныйКлассификаторКлиентСервер.УровниКлассификатораФИАС();
	Иначе
		Возврат Результат;
...
Функция ВариантыАвтоподбора(Текст, Родитель, Уровни, ДополнительныеПараметры) Экспорт 
	...
	Вариант = ДополнительныеПараметры.ФорматАдреса;
	Если Вариант = "КЛАДР" Тогда
		ОграничениеУровней = АдресныйКлассификаторКлиентСервер.УровниКлассификатораКЛАДР();
		
	ИначеЕсли Вариант = "ФИАС" Тогда
		ОграничениеУровней = АдресныйКлассификаторКлиентСервер.УровниКлассификатораФИАС();
...
Функция СписокДомов(ИдентификаторАдресногоОбъекта, Знач СтрокаПоиска) Экспорт
	...
	ВариантыДомов.Колонки.Добавить("ДомЧислом", ОбщегоНазначения.ОписаниеТипаЧисло(5));
...
Процедура УстановитьУровниУлицыПоЧастямАдреса(ИдентификаторНаселенногоПункта, ЧастиАдреса) Экспорт
	...
	Для Индекс = 0 По ЧастиАдреса.Количество() -1 Цикл
		...
		Упорядочить = Упорядочить + РазделительВыбрать + НазваниеТаблицы + ".Актуален Убыв";
		РазделительВыбрать = ", ";
	КонецЦикла;
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры)
	...
	ТекстЗапроса =  ТекстЗапросаПодходящиеОбъекты + "
		...
		|	АдресныйОбъект.Уровень       КАК Уровень,
		|
	...
	Для Каждого Строка Из Запрос.Выполнить().Выгрузить() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаРезультата, Строка, "Идентификатор, РегионЗагружен, Неактуален, Уровень");
...
Процедура ЗаполнитьАдресаДляИнтерактивногоВыбораВнутр(Результат, Уровни, Родитель, Уровень, ДополнительныеПараметры) 
	...
	Иначе
		...
		ТекстЗапроса = ТекстЗапроса + "
			...
			|УПОРЯДОЧИТЬ ПО
			|	
			|	Представление " + ТекстСортировкиПорции + ", АдресныйОбъект.Актуален УБЫВ
			|";
	КонецЕсли;
...
Процедура УстановитьКодКЛАДР(КодыКЛАДР, ИмяПоля, Значение, КодКЛАДР)
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаСервис1С(Результат, Текст, Родитель, Уровни, ДополнительныеПараметры)
	...
	Список = Порция.ПолучитьСписок("Item");
	ТребуетсяПоискПоДополнительнымТерритория = Уровни.Найти(90) <> Неопределено;
	
	Для Каждого Строка Из Список Цикл
		СтрокаРезультата = Данные.Добавить();
		СтрокаРезультата.РегионЗагружен = Истина;
		Если ТребуетсяПоискПоДополнительнымТерритория Тогда
			ДанныеПоСтроке = Сервис.GetActualInfo(Строка.ID, КодЯзыка, Метаданные.Имя);
			Если ДанныеПоСтроке.ДопАдрЭл.Количество() > 0 Тогда
				Для каждого Вариант Из ДанныеПоСтроке.ДопАдрЭл Цикл
					Если СтрСравнить(Вариант.Значение, Строка.Presentation) = 0 Тогда
						СтрокаРезультата.Уровень = 90;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Функция ИдентификаторЭлементаАдресаСервис1С(Наименование, Сокращение, ИдентификаторРодитель, Уровень)
	...
	Если СтрДлина(Наименование) < 3 Тогда
		Возврат Неопределено; // Autocomplete в веб-сервисе принимает строки с длинной 3 и более
	КонецЕсли;
...
Процедура ОпределитьИдентификаторыАдресаСервис1С(Результат, АдресДляПроверки)
	...
	ПроверяемыйАдрес =  СписокДляПроверки.Item.Добавить(Сервис.ФабрикаXDTO.Создать(ТипЭлементаСписка));
	ПроверяемыйАдрес.Levels  = АдресныйКлассификаторКлиентСервер.УровниКлассификатораФИАС();
...
Процедура ЗаполнитьКодыПоИдентификаторуДома(ИдентификаторАдресногоОбъекта, СведенияОДоме, Коды)
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Для Каждого ДополнительныйЭлемент Из Адрес.ПолучитьСписок("ДопАдрЭл") Цикл
		...
		// У ФИАС некоторые сокращения ошибочно содержат в конце точку. Для поиска вариантов эту точку необходимо убрать.
		СокращениеВарианта = ?( СтрЗаканчиваетсяНа(Вариант.Сокращение, "."), Лев(Вариант.Сокращение, СтрДлина(Вариант.Сокращение) - 1), Вариант.Сокращение);
		// У ФИАС необходимо учитывать кв-л
		ДополнитьТаблицуСокращений(ТаблицаСокращенийДополнительных);
		
		СтрокиВариантов = ТаблицаСокращенийДополнительных.НайтиСтроки(Новый Структура("Сокращение, Значение, Идентификатор", СокращениеВарианта, ТипАдресногоЭлемента, 90));
		Если СтрокиВариантов.Количество() > 0 Тогда
			Код = СтрокиВариантов[0].Идентификатор;
			Дополнительный = Вариант;
		Иначе
			СтрокиВариантов = ТаблицаСокращенийДополнительных.НайтиСтроки(Новый Структура("Сокращение, Значение, Идентификатор", СокращениеВарианта, ТипАдресногоЭлемента, 91));
			Если СтрокиВариантов.Количество() > 0 Тогда
				Подчиненный = Вариант;
			КонецЕсли;
		КонецЕсли;
	...
	ТекстУпорядочить = "УПОРЯДОЧИТЬ ПО СубъектРФ.Актуален УБЫВ";
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстУпорядочить, ПроверятьОкруг,                "Округ");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстУпорядочить, ПроверятьРайон,                "Район");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстУпорядочить, ПроверятьГород,                "Город");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстУпорядочить, ПроверятьВнутригородскойРайон, "ВнутригородскойРайон");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстУпорядочить, ПроверятьНаселенныйПункт,      "НаселенныйПункт");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстУпорядочить, ПроверятьУлицу,                "Улица");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстУпорядочить, ПроверятьДополнительный,       "Дополнительный");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстУпорядочить, ПроверятьПодчиненный,          "Подчиненный");
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		Если ПроверятьОкруг И Не Запись.ОкругНайден И Запись.ОкругАктуален = Истина Тогда
		...
		Если ПроверятьРайон И Не Запись.РайонНайден И Запись.РайонАктуален = Истина Тогда
		...
		Если ПроверятьГород И Не Запись.ГородНайден И Запись.ГородАктуален = Истина Тогда
		...
		Если ПроверятьВнутригородскойРайон И Не Запись.ВнутригородскойРайонНайден И Запись.ВнутригородскойРайонАктуален = Истина Тогда
		...
		Если ПроверятьНаселенныйПункт И Не Запись.НаселенныйПунктНайден И Запись.НаселенныйПунктАктуален = Истина Тогда
		...
		Если ПроверятьУлицу И Запись.УлицаНайден И Запись.УлицаАктуален = Истина Тогда
			УлицаНайдена = Истина;
		КонецЕсли;
			
		Если ПроверятьДополнительный И Не Запись.ДополнительныйНайден И Запись.ДополнительныйАктуален = Истина Тогда
		...
		Если ПроверятьПодчиненный И Не Запись.ПодчиненныйНайден И Запись.ПодчиненныйАктуален = Истина Тогда
	...
	Если НЕ Уровни.Найти(90) = Неопределено Тогда
		Для Каждого Запись Из КорректныеЗаписи Цикл
			...
			Если ЗначениеЗаполнено(Запись.Строения) Тогда
				...
				КонецЕсли;
			Иначе
				ИндексКорректный = Истина; // Индекс на уровни домов доп. территории считаем корректным.
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ДополнитьТаблицуСокращений(ТаблицаСокращенийДополнительных)
	
	НоваяСтрока = ТаблицаСокращенийДополнительных.Вставить(0);
	НоваяСтрока.Идентификатор = 90;
	НоваяСтрока.Значение      = "10200000";
	НоваяСтрока.Сокращение   = "кв-л";
...
Процедура ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдреса(ТекстЗапроса, Флаг, ИмяПоля)
	Если Флаг Тогда
		ТекстЗапроса = ТекстЗапроса + ", " + Символы.ПС + ИмяПоля + ".Актуален УБЫВ";
	КонецЕсли;
КонецПроцедуры
...
Функция ТекстОпциональныхПолейЗапросаПроверкиАдреса(ФлагПроверки, ИмяПоля)
	...
	Если ФлагПроверки Тогда
		Вариант = "
			...
			|	[Поле].Актуален                      КАК [Поле]Актуален,
...
Функция ИдентификаторыАдресаПоКлассификатору(ПоляАдреса, ИскатьПоУлице, ВсеИдентификаторыАдреса = Ложь)
	...
	ТекстСоединения = "";
	ТекстУпорядочить = " УПОРЯДОЧИТЬ ПО АдресныйОбъектРегион.Актуален УБЫВ";
	...
	Иначе
		...
		|	И АдресныеОбъектыОкруг.КодПодчиненногоЭлемента = 0";
		сОкруг = "АдресныеОбъектыОкруг.КодОкруга";
		ТекстУпорядочить = ТекстУпорядочить + ", АдресныеОбъектыОкруг.Актуален УБЫВ";
		Если ВсеИдентификаторыАдреса Тогда
	...
	Иначе
		...
		|	И АдресныеОбъектыРайон.КодПодчиненногоЭлемента = 0";
		сРайон = "АдресныеОбъектыРайон.КодРайона";
		ТекстУпорядочить = ТекстУпорядочить + ", АдресныеОбъектыРайон.Актуален УБЫВ";
		Если ВсеИдентификаторыАдреса Тогда
	...
	Иначе
		...
		|	И АдресныеОбъектыГород.КодПодчиненногоЭлемента = 0";
		сГород = "АдресныеОбъектыГород.КодГорода";
		ТекстУпорядочить = ТекстУпорядочить + ", АдресныеОбъектыГород.Актуален УБЫВ";
		Если ВсеИдентификаторыАдреса Тогда
	...
	Иначе
		...
		|	И АдресныеОбъектыВнутригородскойРайон.КодПодчиненногоЭлемента = 0";
		сВнутригородскойРайон = "АдресныеОбъектыВнутригородскойРайон.КодВнутригородскогоРайона";
		ТекстУпорядочить = ТекстУпорядочить + ", АдресныеОбъектыВнутригородскойРайон.Актуален УБЫВ";
		Если ВсеИдентификаторыАдреса Тогда
	...
	Иначе
		...
		|	И АдресныеОбъектыНаселенныйПункт.КодПодчиненногоЭлемента = 0";
		сНаселенногоПункта = "АдресныеОбъектыНаселенныйПункт.КодНаселенногоПункта";
		ТекстУпорядочить = ТекстУпорядочить + ", АдресныеОбъектыНаселенныйПункт.Актуален УБЫВ";
		Если ВсеИдентификаторыАдреса Тогда
	...
	Если ИскатьПоУлице И НЕ ПустаяСтрока(ПоляАдреса["Улица"].Наименование) Тогда
		...
		|	И АдресныеОбъектыУлица.КодДополнительногоЭлемента = 0
		|	И АдресныеОбъектыУлица.КодПодчиненногоЭлемента = 0";
		ТекстУпорядочить = ТекстУпорядочить + ", АдресныеОбъектыУлица.Актуален УБЫВ";
		Если ВсеИдентификаторыАдреса Тогда
	...
	Пока Позиция <= ЧислоСтрок Цикл
		...
		Если Строка = "H" Тогда
			...
			Если СтрНайти(НомерСтроения, "+") > 0 Тогда
				НомерСтроения = СокрЛП(Лев(НомерСтроения, СтрНайти(НомерСтроения, "+") - 1)); // отрезаем идентификатор
			КонецЕсли;
...
Процедура ФоновоеЗаданиеЗагрузкиКлассификатораАдресовССайта(Параметры, АдресРезультата) Экспорт
	...
	ЗагрузитьКлассификаторАдресовССайта(Параметры[0]);
...
Функция ЗаполненныеРегионыУстаревшегоКлассификатора() Экспорт
	...
	Если Метаданные.РегистрыСведений.Найти("УдалитьАдресныйКлассификатор") <> Неопределено
		И ПравоДоступа("Изменение", Метаданные.РегистрыСведений.АдресныеОбъекты) Тогда
	...
	Для Каждого Строка Из Список Цикл
		...
		Для каждого Дом Из МассивДомов Цикл
			...
			Если ТипАдресногоОбъекта = НСтр("ru = 'ДОМ'")
				...
				ОписаниеДома.НазваниеВладения = ПерваяБукваЗаглавная(ТипАдресногоОбъекта);
...
Процедура ЗагрузитьКлассификаторАдресовССайта(КодыСубъектовРФ = Неопределено, ИсходнаяАвторизация = Неопределено)
	...
	Для Каждого КодСубъекта Из КодыСубъектовРФ Цикл
		...
		Если Не ЗагруженныйФайл.Статус Тогда
			УдалитьВременныйФайл(ЗагруженныйФайл.Путь);
...
Процедура ЗагрузитьДомаЗданияСтроения(КодСубъектаРФ, КаталогФайлов, ВидыСведений)
	...
	Если ФайлЗагрузки.РазмерФайла > 10485760 И СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86 Тогда
		ЗагрузитьДомаЗданияСтроенияПорциями(КодСубъектаРФ, ФайлЗагрузки);
	Иначе
		ЗагрузитьДомаЗданияСтроенияОднимНабором(КодСубъектаРФ, ФайлЗагрузки);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗагрузитьДомаЗданияСтроенияПорциями(КодСубъектаРФ, ФайлЗагрузки)
	
	ВременнаяТаблица = Новый ТаблицаЗначений;
	ВременнаяТаблица.Колонки.Добавить("КодСубъектаРФ", Новый ОписаниеТипов("Число"));
	ВременнаяТаблица.Колонки.Добавить("АдресныйОбъект", Новый ОписаниеТипов("УникальныйИдентификатор"));
	ВременнаяТаблица.Колонки.Добавить("ПочтовыйИндекс", Новый ОписаниеТипов("Число"));
	ВременнаяТаблица.Колонки.Добавить("Строения");
	ВременнаяТаблица.Индексы.Добавить("ПочтовыйИндекс");
	...
	Пока СледующаяЗаписьФайлаЗагрузкиСубъектаРФ(ФайлЗагрузки, ТекущаяЗапись) Цикл
		...
		Индексы.Вставить(ТекущаяЗапись.POSTALCODE, Истина);
		
		НоваяСтрока = ВременнаяТаблица.Добавить();
		НоваяСтрока.КодСубъектаРФ  = КодСубъектаРФ;
		НоваяСтрока.АдресныйОбъект = УникальныйИдентификаторИзДвоичныхДанных(ТекущаяЗапись.AOGUID);
		НоваяСтрока.ПочтовыйИндекс = ТекущаяЗапись.POSTALCODE;
		
		ОписаниеЗданийXDTO =  ТекущаяЗапись.ПолучитьXDTO("BUILDINGS");
		НоваяСтрока.Строения = Новый ХранилищеЗначения(ОписаниеЗданийXDTO.ЛексическоеЗначение, Сжатие);
		
		ЗаполнитьЗначенияСвойств(Таблица.Добавить(), НоваяСтрока);
	КонецЦикла;
	ЗакрытьФайлЗагрузкиСубъектаРФ(ФайлЗагрузки);
	...
	Для каждого Индекс Из Индексы Цикл
		...
		Отбор = Новый Структура("ПочтовыйИндекс", Индекс.Ключ);
		НайденныеСтроки = ВременнаяТаблица.НайтиСтроки(Отбор);
		
		Набор = РегистрыСведений.ДомаЗданияСтроения.СоздатьНаборЗаписей();
		Набор.Отбор.КодСубъектаРФ.Установить(КодСубъектаРФ);
		Набор.Отбор.ПочтовыйИндекс.Установить(Индекс.Ключ);
		
		Для каждого Строка Из НайденныеСтроки Цикл
			
			Запись = Набор.Добавить();
			Запись.КодСубъектаРФ  = КодСубъектаРФ;
			Запись.АдресныйОбъект = Строка.АдресныйОбъект;
			Запись.ПочтовыйИндекс = Строка.ПочтовыйИндекс;
			Запись.Строения = Строка.Строения;
...
Процедура ЗагрузитьДомаЗданияСтроенияОднимНабором(КодСубъектаРФ, ФайлЗагрузки)
...
Процедура ЗагрузитьИсториюАдресныхОбъектов(КодСубъектаРФ, КаталогФайлов, ВидыСведений)
	...
	Если ФайлЗагрузки.РазмерФайла > 10485760 И СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86 Тогда
		ЗагрузитьИсториюАдресныхОбъектовПорциями(КодСубъектаРФ, ФайлЗагрузки);
	Иначе
		ЗагрузитьИсториюАдресныхОбъектовОднимНабором(КодСубъектаРФ, ФайлЗагрузки);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗагрузитьИсториюАдресныхОбъектовПорциями(КодСубъектаРФ, ФайлЗагрузки)
	
	ВременнаяТаблица = Новый ТаблицаЗначений;
	ВременнаяТаблица.Колонки.Добавить("Уровень", ОбщегоНазначения.ОписаниеТипаЧисло(2));
	ВременнаяТаблица.Колонки.Добавить("КодСубъектаРФ", ОбщегоНазначения.ОписаниеТипаЧисло(2));
	ВременнаяТаблица.Колонки.Добавить("КодОкруга", ОбщегоНазначения.ОписаниеТипаЧисло(1));
	ВременнаяТаблица.Колонки.Добавить("КодРайона", ОбщегоНазначения.ОписаниеТипаЧисло(3));
	ВременнаяТаблица.Колонки.Добавить("КодГорода", ОбщегоНазначения.ОписаниеТипаЧисло(3));
	ВременнаяТаблица.Колонки.Добавить("КодВнутригородскогоРайона", ОбщегоНазначения.ОписаниеТипаЧисло(3));
	ВременнаяТаблица.Колонки.Добавить("КодНаселенногоПункта", ОбщегоНазначения.ОписаниеТипаЧисло(3));
	ВременнаяТаблица.Колонки.Добавить("КодУлицы", ОбщегоНазначения.ОписаниеТипаЧисло(4));
	ВременнаяТаблица.Колонки.Добавить("КодДополнительногоЭлемента", ОбщегоНазначения.ОписаниеТипаЧисло(4));
	ВременнаяТаблица.Колонки.Добавить("КодПодчиненногоЭлемента", ОбщегоНазначения.ОписаниеТипаЧисло(3));
	ВременнаяТаблица.Колонки.Добавить("ТекущийКодСубъектаРФ", ОбщегоНазначения.ОписаниеТипаЧисло(2));
	
	ВременнаяТаблица.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("УникальныйИдентификатор"));
	
	ВременнаяТаблица.Колонки.Добавить("ПочтовыйИндекс", Новый ОписаниеТипов("Число"));
	ВременнаяТаблица.Колонки.Добавить("Наименование", ОбщегоНазначения.ОписаниеТипаСтрока(120));
	ВременнаяТаблица.Колонки.Добавить("Сокращение", ОбщегоНазначения.ОписаниеТипаСтрока(10));
	ВременнаяТаблица.Колонки.Добавить("Дополнительно", Новый ОписаниеТипов("УникальныйИдентификатор"));
	ВременнаяТаблица.Колонки.Добавить("КодКЛАДР", ОбщегоНазначения.ОписаниеТипаЧисло(17));
	ВременнаяТаблица.Колонки.Добавить("АдресныйОбъект", Новый ОписаниеТипов("УникальныйИдентификатор"));
	ВременнаяТаблица.Колонки.Добавить("НачалоДействияЗаписи", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
	ВременнаяТаблица.Колонки.Добавить("ОкончаниеДействияЗаписи", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
	ВременнаяТаблица.Колонки.Добавить("Операция", ОбщегоНазначения.ОписаниеТипаЧисло(2));
	
	ВременнаяТаблица.Индексы.Добавить("КодРайона");
	
	КодыРайонов = Новый Соответствие;
	...
	Пока СледующаяЗаписьФайлаЗагрузкиСубъектаРФ(ФайлЗагрузки, ТекущаяЗапись) Цикл
		...
		НоваяСтрока.Уровень                    = ТекущаяЗапись.AOLEVEL;
		НоваяСтрока.КодСубъектаРФ              = ТекущаяЗапись.REGIONCODE;
		НоваяСтрока.КодОкруга                  = ТекущаяЗапись.AUTOCODE;
		НоваяСтрока.КодРайона                  = ТекущаяЗапись.AREACODE;
		НоваяСтрока.КодГорода                  = ТекущаяЗапись.CITYCODE;
		НоваяСтрока.КодВнутригородскогоРайона  = ТекущаяЗапись.CTARCODE;
		НоваяСтрока.КодНаселенногоПункта       = ТекущаяЗапись.PLACECODE;
		НоваяСтрока.КодУлицы                   = ТекущаяЗапись.STREETCODE;
		НоваяСтрока.КодДополнительногоЭлемента = ТекущаяЗапись.EXTRCODE;
		НоваяСтрока.КодПодчиненногоЭлемента    = ТекущаяЗапись.SEXTCODE;
		НоваяСтрока.Идентификатор              = УникальныйИдентификаторИзДвоичныхДанных(ТекущаяЗапись.AOID);
		НоваяСтрока.ТекущийКодСубъектаРФ       = КодСубъектаРФ;
		
		НоваяСтрока.ПочтовыйИндекс             = ТекущаяЗапись.POSTALCODE;
		НоваяСтрока.Наименование               = ТекущаяЗапись.FORMALNAME;
		НоваяСтрока.Сокращение                 = ТекущаяЗапись.SHORTNAME;
		НоваяСтрока.Дополнительно              = УникальныйИдентификаторИзДвоичныхДанных(ТекущаяЗапись.EXTRAGUID);
		НоваяСтрока.КодКЛАДР                   = ТекущаяЗапись.CODE;
		НоваяСтрока.АдресныйОбъект             = УникальныйИдентификаторИзДвоичныхДанных(ТекущаяЗапись.AOGUID);
		НоваяСтрока.НачалоДействияЗаписи       = ТекущаяЗапись.STARTDATE;
		НоваяСтрока.ОкончаниеДействияЗаписи    = ТекущаяЗапись.ENDDATE;
		НоваяСтрока.Операция                   = ТекущаяЗапись.OPERSTATUS;
		
		КодыРайонов.Вставить(НоваяСтрока.КодРайона, Истина);
		ТекущаяЗапись = Неопределено;
	КонецЦикла;
	...
	Для каждого КодРайона Из КодыРайонов Цикл
		Отбор = Новый Структура("КодРайона", КодРайона.Ключ);
		НайденныеСтроки = ВременнаяТаблица.НайтиСтроки(Отбор);
		Набор = РегистрыСведений.ИсторияАдресныхОбъектов.СоздатьНаборЗаписей();
		Набор.Отбор.КодСубъектаРФ.Установить(КодСубъектаРФ);
		Набор.Отбор.КодРайона.Установить(КодРайона.Ключ);
...
Процедура ЗагрузитьИсториюАдресныхОбъектовОднимНабором(Знач КодСубъектаРФ, Знач ФайлЗагрузки)
