...
Процедура ВидПланаПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Объект.Замещающий и Объект.Товары.Количество() = 0 Тогда
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	СтруктураПоиска = Новый Структура("ИдентификаторНоменклатура, ИдентификаторХарактеристика, ИдентификаторУпаковка, 
		|ИдентификаторВариантКомплектации, ИдентификаторСклад, ИдентификаторНазначение", 0, 0, 0, 0, 0, 0);
	...
	Пока ТаблицаИсходныеКомплектующие.Количество() <> 0 Цикл
		...
		Для каждого СтрокаПодсборка Из ТаблицаПодсборки Цикл
			...
			Если Объект.КроссТаблица Тогда
				...
				СтруктураПоиска = Новый Структура("КодСтрокиТовары,Номенклатура,Характеристика, Назначение", 
												СтрокаПодсборка.КодСтрокиТовары, СтрокаПодсборка.Номенклатура, СтрокаПодсборка.Характеристика, СтрокаПодсборка.Назначение);
...
Функция ПолучитьПараметрыВидаПлана(Знач ВидПлана, Знач ОбновлятьПравило, АдресПравилаЗаполнения, АдресПользовательскихНастроек)
	...
	Реквизиты = "КоличествоПериодов, ЗаполнятьСклад, ЗаполнятьСкладВТЧ, ЗапретитьРедактированиеПравила, Замещающий, ЗаполнятьПоФормуле, ЗаполнятьНазначениеВТЧ";
...
Процедура Подключаемый_УстановитьПроизвольнуюФормулу(Элемент, СтандартнаяОбработка)
	...
	Если Объект.КроссТаблица Тогда
		...
		Если АктивныеПериоды.Количество() > 1 Тогда
			...
			Если СтрНайти(ТекущийЭлементИмя, "ТоварыПоДатамКоличество_") = 0
				И НЕ Объект.ПланироватьПоСумме И СтрНайти(ТекущийЭлементИмя, "ТоварыПоДатамКартинкаКомментарий_") = 0 Тогда
				ПоказатьОповещениеПользователя(НСтр("ru = 'Активируйте колонку с периодом'")); 
				Возврат;
			Иначе
				Для каждого Период Из АктивныеПериоды Цикл
					Если ТекущийЭлементИмя = "ТоварыПоДатамКоличество_" + Период.ИмяКолонки 
						ИЛИ ТекущийЭлементИмя = "ТоварыПоДатамКартинкаКомментарий_" + Период.ИмяКолонки Тогда
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План сборки +15%'"), "[ПланыСборкиКомплекты] * 1.15");
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Максимальные сборки'"),
			"Макс(" +
...
Функция ПараметрыПреобразованияКроссТаблицы()
	...
	ПоляГруппировки = "Номенклатура,Характеристика,Назначение,Упаковка,Склад,ВариантКомплектации,
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьСклад, ПланированиеПоНазначениям");
...
Процедура ОбработатьРезультатЗаполненияПоФормуле()
	...
	Если Объект.КроссТаблица 
		...
		Для каждого Период Из ЭтаФорма.Периоды.НайтиСтроки(Новый Структура("Активная", Истина)) Цикл
			Для Каждого СтрокаТоваров Из ТоварыПоДатам Цикл
