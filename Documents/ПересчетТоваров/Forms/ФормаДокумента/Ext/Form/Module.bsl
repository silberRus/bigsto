...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// ( Storverk Тюняков 
	МодификацияКонфигурацииПереопределяемый.ПриСозданииНаСервере_МестаВПересчетТоваров(ЭтотОбъект);
	// ) Storverk Тюняков
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	//СтруктураДействий.Вставить("НоменклатураПриИзмененииПереопределяемый", Новый Структура("ИмяФормы, ИмяТабличнойЧасти",
	//	ЭтаФорма.ИмяФормы, "Товары"));
	СтруктураДействий.Вставить("НоменклатураПриИзмененииПереопределяемый", Новый Структура("ИмяФормы, ИмяТабличнойЧасти, Склад",
		ЭтаФорма.ИмяФормы, "Товары", Объект.Склад));	
	// ) Storverk Тюняков
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'ОграниченноеРедактирование'");
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СостояниеРедактирования");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'НеРедактируется'");
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СостояниеРедактирования");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'РедактированиеБезСтатусов'");
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре()
	...
	//Storverk Тюняков
	СТ_ОбщегоНазначения.ЗаполнитьМестаХраненияПоНоменклатуреВКоллекции(Объект.Товары, Новый Структура("Склад", Объект.Склад));
	//Storverk Тюняков
...
Процедура ЗаполнитьСервер()
	...
	// ( Storverk Тюняков 
	ВключитьОтборПоМестамХраненияНоменклатурыВСКД(ОтборПересчета);
	// ) Storverk Тюняков
...
//
Функция ВключитьОтборПоМестамХраненияНоменклатурыВСКД(ОбъектНастройки, ИспользоватьТолькоНастройки = Ложь) Экспорт
	
	ПараметрИспользуетсяОтборПоМестамХранения  = КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(
		ОбъектНастройки.Настройки, 
		"ИспользуетсяОтборПоМестамХранения");
	
	Если ПараметрИспользуетсяОтборПоМестамХранения = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПараметрИспользуетсяОтборПоМестамХранения.Значение      = Ложь;
	ПараметрИспользуетсяОтборПоМестамХранения.Использование = Ложь;
	...
	ЭлементыОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(НастройкиОсновнойСхемы.Отбор, "МестоХранения");
	ИспользуетсяОтборПоМестамХранения = Ложь;
	
	Для каждого ЭлементОтбора из ЭлементыОтбора Цикл
		Если ЭлементОтбора.Использование Тогда  
			ИспользуетсяОтборПоМестамХранения = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(ОбъектНастройки, "ИспользуетсяОтборПоМестамХранения", Истина, ИспользуетсяОтборПоМестамХранения);
	
	Возврат ИспользуетсяОтборПоМестамХранения;
