...
Процедура ЗаполнитьПоРеализацииТоваров(Знач Основание, ДанныеЗаполнения)
	...
	|	
	|	ВЫБОР КОГДА СУММА(Тара.СуммаСНДС) > ДанныеДокумента.СуммаПредоплаты ТОГДА
	|		ДанныеДокумента.СуммаПредоплаты
	|	ИНАЧЕ
	|		СУММА(Тара.СуммаСНДС)
	...
	|	ДанныеДокумента.Дата КАК ДатаПлатежа,
	|	
	|	(ДанныеДокумента.СуммаПредоплаты - ЕСТЬNULL(СУММА(Тара.СуммаСНДС), 0))
	|		/ (ДанныеДокумента.СуммаДокумента - ЕСТЬNULL(СУММА(Тара.СуммаСНДС), 0)) * 100 КАК ПроцентПлатежа,
	|	
	|	ДанныеДокумента.СуммаПредоплаты - ЕСТЬNULL(СУММА(Тара.СуммаСНДС), 0) КАК СуммаПлатежа,
	...
	|
	|ИМЕЮЩИЕ
	|	ЕСТЬNULL(СУММА(Тара.СуммаСНДС), 0) < ДанныеДокумента.СуммаПредоплаты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	0 КАК ПроцентПлатежа,
	|	
	|	СУММА(Тара.СуммаСНДС) - ДанныеДокумента.СуммаПредоплаты КАК СуммаПлатежа,
	|	
	|	ИСТИНА КАК ЭтоЗалогЗаТару
	|
	|ИЗ
	...
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка
	|	И ДанныеДокумента.СуммаДокумента - ДанныеДокумента.СуммаПредоплаты > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДокумента.Ссылка
	|
	|ИМЕЮЩИЕ
	|	СУММА(Тара.СуммаСНДС) > ДанныеДокумента.СуммаПредоплаты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ДанныеДокумента.ДатаПлатежа КАК ДатаПлатежа,
	|	
	|	ВЫБОР КОГДА ЕСТЬNULL(СУММА(Тара.СуммаСНДС), 0) < ДанныеДокумента.СуммаПредоплаты ТОГДА
	|		ДанныеДокумента.СуммаДокумента - ДанныеДокумента.СуммаПредоплаты
	|	ИНАЧЕ
	|		ДанныеДокумента.СуммаДокумента - ЕСТЬNULL(СУММА(Тара.СуммаСНДС), 0)
	|	КОНЕЦ
	|		/ (ДанныеДокумента.СуммаДокумента - ЕСТЬNULL(СУММА(Тара.СуммаСНДС), 0)) * 100 КАК ПроцентПлатежа,
	|	
	|	ВЫБОР КОГДА ЕСТЬNULL(СУММА(Тара.СуммаСНДС), 0) < ДанныеДокумента.СуммаПредоплаты ТОГДА
