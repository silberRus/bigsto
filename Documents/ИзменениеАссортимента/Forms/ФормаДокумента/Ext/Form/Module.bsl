...
Процедура ОткрытьПодбор(Команда)
	...
	Форма = ПолучитьФорму("Обработка.ПодборТоваровВДокументПродажи.Форма", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор);
	
	Если ИспользоватьУправлениеКоллекциями Тогда
		
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			Форма.СписокНоменклатура, "Номенклатура.КоллекцияНоменклатуры", Объект.КоллекцияНоменклатуры, 
...
Процедура ДобавитьТоварыПоОтбору(Команда)
	...
	Форма = ПолучитьФорму("Обработка.ПодборТоваровПоОтбору.Форма", ПараметрыФормы, ЭтаФорма);
	Если ИспользоватьУправлениеКоллекциями Тогда
		
		ЭлементОтбора = КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Форма.КомпоновщикНастроек, 
			"Номенклатура.КоллекцияНоменклатуры", 
			Объект.КоллекцияНоменклатуры,
	...
	Форма.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ДобавитьТоварыПоОтборуЗавершение", ЭтотОбъект);
...
Функция СформироватьТекстДляКоманды(ИмяКомандыЗаполнения,РежимЗаполнения)
	...
	Если ИмяКомандыЗаполнения="ЗаполнитьПоСкладскимОстаткамИОборотам" Тогда
		...
		|					Склады.Склад
		|				ИЗ
		|					Склады КАК Склады)"+?(РежимЗаполнения=2," И (НЕ Номенклатура В (&МассивТоваров))","")+") КАК ТоварыНаСкладахОстаткиИОбороты
		|ГДЕ
		|	ВЫБОР
