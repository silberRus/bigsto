...
Функция ПолучитьДополнительныеРеквизитыДляНормативов(ТоварнаяКатегория) Экспорт 
	...
	|ГДЕ
	|	НЕ Набор.ЭтоГруппа
	|	И Набор.Ссылка = ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.Справочник_Номенклатура_Общие)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ГДЕ
	|	НЕ Набор.ЭтоГруппа
	|	И Набор.Ссылка = ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.Справочник_ХарактеристикиНоменклатуры_Общие)
...
//  ТоварнаяКатегория  - СправочникСсылка.ТоварныеКатегории - Товарная категория в разрезе которой получаем свойства
//  Реквизит  - ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения, Строка - Реквизит, для которого заполняются значения норматива
...
// Параметры:
//  ТоварнаяКатегория  - СправочникСсылка.ТоварныеКатегории - Товарная категория в разрезе которой получаем свойства
//  Реквизит  - ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения, Строка - Реквизит, для которого определяется вариант
//
// Возвращаемое значение:
//   ВариантЗаполнения   - Число - Значение "0" для реквизита номенклатуры, "1" для доп.реквизита номенклатуры, "2" для доп.реквизита характеристики
//
Функция ВариантЗаполнения(ТоварнаяКатегория, Реквизит) Экспорт 
	...
	ИначеЕсли ЗначениеЗаполнено(Реквизит) И ТипЗнч(Реквизит) <> Тип("Строка") Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристикиНоменклатуры") Тогда
			...
			|				ИЛИ Набор.Ссылка = ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.Справочник_ХарактеристикиНоменклатуры_Общие))
			|ГДЕ
...
Процедура ПересчитатьПоказателиПоНормативамРаспределения(ТаблицаРезультата, ИменаПолейПоказателей, РаспределятьПоНоменклатуре = Истина) Экспорт 
	...
	Иначе
		ТаблицаИсходныхДанных = ТаблицаРезультата;
	КонецЕсли;
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "//%ПоляПоказателей%", ПоляПоказателей);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "//%МаксимумПолейПоказателей%", МаксимумПолейПоказателей);
	...
	ВыборкаТаблицаРезультата = РезультатыЗапроса[13].Выбрать();
	...
	Если РаспределятьПоНоменклатуре Тогда
		ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "//%ДополнительныеПоля%", ДополнительныеПоля);
	Иначе
		
		ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "//%ДополнительныеПоля%", ДополнительныеПоля + " " + ПоляПоказателей);
	...
	Пока ВыборкаТоварныеКатегории.Следующий() Цикл
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ТекстТестДоля%", ТекстТестДоля);
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ТекстОбщийПоРеквизитам%", ТекстОбщийПоРеквизитам);
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ТекстПоСвойствам%", ТекстПоСвойствам);
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ТекстПоСвойствамХарактеристик%", ТекстПоСвойствамХарактеристик);
...
//  Характеристика  - СправочникСсылка.ХарактеристикаНоменклатуры - Характеристика номенклатуры, для которой 
...
Функция ПолучитьНормативыРаспределения(НаДату, Номенклатура, Характеристика = Неопределено, НормативыПоНоменклатуре = Истина) Экспорт 
	...
	|			И ТаблицаРезультата.Номенклатура.%ИмяРеквизита% = НормативыПоРеквизитам.Значение
	|ГДЕ
	|	ТаблицаРезультата.Реквизит = &Реквизит%НомерНорматива%";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ТекстЗапросаПоРеквизитам = СтрЗаменить(ШаблонЗапросаПоРеквизитам, "%НомерНорматива%", НомерНорматива);
		ТекстЗапросаПоРеквизитам = СтрЗаменить(ТекстЗапросаПоРеквизитам, "%ИмяРеквизита%", ВыборкаДетальныеЗаписи.Реквизит);
...
Процедура СопоставитьЗагружаемыеДанные(АдресЗагружаемыхДанных, АдресТаблицыСопоставления, СписокНеоднозначностей, ПолноеИмяТабличнойЧасти, ДополнительныеПараметры) Экспорт
	...
	Нормативы =  ПолучитьИзВременногоХранилища(АдресТаблицыСопоставления);
