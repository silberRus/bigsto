...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	// silber {
	Если ТипДанныхЗаполнения = Тип("ДокументСсылка.КП") Тогда
		Вариант = АТ_ОбщегоНазначения.Параметр(ДополнительныеСвойства, "ВариантОбеспечения", Перечисления.ВариантыОбеспечения.Обособленно);
		Для Каждого Строка Из Товары Цикл
			Строка.ВариантОбеспечения = Вариант;
		КонецЦикла;
	КонецЕсли;
	// } silber
...
// silber {
Процедура ЗаполнитьДокументНаОснованииКП(Знач Основание)
	
	//Заполнение шапки
	
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 * ИЗ Документ.КП Док ГДЕ Ссылка = &Ссылка");
	
	Запрос.УстановитьПараметр("Ссылка", 		Основание);
	Запрос.УстановитьПараметр("СПокупателем", 	Перечисления.ТипыДоговоров.СПокупателем);
	...
	Если Договор.Пустая() Тогда
		Договор				= АТ_Сервер.ПолучитьДействующийДоговор(Партнер, Новый Структура("ТипДоговора", Перечисления.ТипыДоговоров.СПокупателем));
	...
	Статус 					= Перечисления.СтатусыЗаказовКлиентов.КОбеспечению;
	ГруппаФинансовогоУчета 	= Соглашение.ГруппаФинансовогоУчета;
	ПорядокОплаты			= Соглашение.ПорядокОплаты;
	
	//Заполнение табличной части.
		
	Если Основание.Склад.Пустая() Тогда
		текТовары = Документы.КП.ПолучитьТаблицуТоваровПоДокументам(Основание, Новый Структура("ВидДокумента, Склад", "ЗаказКлиента", Склад));
	Иначе
		Если Склад.Пустая() Тогда
			Склад = Основание.Склад;
		КонецЕсли;
		текТовары = Документы.КП.ПолучитьТаблицуТоваровПоДокументам(Основание);
	КонецЕсли;

	ВариантОбеспечения = ?(ДополнительныеСвойства.Свойство("ВариантОбеспечения"), 
			ДополнительныеСвойства.ВариантОбеспечения,
	...
	текТовары.ЗаполнитьЗначения(ВариантОбеспечения, "ВариантОбеспечения");
	
	Товары.Загрузить(текТовары);
...
Процедура ЗаполнитьДокументНаОснованииКП_ст(Знач Основание)
	
	//// перед этиим Склад должен быть заполнен!
	//
	//Запрос = Новый Запрос(
	//	"ВЫБРАТЬ
	...
	//	|	Соглашение.ПорядокОплаты КАК ПорядокОплаты,
	//	|	Соглашение.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	//	|	ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	//	|	НалогообложениеНДС КАК НалогообложениеНДС,
	//	|	ФормаОплаты КАК ФормаОплаты,
	//	|	ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	//	|	Статус КАК СтатусДокумента,
	//	|	КартаЛояльности КАК КартаЛояльности,
	//	|	СостояниеЗаполненияМногооборотнойТары КАК СостояниеЗаполненияМногооборотнойТары,
	//	|	ВернутьМногооборотнуюТару КАК ВернутьМногооборотнуюТару,
	//	|	СрокВозвратаМногооборотнойТары КАК СрокВозвратаМногооборотнойТары,
	//	|	ТребуетсяЗалогЗаТару КАК ТребуетсяЗалогЗаТару,
	//	|	(НЕ Проведен) КАК ЕстьОшибкиПроведен,
	//	//|	ВЫБОР
	//	//|		КОГДА Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыКП.Создание)
	//	//|			ТОГДА ЛОЖЬ
	//	//|		ИНАЧЕ ИСТИНА
	//	//|	КОНЕЦ КАК ЕстьОшибкиСтатус,
	...
	//	|					ИЛИ Склад.ЭтоГруппа
	//	|				ТОГДА ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	//	|			КОГДА Товары.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка) ТОГДА
	//	|				Ссылка.Склад
	//	|			ИНАЧЕ Товары.Склад
	//	|		КОНЕЦ КАК Склад
	//	|	) КАК Товары,
	...
	//
	//МассивДопустимыхСтатусов = Новый Массив();
	////МассивДопустимыхСтатусов.Добавить(Перечисления.СтатусыКП.Создание);
	//
	//ОбщегоНазначенияУТ.ПроверитьВозможностьВводаНаОсновании(
	//	Выборка.ДокументОснование,
	//	Выборка.СтатусДокумента,
	...
	//Если Основание.Склад.Пустая() Тогда
	//	
	//	ТаблицаТовары = КонвертацияТипов.ПолучитьТаблицуИзНайденныхСтрокТаблицыЗначений(Выборка.Товары.Выгрузить(), Новый Структура("Склад", Склад));
	//	
	//Иначе
	//	
	//	Если Склад.Пустая() Тогда
