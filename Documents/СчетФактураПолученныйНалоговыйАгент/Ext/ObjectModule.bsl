...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ВзаиморасчетыСервер.ЗаполнитьИдентификаторыСтрокВТабличнойЧасти(ДокументыОснования);
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.СчетФактураПолученныйНалоговыйАгент.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		ИначеЕсли ДанныеЗаполнения.Свойство("Корректировочный") И ДанныеЗаполнения.Корректировочный
			...
			И ТипЗнч(ДанныеЗаполнения.ДокументОснование) <> Тип("ДокументСсылка.КорректировкаПриобретения") 
			И ТипЗнч(ДанныеЗаполнения.ДокументОснование) <> Тип("Массив") Тогда
			
			Если ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("ДокументСсылка.СчетФактураПолученныйНалоговыйАгент") Тогда
...
Процедура ЗаполнитьПоДокументуОснованию(ДанныеЗаполнения)
	...
	Если ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") Тогда
...
Процедура ЗаполнитьКорректировкуПоСчетуФактуре(ДанныеЗаполнения)
	...
	РеквизитыДляОбработки = Документы.СчетФактураПолученныйНалоговыйАгент.ПараметрыЗаполненияПоСчетуФактуре(
...
Процедура ЗаполнитьКорректировкуПоДокументуОснованию(ДанныеЗаполнения)
	...
	ТаблицаОснований = Документы.СчетФактураПолученныйНалоговыйАгент.КорректировочныеДокументыПоОснованию(
		ДанныеЗаполнения.ДокументОснование);
...
Функция ПолучитьПараметрыСчетаФактурыПоОснованиям()
	...
	Колонки.Добавить("НалоговаяБаза",                      ОбщегоНазначения.ОписаниеТипаЧисло(14,2));
	Колонки.Добавить("СтавкаНДС");
	Колонки.Добавить("СуммаНДС",                           ОбщегоНазначения.ОписаниеТипаЧисло(14,2));
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		Если ИмяОбъекта = "ПередачаТоваровМеждуОрганизациями" Тогда
		...
		ИначеЕсли Не ОбщегоНазначения.ЕстьРеквизитОбъекта("Грузоотправитель", МетаданныеДокумента) Тогда // возврат от клиента
...
Процедура ЗаполнитьПоказателиНачисленияНДС(ДанныеЗаполнения) Экспорт
	...
	|	СУММА(ПередачаТоваровМеждуОрганизациями.СуммаДокумента) КАК Сумма,
	|	ПередачаТоваровМеждуОрганизациями.Валюта КАК ВалютаДокумента
	|ПОМЕСТИТЬ СуммыОснований
	|ИЗ
	|	Документ.ПередачаТоваровМеждуОрганизациями КАК ПередачаТоваровМеждуОрганизациями
	|ГДЕ
	|	ПередачаТоваровМеждуОрганизациями.Ссылка В(&ДокументыОснования)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СУММА(ПриобретениеТоваровУслуг.СуммаДокумента),
	|	ПриобретениеТоваровУслуг.Валюта
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг КАК ПриобретениеТоваровУслуг
	|ГДЕ
	|	ПриобретениеТоваровУслуг.Ссылка В(&ДокументыОснования)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПриобретениеТоваровУслуг.Ссылка,
	|	ПриобретениеТоваровУслуг.Дата,
	...
	|	КорректировкаПриобретенияРасхождения.Ссылка.Дата,
	|	СУММА(КорректировкаПриобретенияРасхождения.Сумма),
	|	КорректировкаПриобретенияРасхождения.Ссылка.Валюта
	|ИЗ
	|	Документ.КорректировкаПриобретения.Расхождения КАК КорректировкаПриобретенияРасхождения
	|ГДЕ
	|	КорректировкаПриобретенияРасхождения.Ссылка В(&ДокументыОснования)
	|	И КорректировкаПриобретенияРасхождения.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок)
	|
	|СГРУППИРОВАТЬ ПО
	|	КорректировкаПриобретенияРасхождения.Ссылка,
	|	КорректировкаПриобретенияРасхождения.Ссылка.Дата,
	...
	|ГДЕ
	|	КорректировкаПриобретенияРасхождения.Ссылка В(&ДокументыОснования)
	|	И КорректировкаПриобретенияРасхождения.Сумма > 0
	|	И КорректировкаПриобретенияРасхождения.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|
	|СГРУППИРОВАТЬ ПО
	|	КорректировкаПриобретенияРасхождения.Ссылка,
	|	КорректировкаПриобретенияРасхождения.Ссылка.Дата,
	...
	|	0 КАК СуммаНДС,
	|	СуммыОснований.ВалютаДокумента КАК Валюта,
	|	СуммыОснований.Дата КАК Дата
	|ИЗ
	|	(ВЫБРАТЬ
	...
	КурсРегл = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаРегламентированногоУчета, ТекущаяДатаСеанса());
	
	Для Каждого СтрокаОснования Из ТаблицаОснований Цикл
		
		Если СтрокаОснования.Валюта = ВалютаРегламентированногоУчета Тогда
			СуммаРегл = СтрокаОснования.Сумма;
		Иначе
			СуммаРегл = 
				РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(СтрокаОснования.Сумма,
																	РаботаСКурсамиВалют.ПолучитьКурсВалюты(СтрокаОснования.Валюта, СтрокаОснования.Дата),
																	КурсРегл);
		КонецЕсли;
		
		СтрокаОснования.НалоговаяБаза = СуммаРегл * 1.18;
		СтрокаОснования.СуммаНДС = СтрокаОснования.НалоговаяБаза * 18 / 118;
		
	КонецЦикла;
	
	ДокументыОснования.Загрузить(ТаблицаОснований);
