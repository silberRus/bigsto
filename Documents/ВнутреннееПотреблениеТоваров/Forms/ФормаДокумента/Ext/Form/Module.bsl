...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПроверитьЧтоВсеСтатьиРаспределяются(ЭтаФорма);
	...
	ПланыВидовХарактеристик.СтатьиРасходов.ЗаполнитьПризнакРаспределениеНДС(Объект.Товары);
...
Процедура ТекстУведомленияОСписанииГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИнтеграцияГИСМКлиентУТ.ТекстУведомленияОСписанииКиЗГИСМОбработкаНавигационнойСсылки(ЭтотОбъект, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ВопросПользователюПриИзмененииРеквизитаСтроки(
		Новый ОписаниеОповещения("ТоварыНоменклатураПриИзмененииВопросПользователюЗавершение", ЭтотОбъект),
...
Процедура ТоварыХарактеристикаПриИзменении(Элемент)
	...
	ВопросПользователюПриИзмененииРеквизитаСтроки(
		Новый ОписаниеОповещения("ТоварыХарактеристикаПриИзмененииЗавершение", ЭтотОбъект),
		Элементы.Товары.ТекущиеДанные);
...
Процедура ТоварыСтатьяРасходовПриИзмененииСервер(КэшированныеЗначения)
	...
	Если ТипЗнч(СтрокаТаблицы.СтатьяРасходов) = Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.АналитикаРасходов) Тогда
			...
			СтрокаТаблицы.РаспределениеНДС = Ложь;
		КонецЕсли;
	Иначе
	...
	СтруктураДействий = Новый Структура("
		...
		|ЗаполнитьПризнакРаспределениеНДС,
...
Процедура ДатаПриИзмененииСервер()
	...
	АктуализироватьПотреблениеПодДеятельность(Истина);
...
Функция ТоварыНакладной()
	...
	Колонки = "КоличествоУпаковок, Количество, НомерСтроки, Номенклатура, Характеристика,
	          |Назначение, Серия, КодСтроки, Упаковка, ЗаказНаВнутреннееПотребление,
	          |СтатьяРасходов, АналитикаРасходов, ФизическоеЛицо, КатегорияЭксплуатации, ГруппаПродукции, СтатусУказанияСерий";
	...
	Для Каждого Строка Из Таблица Цикл
		Если Не ЗначениеЗаполнено(Строка.ЗаказНаВнутреннееПотребление) Тогда
			Строка.ЗаказНаВнутреннееПотребление = Объект.Ссылка;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПризнакРаспределениеНДС");
...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	Документы.ВнутреннееПотреблениеТоваров.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладная, Объект.Ссылка, ПараметрыЗаполнения);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПроверитьЧтоВсеСтатьиРаспределяются(Форма)
	
	Форма.ВсеСтатьиРаспределяются = 
		Форма.Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию")
		И Форма.УчетНДСПоФактИспользованию
		И (Форма.Объект.Товары.НайтиСтроки(Новый Структура("РаспределениеНДС", Ложь)).Количество() = 0);
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ПотреблениеДляДеятельности.Имя);
	
	ГруппаОтбора1 = Элемент.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаОтбора1.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
	
	ОтборЭлемента = ГруппаОтбора1.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПотреблениеДляДеятельности");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
...
Процедура УстановитьПараметрыВыбораСтатьи()
	...
	ВозможноУказаниеСчетаРасходов = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию);
	Элементы.ТоварыСчетРеглУчетаРасходов.Видимость = ВозможноУказаниеСчетаРасходов;
...
Процедура СформироватьТекстУведомлениеОСписанииКиЗГИСМСервер()
	...
	ИнтеграцияГИСМ_УТ.СформироватьТекстУведомлениеОСписанииКиЗ(ЭтаФорма);
