...
Процедура ПровестиПоРегистрам(Отказ)
	...
	|	ЕСТЬNULL(ТК, ДокументОснование.ЗаказКлиента.ПеревозчикПартнер) ТК,
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	(
	|		ВЫБРАТЬ Объект Заказ, МАКСИМУМ(Перевозчик) ТК, МАКСИМУМ(ЗаСчетКлиента) ЗаСчетКлиента
	|		ИЗ 		РегистрСведений.АТ_МаршрутДоставки
	|		СГРУППИРОВАТЬ ПО Объект
	|	) Маршруты
	|ПО
	|	ЕСТЬNULL(ДокументОснование.ЗаказКлиента, ДокументОснование.ЗаказНаПеремещение) = Маршруты.Заказ
	|	
	|
	|ГДЕ
	|	Ссылка = &Ссылка
