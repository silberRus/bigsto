...
Процедура ТекстДокументИБНажатие()
	...
	Если КоличествоДокументов = 1 Тогда
		...
		Если Объект.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.КаталогТоваров") Тогда
			ОбменСКонтрагентамиКлиент.ПерезаполнитьДокумент(МассивДокументов[0], , , Объект.Ссылка);
		Иначе
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	|СтатусЭД,НаправлениеЭД,ТипЭлементаВерсииЭД,ДополнительнаяИнформация,Расширение");
	...
	Если ПараметрыВыводаПредупреждения = Неопределено Тогда
		Если НРег(СвойстваФайлаЭД.Расширение) = "xml" Тогда
	...
	Если СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Элементы.КомандаУтвердить.Видимость = Объект.СостояниеЭДО = Перечисления.СостоянияВерсийЭД.НаУтверждении
			И НЕ СтатусЭД = Перечисления.СтатусыЭД.Утвержден;
		...
		КомандаПодписиОтправки.Видимость = (ЕстьВозможностьПодписания И ТребуетсяПодпись(Объект.Ссылка) 
