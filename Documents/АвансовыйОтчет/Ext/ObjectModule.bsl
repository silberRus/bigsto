...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ПрочиеРасходы.Количество() И Не ОплатаПоставщикам.Количество() И Не КонвертацияВалюты.Количество() Тогда
		Текст = НСтр("ru = 'Сумма израсходованных денежных средств равна 0. Необходимо заполнить данные на закладках ""Расходы "" или ""Оплата поставщикам"" или ""Конвертация валюты""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
			ЭтотОбъект,
			"ИтогоИзрасходовано",
	...
	Для ТекИндекс = 0 По ПрочиеРасходы.Количество()-1 Цикл
		...
		АдресОшибки = НСтр("ru='в строке %НомерСтроки% списка ""Расходы""'");
		...
		Если Не ПрочиеРасходы[ОшибкаЗаполненияАналитики.НомерСтроки].Отменено Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					ОшибкаЗаполненияАналитики.НомерСтроки + 1) + " " + НСтр("ru='списка ""Расходы""'"),
				ЭтотОбъект,
	...
	Для каждого СтрокаКонвертации Из КонвертацияВалюты Цикл
		Если СтрокаКонвертации.Валюта = СтрокаКонвертации.ВалютаКонвертации Тогда
			...
			АдресОшибки = НСтр("ru='в строке %НомерСтроки% списка ""Конвертация валюты""'");
			АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", СтрокаКонвертации.НомерСтроки);
