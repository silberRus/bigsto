...
Процедура ДетализацияПартийПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		...
		ТекущиеДанные.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС18");
	КонецЕсли;
...
&НаКлиенте
Процедура РассчитатьНДСВСтроке()
	
	ТекущаяСтрока = Элементы.ДетализацияПартий.ТекущиеДанные;
	
	СтруктураДействий = Новый Структура;
	СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПересчетаСуммыНДСВСтрокеТЧ(Новый Структура("ЦенаВключаетНДС", Ложь));
	...
	Если ВалютыУпрИРеглУчетаСовпадают Тогда
		ТекущаяСтрока.НДСУпр = ТекущаяСтрока.НДСРегл;
	КонецЕсли;
