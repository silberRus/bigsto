...
Процедура ПриОткрытии(Отказ)
	...
	Если ОткрытьПодборПриОткрытии Тогда
		ПодключитьОбработчикОжидания("ОткрытьПодборАвтоматически", 0.1, Истина);
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ПодборОткрыт Тогда
		Оповестить("ЗакрытьСуперПодбор", Новый Структура("КП", Объект.Ссылка), Элементы.Товары);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОткрытьПодборПриОткрытии = Параметры.ОткрытьПодборПриОткрытии;
	...
	ТекущаяСтраница = "СтраницаОсновное";
	
	Если Объект.ДатаОтгрузки = '00010101' И Объект.Ссылка.Пустая() Тогда
	...
	ФормаОплатыДоИзменения = Объект.ФормаОплаты;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.КП.Форма.РеквизитыПечати" Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "СозданыРеализацииИзКП" Тогда
		Прочитать();
		прочитатьДанныеПоКП();
	ИначеЕсли ИмяСобытия = "РаспечатанПакетДокументов" И Источник.Найти(Объект.Ссылка) <> Неопределено Тогда
		Закрыть();
	КонецЕсли;
...
Процедура ФормаОплатыПриИзменении(Элемент)
	...
	Если ФормаОплатыДоИзменения <> Объект.ФормаОплаты Тогда
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле = Элементы.ТоварыЦена Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ВидЦены) И НЕ Объект.Согласован И // Тогда
			// silber {
			Не ТекущиеДанные.ЭтоИзКП Тогда
		// } silber
...
&НаКлиенте
Процедура ТоварыНоменклатураПриИзменении(Элемент, ТекСтрока = Неопределено)
	
	ТекущаяСтрока = ?(ТекСтрока = Неопределено, Элементы.Товары.ТекущиеДанные, ТекСтрока);
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	ОбработкаВыбораТовараНаСервере(Элементы.Товары.ТекущаяСтрока);
...
Функция НастройкиКП()
	...
	рольМенеджер = Справочники.РолиИсполнителей.НайтиПоНаименованию("Менеджер");
...
Процедура прочитатьДанныеПоКП()
	
	Элементы.СформироватьРеализацию.Видимость 	= Объект.Статус = Перечисления.СтатусыКП.ГотовКОтгрузке;
	Элементы.ПечатьКомплекта.Видимость			= Объект.Статус = Перечисления.СтатусыКП.Выполнено;
	
	// резервирование
	Элементы.НадписьРезерв.Заголовок = ?(Документы.КП.СтатусыКоторыеРезервируют().Найти(Объект.Статус) = Неопределено, " ", "зарезервировано");
...
&НаСервере
Процедура ПриИзмененииФормыОплаты();
	
	Соглашение = АТ_Сервер.ПолучитьСформироватьСоглашение(Объект.Партнер,
					Новый Структура("Организация, ФормаОплаты", Объект.Организация, Объект.ФормаОплаты));
					
	Если Не Соглашение.Пустая() Тогда
		Объект.Соглашение = Соглашение;
	КонецЕсли;
	
	Если Не Объект.Контрагент.ЭтоРозничный Тогда
		Объект.Контрагент = АТ_Сервер.ПолучитьСформироватьРозничногоКонтрагента(Объект.Партнер, Новый Структура("СоздатьДоговор", Истина), Объект.Договор);
	КонецЕсли;
...
Процедура УправлениеВидимостьюДоступностью()
	...
	Кнопки 		= Новый Структура("ВРаботе,Актуальное,Согласовано,Сборка,Закрыто,СоздатьРеализацию", 
	...
	Элементы.НадписьЗонаЗпрещена.Видимость = ЗапрещеннаяЗонаДоставки;
	
	// Доступность товаров
	
	МожетРедактироватьТовары = ЭтоМенеджер И 
			(	Объект.Статус.Пустая() ИЛИ 
				Объект.Статус = Перечисления.СтатусыКП.Вработе ИЛИ
				Объект.Статус = Перечисления.СтатусыКП.Новый);
	
	Элементы.ВыбратьТоварыИначатьСуперПодбор.Видимость 	= МожетРедактироватьТовары;
	//Элементы.ТоварыСуперПодбор.Видимость 				= МожетРедактироватьТовары;
	Элементы.ГруппаТоварыПодобратьТовары.Видимость 		= МожетРедактироватьТовары;
	
	// Проверим возможность продать сразу
	
	Если ЭтоМенеджер И Не Объект.Склад.Пустая() И (Кнопки.ВРаботе Или Кнопки.Сборка) Тогда
		Если МожноБыстроПродать(ЭтоМенеджер) Тогда
			Кнопки.СоздатьРеализацию 	= Истина;
			Кнопки.Сборка 				= Ложь;
		КонецЕсли;
	КонецЕсли;
...
КонецПроцедуры
&НаСервере
Функция МожноБыстроПродать(ЭтоМенеджер = Неопределено)
	
	Если ЭтоМенеджер = Неопределено Тогда
		ЭтоМенеджер = АТ_Кэш.ДоступнаРоль(рольМенеджер);
	КонецЕсли;
	...
	Возврат КолСтрок И
			ЭтоМенеджер И Не 
			Объект.Склад.Пустая() И 
			Объект.Статус <> Перечисления.СтатусыКП.Закрыто И
			Объект.Товары.НайтиСтроки(Новый Структура("Склад", Объект.Склад)).Количество() = КолСтрок И
			Не Объект.Товары.НайтиСтроки(Новый Структура("ЭтоПерекуп", Истина)).Количество();					
			
КонецФункции									
&НаСервере
Процедура ПроверитьЗапрещенаДоставка()

	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 ИСТИНА ИЗ РегистрСведений.АТ_ЗапрещенныеЗоныДоставки ГДЕ Склад = &Склад И Зона = &Зона");
	Запрос.УстановитьПараметр("Склад", 	Объект.Склад);
	Запрос.УстановитьПараметр("Зона", 	Объект.ЗонаДоставки);
	
	ЗапрещеннаяЗонаДоставки = Не Запрос.Выполнить().Пустой();
...
Процедура БП_Действие(Команда)
	...
	//ИначеЕсли ИмяСтатуса = "Закрыто" Тогда
	//	
	//	БП_ОтказОтКП();
...
&НаКлиенте
Процедура ПродатьСразуБП(Ответ, ДопПараметры) Экспорт
	
	Если 	Ответ = КодВозвратаДиалога.Да И
			СоздатьРеализациюПоКП() Тогда
			
		//Если ДопПараметры.ЗакрытьПодбор = Истина Тогда
		//	Оповестить("ЗакрытьСуперПодбор", Новый Структура("КП", Объект.Ссылка), Элементы.Товары);
		//КонецЕсли;

		УстановитьСтатусИЗакрыть("Выполнено");
	Иначе
		Прочитать();
		ПрочитатьДанныеПоКП();
	КонецЕсли;
...
КонецПроцедуры
&НаКлиенте
Процедура БП_СоздатьРеализацию(Команда, ЗакрытьПодбор = Неопределено)
	
	Если ПройтиПроверкуНаКлиенте() И ПройтиРазрешениеСкладов(РазрешенныеСклады()) И ПартнеруРазрешенаОтгрузка() Тогда		
		ПоказатьВопрос(Новый ОписаниеОповещения("ПродатьСразуБП", ЭтаФорма, Новый Структура("ЗакрытьПодбор", ЗакрытьПодбор)), 
				"Сформировать реализацию товаров, а КП закрыть?", 
				РежимДиалогаВопрос.ДаНет,,,
				"Завершение формирования КП");
	КонецЕсли;
...
КонецПроцедуры
&НаКлиенте
Функция ПройтиПроверкуНаКлиенте()
	...
	Если Объект.Склад.Пустая() Тогда
		...
		ТекущаяСтраница = "СтраницаОсновное";
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы[ТекущаяСтраница];
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбран склад отгрузки клиенту.", Объект.Ссылка, "Склад", "Объект");		
		
	ИначеЕсли Объект.Контрагент.Пустая() Тогда
		
		ТекущаяСтраница = "страницаДополнительно";
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы[ТекущаяСтраница];
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбран контрагент.", Объект.Ссылка, "Контрагент", "Объект");		
		
	ИначеЕсли Объект.Договор.Пустая() Тогда
		
		ТекущаяСтраница = "страницаДополнительно";
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы[ТекущаяСтраница];
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбран договор.", Объект.Ссылка, "Договор", "Объект");		
		
	ИначеЕсли ЗапрещеннаяЗонаДоставки Тогда
		ТекущаяСтраница = "СтраницаОсновное";
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы[ТекущаяСтраница];
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Зона доставки - запрещена.");
		
	ИначеЕсли Объект.ДатаОтгрузки = '00010101' Тогда		
...
КонецФункции
&НаСервере
Функция РазрешенныеСклады()
	
	//Запрос = Новый Запрос("ВЫБРАТЬ Ссылка ИЗ Справочник.Склады ГДЕ АТ_РазрешитьВКП И НЕ ПометкаУдаления");
	//Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	Возврат АТ_Сервер.ПолучитьДоступныеСкладыПоПартнеру(Объект.Партнер);
	
КонецФункции
&НаСервере
Функция ПартнеруРазрешенаОтгрузкаНаСервере()
	
	Возврат Не СегментыСервер.ПартнерВходитВСегментыЗапретаОтгрузки(Объект.Партнер);
	
КонецФункции
&НаКлиенте
Функция ПартнеруРазрешенаОтгрузка()
	
	Разрешено = ПартнеруРазрешенаОтгрузкаНаСервере();
	
	Если Не Разрешено Тогда
		
		ПоказатьПредупреждение(,"Партнеру запрещена отгрузка.",,"Контроль отгрузки");
...
КонецФункции
&НаКлиенте
Функция ПройтиРазрешениеСкладов(РазрешенныеСклады)
	
	Инд = -1;
	Для Каждого Строка ИЗ Объект.Товары Цикл Инд = Инд + 1;
		
		Если Не Строка.Склад.Пустая() И РазрешенныеСклады.Найти(Строка.Склад) = Неопределено Тогда
			ТекущаяСтраница = "страницаТовары";
			Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы[ТекущаяСтраница];
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон("Склад %1 запрещенный.", СтрЗаменить(Строка.Склад, "Склад", "")), Объект.Ссылка, "Склад", СтрШаблон("Объект.Товары[%1]", Инд));
			Возврат Ложь;
		КонецЕсли;
...
&НаСервере
Функция ЗарытьКПИСвязанныеДокументы()
	
	// Закрывает документы
	//	Возвращает 	ИСТИНА если есть возможность закрыть
	//				ЛОЖЬ если существущие ситуацию уже ничем не исправить
	// если было перемещение то убирает в нем назначение
	// если был заказ на перемещение то убирает в нем назначение
	// если был приходный ордер то убирает в нем назначение
	// если был заказ то устанавливает "не обеспечивать"
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ 	СвязанныйДокумент КАК Ссылка
	|ПОМЕСТИТЬ 	Доки
	|ИЗ (
	|	ВЫБРАТЬ СвязанныйДокумент ИЗ РегистрСведений.АТ_СвязьОбъектовКП ГДЕ КП = &Ссылка ОБЪЕДИНИТЬ ВСЕ
	|	ВЫБРАТЬ ДокументРезультат ИЗ РегистрСведений.АТ_СвязьОбъектовКП ГДЕ КП = &Ссылка
	|) Запрос;
...
КонецФункции
&НаСервере
Функция ЕстьДокументыПоДвижению()
	
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 ИСТИНА ИЗ РегистрСведений.АТ_СвязьОбъектовКП ГДЕ КП = &Ссылка");
...
&НаКлиенте
Процедура БП_ОтказОтКП()
	
	Если ЕстьДокументыПоДвижению() Тогда
		
		ПоказатьВопрос(Новый ОписаниеОповещения("ПользовательРешилЗакрытьКП", ЭтаФорма), "На основании уже сформированы другие документы, при закрытии эти документы будут скорректированы
					|Продолжить закрывать КП?",РежимДиалогаВопрос.ДаНет,,,"Может случайно нажали?");
	Иначе
		
		Если ЗарытьКПИСвязанныеДокументы() Тогда
			Закрыть();
		КонецЕсли;
...
КонецПроцедуры
&НаКлиенте
Процедура ПользовательРешилЗакрытьКП(Ответ, ДопПараметры) Экспорт
	
	Если 	Ответ = КодВозвратаДиалога.Да И
			ЗарытьКПИСвязанныеДокументы() Тогда
...
КонецФункции
&НаСервере
Процедура ДобавитьСвязанный(РегСвязи, Ссылка, ДокРезультат = Неопределено)
	...
	Если Не Ссылка.Пустая() Тогда
		...
		Если ДокРезультат <> Неопределено Тогда
			Запись.ДокументРезультат = ДокРезультат;
		КонецЕсли;
...
Функция ОпределитьСкладПоставщика(текПоставщик)
	...
	Склад = Объект.Товары.НайтиСтроки(Новый Структура("КлючСтроки", 
				Объект.ВариантыЗакупки.НайтиСтроки(Новый Структура("Поставщик", текПоставщик))[0].КлючСтроки))[0].Склад;
				
	Возврат ?(Склад.Пустая(), Объект.Склад, Склад);
				
КонецФункции
&НаСервере
Функция ОпределитьСтруктуруПостащика(Поставщик)
	
	// Возвращает первого попавшего контрагента и договор поставщика если он есть
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ 	ПЕРВЫЕ 1 Ссылка Договор, Контрагент, &Партнер Партнер 
	|ИЗ 	Справочник.ДоговорыКонтрагентов 
	|ГДЕ 	Партнер = &Партнер И НЕ ПометкаУдаления И ТипДоговора = &СПоставщиком
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1 ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка), Ссылка, &Партнер Партнер 
...
Функция СоздатьНаОснованииДокументыПоКП()
	...
	// если склад отправитель ордерный тогда делаем еще и перемещение
	//
	...
	Если Записать(Новый Структура("РежимЗаписи", РежимЗаписиДокумента.Проведение)) Тогда
		...
		Если СкладУказан Тогда
			ЗакСсылка = АТ_Сервер.ЗаполнитьИПровестиДокОснование(Документы.ЗаказКлиента, Новый Структура, Объект.Ссылка);
		...
		Для Каждого Склад Из Склады Цикл
			Если СкладУказан Тогда
				Если Склад <> Объект.Склад Тогда
					...
					ЭтоОрдернаяСхема = Склад.ДатаНачалаОрдернойСхемыПриОтгрузке <> '00010101' И Склад.ДатаНачалаОрдернойСхемыПриОтгрузке < ТекущаяДата();
					
					// Заказ на перемещение
					
					МассивЗаказов 		= новый Массив;
					СвойстваПеремещения = Новый Структура("СкладОтправитель, СкладПолучатель", Склад, Объект.Склад);
					МассивЗаказов.Добавить(АТ_Сервер.ЗаполнитьИПровестиДокОснование(Документы.ЗаказНаПеремещение, СвойстваПеремещения,  Объект.Ссылка,
								Новый Структура("ДокументОснование, ЗаказНазначение, КонтрольОстатковОтключен", ЗакСсылка, ЗакСсылка, Истина),
								Новый Структура("ЗонаДоставки, СпособДоставки, АдресДоставки, ВремяДоставкиС, ВремяДоставкиПо",
												Объект.ЗонаДоставки,
												Перечисления.СпособыДоставки.ДоКлиента,
												АТ_Сервер.ПолучитьАдресДоставкиСклада(Объект.Склад).Представление,
												'00010101080000',
												'00010101180000')));
					// Перемещение товаров
												
					ДобавитьСвязанный(РегСвязи, МассивЗаказов[0], 
							?(ЭтоОрдернаяСхема,
								АТ_Сервер.ЗаполнитьИПровестиДокОснование(Документы.ПеремещениеТоваров, 
										СвойстваПеремещения, 
										Новый Структура("МассивЗаказов, Склад, РеквизитыШапки", МассивЗаказов, Склад,
												Документы.ПеремещениеТоваров.ДанныеЗаполненияНакладной(МассивЗаказов)),,
										Новый Структура("Статус", Перечисления.СтатусыПеремещенийТоваров.Принято)),
								Неопределено));
				КонецЕсли;		
			ИначеЕсли Не Склад.Пустая() Тогда
				...
				ЗакСсылка = АТ_Сервер.ЗаполнитьИПровестиДокОснование(Документы.ЗаказКлиента, Новый Структура("Склад", Склад), Объект.Ссылка);
				ДобавитьСвязанный(РегСвязи, ЗакСсылка);
		...
		Для Каждого текПоставщик Из Поставщики Цикл
			...
			Если Не ЗначениеЗаполнено(СтруктураПоставщика.Контрагент) Тогда
				ВызватьИсключение СтрШаблон("Не могу найти договор с поставщиком %1", текПоставщик);
			КонецЕсли;
			
			// Заказ поставщику
			
			ЗаказПоставщикуСсылка = АТ_Сервер.ЗаполнитьИПровестиДокОснование(
			...
			Если ЗаказПоставщикуСсылка.Пустая() Тогда
				ВызватьИсключение СтрШаблон("Не удалось сформировать заказ поставщику %1", текПоставщик);
			Иначе
				ДобавитьСвязанный(РегСвязи, ЗаказПоставщикуСсылка);
			КонецЕсли;
...
КонецФункции
&НаСервере
Функция СоздатьРеализациюПоКП()
	
	// Формирует на основании КП заказ реализацию
	
	Если Записать(Новый Структура("РежимЗаписи", РежимЗаписиДокумента.Проведение)) Тогда
		
		НачатьТранзакцию();
		
		ЗакСсылка = АТ_Сервер.ЗаполнитьИПровестиДокОснование(Документы.ЗаказКлиента, 
										Новый Структура("Склад", Объект.Склад), 
										Объект.Ссылка, 
										Новый Структура("ВариантОбеспечения, КонтрольОстатковОтключен", 
															Перечисления.ВариантыОбеспечения.Отгрузить, 
															Истина));
															
		Если Не Документы.КП.СформироватьРеализацию(Объект.Ссылка, ЗакСсылка).Пустая() Тогда
			ЗафиксироватьТранзакцию();
			Возврат Истина;
		КонецЕсли;
		ОтменитьТранзакцию();
...
Процедура ПриИзмененииКонтрагента()
	...
	Если Объект.Договор.Партнер <> объект.Партнер Тогда
		Объект.Договор = АТ_Сервер.ПолучитьДействующийДоговорЕслиДоговорОдин(Объект.Партнер, Новый Структура("Контрагент", Объект.Контрагент));
...
Процедура ПриИзмененииПартнера()
	...
	Если Объект.Договор.Партнер <> объект.Партнер Тогда
		Объект.Договор = АТ_Сервер.ПолучитьДействующийДоговорЕслиДоговорОдин(Объект.Партнер);
		ПродажиСервер.ЗаполнитьБанковскиеСчетаПоДоговору(Объект.Договор, Объект.БанковскийСчет, Объект.БанковскийСчетКонтрагента);
	КонецЕсли;
	
	Если Объект.Контрагент <> Объект.Договор.Контрагент Тогда
...
&НаСервере
Функция ПолучитьСписокАдресовДоставкиПоставщика(Поставщик)
	
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ Ссылка, Представление ИЗ Справочник.БП_АдресаДоставкиКонтрагентов ГДЕ Партнер = &Партнер И НЕ ПометкаУдаления");
	Запрос.УстановитьПараметр("Партнер", Поставщик);

	Возврат КонвертацияТипов.ПолучитьСписокЗначенийИзВыборки(Запрос.Выполнить().Выбрать(), "Ссылка", "Представление");
	
КонецФункции	
&НаКлиенте
Процедура ВариантыЗакупкиАдресДоставкиДляПоставщикаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	текДанные = Элементы.ВариантыЗакупки.ТекущиеДанные;
	
	Если текДанные <> Неопределено И Не текДанные.Поставщик.Пустая() Тогда
		Список = ПолучитьСписокАдресовДоставкиПоставщика(текДанные.Поставщик);
		Список.Добавить("Создать новый адрес",,,БиблиотекаКартинок.СоздатьЭлементСписка);
		Если Список.Количество() = 1 Тогда
			ВыбранАдресДоставкиПостащика(Список.Получить(0));
		ИначеЕсли Список.Количество() > 1 Тогда
			ПоказатьВыборИзМеню(Новый ОписаниеОповещения("ВыбранАдресДоставкиПостащика", ЭтаФорма), Список, Элемент);
		КонецЕсли;
	КонецЕсли;	
	
КонецПроцедуры
&НаКлиенте
Процедура ВыбранАдресДоставкиПостащика(Элемент, ДопПараметры = Неопределено) Экспорт
	
	Если Элемент <> Неопределено Тогда
		Если Элемент.Значение = "Создать новый адрес" Тогда
			ОткрытьФорму("Справочник.БП_АдресаДоставкиКонтрагентов.ФормаОбъекта", 
						Новый Структура("Партнер", Элементы.ВариантыЗакупки.ТекущиеДанные.Поставщик),
						ЭтаФорма,,,,
						Новый ОписаниеОповещения("СозданНовыйАдресПостащика", ЭтаФорма),
						РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
			ЗаполнитьТекВариантАдресомПоставщика(Элемент.Значение, Элемент.Представление);
		КонецЕсли;
	КонецЕсли;
		
КонецПроцедуры
&НаКлиенте
Процедура СозданНовыйАдресПостащика(Ссылка, ДопПараметры) Экспорт
	
	Если ЗначениеЗаполнено(Ссылка) Тогда
		ЗаполнитьТекВариантАдресомПоставщика(Ссылка, Строка(Ссылка));
	КонецЕсли;
	
КонецПроцедуры
&НаКлиенте
Процедура ЗаполнитьТекВариантАдресомПоставщика(Ссылка, Представление)
	
	текДанные = Элементы.ВариантыЗакупки.ТекущиеДанные;
	текДанные.АдресДоставкиДляПоставщика 		= Представление;
	текДанные.АдресДоставкиДляПоставщикаСсылка	= Ссылка;
...
&НаКлиенте
Процедура ОткрытьПодборАвтоматически()
	
	ТекущаяСтраница = "СтраницаТовары";
	ТекущаяСтраницаПриИзменении();
	
	СуперПодбор();
	
КонецПроцедуры
&НаКлиенте
Процедура СуперПодбор(Команда = Неопределено)
...
Процедура ТоварыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	стЦена = НовСтрока.ЦенаСоСкидкой;
	
	Элементы.Товары.ТекущаяСтрока = НовСтрока.ПолучитьИдентификатор();
	ТоварыНоменклатураПриИзменении(Элементы.ТоварыНоменклатура, НовСтрока);
	
	ОбработкаВыбораТовараНаСервере(НовСтрока.ПолучитьИдентификатор());
	
	НовСтрока.ЦенаСоСкидкой = стЦена;
...
КонецПроцедуры
&НаСервере
Процедура ОбраотатьСтрокуТовара()
	
	// Убирает копейки, выставляет НДС
...
Процедура РассчитатьИтоги()
	...
	//в жопу типовую РассчитатьИтоговыеПоказателиКоммерческогоПредложения(()
...
&НаКлиенте
Процедура ТекущаяСтраницаПриИзменении(Элемент = Неопределено)
...
&НаКлиенте
Процедура АТ_РаспознатьРеквизитыИзСкана(Команда)
	
	// Эксперемент
	
	АТ_СканерКлиент.НачатьРаспознованиеРеквизитов(ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура ДосьюеПартнера(Команда)
	
	ОткрытьФорму("Отчет.ДосьеПартнера.Форма.ФормаОтчета", Новый Структура("Партнер, СформироватьПриОткрытии", Объект.Партнер, Истина), ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьТоварыИначатьСуперПодбор(Команда)
	
	Если ПодборОткрыт Тогда
		ЗакрытьПодбор(Команда);
	Иначе
		ОткрытьПодборАвтоматически();
	КонецЕсли;
...
Процедура АдресДоставкиОткрытие(Элемент, СтандартнаяОбработка)   
	...
	Если Элемент.Имя = "ВариантыЗакупкиАдресДоставкиДляПоставщика" Тогда
		сткПараметров.СсылкаАдреса = Элементы.ВариантыЗакупки.ТекущиеДанные.АдресДоставкиДляПоставщикаСсылка;
...
Процедура ОкончаниеОткрытияАдреса(Результат, ДопПараметры)        Экспорт
	...
	ИначеЕсли ДопПараметры.Свойство("ИдентификаторВариантЗакупок") Тогда
		...
		Если Результат <> Неопределено Тогда
			...
			текСтрока.АдресДоставкиДляПоставщика = Результат.Ссылка;
		КонецЕсли;
...
&НаСервере
Процедура ПодборПриСоздании()
	
	ПодборОткрыт = Ложь;
	//СписокТоваров.ТекстЗапроса = "ВЫБРАТЬ &Доставка Доставка, 1 ПорядокСклада, null Производитель, null Артикул";
...
Процедура ОбновитьПараметрыСпискаТоваров()
	...
	//ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокТоваров, "ДатаПолученияЦен", 	КонецДня(?(Объект.Ссылка.Пустая(), ТекущаяДата(), Объект.Дата)));
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокТоваров, "ТолькоВНаличии", 	ТолькоВНаличии);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокТоваров, "БизнесРегион",		Объект.Партнер.БизнесРегион);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокТоваров, "Соглашение", 		Объект.Соглашение);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокТоваров, "ВидыЦен", 			Объект.Соглашение.ВидЦен);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокТоваров, "Валюта", 			Объект.Валюта);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокТоваров, "СкидкаПоСоглашению", СТ_ОбщегоНазначения.ПолучитьПроцентСкидкиПоСоглашению(Объект.Соглашение));
...
Процедура ОбновитьТекстЗапросаПоСкладам()
	...
	стр = Новый Массив;
	Ном = 0;
	Для Каждого Элемент Из СкладыСписокПорядка Цикл Ном = Ном + 1;
...
&НаСервере	
Процедура ОткрытьСуперПодбор()
	
	ПодборОткрыт = Истина;
	Элементы.ГруппаПодбор.Видимость = Истина;
	
	СписокТоваров.Параметры.УстановитьЗначениеПараметра("СвойствоРазмер", 	ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПолучитьСсылку(Новый УникальныйИдентификатор("f0ac96d2-b5ae-11e4-8e7b-00155dc0ba09")));
	...
	ОбновитьПорядокСкладов(Объект.Склад);
...
&НаСервере
Функция ПолучитьМассивСсылокПоМассивуАртикулов(МассивАртикулов, ИскатьВКроссах = Истина)
	...
	Для Каждого Артикул Из МассивАртикулов Цикл
		Таб.Добавить().Артикул = Артикул;
	КонецЦикла;
	
	Запрос = Новый Запрос(СтрШаблон(?(ИскатьВКроссах, "
	...
	|	(	%1 Спр.АртикулДляПоиска %2	ПОДОБНО Артикулы.Артикул ИЛИ
	|		%1 Наименование %2			ПОДОБНО Артикулы.Артикул ИЛИ
	...
	|
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ Артикулы
	|ПО		%1 Спр.АртикулДляПоиска %2	ПОДОБНО Артикулы.Артикул ИЛИ
	|		%1 Наименование %2			ПОДОБНО Артикулы.Артикул
	|ГДЕ
	|	НЕ Спр.ЭтоГруппа И
	|	НЕ Спр.ПометкаУдаления И
	|	Спр.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар)
...
Функция ПолучитьМассивСсылокПоАртикулу()
	...
	|ИЗ 				Справочник.Номенклатура Спр
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ	РегистрСведений.АналогиТоваров.СрезПоследних( ,АртикулДляПоиска %1) Кросс
	|ПО					НЕ Кросс.Удалено И
	|					Спр.Ссылка = Кросс.Номенклатура
	|ГДЕ
	|	НЕ Спр.ЭтоГруппа И
	...
	|
	|	(	Спр.Артикул %1 ИЛИ Спр.АртикулДляПоиска %1 ИЛИ Наименование %1 ИЛИ НЕ Кросс.Номенклатура ЕСТЬ NULL)
	|", ?(ТочноеСоответствие, 
			"=""" + СтрокаПоискаАртикула + """", 
			"ПОДОБНО ""%" + СТ_ОбщегоНазначения.ПолучитьКлючПоСтроке(СтрокаПоискаАртикула) + "%""")));
...
Процедура ОбновитьПорядокСкладов(ВыбСклад)
	...
	СкладыСписокПорядка.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Склад"));
	СкладыСписокПорядка.Вставить(0, ВыбСклад);
...
Процедура ГиперсылкаВыбораСкладаПорядкаНажатие(Элемент)
	...
	ОткрытьФорму("Справочник.Склады.Форма.ФормаВыбора", Новый Структура("Отбор", Новый Структура("АТ_РазрешитьВКП", Истина)), ЭтаФорма,,,,Новый ОписаниеОповещения("ВыбранСкладПорядка", ЭтаФорма));
...
Процедура СкладыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ОткрытьФорму("Документ.КП.Форма.ВыборПорядкаСкладов", Новый Структура("СписокСкладов", СкладыСписокПорядка), ЭтаФорма,,,,Новый ОписаниеОповещения("ВыбранПорядокСкладов", ЭтаФорма));
	
КонецПроцедуры
&НаКлиенте
Процедура ВыбранПорядокСкладов(ВыбСписок, ДопПараметры) Экспорт
	
	Если ВыбСписок <> Неопределено Тогда
		
		СкладыСписокПорядка = ВыбСписок;
		ОбновитьТекстЗапросаПоСкладам();
...
Процедура СписокТоваровВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если текДанные <> Неопределено Тогда
		ТоварыОбработкаВыбора(
			Элементы.Товары, 
			Новый Структура("Номенклатура, Склад, Количество, КоличествоУпаковок, Цена, ЦенаСоСкидкой, СкидкаПоСоглашению", 
...
Процедура ОткрытьКарточкуТовара(Команда)
	...
	текДанные = Элементы.СписокТоваров.ТекущиеДанные;
	Если текДанные <> Неопределено И Не текДанные.Номенклатура.Пустая() Тогда
		ПоказатьЗначение(,текДанные.Номенклатура);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьИзКП(Команда)
	
	ОткрытьФорму("Документ.КП.ФормаВыбора", Новый Структура("РежимВыбора", Истина), ЭтаФорма,,,,Новый ОписаниеОповещения("ВыбраноКПДляЗаполнения", ЭтаФорма), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры
&НаКлиенте
Процедура ВыбраноКПДляЗаполнения(ВыбДок, ДопПараметры) Экспорт
	
	Если ВыбДок <> Неопределено Тогда
		
		Ном = Объект.Товары.Количество() - 1;
		ЗаполнитьПоКП(ВыбДок);
		
		Если Ном >= 0 Тогда
			Для Инд = Ном По Объект.Товары.Количество() - 1 Цикл
				ТоварыНоменклатураПриИзменении(Элементы.Товары, Объект.Товары[Инд]);
			КонецЦикла;
			РассчитатьСкидкиНаценкиКлиент();
		КонецЕсли;
...
КонецПроцедуры
&НаСервере
Процедура ЗаполнитьПоКП(КПСсылка)
	
	Запрос = Новый Запрос("ВЫБРАТЬ * ИЗ Документ.КП.Товары ГДЕ Ссылка = &Ссылка");
	Запрос.УстановитьПараметр("Ссылка", КПСсылка);
	КонвертацияТипов.ДобавитьТаблицуКДругойТаблице(Объект.Товары, Запрос.Выполнить().Выгрузить());
...
&НаСервере
Функция ПолучитьСписокГруппLaximo()
	
	// Вытаскиваем данные из кеша и возвращаем список
	// если в кеше данных нет тогда запрашиваем в интрнете и заполняем кеш
	
	Таблица = КонвертацияТипов.ПолучитьТаблицуИзНайденныхСтрокТаблицыЗначений(ТаблицаLaximo.Выгрузить(), Новый Структура("СтрокаПоиска", СтрокаПоискаАртикула), "НаименованиеГруппы,Артикул,СтрокаПоиска");
	Если Не Таблица.Количество() Тогда
		
		КонвертацияТипов.ВыполнитьВыражениеВКаждойСтрокеТаблицы(АТ_Сервер.Laximo_ПолучитьТаблицуАртикулов(СтрокаПоискаАртикула), "
		|НовСтрока = Параметры.ТаблицаLaximo.Добавить();
		|НовСтрока.СтрокаПоиска 		= Параметры.Поиск;
		|НовСтрока.НаименованиеГруппы 	= Строка.detailsname;
		|НовСтрока.Артикул 				= Строка.oem;
		|", Новый Структура("ТаблицаLaximo, Поиск", Таблица, СтрокаПоискаАртикула));
		
		КонвертацияТипов.ДобавитьТаблицуКДругойТаблице(ТаблицаLaximo, Таблица);
		
	КонецЕсли;
	
	Список = Новый СписокЗначений;
	Список.ЗагрузитьЗначения(КонвертацияТипов.ПолучитьУникальныйМассивИзНайденныхЗначенийСтрокТаблицыЗначений(Таблица, Новый Структура("СтрокаПоиска", СтрокаПоискаАртикула), "НаименованиеГруппы"));
...
Процедура НачатьПоискВlaximo()
	...
	ПоказатьОповещениеПользователя("Ищем в интернете кроссы",,СтрокаПоискаАртикула,БиблиотекаКартинок.Интернет);
	СписокГрупп = ПолучитьСписокГруппLaximo();
	
	ОбновитьТекстЗапросаПоФильтруСписка(ПолучитьМассивСсылокПоМассивуАртикулов(ПолучитьМассивАртикуловИзКешаLaximo()));
	
	//Если СписокГрупп.Количество() = 1 Тогда
	//	ОбновитьТекстЗапросаПоФильтруСписка(ПолучитьМассивСсылокПоМассивуАртикулов(ПолучитьМассивАртикуловИзКешаLaximo(СписокГрупп[0].Значение)));
	//ИначеЕсли СписокГрупп.Количество() = 0 Тогда
	//	ПоказатьПредупреждение(,"В интернете ничего не найдено. Поиск: " + СтрокаПоискаАртикула,,"Нет артикула");
	//Иначе
	//	ПоказатьВыборИзМеню(Новый ОписаниеОповещения("ВыбранаГруппаLaximo", ЭтаФорма), ПолучитьСписокГруппLaximo(), Элементы.СтрокаПоиск);
	//КонецЕсли;

КонецПроцедуры
//&НаКлиенте
//Процедура ВыбранаГруппаLaximo(ВыбЭлемент, ДопПараметры) Экспорт
//	
//	Если ВыбЭлемент <> Неопределено Тогда
//		
//		ОбновитьТекстЗапросаПоФильтруСписка(
//			ПолучитьМассивСсылокПоМассивуАртикулов(
//				ПолучитьМассивАртикуловИзКешаLaximo(ВыбЭлемент.Значение)));
//	КонецЕсли;
//	
//КонецПроцедуры
&НаКлиенте
Функция ПолучитьМассивАртикуловИзКешаLaximo()
	
	Массив = Новый Массив;
	Строки = ТаблицаLaximo.НайтиСтроки(Новый Структура("СтрокаПоиска", СтрокаПоискаАртикула));
	Для Каждого Строка Из Строки Цикл
		Если Массив.Найти(Строка.Артикул) = Неопределено Тогда
...
КонецФункции
//&НаКлиенте
//Функция ПолучитьМассивАртикуловИзКешаLaximo(НаименованиеГруппы)
//	
//	Массив = Новый Массив;
//	Строки = ТаблицаLaximo.НайтиСтроки(Новый Структура("СтрокаПоиска, НаименованиеГруппы", СтрокаПоискаАртикула, НаименованиеГруппы));
//	Для Каждого Строка Из Строки Цикл
//		Если Массив.Найти(Строка.Артикул) = Неопределено Тогда
//			Массив.Добавить(Строка.Артикул);
//		КонецЕсли;
...
Процедура РеквизитыПечати(Команда)
	...
	ОткрытьФорму("Документ.КП.Форма.РеквизитыПечати", СтруктураПараметров, ЭтаФорма,,,, Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СтруктураПодчиненности(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.СтруктураПодчиненности",Новый Структура("ОбъектОтбора", Объект.Ссылка), ЭтаФорма);
...
Процедура ДвиженияДокумента(Команда)
	...
	ОткрытьФорму("Отчет.ДвиженияДокумента.ФормаОбъекта", Новый Структура("СформироватьПриОткрытии, ПараметрКоманды", Истина, Объект.Ссылка), ЭтаФорма);
...
Процедура ИницилизироватьДоставку()
	...
	Если МультидоставкаВключена() Тогда
		...
		Элементы.ТекущаяСтраница.СписокВыбора.НайтиПоЗначению("СтраницаДоставка").Значение = "СтраницаДоставкаАТ";
...
Процедура СоздатьЭлементыДляДоставкиНаСервере(АдресЭлементов)
	...
	ГруппаРодительДоставки = Элементы.СтраницаДоставкаАТ;
