...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы и Модифицированность
	 И МассивСтрок.Количество() > 0 Тогда
		Отказ = Истина;
		Оповещение = Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект);
		ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияФормы(Оповещение, Отказ, ЗавершениеРаботы, НСтр("ru = 'Выбранные товары не перенесены в документ. Перенести?'"));
	КонецЕсли;
...
Процедура ПеренестиВДокумент()
	...
	СтруктураЗначенияВыбора = Новый Структура("АдресПлатежейВХранилище, Предназначение, Подразделение",
		АдресПлатежейВХранилище,
		Предназначение,
		Подразделение);
	СтруктураЗначенияВыбора.Вставить("Менеджер", Менеджер);
	СтруктураЗначенияВыбора.Вставить("Сделка", Сделка);
...
Процедура ЗаполнитьТаблицуТоваров()
	...
	|	ВидыЗапасов.УдалитьПредназначение КАК Предназначение,
	|	ВидыЗапасов.УстарелоСделка КАК Сделка,
	|	ВидыЗапасов.УстарелоМенеджер КАК Менеджер,
	|	ВидыЗапасов.УстарелоПодразделение КАК Подразделение
	|
	...
	|	И ВидыЗапасов.УстарелоМенеджер = &Менеджер
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
