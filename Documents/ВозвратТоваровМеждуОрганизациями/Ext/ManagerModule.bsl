...
Функция ТекстЗапросаВтЗапасыПолучателя(Запрос, ТекстыЗапроса)
	...
	|	ЕСТЬNULL(Передача.Ссылка,
	|		ЕСТЬNULL(ОснованиеПередача.Ссылка,
	|			ЗНАЧЕНИЕ(Документ.ПередачаТоваровМеждуОрганизациями.ПустаяСсылка)))                 КАК ДокументРеализации,
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	И ВидыЗапасов.ДокументРеализации = ЗНАЧЕНИЕ(Документ.ПередачаТоваровМеждуОрганизациями.ПустаяСсылка)
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР КОГДА ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|	 И ВидыЗапасов.ДокументРеализации <> ЗНАЧЕНИЕ(Документ.ПередачаТоваровМеждуОрганизациями.ПустаяСсылка) ТОГДА
	|			0
	|		ИНАЧЕ
	|			-ВидыЗапасов.СуммаСНДСУпр
	|	КОНЕЦ КАК Стоимость,
	...
	|	(ВЫБОР
	|		КОГДА ВидыЗапасов.ДокументРеализации = ЗНАЧЕНИЕ(Документ.ПередачаТоваровМеждуОрганизациями.ПустаяСсылка)
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ТОГДА -(ЕСТЬNULL(ТаблицаТоваровУпр.СуммаНДС, Строки.СуммаНДС))
	|		ИНАЧЕ 0
	|	КОНЕЦ                                  КАК НДСУпр,
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ТаблицаТоваровВВалютеУпр КАК ТаблицаТоваровУпр
	|	ПО
	|		Строки.ИдентификаторСтроки = ТаблицаТоваровУпр.ИдентификаторСтроки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрСведений.АналитикаУчетаНоменклатуры КАК Аналитика
	|	ПО
...
Процедура ЗаполнитьТоварыПоОстаткамКВозврату(Объект, НачалоПериода = Неопределено, КонецПериода = Неопределено) Экспорт
	Запрос = Новый Запрос("
		...
		|	КОНЕЦ									КАК Количество,
		|	Аналитика.Номенклатура					КАК Номенклатура,
		|	Аналитика.Номенклатура.СтавкаНДС		КАК СтавкаНДС,
		|	Аналитика.Характеристика				КАК Характеристика,
