...
Процедура ВозвращаемыеТоварыЗаполнитьЦеныПоСоглашению()
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоСоглашению(Объект, "ВозвращаемыеТовары", НСтр("ru='Возвращаемые товары'")) Тогда
...
Процедура ВозвращаемыеТоварыЗаполнитьЦеныВыделенныхСтрокПоВидуЦен(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "ВозвращаемыеТовары", НСтр("ru='Возвращаемые товары'")) Тогда
...
Процедура ПодобратьТоварыПоРеализациям(Команда)
	...
	Если Не Отказ Тогда
		...
		ПараметрыПодбора = Новый Структура(
			"Партнер, Документ, Валюта, Дата, НалогообложениеНДС, ЦенаВключаетНДС, Соглашение, Организация, Контрагент, ВозвратОтРозничногоПокупателя, ЧекККМ, ПоказыватьТовары, ПоказыватьТару, НаправлениеДеятельности",
			Объект.Партнер,
			Объект.Ссылка,
			Объект.Валюта,
...
Процедура ЗаменяющиеТоварыЗаполнитьЦеныПоСоглашению()
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоСоглашению(Объект,"ЗаменяющиеТовары", НСтр("ru='Заменяющие товары'")) Тогда
...
Процедура ЗаменяющиеТоварыЗаполнитьЦеныВыделенныхСтрокПоВидуЦен(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "ЗаменяющиеТовары", НСтр("ru='Заменяющие товары'")) Тогда
...
Процедура НазначитьРучнуюСкидку(Команда)
	...
	Если Не СкидкиНаценкиКлиент.ВозможноНазначениеРучнойСкидкиНаценки(Объект, "ЗаменяющиеТовары", НСтр("ru='Заменяющие товары'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьРучныеСкидки(Команда)
	...
	Если Не СкидкиНаценкиКлиент.ВозможнаОтменаРучныхСкидокНаценок(Объект, "ЗаменяющиеТовары", НСтр("ru='Заменяющие товары'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура СостояниеОбеспечения(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(
		Истина, "Документ.ЗаявкаНаВозвратТоваровОтКлиента.Форма.ФормаДокумента.СостояниеОбеспечения");
...
Процедура УстановитьУсловноеОформление()
	...
	УчетНДСУТ.УстановитьУсловноеОформлениеСтавкиНДС(ЭтаФорма, "ВозвращаемыеТоварыСтавкаНДС");
	УчетНДСУТ.УстановитьУсловноеОформлениеСтавкиНДС(ЭтаФорма, "ЗаменяющиеТоварыСтавкаНДС");
...
Процедура ОбработкаВыбораПодборНаКлиенте(ВыбранноеЗначение)
	...
	Если ИспользоватьРасширеннуюФормуПодбораКоличестваИВариантовОбеспечения Тогда
		...
		ОбеспечениеКлиент.ПоказатьВопросОбОтгрузкеОднойДатой(
			...
			Объект.ЗаменяющиеТовары.Количество() > 0);
...
Процедура ОбработатьШтрихкодыСервер(СтруктураПараметровДействия,КэшированныеЗначения)
	...
	Если КэшированныеЗначения.Свойство("ОбработанныеСтроки") Тогда
		ОбеспечениеСервер.РассчитатьДатуОтгрузкиВСтрокахТабличнойЧасти(КэшированныеЗначения.ОбработанныеСтроки, Объект, Объект.ЗаменяющиеТовары);
	КонецЕсли;
	ПриИзмененииДатыОтгрузкиВТабЧасти();
...
Процедура ОбновитьСостояниеСервер()
	...
	Если Состояние = Перечисления.СостоянияЗаявокНаВозвратТоваровОтКлиентов.Отклонена
		ИЛИ Состояние = Перечисления.СостоянияЗаявокНаВозвратТоваровОтКлиентов.Выполнена Тогда
...
Процедура ОбновитьСостояниеЭлементовУправленияИтоговПоРасчетам()
	...
	Иначе
		...
		СтруктураРасчетов = Документы.ЗаявкаНаВозвратТоваровОтКлиента.РасчетыПоЗаявке(Объект.Ссылка);
		...
		Иначе
			...
			Если СтруктураРасчетов.НашДолг > СтруктураРасчетов.ДолгКлиента Тогда
				Расчеты = НСтр("ru='Наш долг по заявке'") + ": "+ Формат(ИтогРасчетов,"ЧДЦ=2") + ВалютаДокумента;
			ИначеЕсли СтруктураРасчетов.ДолгКлиента > СтруктураРасчетов.НашДолг Тогда
				Расчеты = НСтр("ru='Долг клиента по заявке'") + ": "+ Формат(ИтогРасчетов*-1,"ЧДЦ=2") + ВалютаДокумента;
			Иначе
...
Процедура ДобавитьЭтапыОплатыВозвратДоплата()
	...
	ДатаПлатежа = ?(ЗначениеЗаполнено(Объект.ЖелаемаяДатаПоступления),Объект.ЖелаемаяДатаПоступления,ОбщегоНазначенияКлиент.ДатаСеанса());
	СуммаОплатыДокумента = СуммаЗаменяющихТоваров - СуммаЗалогаЗаТару;
...
Процедура ОбработатьВыборВариантаОбеспечения(СтруктураПодобранныеТовары, ДополнительныеПараметры) Экспорт 
	...
	ОбеспечениеКлиент.ПоказатьВопросОбОтгрузкеОднойДатой(
		...
		Объект.ЗаменяющиеТовары.Количество() > 1);
...
Функция ЗаполнитьОбеспечениеВУстановленномПорядке(ВыбранноеЗначение)
	...
	ПараметрыДокумента.Вставить("СтатусКВыполнению",
		Объект.Статус <> Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована
		И Объект.Статус <> Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КВозврату);
	...
	Если ТипЗнч(Таблица) <> Тип("ТаблицаЗначений") Тогда
		Возврат Новый Структура(
			...
			ОбеспечениеКлиентСервер.ОшибкиКонтроляОтгрузкиИОбеспечения(
				Таблица, "ЗаменяющиеТовары", НСтр("ru = 'Заменяющие товары'")));
	КонецЕсли;
