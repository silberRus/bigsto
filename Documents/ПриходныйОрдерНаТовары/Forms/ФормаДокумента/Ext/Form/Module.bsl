...
Процедура Автотест_ЗаполнитьПоРаспоряжению(ВидЗаполнения) Экспорт
	...
	Если ВидЗаполнения = "Количество" Тогда
		...
		Если Объект.Товары.Итог("КоличествоУпаковок") <> 0 Тогда
			...
			КонецЕсли;
			ПоказатьВопрос(Новый ОписаниеОповещения("Автотест_ЗаполнитьПоРаспоряжениюЗавершение2", ЭтотОбъект, Новый Структура("ВидЗаполнения", ВидЗаполнения)), ТекстСообщения, РежимДиалогаВопрос.ДаНет);
            Возврат;
	...
	Иначе
		...
		Если Объект.Товары.Количество() > 0 Тогда
			...
			КонецЕсли;			
			ПоказатьВопрос(Новый ОписаниеОповещения("Автотест_ЗаполнитьПоРаспоряжениюПослеВопроса", ЭтотОбъект, Новый Структура("ВидЗаполнения", ВидЗаполнения)), ТекстСообщения, РежимДиалогаВопрос.ДаНет);
            Возврат;
...
Процедура РаспоряжениеВыборКлиент(ВыбранноеЗначение, Параметры) Экспорт
	Если ВыбранноеЗначение <> Неопределено Тогда
		...
		СкладскаяОперация = СкладыКлиентСервер.СкладскаяОперацияПриемкиПоХозяйственнойОперации(Объект.ХозяйственнаяОперация);
		
		ПоказатьПредставлениеРаспоряжения(РаспоряжениеПредставление, Объект.Распоряжение);
		
 		РаспоряжениеВыборСервер(СкладскаяОперация);
