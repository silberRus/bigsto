...
#Область ФактыПродаж
	...
	ДопПараметр.Вставить("СоздаватьОперанд", НЕ ЭтоПланПродажПоКатегориям 
		И (НЕ ИспользоватьПланыПродаж 
		или ЭтоПланПродаж));
	ДопПараметр.Вставить("ЗаголовокОперанда", НСтр("ru='Факт продаж'"));
...
Функция СформироватьПечатнуюФормуПлана(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ВыводитьШапку Тогда
			...
			ТекстЗаголовка = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеПечати, НСтр("ru='План продаж'"));
		...
		Если ДанныеПечати.ЗаполнятьПартнераВТЧ Тогда
			СтруктураДопПоля = Новый Структура;
			СтруктураДопПоля.Вставить("Заголовок", НСтр("ru='Партнер'"));
...
Функция ТекстЗапросаДанныеКОбработке(ВыбратьДляРегистрацииДвиженийОборотыДвижений = Ложь) Экспорт
	...
	|	ПланПродаж.Статус.Порядок КАК СтатусПорядок,
	...
	|ГДЕ
	|	ПланПродаж.Замещающий
	|	И ПланПродаж.Проведен
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПланыПродаж.Ссылка КАК ЗамещающийПлан,
	|	ПланыПродаж1.Ссылка КАК ЗамещенныйПлан
	|ПОМЕСТИТЬ ЗамещающиеИЗамещенныеПланыПродаж
	|ИЗ
	|	ПланыПродаж КАК ПланыПродаж
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланыПродаж КАК ПланыПродаж1
	|		ПО ПланыПродаж.ОкончаниеПериода >= ПланыПродаж1.НачалоПериода
	|			И ПланыПродаж.НачалоПериода <= ПланыПродаж1.ОкончаниеПериода
	|			И ПланыПродаж.Ссылка <> ПланыПродаж1.Ссылка
	|			И ПланыПродаж.ВидПлана = ПланыПродаж1.ВидПлана
	|			И ПланыПродаж.СтатусПорядок >= ПланыПродаж1.СтатусПорядок
	|			И ПланыПродаж.Дата > ПланыПродаж1.Дата
	|			И ПланыПродаж.Подразделение = ПланыПродаж1.Подразделение
	|			И ПланыПродаж.Партнер = ПланыПродаж1.Партнер
	|			И ПланыПродаж.Соглашение = ПланыПродаж1.Соглашение
	|			И ПланыПродаж.Склад = ПланыПродаж1.Склад
	|			И ПланыПродаж.Менеджер = ПланыПродаж1.Менеджер
	|			И ПланыПродаж.ФорматМагазина = ПланыПродаж1.ФорматМагазина
	|			И ПланыПродаж.Назначение = ПланыПродаж1.Назначение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗамещающиеИЗамещенныеПланыПродаж.ЗамещенныйПлан КАК Ссылка
	|ИЗ
	|	ЗамещающиеИЗамещенныеПланыПродаж КАК ЗамещающиеИЗамещенныеПланыПродаж
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗамещениеПланов КАК ЗамещениеПланов
	|		ПО ЗамещающиеИЗамещенныеПланыПродаж.ЗамещающийПлан = ЗамещениеПланов.ЗамещающийПлан
	|			И ЗамещающиеИЗамещенныеПланыПродаж.ЗамещенныйПлан = ЗамещениеПланов.ЗамещенныйПлан
	|ГДЕ
	|	ЗамещениеПланов.ЗамещенныйПлан ЕСТЬ NULL";
	Если ВыбратьДляРегистрацииДвиженийОборотыДвижений Тогда
		ТекстЗапроса = ТекстЗапроса +
		"
		|	И ЗамещающиеИЗамещенныеПланыПродаж.ЗамещающийПлан.ОтражаетсяВБюджетировании";
	КонецЕсли;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.ПланПродаж";
	МетаданныеДокумента = Метаданные.Документы.ПланПродаж;
