...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если Объект.Замещающий и Объект.Товары.Количество() = 0 и Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаТовары Тогда
...
Функция ПолучитьПараметрыВидаПлана(Знач ВидПлана, Знач ОбновлятьПравило, АдресПравилаЗаполнения, АдресПользовательскихНастроек)
	...
	Реквизиты = "КоличествоПериодов, ЗаполнятьПодразделение, ЗаполнятьПартнера, 
		...
		|ЗаполнятьСкладВТЧ, ЗапретитьРедактированиеПравила, ЗаполнятьПоФормуле, Замещающий,
	...
	Иначе
		...
		Для каждого СуммаНаДату Из СуммыПоДатам Цикл
			...
			Для каждого ЭтапОплаты Из ПараметрыЗаполнения.ЭтапыГрафикаОплаты Цикл
				...
				НоваяСтрока.ДатаОтгрузки = СуммаНаДату.ДатаОтгрузки;
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План продаж +15%'"), "[ПланыПродаж] * 1.15");
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Факт продаж с учетом наличия товаров на остатках'"), 
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьПартнера, ЗаполнятьСоглашение, ЗаполнятьСклад,
		|ЗаполнятьМенеджера, ЗаполнятьФорматМагазина, ПланированиеПоНазначениям");
