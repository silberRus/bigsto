...
Функция ТекстЗапросаТаблицаТоварыКОтбору(Запрос, ТекстыЗапроса, Регистры)
	...
	|	0 КАК КОтбору,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТоварыОтбор.Упаковка.ТипУпаковки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыУпаковокНоменклатуры.ТоварноеМесто)
	|			ТОГДА ТоварыОтбор.КоличествоУпаковок
	|		ИНАЧЕ ТоварыОтбор.Количество
	|	КОНЕЦ КАК Отбирается,
	|	0 КАК Отобрано,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТоварыОтбор.Упаковка.ТипУпаковки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыУпаковокНоменклатуры.ТоварноеМесто)
	|			ТОГДА ТоварыОтбор.КоличествоУпаковокОтобрано
	|		ИНАЧЕ ТоварыОтбор.КоличествоОтобрано
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТоварыОтбор.Назначение.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
	|			ТОГДА ТоварыОтбор.Назначение
