...
Функция ПодсказкаПоТипуНоменклатуры(ТипНоменклатуры) Экспорт
	...
	ИначеЕсли ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор Тогда
		Возврат НСтр("ru = 'Комплекты, которые не хранятся на складе, а собираются динамически в процессе отгрузки. Используются для удобного выбора связанных позиций в документах реализации клиентам, ценообразовании и печати.'");
	...
	ИначеЕсли ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
		Возврат НСтр("ru = 'Нематериальные ценности, которые закупаются или производятся, реализуются клиентам и учитываются в подразделении-получателе. Ведется количественный учет и учет себестомости.'");
...
Функция ВключеныФООсобенностейУчетаНоменклатуры()Экспорт
	...
	Возврат ПолучитьФункциональнуюОпцию("ВестиСведенияДляДекларацийПоАлкогольнойПродукции")
		Или ПолучитьФункциональнуюОпцию("ИспользоватьУчетДрагоценныхМатериалов")
