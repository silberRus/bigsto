...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.РасширенныйРежим.Видимость = НастройкиОтчета.РазрешеноИзменятьВарианты;
...
Процедура РасширенныйРежимПриИзменении(Элемент)
	...
	КонецЕсли;
	ЗаполнитьБыстрыеНастройкиКлиент(ПараметрыЗаполнения);
КонецПроцедуры
...
Процедура ПодсказкаЕстьВложенныеОтчетыОбработкаНавигационнойСсылки(Элемент, Адрес, СтандартнаяОбработка)
	...
	ИзменитьУзелВарианта(СтрокаТаблицы, Неопределено, Истина);
КонецПроцедуры
...
Процедура Подключаемый_ПолеВвода_ПриИзменении(Элемент)
	...
	Если ТипЗнч(ПользовательскаяНастройкаКД) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
		...
		Если НастройкиОтчета.ПолноеИмя = "Отчет.УниверсальныйОтчет" Тогда
			ОбновитьПараметрыУниверсальногоОтчета(ПользовательскаяНастройкаКД);
		КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_СписокСПодбором_ПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	// Установка связей и параметров выбора.
	ПараметрыОтбора = ОтчетыКлиент.ПараметрыВыбораОтбора(ЭтотОбъект, Элемент);
	КолонкаЗначение = Элементы.Найти(СтрЗаменить(Элемент.Имя, "_СписокЗначений_", "_Колонка_Значение_"));
	КолонкаЗначение.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыОтбора.ПараметрыВыбора);
	КолонкаЗначение.ДоступныеТипы   = Новый ОписаниеТипов(ПараметрыОтбора.ОписаниеТипов);
	...
	Если НоваяСтрока Тогда
		ЭлементСпискаВФорме.Пометка = Истина;
	КонецЕсли;
	...
	ЭлементСпискаДоНачалаИзмененияНаКлиенте = ЭлементСпискаДоНачалаИзменения;
КонецПроцедуры
...
Процедура Подключаемый_СписокСПодбором_ПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаНачалаРедактирования, ОтменаЗавершенияРедактирования)
	...
	ЭлементСпискаДоНачалаИзменения = ЭлементСпискаДоНачалаИзмененияНаКлиенте;
...
Процедура Подключаемый_СписокСПодбором_ВставитьИзБуфера_Завершение(НайденныеОбъекты, ПараметрыВыполнения) Экспорт
	...
	Если ТипЗнч(ПользовательскаяНастройкаКД) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
		Если ПользовательскаяНастройкаКД.ПравоеЗначение = Неопределено Тогда
			ПользовательскаяНастройкаКД.ПравоеЗначение = Новый СписокЗначений;
		КонецЕсли;
...
Процедура СортировкаПослеВыбораПоля(ДоступноеПолеКД, ПараметрыВыполнения) Экспорт
	...
	СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Элемент");
...
Процедура Сортировка_ПереместитьВверх(Команда)
	...
	Контекст = НовыйКонтекст("Сортировка", "Переместить");
...
Процедура СортировкаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ИдентификаторЦели, Поле)
	...
	СтандартнаяОбработка = Ложь;
	ИсточникПеретаскивания = ИсточникПеретаскиванияНаКлиенте;
...
Процедура ВыбранныеПоля_ПереместитьВверх(Команда)
	...
	Контекст = НовыйКонтекст("ВыбранныеПоля", "Переместить");
	Контекст.Вставить("Направление", -1);
	ОпределитьВыделенныеСтроки(Контекст);
	Если ЗначениеЗаполнено(Контекст.ПричинаОтказа) Тогда
		ПоказатьПредупреждение(, Контекст.ПричинаОтказа);
...
Процедура ВыбранныеПоля_Сгруппировать(Команда)
	...
	Контекст = НовыйКонтекст("ВыбранныеПоля", "Сгруппировать");
	Контекст.Вставить("ТипГруппыКД", Тип("ГруппаВыбранныхПолейКомпоновкиДанных"));
	ОпределитьВыделенныеСтроки(Контекст);
	Если ЗначениеЗаполнено(Контекст.ПричинаОтказа) Тогда
		ПоказатьПредупреждение(, Контекст.ПричинаОтказа);
		Возврат;
	...
	Обработчик = Новый ОписаниеОповещения("ВыбранныеПоля_СгруппироватьЗавершение", ЭтотОбъект, Контекст);
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ГруппаВыбранныхПолей", ПараметрыФормы, 
		ЭтотОбъект, Истина, , , Обработчик, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыбранныеПоля_СгруппироватьЗавершение(ВыборПользователя, Контекст) Экспорт
	
	Если ВыборПользователя = Неопределено Или ВыборПользователя = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	КонецЕсли;
	
	Контекст.Вставить("ЗаголовокГруппы", ВыборПользователя.ЗаголовокГруппы);
	Контекст.Вставить("Расположение", ВыборПользователя.Расположение);
	СгруппироватьСтроки(Контекст);
...
Процедура Отборы_Сгруппировать(Команда)
	...
	Контекст = НовыйКонтекст("Отборы", "Сгруппировать");
	Контекст.Вставить("ТипГруппыКД", Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ОпределитьВыделенныеСтроки(Контекст);
	Если ЗначениеЗаполнено(Контекст.ПричинаОтказа) Тогда
		ПоказатьПредупреждение(, Контекст.ПричинаОтказа);
		Возврат;
...
Процедура Отборы_ПереместитьВверх(Команда)
	...
	Контекст = НовыйКонтекст("Отборы", "Переместить");
	Контекст.Вставить("Направление", -1);
	ОпределитьВыделенныеСтроки(Контекст);
	Если ЗначениеЗаполнено(Контекст.ПричинаОтказа) Тогда
		ПоказатьПредупреждение(, Контекст.ПричинаОтказа);
...
Процедура Отборы_ПоказыватьВШапкеОтчета(Команда)
	ОтборыУстановитьУровеньДоступа("ПоказыватьВШапкеОтчета");
КонецПроцедуры

&НаКлиенте
Процедура Отборы_ПоказыватьВНастройкахОтчета(Команда)
	ОтборыУстановитьУровеньДоступа("ПоказыватьВНастройкахОтчета");
КонецПроцедуры

&НаКлиенте
Процедура Отборы_ПоказыватьТолькоФлажокВШапкеОтчета(Команда)
	ОтборыУстановитьУровеньДоступа("ПоказыватьТолькоФлажокВШапкеОтчета");
КонецПроцедуры

&НаКлиенте
Процедура Отборы_ПоказыватьТолькоФлажокВНастройкахОтчета(Команда)
	ОтборыУстановитьУровеньДоступа("ПоказыватьТолькоФлажокВНастройкахОтчета");
КонецПроцедуры

&НаКлиенте
Процедура Отборы_НеПоказывать(Команда)
	ОтборыУстановитьУровеньДоступа("НеПоказывать");
КонецПроцедуры
...
Процедура СтруктураВариантаВыбор(Элемент, СтрокаИдентификатор, Поле, СтандартнаяОбработка)
	...
	Если ИмяКолонки = "СтруктураВариантаПредставление"
		...
		КонецЕсли;
		ИзменитьУзелВарианта(СтрокаТаблицы, ИмяСтраницы, Неопределено);
	Иначе
		СтандартнаяОбработка = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура СтруктураВариантаДобавитьГруппировкуПослеВыбораПоля(ДоступноеПолеКД, ПараметрыВыполнения) Экспорт
	...
	СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Группировка");
...
Процедура ДобавитьТаблицуИлиДиаграмму(ТипЭлемента)
	...
	СтрокаТаблицы.ФлажокДоступен = Истина;
	СтрокаТаблицы.Использование  = ЭлементКД.Использование;
	СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки(СтрокаТаблицы.Тип);
	...
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		...
		СтрокаТаблицы.Представление = НСтр("ru = 'Диаграмма'");
		...
		ВложеннаяСтрока.Представление = НСтр("ru = 'Точки'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru = 'Серии'");
		
	ИначеЕсли СтрокаТаблицы.Тип = "Таблица" Тогда
		
		СтрокаТаблицы.Представление = НСтр("ru = 'Таблица'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru = 'Строки'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru = 'Колонки'");
...
Процедура СоставГруппировкиПослеВыбораПоля(ДоступноеПолеКД, ПараметрыВыполнения) Экспорт
	...
	Если ДоступноеПолеКД.Ресурс Тогда
		СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Ресурс");
	ИначеЕсли ДоступноеПолеКД.Таблица Тогда
		СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Таблица");
	ИначеЕсли ДоступноеПолеКД.Папка Тогда
		СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Группа");
	Иначе
		СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Элемент");
	КонецЕсли;
...
Процедура СоставГруппировки_ПереместитьВверх(Команда)
	...
	Контекст = НовыйКонтекст("СоставГруппировки", "Переместить");
	Контекст.Вставить("Направление", -1);
	ОпределитьВыделенныеСтроки(Контекст);
	Если ЗначениеЗаполнено(Контекст.ПричинаОтказа) Тогда
		ПоказатьПредупреждение(, Контекст.ПричинаОтказа);
...
Процедура ОформлениеИспользованиеПриИзменении(Элемент)
	ТаблицыПолейИзменитьИспользование("Оформление");
КонецПроцедуры

&НаКлиенте
Процедура Оформление_ПереместитьВверх(Команда)

	Контекст = НовыйКонтекст("Оформление", "Переместить");
	Контекст.Вставить("Направление", -1);
	ОпределитьВыделенныеСтроки(Контекст);
	Если ЗначениеЗаполнено(Контекст.ПричинаОтказа) Тогда
		ПоказатьПредупреждение(, Контекст.ПричинаОтказа);
...
Процедура Оформление_УстановитьФлажки(Команда)
	УстановитьФлажкиИспользованиеВоВсемДереве("Оформление", Истина);
КонецПроцедуры

&НаКлиенте
Процедура Оформление_СнятьФлажки(Команда)
	УстановитьФлажкиИспользованиеВоВсемДереве("Оформление", Ложь);
КонецПроцедуры
...
Процедура ОчиститьНастройкиОтНесуществующихПолей(Команда)
	РасширенныйРежим = 1;
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ИмяСобытия", "ОчиститьНастройкиОтНесуществующихПолей");
	ПараметрыЗаполнения.Вставить("ВариантМодифицирован", Истина);
	ПараметрыЗаполнения.Вставить("ПользовательскиеНастройкиМодифицированы", Истина);
	ПараметрыЗаполнения.Вставить("ОчиститьНастройкиОтНесуществующихПолей", Истина);
	ПараметрыЗаполнения.Вставить("СброситьПользовательскиеНастройки", Истина);
	ЗаполнитьБыстрыеНастройкиКлиент(ПараметрыЗаполнения);
КонецПроцедуры
...
&НаКлиенте
Процедура ОтборыПриИзмененииТекущейСтроки() Экспорт
	
	СтрокаТаблицы = Элементы.Отборы.ТекущиеДанные;
	Если СтрокаТаблицы = Неопределено Тогда
		ОтборыПриИзмененииТекущейСтрокиНаСервере(Ложь, Ложь);
		Возврат;
	КонецЕсли;
	
	ЭтоОтбор = Не СтрокаТаблицы.ЭтоПараметр И Не СтрокаТаблицы.ЭтоРаздел;
	ЭтоПараметрИлиОтбор = Не СтрокаТаблицы.ЭтоРаздел;
	
	ОтборыПриИзмененииТекущейСтрокиНаСервере(ЭтоОтбор, ЭтоПараметрИлиОтбор);
...
&НаСервере
Процедура ОтборыПриИзмененииТекущейСтрокиНаСервере(ЭтоОтбор, ЭтоПараметрИлиОтбор)
	...
	Элементы.КомандыОтборов_Показывать.Доступность  = ЭтоПараметрИлиОтбор;
	Элементы.КомандыОтборов_Показывать1.Доступность = ЭтоПараметрИлиОтбор;
...
&НаКлиенте
Процедура ЗаполнитьБыстрыеНастройкиКлиент(ПараметрыЗаполнения = Неопределено)
	Если ПараметрыЗаполнения = Неопределено Тогда
		ПараметрыЗаполнения = Новый Структура("ИмяСобытия", "ПослеИзмененияКоллекции");
	КонецЕсли;
	ПараметрыЗаполненияНаКлиенте = ПараметрыЗаполнения;
	ПодключитьОбработчикОжидания("ЗаполнитьБыстрыеНастройкиОтложенно", 0.1, Истина);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьБыстрыеНастройкиОтложенно()
	БыстрыеНастройкиЗаполнить(ПараметрыЗаполненияНаКлиенте);
	ПоказатьРезультатЗаполненияНастроек();
КонецПроцедуры
...
Процедура ПоказатьРезультатЗаполненияНастроек()
	...
	// Разворачивание узлов деревьев.
	РазвернутьУзлыДеревьев = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатЗаполненияНастроек, "РазвернутьУзлыДеревьев");
	Если ТипЗнч(РазвернутьУзлыДеревьев) = Тип("Массив") Тогда
		Для Каждого ИмяЭлементаФормы Из РазвернутьУзлыДеревьев Цикл
			СтандартныеПодсистемыКлиент.РазвернутьУзлыДерева(ЭтотОбъект, ИмяЭлементаФормы, "*", Истина);
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьПараметрыУниверсальногоОтчета(ПользовательскаяНастройкаКД)
	
	НеобходимоОбновитьНастройки = Ложь;
	ОчищаемыеПараметры = Новый Массив;
	
	ПараметрТипОбъектаМетаданных = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ТипОбъектаМетаданных");
	Если ПараметрТипОбъектаМетаданных <> Неопределено Тогда
		Если ПользовательскаяНастройкаКД.Параметр = ПараметрТипОбъектаМетаданных.Параметр
				И ПараметрТипОбъектаМетаданных.Значение <> ПользовательскаяНастройкаКД.Значение Тогда
			ПараметрТипОбъектаМетаданных.Значение = ПользовательскаяНастройкаКД.Значение;
			НеобходимоОбновитьНастройки = Истина;
			ОчищаемыеПараметры.Добавить("ПолноеИмяОбъектаМетаданных");
			ОчищаемыеПараметры.Добавить("ИмяТаблицы");
		КонецЕсли;
	КонецЕсли;
	ПараметрПолноеИмяОбъектаМетаданных = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ПолноеИмяОбъектаМетаданных");
	Если ПараметрПолноеИмяОбъектаМетаданных <> Неопределено Тогда
		Если ПользовательскаяНастройкаКД.Параметр = ПараметрПолноеИмяОбъектаМетаданных.Параметр
				И ПараметрПолноеИмяОбъектаМетаданных.Значение <> ПользовательскаяНастройкаКД.Значение Тогда
			ПараметрПолноеИмяОбъектаМетаданных.Значение = ПользовательскаяНастройкаКД.Значение;
			НеобходимоОбновитьНастройки = Истина;
			ОчищаемыеПараметры.Добавить("ИмяТаблицы");
		КонецЕсли;
	КонецЕсли;
	ПараметрИмяТаблицы = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ИмяТаблицы");
	Если ПараметрИмяТаблицы <> Неопределено Тогда
		Если ПользовательскаяНастройкаКД.Параметр = ПараметрИмяТаблицы.Параметр
				И ПараметрИмяТаблицы.Значение <> ПользовательскаяНастройкаКД.Значение Тогда
			НеобходимоОбновитьНастройки = Истина;
			ПараметрИмяТаблицы.Значение = ПользовательскаяНастройкаКД.Значение;
		КонецЕсли;
	КонецЕсли;
	
	Для Каждого ОчищаемыйПараметр Из ОчищаемыеПараметры Цикл
		Параметр = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти(ОчищаемыйПараметр);
...
Процедура ОбновитьФорму(Контекст)
	ПараметрыЗаполнения = Новый Структура("ИмяСобытия", Контекст.ТаблицаИмя + "." + Контекст.Действие);
	ЗаполнитьБыстрыеНастройкиКлиент(ПараметрыЗаполнения);
КонецПроцедуры
...
Процедура ОпределитьВыделенныеСтроки(Контекст)
	...
	Особенности.МогутБытьРазделы = (Контекст.ТаблицаИмя = "Отборы" Или Контекст.ТаблицаИмя = "ВыбранныеПоля" Или Контекст.ТаблицаИмя = "Сортировка");
	...
	Для Каждого СтрокаИдентификатор Из ВыделенныеСтроки Цикл
		...
		Если Не ДобавитьВыделеннуюСтроку(Контекст, СтрокаДерева, Особенности) Тогда
			Возврат;
		КонецЕсли;
...
&НаКлиенте
Функция ДобавитьВыделеннуюСтроку(Строки, СтрокаДерева, Особенности)
...
&НаКлиенте
Процедура СгруппироватьСтроки(Контекст)
	ТекущийРодитель = Контекст.ТекущийРодитель;
	ТаблицаЭлемент = Элементы[Контекст.ТаблицаИмя];
	УзелКД = ТаблицыПолейНайтиУзел(ЭтотОбъект, Контекст.ТаблицаИмя, Неопределено);
	
	Если Контекст.ТекущийРодитель = Неопределено Тогда
		ТаблицаРеквизит = ЭтотОбъект[Контекст.ТаблицаИмя];
		Если Контекст.ТаблицаИмя = "Отборы" И Не РежимИзмененияУзлаВарианта Тогда
	...
	Группы = ТаблицыПолейВставить(Контекст.ТаблицаИмя, Контекст.ТипГруппыКД, 0, Ложь);
	...
	Иначе
		ЭлементКД.Заголовок    = Контекст.ЗаголовокГруппы;
		ЭлементКД.Расположение = Контекст.Расположение;
		ГруппаДерева.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Группа");
	...
	Для Каждого СтараяСтрокаДерева Из Контекст.СтрокиДерева Цикл
...
&НаКлиенте
Процедура УдалитьСтроки(Контекст)
	
	ТаблицыПолейУдалить(Контекст.ТаблицаИмя, Контекст.СтрокиДерева, Ложь);
	ПользовательскиеНастройкиМодифицированы = Истина;
	Если РасширенныйРежим = 1 Тогда
		ВариантИзменен = Истина;
	КонецЕсли;
...
Процедура ОтборыПоказатьВыборСсылки(СтрокаТаблицы, СтандартнаяОбработка)
	ПараметрыВыбора = ПараметрыВыбораОтбора(СтрокаТаблицы);
	...
	ПараметрыВыбора.Вставить("Отмеченные",         ОтчетыКлиентСервер.ЗначенияСписком(СтрокаТаблицы.Значение));
	ПараметрыВыбора.Вставить("Представление",      СтрокаТаблицы.Представление);
	
	Контекст = Новый Структура;
	Контекст.Вставить("СтрокаИдентификатор", СтрокаТаблицы.ПолучитьИдентификатор());
	Контекст.Вставить("ПараметрыВыбора",     ПараметрыВыбора);
	Контекст.Вставить("ТекущееЗначение",     СтрокаТаблицы.Значение);
	Контекст.Вставить("ПредставлениеПоля",   СтрокаТаблицы.Представление);
...
&НаКлиенте
Процедура ОтборыПоказатьВыборСсылкиПослеВыбораТипа(ЭлементСписка, Контекст) Экспорт
	...
	Если ПараметрыВыбораТипа = Неопределено Тогда
		ОтчетыКлиент.ВыбратьЗначениеПримитивногоТипа(
			ЭтотОбъект,
			Тип,
			ПараметрыВыбора.ОписаниеТипов,
			Контекст.ТекущееЗначение,
			Контекст.ПредставлениеПоля,
			Обработчик);
	ИначеЕсли ПараметрыВыбораТипа.БыстрыйВыбор Тогда
		ПоказатьВыборИзМеню(Обработчик, ПараметрыВыбораТипа.ЗначенияДляВыбора);
	Иначе
		ОткрытьФорму(
...
&НаКлиенте
Процедура ОтборыЗавершениеВыбораСсылки(СсылкаИлиЭлементСписка, Контекст) Экспорт
	Если ТипЗнч(СсылкаИлиЭлементСписка) = Тип("ЭлементСпискаЗначений") Тогда
		НовоеЗначение = СсылкаИлиЭлементСписка.Значение;
	Иначе
		НовоеЗначение = СсылкаИлиЭлементСписка;
	КонецЕсли;
	Если Не Контекст.ПараметрыВыбора.ОписаниеТипов.СодержитТип(ТипЗнч(НовоеЗначение)) Или Не ЗначениеЗаполнено(НовоеЗначение) Тогда
		Возврат;
	КонецЕсли;
	...
	УстановитьЗначение(СтрокаТаблицы, УзелКД, ЭлементКД, НовоеЗначение);
...
Процедура УстановитьЗначение(СтрокаТаблицы, УзелКД, ЭлементКД, НовоеЗначение)
	Если УзелКД <> Неопределено Тогда // Поле доступно по функциональным опциям
		Если СтрокаТаблицы.ЭтоПараметр Тогда
			Если ТипЗнч(УзелКД) = Тип("ДоступныйПараметрКомпоновкиДанных") Тогда
				ДоступныйЭлементСКД = УзелКД;
			Иначе
				ДоступныйЭлементСКД = УзелКД.ДоступныеПараметры.НайтиПараметр(ЭлементКД.Параметр);
			КонецЕсли;
			ФорматРедактирования = ДоступныйЭлементСКД.ФорматРедактирования;
			ЭлементКД.Значение = НовоеЗначение;
		Иначе
			Если ТипЗнч(УзелКД) = Тип("ДоступноеПолеОтбораКомпоновкиДанных") Тогда
				ДоступныйЭлементСКД = УзелКД;
			Иначе
			ДоступныйЭлементСКД = УзелКД.ДоступныеПоляОтбора.НайтиПоле(ЭлементКД.ЛевоеЗначение);
			КонецЕсли;
			ФорматРедактирования = ДоступныйЭлементСКД.ФорматРедактирования;
			ЭлементКД.ПравоеЗначение = НовоеЗначение;
		КонецЕсли;
	КонецЕсли;
	Если Не ПустаяСтрока(ФорматРедактирования) Тогда
		СтрокаТаблицы.Значение = Формат(НовоеЗначение, ФорматРедактирования);
	Иначе	
		СтрокаТаблицы.Значение = НовоеЗначение;
	КонецЕсли;
КонецПроцедуры
...
Функция ПараметрыВыбораОтбора(СтрокаТаблицы)
	...
	ВыборГруппИЭлементов = ОтчетыКлиентСервер.ПривестиЗначениеКТипуИспользованиеГруппИЭлементов(
		СтрокаТаблицы.Условие,
		СтрокаТаблицы.ВыборГруппИЭлементов);
	...
	Для Каждого Связь Из Связи Цикл
		...
		Если Связь.ТипСвязи = "ПоТипу" Тогда
			Если СтрокаВедущий.Условие <> ВидСравненияКомпоновкиДанных.Равно
				И СтрокаВедущий.Условие <> ВидСравненияКомпоновкиДанных.ВИерархии Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ОтборыВыбратьВидСравнения(СтрокаТаблицы)
	...
	ТолькоБулево = ИнформацияОТипах.СодержитТипБулево И ИнформацияОТипах.КоличествоПримитивныхТипов = 1;
	...
	Если ИнформацияОТипах.ОграниченнойДлины Или ФиксированныйСписок Тогда
		...
		Если ИнформацияОТипах.СодержитОбъектныеТипы Или ФиксированныйСписок Тогда
	...
	Если ИнформацияОТипах.ОграниченнойДлины И ИнформацияОТипах.СодержитОбъектныеТипы Тогда
		
		Список.Добавить(ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии); // В списке включая подчиненные
		Список.Добавить(ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии); // Не в списке включая подчиненные
		
		Список.Добавить(ВидСравненияКомпоновкиДанных.ВИерархии); // В группе
		Список.Добавить(ВидСравненияКомпоновкиДанных.НеВИерархии); // Не в группе
		
	КонецЕсли;
	
	Если ИнформацияОТипах.ОграниченнойДлины И ИнформацияОТипах.КоличествоПримитивныхТипов > 0 И Не ТолькоБулево Тогда
	...
	Если ИнформацияОТипах.СодержитТипДата Или ИнформацияОТипах.СодержитТипСтрока 
		Или ФиксированныйСписок Или ИнформацияОТипах.СодержитОбъектныеТипы Тогда
...
Процедура ОтборыЗавершениеВыбораВидаСравнения(Результат, Контекст) Экспорт
	...
	Если СтрокаТаблицы.Условие = ВидСравненияКомпоновкиДанных.ВИерархии
		Или СтрокаТаблицы.Условие = ВидСравненияКомпоновкиДанных.НеВИерархии Тогда
		СтрокаТаблицы.ВыборГруппИЭлементов = ГруппыИЭлементы.Группы;
	Иначе
		...
		КонецЕсли;
		СтрокаТаблицы.ВыборГруппИЭлементов = ГруппыИЭлементы.ГруппыИЭлементы;
	КонецЕсли;
	...
	Если БылВводСписком <> СталВводСписком Тогда
		Если БылВводСписком Тогда
			...
			КонецЕсли;
		Иначе
			Если ТипЗнч(ЭлементКД.ПравоеЗначение) = Тип("ПолеКомпоновкиДанных") Тогда
				ЭлементКД.ПравоеЗначение = Неопределено;
			Иначе
...
&НаКлиенте
Процедура ОтборыУстановитьУровеньДоступа(УровеньДоступа)
	СтрокаТаблицы = Элементы.Отборы.ТекущиеДанные;
	СтрокаИдентификатор = Элементы.Отборы.ТекущаяСтрока;
	Если СтрокаТаблицы = Неопределено Или СтрокаИдентификатор = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТаблицыПолейИзменитьУровеньДоступа("Отборы", СтрокаИдентификатор, Истина, Не СтрокаТаблицы.ЭтоПараметр, УровеньДоступа);
КонецПроцедуры
...
&НаКлиенте
Процедура ИзменитьУзелВарианта(СтрокаТаблицы, ИмяСтраницы, ИспользоватьФормуВарианта)
	Если СтрокаТаблицы = Неопределено Тогда
		ЭлементыСтруктуры = СтруктураВарианта.ПолучитьЭлементы();
		Если ЭлементыСтруктуры.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		СтрокаТаблицы = ЭлементыСтруктуры[0];
	КонецЕсли;
	Если ИспользоватьФормуВарианта = Неопределено Тогда
		ИспользоватьФормуВарианта = (СтрокаТаблицы.Тип = "Таблица"
			Или СтрокаТаблицы.Тип = "НастройкиВложенногоОбъекта");
	КонецЕсли;
	
	Обработчик = Новый ОписаниеОповещения("ИзменитьУзелВариантаЗавершение", ЭтотОбъект);
	...
	ОткрытьФорму(ИмяОткрываемойФормы, ПараметрыФормы, ЭтотОбъект, , , , Обработчик, Режим);
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьУзелВариантаЗавершение(Результат, Контекст) Экспорт
	Если Результат = Неопределено Или Результат = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	КонецЕсли;
	ЗаполнитьБыстрыеНастройкиКлиент(Результат);
КонецПроцедуры
...
&НаКлиенте
Процедура ТаблицыПолейИзменитьУровеньДоступа(ТаблицаИмя, СтрокаИдентификатор, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков, УровеньДоступа = Неопределено)
	...
	Если УровеньДоступа = Неопределено Тогда
		ПоказатьВыборИзМеню(Обработчик, Список);
	Иначе
		ЭлементСписка = Список.НайтиПоЗначению(УровеньДоступа);
		ВыполнитьОбработкуОповещения(Обработчик, ЭлементСписка);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ТаблицыПолейИзменитьУровеньДоступаЗавершение(ЭлементСписка, Контекст) Экспорт
	Если ТипЗнч(ЭлементСписка) <> Тип("ЭлементСпискаЗначений") Тогда
		Возврат;
	КонецЕсли;
	УровеньДоступа = ЭлементСписка.Значение;
	Если УровеньДоступа = "ПоказыватьВШапкеОтчета" Тогда
		ИндексКартинкиДоступа = 2;
	ИначеЕсли УровеньДоступа = "ПоказыватьТолькоФлажокВШапкеОтчета" Тогда
		ИндексКартинкиДоступа = 1;
	ИначеЕсли УровеньДоступа = "ПоказыватьВНастройкахОтчета" Тогда
		ИндексКартинкиДоступа = 4;
	ИначеЕсли УровеньДоступа = "ПоказыватьТолькоФлажокВНастройкахОтчета" Тогда
		ИндексКартинкиДоступа = 3;
	Иначе
		ИндексКартинкиДоступа = 5;
	КонецЕсли;
...
Процедура ТаблицыПолейПередУдалением(ТаблицаИмя, Отказ)
	...
	Контекст = НовыйКонтекст(ТаблицаИмя, "Удалить");
	ОпределитьВыделенныеСтроки(Контекст);
	Если ЗначениеЗаполнено(Контекст.ПричинаОтказа) Тогда
		ПоказатьПредупреждение(, Контекст.ПричинаОтказа);
		Возврат;
...
Процедура УстановитьФлажкиИспользованиеВоВсемДереве(ТаблицаИмя, Использование, МассивСтрок = Неопределено, УзелКД = Неопределено)
	...
	Для Каждого СтрокаДерева Из МассивСтрок Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтрокаДерева, "ВыводитьФлажок") <> Ложь
			...
			ЭлементКД = УзелКД.ПолучитьОбъектПоИдентификатору(СтрокаДерева.ИдентификаторКД);
			Если ЭлементКД <> Неопределено И ТипЗнч(ЭлементКД) <> Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных")
				И ТипЗнч(ЭлементКД) <> Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
...
Процедура ТаблицыПолейИзменитьЗначение(ТаблицаИмя)
	...
	УстановитьЗначение(СтрокаТаблицы, УзелКД, ЭлементКД, СтрокаТаблицы.Значение);
...
&НаКлиенте
Процедура ТаблицыПолейУдалить(Знач ТаблицаИмя, Знач СтрокаТаблицыИлиМассивСтрок, Знач ИспользованиеАвтоПолей)
	Таблица = ЭтотОбъект[ТаблицаИмя];
	УзелКД = ТаблицыПолейНайтиУзел(ЭтотОбъект, ТаблицаИмя);
	Если ТипЗнч(СтрокаТаблицыИлиМассивСтрок) = Тип("ДанныеФормыЭлементДерева") Тогда
		ТаблицыПолейУдалитьМеханика(ТаблицаИмя, Таблица, УзелКД, СтрокаТаблицыИлиМассивСтрок, ИспользованиеАвтоПолей);
	Иначе
		Количество = СтрокаТаблицыИлиМассивСтрок.Количество();
		Для Номер = 1 По Количество Цикл
			ОбратныйИндекс = Количество - Номер;
			СтрокаТаблицы = СтрокаТаблицыИлиМассивСтрок[ОбратныйИндекс];
			ТаблицыПолейУдалитьМеханика(ТаблицаИмя, Таблица, УзелКД, СтрокаТаблицы, ИспользованиеАвтоПолей);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ТаблицыПолейУдалитьМеханика(ТаблицаИмя, Таблица, УзелКД, СтрокаТаблицы, ИспользованиеАвтоПолей)
	ЭлементКД = НайтиОбъектКД(УзелКД, СтрокаТаблицы);
	...
	Если ТаблицаИмя = "ВыбранныеПоля" Тогда
		...
		ИначеЕсли СтрокаТаблицы.ТипНастройки = "АвтоВыбранноеПоле" Тогда
			Если ИспользованиеАвтоПолей <> Неопределено И ЭлементКД.Использование <> ИспользованиеАвтоПолей Тогда
				СтрокаТаблицы.Использование = ИспользованиеАвтоПолей;
				ЭлементКД.Использование     = ИспользованиеАвтоПолей;
			КонецЕсли;
			Возврат;
		КонецЕсли;
		СвязаннаяТаблицаИмя = "СоставГруппировки";
...
Функция БыстрыеНастройкиЗаполнить(Знач ПараметрыКлиента)
	...
	ОбновитьВидимостьДоступность(ПараметрыЗаполнения.ИмяСобытия);
	...
	Информация.Очистить();
	ПараметрыЗаполнения.Очистить();
КонецФункции

&НаСервере
Процедура ОбновитьВидимостьДоступность(ИмяСобытия = "")
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Отборы.ФиксированныйСписокВыбора");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СортировкаНаправление.Имя);
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Сортировка.ТипНастройки");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = "АвтоЭлементПорядка";
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СтруктураВарианта.Подсвечивать");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
...
Функция БыстрыеНастройкиДозаполнитьПараметры(ПараметрыКлиента)
	...
	Если Не ПараметрыЗаполнения.Свойство("Результат") Тогда
		...
		ПараметрыЗаполнения.Результат.Вставить("РазвернутьУзлыДеревьев", Новый Массив);
	КонецЕсли;
...
Процедура БыстрыеНастройкиЗапомнитьСостояние(ПараметрыЗаполнения)
	...
	ИменаТаблиц = "Отборы, ВыбранныеПоля, Сортировка, " + ?(РежимИзмененияУзлаВарианта, "СоставГруппировки", "СтруктураВарианта");
	...
	КонецЦикла;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПараметрыЗаполнения.Результат.РазвернутьУзлыДеревьев, ИменаТаблиц);
КонецПроцедуры
...
Процедура БыстрыеНастройкиЗагрузитьНастройкиВКомпоновщик(ПараметрыЗаполнения)
	...
	ОчиститьНастройкиОтНесуществующихПолей = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаполнения, "ОчиститьНастройкиОтНесуществующихПолей", Ложь);
	...
	Если ОбновитьНастройкиВарианта Или ОчиститьНастройкиОтНесуществующихПолей Тогда
		НовыеНастройкиКД = Отчет.КомпоновщикНастроек.ПолучитьНастройки();
		Если ОчиститьНастройкиОтНесуществующихПолей Тогда
			ОчиститьНастройкиОтНесуществующихПолейНаСервере(НовыеНастройкиКД);
		КонецЕсли;
	...
	Если СброситьПользовательскиеНастройки Или ОчиститьНастройкиОтНесуществующихПолей Тогда
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Для Каждого СвойстваНастройки Из ДобавленныеСпискиЗначений Цикл
		...
		ТаблицаФормы.УстановитьДействие("ПриНачалеРедактирования", "Подключаемый_СписокСПодбором_ПриНачалеРедактирования");
...
Процедура РасширенныеНастройкиЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	ВидимостьВыбранныхПолей = ЗарегистрироватьЭлементыВыбранныхПолей(УзелКД);
	Если ВидимостьВыбранныхПолей Тогда
		ПараметрыЗаполнения.Результат.РазвернутьУзлыДеревьев.Добавить("ВыбранныеПоля");
	КонецЕсли;
	...
	Если ВидимостьСортировок Тогда
		ПараметрыЗаполнения.Результат.РазвернутьУзлыДеревьев.Добавить("Сортировка");
	КонецЕсли;
	...
	Если ВидимостьТаблицыОтборов Тогда
		...
		ЗарегистрироватьЭлементыПараметров(Информация);
		ЗарегистрироватьЭлементыОтборов(Информация);
		ПараметрыЗаполнения.Результат.РазвернутьУзлыДеревьев.Добавить("Отборы");
	КонецЕсли;
	...
	Если ВидимостьУсловногоОформления Тогда
		...
		ПараметрыЗаполнения.Результат.РазвернутьУзлыДеревьев.Добавить("Оформление");
	КонецЕсли;
	...
	Если Не РежимИзмененияУзлаВарианта И КорневаяСтрока.Тип <> "НастройкиВложенногоОбъекта" И НастройкиОтчета.РазрешеноИзменятьСтруктуру Тогда
		...
		ПараметрыЗаполнения.Результат.РазвернутьУзлыДеревьев.Добавить("СтруктураВарианта");
	...
	Элементы.ГруппаЕстьНесуществующиеПоля.Видимость = Информация.ЕстьНесуществующиеПоля
		И (РасширенныйРежим = 1)
		И (ИдентификаторТекущегоУзлаКД = Неопределено);
...
Процедура ПеретаскиваниеВПределахТаблицы(ИсточникПеретаскивания, ИдентификаторЦели)
	...
	Для Каждого ПеремещаемаяСтрока Из ПеремещаемыеСтроки Цикл
		...
		// Бантики.
		НоваяСтрокаИдентификатор = Результат.СтрокаТаблицы.ПолучитьИдентификатор();
		Если ЭтоТекущая Тогда
			ТаблицаЭлемент.ТекущаяСтрока = НоваяСтрокаИдентификатор;
		КонецЕсли;
...
Процедура ПеретаскиваниеВыбранныхПолейВСортировки(ИсточникПеретаскивания, ИдентификаторЦели)
	...
	Пока Истина Цикл
		...
		Для Каждого ИсточникСтрокаТаблицы Из ТекущаяКоллекция Цикл
			...
			НоваяСтрока.ИндексКартинки  = ОтчетыКлиентСервер.ИндексКартинки("Элемент");
...
Функция ЗарегистрироватьНачалоПеретаскивания(ТаблицаИмя, ИсточникПеретаскивания = Неопределено)
	...
	Контекст = НовыйКонтекст(ТаблицаИмя, "НачалоПеретаскивания");
	ОпределитьВыделенныеСтроки(Контекст);
	Если ЗначениеЗаполнено(Контекст.ПричинаОтказа) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(ИсточникПеретаскивания, Контекст);
	ИсточникПеретаскиванияНаКлиенте = ИсточникПеретаскивания;
	Возврат Истина;
...
&НаКлиенте
Процедура Обновить()
	ЗаполнитьБыстрыеНастройкиКлиент();
КонецПроцедуры
...
Функция ЗарегистрироватьЭлементСтруктурыОтчета(СтрокаДерева, СтрокиРодителя)
	Если СтрокаДерева.Тип = "КоллекцияЭлементовСтруктурыДиаграммы"
		Или СтрокаДерева.Тип = "КоллекцияЭлементовСтруктурыТаблицы"
		Или СтрокаДерева.Тип = "Настройки" Тогда
		Использование = Истина;
		ФлажокДоступен = Ложь;
	Иначе
		Если СтрокаДерева.ПользовательскаяНастройка = Неопределено Тогда
			Использование = СтрокаДерева.УзелКД.Использование;
			ФлажокДоступен = (РасширенныйРежим = 1);
		Иначе
			Использование = СтрокаДерева.ПользовательскаяНастройка.ПользовательскаяНастройкаКД.Использование;
			ФлажокДоступен = Истина;
		КонецЕсли;
	КонецЕсли;
	Если Не Использование И Не ФлажокДоступен Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	СтрокаФормы.ФлажокДоступен = ФлажокДоступен;
	СтрокаФормы.Использование = Использование;
	Если СтрокаФормы.ФлажокДоступен И РасширенныйРежим = 0 Тогда
	...
	// Типы: "Группировка", "ГруппировкаТаблицы", "ГруппировкаДиаграммы",
	//       "Таблица", "Диаграмма", "НастройкиВложенногоОбъекта".
	СтрокаФормы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки(СтрокаДерева.Тип, СтрокаДерева.Состояние);
...
Процедура ЗарегистрироватьЭлементыСоставаГруппировки(УзелКД = Неопределено, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено)
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоПолеГруппировки" Тогда
			...
			СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Элемент", "Предопределенный");
		ИначеЕсли ТипНастройки = "ПолеГруппировки" Тогда
			СтрокаТаблицы.ТипГруппировки = ЭлементКД.ТипГруппировки;
			СтрокаТаблицы.ТипДополнения  = ЭлементКД.ТипДополнения;
			ДоступноеПолеКД = ДоступныеПоля.НайтиПоле(ЭлементКД.Поле);
			Если ДоступноеПолеКД = Неопределено Тогда
				СтрокаТаблицы.Представление = ОтчетыКлиентСервер.ИмяВПредставление(Строка(ЭлементКД.Поле));
				СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Элемент", "ПометкаУдаления");
			Иначе
				СтрокаТаблицы.Представление = ДоступноеПолеКД.Заголовок;
				Если ДоступноеПолеКД.Ресурс Тогда
					СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Ресурс");
...
&НаСервере
Функция ЗарегистрироватьЭлементыВыбранныхПолей(УзелКД, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено, ДоступныеПоля = Неопределено)
	Если УзелКД = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Если НаборСтрокКД = Неопределено Тогда
	...
	Если НаборСтрок = Неопределено Тогда
		...
		СтрокаРаздел.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("ВыбранныеПоля");
	...
	ИндексКартинкиГруппы = ОтчетыКлиентСервер.ИндексКартинки("Группа");
	ИндексКартинкиЭлемента = ОтчетыКлиентСервер.ИндексКартинки("Элемент");
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		ТипНастройки = ОтчетыКлиентСервер.ТипНастройкиСтрокой(ТипЗнч(ЭлементКД));
		ЭтоГруппа = Ложь;
		ПолеКД = Неопределено;
		...
		Иначе
			Если ТипНастройки = "ГруппаВыбранныхПолей" Тогда
				ЭтоГруппа = Истина;
				ИндексКартинки = ИндексКартинкиГруппы;
			Иначе
				ИндексКартинки = ИндексКартинкиЭлемента;
			КонецЕсли;
...
Функция ЗарегистрироватьЭлементыСортировки()
	...
	СтрокаРаздел.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Сортировки");
...
&НаСервере
Процедура ЗарегистрироватьЭлементыПараметров(Информация)
	Если РежимИзмененияУзлаВарианта Тогда
		Возврат;
	КонецЕсли;
	...
	СтрокаРаздел.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("ПараметрыДанных");
	...
	Для Каждого НастройкаВарианта Из НаборСтрокВарианта Цикл
		...
		Если ЗначениеЗаполнено(ДоступнаяНастройкаКД.Заголовок) Тогда
		...
		СтрокаТаблицы.ИндексКартинки  = -1;
		СтрокаТаблицы.ТипЗначения     = ДоступнаяНастройкаКД.ТипЗначения;
		...
		УстановитьЗначение(СтрокаТаблицы, ДоступнаяНастройкаКД, ЭлементКД, НастройкаВарианта.Значение);
		...
		Дополнительно = Новый Структура(
			"ОписаниеТипов, БыстрыйВыбор, ОграничиватьВыборУказаннымиЗначениями,
			|ПараметрыВыбора, ФормаВыбора, ЗначенияДляВыбора, ЗначенияДляВыбораЗаполнены");
		ЗаполнитьЗначенияСвойств(Дополнительно, НастройкаВарианта);
		Если Дополнительно.ЗначенияДляВыбораЗаполнены = Неопределено Тогда
			Дополнительно.ЗначенияДляВыбораЗаполнены = Ложь;
		КонецЕсли;
...
Процедура ЗарегистрироватьЭлементыОтборов(Информация, НаборСтрокВарианта = Неопределено, НаборСтрокФормы = Неопределено)
	...
	Если НаборСтрокФормы = Неопределено Тогда
		...
		СтрокаРаздел.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Отборы");
	...
	Для Каждого НастройкаВарианта Из НаборСтрокВарианта Цикл
		...
		Если НастройкаВарианта.Состояние = "ПометкаУдаления" Тогда
			СтрокаТаблицы.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Ошибка");
		КонецЕсли;
		
		Если ЭтоГруппа Тогда
			ЗарегистрироватьЭлементыОтборов(Информация, НастройкаВарианта.Строки, СтрокаТаблицы.ПолучитьЭлементы());
		Иначе
			УстановитьЗначение(СтрокаТаблицы, НастройкаВарианта.ДоступнаяНастройкаКД, ЭлементКД, НастройкаВарианта.Значение);
			Если НастройкаВарианта.ОграничиватьВыборУказаннымиЗначениями
				И ТипЗнч(НастройкаВарианта.ЗначенияДляВыбора) = Тип("СписокЗначений") Тогда
				Если СтрокаТаблицы.ВводСписком Тогда
					Если ТипЗнч(СтрокаТаблицы.Значение) = Тип("СписокЗначений") Тогда
...
// Очистка настроек от несуществующих полей.

&НаСервереБезКонтекста
Процедура ОчиститьНастройкиОтНесуществующихПолейНаСервере(НастройкиКД)
	// Метод ".Восстановить(СпособВосстановленияНастроекКомпоновкиДанных.Полное)" не подходит, поскольку:
	// 1. Цель "удалить", а не "отключить".
	// 2. После его использования могут остаться группировки <Детальные записи> с отключенными недоступными полями.
	
	УсловияВывода = Новый Структура;
	УсловияВывода.Вставить("ТолькоПользовательские",      Ложь);
	УсловияВывода.Вставить("ТолькоБыстрые",               Ложь);
	УсловияВывода.Вставить("ИдентификаторТекущегоУзлаКД", Неопределено);
	
	Информация = ОтчетыСервер.РасширеннаяИнформацияОНастройках(
		Новый Структура("Настройки, ПользовательскиеНастройки", НастройкиКД, Новый ПользовательскиеНастройкиКомпоновкиДанных),
		ОтчетыКлиентСервер.ПолучитьНастройкиОтчетаПоУмолчанию(),
		Неопределено,
		УсловияВывода);
	
	Найденные = Информация.ДеревоВарианта.Строки.НайтиСтроки(Новый Структура("Состояние", "ПометкаУдаления"), Истина);
	Для Каждого СтрокаДерева Из Найденные Цикл
		НастройкаВариантаКД = СтрокаДерева.УзелКД;
		Если СтрокаДерева.Тип = "Группировка"
			Или СтрокаДерева.Тип = "ГруппировкаТаблицы"
			Или СтрокаДерева.Тип = "ГруппировкаДиаграммы" Тогда
			ПоляГруппировкиКД = НастройкаВариантаКД.ПоляГруппировки;
			ДоступныеПоляКД = ПоляГруппировкиКД.ДоступныеПоляПолейГруппировок;
			Количество = ПоляГруппировкиКД.Элементы.Количество();
			ОбратныйИндекс = Количество;
			Пока ОбратныйИндекс > 0 Цикл
				ОбратныйИндекс = ОбратныйИндекс - 1;
				ПолеГруппировкиКД = ПоляГруппировкиКД.Элементы[ОбратныйИндекс];
				Если ТипЗнч(ПолеГруппировкиКД) = Тип("ПолеГруппировкиКомпоновкиДанных")
					...
					Количество = Количество - 1;
					ПоляГруппировкиКД.Элементы.Удалить(ПолеГруппировкиКД);
				КонецЕсли;
			КонецЦикла;
			Если Количество > 0 Тогда
				Продолжить; // В группировке остались не помеченные поля.
			КонецЕсли;
		КонецЕсли;
		
		// Переместить подчиненные элементы на уровень с удаляемым элементом.
		КорневаяСтрока = СтрокаДерева;
		Пока КорневаяСтрока.Тип <> "Настройки" Цикл
			КорневаяСтрока = КорневаяСтрока.Родитель;
		КонецЦикла;
		Для Каждого ПодчиненнаяСтрокаДерева Из СтрокаДерева.Строки Цикл
			ПеремещениеСвязанныхСтрок(Информация.ДеревоВарианта, КорневаяСтрока.УзелКД, ПодчиненнаяСтрокаДерева, СтрокаДерева.Родитель, СтрокаДерева);
		КонецЦикла;
		
		// Удалить элемент КД.
		СтрокиРодителяКД = ПолучитьЭлементыКД(НастройкиКД, СтрокаДерева.Родитель.УзелКД);
		СтрокиРодителяКД.Удалить(СтрокаДерева.УзелКД);
	КонецЦикла;
	
	Найденные = Информация.НастройкиВарианта.Строки.НайтиСтроки(Новый Структура("Состояние", "ПометкаУдаления"), Истина);
	Для Каждого СтрокаДерева Из Найденные Цикл
		Если СтрокаДерева.Родитель = Неопределено Тогда
			Продолжить; // Корневые коллекции нельзя удалить (типа "Настройки.Отбор").
		КонецЕсли;
		УзелКД = СтрокаДерева.Владелец.ЭлементКД;
		РодительКД = СтрокаДерева.Родитель.ЭлементКД;
		СтрокиРодителяКД = ПолучитьЭлементыКД(УзелКД, РодительКД);
		СтрокиРодителяКД.Удалить(СтрокаДерева.ЭлементКД);
	КонецЦикла;
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ПеремещениеСвязанныхСтрок(Коллекция, УзелКД, Знач ЭлементДерева, Знач НовыйРодитель,
	Знач ПередЧемВставить = Неопределено, Знач Индекс = Неопределено, Знач ИндексКД = Неопределено)
	Результат = Новый Структура("ЭлементДерева, ЭлементКД, Индекс, ИндексКД");
	
	ДобавлятьВКонец = (НовыйРодитель = Неопределено);
	КудаВставить = ?(НовыйРодитель = Неопределено, Коллекция.Строки, НовыйРодитель.Строки);
	Если ПередЧемВставить = Неопределено Тогда
		ЭтоРаздел = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НовыйРодитель, "ЭтоРаздел");
		ЭтоГруппа = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НовыйРодитель, "ЭтоГруппа");
		Если ЭтоРаздел <> Истина И ЭтоГруппа <> Истина Тогда // На один уровень с родителем.
			ПередЧемВставить = НовыйРодитель;
			НовыйРодитель = ?(НовыйРодитель.Родитель = Неопределено, Коллекция, НовыйРодитель.Родитель);
			КудаВставить = ?(НовыйРодитель = Неопределено, Коллекция.Строки, НовыйРодитель.Строки);
		КонецЕсли;
	КонецЕсли;
	
	ЭлементКД          = НайтиОбъектКД(УзелКД, ЭлементДерева);
	НовыйРодительКД    = НайтиОбъектКД(УзелКД, НовыйРодитель);
	КудаВставитьКД     = ПолучитьЭлементыКД(УзелКД, НовыйРодительКД);
	ПередЧемВставитьКД = НайтиОбъектКД(УзелКД, ПередЧемВставить);
	
	СтарыйРодитель    = ?(ЭлементДерева.Родитель = Неопределено, Коллекция, ЭлементДерева.Родитель);
	ОткудаПереместить = ?(СтарыйРодитель = Неопределено, Коллекция.Строки, СтарыйРодитель.Строки);
	...
	Если ЭлементКД = ПередЧемВставитьКД Тогда
		Результат.ЭлементКД     = ЭлементКД;
		Результат.ЭлементДерева = ЭлементДерева;
	Иначе
		Если Индекс = Неопределено Или ИндексКД = Неопределено Тогда
			Если ПередЧемВставитьКД = Неопределено Тогда
				Если ДобавлятьВКонец Тогда
			...
			Иначе
				...
				ИндексКД = КудаВставитьКД.Индекс(ПередЧемВставитьКД);
				Если СтарыйРодитель = НовыйРодитель Тогда
					СтарыйИндекс = ОткудаПереместить.Индекс(ЭлементДерева);
					Если СтарыйИндекс <= Индекс Тогда
						Индекс   = Индекс + 1;
						ИндексКД = ИндексКД + 1;
					КонецЕсли;
		...
		ПоискСтрокТаблицы = Новый Соответствие;
		Результат.ЭлементДерева = ОтчетыКлиентСервер.СкопироватьРекурсивно(Неопределено, ЭлементДерева, КудаВставить, Индекс, ПоискСтрокТаблицы);
		...
		ОткудаПереместить.Удалить(ЭлементДерева);
		ОткудаПереместитьКД.Удалить(ЭлементКД);
	КонецЕсли;
	
	Результат.Индекс   = КудаВставить.Индекс(Результат.ЭлементДерева);
	Результат.ИндексКД = КудаВставитьКД.Индекс(Результат.ЭлементКД);
