...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры) Тогда
		...
		ОткрытьФорму("Справочник.ПрофилиНастроекЭДО.Форма.ПомощникПодключенияЭДО",
			ПараметрыФормы, ЭтотОбъект, , , , ОписаниеОповещенияОЗакрытии);
	...
	Если ПодключитьсяКЭДО() Тогда
		...
		Иначе
			...
			Если ТипПодключенияЭДО = 1 Тогда
				...
				ОткрытьФорму("Справочник.ПрофилиНастроекЭДО.Форма.ПомощникПодключенияЭДО", ПараметрыФормы, ЭтотОбъект);
			Иначе
				...
				ОткрытьФорму(ФормаПомощникаБизнесСети, ПараметрыФормы, ЭтотОбъект);
			КонецЕсли;
...
Процедура ПодготовитьФорму(Отказ)
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОрганизация", "ПрофилиНастроекЭДО.Организация = &Организация");
	...
	Если Не Результат[0].Пустой() Тогда
		...
		КонецЕсли;
	ИначеЕсли Не Результат[1].Пустой() Тогда // нет профиля обмена через Такском
