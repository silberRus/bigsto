...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Замер.Идентификатор = Замер.МодульОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, Замер.Имя);
...
Функция НайтиВариантПоИмениЭлемента(НадписьИмя)
	Идентификатор = БыстрыйПоискВариантовПоИмениЭлемента[НадписьИмя];
...
Функция НайтиПодсистемуПоСсылке(Форма, Ссылка)
	Идентификатор = Форма.БыстрыйПоискПодсистемПоСсылке[Ссылка];
	Если Идентификатор <> Неопределено Тогда
...
Процедура ЗаполнитьИнформациюОПодсистемахИУстановитьЗаголовок()
	...
	ПоискПодсистем = Новый Соответствие;
	Для Каждого СтрокаРаздел Из ВсеРазделы Цикл
		...
		ПоискПодсистем.Вставить(СтрокаТаблицы.Ссылка, СтрокаТаблицы.ПолучитьИдентификатор());
	...
	БыстрыйПоискПодсистемПоСсылке = Новый ФиксированноеСоответствие(ПоискПодсистем);
КонецПроцедуры
...
Процедура ЗаполнитьПанельОтчетов()
	...
	БыстрыйПоискВариантовПоИмениЭлемента = Новый ФиксированноеСоответствие(ПараметрыЗаполнения.ПоискВариантов);
КонецПроцедуры
...
Процедура НайтиВариантыОтчетовДляВывода(ПараметрыЗаполнения)
	...
	|	КОНЕЦ КАК Описание,
	|	ВариантыРазмещение.Ссылка.ТолькоДляАвтора КАК ТолькоДляАвтора,
	|	ВариантыРазмещение.Ссылка.Наименование КАК Наименование,
	|	ВариантыРазмещение.Ссылка.Отчет КАК Отчет,
	|	ВариантыРазмещение.Ссылка.ТипОтчета КАК ТипОтчета,
	|	ВариантыРазмещение.Ссылка.КлючВарианта КАК КлючВарианта,
	|	ВариантыРазмещение.Ссылка.Автор КАК Автор,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК Родитель,
	|	ВариантыРазмещение.Ссылка.ВидимостьПоУмолчанию КАК ВидимостьПоУмолчанию,
	...
	|	И (НЕ ВариантыРазмещение.Ссылка.Пользовательский
	|			ИЛИ НЕ ВариантыРазмещение.Ссылка.ИнтерактивнаяПометкаУдаления)
	|	И (ВариантыРазмещение.Ссылка.Пользовательский
	|			ИЛИ НЕ ВариантыРазмещение.Ссылка.ПометкаУдаления)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	ОбъектыОтчетов = Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторам(ИдентификаторыОтчетов);
	Для Каждого ОтчетДляВывода Из ТаблицаРезультат Цикл
		Если ОтчетДляВывода.ТипОтчета = Перечисления.ТипыОтчетов.Внутренний
			Или ОтчетДляВывода.ТипОтчета = Перечисления.ТипыОтчетов.Расширение Тогда
				...
				Если ОтчетДляВывода.ИмяОтчета <> МетаданныеОтчета.Имя Тогда
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	ПараметрыЗаполнения.ПоискВариантов.Вставить(НадписьИмя, СтрокаТаблицы.ПолучитьИдентификатор());
