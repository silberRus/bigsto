...
Процедура ВыполнитьОбмен()
	...
	ИнтеграцияЕГАИСКлиент.ВыполнитьОбмен(МассивОрганизацийЕГАИС, ОповещениеПриЗавершении);
...
Функция НайтиОбработанныеСообщения(Сообщения)
	...
	|		ПО (ВходящиеСообщения.ТипСообщения = ЗНАЧЕНИЕ(Перечисление.ТипыЗапросовЕГАИС.Входящий))
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщенияXML = ИнтеграцияЕГАИС.ТекстСообщенияXMLИзПротокола(Выборка.ВходящееСообщение);
...
Функция ТекстПакетаЗапросов()
	...
	Для Каждого СтрокаТаблицы Из ОбработанныеЗапросы Цикл
		...
		Иначе
			Строки.Добавить(БиблиотекаКартинок.ОтказЕГАИС);
		КонецЕсли;
...
Функция ТекстОбработанногоЗапроса(ДанныеЗапроса, Объект, ТекстОшибки = "", ИндексТаблицыЗапросов = "")
	...
	Если Объект <> Неопределено Тогда
		...
		ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросАлкогольнойПродукции") Тогда
			...
			Строки.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(НСтр("ru = 'Загружено позиций: %1'"), Объект.Количество())));
			Строки.Добавить(" ");
