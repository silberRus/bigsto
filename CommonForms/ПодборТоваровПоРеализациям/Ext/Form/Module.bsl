...
Процедура ЗаполнитьТаблицуТоваров()
	...
	Иначе
		...
		|СГРУППИРОВАТЬ ПО
		|	ЧекККМТовары.Ссылка,
		|	ЧекККМТовары.НоменклатураЕГАИС,
		...
		|	ЧекККМВозвратТовары.НоменклатураЕГАИС КАК НоменклатураЕГАИС,
		...
		|	ЧекККМВозвратТовары.НоменклатураЕГАИС,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ТаблицаПроверяемыеТовары.НоменклатураЕГАИС,
		|	ТаблицаПроверяемыеТовары.НоменклатураНабора,
		|	ТаблицаПроверяемыеТовары.ХарактеристикаНабора,
		|	ТаблицаПроверяемыеТовары.Номенклатура,
		|	ТаблицаПроверяемыеТовары.Характеристика,
		...
		|	КОНЕЦ КАК ИндексНабора,
		|	ВариантыКомплектацииНоменклатуры.ВариантРасчетаЦеныНабора КАК ВариантРасчетаЦеныНабора,
		|	ЧекиККМ.НоменклатураЕГАИС КАК НоменклатураЕГАИС,
		...
		|			И ЧекиККМ.НоменклатураЕГАИС = ЧекиККМВозврат.НоменклатураЕГАИС
		...
		|			И ЧекиККМ.НоменклатураЕГАИС = ДанныеВозврата.НоменклатураЕГАИС
...
Процедура СформироватьИнформационнуюНадписьОтборы()
	...
	Иначе
		...
		Если НЕ ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
			...
			ТоварыИнформационнаяНадписьОтборы = СтрЗаменить(ТоварыИнформационнаяНадписьОтборы,"%Партнер%", Партнер);
		Иначе
			ТоварыИнформационнаяНадписьОтборы = НСтр("ru='Отбор по: %Соглашение%, %Контрагент%, %НалогообложениеНДС%'");
		КонецЕсли;
