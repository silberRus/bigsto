...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТаблицаПлатежей.ТекстЗапроса = ТекстЗапросаТаблицаПлатежей();
	
	Если Параметры.Свойство("ТипНалога") Тогда
		ТипНалогаОтбор = Параметры.ТипНалога;
		Элементы.ТипНалогаОтбор.Видимость = Истина;
		ТаблицаПлатежей.Параметры.УстановитьЗначениеПараметра("НеОтбиратьПоТипуНалога", Ложь);
	Иначе
		ТаблицаПлатежей.Параметры.УстановитьЗначениеПараметра("НеОтбиратьПоТипуНалога", Истина);
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаПлатежей()
	...
	|		СписаниеБДС.ТипНалога) КАК РеквизитыПлатежей
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК СписаниеБДСРасшифровка
	|	ПО
	|		РеквизитыПлатежей.Ссылка = СписаниеБДСРасшифровка.Ссылка
	|";
