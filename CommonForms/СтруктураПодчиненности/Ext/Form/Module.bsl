...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ОбъектСсылка) Тогда
		//СТОК Бойков начало
		Попытка 
			Если ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
				Если ЗначениеЗаполнено(ОбъектСсылка.ЗаказКлиента) Тогда
					ОбъектСсылка = ОбъектСсылка.ЗаказКлиента;
				КонецЕсли;
			ИначеЕсли ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств") 
				ИЛИ ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") 
				
				Тогда
				Если ОбъектСсылка.РасшифровкаПлатежа.Количество() = 1 Тогда
					Если ЗначениеЗаполнено(ОбъектСсылка.РасшифровкаПлатежа[0].Заказ) Тогда
						ОбъектСсылка = ОбъектСсылка.РасшифровкаПлатежа[0].Заказ;
					КонецЕсли
				КонецЕсли
			КонецЕсли;
...
Функция ИмяРеквизитаДокумента(Знач МетаданныеОбъекта, Знач ИмяРеквизита) 
	...
	ИмяРеквизитаДокумента = СтруктураПодчиненностиПереопределяемый.ИмяРеквизитаДокумента(МетаданныеОбъекта.Имя, ИмяРеквизита); 
