...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьАкцизныеМарки            = Параметры.ИспользоватьАкцизныеМарки;
	...
	Элементы.ТоварыАкцизныеМарки.Видимость       = ИспользоватьАкцизныеМарки;
	...
	Если ИспользоватьАкцизныеМарки Тогда
		ИнтеграцияЕГАИСУТ.АкцизныеМаркиЗаполнитьСлужебныеРеквизиты(ЭтотОбъект, "КоличествоФакт");
	КонецЕсли;
...
Процедура НоменклатураПриИзменении(Элемент)
	...
	Если ИспользоватьАкцизныеМарки Тогда
		СтруктураДействий.Вставить("ЗаполнитьПризнакМаркируемаяАлкогольнаяПродукция");
		...
		ПараметрыЗаполненияНоменклатурыЕГАИС.Вставить("ЗаполнитьФлагМаркируемаяАлкогольнаяПродукция", Истина);
...
Процедура ЗавершитьПроверку(Команда)
	...
	Иначе
		...
		Если АкцизныеМарки.Количество() > 0 Тогда
			РезультатЗакрытия = РезультатСканированияАкцизныхМарок();
		КонецЕсли;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	Если ИспользоватьАкцизныеМарки Тогда
		...
		СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьИндексАкцизнойМарки", Новый Структура("ИмяКолонкиКоличество", "КоличествоФакт"));
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		...
		Если ИспользоватьАкцизныеМарки Тогда
			СтруктураДействийСИзмененнымиСтроками.Вставить("ЗаполнитьИндексАкцизнойМарки", Новый Структура("ИмяКолонкиКоличество", "КоличествоФакт"));
		КонецЕсли;
	КонецЕсли;
	...
	СтруктураДействий.МаркируемаяАлкогольнаяПродукцияВТЧ     = ИспользоватьАкцизныеМарки;
...
Функция ОбработатьШтрихкодDataMatrix(Форма, ДанныеШтрихкода, ПараметрыСканированияАкцизныхМарок) Экспорт
	...
	ПараметрыОтбора.Вставить("Номенклатура",      ДанныеШтрихкода.Номенклатура);
	ПараметрыОтбора.Вставить("Характеристика",    ДанныеШтрихкода.Характеристика);
	ПараметрыОтбора.Вставить("НоменклатураЕГАИС", ДанныеШтрихкода.АлкогольнаяПродукция);
	ПараметрыОтбора.Вставить("Серия",             ДанныеШтрихкода.Серия);
	
	МассивСтрок = Форма.Товары.НайтиСтроки(ПараметрыОтбора);
	...
	Иначе
		...
		СтрокаТЧ.НоменклатураЕГАИС = ДанныеШтрихкода.АлкогольнаяПродукция;
		СтрокаТЧ.Номенклатура      = ДанныеШтрихкода.Номенклатура;
		СтрокаТЧ.Характеристика    = ДанныеШтрихкода.Характеристика;
		СтрокаТЧ.Серия             = ДанныеШтрихкода.Серия;
		
		СтрокаТЧ.КоличествоУпаковокФакт = 1;
...
Функция ОбработатьШтрихкодУпаковки(Форма, ДанныеШтрихкода, ВложенныеШтрихкоды, ПараметрыСканированияАкцизныхМарок) Экспорт
	...
	Если Форма.Элементы.Товары.ТекущаяСтрока <> Неопределено Тогда
		ПараметрыЗаполнения.ТекущаяСтрока = Форма.Товары.НайтиПоИдентификатору(Форма.Элементы.Товары.ТекущаяСтрока);
	КонецЕсли;
	
	АкцизныеМаркиЕГАИС.ЗаполнитьТоварыИАкцизныеМарки(Форма, ВложенныеШтрихкоды.ДеревоУпаковок, ПараметрыЗаполнения);
...
Функция ПослеОбработкиШтрихкодов(РезультатОбработкиШтрихкода, КэшированныеЗначения)
	...
	Если РезультатОбработкиШтрихкода.ТребуетсяОбработкаШтрихкода Тогда
		...
		ДанныеДляОбработки.МаркируемаяАлкогольнаяПродукцияВТЧ     = ИспользоватьАкцизныеМарки;
...
Функция АдресДанныхПроверки()
	...
	ДанныеХранилища.ДеревоУпаковок = АкцизныеМаркиЕГАИС.ШтрихкодыУпаковок(ЭтотОбъект, ОрганизацияЕГАИС, "НоменклатураЕГАИС").ДеревоУпаковок;
	
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЧекККМ") Тогда
		
		ДанныеХранилища.ТаблицаАлкогольнойПродукцииКОпределениюСправок2 = Документы.ЧекККМ.ТаблицаАлкогольнойПродукцииКОпределениюСправок2(Ссылка);
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЧекККМВозврат") Тогда
		
		ДанныеХранилища.ТаблицаАлкогольнойПродукцииКОпределениюСправок2 = Документы.ЧекЕГАИСВозврат.ТаблицаАлкогольнойПродукцииКОпределениюСправок2(Ссылка);
...
Процедура ОткрытьФормуСканированияАлкогольнойПродукции()
	...
	ПараметрыОткрытия.Вставить("ОрганизацияЕГАИС",                                       ОрганизацияЕГАИС);
...
Процедура СканированиеОтгружаемойПродукцииПослеЗакрытияНаСервере(АдресВременногоХранилища)
	...
	АкцизныеМаркиЕГАИС.ЗаполнитьТоварыИАкцизныеМарки(
		ЭтотОбъект, ДеревоУпаковок, ПараметрыЗаполнения);
...
&НаСервере
Функция РезультатСканированияАкцизныхМарок()
	...
	ПараметрыЗакрытия.Вставить("АкцизныеМарки", ПоместитьВоВременноеХранилище(АкцизныеМарки.Выгрузить()));
