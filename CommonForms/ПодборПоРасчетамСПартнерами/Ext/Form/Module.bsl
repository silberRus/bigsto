...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию
		...
		Или ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровМеждуОрганизациями Тогда
...
Процедура УстановитьВидимость()
	...
	Если СуммаДокумента = 0 Тогда
		Элементы.ТаблицаОстатковРасчетовСумма.Видимость = Ложь;
		Элементы.ТаблицаОстатковРасчетовВалютаДокумента.Видимость = Ложь;
		Элементы.ДекорацияИтоги.Видимость = Ложь;
		Элементы.СуммаПлатежей.Видимость = Ложь;
...
Процедура ПоместитьПлатежиВХранилище()
	...
	Платежи = ТаблицаОстатковРасчетов.Выгрузить(, "Выбран, Сумма, Заказ, СтатьяДвиженияДенежныхСредств, ВалютаВзаиморасчетов, Партнер, Контрагент, Договор, ПлатежиПо275ФЗ, ТипРасчетов, Организация, ДолгПартнера, НашДолг");
	...
	ОплатаОтКлиента = (ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом) Или Не ЗначениеЗаполнено(ТипРасчетов);
	
	ТаблицаНДС = ДенежныеСредстваСервер.РасшифровкаПлатежаНДС(ДатаДокумента, Валюта, Платежи.ВыгрузитьКолонку("Заказ"), ОплатаОтКлиента);
