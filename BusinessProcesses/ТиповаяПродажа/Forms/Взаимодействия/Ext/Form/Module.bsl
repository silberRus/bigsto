...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ШаблонЗаголовка = НСтр("ru='Взаимодействия (%1)'");
	ПравоПросмотраВзаимодействия = ПравоДоступа("Просмотр", Метаданные.ЖурналыДокументов.Взаимодействия);
	Если ПравоПросмотраВзаимодействия Тогда
		ЗначениеПараметраШаблона = Строка(КоличествоВзаимодействий(Задание.Предмет));
	Иначе
		Элементы.ДекорацияВзаимодействия.Доступность = Ложь;
		ЗначениеПараметраШаблона = НСтр("ru = 'Нет прав для просмотра'")
	КонецЕсли;
	ТекстЗаголовка =  СтрШаблон(ШаблонЗаголовка, ЗначениеПараметраШаблона);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Источник) И Параметр.Предмет = Задание.Предмет  Тогда
		ТекстЗаголовка = НСтр("ru='Взаимодействия (%КоличествоВзаимодействий%)'");
		Элементы.ДекорацияВзаимодействия.Заголовок = СтрЗаменить(
