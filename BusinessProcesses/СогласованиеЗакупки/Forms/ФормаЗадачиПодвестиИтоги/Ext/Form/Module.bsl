...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если БизнесПроцессы.СогласованиеЗакупки.ИспользуетсяВерсионированиеПредмета(ТипПредмета) Тогда
			
			ЕстьОтличияВерсий = БизнесПроцессы.СогласованиеЗакупки.ПоследняяВерсияПредметаОтличаетсяОтСогласованных(Объект.БизнесПроцесс);
			
			Если ЕстьОтличияВерсий Тогда
				
				НомерПоследнейВерсии = БизнесПроцессы.СогласованиеЗакупки.НомерПоследнейВерсииПредмета(Объект.БизнесПроцесс);
	...
	Если Объект.Выполнена Тогда
		...
		РезультатСогласования = БизнесПроцессы.СогласованиеЗакупки.РезультатСогласованияПоТочкеМаршрута(Объект.БизнесПроцесс, Объект.ТочкаМаршрута);
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		БизнесПроцессыИЗадачиСервер.ФормаЗадачиПриСозданииНаСервере(ЭтаФорма, Объект,Элементы.ГруппаСостояние, Элементы.ДатаИсполнения);
	КонецЕсли;
...
&НаКлиенте
Процедура ОтклоненияОтУсловийЗакупок(Команда)
	ПараметрыФормы = Новый Структура("ДокументЗакупки", Объект.Предмет);
	ОткрытьФорму("Отчет.ОтклоненияОтУсловийЗакупок.Форма", ПараметрыФормы);
КонецПроцедуры
