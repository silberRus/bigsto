...
&НаКлиенте
Перем СтатусГотов, СтатусОтгружен, ТипЗаказ;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("СвойствоНомерССайта", АТ_Кэш.СвойствоНомерССайта());
	...
	ОтборыСписковКлиентСервер.ОтборПоЗначениюСпискаПриСозданииНаСервере(Список, "Склад", Склад, СтруктураБыстрогоОтбора);
...
Процедура ПриОткрытии(Отказ)
	...
	СтатусГотов 	= ПредопределенноеЗначение("Перечисление.СтатусыКП.ГотовКОтгрузке");
	СтатусОтгружен 	= ПредопределенноеЗначение("Перечисление.СтатусыКП.Выполнено");
	
	ТипЗаказ = Тип("ДокументСсылка.ЗаказКлиента");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "СформированоКПИзЗаказа" Тогда
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ОтборыСписковКлиентСервер.ОтборПоЗначениюСпискаПриЗагрузкеИзНастроек(Список, "Склад", Склад, СтруктураБыстрогоОтбора, Настройки);
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если НЕ (Копирование И ТипЗнч(Элемент.ТекущаяСтрока) = Тип("ДокументСсылка.КП")) Тогда
		
		Отказ = Истина;
		ОткрытьФорму("Документ.КП.ФормаОбъекта",, Элементы.Список);
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	ОбщееСостояние = текДанные <> Неопределено И Не текДанные.ПометкаУдаления;
	
	Элементы.СформироватьРеализацию.Видимость 	= ОбщееСостояние И текДанные.Статус = СтатусГотов;
	Элементы.ПечатьКомплекта.Видимость 			= ОбщееСостояние И текДанные.Статус = СтатусОтгружен;
	Элементы.КПИзЗаказа.Видимость 				= ОбщееСостояние И текДанные.Тип 	= ТипЗаказ И ЗначениеЗаполнено(текДанные.НомераНаСайте) И (ЭтоПолныеПрава Или текДанные.Менеджер = текПользователь);
...
Процедура ПечатьСчета(Команда)
	...
	Для Каждого ДокументСсылка Из Элементы.Список.ВыделенныеСтроки Цикл
		
		ВПФ = НайтиВнешнююПечатнуюФорму("Счет на оплату (без этапов оплаты, с рекламой)");
		...
		ПараметрыОткрытия.ПараметрыИсточника.ИдентификаторКоманды = "СчетНаОплатуСРекламой";
